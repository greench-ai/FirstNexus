...
ASC_KEYCHAIN_SERVICE=FirstNexus-asc-key
...
```bash
ASC_APP_IDENTIFIER=ai.FirstNexus.client
# or
...
Tip: run `scripts/ios-team-id.sh` from repo root to print a Team ID for `.env`. The helper prefers the canonical FirstNexus team (`Y5PE65HELJ`) when present locally; otherwise it prefers the first non-personal team from your Xcode account (then personal team if needed). Fastlane uses this helper automatically if `IOS_DEVELOPMENT_TEAM` is missing.

For local/manual iOS builds that stay on direct APNs, configure the gateway host separately with `NEXISCLAW_APNS_TEAM_ID`, `NEXISCLAW_APNS_KEY_ID`, and either `NEXISCLAW_APNS_PRIVATE_KEY_P8` or `NEXISCLAW_APNS_PRIVATE_KEY_PATH`. Those gateway runtime env vars are separate from Fastlane's `.env`.
...
- confirm `apps/ios/build/beta/FirstNexus-<version>.ipa` exists
...
- The beta flow regenerates `apps/ios/FirstNexus.xcodeproj` from `apps/ios/project.yml` before archiving
