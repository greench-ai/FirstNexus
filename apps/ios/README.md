# FirstNexus iOS (Super Alpha)

This iPhone app is super-alpha and internal-use only. It connects to an FirstNexus Gateway as a `role: node`.
...
- Beta release uses canonical `ai.FirstNexus.client*` bundle IDs through a temporary generated xcconfig in `apps/ios/build/BetaRelease.xcconfig`.
- Beta release also switches the app to `FirstNexusPushTransport=relay`, `FirstNexusPushDistribution=official`, and `FirstNexusPushAPNsEnvironment=production`.
...
- `NEXISCLAW_PUSH_RELAY_BASE_URL=https://relay.example.com`
...
```bash
export NEXISCLAW_PUSH_RELAY_BASE_URL=https://relay.example.com
```
...
7. Expected outputs after a successful run:
   - `apps/ios/build/beta/FirstNexus-<version>.ipa`
   - `apps/ios/build/beta/FirstNexus-<version>.app.dSYM.zip`
...
- `apps/ios/Sources/FirstNexus.entitlements` sets `aps-environment` to `development`.
- APNs token registration to gateway happens only after gateway connection (`push.apns.register`).
- Local/manual builds default to `FirstNexusPushTransport=direct` and `FirstNexusPushDistribution=local`.
- Your selected team/profile must support Push Notifications for the app bundle ID you are signing.
- If push capability or provisioning is wrong, APNs registration fails at runtime (check Xcode logs for `APNs registration failed`).
- The gateway host also needs direct APNs auth configured separately with `NEXISCLAW_APNS_TEAM_ID`, `NEXISCLAW_APNS_KEY_ID`, and either `NEXISCLAW_APNS_PRIVATE_KEY_P8` or `NEXISCLAW_APNS_PRIVATE_KEY_PATH`.
- Recommended gateway-host storage for the APNs `.p8` file is `~/.FirstNexus/credentials/apns/AuthKey_<KEYID>.p8` with restrictive permissions, then point `NEXISCLAW_APNS_PRIVATE_KEY_PATH` at that file.
...
- Debug builds default to `FirstNexusPushAPNsEnvironment=sandbox`; Release builds default to `production`.
...
- Gateway-side relay sending is configured through `gateway.push.apns.relay.baseUrl` in `FirstNexus.json`. `NEXISCLAW_APNS_RELAY_BASE_URL` remains a temporary env override only.
...
This exists to keep the hosted relay limited to genuine FirstNexus official builds and to ensure a
...
The iOS app is not a Codex Computer Use backend. Computer Use and `cua-driver mcp` are macOS desktop-control paths; iOS exposes device capabilities as FirstNexus node commands through the gateway. Agents can drive the iPhone canvas, camera, screen, location, voice, and other node capabilities with `node.invoke`, subject to iOS foreground/background limits.
