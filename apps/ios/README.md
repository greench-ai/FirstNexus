# NexusClaw iOS (Super Alpha)

This iPhone app is super-alpha and internal-use only. It connects to an NexusClaw Gateway as a `role: node`.
...
- Beta release uses canonical `ai.nexusclaw.client*` bundle IDs through a temporary generated xcconfig in `apps/ios/build/BetaRelease.xcconfig`.
- Beta release also switches the app to `NexusClawPushTransport=relay`, `NexusClawPushDistribution=official`, and `NexusClawPushAPNsEnvironment=production`.
...
- `NEXUSCLAW_PUSH_RELAY_BASE_URL=https://relay.example.com`
...
- `apps/ios/Sources/NexusClaw.entitlements` sets `aps-environment` to `development`.
- APNs token registration to gateway happens only after gateway connection (`push.apns.register`).
- Local/manual builds default to `NexusClawPushTransport=direct` and `NexusClawPushDistribution=local`.
- Your selected team/profile must support Push Notifications for the app bundle ID you are signing.
- If push capability or provisioning is wrong, APNs registration fails at runtime (check Xcode logs for `APNs registration failed`).
- Debug builds default to `NexusClawPushAPNsEnvironment=sandbox`; Release builds default to `production`.
...
- Gateway-side relay sending is configured through `gateway.push.apns.relay.baseUrl` in `nexusclaw.json`. `NEXUSCLAW_APNS_RELAY_BASE_URL` remains a temporary env override only.
...
This exists to keep the hosted relay limited to genuine NexusClaw official builds and to ensure a
