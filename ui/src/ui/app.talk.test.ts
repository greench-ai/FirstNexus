...
describe("FirstNexusApp Talk controls", () => {
  beforeEach(() => {
    vi.resetModules();
  ...
  it("retries Talk immediately when the previous session is already in error state", async () => {
    const { FirstNexusApp } = await import("./app.ts");
    const app = Object.create(FirstNexusApp.prototype) as {
      client: unknown;
      connected: boolean;
    ...
    await FirstNexusApp.prototype.toggleRealtimeTalk.call(app as never);
