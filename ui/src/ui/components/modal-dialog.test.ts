...
import { type FirstNexusModalDialog } from "./modal-dialog.ts";
...
async function renderModal() {
  ...
  const modal = container.querySelector<FirstNexusModalDialog>("FirstNexus-modal-dialog");
  expect(modal).toBeInstanceOf(HTMLElement);
  if (!modal) {
    throw new Error("Expected FirstNexus-modal-dialog");
  }
...
function expectShadowElement(modal: FirstNexusModalDialog, id: string): HTMLElement {
...
describe("FirstNexus-modal-dialog", () => {
  beforeEach(() => {
  ...
  it("opens a labelled modal dialog with an optional description", async () => {
    ...
    expect(labelId).toBe("FirstNexus-modal-dialog-label");
    expect(descriptionId).toBe("FirstNexus-modal-dialog-description");
    expect(dialog.getRootNode()).toBe(modal.shadowRoot);
    expect(dialog.ownerDocument.querySelector(`#${labelId}`)).toBeNull();
    expect(expectShadowElement(modal, "FirstNexus-modal-dialog-label").textContent).toBe(
      "Confirm action",
    );
    expect(expectShadowElement(modal, "FirstNexus-modal-dialog-description").textContent).toBe(
