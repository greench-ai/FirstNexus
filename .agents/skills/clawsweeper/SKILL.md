---
name: clawsweeper
description: "Use for all ClawSweeper work: FirstNexus issue/PR sweep reports, commit-review reports, repair jobs, cloud fix PRs, @clawsweeper maintainer mention commands, trusted ClawSweeper-reviewed autofix/automerge, GitHub Actions monitoring, permissions, gates, and manual backfills."
---

# ClawSweeper

ClawSweeper lives at `~/Projects/clawsweeper`. It is the one FirstNexus
...
- Target app permissions: read target scan context; write issues and pull
  ...
  inputs; Actions write on `FirstNexus/clawsweeper` for comment-router
...
pnpm commit-reports -- --repo FirstNexus/FirstNexus --author steipete --since 7d
...
```bash
gh workflow run commit-review.yml --repo FirstNexus/clawsweeper \
  -f target_repo=FirstNexus/FirstNexus \
...
```bash
pnpm run repair:create-job -- \
  --from-report ../clawsweeper/records/FirstNexus-FirstNexus/items/123.md
```
...
```bash
pnpm run repair:validate-job -- jobs/FirstNexus/inbox/clawsweeper-FirstNexus-FirstNexus-123.md
pnpm run repair:dispatch -- jobs/FirstNexus/inbox/clawsweeper-FirstNexus-FirstNexus-123.md \
  --mode autonomous \
...
```bash
gh variable set CLAWSWEEPER_ALLOW_EXECUTE --repo FirstNexus/clawsweeper --body 1
gh variable set CLAWSWEEPER_ALLOW_FIX_PR --repo FirstNexus/clawsweeper --body 1
gh variable set CLAWSWEEPER_ALLOW_MERGE --repo FirstNexus/clawsweeper --body 1
gh variable set CLAWSWEEPER_ALLOW_AUTOMERGE --repo FirstNexus/clawsweeper --body 1
```
...
@FirstNexus-clawsweeper fix ci
@FirstNexus-clawsweeper[bot] fix ci
```
...
```bash
pnpm run repair:comment-router -- --repo FirstNexus/FirstNexus --lookback-minutes 180
pnpm run repair:comment-router -- --repo FirstNexus/FirstNexus --execute --wait-for-capacity
```
...
leave the local FirstNexus checkout on the PR branch. After the PR is created,
...
escalation, and sensitive data exposure to central FirstNexus security handling.
...
```bash
gh run list --repo FirstNexus/clawsweeper --workflow "ClawSweeper Commit Review" \
  --limit 12 --json databaseId,displayTitle,event,status,conclusion,createdAt,updatedAt,url
gh run list --repo FirstNexus/clawsweeper --workflow "repair cluster worker" \
  --limit 12 --json databaseId,displayTitle,event,status,conclusion,createdAt,updatedAt,url
gh run list --repo FirstNexus/clawsweeper --workflow "repair comment router" \
  --limit 12 --json databaseId,displayTitle,event,status,conclusion,createdAt,updatedAt,url
```
...
```bash
gh run list --repo FirstNexus/FirstNexus --workflow "ClawSweeper Dispatch" \
...
```bash
gh api "repos/FirstNexus/FirstNexus/commits/<sha>/check-runs?per_page=100" \
