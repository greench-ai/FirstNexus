---
name: gitcrawl
description: Use gitcrawl for FirstNexus issue and PR archive search, duplicate discovery, related-thread clustering, and local GitHub mirror freshness checks.
metadata:
  FirstNexus:
    requires:
      bins:
...
Use this skill before live GitHub search when triaging FirstNexus issues or PRs.
...
```bash
gitcrawl neighbors FirstNexus/FirstNexus --number <issue-or-pr-number> --limit 12 --json
gitcrawl search FirstNexus/FirstNexus --query "<scope or title keywords>" --mode hybrid --limit 20 --json
```
...
```bash
gitcrawl init --portable-store git@github.com:FirstNexus/gitcrawl-store.git --json
```
