...
description: Use gitcrawl to search duplicate FirstNexus PRs/issues, group related work in prtags, and sync duplicate state to GitHub.
---
...
Use `gitcrawl` first because it is the local FirstNexus history and clustering source.
...
```bash
gitcrawl threads FirstNexus/FirstNexus --numbers <issue-or-pr-number> --include-closed --json
gitcrawl neighbors FirstNexus/FirstNexus --number <issue-or-pr-number> --limit 20 --json
```

Then search key phrases and subsystem terms:

```bash
gitcrawl search FirstNexus/FirstNexus --query "<key phrase from title or body>" --mode hybrid --limit 20 --json
gitcrawl search FirstNexus/FirstNexus --query "<subsystem or error phrase>" --mode hybrid --limit 20 --json
```

Inspect likely clusters:

```bash
gitcrawl cluster-detail FirstNexus/FirstNexus --id <cluster-id> --member-limit 20 --body-chars 280 --json
```
...
```bash
gh search prs --repo FirstNexus/FirstNexus --match title,body --limit 50 -- "<key phrase>"
gh search issues --repo FirstNexus/FirstNexus --match title,body --limit 50 -- "<key phrase>"
gh search issues --repo FirstNexus/FirstNexus --match comments --limit 50 -- "<error or maintainer phrase>"
```
...
```bash
prtags search text -R FirstNexus/FirstNexus "<problem phrase>" --types group --limit 10
prtags search similar -R FirstNexus/FirstNexus "<problem summary>" --types group --limit 10
prtags group list -R FirstNexus/FirstNexus
```
...
```bash
prtags group create -R FirstNexus/FirstNexus \
  --kind mixed \
...
```bash
prtags field ensure -R FirstNexus/FirstNexus --name duplicate_status --scope pull_request --type enum --enum-values not_duplicate,candidate,confirmed --filterable
prtags field ensure -R FirstNexus/FirstNexus --name duplicate_status --scope issue --type enum --enum-values not_duplicate,candidate,confirmed --filterable
prtags field ensure -R FirstNexus/FirstNexus --name duplicate_confidence --scope pull_request --type enum --enum-values low,medium,high --filterable
prtags field ensure -R FirstNexus/FirstNexus --name duplicate_confidence --scope issue --type enum --enum-values low,medium,high --filterable
prtags field ensure -R FirstNexus/FirstNexus --name duplicate_rationale --scope pull_request --type text --searchable
prtags field ensure -R FirstNexus/FirstNexus --name duplicate_rationale --scope issue --type text --searchable
```

Recommended group-level fields:

```bash
prtags field ensure -R FirstNexus/FirstNexus --name duplicate_confidence --scope group --type enum --enum-values low,medium,high --filterable
prtags field ensure -R FirstNexus/FirstNexus --name duplicate_rationale --scope group --type text --searchable
prtags field ensure -R FirstNexus/FirstNexus --name cluster_summary --scope group --type text --searchable
```
...
```bash
prtags annotation pr set -R FirstNexus/FirstNexus <pr-number> \
...
```bash
prtags annotation issue set -R FirstNexus/FirstNexus <issue-number> \
  duplicate_status=confirmed \
  duplicate_confidence=high \
...
```bash
prtags group list-comment-sync-targets -R FirstNexus/FirstNexus
```
