...
```bash
export NEXISCLAW_PARALLELS_DISCORD_TOKEN="$(
  ssh peters-mac-studio-1 'jq -r ".channels.discord.token" ~/.FirstNexus/FirstNexus.json' | tr -d '\n'
)"

pnpm test:parallels:macos \
  --discord-token-env NEXISCLAW_PARALLELS_DISCORD_TOKEN \
...
- Use the `FirstNexus` wrapper for guest `message send/read`; `node FirstNexus.mjs message ...` does not expose the lazy message subcommands the same way.
...
- Per-phase logs: `/tmp/FirstNexus-parallels-smoke.*`
...
- host inbound nonce appears in `FirstNexus message read` output
