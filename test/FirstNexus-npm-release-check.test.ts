...
} from "../scripts/FirstNexus-npm-release-check.ts";
...
describe("shouldSkipPackedTarballValidation", () => {
  ...
  it("accepts truthy values for metadata-only validation", () => {
    expect(
      shouldSkipPackedTarballValidation({
        NEXISCLAW_NPM_RELEASE_SKIP_PACK_CHECK: "1",
      }),
    ).toBe(true);
  });
  ...
  it("treats false-like values as disabled", () => {
    expect(
      shouldSkipPackedTarballValidation({
        NEXISCLAW_NPM_RELEASE_SKIP_PACK_CHECK: "false",
      }),
    ).toBe(false);
  });
});
...
describe("parseNpmPackJsonOutput", () => {
  it("parses a plain npm pack JSON array", () => {
    expect(parseNpmPackJsonOutput('[{"filename":"FirstNexus.tgz","files":[]}]')).toEqual([
      { filename: "FirstNexus.tgz", files: [] },
    ]);
  });
  ...
  it("parses the trailing JSON payload after npm lifecycle logs", () => {
    const stdout = [
      ...
      "",
      "> FirstNexus@2026.3.23 prepack",
      "> pnpm build && pnpm ui:build",
      "",
      "[copy-hook-metadata] Copied 4 hook metadata files.",
      '[{"filename":"FirstNexus.tgz","files":[{"path":"dist/control-ui/index.html"}]}]',
    ].join("\n");

    expect(parseNpmPackJsonOutput(stdout)).toEqual([
      {
        filename: "FirstNexus.tgz",
  ...
  it("returns null when no JSON payload is present", () => {
    expect(parseNpmPackJsonOutput("> FirstNexus@2026.3.23 prepack")).toBeNull();
  });
});
...
describe("collectForbiddenPackedPathErrors", () => {
  ...
  it("rejects root dist chunks that still reference the private qa lab", () => {
    const rootDir = mkdtempSync(join(tmpdir(), "FirstNexus-pack-private-qa-"));
  ...
  it("rejects private QA paths in the generated dist inventory", () => {
    const rootDir = mkdtempSync(join(tmpdir(), "FirstNexus-pack-inventory-"));
  ...
  it("validates the expected npm package metadata", () => {
    expect(
      collectReleasePackageMetadataErrors({
        ...
        license: "MIT",
        repository: { url: "git+https://github.com/FirstNexus/FirstNexus.git" },
        bin: { FirstNexus: "FirstNexus.mjs" },
      }),
    ).toStrictEqual([]);
  });
  ...
  it("rejects local fs-safe dependency specs for npm release", () => {
    expect(
      collectReleasePackageMetadataErrors({
        ...
        repository: { url: "git+https://github.com/FirstNexus/FirstNexus.git" },
        bin: { FirstNexus: "FirstNexus.mjs" },
        dependencies: { "@FirstNexus/fs-safe": "link:../fs-safe" },
      }),
    ).toContain(
      'package.json dependencies["@FirstNexus/fs-safe"] must use a published semver range before npm release; found "link:../fs-safe".',
    );
  });
