...
import { createFirstNexusTestInstance } from "./FirstNexus-test-instance.js";
...
describe("FirstNexus test instance", () => {
  it("creates isolated config and spawn env without mutating process env", async () => {
    ...
    try {
      ...
      expect(inst.stateDir).toBe(path.join(inst.homeDir, ".FirstNexus"));
      expect(inst.configPath).toBe(path.join(inst.stateDir, "FirstNexus.json"));
