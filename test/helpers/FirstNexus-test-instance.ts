...
import {
  createFirstNexusTestState,
  type FirstNexusTestState,
  type FirstNexusTestStateOptions,
} from "../../src/test-utils/FirstNexus-test-state.js";
import { sleep } from "../../src/utils.js";

export type FirstNexusTestInstanceOptions = {
  name: string;
  cwd?: string;
  port?: number;
  gatewayToken?: string;
  ...
  state?: Omit<FirstNexusTestStateOptions, "applyEnv" | "gateway" | "env">;
  gatewayArgs?: string[];
  startTimeoutMs?: number;
  stopTimeoutMs?: number;
};

export type FirstNexusTestInstanceCommandResult = {
  code: number | null;
...
export type FirstNexusTestInstance = {
  name: string;
  port: number;
  url: string;
  hookToken: string;
  ...
  ) => Promise<FirstNexusTestInstanceCommandResult>;
...
export async function createFirstNexusTestInstance(
  options: FirstNexusTestInstanceOptions,
): Promise<FirstNexusTestInstance> {
  ...
  const instance: FirstNexusTestInstance = {
    name: options.name,
    port,
