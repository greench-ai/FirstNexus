...
import type { FirstNexusConfig } from "../../../src/config/types.FirstNexus.js";
import type {
  AnyAgentTool,
...
import { createFirstNexusCodingTools } from "../../../src/plugin-sdk/agent-harness.js";
...
const WORKSPACE_DIR = "/tmp/FirstNexus-happy-path/workspace";
const AGENT_DIR = "/tmp/FirstNexus-happy-path/agent";
const SESSION_FILE = "/tmp/FirstNexus-happy-path/session.jsonl";
...
const CODEX_WORKSPACE_BOOTSTRAP_INSTRUCTIONS = [
  "FirstNexus loaded these user-editable workspace files. Treat them as project/user context. Codex loads AGENTS.md natively, so AGENTS.md is not repeated here.",
  "",
  "# Project Context",
  "",
...
const baseConfig: FirstNexusConfig = {
  messages: {
...
const dynamicToolsConfig: FirstNexusConfig = {
  ...baseConfig,
  plugins: {
    enabled: true,
    slots: {
...
function createScenarios(): PromptScenario[] {
  ...
  const discordGroupCtx: TemplateContext = {
    ...
    SenderName: "Pash",
    SenderUsername: "pash",
    GroupSubject: "FirstNexus maintainers",
    GroupChannel: "#agent-sandbox",
    GroupSpace: "FirstNexus",
    ConversationLabel: "FirstNexus/#agent-sandbox",
    WasMentioned: true,
    InboundHistory: [
      {
        sender: "Peter",
      ...
      {
        sender: "Pash",
        body: "@FirstNexus please verify the Codex happy path too.",
      },
    ],
    Body: "@FirstNexus can you audit whether this prompt path has conflicting silence instructions?",
  ...
  return [
    {
      ...
      notes: [
        ...
        "A quiet turn is represented by not calling `message(action=send)`; the normal final assistant text is private to FirstNexus/Codex.",
      ],
      trigger: "user",
...
function renderModelBoundPromptLayers(params: {
  ...
  return [
    ...
    "",
    "This is the deterministic model-bound layer stack FirstNexus can snapshot for the Codex happy path. It uses a pinned Codex `gpt-5.5` prompt fixture generated from Codex's model catalog/cache shape, then adds the Codex permission developer text, simulated FirstNexus workspace bootstrap config instructions, FirstNexus developer instructions, turn-scoped collaboration-mode instructions when FirstNexus provides them, turn input, and the FirstNexus dynamic tool catalog. Codex can still add runtime-owned context such as native workspace `AGENTS.md`, environment context, memories, app/plugin instructions, and built-in collaboration-mode instructions inside the Codex runtime.",
    "",
    "### Layer Metadata",
    "",
    markdownFence(
    ...
    "",
    "### User: Codex Config Instructions (FirstNexus Workspace Bootstrap Context)",
    "",
    markdownFence("text", codexConfigInstructions),
    "",
    "### Developer: FirstNexus Runtime Instructions",
    "",
...
function renderScenarioSnapshot(scenario: PromptScenario): string {
  ...
  return [
    ...
    "- This captures the FirstNexus-owned Codex app-server inputs and reconstructs the stable Codex model/permission layers from committed Codex prompt fixtures.",
...
function renderReadme(scenarios: PromptScenario[]): string {
  return [
    ...
    "",
    "The Markdown files show selected app-server thread/turn params plus a reconstructed model-bound prompt layer stack: Codex `gpt-5.5` model instructions from a pinned Codex model catalog fixture, Codex permission developer instructions for the happy-path yolo profile, simulated FirstNexus workspace bootstrap config instructions, FirstNexus developer instructions, user turn input, and references to the complete dynamic tool catalog.",
    "",
    "The workspace bootstrap simulation includes dummy `SOUL.md`, `TOOLS.md`, and `HEARTBEAT.md` contents so prompt reviewers can see how those FirstNexus project/user context files are forwarded to Codex. `AGENTS.md` is intentionally not repeated here because Codex loads it natively.",
    "",
    "The tool catalog is pinned to the canonical happy-path FirstNexus tools so optional locally installed plugin tools do not create fixture churn.",
    "",
    ...
    "",
    "These snapshots are still not a byte-for-byte raw OpenAI request capture. Codex-owned native `AGENTS.md`, environment context, memories, app/plugin instructions, and built-in collaboration-mode instructions can be added inside the Codex runtime after FirstNexus sends thread and turn params.",
    "",
    "Regenerate with:",
    "",
