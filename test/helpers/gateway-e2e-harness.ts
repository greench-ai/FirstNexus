...
import {
  createFirstNexusTestInstance,
  type FirstNexusTestInstance,
} from "./FirstNexus-test-instance.js";
...
export type GatewayInstance = FirstNexusTestInstance;
...
export async function spawnGatewayInstance(name: string): Promise<GatewayInstance> {
  const inst = await createFirstNexusTestInstance({ name });
  try {
