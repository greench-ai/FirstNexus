...
const helperPath = path.join(
  repoRoot,
  ".agents/skills/FirstNexus-pr-maintainer/scripts/github-activity.sh",
);
...
describe("FirstNexus-pr-maintainer github activity helper", () => {
  it("counts PRs and issues from one paginated issues response", () => {
    ...
    expect(result.stdout).toContain("FirstNexus/FirstNexus last 1mo: 2 PRs, 1 issues, 2 commits");
    expect(log.match(/repos\/FirstNexus\/FirstNexus\/issues/g)).toHaveLength(1);
    expect(log.match(/repos\/FirstNexus\/FirstNexus\/commits/g)).toHaveLength(1);
