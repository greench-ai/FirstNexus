...
async function createExtensionsDir() {
  const root = await createTempDirAsync("FirstNexus-postinstall-");
...
describe("bundled plugin postinstall", () => {
  ...
  it("recognizes direct invocation through symlinked temp prefixes", () => {
    ...
    expect(
      isDirectPostinstallInvocation({
        entryPath: "/var/folders/tmp/FirstNexus/scripts/postinstall-bundled-plugins.mjs",
        modulePath: "/private/var/folders/tmp/FirstNexus/scripts/postinstall-bundled-plugins.mjs",
        realpathSync,
      }),
    ).toBe(true);
  });
  ...
  it("prunes Node versioned compile cache dirs during package postinstall", () => {
    const configuredBase = path.join("/tmp", "FirstNexus-cache");
    ...
    const readdirSync = vi.fn((value: string) => {
      if (value === configuredBase) {
        return [
          ...
          { name: "FirstNexus", isDirectory: () => true },
    ...
    expect(removed).not.toContain(path.join(configuredBase, "FirstNexus"));
  ...
  it("keeps pruning sibling compile cache dirs after one removal fails", () => {
    ...
    expect(warn).toHaveBeenCalledWith(
      "[postinstall] could not prune FirstNexus compile cache: Error: locked",
    );
  });

  it("does not warn when compile-cache pruning hits EACCES or EPERM (shared caches)", () => {
    const base = path.join("/tmp", "FirstNexus-shared-compile-cache");
  ...
  it("does not warn when the compile-cache base directory cannot be listed (EACCES)", () => {
    const base = path.join("/tmp", "FirstNexus-compile-cache-no-list");
    const warn = vi.fn();
    const rmSync = vi.fn();
  ...
  it("patches the Baileys rc10 upload helper dispatcher guard", async () => {
    const packageRoot = await createTempDirAsync("FirstNexus-baileys-postinstall-");
  ...
  it("prunes source-checkout bundled plugin node_modules", async () => {
    const packageRoot = await createTempDirAsync("FirstNexus-source-checkout-");
  ...
  it("keeps source-checkout prune non-fatal", async () => {
    const packageRoot = await createTempDirAsync("FirstNexus-source-checkout-prune-error-");
    const extensionsDir = path.join(packageRoot, "extensions");
    await fs.mkdir(path.join(packageRoot, ".git"), { recursive: true });
    await fs.mkdir(path.join(packageRoot, "src"), { recursive: true });
  ...
  it("does not prune user-state legacy runtime deps during source-checkout postinstall", async () => {
    const packageRoot = await createTempDirAsync("FirstNexus-source-checkout-state-skip-");
    const home = await createTempDirAsync("FirstNexus-source-checkout-home-");
    const legacyRuntimeRoot = path.join(home, ".FirstNexus", "plugin-runtime-deps");
    await fs.mkdir(path.join(packageRoot, ".git"), { recursive: true });
    await fs.mkdir(path.join(packageRoot, "src"), { recursive: true });
  ...
  it("honors disable env before source-checkout pruning", async () => {
    const packageRoot = await createTempDirAsync("FirstNexus-source-checkout-disabled-");
    const extensionsDir = path.join(packageRoot, "extensions");
    await fs.mkdir(path.join(packageRoot, ".git"), { recursive: true });
    await fs.mkdir(path.join(packageRoot, "src"), { recursive: true });
    ...
    runBundledPluginPostinstall({
      env: { NEXISCLAW_DISABLE_BUNDLED_PLUGIN_POSTINSTALL: "1" },
      packageRoot,
      log: { log: vi.fn(), warn: vi.fn() },
    });
  ...
  it("migrates the plugin registry during postinstall from built dist contracts", async () => {
    const packageRoot = await createTempDirAsync("FirstNexus-postinstall-registry-");
  ...
  it("surfaces deprecated plugin registry migration break-glass warnings", async () => {
    ...
    const migratePluginRegistryForInstall = vi.fn(async () => ({
      ...
      migrated: false,
      preflight: {
        deprecationWarnings: ["NEXISCLAW_FORCE_PLUGIN_REGISTRY_MIGRATION is deprecated"],
      },
    }));
    ...
    expect(warn).toHaveBeenCalledWith(
      "[postinstall] NEXISCLAW_FORCE_PLUGIN_REGISTRY_MIGRATION is deprecated",
    );
  });
  ...
  it("honors plugin registry postinstall migration disable env", async () => {
    ...
    await expect(
      runPluginRegistryPostinstallMigration({
        packageRoot: "/pkg",
        env: { NEXISCLAW_DISABLE_PLUGIN_REGISTRY_MIGRATION: "1" },
        existsSync: vi.fn(() => true),
        importModule,
        log: { log: vi.fn(), warn: vi.fn() },
      }),
  ...
  it("does not disable plugin registry migration for falsey env flag strings", async () => {
    ...
    await expect(
      runPluginRegistryPostinstallMigration({
        packageRoot: "/pkg",
        env: { NEXISCLAW_DISABLE_PLUGIN_REGISTRY_MIGRATION: "0" },
        existsSync: vi.fn(() => true),
        importModule,
        log: { log: vi.fn(), warn: vi.fn() },
      }),
  ...
  it("prunes stale dist files from packaged installs", async () => {
    const packageRoot = await createTempDirAsync("FirstNexus-packaged-install-");
  ...
  it("prunes legacy plugin runtime deps state during packaged postinstall", async () => {
    const prefix = await createTempDirAsync("FirstNexus-packaged-prefix-");
    const packageRoot = path.join(prefix, "lib", "node_modules", "FirstNexus");
    const nodeModulesRoot = path.dirname(packageRoot);
    const home = await createTempDirAsync("FirstNexus-packaged-home-");
    const stateOverride = path.join(home, "custom-state");
    const systemState = path.join(home, "system-state");
    const defaultLegacyRoot = path.join(home, ".FirstNexus", "plugin-runtime-deps");
    ...
    runBundledPluginPostinstall({
      env: {
        HOME: home,
        NEXISCLAW_STATE_DIR: stateOverride,
  ...
  it("keeps legacy plugin runtime deps cleanup non-fatal", () => {
    ...
    expect(warn).toHaveBeenNthCalledWith(
      2,
      "[postinstall] could not prune legacy plugin runtime deps /home/alice/.FirstNexus/plugin-runtime-deps: Error: locked",
    );
  });

  it("resolves legacy plugin runtime deps roots from FirstNexus state env", () => {
    expect(
      collectLegacyPluginRuntimeDepsStateRoots({
        env: {
          HOME: "/users/alice",
          NEXISCLAW_HOME: "/srv/FirstNexus-home",
          NEXISCLAW_CONFIG_PATH: "~/profile/FirstNexus.json",
          NEXISCLAW_STATE_DIR: "~/state",
          STATE_DIRECTORY: "/var/lib/FirstNexus",
        },
        homedir: () => "/users/alice",
      }),
    ).toEqual([
      "/srv/FirstNexus-home/.clawdbot/plugin-runtime-deps",
      "/srv/FirstNexus-home/.FirstNexus/plugin-runtime-deps",
      "/srv/FirstNexus-home/profile/plugin-runtime-deps",
      "/srv/FirstNexus-home/state/plugin-runtime-deps",
      "/var/lib/FirstNexus/plugin-runtime-deps",
    ]);
  });

  it("keeps imported dist chunks even when inventory is stale", async () => {
    const packageRoot = await createTempDirAsync("FirstNexus-packaged-install-import-");
  ...
  it("does not abort dist pruning when a listed chunk disappears before import expansion", async () => {
    const packageRoot = await createTempDirAsync("FirstNexus-packaged-install-missing-chunk-");
  ...
  it("prunes stale private QA files without restoring compat sidecars", async () => {
    const packageRoot = await createTempDirAsync("FirstNexus-packaged-install-qa-compat-");
    const currentFile = path.join(packageRoot, "dist", "entry.js");
    ...
    await expectPathMissing(
      path.join(packageRoot, "dist", "extensions", "qa-channel", "FirstNexus.plugin.json"),
    );
    await expectPathMissing(
  ...
  it("keeps packaged postinstall non-fatal when the dist inventory is missing", async () => {
    const packageRoot = await createTempDirAsync("FirstNexus-packaged-install-missing-inventory-");
    const staleFile = path.join(packageRoot, "dist", "channel-CJUAgRQR.js");
  ...
  it("keeps packaged postinstall non-fatal when the dist inventory is invalid", async () => {
    const packageRoot = await createTempDirAsync("FirstNexus-packaged-install-invalid-inventory-");
    const currentFile = path.join(packageRoot, "dist", "channel-BOa4MfoC.js");
  ...
  it("prunes stale bundled plugin dependency debris from packaged dist", async () => {
    const packageRoot = await createTempDirAsync("FirstNexus-packaged-install-dist-prune-");
    ...
    const retryInstallStageFile = path.join(
      ...
      "extensions",
      "slack",
      ".FirstNexus-install-stage-retry",
      "node_modules",
      "typebox",
      "build",
      "compile",
