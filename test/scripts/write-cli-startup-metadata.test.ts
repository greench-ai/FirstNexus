...
describe("write-cli-startup-metadata", () => {
  ...
  it("writes startup metadata with populated root help text when dist falls back to source rendering", async () => {
    const tempRoot = createTempDir("FirstNexus-startup-metadata-");
    ...
    await writeCliStartupMetadata({
      ...
      renderSourceRootHelpText: () => "Usage: FirstNexus\n",
      renderSourceBrowserHelpText: () => "Usage: FirstNexus browser\n",
    });
    ...
    expect(written.browserHelpText).toContain("FirstNexus browser");
    expect(written.rootHelpText).toContain("Usage:");
    expect(written.rootHelpText).toContain("FirstNexus");
  });
});
