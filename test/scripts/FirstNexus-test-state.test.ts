...
const scriptPath = path.join(repoRoot, "scripts/lib/FirstNexus-test-state.mjs");
...
describe("scripts/lib/FirstNexus-test-state", () => {
  it("creates a sourceable env file and JSON description", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "FirstNexus-test-state-script-"));
    ...
    try {
      ...
      expect(payload.stateDir).toBe(path.join(payload.home, ".FirstNexus"));
      expect(payload.configPath).toBe(path.join(payload.stateDir, "FirstNexus.json"));
  ...
  it("renders a Docker-friendly shell snippet", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "FirstNexus-test-state-shell-"));
    ...
    try {
      ...
      expect(stdout).toContain(
        'mktemp -d "$NEXISCLAW_TEST_STATE_TMP_ROOT/FirstNexus-update-channel-switch-update-stable-home.XXXXXX"',
      );
      ...
      const probe = await execFileAsync("bash", [
        "-lc",
        `source ${shellQuote(snippetFile)}; node -e 'const fs=require("node:fs"); const config=JSON.parse(fs.readFileSync(process.env.NEXISCLAW_CONFIG_PATH,"utf8")); process.stdout.write(JSON.stringify({home:process.env.HOME,FirstNexusHome:process.env.NEXISCLAW_HOME,workspace:process.env.NEXISCLAW_TEST_WORKSPACE_DIR,channel:config.update.channel}));'; rm -rf "$HOME"`,
      ]);

      const payload = JSON.parse(probe.stdout);
      expect(payload.home.startsWith(os.tmpdir())).toBe(true);
      expect(path.basename(payload.home)).toMatch(
        /^FirstNexus-update-channel-switch-update-stable-home\./u,
      );
      expect(payload.FirstNexusHome).toBe(payload.home);
      ...
      expect(customPayload.home).toMatch(
        new RegExp(
          `^${escapeRegex(customTemp)}/FirstNexus-update-channel-switch-update-stable-home\\.`,
        ),
      );
    } finally {
      await fs.rm(tempRoot, { recursive: true, force: true });
  ...
  it("renders a reusable Docker shell function", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "FirstNexus-test-state-function-"));
    ...
    try {
      ...
      expect(stdout).toContain("FirstNexus_test_state_create()");
      ...
      const existingProbe = await execFileAsync("bash", [
        "-lc",
        `source ${shellQuote(snippetFile)}; FirstNexus_test_state_create ${shellQuote(existingHome)} minimal; printf '{"kept":true}\\n' > "$NEXISCLAW_CONFIG_PATH"; FirstNexus_test_state_create ${shellQuote(existingHome)} empty; node -e 'const fs=require("node:fs"); const config=JSON.parse(fs.readFileSync(process.env.NEXISCLAW_CONFIG_PATH,"utf8")); process.stdout.write(JSON.stringify({home:process.env.HOME,config}));'`,
      ]);
  ...
  it("keeps onboard Docker temp homes on the shared test-state helper", async () => {
    ...
    expect(scenarioText).toContain("set_isolated_FirstNexus_env local-basic");
