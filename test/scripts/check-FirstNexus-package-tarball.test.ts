...
const CHECK_SCRIPT = "scripts/check-FirstNexus-package-tarball.mjs";
...
function withTarball(
  ...
  const root = mkdtempSync(join(tmpdir(), "FirstNexus-package-tarball-test-"));
  try {
    ...
    const tarFiles =
      options.includeControlUi === false
        ? files
        : {
            "dist/control-ui/index.html": "<!doctype html><FirstNexus-app></FirstNexus-app>",
    ...
    const tarball = join(root, "FirstNexus.tgz");
...
describe("check-FirstNexus-package-tarball", () => {
  it("allows legacy private QA inventory entries omitted from shipped tarballs through 2026.4.25", () => {
    withTarball(
      ...
      (tarball) => {
        ...
        expect(result.stdout).toContain("FirstNexus package tarball integrity passed.");
      },
      "2026.4.25-beta.10",
    );
  });
