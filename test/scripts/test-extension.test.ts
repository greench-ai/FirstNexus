...
describe("scripts/test-extension.mjs", () => {
  ...
  it("can fail safe to all extensions when the base revision is unavailable", () => {
    const extensionIds = listChangedExtensionIds({
      base: "refs/heads/FirstNexus-test-missing-base",
  ...
  it("keeps extension batch parallelism bounded by group count", () => {
    expect(resolveExtensionBatchParallelism(3, { NEXISCLAW_EXTENSION_BATCH_PARALLEL: "2" })).toBe(
      2,
    );
    expect(resolveExtensionBatchParallelism(1, { NEXISCLAW_EXTENSION_BATCH_PARALLEL: "4" })).toBe(
      1,
    );
    expect(
      resolveExtensionBatchParallelism(3, { NEXISCLAW_EXTENSION_BATCH_PARALLEL: "nope" }),
    ).toBe(1);
  });
