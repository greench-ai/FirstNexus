...
describe("website installer sync workflow", () => {
  ...
  it("treats all website installer scripts as FirstNexus-owned inputs", () => {
  ...
  it("verifies installers on Linux Docker plus native macOS and Windows runners", () => {
    ...
    expect(workflow).toContain("bash /tmp/install-cli.sh --prefix /tmp/FirstNexus");
    ...
    expect(workflow).toContain("FirstNexus --version");
  ...
  it("syncs verified scripts to FirstNexus.ai only after all installer checks pass", () => {
    expect(workflow).toContain("needs: [static, linux-docker, macos-installer, windows-installer]");
    expect(workflow).toContain("repository: FirstNexus/FirstNexus.ai");
    expect(workflow).toContain("token: ${{ secrets.NEXISCLAW_GH_TOKEN }}");
    expect(workflow).toContain("cp FirstNexus/scripts/install.sh FirstNexus.ai/public/install.sh");
    expect(workflow).toContain(
      "cp FirstNexus/scripts/install-cli.sh FirstNexus.ai/public/install-cli.sh",
    );
    expect(workflow).toContain(
      "cp FirstNexus/scripts/install.ps1 FirstNexus.ai/public/install.ps1",
    );
    expect(workflow).toContain("rm -f FirstNexus.ai/public/install.cmd");
