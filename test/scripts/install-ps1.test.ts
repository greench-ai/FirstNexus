...
describe("install.ps1 failure handling", () => {
  ...
  it("keeps failure termination in the top-level completion handler", () => {
    ...
    expect(completeInstallBody).toMatch(/\bthrow "FirstNexus installation failed with exit code/);
  });

  it("runs npm install through the resolved command with quiet CI defaults", () => {
    const npmInstallBody = extractFunctionBody(source, "Install-FirstNexus");
  ...
  it("cleans legacy git submodules only from the selected git checkout", () => {
    const gitInstallBody = extractFunctionBody(source, "Install-FirstNexusFromGit");
    const mainBody = extractFunctionBody(source, "Main");
  ...
  runIfPowerShell("exits non-zero when run as a script file", () => {
    const tempDir = harness.createTempDir("FirstNexus-install-ps1-");
  ...
  runIfPowerShell("throws without killing the caller when run as a scriptblock", () => {
    ...
    expect(result.stdout).toContain("caught=FirstNexus installation failed with exit code 1.");
  ...
  runIfPowerShell("keeps npm chatter out of Main's success return value", () => {
    ...
    writeFileSync(
      ...
      [
        ...
        "function Check-ExistingFirstNexus { return $false }",
        "function Add-ToPath { param([string]$Path) }",
        "function Install-FirstNexus { Write-Output 'npm stdout'; return $true }",
        "function Ensure-FirstNexusOnPath { return $true }",
        "function Refresh-GatewayServiceIfLoaded { }",
        "function Invoke-FirstNexusCommand { return 'FirstNexus test-version' }",
        "$NoOnboard = $true",
        "$result = Main",
