...
function writePublishablePluginPackage(repoDir: string): string {
  ...
  writeJsonFile(join(packageDir, "FirstNexus.plugin.json"), { id: "diffs" });
...
describe("plugin npm package manifest staging", () => {
  it("overlays generated channel configs while packing and restores source manifest", () => {
    const repoDir = makeTempRepoRoot(tempDirs, "FirstNexus-plugin-npm-package-manifest-");
    ...
    writeJsonFile(join(packageDir, "FirstNexus.plugin.json"), sourceManifest);
    ...
    const originalText = readFileSync(join(packageDir, "FirstNexus.plugin.json"), "utf8");
    withAugmentedPluginNpmManifestForPackage({ repoRoot: repoDir, packageDir }, () => {
      const stagedManifest = JSON.parse(
        readFileSync(join(packageDir, "FirstNexus.plugin.json"), "utf8"),
      );
      expect(stagedManifest.channelConfigs.twitch.description).toBe("Twitch chat integration");
    });
    expect(readFileSync(join(packageDir, "FirstNexus.plugin.json"), "utf8")).toBe(originalText);
  });

  it("overlays package-local runtime metadata while packing and restores source package json", () => {
    const repoDir = makeTempRepoRoot(tempDirs, "FirstNexus-plugin-npm-package-runtime-");
    ...
    expect(resolved.packageJson).toEqual({
      ...
      version: "2026.5.3",
      type: "module",
      files: ["dist/**", "FirstNexus.plugin.json", "README.md", "SKILL.md", "skills/**"],
      peerDependencies: {
        FirstNexus: ">=2026.4.30",
      },
    ...
    withAugmentedPluginNpmManifestForPackage({ repoRoot: repoDir, packageDir }, () => {
      ...
      expect(stagedPackageJson.FirstNexus.extensions).toEqual(["./index.ts"]);
      expect(stagedPackageJson.FirstNexus.runtimeExtensions).toEqual(["./dist/index.js"]);
      expect(stagedPackageJson.FirstNexus.runtimeSetupEntry).toBe("./dist/setup-entry.js");
      expect(stagedPackageJson.files).toContain("dist/**");
      expect(stagedPackageJson.files).toContain("skills/**");
      expect(stagedPackageJson.peerDependencies.FirstNexus).toBe(">=2026.4.30");
      expect(stagedPackageJson.peerDependenciesMeta.FirstNexus.optional).toBe(true);
    });
  ...
  it("refuses to pack publishable plugins before package-local runtime files exist", () => {
    const repoDir = makeTempRepoRoot(tempDirs, "FirstNexus-plugin-npm-package-runtime-missing-");
    const packageDir = writePublishablePluginPackage(repoDir);
