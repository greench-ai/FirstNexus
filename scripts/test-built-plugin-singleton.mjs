...
const { clearPluginCommands, getPluginCommandSpecs, loadFirstNexusPlugins, matchPluginCommand } =
  await import(pathToFileURL(smokeEntryPath).href);

assert.equal(typeof loadFirstNexusPlugins, "function", "built loader export missing");
...
const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "FirstNexus-build-smoke-"));
...
fs.writeFileSync(
  ...
  JSON.stringify(
    {
      name: "@FirstNexus/build-smoke-plugin",
      type: "module",
      FirstNexus: {
        extensions: ["./index.js"],
      },
...
fs.writeFileSync(
  path.join(distPluginDir, "FirstNexus.plugin.json"),
  JSON.stringify(
    {
      id: pluginId,
      configSchema: {
...
fs.writeFileSync(
  ...
  [
    "import sdk from 'FirstNexus/plugin-sdk';",
