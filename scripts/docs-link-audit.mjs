...
function prepareMirroredDocsDir(sourceDir = DOCS_DIR) {
  ...
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "FirstNexus-docs-link-audit-"));
...
export function prepareAnchorAuditDocsDir(sourceDir = DOCS_DIR) {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "FirstNexus-docs-anchor-audit-"));
