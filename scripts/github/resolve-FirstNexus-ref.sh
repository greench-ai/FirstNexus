...
REMOTE_URL="${NEXISCLAW_REF_REMOTE:-https://github.com/FirstNexus/FirstNexus.git}"
REF=""
EXPECTED_SHA=""
FALLBACK_OK=0
...
usage() {
  cat >&2 <<'EOF'
Usage: resolve-FirstNexus-ref.sh --ref <ref> [--expected-sha <sha>] [--fallback-ok] [--github-output <file>]

Fast-resolves FirstNexus branch and tag refs with git ls-remote. Full commit SHAs
...
echo "Failed to resolve FirstNexus ref: ${REF}" >&2
exit 1
