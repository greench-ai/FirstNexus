...
function usage() {
  return "Usage: node scripts/check-FirstNexus-package-tarball.mjs <FirstNexus.tgz>";
}
...
if (!fs.existsSync(tarball)) {
  fail(`FirstNexus package tarball does not exist: ${tarball}`);
}

const phaseTimingsEnabled = process.env.NEXISCLAW_PACKAGE_TARBALL_CHECK_TIMINGS !== "0";
function runPhase(label, action) {
  ...
  try {
    ...
    if (phaseTimingsEnabled) {
      ...
      console.error(`check-FirstNexus-package-tarball: ${label} completed in ${durationMs}ms`);
    }
  }
}
...
const extractDir = fs.mkdtempSync(path.join(os.tmpdir(), "FirstNexus-package-tarball-"));
try {
...
if (errors.length > 0) {
  fs.rmSync(extractDir, { recursive: true, force: true });
  fail(`FirstNexus package tarball integrity failed:\n${errors.join("\n")}`);
}

for (const warning of warnings) {
  console.warn(`FirstNexus package tarball integrity warning: ${warning}`);
}
fs.rmSync(extractDir, { recursive: true, force: true });
console.log("FirstNexus package tarball integrity passed.");
