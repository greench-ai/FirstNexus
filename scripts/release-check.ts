...
export const CRITICAL_PLUGIN_SDK_SIZE_CHECK_SPECIFIERS = [
  "FirstNexus/plugin-sdk/agent-runtime-test-contracts",
  "FirstNexus/plugin-sdk/plugin-test-contracts",
  "FirstNexus/plugin-sdk/provider-test-contracts",
] as const;
...
function resolveInstalledBinaryPath(prefixDir: string): string {
  return process.platform === "win32"
    ? join(prefixDir, "FirstNexus.cmd")
    : join(prefixDir, "bin", "FirstNexus");
}
...
export function writePackedBundledPluginActivationConfig(homeDir: string): void {
  const configPath = join(homeDir, ".FirstNexus", "FirstNexus.json");
  mkdirSync(join(homeDir, ".FirstNexus"), { recursive: true });
  writeFileSync(
    configPath,
    `${JSON.stringify(
      {
        ...
        models: {
          providers: {
            openai: {
              apiKey: "sk-FirstNexus-release-check",
...
function runPackedBundledPluginActivationSmoke(packageRoot: string, tmpRoot: string): void {
  ...
  const env = createPackedCliSmokeEnv(process.env, {
    HOME: homeDir,
    OPENAI_API_KEY: "sk-FirstNexus-release-check",
  });

  writePackedBundledPluginActivationConfig(homeDir);
  execFileSync(
    process.execPath,
    [join(packageRoot, "FirstNexus.mjs"), ...PACKED_BUNDLED_RUNTIME_DEPS_REPAIR_ARGS],
    {
      cwd: packageRoot,
      stdio: "inherit",
      env,
  ...
  execFileSync(process.execPath, [join(packageRoot, "FirstNexus.mjs"), "plugins", "doctor"], {
    cwd: packageRoot,
    stdio: "inherit",
    env,
  });
...
function runPackedBundledChannelEntrySmoke(): void {
  const tmpRoot = mkdtempSync(join(tmpdir(), "FirstNexus-release-pack-smoke-"));
  try {
    ...
    const packageRoot = join(resolveGlobalRoot(prefixDir, tmpRoot), "FirstNexus");
    ...
    execFileSync(
      process.execPath,
      [join(packageRoot, "FirstNexus.mjs"), ...PACKED_COMPLETION_SMOKE_ARGS],
      {
        cwd: packageRoot,
        stdio: "inherit",
  ...
  return [...paths]
    .filter(
      (path) =>
        ...
        /(^|\/)\.FirstNexus-runtime-deps-[^/]+(\/|$)/u.test(path) ||
        path.endsWith("/.FirstNexus-runtime-deps-stamp.json") ||
...
// Critical functions that channel extension plugins import from FirstNexus/plugin-sdk.
...
export function collectCriticalPluginSdkEntrypointSizeErrors(rootDir = process.cwd()): string[] {
  ...
  for (const specifier of CRITICAL_PLUGIN_SDK_SIZE_CHECK_SPECIFIERS) {
    const subpath = specifier.slice("FirstNexus/plugin-sdk/".length);
