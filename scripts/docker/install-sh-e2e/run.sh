...
SKIP_PREVIOUS="${NEXISCLAW_INSTALL_E2E_SKIP_PREVIOUS:-0}"
...
AGENT_TOOL_SMOKE="${NEXISCLAW_INSTALL_E2E_AGENT_TOOL_SMOKE:-1}"
OPENAI_AGENT_MODEL="${NEXISCLAW_INSTALL_E2E_OPENAI_MODEL:-openai/gpt-5.5}"
OPENAI_PROVIDER_TIMEOUT_SECONDS="${NEXISCLAW_INSTALL_E2E_OPENAI_PROVIDER_TIMEOUT_SECONDS:-${AGENT_TURN_TIMEOUT_SECONDS}}"
...
if [[ "$MODELS_MODE" != "both" && "$MODELS_MODE" != "openai" && "$MODELS_MODE" != "anthropic" ]]; then
  echo "ERROR: NEXISCLAW_E2E_MODELS must be one of: both|openai|anthropic" >&2
  exit 2
fi

if [[ "$MODELS_MODE" == "both" ]]; then
  if [[ -z "$OPENAI_API_KEY" ]]; then
    echo "ERROR: NEXISCLAW_E2E_MODELS=both requires OPENAI_API_KEY." >&2
    exit 2
  fi
  if [[ -z "$ANTHROPIC_API_TOKEN" && -z "$ANTHROPIC_API_KEY" ]]; then
    echo "ERROR: NEXISCLAW_E2E_MODELS=both requires ANTHROPIC_API_TOKEN or ANTHROPIC_API_KEY." >&2
    exit 2
  fi
elif [[ "$MODELS_MODE" == "openai" && -z "$OPENAI_API_KEY" ]]; then
  echo "ERROR: NEXISCLAW_E2E_MODELS=openai requires OPENAI_API_KEY." >&2
  exit 2
elif [[ "$MODELS_MODE" == "anthropic" && -z "$ANTHROPIC_API_TOKEN" && -z "$ANTHROPIC_API_KEY" ]]; then
  echo "ERROR: NEXISCLAW_E2E_MODELS=anthropic requires ANTHROPIC_API_TOKEN or ANTHROPIC_API_KEY." >&2
  exit 2
fi
...
run_official_installer() {
  if [[ "$INSTALL_TAG" == "beta" ]]; then
    curl -fsSL "$INSTALL_URL" | NEXISCLAW_BETA=1 bash
  elif [[ "$INSTALL_TAG" != "latest" ]]; then
    curl -fsSL "$INSTALL_URL" | NEXISCLAW_VERSION="$INSTALL_TAG" bash
  else
...
run_profile() {
  ...
  if [[ "$AGENT_TOOL_SMOKE" == "0" ]]; then
    echo "Skip agent tool smoke ($profile, NEXISCLAW_INSTALL_E2E_AGENT_TOOL_SMOKE=0)"
    cleanup_profile
    trap - EXIT
    return 0
  fi
