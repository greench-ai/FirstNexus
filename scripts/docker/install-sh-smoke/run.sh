...
UPDATE_BASELINE_VERSION="${NEXISCLAW_INSTALL_UPDATE_BASELINE:-latest}"
UPDATE_BASELINE_TAG_URL="${NEXISCLAW_INSTALL_UPDATE_BASELINE_TAG_URL:-}"
UPDATE_EXPECT_VERSION="${NEXISCLAW_INSTALL_UPDATE_EXPECT_VERSION:-}"
UPDATE_TAG_URL="${NEXISCLAW_INSTALL_UPDATE_TAG_URL:-}"
HEARTBEAT_INTERVAL="${NEXISCLAW_INSTALL_SMOKE_HEARTBEAT_INTERVAL:-60}"
INSTALL_COMMAND_TIMEOUT="${NEXISCLAW_INSTALL_SMOKE_COMMAND_TIMEOUT:-900}"
...
run_install_smoke() {
  if [[ -n "$FRESH_VERSION" && -n "$FRESH_TAG_URL" ]]; then
    ...
    if [[ -n "${NEXISCLAW_INSTALL_LATEST_OUT:-}" ]]; then
      ...
      if [[ -n "$latest_npm_version" ]]; then
        printf "%s" "$latest_npm_version" > "${NEXISCLAW_INSTALL_LATEST_OUT:-}"
      else
        printf "%s" "$FRESH_VERSION" > "${NEXISCLAW_INSTALL_LATEST_OUT:-}"
      fi
    fi
...
NODE
  ...
  if [[ "$SKIP_PREVIOUS" == "1" ]]; then
    echo "==> Skip preinstall previous (NEXISCLAW_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
  else
  ...
  echo "==> Verify installed version"
  if [[ -n "${NEXISCLAW_INSTALL_LATEST_OUT:-}" ]]; then
    printf "%s" "$LATEST_VERSION" > "${NEXISCLAW_INSTALL_LATEST_OUT:-}"
  fi
...
run_update_smoke() {
  if [[ -z "$UPDATE_EXPECT_VERSION" ]]; then
    echo "ERROR: NEXISCLAW_INSTALL_UPDATE_EXPECT_VERSION is required for update mode" >&2
    return 1
  fi
  if [[ -z "$UPDATE_TAG_URL" ]]; then
    echo "ERROR: NEXISCLAW_INSTALL_UPDATE_TAG_URL is required for update mode" >&2
    return 1
  fi
...
run_npm_global_smoke() {
  if [[ -z "$UPDATE_EXPECT_VERSION" ]]; then
    echo "ERROR: NEXISCLAW_INSTALL_UPDATE_EXPECT_VERSION is required for npm-global mode" >&2
    return 1
  fi
  if [[ -z "$UPDATE_TAG_URL" ]]; then
    echo "ERROR: NEXISCLAW_INSTALL_UPDATE_TAG_URL is required for npm-global mode" >&2
    return 1
  fi
...
case "$SMOKE_MODE" in
  ...
  npm-global)
    ...
    ;;
  *)
    echo "ERROR: unsupported NEXISCLAW_INSTALL_SMOKE_MODE=$SMOKE_MODE" >&2
    exit 1
    ;;
esac
