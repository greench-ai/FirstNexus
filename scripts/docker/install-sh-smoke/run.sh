...
INSTALL_URL="${NEXUSCLAW_INSTALL_URL:-https://nexusclaw.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${NEXUSCLAW_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${NEXUSCLAW_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="nexusclaw"
PACKAGE_NAME="${NEXUSCLAW_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (NEXUSCLAW_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${NEXUSCLAW_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${NEXUSCLAW_INSTALL_LATEST_OUT:-}"
fi
