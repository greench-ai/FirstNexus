#!/usr/bin/env bash
# ClawDock - Docker helpers for NexusClaw
# Inspired by Simon Willison's "Running NexusClaw in Docker"
# https://til.simonwillison.net/llms/nexusclaw-docker
#
# Installation:
#   mkdir -p ~/.clawdock && curl -sL https://raw.githubusercontent.com/nexusclaw/nexusclaw/main/scripts/shell-helpers/clawdock-helpers.sh -o ~/.clawdock/clawdock-helpers.sh
...
# Common paths to check for NexusClaw
CLAWDOCK_COMMON_PATHS=(
  "${HOME}/nexusclaw"
  "${HOME}/workspace/nexusclaw"
  "${HOME}/projects/nexusclaw"
  "${HOME}/dev/nexusclaw"
  "${HOME}/code/nexusclaw"
  "${HOME}/src/nexusclaw"
)
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "🦞 Found NexusClaw at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      echo "  export CLAWDOCK_DIR=/path/to/nexusclaw"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    echo "❌ NexusClaw not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
    echo "  git clone https://github.com/nexusclaw/nexusclaw.git ~/nexusclaw"
    echo "  cd ~/nexusclaw && ./scripts/docker/setup.sh"
    echo ""
...
_clawdock_read_env_token() {
  ...
  local raw
  raw=$(sed -n 's/^NEXUSCLAW_GATEWAY_TOKEN=//p' "${CLAWDOCK_DIR}/.env" | head -n 1)
  if [[ -z "$raw" ]]; then
    return 1
  fi
  _clawdock_trim_quotes "$raw"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d nexusclaw-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart nexusclaw-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f nexusclaw-gateway
}
...
clawdock-workspace() {
  cd ~/.nexusclaw/workspace
}

# Container Access
clawdock-shell() {
  _clawdock_compose exec nexusclaw-gateway \
    bash -c 'echo "alias nexusclaw=\"./nexusclaw.mjs\"" > /tmp/.bashrc_nexusclaw && bash --rcfile /tmp/.bashrc_nexusclaw'
}
...
clawdock-cli() {
  _clawdock_compose run --rm nexusclaw-cli "$@"
}

# Maintenance
clawdock-rebuild() {
  _clawdock_compose build nexusclaw-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "NEXUSCLAW_GATEWAY_TOKEN=$token" nexusclaw-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" nexusclaw-gateway \
    bash -c './nexusclaw.mjs config set gateway.remote.token "$TOKEN" && ./nexusclaw.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec nexusclaw-gateway \
    bash -c "./nexusclaw.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart nexusclaw-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm nexusclaw-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec nexusclaw-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
  ...
  if [ $exit_status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'nexusclaw config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🦞 ClawDock - Docker Helpers for NexusClaw${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (nexusclaw alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to nexusclaw project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.nexusclaw)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'nexusclaw channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'nexusclaw status')"
  echo ""
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.nexusclaw.ai${_CLR_RESET}"
  echo ""
}
