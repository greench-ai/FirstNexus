...
function usage() {
  return `Usage:
  node scripts/lib/FirstNexus-test-state.mjs -- create [--label <name>] [--scenario <name>] [--env-file <path>] [--json]
  node scripts/lib/FirstNexus-test-state.mjs shell [--label <name>] [--scenario <name>]
  node scripts/lib/FirstNexus-test-state.mjs shell-function
...
function scenarioConfig(scenario, options = {}) {
  ...
  if (scenario === "upgrade-survivor") {
    return {
      ...
      skills: {
        allowBundled: ["memory", "FirstNexus-testing"],
        limits: {
          maxSkillsInPrompt: 8,
  ...
  if (scenario === "gateway-loopback") {
    return {
      gateway: {
        ...
        auth: {
          mode: "token",
          token: options.token || "FirstNexus-test-token",
        },
        controlUi: {
          enabled: false,
        },
...
function renderConfigWrite(configPathExpression, config) {
  ...
  return [
    `cat > ${configPathExpression} <<'NEXISCLAW_TEST_STATE_JSON'`,
    json,
    "NEXISCLAW_TEST_STATE_JSON",
  ].join("\n");
}
...
export async function createState(options = {}) {
  const label = normalizeLabel(options.label);
  const root = await fs.mkdtemp(path.join(os.tmpdir(), `FirstNexus-${label}-`));
...
export function renderShellSnippet(options = {}) {
  ...
  const homeTemplate = `FirstNexus-${label}-${scenario}-home.XXXXXX`;
  const lines = [
    'NEXISCLAW_TEST_STATE_TMP_ROOT="${NEXISCLAW_TEST_STATE_TMPDIR:-${TMPDIR:-/tmp}}"',
    "export NEXISCLAW_TEST_STATE_TMP_ROOT",
    'mkdir -p "$NEXISCLAW_TEST_STATE_TMP_ROOT"',
    `NEXISCLAW_TEST_STATE_HOME="$(mktemp -d "$NEXISCLAW_TEST_STATE_TMP_ROOT/${homeTemplate}")"`,
    'export HOME="$NEXISCLAW_TEST_STATE_HOME"',
    'export USERPROFILE="$NEXISCLAW_TEST_STATE_HOME"',
    'export NEXISCLAW_HOME="$NEXISCLAW_TEST_STATE_HOME"',
    'export NEXISCLAW_STATE_DIR="$NEXISCLAW_TEST_STATE_HOME/.FirstNexus"',
    'export NEXISCLAW_CONFIG_PATH="$NEXISCLAW_STATE_DIR/FirstNexus.json"',
    'export NEXISCLAW_TEST_WORKSPACE_DIR="$NEXISCLAW_TEST_STATE_HOME/workspace"',
    'mkdir -p "$NEXISCLAW_STATE_DIR" "$NEXISCLAW_TEST_WORKSPACE_DIR"',
  ];
  for (const [key, value] of Object.entries(env)) {
    lines.push(`export ${key}=${shellQuote(value)}`);
  }
  const configWrite = renderConfigWrite('"$NEXISCLAW_CONFIG_PATH"', config);
  if (configWrite) {
...
export function renderShellFunction() {
  return `FirstNexus_test_state_create() {
  ...
  case "$scenario" in
    ...
    *)
      echo "unknown FirstNexus test-state scenario: $scenario" >&2
      return 1
      ;;
  esac
  case "$raw_label" in
    /*)
      NEXISCLAW_TEST_STATE_HOME="$raw_label"
      mkdir -p "$NEXISCLAW_TEST_STATE_HOME"
      ;;
    *)
      label="$(printf "%s" "$label" | tr -cs "A-Za-z0-9_.-" "-" | sed -e "s/^-*//" -e "s/-*$//")"
      [ -n "$label" ] || label="state"
      local tmp_root="\${NEXISCLAW_TEST_STATE_TMPDIR:-\${TMPDIR:-/tmp}}"
      mkdir -p "$tmp_root"
      NEXISCLAW_TEST_STATE_HOME="$(mktemp -d "$tmp_root/FirstNexus-$label-$scenario-home.XXXXXX")"
      ;;
  esac
  export HOME="$NEXISCLAW_TEST_STATE_HOME"
  export USERPROFILE="$NEXISCLAW_TEST_STATE_HOME"
  ...
  export NEXISCLAW_CONFIG_PATH="$NEXISCLAW_STATE_DIR/FirstNexus.json"
  export NEXISCLAW_TEST_WORKSPACE_DIR="$NEXISCLAW_TEST_STATE_HOME/workspace"
  unset NEXISCLAW_AGENT_DIR
  unset PI_CODING_AGENT_DIR
  unset NEXISCLAW_SERVICE_REPAIR_POLICY
  mkdir -p "$NEXISCLAW_STATE_DIR" "$NEXISCLAW_TEST_WORKSPACE_DIR"
  case "$scenario" in
    minimal)
      cat > "$NEXISCLAW_CONFIG_PATH" <<'NEXISCLAW_TEST_STATE_JSON'
{}
NEXISCLAW_TEST_STATE_JSON
      ;;
    update-stable)
...
NEXISCLAW_TEST_STATE_JSON
      ;;
    external-service)
      export NEXISCLAW_SERVICE_REPAIR_POLICY="external"
      cat > "$NEXISCLAW_CONFIG_PATH" <<'NEXISCLAW_TEST_STATE_JSON'
{}
NEXISCLAW_TEST_STATE_JSON
      ;;
