...
FirstNexus_e2e_eval_test_state_from_b64() { eval "$(printf '%s' "${1:?missing FirstNexus test-state script}" | base64 -d)"; }
FirstNexus_e2e_resolve_entrypoint() {
  local entry
  for entry in dist/index.mjs dist/index.js; do
    [ -f "$entry" ] && { printf '%s\n' "$entry"; return 0; }
  done
  echo "FirstNexus entrypoint not found under dist/" >&2
  return 1
}
FirstNexus_e2e_package_root() {
  local prefix="${1:-}"
  if [ -n "$prefix" ]; then
...
FirstNexus_e2e_package_entrypoint() {
  local root="${1:?missing package root}"
  local entry
  for entry in "$root/dist/index.mjs" "$root/dist/index.js"; do
    [ -f "$entry" ] && { printf '%s\n' "$entry"; return 0; }
  done
  echo "FirstNexus package entrypoint not found under $root/dist/" >&2
  return 1
}
FirstNexus_e2e_install_package() {
  local log_file="$1"
  local label="${2:-mounted FirstNexus package}"
  local prefix="${3:-}"
  local package_tgz="${NEXISCLAW_CURRENT_PACKAGE_TGZ:?missing NEXISCLAW_CURRENT_PACKAGE_TGZ}"
  local args=(-g)
  if [ -n "$prefix" ]; then
    args+=("--prefix" "$prefix")
  fi
...
FirstNexus_e2e_assert_package_extensions() {
  local root="$1"
  shift
  local extension
  for extension in "$@"; do
...
FirstNexus_e2e_write_state_env() {
  local target="${1:-/tmp/FirstNexus-test-state-env}"
  {
    printf 'export HOME=%q\n' "$HOME"
    printf 'export NEXISCLAW_HOME=%q\n' "$NEXISCLAW_HOME"
    printf 'export NEXISCLAW_STATE_DIR=%q\n' "$NEXISCLAW_STATE_DIR"
    printf 'export NEXISCLAW_CONFIG_PATH=%q\n' "$NEXISCLAW_CONFIG_PATH"
    printf 'export NEXISCLAW_AGENT_DIR=%q\n' "${NEXISCLAW_AGENT_DIR-}"
...
FirstNexus_e2e_install_trash_shim() {
  ...
  cat >/tmp/FirstNexus-bin/trash <<'TRASH'
#!/usr/bin/env bash
set -euo pipefail
...
TRASH
  chmod +x /tmp/FirstNexus-bin/trash
}
FirstNexus_e2e_stop_process() {
  local pid="${1:-}" _
  [ -n "$pid" ] || return 0
...
FirstNexus_e2e_terminate_gateways() {
  local pid="${1:-}" _
...
FirstNexus_e2e_start_mock_openai() { MOCK_PORT="$1" node scripts/e2e/mock-openai-server.mjs >"$2" 2>&1 & printf '%s\n' "$!"; }
FirstNexus_e2e_wait_mock_openai() {
...
FirstNexus_e2e_start_gateway() { node "$1" gateway --port "$2" --bind loopback --allow-unconfigured >"$3" 2>&1 & printf '%s\n' "$!"; }
FirstNexus_e2e_exec_gateway() { exec node "$1" gateway --port "$2" --bind "${3:-loopback}" --allow-unconfigured >"$4" 2>&1; }
FirstNexus_e2e_wait_gateway_ready() {
...
FirstNexus_e2e_probe_http_status() {
  node -e 'fetch(process.argv[1]).then(r=>process.exit(r.status===Number(process.argv[2])?0:1)).catch(()=>process.exit(1))' "$1" "${2:-200}"
}
FirstNexus_e2e_assert_file() { [ -f "$1" ] || { echo "Missing file: $1"; exit 1; }; }
FirstNexus_e2e_assert_dir() { [ -d "$1" ] || { echo "Missing dir: $1"; exit 1; }; }
FirstNexus_e2e_assert_log_not_contains() {
  ! grep -q "$2" "$1" || { echo "Unexpected log output: $2"; exit 1; }
}
FirstNexus_e2e_run_logged() {
  local label="$1" log_path="/tmp/FirstNexus-onboard-${1}.log"
  shift
...
FirstNexus_e2e_dump_logs() {
  ...
  for path in "$@"; do
    [ -f "$path" ] || continue
    echo "--- $path ---"; tail -n "${NEXISCLAW_E2E_LOG_TAIL_LINES:-120}" "$path" || true
  done
}
