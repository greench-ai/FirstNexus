...
const DEFAULT_REPO = "FirstNexus/FirstNexus";
...
export function parseReleaseVerifyBetaArgs(argv: string[]): ReleaseVerifyBetaArgs {
  ...
  if (!version || version.startsWith("-")) {
    throw new Error(
      "Usage: pnpm release:verify-beta -- <version> [--FirstNexus-npm-run ID] [--plugin-npm-run ID] [--plugin-clawhub-run ID]",
    );
  }
  ...
  for (let index = 0; index < values.length; index += 1) {
    ...
    switch (arg) {
      ...
      case "--registry":
        ...
        break;
      case "--FirstNexus-npm-run":
        parsed.workflowRuns.FirstNexusNpm = next();
        break;
      case "--plugin-npm-run":
...
export async function verifyBetaRelease(
  ...
  verifyNpmPackage("FirstNexus", args.version, args.distTag);
  lines.push(`FirstNexus npm OK: ${args.version} (${args.distTag})`);

  if (!args.skipPostpublish) {
    runCommandInherited("node", [
      "--import",
      "tsx",
      "scripts/FirstNexus-npm-postpublish-verify.ts",
      args.version,
    ]);
    lines.push("FirstNexus postpublish verifier OK");
  }
  ...
  if (args.workflowRuns.FirstNexusNpm !== undefined) {
    workflowRuns.push(
      verifyWorkflowRun({
        id: args.workflowRuns.FirstNexusNpm,
        label: "FirstNexus NPM Release",
        repo: args.repo,
        rerunFailed: false,
      }),
    );
