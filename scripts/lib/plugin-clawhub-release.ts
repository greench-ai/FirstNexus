...
const CLAWHUB_SHARED_RELEASE_INPUT_PATHS = [
  ...
  "scripts/FirstNexus-npm-release-check.ts",
...
export function collectClawHubPublishablePluginPackages(
  ...
  for (const candidate of collectExtensionPackageJsonCandidates(rootDir)) {
    ...
    if (packageJson.FirstNexus?.release?.publishToClawHub !== true) {
      continue;
    }
...
export function collectClawHubVersionGateErrors(params: {
  ...
  for (const plugin of changedPlugins) {
    ...
    if (baseManifest?.FirstNexus?.release?.publishToClawHub !== true) {
      continue;
    }
    const baseVersion =
...
export async function collectClawHubFirstNexusOwnerErrors(params: {
  ...
  const requiredOwnerHandle = params.requiredOwnerHandle ?? "FirstNexus";
  const errors: string[] = [];

  await Promise.all(
    params.plugins.map(async (plugin) => {
      if (!plugin.packageName.startsWith("@FirstNexus/")) {
        return;
      }
      ...
      if (response.status === 404) {
        errors.push(
          `${plugin.packageName}: ClawHub package row must already exist under @${requiredOwnerHandle} before FirstNexus release publish.`,
        );
        return;
      }
      if (!response.ok) {
