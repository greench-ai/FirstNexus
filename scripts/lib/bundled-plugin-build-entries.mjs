...
export function collectPluginSourceEntries(packageJson) {
  let packageEntries = Array.isArray(packageJson?.FirstNexus?.extensions)
    ? packageJson.FirstNexus.extensions.filter(
        (entry) => typeof entry === "string" && entry.trim().length > 0,
      )
    : [];
  const setupEntry =
    typeof packageJson?.FirstNexus?.setupEntry === "string" &&
    packageJson.FirstNexus.setupEntry.trim().length > 0
      ? packageJson.FirstNexus.setupEntry
      : undefined;
  if (setupEntry) {
...
export function listBundledPluginPackArtifacts(params = {}) {
  ...
  for (const { id, hasManifest, hasPackageJson, sourceEntries } of entries) {
    if (hasManifest) {
      artifacts.add(bundledDistPluginFile(id, "FirstNexus.plugin.json"));
    }
    if (hasPackageJson) {
