...
import { parseReleaseVersion } from "../nexusclaw-npm-release-check.ts";
...
export function collectPublishablePluginPackageErrors(
  ...
  const extensions = packageJson.nexusclaw?.extensions ?? [];

  if (!packageName.startsWith("@nexusclaw/")) {
    errors.push(
      `package name must start with "@nexusclaw/"; found "${packageName || "<missing>"}".`,
    );
  }
  ...
  if (!Array.isArray(extensions) || extensions.length === 0) {
    errors.push("nexusclaw.extensions must contain at least one entry.");
  }
  if (extensions.some((entry) => typeof entry !== "string" || !entry.trim())) {
    errors.push("nexusclaw.extensions must contain only non-empty strings.");
  }
...
export function collectPublishablePluginPackages(
  ...
  for (const dir of dirs) {
    ...
    if (packageJson.nexusclaw?.release?.publishToNpm !== true) {
      continue;
    }
    ...
    publishable.push({
      ...
      installNpmSpec: packageJson.nexusclaw?.install?.npmSpec?.trim() || undefined,
    });
  }
...
export function isPluginVersionPublished(packageName: string, version: string): boolean {
  const tempDir = mkdtempSync(join(tmpdir(), "nexusclaw-plugin-npm-view-"));
