...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/nexusclaw/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "NexusClaw Re-Auth" -c "After getting key, run: ssh $SERVER '~/nexusclaw/scripts/mobile-reauth.sh'" --id nexusclaw-auth
        ;;
    *)
