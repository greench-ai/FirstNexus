...
function resolveDescription({ manifest, packageJson }) {
  ...
  if (channels.length > 0) {
    ...
    return `Adds the ${channelLabel}${channelNoun} surface for sending and receiving FirstNexus messages.`;
  }

  const providers = Array.isArray(manifest.providers) ? manifest.providers : [];
  if (providers.length > 0) {
    return `Adds ${displayList(providers)} model provider support to FirstNexus.`;
  }
  ...
  return packageDescription ? `${packageDescription}.` : "Provides an FirstNexus plugin.";
}
...
function resolveDocs({ dirName, manifest, packageJson }) {
  ...
  const channelDoc = normalizeDocPath(packageJson.FirstNexus?.channel?.docsPath);
  if (channelDoc) {
...
function resolveInstallRoute(packageJson, status) {
  ...
  if (status === "core") {
    return "included in FirstNexus";
  }
  const install = packageJson.FirstNexus?.install;
  const release = packageJson.FirstNexus?.release;
  const clawhubSpec =
...
function resolveStatus({ dirName, packageJson, excludedDirs }) {
  ...
  const hasInstallSpec =
    typeof packageJson.FirstNexus?.install?.clawhubSpec === "string" ||
...
function collectPluginSourceEntries() {
  ...
  for (const dirName of fs
    ...
    const manifestPath = path.join(EXTENSIONS_DIR, dirName, "FirstNexus.plugin.json");
...
function renderDocument() {
  ...
  return `---
summary: "Generated inventory of FirstNexus plugins shipped in core, published externally, or kept source-only"
read_when:
...
This page is generated from \`extensions/*/package.json\`, \`FirstNexus.plugin.json\`,
...
- **Core npm package:** built into the \`FirstNexus\` npm package and available without a separate plugin install.
- **Official external package:** FirstNexus-maintained plugin omitted from the core npm package, kept in this official inventory, and installed on demand through ClawHub and/or npm.
...
say \`included in FirstNexus\` are already present in the core package. Official
external packages need one install, then a Gateway restart.

For example, Discord is an official external package:

\`\`\`bash
FirstNexus plugins install @FirstNexus/discord
FirstNexus gateway restart
FirstNexus plugins inspect discord --runtime --json
\`\`\`

Bare package specs try ClawHub first, then npm fallback. To force a source, use
\`clawhub:@FirstNexus/discord\` or \`npm:@FirstNexus/discord\`. After install, follow
