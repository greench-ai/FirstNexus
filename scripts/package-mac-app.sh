...
# Build and bundle NexusClaw into a minimal .app we can open.
# Outputs to dist/NexusClaw.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/NexusClaw.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="NexusClaw"
BUNDLE_ID="${BUNDLE_ID:-ai.nexusclaw.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/nexusclaw/nexusclaw/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/NexusClaw/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :NexusClawBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :NexusClawGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/NexusClaw"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/NexusClaw"
fi
chmod +x "$APP_ROOT/Contents/MacOS/NexusClaw"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/NexusClaw" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/NexusClaw/Resources/NexusClaw.icns" "$APP_ROOT/Contents/Resources/NexusClaw.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/NexusClaw/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying NexusClawKit resources"
NEXUSCLAWKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/NexusClawKit_NexusClawKit.bundle"
if [ -d "$NEXUSCLAWKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/NexusClawKit_NexusClawKit.bundle"
  cp -R "$NEXUSCLAWKIT_BUNDLE" "$APP_ROOT/Contents/Resources/NexusClawKit_NexusClawKit.bundle"
else
  echo "WARN: NexusClawKit resource bundle not found at $NEXUSCLAWKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running NexusClaw"
killall -q NexusClaw 2>/dev/null || true
