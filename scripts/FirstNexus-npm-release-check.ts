...
const EXPECTED_REPOSITORY_URL = "https://github.com/FirstNexus/FirstNexus";
const OPTIONAL_LOCAL_EMBEDDING_RUNTIME_PACKAGE = "node-llama-cpp";
const FS_SAFE_PACKAGE = "@FirstNexus/fs-safe";
...
const skipPackValidationEnv = "NEXISCLAW_NPM_RELEASE_SKIP_PACK_CHECK";
...
export function collectReleasePackageMetadataErrors(pkg: PackageJson): string[] {
  ...
  if (pkg.name !== "FirstNexus") {
    errors.push(`package.json name must be "FirstNexus"; found "${pkg.name ?? ""}".`);
  }
  ...
  if (pkg.bin?.FirstNexus !== "FirstNexus.mjs") {
    errors.push(
      `package.json bin.FirstNexus must be "FirstNexus.mjs"; found "${pkg.bin?.FirstNexus ?? ""}".`,
    );
  }
...
async function main(): Promise<number> {
  ...
  if (errors.length > 0) {
    for (const error of errors) {
      console.error(`FirstNexus-npm-release-check: ${error}`);
    }
    return 1;
  }
  ...
  console.log(
    `FirstNexus-npm-release-check: validated ${channel} release ${pkg.version} (${dayDistance} day UTC delta${skipPackValidation ? "; metadata-only" : ""}).`,
  );
  return 0;
}
