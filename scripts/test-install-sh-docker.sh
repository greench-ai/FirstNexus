...
resolve_default_smoke_platform() {
  local host_os
  local host_arch
  if [[ -n "${NEXISCLAW_INSTALL_SMOKE_PLATFORM:-}" ]]; then
    printf "%s" "$NEXISCLAW_INSTALL_SMOKE_PLATFORM"
    return
  fi
...
const parsed = JSON.parse(raw);
const budgetOverride = process.env.NEXISCLAW_INSTALL_SMOKE_PACK_UNPACKED_BUDGET_BYTES;
const budgetBytes = budgetOverride ? Number(budgetOverride) : undefined;
if (budgetOverride && !Number.isFinite(budgetBytes)) {
  throw new Error(
    `NEXISCLAW_INSTALL_SMOKE_PACK_UNPACKED_BUDGET_BYTES must be numeric, got ${JSON.stringify(
      budgetOverride,
    )}`,
  );
}
...
SKIP_UPDATE="${NEXISCLAW_INSTALL_SMOKE_SKIP_UPDATE:-0}"
SKIP_NPM_GLOBAL="${NEXISCLAW_INSTALL_SMOKE_SKIP_NPM_GLOBAL:-0}"
UPDATE_BASELINE_VERSION="${NEXISCLAW_INSTALL_SMOKE_UPDATE_BASELINE:-latest}"
UPDATE_PACKAGE_SPEC="${NEXISCLAW_INSTALL_SMOKE_UPDATE_PACKAGE_SPEC:-}"
UPDATE_DIST_IMAGE="${NEXISCLAW_INSTALL_SMOKE_UPDATE_DIST_IMAGE:-}"
UPDATE_SKIP_LOCAL_BUILD="${NEXISCLAW_INSTALL_SMOKE_UPDATE_SKIP_LOCAL_BUILD:-0}"
UPDATE_HOST_ALIAS="${NEXISCLAW_INSTALL_SMOKE_UPDATE_HOST:-host.docker.internal}"
UPDATE_PORT="${NEXISCLAW_INSTALL_SMOKE_UPDATE_PORT:-}"
UPDATE_EXPECT_VERSION="${NEXISCLAW_INSTALL_SMOKE_UPDATE_EXPECT_VERSION:-}"
LATEST_DIR="$(mktemp -d)"
...
NPM_CACHE_DIR="${NEXISCLAW_INSTALL_SMOKE_NPM_CACHE_DIR:-}"
NPM_CACHE_OWNED=0
...
if [[ "$SKIP_UPDATE" == "1" ]]; then
  echo "==> Skip update smoke (NEXISCLAW_INSTALL_SMOKE_SKIP_UPDATE=1)"
else
  prepare_update_tarball
  prepare_update_host_access
  prepare_npm_cache
  ...
  docker run --rm -t \
    ...
    -e NEXISCLAW_INSTALL_PACKAGE="$PACKAGE_NAME" \
    -e NEXISCLAW_INSTALL_METHOD=npm \
    -e NEXISCLAW_INSTALL_FRESH_VERSION="$UPDATE_EXPECT_VERSION" \
    -e NEXISCLAW_INSTALL_FRESH_TAG_URL="$FRESH_TAG_URL" \
    -e NEXISCLAW_INSTALL_LATEST_OUT="/out/latest" \
    -e NEXISCLAW_NO_ONBOARD=1 \
    -e NEXISCLAW_NO_PROMPT=1 \
  ...
  docker run --rm -t \
    ...
    "${NPM_CACHE_DOCKER_ARGS[@]}" \
    -e NEXISCLAW_INSTALL_PACKAGE="$PACKAGE_NAME" \
    -e NEXISCLAW_INSTALL_SMOKE_MODE=update \
    -e NEXISCLAW_INSTALL_UPDATE_BASELINE="$UPDATE_BASELINE_VERSION" \
    -e NEXISCLAW_INSTALL_UPDATE_BASELINE_TAG_URL="$BASELINE_TAG_URL" \
    -e NEXISCLAW_INSTALL_UPDATE_EXPECT_VERSION="$UPDATE_EXPECT_VERSION" \
    -e NEXISCLAW_INSTALL_UPDATE_TAG_URL="$UPDATE_TAG_URL" \
    -e NEXISCLAW_NO_ONBOARD=1 \
    -e NEXISCLAW_NO_PROMPT=1 \
    -e DEBIAN_FRONTEND=noninteractive \
    "$SMOKE_IMAGE"
  ...
  if [[ "$SKIP_NPM_GLOBAL" == "1" ]]; then
    echo "==> Skip direct npm global smoke (NEXISCLAW_INSTALL_SMOKE_SKIP_NPM_GLOBAL=1)"
  else
    ...
    docker run --rm -t \
      ...
      "${NPM_CACHE_DOCKER_ARGS[@]}" \
      -e NEXISCLAW_INSTALL_PACKAGE="$PACKAGE_NAME" \
      -e NEXISCLAW_INSTALL_SMOKE_MODE=npm-global \
      -e NEXISCLAW_INSTALL_UPDATE_BASELINE="$UPDATE_BASELINE_VERSION" \
      -e NEXISCLAW_INSTALL_UPDATE_BASELINE_TAG_URL="$BASELINE_TAG_URL" \
      -e NEXISCLAW_INSTALL_UPDATE_EXPECT_VERSION="$UPDATE_EXPECT_VERSION" \
      -e NEXISCLAW_INSTALL_UPDATE_TAG_URL="$UPDATE_TAG_URL" \
...
if [[ "$SKIP_NONROOT" == "1" ]]; then
  echo "==> Skip non-root installer smoke (NEXISCLAW_INSTALL_SMOKE_SKIP_NONROOT=1)"
else
  ...
  docker run --rm -t \
    ...
    -e NEXISCLAW_INSTALL_PACKAGE="$PACKAGE_NAME" \
    -e NEXISCLAW_INSTALL_METHOD=npm \
    -e NEXISCLAW_INSTALL_EXPECT_VERSION="$LATEST_VERSION" \
    -e NEXISCLAW_NO_ONBOARD=1 \
    -e NEXISCLAW_NO_PROMPT=1 \
    -e DEBIAN_FRONTEND=noninteractive \
    "$NONROOT_IMAGE"
...
if [[ "${NEXISCLAW_INSTALL_SMOKE_SKIP_CLI:-0}" == "1" ]]; then
  echo "==> Skip CLI installer smoke (NEXISCLAW_INSTALL_SMOKE_SKIP_CLI=1)"
  exit 0
fi
...
docker run --rm -t \
  ...
  --entrypoint /bin/bash \
  -e NEXISCLAW_INSTALL_URL="$INSTALL_URL" \
  -e NEXISCLAW_INSTALL_CLI_URL="$CLI_INSTALL_URL" \
  -e NEXISCLAW_NO_ONBOARD=1 \
  -e NEXISCLAW_NO_PROMPT=1 \
  -e DEBIAN_FRONTEND=noninteractive \
