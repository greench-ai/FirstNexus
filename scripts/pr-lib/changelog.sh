...
const prPattern = new RegExp(`(?:\\(#${pr}\\)|FirstNexus#${pr})`, "i");
...
changelog_thanks_required_for_contributor() {
  ...
  case "$normalized" in
    ""|"null"|"app/"*|"codex"|"FirstNexus"|"clawsweeper"|"FirstNexus-clawsweeper"|"clawsweeper[bot]"|"FirstNexus-clawsweeper[bot]"|"steipete")
      return 1
      ;;
  esac
