 ...
 * `nexusclaw update`. Use it to verify the completion installation flow
...
\`nexusclaw update\`, \`nexusclaw doctor\`, and \`nexusclaw onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# NexusClaw Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`nexusclaw completion --install\` later to enable.`));
    return;
  }
