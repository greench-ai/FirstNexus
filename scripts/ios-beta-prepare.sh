...
- regenerates apps/ios/NexusClaw.xcodeproj via xcodegen
EOF
}
...
PUSH_RELAY_BASE_URL="${NEXUSCLAW_PUSH_RELAY_BASE_URL:-${IOS_PUSH_RELAY_BASE_URL:-}}"
...
validate_push_relay_base_url() {
  ...
  if [[ "${value}" =~ [[:space:]] ]]; then
    echo "Invalid NEXUSCLAW_PUSH_RELAY_BASE_URL: whitespace is not allowed." >&2
    exit 1
  fi

  if [[ "${value}" == *'$'* || "${value}" == *'('* || "${value}" == *')'* || "${value}" == *'='* ]]; then
    echo "Invalid NEXUSCLAW_PUSH_RELAY_BASE_URL: contains forbidden xcconfig characters." >&2
    exit 1
  fi

  if [[ ! "${value}" =~ ^https://[A-Za-z0-9.-]+(:([0-9]{1,5}))?(/[A-Za-z0-9._~!&*+,;:@%/-]*)?$ ]]; then
    echo "Invalid NEXUSCLAW_PUSH_RELAY_BASE_URL: expected https://host[:port][/path]." >&2
    exit 1
  fi

  local port="${BASH_REMATCH[2]:-}"
  if [[ -n "${port}" ]] && (( 10#${port} > 65535 )); then
    echo "Invalid NEXUSCLAW_PUSH_RELAY_BASE_URL: port must be between 1 and 65535." >&2
    exit 1
  fi
}
...
if [[ -z "${PUSH_RELAY_BASE_URL}" ]]; then
  echo "Missing NEXUSCLAW_PUSH_RELAY_BASE_URL (or IOS_PUSH_RELAY_BASE_URL) for beta relay registration." >&2
  exit 1
fi
...
PUSH_RELAY_BASE_URL_XCCONFIG="$(
  printf '%s' "${PUSH_RELAY_BASE_URL}" \
    | sed 's#//#$(NEXUSCLAW_URL_SLASH)$(NEXUSCLAW_URL_SLASH)#g'
)"
...
NEXUSCLAW_CODE_SIGN_STYLE = Automatic
NEXUSCLAW_DEVELOPMENT_TEAM = ${TEAM_ID}
NEXUSCLAW_IOS_SELECTED_TEAM = ${TEAM_ID}
NEXUSCLAW_APP_BUNDLE_ID = ai.nexusclaw.client
NEXUSCLAW_SHARE_BUNDLE_ID = ai.nexusclaw.client.share
NEXUSCLAW_ACTIVITY_WIDGET_BUNDLE_ID = ai.nexusclaw.client.activitywidget
NEXUSCLAW_WATCH_APP_BUNDLE_ID = ai.nexusclaw.client.watchkitapp
NEXUSCLAW_WATCH_EXTENSION_BUNDLE_ID = ai.nexusclaw.client.watchkitapp.extension
NEXUSCLAW_APP_PROFILE =
NEXUSCLAW_SHARE_PROFILE =
NEXUSCLAW_PUSH_TRANSPORT = relay
NEXUSCLAW_PUSH_DISTRIBUTION = official
NEXUSCLAW_URL_SLASH = /
NEXUSCLAW_PUSH_RELAY_BASE_URL = ${PUSH_RELAY_BASE_URL_XCCONFIG}
NEXUSCLAW_PUSH_APNS_ENVIRONMENT = production
EOF
