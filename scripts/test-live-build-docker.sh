...
DOCKER_BUILD_EXTENSIONS="${NEXISCLAW_DOCKER_BUILD_EXTENSIONS:-${NEXISCLAW_EXTENSIONS:-}}"
...
if [[ -n "${DOCKER_BUILD_EXTENSIONS}" ]]; then
  DOCKER_BUILD_ARGS+=(--build-arg "NEXISCLAW_EXTENSIONS=${DOCKER_BUILD_EXTENSIONS}")
fi

if [[ "${NEXISCLAW_SKIP_DOCKER_BUILD:-}" == "1" ]]; then
  ...
  echo "==> Live-test image not available; building because NEXISCLAW_DOCKER_BUILD_ON_MISSING/NEXISCLAW_TESTBOX allows fallback."
fi
