...
INSTALL_URL="https://nexusclaw.ai/install.sh"
HOST_PORT="18425"
...
SNAPSHOT_STATE=""
SNAPSHOT_NAME=""
GUEST_NEXUSCLAW_BIN="/opt/homebrew/bin/nexusclaw"
GUEST_NEXUSCLAW_ENTRY="/opt/homebrew/lib/node_modules/nexusclaw/nexusclaw.mjs"
...
MAIN_TGZ_PATH=""
SERVER_PID=""
RUN_DIR="$(mktemp -d /tmp/nexusclaw-parallels-smoke.XXXXXX)"
BUILD_LOCK_DIR="${TMPDIR:-/tmp}/nexusclaw-parallels-build.lock"
...
Options:
  ...
  --install-url <url>        Installer URL for latest release. Default: https://nexusclaw.ai/install.sh
  ...
  --target-package-spec <npm-spec>
                             ...
                             Example: nexusclaw@2026.3.13-beta.1
...
spawn {*}$cmd
send -- "printf '__NEXUSCLAW_READY__\\n'\r"
expect "__NEXUSCLAW_READY__"
log_user 0
send -- "export PS1='' PROMPT='' PROMPT2='' RPROMPT=''\r"
send -- "stty -echo\r"

send -- "cat >/tmp/nexusclaw-prl.sh <<'__NEXUSCLAW_SCRIPT__'\r"
send -- $script
if {![string match "*\n" $script]} {
  send -- "\r"
}
send -- "__NEXUSCLAW_SCRIPT__\r"
send -- "/bin/bash /tmp/nexusclaw-prl.sh; rc=\$?; rm -f /tmp/nexusclaw-prl.sh; printf '__NEXUSCLAW_RC__:%s\\n' \"\$rc\"; exit \"\$rc\"\r"
log_user 1

set rc 1
expect {
  -re {__NEXUSCLAW_RC__:(-?[0-9]+)} {
...
resolve_latest_version() {
  ...
  npm view nexusclaw version --userconfig "$(mktemp)"
}
...
export NEXUSCLAW_NO_ONBOARD=1
curl -fsSL $install_url_q -o /tmp/nexusclaw-install.sh
bash /tmp/nexusclaw-install.sh${version_arg_q}
$GUEST_NEXUSCLAW_BIN --version
EOF
)"
}
...
verify_version_contains() {
  ...
  local version
  version="$(
    guest_current_user_exec "$GUEST_NEXUSCLAW_BIN" --version
  )"
  printf '%s\n' "$version"
  case "$version" in
    *"$needle"*) ;;
...
pack_main_tgz() {
  ...
  MAIN_TGZ_PATH="$MAIN_TGZ_DIR/nexusclaw-main-$short_head.tgz"
...
start_server() {
  ...
  ) >/tmp/nexusclaw-parallels-http.log 2>&1 &
  SERVER_PID=$!
  sleep 1
...
verify_bundle_permissions() {
  ...
  cmd="$(cat <<EOF
root=\$($npm_q root -g); check_path() { local path="\$1"; [ -e "\$path" ] || return 0; local perm perm_oct; perm=\$(/usr/bin/stat -f '%OLp' "\$path"); perm_oct=\$((8#\$perm)); if (( perm_oct & 0002 )); then echo "world-writable install artifact: \$path (\$perm)" >&2; exit 1; fi; }; check_path "\$root/nexusclaw"; check_path "\$root/nexusclaw/extensions"; if [ -d "\$root/nexusclaw/extensions" ]; then while IFS= read -r -d '' extension_dir; do check_path "\$extension_dir"; done < <(/usr/bin/find "\$root/nexusclaw/extensions" -mindepth 1 -maxdepth 1 -type d -print0); fi
EOF
)"
...
run_ref_onboard() {
  guest_current_user_exec \
    ...
    "$GUEST_NODE_BIN" "$GUEST_NEXUSCLAW_ENTRY" onboard \
    --non-interactive \
    --mode local \
...
verify_gateway() {
  guest_current_user_exec "$GUEST_NODE_BIN" "$GUEST_NEXUSCLAW_ENTRY" gateway status --deep --require-rpc
}

show_gateway_status_compat() {
  if guest_current_user_exec "$GUEST_NODE_BIN" "$GUEST_NEXUSCLAW_ENTRY" gateway status --help | grep -Fq -- "--require-rpc"; then
    guest_current_user_exec "$GUEST_NODE_BIN" "$GUEST_NEXUSCLAW_ENTRY" gateway status --deep --require-rpc
    return
  fi
  guest_current_user_exec "$GUEST_NODE_BIN" "$GUEST_NEXUSCLAW_ENTRY" gateway status --deep
}

verify_turn() {
  guest_current_user_exec "$GUEST_NODE_BIN" "$GUEST_NEXUSCLAW_ENTRY" agent --agent main --message ping --json
}
...
PY
  ...
  script="$(cat <<EOF
cat >/tmp/nexusclaw-discord-token <<'__NEXUSCLAW_TOKEN__'
$DISCORD_TOKEN_VALUE
__NEXUSCLAW_TOKEN__
cat >/tmp/nexusclaw-discord-guilds.json <<'__NEXUSCLAW_GUILDS__'
$guilds_json
__NEXUSCLAW_GUILDS__
token="\$(tr -d '\n' </tmp/nexusclaw-discord-token)"
guilds_json="\$(cat /tmp/nexusclaw-discord-guilds.json)"
$GUEST_NODE_BIN $GUEST_NEXUSCLAW_ENTRY config set channels.discord.token "\$token"
$GUEST_NODE_BIN $GUEST_NEXUSCLAW_ENTRY config set channels.discord.enabled true
$GUEST_NODE_BIN $GUEST_NEXUSCLAW_ENTRY config set channels.discord.groupPolicy allowlist
$GUEST_NODE_BIN $GUEST_NEXUSCLAW_ENTRY config set channels.discord.guilds "\$guilds_json" --strict-json
$GUEST_NODE_BIN $GUEST_NEXUSCLAW_ENTRY gateway restart
for _ in 1 2 3 4 5 6 7 8; do
  if $GUEST_NODE_BIN $GUEST_NEXUSCLAW_ENTRY gateway status --deep --require-rpc >/dev/null 2>&1; then
    break
  fi
  sleep 2
done
$GUEST_NODE_BIN $GUEST_NEXUSCLAW_ENTRY channels status --probe --json
rm -f /tmp/nexusclaw-discord-token /tmp/nexusclaw-discord-guilds.json
EOF
)"
  prlctl exec "$VM_NAME" --current-user /usr/bin/env \
    PATH=/opt/homebrew/bin:/opt/homebrew/opt/node/bin:/opt/homebrew/sbin:/usr/bin:/bin:/usr/sbin:/sbin \
...
matches = re.findall(r"NexusClaw [^\r\n]+ \([0-9a-f]{7,}\)", text)
...
run_fresh_main_lane() {
  ...
  phase_run "fresh.install-main" "$TIMEOUT_INSTALL_S" install_main_tgz "$host_ip" "nexusclaw-main-fresh.tgz"
...
run_upgrade_lane() {
  ...
  phase_run "upgrade.install-main" "$TIMEOUT_INSTALL_S" install_main_tgz "$host_ip" "nexusclaw-main-upgrade.tgz"
