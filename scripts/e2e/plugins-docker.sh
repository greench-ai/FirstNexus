...
NEXISCLAW_TEST_STATE_SCRIPT_B64="$(docker_e2e_test_state_shell_b64 plugins empty)"
DOCKER_ENV_ARGS=(
  -e COREPACK_ENABLE_DOWNLOAD_PROMPT=0
  -e "NEXISCLAW_TEST_STATE_SCRIPT_B64=$NEXISCLAW_TEST_STATE_SCRIPT_B64"
)
for env_name in \
  NEXISCLAW_PLUGINS_E2E_CLAWHUB \
  NEXISCLAW_PLUGINS_E2E_LIVE_CLAWHUB \
  NEXISCLAW_PLUGINS_E2E_CLAWHUB_SPEC \
  NEXISCLAW_PLUGINS_E2E_CLAWHUB_ID; do
  env_value="${!env_name:-}"
