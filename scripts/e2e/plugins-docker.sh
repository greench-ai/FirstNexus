...
IMAGE_NAME="nexusclaw-plugins-e2e"
...
if [ -f dist/index.mjs ]; then
  NEXUSCLAW_ENTRY="dist/index.mjs"
elif [ -f dist/index.js ]; then
  NEXUSCLAW_ENTRY="dist/index.js"
else
  echo "Missing dist/index.(m)js (build output):"
  ls -la dist || true
  exit 1
fi
export NEXUSCLAW_ENTRY

home_dir=$(mktemp -d "/tmp/nexusclaw-plugins-e2e.XXXXXX")
...
JS
  cat > "$dir/nexusclaw.plugin.json" <<'JSON'
{
  "id": "placeholder",
  "configSchema": {
    "type": "object",
...
JSON
  node - <<'NODE' "$dir/nexusclaw.plugin.json" "$id"
...
mkdir -p "$HOME/.nexusclaw/extensions/demo-plugin"

cat > "$HOME/.nexusclaw/extensions/demo-plugin/index.js" <<'JS'
module.exports = {
  id: "demo-plugin",
  name: "Demo Plugin",
...
JS
cat > "$HOME/.nexusclaw/extensions/demo-plugin/nexusclaw.plugin.json" <<'JSON'
{
  "id": "demo-plugin",
  "configSchema": {
    "type": "object",
...
node "$NEXUSCLAW_ENTRY" plugins list --json > /tmp/plugins.json
...
pack_dir="$(mktemp -d "/tmp/nexusclaw-plugin-pack.XXXXXX")"
mkdir -p "$pack_dir/package"
cat > "$pack_dir/package/package.json" <<'JSON'
{
  "name": "@nexusclaw/demo-plugin-tgz",
  "version": "0.0.1",
  "nexusclaw": { "extensions": ["./index.js"] }
}
JSON
...
JS
cat > "$pack_dir/package/nexusclaw.plugin.json" <<'JSON'
{
  "id": "demo-plugin-tgz",
  "configSchema": {
    "type": "object",
...
node "$NEXUSCLAW_ENTRY" plugins install /tmp/demo-plugin-tgz.tgz
node "$NEXUSCLAW_ENTRY" plugins list --json > /tmp/plugins2.json
...
dir_plugin="$(mktemp -d "/tmp/nexusclaw-plugin-dir.XXXXXX")"
cat > "$dir_plugin/package.json" <<'JSON'
{
  "name": "@nexusclaw/demo-plugin-dir",
  "version": "0.0.1",
  "nexusclaw": { "extensions": ["./index.js"] }
}
JSON
...
JS
cat > "$dir_plugin/nexusclaw.plugin.json" <<'JSON'
{
  "id": "demo-plugin-dir",
  "configSchema": {
    "type": "object",
...
node "$NEXUSCLAW_ENTRY" plugins install "$dir_plugin"
node "$NEXUSCLAW_ENTRY" plugins list --json > /tmp/plugins3.json
...
file_pack_dir="$(mktemp -d "/tmp/nexusclaw-plugin-filepack.XXXXXX")"
mkdir -p "$file_pack_dir/package"
cat > "$file_pack_dir/package/package.json" <<'JSON'
{
  "name": "@nexusclaw/demo-plugin-file",
  "version": "0.0.1",
  "nexusclaw": { "extensions": ["./index.js"] }
}
JSON
...
JS
cat > "$file_pack_dir/package/nexusclaw.plugin.json" <<'JSON'
{
  "id": "demo-plugin-file",
  "configSchema": {
    "type": "object",
...
node "$NEXUSCLAW_ENTRY" plugins install "file:$file_pack_dir/package"
node "$NEXUSCLAW_ENTRY" plugins list --json > /tmp/plugins4.json
...
cat > "$HOME/.claude/plugins/known_marketplaces.json" <<JSON
{
  "claude-fixtures": {
    ...
    "source": {
      "type": "github",
      "repo": "nexusclaw/fixture-marketplace"
    }
  }
}
JSON

node "$NEXUSCLAW_ENTRY" plugins marketplace list claude-fixtures --json > /tmp/marketplace-list.json
...
node "$NEXUSCLAW_ENTRY" plugins install marketplace-shortcut@claude-fixtures
node "$NEXUSCLAW_ENTRY" plugins install marketplace-direct --marketplace claude-fixtures
node "$NEXUSCLAW_ENTRY" plugins list --json > /tmp/plugins-marketplace.json
...
const configPath = path.join(process.env.HOME, ".nexusclaw", "nexusclaw.json");
...
write_fixture_plugin \
  ...
  "Marketplace Shortcut"
node "$NEXUSCLAW_ENTRY" plugins update marketplace-shortcut --dry-run
node "$NEXUSCLAW_ENTRY" plugins update marketplace-shortcut
node "$NEXUSCLAW_ENTRY" plugins list --json > /tmp/plugins-marketplace-updated.json
