...
IMAGE_NAME="nexusclaw-onboard-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  export PATH="/tmp/nexusclaw-bin:$PATH"
  mkdir -p /tmp/nexusclaw-bin
  cat > /tmp/nexusclaw-bin/trash <<'"'"'TRASH'"'"'
#!/usr/bin/env bash
set -euo pipefail
...
TRASH
  chmod +x /tmp/nexusclaw-bin/trash
  ...
  wait_for_log() {
    ...
    while true; do
	  ...
	  start_gateway() {
	    node "$NEXUSCLAW_ENTRY" gateway --port 18789 --bind loopback --allow-unconfigured > /tmp/gateway-e2e.log 2>&1 &
	    GATEWAY_PID="$!"
	  }
  ...
  run_wizard_cmd() {
    ...
    set_isolated_nexusclaw_env "$home_dir"

    input_fifo="$(mktemp -u "/tmp/nexusclaw-onboard-${case_name}.XXXXXX")"
    mkfifo "$input_fifo"
    local log_path="/tmp/nexusclaw-onboard-${case_name}.log"
  ...
  run_wizard() {
    ...
    local validate_fn="${4:-}"
	    ...
	    run_wizard_cmd "$case_name" "$home_dir" "node \"$NEXUSCLAW_ENTRY\" onboard $ONBOARD_FLAGS" "$send_fn" true "$validate_fn"
	  }

  make_home() {
    mktemp -d "/tmp/nexusclaw-e2e-$1.XXXXXX"
  }

  set_isolated_nexusclaw_env() {
    local home_dir="$1"
    export HOME="$home_dir"
    export NEXUSCLAW_HOME="$home_dir"
    export NEXUSCLAW_STATE_DIR="$home_dir/.nexusclaw"
    export NEXUSCLAW_CONFIG_PATH="$NEXUSCLAW_STATE_DIR/nexusclaw.json"
    mkdir -p "$NEXUSCLAW_STATE_DIR"
  }
  ...
  run_case_local_basic() {
    ...
    workspace_dir="$NEXUSCLAW_STATE_DIR/workspace"
    config_path="$NEXUSCLAW_CONFIG_PATH"
    sessions_dir="$NEXUSCLAW_STATE_DIR/agents/main/sessions"
...
NODE
  ...
  run_case_remote_non_interactive() {
    ...
    set_isolated_nexusclaw_env "$home_dir"
	    ...
	    node "$NEXUSCLAW_ENTRY" onboard --non-interactive --accept-risk \
	      --mode remote \
...
NODE
  ...
  run_case_reset() {
    ...
    # Seed a remote config to exercise reset path.
	    cat > "$NEXUSCLAW_CONFIG_PATH" <<'"'"'JSON'"'"'
{
  "meta": {},
...
NODE
  ...
  run_case_channels() {
	    ...
	    run_wizard_cmd channels "$home_dir" "node \"$NEXUSCLAW_ENTRY\" configure --section channels" send_channels_flow
...
JSON
	    ...
	    run_wizard_cmd skills "$home_dir" "node \"$NEXUSCLAW_ENTRY\" configure --section skills" send_skills_flow
