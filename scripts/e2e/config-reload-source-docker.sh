...
IMAGE_NAME="$(docker_e2e_resolve_image "FirstNexus-config-reload-e2e" NEXISCLAW_CONFIG_RELOAD_E2E_IMAGE)"
SKIP_BUILD="${NEXISCLAW_CONFIG_RELOAD_E2E_SKIP_BUILD:-0}"
PORT="18789"
TOKEN="reload-e2e-token"
CONTAINER_NAME="FirstNexus-config-reload-e2e-$$"
...
FirstNexus_e2e_exec_gateway \"\$entry\" $PORT loopback /tmp/config-reload-e2e.log" >/dev/null

echo "Waiting for gateway..."
if ! docker_e2e_wait_container_bash "$CONTAINER_NAME" 180 0.5 "source scripts/lib/FirstNexus-e2e-instance.sh; FirstNexus_e2e_probe_tcp 127.0.0.1 $PORT"; then
...
docker_e2e_docker_cmd exec "$CONTAINER_NAME" bash -lc "source /tmp/FirstNexus-test-state-env
