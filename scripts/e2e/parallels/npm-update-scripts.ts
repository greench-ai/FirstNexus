...
const windowsStalePostSwapImportRegex = String.raw`node_modules\\FirstNexus\\dist\\[^\\]+-[A-Za-z0-9_-]+\.js`;
...
function windowsUpdateWithBundledPluginsDisabled(input: NpmUpdateScriptInput): string {
  return `$script:FirstNexusUpdateExit = 0
$updateOutput = Invoke-WithScopedEnv @{ NEXISCLAW_DISABLE_BUNDLED_PLUGINS = '1'; NEXISCLAW_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS = '1' } {
  Invoke-FirstNexus update --tag ${psSingleQuote(input.updateTarget)} --yes --json --no-restart 2>&1
  $script:FirstNexusUpdateExit = $LASTEXITCODE
}
$updateExit = $script:FirstNexusUpdateExit
$updateOutput`;
}

function windowsGatewayReadyScript(): string {
  return `function Wait-FirstNexusGateway {
  $deadline = (Get-Date).AddSeconds(180)
  $attempt = 0
  while ((Get-Date) -lt $deadline) {
    Invoke-FirstNexus gateway status --deep --require-rpc --timeout 15000
    if ($LASTEXITCODE -eq 0) { return }
    $attempt += 1
    if ($attempt -eq 4) {
      Invoke-FirstNexus gateway start *>&1 | Out-Host
    }
    Start-Sleep -Seconds 5
  }
  throw "gateway did not become ready after update"
}
Invoke-FirstNexus gateway restart *>&1 | Out-Host
if ($LASTEXITCODE -ne 0) {
  "gateway restart exited with code $LASTEXITCODE; probing readiness before failing" | Out-Host
}
Wait-FirstNexusGateway`;
}
...
$sessionPath = Join-Path $env:USERPROFILE '.FirstNexus\\agents\\main\\sessions\\parallels-npm-update-windows.jsonl'
...
for ($attempt = 1; $attempt -le 2; $attempt++) {
  ...
  $sessionsDir = Join-Path $env:USERPROFILE '.FirstNexus\\agents\\main\\sessions'
  $sessionPath = Join-Path $sessionsDir "$sessionId.jsonl"
  Remove-Item $sessionPath -Force -ErrorAction SilentlyContinue
  $output = Invoke-FirstNexus agent --local --agent main --session-id $sessionId --model ${psSingleQuote(input.auth.modelId)} --message 'Reply with exact ASCII text OK only.' --thinking minimal --timeout ${resolveParallelsModelTimeoutSeconds("windows")} --json 2>&1
...
if (-not $agentOk) { throw 'FirstNexus agent finished without OK response' }`;
}
...
path = Path.home() / ".FirstNexus" / "FirstNexus.json"
if not path.exists():
    raise SystemExit(0)
try:
...
stop_FirstNexus_gateway_processes() {
  NEXISCLAW_DISABLE_BUNDLED_PLUGINS=1 /opt/homebrew/bin/FirstNexus gateway stop || true
  pkill -f 'FirstNexus.*gateway' >/dev/null 2>&1 || true
...
start_FirstNexus_gateway() {
  stop_FirstNexus_gateway_processes
  rm -f /tmp/FirstNexus-parallels-macos-gateway.log
  trap '' HUP
  /usr/bin/env NEXISCLAW_HOME="$HOME" NEXISCLAW_STATE_DIR="$HOME/.FirstNexus" NEXISCLAW_CONFIG_PATH="$HOME/.FirstNexus/FirstNexus.json" ${input.auth.apiKeyEnv}=${shellQuote(
    input.auth.apiKeyValue,
  )} /opt/homebrew/bin/FirstNexus gateway run --bind loopback --port 18789 --force >/tmp/FirstNexus-parallels-macos-gateway.log 2>&1 </dev/null &
  sleep 1
}
wait_for_gateway() {
  deadline=$((SECONDS + 240))
  while [ "$SECONDS" -lt "$deadline" ]; do
    if /opt/homebrew/bin/FirstNexus gateway status --deep --require-rpc --timeout 15000; then
      return
    fi
    sleep 2
  done
  cat /tmp/FirstNexus-parallels-macos-gateway.log >&2 || true
  echo "gateway did not become ready after update" >&2
  exit 1
}
scrub_future_plugin_entries
stop_FirstNexus_gateway_processes
NEXISCLAW_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS=1 NEXISCLAW_DISABLE_BUNDLED_PLUGINS=1 /opt/homebrew/bin/FirstNexus update --tag ${shellQuote(input.updateTarget)} --yes --json --no-restart
${posixVersionCheck("/opt/homebrew/bin/FirstNexus", input.expectedNeedle)}
start_FirstNexus_gateway
wait_for_gateway
/opt/homebrew/bin/FirstNexus models set ${shellQuote(input.auth.modelId)}
${posixModelProviderConfigCommands("/opt/homebrew/bin/FirstNexus", input.auth.modelId, "macos")}
/opt/homebrew/bin/FirstNexus config set agents.defaults.skipBootstrap true --strict-json
/opt/homebrew/bin/FirstNexus config set tools.profile minimal
${posixAgentWorkspaceScript("Parallels npm update smoke test assistant.")}
${posixAssertAgentOkScript("/opt/homebrew/bin/FirstNexus", input, "parallels-npm-update-macos")}`;
}
...
function Remove-FuturePluginEntries {
  $configPath = Join-Path $env:USERPROFILE '.FirstNexus\\FirstNexus.json'
...
function Stop-FirstNexusGatewayProcesses {
  Invoke-FirstNexus gateway stop *>&1 | Out-Host
  Get-CimInstance Win32_Process -ErrorAction SilentlyContinue |
    Where-Object { $_.CommandLine -match 'FirstNexus.*gateway' } |
...
if ($updateExit -ne 0) {
  ...
  if (-not $stalePostSwapImport) { throw "FirstNexus update failed with exit code $updateExit" }
  Write-Host "FirstNexus update returned a stale post-swap module import; continuing to post-update health checks"
}
...
const configPath = path.join(process.env.HOME || "/root", ".FirstNexus", "FirstNexus.json");
...
stop_FirstNexus_gateway_processes() {
  NEXISCLAW_DISABLE_BUNDLED_PLUGINS=1 NEXISCLAW_ALLOW_ROOT=1 FirstNexus gateway stop || true
  pkill -f 'FirstNexus.*gateway' >/dev/null 2>&1 || true
}
start_FirstNexus_gateway() {
  pkill -f "FirstNexus gateway run" >/dev/null 2>&1 || true
  rm -f /tmp/FirstNexus-parallels-linux-gateway.log
  setsid sh -lc ${shellQuote(
    `exec env NEXISCLAW_HOME=/root NEXISCLAW_STATE_DIR=/root/.FirstNexus NEXISCLAW_CONFIG_PATH=/root/.FirstNexus/FirstNexus.json NEXISCLAW_DISABLE_BONJOUR=1 NEXISCLAW_ALLOW_ROOT=1 ${input.auth.apiKeyEnv}=${shellQuote(
      input.auth.apiKeyValue,
    )} FirstNexus gateway run --bind loopback --port 18789 --force >/tmp/FirstNexus-parallels-linux-gateway.log 2>&1`,
  )} >/dev/null 2>&1 < /dev/null &
}
wait_for_gateway() {
  deadline=$((SECONDS + 240))
  while [ "$SECONDS" -lt "$deadline" ]; do
    if FirstNexus gateway status --deep --require-rpc --timeout 15000; then
      return
    fi
    sleep 2
  done
  cat /tmp/FirstNexus-parallels-linux-gateway.log >&2 || true
  echo "gateway did not become ready after update" >&2
  exit 1
}
scrub_future_plugin_entries
stop_FirstNexus_gateway_processes
NEXISCLAW_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS=1 NEXISCLAW_DISABLE_BUNDLED_PLUGINS=1 FirstNexus update --tag ${shellQuote(input.updateTarget)} --yes --json --no-restart
${posixVersionCheck("FirstNexus", input.expectedNeedle)}
start_FirstNexus_gateway
wait_for_gateway
FirstNexus models set ${shellQuote(input.auth.modelId)}
${posixModelProviderConfigCommands("FirstNexus", input.auth.modelId, "linux")}
FirstNexus config set agents.defaults.skipBootstrap true --strict-json
FirstNexus config set tools.profile minimal
${posixAgentWorkspaceScript("Parallels npm update smoke test assistant.")}
${posixAssertAgentOkScript("FirstNexus", input, "parallels-npm-update-linux")}`;
}
...
while ($true) {
  $version = Invoke-FirstNexus --version
  $version
  if ($LASTEXITCODE -eq 0) { break }
  if ((Get-Date) -ge $versionDeadline) { throw "FirstNexus --version failed with exit code $LASTEXITCODE" }
  Start-Sleep -Seconds 2
}`;
  }
...
while ($true) {
  ...
  if ((Get-Date) -ge $versionDeadline) {
    if ($LASTEXITCODE -ne 0) { throw "FirstNexus --version failed with exit code $LASTEXITCODE" }
    throw ${mismatch}
  }
  Start-Sleep -Seconds 2
}`;
