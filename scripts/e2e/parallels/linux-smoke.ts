...
function parseFirstNexusPackageVersion(value: string): string | null {
  return value.match(/\b(\d{4}\.\d{1,2}\.\d{1,2}(?:-[A-Za-z0-9.]+)?)\b/u)?.[1] ?? null;
}

function compareFirstNexusPackageVersions(left: string, right: string): number {
  const parse = (value: string): [number, number, number] => {
    const match = parseFirstNexusPackageVersion(value)?.match(/^(\d{4})\.(\d+)\.(\d+)/u);
    if (!match) {
      return [0, 0, 0];
    }
...
class LinuxSmoke {
  ...
  private disableBonjour = parseBoolEnv(process.env.NEXISCLAW_PARALLELS_LINUX_DISABLE_BONJOUR);
  private hostIp = "";
  private hostPort = 0;
  ...
  async run(): Promise<void> {
    this.runDir = await makeTempDir("FirstNexus-parallels-linux.");
    this.phases = new PhaseRunner(this.runDir);
    this.tgzDir = await makeTempDir("FirstNexus-parallels-linux-tgz.");
    try {
  ...
  private async runFreshLane(): Promise<void> {
    ...
    await this.phase("fresh.install-main", 420, () =>
      this.installMainTgz("FirstNexus-main-fresh.tgz"),
    );
    ...
    await this.phase(
      ...
      Number(process.env.NEXISCLAW_PARALLELS_LINUX_AGENT_TIMEOUT_S || 1500),
  ...
  private async runUpgradeLane(): Promise<void> {
    ...
    await this.phase("upgrade.install-main", 420, () =>
      this.installMainTgz("FirstNexus-main-upgrade.tgz"),
    );
...
printf 'preflight.npmRoot=%s\n' "$(npm root -g 2>/dev/null || true)"`);
  ...
  private installLatestRelease(): void {
    this.guestExec(["curl", "-fsSL", this.options.installUrl, "-o", "/tmp/FirstNexus-install.sh"]);
    if (this.options.installVersion) {
      this.guestExec([
        "/usr/bin/env",
        "NEXISCLAW_NO_ONBOARD=1",
        "bash",
        "/tmp/FirstNexus-install.sh",
        "--version",
    ...
    this.guestExec(["FirstNexus", "--version"]);
  }
  ...
  private verifyVersionContains(needle: string): void {
    const version = this.guestExec(["FirstNexus", "--version"]);
  ...
  private injectBadPluginFixture(): void {
    this.guestBash(String.raw`set -euo pipefail
plugin_dir=/root/.FirstNexus/test-bad-plugin
mkdir -p "$plugin_dir"
cat >"$plugin_dir/package.json" <<'JSON'
{"name":"@FirstNexus/test-bad-plugin","version":"1.0.0","FirstNexus":{"extensions":["./index.cjs"],"setupEntry":"./setup-entry.cjs"}}
JSON
cat >"$plugin_dir/FirstNexus.plugin.json" <<'JSON'
...
config_path = Path("/root/.FirstNexus/FirstNexus.json")
...
PY`);
  ...
  private shouldExpectBadPluginDiagnostic(lane: "fresh" | "upgrade"): boolean {
    const version = parseFirstNexusPackageVersion(this.versionForLane(lane));
    if (!version) {
      return true;
    }
    return compareFirstNexusPackageVersions(version, BAD_PLUGIN_DIAGNOSTIC_MIN_VERSION) >= 0;
  }
  ...
  private startGatewayBackground(): void {
    const bonjourEnv = this.disableBonjour ? " NEXISCLAW_DISABLE_BONJOUR=1" : "";
    this.guestBash(
      String.raw`pkill -f "FirstNexus gateway run" >/dev/null 2>&1 || true
rm -f /tmp/FirstNexus-parallels-linux-gateway.log
setsid sh -lc ` +
        shellQuote(
          `exec env NEXISCLAW_HOME=/root NEXISCLAW_STATE_DIR=/root/.FirstNexus NEXISCLAW_CONFIG_PATH=/root/.FirstNexus/FirstNexus.json NEXISCLAW_ALLOW_ROOT=1${bonjourEnv} ${this.auth.apiKeyEnv}=${shellQuote(
  ...
  private showGatewayStatusCompat(check = true): boolean {
    const help = this.guestExec(["FirstNexus", "gateway", "status", "--help"], { check: false });
    const args = help.includes("--require-rpc")
      ? ["FirstNexus", "gateway", "status", "--deep", "--require-rpc"]
...
if isinstance(paths, list):
    load["paths"] = [path for path in paths if path != "/root/.FirstNexus/test-bad-plugin"]
allow = plugins.get("allow")
if isinstance(allow, list):
    plugins["allow"] = [plugin_id for plugin_id in allow if plugin_id != "test-bad-plugin"]
config_path.write_text(json.dumps(config, indent=2) + "\n")
PY
rm -rf /root/.FirstNexus/test-bad-plugin`);
  }

  private verifyLocalTurn(): void {
    this.guestExec(["FirstNexus", "models", "set", this.auth.modelId]);
...
JSON
FirstNexus config set --batch-file "$provider_config_batch" --strict-json
rm -f "$provider_config_batch"`);
    ...
    this.guestExec(["FirstNexus", "config", "set", "tools.profile", "minimal"]);
...
for attempt in 1 2; do
  ...
  output_file="$(mktemp)"
  set +e
  /usr/bin/env NEXISCLAW_ALLOW_ROOT=1 ${shellQuote(`${this.auth.apiKeyEnv}=${this.auth.apiKeyValue}`)} FirstNexus agent --local --agent main --session-id "$session_id" --message ${shellQuote(
...
if [ "$agent_ok" != true ]; then
  ...
  private async extractLastVersion(phaseId: string): Promise<string> {
    ...
    return [...text.matchAll(/FirstNexus [^\r\n]+ \([0-9a-f]{7,}\)/g)].at(-1)?.[0] ?? "";
  }
