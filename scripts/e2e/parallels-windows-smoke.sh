...
VM_NAME="Windows 11"
SNAPSHOT_HINT="pre-nexusclaw-native-e2e-2026-03-12"
MODE="both"
OPENAI_API_KEY_ENV="OPENAI_API_KEY"
INSTALL_URL="https://nexusclaw.ai/install.ps1"
HOST_PORT="18426"
...
MINGIT_ZIP_NAME=""
SERVER_PID=""
RUN_DIR="$(mktemp -d /tmp/nexusclaw-parallels-windows.XXXXXX)"
...
Options:
  ...
  --snapshot-hint <name>     Snapshot name substring/fuzzy match.
                             Default: "pre-nexusclaw-native-e2e-2026-03-12"
  --mode <fresh|upgrade|both>
  --openai-api-key-env <var> Host env var name for OpenAI API key.
                             Default: OPENAI_API_KEY
  --install-url <url>        Installer URL for latest release. Default: https://nexusclaw.ai/install.ps1
...
guest_run_nexusclaw() {
  ...
  stdout_name="nexusclaw-stdout-$RANDOM-$RANDOM.log"
  stderr_name="nexusclaw-stderr-$RANDOM-$RANDOM.log"
...
try {
  ...
  \$proc = Start-Process -FilePath (Join-Path \$env:APPDATA 'npm\nexusclaw.cmd') -ArgumentList $args_literal -NoNewWindow -PassThru -RedirectStandardOutput \$stdout -RedirectStandardError \$stderr
  \$proc.WaitForExit()
  if (Test-Path \$stdout) {
    Get-Content \$stdout
  }
...
req = urllib.request.Request(
    ...
    headers={
        "User-Agent": "nexusclaw-parallels-smoke",
...
ensure_guest_git() {
  ...
  guest_exec cmd.exe /d /s /c "if exist \"%LOCALAPPDATA%\\NexusClaw\\deps\\portable-git\" rmdir /s /q \"%LOCALAPPDATA%\\NexusClaw\\deps\\portable-git\""
  guest_exec cmd.exe /d /s /c "if not exist \"%LOCALAPPDATA%\\NexusClaw\\deps\" mkdir \"%LOCALAPPDATA%\\NexusClaw\\deps\""
  guest_exec cmd.exe /d /s /c "mkdir \"%LOCALAPPDATA%\\NexusClaw\\deps\\portable-git\""
  guest_exec cmd.exe /d /s /c "curl.exe -fsSL \"$mingit_url\" -o \"%TEMP%\\$MINGIT_ZIP_NAME\""
  guest_exec cmd.exe /d /s /c "tar.exe -xf \"%TEMP%\\$MINGIT_ZIP_NAME\" -C \"%LOCALAPPDATA%\\NexusClaw\\deps\\portable-git\""
  guest_exec cmd.exe /d /s /c "del /q \"%TEMP%\\$MINGIT_ZIP_NAME\" & set \"PATH=%LOCALAPPDATA%\\NexusClaw\\deps\\portable-git\\cmd;%LOCALAPPDATA%\\NexusClaw\\deps\\portable-git\\mingw64\\bin;%LOCALAPPDATA%\\NexusClaw\\deps\\portable-git\\usr\\bin;%PATH%\" && git.exe --version"
}
...
start_server() {
  ...
  while :; do
    ...
    ) >/tmp/nexusclaw-parallels-windows-http.log 2>&1 &
    SERVER_PID=$!
    sleep 1
...
install_main_tgz() {
  ...
  guest_exec cmd.exe /d /s /c "set \"PATH=%LOCALAPPDATA%\\NexusClaw\\deps\\portable-git\\cmd;%LOCALAPPDATA%\\NexusClaw\\deps\\portable-git\\mingw64\\bin;%LOCALAPPDATA%\\NexusClaw\\deps\\portable-git\\usr\\bin;%PATH%\" && curl.exe -fsSL \"$tgz_url\" -o \"%TEMP%\\$temp_name\" && npm.cmd install -g \"%TEMP%\\$temp_name\" --no-fund --no-audit && \"%APPDATA%\\npm\\nexusclaw.cmd\" --version"
}

verify_version_contains() {
  local needle="$1"
  local version
  version="$(guest_run_nexusclaw "" "" "--version")"
  printf '%s\n' "$version"
  case "$version" in
    *"$needle"*) ;;
    *)
...
run_ref_onboard() {
  ...
  runner_name="nexusclaw-onboard-$RANDOM-$RANDOM.ps1"
  log_name="nexusclaw-onboard-$RANDOM-$RANDOM.log"
  done_name="nexusclaw-onboard-$RANDOM-$RANDOM.done"
...
try {
  \$nexusclaw = Join-Path \$env:APPDATA 'npm\nexusclaw.cmd'
  \$cmdLine = ('"{0}" onboard --non-interactive --mode local --auth-choice openai-api-key --secret-input-mode ref --gateway-port 18789 --gateway-bind loopback --install-daemon --skip-skills --accept-risk --json > "{1}" 2>&1' -f \$nexusclaw, \$log)
...
verify_gateway() {
  guest_run_nexusclaw "" "" gateway status --deep --require-rpc
}

show_gateway_status_compat() {
  if guest_run_nexusclaw "" "" gateway status --help | grep -Fq -- "--require-rpc"; then
    guest_run_nexusclaw "" "" gateway status --deep --require-rpc
    return
  fi
  guest_run_nexusclaw "" "" gateway status --deep
}

verify_turn() {
  guest_run_nexusclaw "" "" agent --agent main --message "Reply with exact ASCII text OK only." --json
}
...
run_fresh_main_lane() {
  ...
  phase_run "fresh.install-main" "$TIMEOUT_INSTALL_S" install_main_tgz "$host_ip" "nexusclaw-main-fresh.tgz" || return $?
...
run_upgrade_lane() {
  ...
  phase_run "upgrade.install-main" "$TIMEOUT_INSTALL_S" install_main_tgz "$host_ip" "nexusclaw-main-upgrade.tgz" || return $?
