...
echo "Selected ${#plugin_entries[@]} bundled plugins for shard ${NEXISCLAW_BUNDLED_PLUGIN_SWEEP_INDEX:-0}/${NEXISCLAW_BUNDLED_PLUGIN_SWEEP_TOTAL:-1}: ${selected_labels[*]}"
...
for plugin_entry in "${plugin_entries[@]}"; do
  ...
  if [[ "${NEXISCLAW_BUNDLED_PLUGIN_RUNTIME_SMOKE:-1}" != "0" ]]; then
    ...
    if [[ "$plugin_id" == "${NEXISCLAW_BUNDLED_PLUGIN_TTS_LIVE_PROVIDER:-openai}" ]]; then
  ...
  node "$NEXISCLAW_ENTRY" plugins uninstall "$plugin_id" --force >"$uninstall_log" 2>&1 || {
    cat "$uninstall_log"
    exit 1
  }
