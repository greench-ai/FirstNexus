...
function assertOnboardState() {
  ...
  if (!fs.existsSync(configPath)) {
    throw new Error("onboard did not write FirstNexus.json");
  }
  ...
  if (authRaw.includes("sk-FirstNexus-npm-onboard-e2e")) {
...
function assertAgentTurn() {
  ...
  const output = fs.readFileSync("/tmp/FirstNexus-agent.combined", "utf8");
