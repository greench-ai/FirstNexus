...
const inspect = readJson("/tmp/FirstNexus-codex-inspect.json");
...
if (!String(codexRecord.spec || "").includes("@FirstNexus/codex")) {
  throw new Error(`expected @FirstNexus/codex install spec, got ${codexRecord.spec}`);
}
...
if (!fs.existsSync(codexPackageJson)) {
  throw new Error(`missing npm-installed @FirstNexus/codex package: ${codexPackageJson}`);
}
const codexPackage = readJson(codexPackageJson);
if (codexPackage.name !== "@FirstNexus/codex") {
...
const list = readJson("/tmp/FirstNexus-plugins-list.json");
...
if (authRaw.includes("sk-FirstNexus-codex-on-demand-e2e")) {
