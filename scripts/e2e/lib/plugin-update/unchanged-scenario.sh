...
FirstNexus_e2e_install_package /tmp/FirstNexus-install.log "mounted FirstNexus package" /tmp/npm-prefix
...
node scripts/e2e/lib/plugin-update/registry-server.mjs >/tmp/FirstNexus-e2e-registry.log 2>&1 &
registry_pid=$!
...
if ! node "$probe" wait-registry; then
  ...
  cat /tmp/FirstNexus-e2e-registry.log || true
  exit 1
fi
