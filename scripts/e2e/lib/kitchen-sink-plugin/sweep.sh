...
start_kitchen_sink_clawhub_fixture_server() {
  ...
  local wait_attempts="${NEXISCLAW_CLAWHUB_FIXTURE_WAIT_ATTEMPTS:-600}"
...
run_success_scenario() {
  ...
  if [ -n "${KITCHEN_SINK_PREINSTALL_SPEC:-}" ]; then
    run_logged_print "kitchen-sink-preinstall-${KITCHEN_SINK_LABEL}" node "$NEXISCLAW_ENTRY" plugins install "$KITCHEN_SINK_PREINSTALL_SPEC"
    assert_kitchen_sink_cutover_preinstalled
    install_args+=("--force")
  fi
  run_logged_print "kitchen-sink-install-${KITCHEN_SINK_LABEL}" node "$NEXISCLAW_ENTRY" plugins install "${install_args[@]}"
  configure_kitchen_sink_runtime
  run_logged_print "kitchen-sink-enable-${KITCHEN_SINK_LABEL}" node "$NEXISCLAW_ENTRY" plugins enable "$KITCHEN_SINK_ID"
  node "$NEXISCLAW_ENTRY" plugins list --json >"/tmp/kitchen-sink-${KITCHEN_SINK_LABEL}-plugins.json"
  node "$NEXISCLAW_ENTRY" plugins inspect "$KITCHEN_SINK_ID" --runtime --json >"/tmp/kitchen-sink-${KITCHEN_SINK_LABEL}-inspect.json"
  node "$NEXISCLAW_ENTRY" plugins inspect --all --runtime --json >"/tmp/kitchen-sink-${KITCHEN_SINK_LABEL}-inspect-all.json"
  assert_kitchen_sink_installed
  if [ "$KITCHEN_SINK_SOURCE" = "clawhub" ]; then
    run_logged_print "kitchen-sink-uninstall-${KITCHEN_SINK_LABEL}" node "$NEXISCLAW_ENTRY" plugins uninstall "$KITCHEN_SINK_SPEC" --force
  else
    run_logged_print "kitchen-sink-uninstall-${KITCHEN_SINK_LABEL}" node "$NEXISCLAW_ENTRY" plugins uninstall "$KITCHEN_SINK_ID" --force
  fi
  remove_kitchen_sink_channel_config
  node "$NEXISCLAW_ENTRY" plugins list --json >"/tmp/kitchen-sink-${KITCHEN_SINK_LABEL}-uninstalled.json"
  assert_kitchen_sink_removed
}

run_failure_scenario() {
  echo "Testing expected ${KITCHEN_SINK_LABEL} install failure from ${KITCHEN_SINK_SPEC}..."
  run_expect_failure "install-${KITCHEN_SINK_LABEL}" node "$NEXISCLAW_ENTRY" plugins install "$KITCHEN_SINK_SPEC"
  remove_kitchen_sink_channel_config
  node "$NEXISCLAW_ENTRY" plugins list --json >"/tmp/kitchen-sink-${KITCHEN_SINK_LABEL}-uninstalled.json"
  assert_kitchen_sink_removed
}
...
while IFS='|' read -r label spec plugin_id source expectation surface_mode personality preinstall_spec; do
  ...
  export NEXISCLAW_KITCHEN_SINK_PERSONALITY="${personality:-}"
