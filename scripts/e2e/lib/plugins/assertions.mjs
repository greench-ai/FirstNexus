...
function getInstallRecords() {
  ...
  const allowLegacyCompat = process.env.NEXISCLAW_PACKAGE_ACCEPTANCE_LEGACY_COMPAT === "1";
...
function assertPluginRemoved(params) {
  ...
  const config = readFirstNexusConfig();
...
function recordFixturePluginTrust() {
  ...
  const ledgerPath = path.join(process.env.HOME, ".FirstNexus", "plugins", "installs.json");
  const ledger = fs.existsSync(ledgerPath)
    ? readJson(ledgerPath)
    : {
        version: 1,
        warning:
          "DO NOT EDIT. This file is generated by FirstNexus plugin install/update/uninstall commands. Use `FirstNexus plugins install/update/uninstall` instead.",
        records: {},
      };
...
function assertGitPlugin() {
  ...
  const gitRoot = path.join(process.env.HOME, ".FirstNexus", "git");
...
function assertClawHubExternalInstallContract(installPath) {
  ...
  if (!fs.existsSync(FirstNexusPeerPath)) {
    throw new Error(`missing ClawHub FirstNexus peer symlink: ${FirstNexusPeerPath}`);
  }
  if (!fs.lstatSync(FirstNexusPeerPath).isSymbolicLink()) {
    throw new Error(`ClawHub FirstNexus peer is not a symlink: ${FirstNexusPeerPath}`);
  }
  ...
  if (linkedHostRoot !== hostRoot) {
    throw new Error(`expected ClawHub FirstNexus peer ${linkedHostRoot} to target ${hostRoot}`);
  }
...
function assertNpmPlugin() {
  ...
  if (record.spec !== "@FirstNexus/demo-plugin-npm@0.0.1") {
    throw new Error(`unexpected npm spec: ${record.spec}`);
  }
  if (record.resolvedName !== "@FirstNexus/demo-plugin-npm") {
...
async function assertClawHubPreflight() {
  ...
  const baseUrl = (
    process.env.NEXISCLAW_CLAWHUB_URL ||
    process.env.CLAWHUB_URL ||
    "https://clawhub.ai"
  ).replace(/\/+$/, "");
  const token =
    process.env.NEXISCLAW_CLAWHUB_TOKEN ||
...
function assertClawHubInstalled() {
  ...
  const installPath = record.installPath.replace(/^~(?=$|\/)/u, process.env.HOME);
  const extensionsRoot = path.join(process.env.HOME, ".FirstNexus", "extensions");
...
function assertClawHubRemoved() {
  ...
  const configAfterUninstallPath = path.join(process.env.HOME, ".FirstNexus", "FirstNexus.json");
