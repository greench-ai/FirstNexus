...
IMAGE_NAME="nexusclaw-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="nexusclaw-net-e2e-$$"
GW_NAME="nexusclaw-gateway-e2e-$$"
...
docker run -d \
  --name "$GW_NAME" \
  --network "$NET_NAME" \
  -e "NEXUSCLAW_GATEWAY_TOKEN=$TOKEN" \
  -e "NEXUSCLAW_SKIP_CHANNELS=1" \
  -e "NEXUSCLAW_SKIP_GMAIL_WATCHER=1" \
  -e "NEXUSCLAW_SKIP_CRON=1" \
  -e "NEXUSCLAW_SKIP_CANVAS_HOST=1" \
  "$IMAGE_NAME" \
