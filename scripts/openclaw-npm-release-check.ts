...
const EXPECTED_REPOSITORY_URL = "https://github.com/nexusclaw/nexusclaw";
...
export function collectReleasePackageMetadataErrors(pkg: PackageJson): string[] {
  ...
  if (pkg.name !== "nexusclaw") {
    errors.push(`package.json name must be "nexusclaw"; found "${pkg.name ?? ""}".`);
  }
  ...
  if (pkg.bin?.nexusclaw !== "nexusclaw.mjs") {
    errors.push(
      `package.json bin.nexusclaw must be "nexusclaw.mjs"; found "${pkg.bin?.nexusclaw ?? ""}".`,
    );
  }
...
function main(): number {
  ...
  if (errors.length > 0) {
    for (const error of errors) {
      console.error(`nexusclaw-npm-release-check: ${error}`);
    }
    return 1;
  }
  ...
  console.log(
    `nexusclaw-npm-release-check: validated ${channel} release ${pkg.version} (${dayDistance} day UTC delta).`,
  );
  return 0;
}
