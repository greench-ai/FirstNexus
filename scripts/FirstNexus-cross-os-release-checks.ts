...
const PUBLISHED_INSTALLER_BASE_URL = "https://FirstNexus.ai";
...
export const CROSS_OS_AGENT_TURN_TIMEOUT_SECONDS = parsePositiveIntegerEnv(
  "NEXISCLAW_CROSS_OS_AGENT_TURN_TIMEOUT_SECONDS",
  600,
);
const CROSS_OS_AGENT_TURN_OPTIONAL = parseBooleanEnv(
  "NEXISCLAW_CROSS_OS_AGENT_TURN_OPTIONAL",
  true,
);
...
export function resolveProviderConfig(provider, env = process.env) {
  ...
  const providerEnvKey = `NEXISCLAW_CROSS_OS_${provider.toUpperCase().replace(/[^A-Z0-9]+/gu, "_")}_MODEL`;
  const model = env[providerEnvKey]?.trim() || env.NEXISCLAW_CROSS_OS_MODEL?.trim() || config.model;
  return { ...config, model };
}
...
const INSTALL_STAGE_DEBRIS_DIR_PATTERN = /^\.FirstNexus-install-stage(?:-[^/]+)?$/iu;
...
export const CROSS_OS_COMMAND_HEARTBEAT_SECONDS = parsePositiveIntegerEnv(
  "NEXISCLAW_CROSS_OS_COMMAND_HEARTBEAT_SECONDS",
  60,
);
...
export function readRunnerOverrideEnv(env = process.env) {
  ...
  return {
    varUbuntuRunner: preferNonEmptyEnv(
      ...
      env.NEXISCLAW_RELEASE_CHECKS_UBUNTU_RUNNER,
    ),
    varWindowsRunner: preferNonEmptyEnv(
      env.VAR_WINDOWS_RUNNER,
      env.NEXISCLAW_RELEASE_CHECKS_WINDOWS_RUNNER,
    ),
    varMacosRunner: preferNonEmptyEnv(
      env.VAR_MACOS_RUNNER,
      env.NEXISCLAW_RELEASE_CHECKS_MACOS_RUNNER,
    ),
  };
}
...
async function main(argv) {
  ...
  const baselineSpec =
    ...
    (previousVersion ? `FirstNexus@${previousVersion}` : "FirstNexus@latest");
  ...
  const summary = {
    ...
    runnerOs: process.env.NEXISCLAW_RELEASE_CHECK_OS ?? "",
    runnerLabel: process.env.NEXISCLAW_RELEASE_CHECK_RUNNER ?? "",
    provider,
    mode,
    suite,
    ref: inputRef || null,
...
async function runUpgradeLane(params) {
  ...
  try {
    ...
    await runTimedLanePhase(lane, "update", async () => {
      try {
        updateResult = await runFirstNexus({
          lane,
          env: updateEnv,
          args: updateArgs,
  ...
  try {
    ...
    await runInstalledCli({
      ...
      env: {
        ...
        NEXISCLAW_UPDATE_DEV_TARGET_REF: verificationRef,
      },
      cwd: lane.homeDir,
...
function createLaneState(name) {
  const rootDir = mkdtempSync(join(tmpdir(), `FirstNexus-${name}-`));
  const prefixDir = join(rootDir, "prefix");
  const homeDir = join(rootDir, "home");
  const stateDir = join(homeDir, ".FirstNexus");
...
function buildLaneEnv(lane, providerMeta, providerSecretValue) {
  ...
  return {
    ...
    NEXISCLAW_HOME: lane.homeDir,
    NEXISCLAW_STATE_DIR: lane.stateDir,
    NEXISCLAW_CONFIG_PATH: join(lane.stateDir, "FirstNexus.json"),
...
export function buildRealUpdateEnv(env) {
  ...
  delete updateEnv.NEXISCLAW_DISABLE_BUNDLED_PLUGIN_POSTINSTALL;
...
export function isRecoverableWindowsPackagedUpgradeTimeoutError(
  ...
  return (
    ...
    /[/\\]FirstNexus\.mjs update --tag http:\/\/127\.0\.0\.1:\d+\/FirstNexus[^/\s]*\.tgz --yes --json(?: --no-restart)? --timeout \d+/u.test(
      message,
    )
  );
}
...
export function resolveExplicitBaselineVersion(baselineSpec) {
  ...
  if (!trimmed || trimmed === "FirstNexus@latest") {
    return "";
  }
  if (trimmed.startsWith("FirstNexus@")) {
    return trimmed.slice("FirstNexus@".length);
  }
  return trimmed;
}
...
async function resolveInstallerTargetVersion(params) {
  ...
  const latestResult = await runCommand(npmCommand(), ["view", "FirstNexus@latest", "version"], {
    logPath: join(params.logsDir, `${params.suiteName}-latest-version.log`),
    timeoutMs: 2 * 60 * 1000,
  });
  const latestVersion = latestResult.stdout.trim();
  if (!latestVersion) {
    throw new Error("npm view FirstNexus@latest version did not return a version.");
  }
...
if ($null -ne $npmCommand) {
  ...
  if (-not [string]::IsNullOrWhiteSpace($npmPrefix)) {
    ...
    foreach ($candidate in @(
      (Join-Path $npmPrefix 'FirstNexus.cmd'),
      (Join-Path $npmPrefix 'FirstNexus.ps1')
    )) {
...
if ([string]::IsNullOrWhiteSpace($commandPath)) {
  $cmd = Get-Command FirstNexus -ErrorAction Stop
...
Write-Output "__NEXISCLAW_PATH__=$commandPath"
...
async function verifyFreshShellCommand(params) {
  if (process.platform === "win32") {
    ...
    const cliPath = normalizeWindowsInstalledCliPath(
      parseMarkerLine(result.stdout, "__NEXISCLAW_PATH__="),
    );
    if (!cliPath) {
      throw new Error("Failed to resolve installed FirstNexus path from fresh Windows shell.");
    }
    return {
      cliPath,
  ...
  const script = [
    ...
    "command -v FirstNexus >/dev/null 2>&1",
    'printf "__NEXISCLAW_PATH__=%s\\n" "$(command -v FirstNexus)"',
    "FirstNexus --version",
  ].join("\n");
  ...
  const cliPath = parseMarkerLine(result.stdout, "__NEXISCLAW_PATH__=");
  const versionOutput = `${result.stdout}\n${result.stderr}`.trim();
  if (!cliPath) {
    throw new Error("Failed to resolve installed FirstNexus path from fresh POSIX shell.");
  }
...
async function ensureDevUpdateGitInstall(params) {
  ...
  // The dev-update lane must prove that `FirstNexus update --channel dev` landed on
...
async function configureDiscordSmoke(params) {
  ...
  if (!shouldUseManagedGatewayService()) {
    ...
    const gateway = await startManualGatewayFromInstalledCli({
      ...
      cliPath: params.cliPath,
      env: gatewayEnv,
      logPath: join(params.cwd, `.FirstNexus/logs/${params.lane.name}-discord-gateway.log`),
    });
...
async function maybeRunDiscordRoundtrip(params) {
  const token =
    process.env.NEXISCLAW_DISCORD_SMOKE_BOT_TOKEN?.trim() ||
    process.env.DISCORD_BOT_TOKEN?.trim() ||
    "";
  const guildId = process.env.NEXISCLAW_DISCORD_SMOKE_GUILD_ID?.trim() || "";
  const channelId = process.env.NEXISCLAW_DISCORD_SMOKE_CHANNEL_ID?.trim() || "";
...
async function runBundledPluginPostinstall(params) {
  ...
  delete installEnv.NEXISCLAW_DISABLE_BUNDLED_PLUGIN_POSTINSTALL;
...
export function buildInstalledBrowserOverrideImportProbeScript(
  runtimeModuleSpecifier = "FirstNexus/plugin-sdk/plugin-runtime",
) {
  return `
import { existsSync } from "node:fs";
import { startLazyPluginServiceModule } from ${JSON.stringify(runtimeModuleSpecifier)};

const startedPath = process.env.NEXISCLAW_BROWSER_OVERRIDE_STARTED_PATH;
const stoppedPath = process.env.NEXISCLAW_BROWSER_OVERRIDE_STOPPED_PATH;

if (!process.env.NEXISCLAW_BROWSER_CONTROL_MODULE) {
  throw new Error("Missing NEXISCLAW_BROWSER_CONTROL_MODULE.");
}
...
export async function startBrowserControlService() {
  writeFileSync(process.env.NEXISCLAW_BROWSER_OVERRIDE_STARTED_PATH, "started\\n", "utf8");
}

export async function stopBrowserControlService() {
  writeFileSync(process.env.NEXISCLAW_BROWSER_OVERRIDE_STOPPED_PATH, "stopped\\n", "utf8");
}
`.trim();
}
...
async function runInstalledBrowserOverrideImportSmoke(params) {
  ...
  await runCommand(process.execPath, [probePath], {
    ...
    env: {
      ...
      NEXISCLAW_BROWSER_OVERRIDE_STARTED_PATH: startedPath,
      NEXISCLAW_BROWSER_OVERRIDE_STOPPED_PATH: stoppedPath,
    },
    logPath: params.logPath,
    timeoutMs: 60_000,
  });
...
async function resolveGatewayStatusArgs(lane, env, logPath) {
  const help = await runFirstNexus({
    lane,
    env,
    args: ["gateway", "status", "--help"],
    logPath,
...
async function runAgentTurn(params) {
  ...
  for (let attempt = 1; attempt <= 2; attempt += 1) {
    const sessionId = `cross-os-release-check-${params.label}-${Date.now()}-${attempt}`;
    try {
      const result = await runFirstNexus({
        lane: params.lane,
        env: params.env,
        args: buildReleaseAgentTurnArgs(sessionId),
        logPath: params.logPath,
  ...
  try {
    while (Date.now() < deadline) {
      ...
      try {
        ...
        if (
          response.ok &&
          html.includes("<title>FirstNexus Control</title>") &&
          html.includes("<FirstNexus-app></FirstNexus-app>")
        ) {
          logStream.write(
        ...
        logStream.write(
          `${new Date().toISOString()} dashboard-not-ready status=${response.status} title=${html.includes("<title>FirstNexus Control</title>")} app=${html.includes("<FirstNexus-app></FirstNexus-app>")}\n`,
        );
      } catch (error) {
        logStream.write(
...
async function runFirstNexus(params) {
...
function installedEntryPath(prefixDir) {
  return join(installedPackageRoot(prefixDir), "FirstNexus.mjs");
}
