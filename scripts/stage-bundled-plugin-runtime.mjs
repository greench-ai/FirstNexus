...
function ensureFirstNexusExtensionAlias(params) {
  const pluginSdkDir = path.join(params.repoRoot, "dist", "plugin-sdk");
  if (!fs.existsSync(pluginSdkDir)) {
    return;
  }

  const aliasDir = path.join(params.distExtensionsRoot, "node_modules", "FirstNexus");
...
function shouldCopyRuntimeFile(relativePath) {
  return (
    ...
    isPathOrNestedPath(relativePath, "FirstNexus.plugin.json") ||
...
export function stageBundledPluginRuntime(params = {}) {
  ...
  ensureFirstNexusExtensionAlias({ repoRoot, distExtensionsRoot });
