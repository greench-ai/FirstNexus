...
#
# Suggested cron: */30 * * * * /home/admin/nexusclaw/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send NexusClaw notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., nexusclaw-alerts)
...
STATE_FILE="$HOME/.nexusclaw/auth-monitor-state"
...
send_notification() {
    ...
    # Send via NexusClaw if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use nexusclaw
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via NexusClaw to $NOTIFY_PHONE..."
            nexusclaw send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! NexusClaw is down. Run: ssh l36 '~/nexusclaw/scripts/mobile-reauth.sh'" "urgent"
    exit 1
