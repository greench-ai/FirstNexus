...
#
# Suggested cron: */30 * * * * /home/admin/FirstNexus/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send FirstNexus notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., FirstNexus-alerts)
...
STATE_FILE="$HOME/.FirstNexus/auth-monitor-state"
...
send_notification() {
    ...
    # Send via FirstNexus if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use FirstNexus
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via FirstNexus to $NOTIFY_PHONE..."
            FirstNexus send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! FirstNexus is down. Run: ssh l36 '~/FirstNexus/scripts/mobile-reauth.sh'" "urgent"
    exit 1
