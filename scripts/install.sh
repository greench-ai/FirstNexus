...
# FirstNexus Installer for macOS and Linux
# Usage: curl -fsSL --proto '=https' --tlsv1.2 https://FirstNexus.ai/install.sh | bash
...
DEFAULT_TAGLINE="All your chats, one FirstNexus."
...
print_installer_banner() {
    if [[ -n "$GUM" ]]; then
        ...
        title="$("$GUM" style --foreground "#ff4d4d" --bold "🦞 FirstNexus Installer")"
...
detect_os_or_die() {
    ...
    if [[ "$OS" == "unknown" ]]; then
        ...
        echo "For Windows, use: iwr -useb https://FirstNexus.ai/install.ps1 | iex"
        exit 1
    fi
...
show_footer_links() {
    local faq_url="https://docs.FirstNexus.ai/start/faq"
    if [[ -n "$GUM" ]]; then
        local content
...
cleanup_npm_FirstNexus_paths() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || "$npm_root" != *node_modules* ]]; then
        return 1
    fi
    rm -rf "$npm_root"/.FirstNexus-* "$npm_root"/FirstNexus 2>/dev/null || true
}

extract_FirstNexus_conflict_path() {
    local log="$1"
    local path=""
...
cleanup_FirstNexus_bin_conflict() {
    local bin_path="$1"
    ...
    if [[ -n "$npm_bin" && "$bin_path" != "$npm_bin/FirstNexus" ]]; then
        case "$bin_path" in
            "/opt/homebrew/bin/FirstNexus"|"/usr/local/bin/FirstNexus")
                ;;
            *)
                return 1
                ;;
    ...
    if [[ -L "$bin_path" ]]; then
        ...
        if [[ "$target" == *"/node_modules/FirstNexus/"* ]]; then
            rm -f "$bin_path"
            ui_info "Removed stale FirstNexus symlink at ${bin_path}"
            return 0
        fi
        return 1
    fi
    ...
    if mv "$bin_path" "$backup"; then
        ui_info "Moved existing FirstNexus binary to ${backup}"
        return 0
    fi
    return 1
}
...
run_npm_global_install() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        printf -v log_quoted '%q' "$log"
        run_with_spinner "Installing FirstNexus package" bash -c "${cmd_quoted}>${log_quoted} 2>&1"
        return $?
    fi
...
install_FirstNexus_npm() {
    ...
    if ! run_npm_global_install "$spec" "$log"; then
        ...
        if auto_install_build_tools_for_npm_failure "$log"; then
            ...
            if run_npm_global_install "$spec" "$log"; then
                ui_success "FirstNexus npm package installed"
                return 0
            fi
        fi
        ...
        if grep -q "ENOTEMPTY: directory not empty, rename .*FirstNexus" "$log"; then
        ...
        if grep -q "EEXIST" "$log"; then
            local conflict=""
            conflict="$(extract_FirstNexus_conflict_path "$log" || true)"
            if [[ -n "$conflict" ]] && cleanup_FirstNexus_bin_conflict "$conflict"; then
                if run_npm_global_install "$spec" "$log"; then
                    ui_success "FirstNexus npm package installed"
                    return 0
                fi
            ...
            ui_error "npm failed because an FirstNexus binary already exists"
            if [[ -n "$conflict" ]]; then
...
GIT_DIR_DEFAULT="${HOME}/FirstNexus"
...
print_usage() {
    cat <<EOF
FirstNexus installer (macOS + Linux)

Usage:
  curl -fsSL --proto '=https' --tlsv1.2 https://FirstNexus.ai/install.sh | bash -s -- [options]
...
Examples:
  curl -fsSL --proto '=https' --tlsv1.2 https://FirstNexus.ai/install.sh | bash
  curl -fsSL --proto '=https' --tlsv1.2 https://FirstNexus.ai/install.sh | bash -s -- --no-onboard
  curl -fsSL --proto '=https' --tlsv1.2 https://FirstNexus.ai/install.sh | bash -s -- --no-onboard --verify
  curl -fsSL --proto '=https' --tlsv1.2 https://FirstNexus.ai/install.sh | bash -s -- --version main
  curl -fsSL --proto '=https' --tlsv1.2 https://FirstNexus.ai/install.sh | bash -s -- --install-method git --no-onboard
EOF
}
...
choose_install_method_interactive() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        header="Detected FirstNexus checkout in: ${detected_checkout}
...
${WARN}→${NC} Detected a FirstNexus source checkout in: ${INFO}${detected_checkout}${NC}
Choose install method:
...
detect_FirstNexus_checkout() {
    ...
    if ! grep -q '"name"[[:space:]]*:[[:space:]]*"FirstNexus"' "$dir/package.json" 2>/dev/null; then
        return 1
    fi
    echo "$dir"
    return 0
...
print_homebrew_admin_fix() {
    ...
    echo "Then retry:"
    echo "  curl -fsSL https://FirstNexus.ai/install.sh | bash"
}
    ...
    for rc in "$HOME/.bashrc" "$HOME/.zshrc"; do
        if [[ -f "$rc" ]]; then
            if [[ "$(sed -n '1p' "$rc")" != "$path_line" ]]; then
                local tmp_rc="${rc}.FirstNexus-tmp"
                {
                    printf '%s\n' "$path_line"
...
fix_npm_permissions() {
    ...
    ui_warn "Avoid sudo npm i -g for future FirstNexus updates; use npm i -g FirstNexus@latest so npm keeps using this user prefix instead of a different global prefix."
...
ensure_FirstNexus_bin_link() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || ! -d "$npm_root/FirstNexus" ]]; then
        return 1
    fi
    local npm_bin=""
    ...
    mkdir -p "$npm_bin"
    if [[ ! -x "${npm_bin}/FirstNexus" ]]; then
        ln -sf "$npm_root/FirstNexus/dist/entry.js" "${npm_bin}/FirstNexus"
        ui_info "Created FirstNexus bin link at ${npm_bin}/FirstNexus"
    fi
    return 0
}

# Check for existing FirstNexus installation
check_existing_FirstNexus() {
    if [[ -n "$(type -P FirstNexus 2>/dev/null || true)" ]]; then
        ui_info "Existing FirstNexus installation detected, upgrading"
        return 0
    fi
    return 1
}
...
collect_FirstNexus_npm_root_candidates() {
    local root=""
...
find_FirstNexus_global_installs() {
    local seen="|"
    local npm_root=""
    while IFS= read -r npm_root; do
        [[ -n "$npm_root" ]] || continue
        local package_dir="${npm_root%/}/FirstNexus"
        ...
        local version=""
        version="$(FirstNexus_package_version "$package_json")"
        printf '%s\t%s\t%s\n' "$version" "$real_package_dir" "$npm_root"
    done < <(collect_FirstNexus_npm_root_candidates)
}
...
warn_duplicate_FirstNexus_global_installs() {
    ...
    done < <(find_FirstNexus_global_installs)

    if [[ "${#installs[@]}" -le 1 ]]; then
        return 0
    fi

    ui_warn "Multiple FirstNexus global installs detected"
    echo "  Different Node/npm environments can run different FirstNexus versions."

    local active_node active_npm active_FirstNexus
    active_node="$(command -v node 2>/dev/null || true)"
    active_npm="$(command -v npm 2>/dev/null || true)"
    active_FirstNexus="${NEXISCLAW_BIN:-}"
    if [[ -z "$active_FirstNexus" ]]; then
        active_FirstNexus="$(type -P FirstNexus 2>/dev/null || true)"
    fi
    echo -e "  Active node: ${INFO}${active_node:-none}${NC}"
    echo -e "  Active npm: ${INFO}${active_npm:-none}${NC}"
    echo -e "  Active FirstNexus: ${INFO}${active_FirstNexus:-none}${NC}"
    echo ""
    echo "  Found installs:"
    ...
    echo "    npm uninstall -g FirstNexus"
}
...
warn_shell_path_missing_dir() {
    ...
    echo "  This can make FirstNexus show as \"command not found\" in new terminals."
    echo "  Fix (zsh: ~/.zshrc, bash: ~/.bashrc):"
    echo "    export PATH=\"${dir}:\$PATH\""
}

FirstNexus_command_for_user() {
    local claw="${1:-}"
    if [[ -z "$claw" ]]; then
        echo "FirstNexus"
        return 0
...
warn_FirstNexus_not_found() {
    ui_warn "Installed, but FirstNexus is not discoverable on PATH in this shell"
    echo "  Try: hash -r (bash) or rehash (zsh), then retry."
    local t=""
    t="$(type -t FirstNexus 2>/dev/null || true)"
    if [[ "$t" == "alias" || "$t" == "function" ]]; then
        ui_warn "Found a shell ${t} named FirstNexus; it may shadow the real binary"
    fi
    if command -v nodenv &> /dev/null; then
...
resolve_FirstNexus_bin() {
    refresh_shell_command_cache
    local resolved=""
    resolved="$(type -P FirstNexus 2>/dev/null || true)"
    ...
    local npm_bin=""
    npm_bin="$(npm_global_bin_dir || true)"
    if [[ -n "$npm_bin" && -x "${npm_bin}/FirstNexus" ]]; then
        echo "${npm_bin}/FirstNexus"
        return 0
    fi
...
install_FirstNexus_from_git() {
    ...
    if [[ -d "$repo_dir/.git" ]]; then
        ui_info "Installing FirstNexus from git checkout: ${repo_dir}"
    else
        ui_info "Installing FirstNexus from GitHub (${repo_url})"
    fi
    ...
    if [[ ! -d "$repo_dir" ]]; then
        run_quiet_step "Cloning FirstNexus" git clone "$repo_url" "$repo_dir"
    fi
    ...
    run_quiet_step "Building FirstNexus" run_pnpm -C "$repo_dir" build

    ensure_user_local_bin_on_path

    cat > "$HOME/.local/bin/FirstNexus" <<EOF
#!/usr/bin/env bash
set -euo pipefail
exec node "${repo_dir}/dist/entry.js" "\$@"
EOF
    chmod +x "$HOME/.local/bin/FirstNexus"
    ui_success "FirstNexus wrapper installed to \$HOME/.local/bin/FirstNexus"
...
resolve_beta_version() {
    local beta=""
    beta="$(npm view FirstNexus dist-tags.beta 2>/dev/null || true)"
...
resolve_package_install_spec() {
    ...
    normalized_value="$(to_lowercase_ascii "$value")"
    if [[ "$normalized_value" == "main" ]]; then
        echo "github:FirstNexus/FirstNexus#main"
        return 0
    fi
    if is_explicit_package_install_spec "$value"; then
        echo "$value"
...
install_FirstNexus() {
    ...
    if [[ -n "$resolved_version" ]]; then
        ui_info "Installing FirstNexus v${resolved_version}"
    else
        ui_info "Installing FirstNexus (${NEXISCLAW_VERSION})"
    fi
    local install_spec=""
    install_spec="$(resolve_package_install_spec "${package_name}" "${NEXISCLAW_VERSION}")"

    if ! install_FirstNexus_npm "${install_spec}"; then
        ui_warn "npm install failed; retrying"
        cleanup_npm_FirstNexus_paths
        install_FirstNexus_npm "${install_spec}"
    fi

    if [[ "${NEXISCLAW_VERSION}" == "latest" && "${package_name}" == "FirstNexus" ]]; then
        if ! resolve_FirstNexus_bin &> /dev/null; then
            ui_warn "npm install FirstNexus@latest failed; retrying FirstNexus@next"
            cleanup_npm_FirstNexus_paths
            install_FirstNexus_npm "FirstNexus@next"
        fi
    fi

    ensure_FirstNexus_bin_link || true

    ui_success "FirstNexus installed"
}
...
run_doctor() {
    ...
    if [[ -z "$claw" ]]; then
        claw="$(resolve_FirstNexus_bin || true)"
    fi
    if [[ -z "$claw" ]]; then
        ui_info "Skipping doctor (FirstNexus not on PATH yet)"
        warn_FirstNexus_not_found
        return 0
    fi
...
resolve_workspace_dir() {
    ...
    if [[ "${profile}" != "default" ]]; then
        echo "${HOME}/.FirstNexus/workspace-${profile}"
    else
        echo "${HOME}/.FirstNexus/workspace"
    fi
}
...
run_bootstrap_onboarding_if_needed() {
    ...
    local config_path="${NEXISCLAW_CONFIG_PATH:-$HOME/.FirstNexus/FirstNexus.json}"
    ...
    if [[ ! -r /dev/tty || ! -w /dev/tty ]]; then
        local user_claw
        user_claw="$(FirstNexus_command_for_user "${NEXISCLAW_BIN:-}")"
    ...
    if [[ -z "$claw" ]]; then
        ui_info "BOOTSTRAP.md found but FirstNexus not on PATH; skipping onboarding"
        warn_FirstNexus_not_found
        return
    fi
    ...
    "$claw" onboard || {
        local user_claw
        user_claw="$(FirstNexus_command_for_user "$claw")"
...
if ! declare -F extract_FirstNexus_semver >/dev/null 2>&1; then
...
resolve_FirstNexus_version() {
    ...
    local claw="${NEXISCLAW_BIN:-}"
    if [[ -z "$claw" ]] && command -v FirstNexus &> /dev/null; then
        claw="$(command -v FirstNexus)"
    fi
    if [[ -n "$claw" ]]; then
        ...
        version="$(extract_FirstNexus_semver "$raw_version_output")"
        if [[ -z "$version" ]]; then
    ...
    if [[ -z "$version" ]]; then
        local npm_root=""
        npm_root=$(npm root -g 2>/dev/null || true)
        if [[ -n "$npm_root" && -f "$npm_root/FirstNexus/package.json" ]]; then
            version=$(node -e "console.log(require('${npm_root}/FirstNexus/package.json').version)" 2>/dev/null || true)
        fi
    fi
    echo "$version"
}
...
verify_installation() {
    ...
    if [[ -z "$claw" ]]; then
        ui_error "Install verify failed: FirstNexus not on PATH yet"
        warn_FirstNexus_not_found
        return 1
    fi

    run_quiet_step "Checking FirstNexus version" "$claw" --version || return 1

    if is_gateway_daemon_loaded "$claw"; then
        run_quiet_step "Checking gateway service" "$claw" gateway status --deep || {
            ui_error "Install verify failed: gateway service unhealthy"
            ui_info "Run: FirstNexus gateway status --deep"
            return 1
        }
    else
...
main() {
    ...
    detected_checkout="$(detect_FirstNexus_checkout "$PWD" || true)"

    if [[ -z "$INSTALL_METHOD" && -n "$detected_checkout" ]]; then
        if ! is_promptable; then
            ui_info "Found FirstNexus checkout but no TTY; defaulting to npm install"
            INSTALL_METHOD="npm"
        else
    ...
    if check_existing_FirstNexus; then
        is_upgrade=true
    fi
    ...
    ui_stage "Installing FirstNexus"

    local final_git_dir=""
    if [[ "$INSTALL_METHOD" == "git" ]]; then
        # Clean up npm global install if switching to git
        if npm list -g FirstNexus &>/dev/null; then
            ui_info "Removing npm global install (switching to git)"
            npm uninstall -g FirstNexus 2>/dev/null || true
        ...
        install_FirstNexus_from_git "$repo_dir"
    else
        # Clean up git wrapper if switching to npm
        if [[ -x "$HOME/.local/bin/FirstNexus" ]]; then
            ui_info "Removing git wrapper (switching to npm)"
            rm -f "$HOME/.local/bin/FirstNexus"
    ...
    NEXISCLAW_BIN="$(resolve_FirstNexus_bin || true)"
    warn_duplicate_FirstNexus_global_installs || true
    ...
    if [[ -n "$installed_version" ]]; then
        ui_celebrate "🦞 FirstNexus installed successfully (${installed_version})!"
    else
        ui_celebrate "🦞 FirstNexus installed successfully!"
    fi
    ...
    if [[ "$INSTALL_METHOD" == "git" && -n "$final_git_dir" ]]; then
        ...
        ui_kv "Wrapper" "$HOME/.local/bin/FirstNexus"
        ui_kv "Update command" "FirstNexus update"
        ui_kv "Switch to npm" "curl -fsSL --proto '=https' --tlsv1.2 https://FirstNexus.ai/install.sh | bash -s -- --install-method npm"
        ...
        if [[ -r /dev/tty && -w /dev/tty ]]; then
            ...
            ui_info "Running FirstNexus doctor"
            local doctor_ok=0
            if (( ${#doctor_args[@]} )); then
        ...
        if [[ "$NO_ONBOARD" == "1" || "$skip_onboard" == "true" ]]; then
            ...
            if [[ -r /dev/tty && -w /dev/tty ]]; then
                ...
                if [[ -z "$claw" ]]; then
                    ui_info "Skipping onboarding (FirstNexus not on PATH yet)"
                    warn_FirstNexus_not_found
                    return 0
                fi
                exec </dev/tty
    ...
    if command -v FirstNexus &> /dev/null; then
        ...
        if [[ -n "$claw" ]] && is_gateway_daemon_loaded "$claw"; then
            if [[ "$DRY_RUN" == "1" ]]; then
                ui_info "Gateway daemon detected; would restart (FirstNexus daemon restart)"
            else
                ...
                if NEXISCLAW_UPDATE_IN_PROGRESS=1 "$claw" daemon restart >/dev/null 2>&1; then
                    ...
                    ui_warn "Gateway restart failed; try: FirstNexus daemon restart"
                fi
            fi
        fi
    fi
