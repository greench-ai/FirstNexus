...
# NexusClaw Installer for macOS and Linux
# Usage: curl -fsSL --proto '=https' --tlsv1.2 https://nexusclaw.ai/install.sh | bash
...
DEFAULT_TAGLINE="All your chats, one NexusClaw."
...
GUM_VERSION="${NEXUSCLAW_GUM_VERSION:-0.17.0}"
GUM=""
GUM_STATUS="skipped"
GUM_REASON=""
...
print_installer_banner() {
    if [[ -n "$GUM" ]]; then
        ...
        title="$("$GUM" style --foreground "#ff4d4d" --bold "🦞 NexusClaw Installer")"
...
detect_os_or_die() {
    ...
    if [[ "$OS" == "unknown" ]]; then
        ...
        echo "For Windows, use: iwr -useb https://nexusclaw.ai/install.ps1 | iex"
        exit 1
    fi
...
show_install_plan() {
    ...
    ui_kv "Requested version" "$NEXUSCLAW_VERSION"
    if [[ "$USE_BETA" == "1" ]]; then
...
show_footer_links() {
    local faq_url="https://docs.nexusclaw.ai/start/faq"
    if [[ -n "$GUM" ]]; then
        local content
...
cleanup_npm_nexusclaw_paths() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || "$npm_root" != *node_modules* ]]; then
        return 1
    fi
    rm -rf "$npm_root"/.nexusclaw-* "$npm_root"/nexusclaw 2>/dev/null || true
}

extract_nexusclaw_conflict_path() {
    local log="$1"
    local path=""
...
cleanup_nexusclaw_bin_conflict() {
    local bin_path="$1"
    ...
    if [[ -n "$npm_bin" && "$bin_path" != "$npm_bin/nexusclaw" ]]; then
        case "$bin_path" in
            "/opt/homebrew/bin/nexusclaw"|"/usr/local/bin/nexusclaw")
                ;;
            *)
                return 1
                ;;
    ...
    if [[ -L "$bin_path" ]]; then
        ...
        if [[ "$target" == *"/node_modules/nexusclaw/"* ]]; then
            rm -f "$bin_path"
            ui_info "Removed stale nexusclaw symlink at ${bin_path}"
            return 0
        fi
        return 1
    fi
    ...
    if mv "$bin_path" "$backup"; then
        ui_info "Moved existing nexusclaw binary to ${backup}"
        return 0
    fi
    return 1
}
...
run_npm_global_install() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        printf -v log_quoted '%q' "$log"
        run_with_spinner "Installing NexusClaw package" bash -c "${cmd_quoted}>${log_quoted} 2>&1"
        return $?
    fi
...
install_nexusclaw_npm() {
    ...
    if ! run_npm_global_install "$spec" "$log"; then
        ...
        if auto_install_build_tools_for_npm_failure "$log"; then
            ...
            if run_npm_global_install "$spec" "$log"; then
                ui_success "NexusClaw npm package installed"
                return 0
            fi
        fi
        ...
        if grep -q "ENOTEMPTY: directory not empty, rename .*nexusclaw" "$log"; then
        ...
        if grep -q "EEXIST" "$log"; then
            local conflict=""
            conflict="$(extract_nexusclaw_conflict_path "$log" || true)"
            if [[ -n "$conflict" ]] && cleanup_nexusclaw_bin_conflict "$conflict"; then
                if run_npm_global_install "$spec" "$log"; then
                    ui_success "NexusClaw npm package installed"
                    return 0
                fi
            ...
            ui_error "npm failed because an nexusclaw binary already exists"
            if [[ -n "$conflict" ]]; then
...
map_legacy_env "NEXUSCLAW_TAGLINE_INDEX" "CLAWDBOT_TAGLINE_INDEX"
map_legacy_env "NEXUSCLAW_NO_ONBOARD" "CLAWDBOT_NO_ONBOARD"
map_legacy_env "NEXUSCLAW_NO_PROMPT" "CLAWDBOT_NO_PROMPT"
map_legacy_env "NEXUSCLAW_DRY_RUN" "CLAWDBOT_DRY_RUN"
map_legacy_env "NEXUSCLAW_INSTALL_METHOD" "CLAWDBOT_INSTALL_METHOD"
map_legacy_env "NEXUSCLAW_VERSION" "CLAWDBOT_VERSION"
map_legacy_env "NEXUSCLAW_BETA" "CLAWDBOT_BETA"
map_legacy_env "NEXUSCLAW_GIT_DIR" "CLAWDBOT_GIT_DIR"
map_legacy_env "NEXUSCLAW_GIT_UPDATE" "CLAWDBOT_GIT_UPDATE"
map_legacy_env "NEXUSCLAW_NPM_LOGLEVEL" "CLAWDBOT_NPM_LOGLEVEL"
map_legacy_env "NEXUSCLAW_VERBOSE" "CLAWDBOT_VERBOSE"
map_legacy_env "NEXUSCLAW_PROFILE" "CLAWDBOT_PROFILE"
map_legacy_env "NEXUSCLAW_INSTALL_SH_NO_RUN" "CLAWDBOT_INSTALL_SH_NO_RUN"
...
pick_tagline() {
    ...
    if [[ -n "${NEXUSCLAW_TAGLINE_INDEX:-}" ]]; then
        if [[ "${NEXUSCLAW_TAGLINE_INDEX}" =~ ^[0-9]+$ ]]; then
            local idx=$((NEXUSCLAW_TAGLINE_INDEX % count))
            echo "${TAGLINES[$idx]}"
            return
        fi
    fi
...
NO_ONBOARD=${NEXUSCLAW_NO_ONBOARD:-0}
NO_PROMPT=${NEXUSCLAW_NO_PROMPT:-0}
DRY_RUN=${NEXUSCLAW_DRY_RUN:-0}
INSTALL_METHOD=${NEXUSCLAW_INSTALL_METHOD:-}
NEXUSCLAW_VERSION=${NEXUSCLAW_VERSION:-latest}
USE_BETA=${NEXUSCLAW_BETA:-0}
GIT_DIR_DEFAULT="${HOME}/nexusclaw"
GIT_DIR=${NEXUSCLAW_GIT_DIR:-$GIT_DIR_DEFAULT}
GIT_UPDATE=${NEXUSCLAW_GIT_UPDATE:-1}
SHARP_IGNORE_GLOBAL_LIBVIPS="${SHARP_IGNORE_GLOBAL_LIBVIPS:-1}"
NPM_LOGLEVEL="${NEXUSCLAW_NPM_LOGLEVEL:-error}"
NPM_SILENT_FLAG="--silent"
VERBOSE="${NEXUSCLAW_VERBOSE:-0}"
VERIFY_INSTALL="${NEXUSCLAW_VERIFY_INSTALL:-0}"
NEXUSCLAW_BIN=""
PNPM_CMD=()
HELP=0
...
print_usage() {
    cat <<EOF
NexusClaw installer (macOS + Linux)

Usage:
  curl -fsSL --proto '=https' --tlsv1.2 https://nexusclaw.ai/install.sh | bash -s -- [options]
...
Options:
  ...
  --git-dir, --dir <path>             Checkout directory (default: ~/nexusclaw)
...
Environment variables:
  NEXUSCLAW_INSTALL_METHOD=git|npm
  NEXUSCLAW_VERSION=latest|next|main|<semver>|<spec>
  NEXUSCLAW_BETA=0|1
  NEXUSCLAW_GIT_DIR=...
  NEXUSCLAW_GIT_UPDATE=0|1
  NEXUSCLAW_NO_PROMPT=1
  NEXUSCLAW_VERIFY_INSTALL=1
  NEXUSCLAW_DRY_RUN=1
  NEXUSCLAW_NO_ONBOARD=1
  NEXUSCLAW_VERBOSE=1
  NEXUSCLAW_NPM_LOGLEVEL=error|warn|notice  Default: error (hide npm deprecation noise)
  SHARP_IGNORE_GLOBAL_LIBVIPS=0|1    Default: 1 (avoid sharp building against global libvips)

Examples:
  curl -fsSL --proto '=https' --tlsv1.2 https://nexusclaw.ai/install.sh | bash
  curl -fsSL --proto '=https' --tlsv1.2 https://nexusclaw.ai/install.sh | bash -s -- --no-onboard
  curl -fsSL --proto '=https' --tlsv1.2 https://nexusclaw.ai/install.sh | bash -s -- --no-onboard --verify
  curl -fsSL --proto '=https' --tlsv1.2 https://nexusclaw.ai/install.sh | bash -s -- --version main
  curl -fsSL --proto '=https' --tlsv1.2 https://nexusclaw.ai/install.sh | bash -s -- --install-method git --no-onboard
EOF
}
...
choose_install_method_interactive() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        header="Detected NexusClaw checkout in: ${detected_checkout}
...
${WARN}→${NC} Detected a NexusClaw source checkout in: ${INFO}${detected_checkout}${NC}
Choose install method:
...
detect_nexusclaw_checkout() {
    ...
    if ! grep -q '"name"[[:space:]]*:[[:space:]]*"nexusclaw"' "$dir/package.json" 2>/dev/null; then
        return 1
    fi
    echo "$dir"
    return 0
...
print_homebrew_admin_fix() {
    ...
    echo "Then retry:"
    echo "  curl -fsSL https://nexusclaw.ai/install.sh | bash"
}
...
ensure_nexusclaw_bin_link() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || ! -d "$npm_root/nexusclaw" ]]; then
        return 1
    fi
    local npm_bin=""
    ...
    mkdir -p "$npm_bin"
    if [[ ! -x "${npm_bin}/nexusclaw" ]]; then
        ln -sf "$npm_root/nexusclaw/dist/entry.js" "${npm_bin}/nexusclaw"
        ui_info "Created nexusclaw bin link at ${npm_bin}/nexusclaw"
    fi
    return 0
}

# Check for existing NexusClaw installation
check_existing_nexusclaw() {
    if [[ -n "$(type -P nexusclaw 2>/dev/null || true)" ]]; then
        ui_info "Existing NexusClaw installation detected, upgrading"
        return 0
    fi
    return 1
}
...
warn_shell_path_missing_dir() {
    ...
    echo "  This can make nexusclaw show as \"command not found\" in new terminals."
...
warn_nexusclaw_not_found() {
    ui_warn "Installed, but nexusclaw is not discoverable on PATH in this shell"
    echo "  Try: hash -r (bash) or rehash (zsh), then retry."
    local t=""
    t="$(type -t nexusclaw 2>/dev/null || true)"
    if [[ "$t" == "alias" || "$t" == "function" ]]; then
        ui_warn "Found a shell ${t} named nexusclaw; it may shadow the real binary"
    fi
    if command -v nodenv &> /dev/null; then
...
resolve_nexusclaw_bin() {
    refresh_shell_command_cache
    local resolved=""
    resolved="$(type -P nexusclaw 2>/dev/null || true)"
    ...
    local npm_bin=""
    npm_bin="$(npm_global_bin_dir || true)"
    if [[ -n "$npm_bin" && -x "${npm_bin}/nexusclaw" ]]; then
        echo "${npm_bin}/nexusclaw"
        return 0
    fi
...
install_nexusclaw_from_git() {
    local repo_dir="$1"
    local repo_url="https://github.com/nexusclaw/nexusclaw.git"

    if [[ -d "$repo_dir/.git" ]]; then
        ui_info "Installing NexusClaw from git checkout: ${repo_dir}"
    else
        ui_info "Installing NexusClaw from GitHub (${repo_url})"
    fi
    ...
    if [[ ! -d "$repo_dir" ]]; then
        run_quiet_step "Cloning NexusClaw" git clone "$repo_url" "$repo_dir"
    fi
    ...
    run_quiet_step "Building NexusClaw" run_pnpm -C "$repo_dir" build

    ensure_user_local_bin_on_path

    cat > "$HOME/.local/bin/nexusclaw" <<EOF
#!/usr/bin/env bash
set -euo pipefail
exec node "${repo_dir}/dist/entry.js" "\$@"
EOF
    chmod +x "$HOME/.local/bin/nexusclaw"
    ui_success "NexusClaw wrapper installed to \$HOME/.local/bin/nexusclaw"
...
resolve_beta_version() {
    local beta=""
    beta="$(npm view nexusclaw dist-tags.beta 2>/dev/null || true)"
...
resolve_package_install_spec() {
    ...
    local value="$2"
    if [[ "${value,,}" == "main" ]]; then
        echo "github:nexusclaw/nexusclaw#main"
        return 0
    fi
    if is_explicit_package_install_spec "$value"; then
        echo "$value"
...
install_nexusclaw() {
    local package_name="nexusclaw"
    if [[ "$USE_BETA" == "1" ]]; then
        local beta_version=""
        beta_version="$(resolve_beta_version || true)"
        if [[ -n "$beta_version" ]]; then
            NEXUSCLAW_VERSION="$beta_version"
            ui_info "Beta tag detected (${beta_version})"
            package_name="nexusclaw"
        else
            NEXUSCLAW_VERSION="latest"
            ui_info "No beta tag found; using latest"
        fi
    fi

    if [[ -z "${NEXUSCLAW_VERSION}" ]]; then
        NEXUSCLAW_VERSION="latest"
    fi

    local resolved_version=""
    if can_resolve_registry_package_version "${NEXUSCLAW_VERSION}"; then
        resolved_version="$(npm view "${package_name}@${NEXUSCLAW_VERSION}" version 2>/dev/null || true)"
    fi
    if [[ -n "$resolved_version" ]]; then
        ui_info "Installing NexusClaw v${resolved_version}"
    else
        ui_info "Installing NexusClaw (${NEXUSCLAW_VERSION})"
    fi
    local install_spec=""
    install_spec="$(resolve_package_install_spec "${package_name}" "${NEXUSCLAW_VERSION}")"

    if ! install_nexusclaw_npm "${install_spec}"; then
        ui_warn "npm install failed; retrying"
        cleanup_npm_nexusclaw_paths
        install_nexusclaw_npm "${install_spec}"
    fi

    if [[ "${NEXUSCLAW_VERSION}" == "latest" && "${package_name}" == "nexusclaw" ]]; then
        if ! resolve_nexusclaw_bin &> /dev/null; then
            ui_warn "npm install nexusclaw@latest failed; retrying nexusclaw@next"
            cleanup_npm_nexusclaw_paths
            install_nexusclaw_npm "nexusclaw@next"
        fi
    fi

    ensure_nexusclaw_bin_link || true

    ui_success "NexusClaw installed"
}

# Run doctor for migrations (safe, non-interactive)
run_doctor() {
    ui_info "Running doctor to migrate settings"
    local claw="${NEXUSCLAW_BIN:-}"
    if [[ -z "$claw" ]]; then
        claw="$(resolve_nexusclaw_bin || true)"
    fi
    if [[ -z "$claw" ]]; then
        ui_info "Skipping doctor (nexusclaw not on PATH yet)"
        warn_nexusclaw_not_found
        return 0
    fi
...
resolve_workspace_dir() {
    local profile="${NEXUSCLAW_PROFILE:-default}"
    if [[ "${profile}" != "default" ]]; then
        echo "${HOME}/.nexusclaw/workspace-${profile}"
    else
        echo "${HOME}/.nexusclaw/workspace"
    fi
}
...
run_bootstrap_onboarding_if_needed() {
    ...
    local config_path="${NEXUSCLAW_CONFIG_PATH:-$HOME/.nexusclaw/nexusclaw.json}"
    ...
    if [[ ! -r /dev/tty || ! -w /dev/tty ]]; then
        ui_info "BOOTSTRAP.md found but no TTY; run nexusclaw onboard to finish setup"
        return
    fi
    ...
    if [[ -z "$claw" ]]; then
        ui_info "BOOTSTRAP.md found but nexusclaw not on PATH; skipping onboarding"
        warn_nexusclaw_not_found
        return
    fi

    "$claw" onboard || {
        ui_error "Onboarding failed; run nexusclaw onboard to retry"
        return
    }
}
...
resolve_nexusclaw_version() {
    local version=""
    local raw_version_output=""
    local claw="${NEXUSCLAW_BIN:-}"
    if [[ -z "$claw" ]] && command -v nexusclaw &> /dev/null; then
        claw="$(command -v nexusclaw)"
    fi
    if [[ -n "$claw" ]]; then
        raw_version_output=$("$claw" --version 2>/dev/null | head -n 1 | tr -d '\r')
        version="$(extract_nexusclaw_semver "$raw_version_output")"
        if [[ -z "$version" ]]; then
    ...
    if [[ -z "$version" ]]; then
        local npm_root=""
        npm_root=$(npm root -g 2>/dev/null || true)
        if [[ -n "$npm_root" && -f "$npm_root/nexusclaw/package.json" ]]; then
            version=$(node -e "console.log(require('${npm_root}/nexusclaw/package.json').version)" 2>/dev/null || true)
        fi
    fi
    echo "$version"
}
...
verify_installation() {
    ...
    if [[ -z "$claw" ]]; then
        ui_error "Install verify failed: nexusclaw not on PATH yet"
        warn_nexusclaw_not_found
        return 1
    fi

    run_quiet_step "Checking NexusClaw version" "$claw" --version || return 1

    if is_gateway_daemon_loaded "$claw"; then
        run_quiet_step "Checking gateway service" "$claw" gateway status --deep || {
            ui_error "Install verify failed: gateway service unhealthy"
            ui_info "Run: nexusclaw gateway status --deep"
            return 1
        }
    else
...
main() {
    ...
    detected_checkout="$(detect_nexusclaw_checkout "$PWD" || true)"

    if [[ -z "$INSTALL_METHOD" && -n "$detected_checkout" ]]; then
        if ! is_promptable; then
            ui_info "Found NexusClaw checkout but no TTY; defaulting to npm install"
            INSTALL_METHOD="npm"
        else
            ...
            case "$selected_method" in
                ...
                *)
                    ...
                    echo "Re-run with: --install-method git|npm (or set NEXUSCLAW_INSTALL_METHOD)."
                    exit 2
                    ;;
            esac
        fi
    ...
    if check_existing_nexusclaw; then
        is_upgrade=true
    fi
    ...
    ui_stage "Installing NexusClaw"

    local final_git_dir=""
    if [[ "$INSTALL_METHOD" == "git" ]]; then
        # Clean up npm global install if switching to git
        if npm list -g nexusclaw &>/dev/null; then
            ui_info "Removing npm global install (switching to git)"
            npm uninstall -g nexusclaw 2>/dev/null || true
        ...
        install_nexusclaw_from_git "$repo_dir"
    else
        # Clean up git wrapper if switching to npm
        if [[ -x "$HOME/.local/bin/nexusclaw" ]]; then
            ui_info "Removing git wrapper (switching to npm)"
            rm -f "$HOME/.local/bin/nexusclaw"
    ...
    NEXUSCLAW_BIN="$(resolve_nexusclaw_bin || true)"
    ...
    installed_version=$(resolve_nexusclaw_version)

    echo ""
    if [[ -n "$installed_version" ]]; then
        ui_celebrate "🦞 NexusClaw installed successfully (${installed_version})!"
    else
        ui_celebrate "🦞 NexusClaw installed successfully!"
    fi
    ...
    if [[ "$INSTALL_METHOD" == "git" && -n "$final_git_dir" ]]; then
        ...
        ui_kv "Wrapper" "$HOME/.local/bin/nexusclaw"
        ui_kv "Update command" "nexusclaw update --restart"
        ui_kv "Switch to npm" "curl -fsSL --proto '=https' --tlsv1.2 https://nexusclaw.ai/install.sh | bash -s -- --install-method npm"
        ...
        if [[ -r /dev/tty && -w /dev/tty ]]; then
            ...
            ui_info "Running nexusclaw doctor"
            local doctor_ok=0
            if (( ${#doctor_args[@]} )); then
                NEXUSCLAW_UPDATE_IN_PROGRESS=1 "$claw" doctor "${doctor_args[@]}" </dev/tty && doctor_ok=1
            else
                NEXUSCLAW_UPDATE_IN_PROGRESS=1 "$claw" doctor </dev/tty && doctor_ok=1
            fi
            if (( doctor_ok )); then
                ui_info "Updating plugins"
                NEXUSCLAW_UPDATE_IN_PROGRESS=1 "$claw" plugins update --all || true
            else
                ui_warn "Doctor failed; skipping plugin updates"
            fi
        else
            ui_info "No TTY; run nexusclaw doctor and nexusclaw plugins update --all manually"
        fi
    else
        if [[ "$NO_ONBOARD" == "1" || "$skip_onboard" == "true" ]]; then
            ui_info "Skipping onboard (requested); run nexusclaw onboard later"
        else
            local config_path="${NEXUSCLAW_CONFIG_PATH:-$HOME/.nexusclaw/nexusclaw.json}"
            if [[ -f "${config_path}" || -f "$HOME/.clawdbot/clawdbot.json" || -f "$HOME/.moltbot/moltbot.json" || -f "$HOME/.moldbot/moldbot.json" ]]; then
            ...
            if [[ -r /dev/tty && -w /dev/tty ]]; then
                ...
                if [[ -z "$claw" ]]; then
                    ui_info "Skipping onboarding (nexusclaw not on PATH yet)"
                    warn_nexusclaw_not_found
                    return 0
                fi
                exec </dev/tty
            ...
            ui_info "No TTY; run nexusclaw onboard to finish setup"
            return 0
        fi
    fi

    if command -v nexusclaw &> /dev/null; then
        local claw="${NEXUSCLAW_BIN:-}"
        if [[ -z "$claw" ]]; then
            claw="$(resolve_nexusclaw_bin || true)"
        fi
        if [[ -n "$claw" ]] && is_gateway_daemon_loaded "$claw"; then
            if [[ "$DRY_RUN" == "1" ]]; then
                ui_info "Gateway daemon detected; would restart (nexusclaw daemon restart)"
            else
                ui_info "Gateway daemon detected; restarting"
                if NEXUSCLAW_UPDATE_IN_PROGRESS=1 "$claw" daemon restart >/dev/null 2>&1; then
                    ui_success "Gateway restarted"
                else
                    ui_warn "Gateway restart failed; try: nexusclaw daemon restart"
                fi
            fi
        fi
    fi
...
if [[ "${NEXUSCLAW_INSTALL_SH_NO_RUN:-0}" != "1" ]]; then
    parse_args "$@"
    configure_verbose
    main
fi
