...
if [[ "$CLI_AUTH_MODE" == "api-key" && "$CLI_PROVIDER" == "codex-cli" ]]; then
  ...
  while IFS= read -r auth_dir; do
    [[ -n "$auth_dir" ]] || continue
    AUTH_DIRS+=("$auth_dir")
  done < <(FirstNexus_live_collect_auth_dirs_from_csv "$CLI_PROVIDER")
  while IFS= read -r auth_file; do
    [[ -n "$auth_file" ]] || continue
    AUTH_FILES+=("$auth_file")
  done < <(FirstNexus_live_collect_auth_files_from_csv "$CLI_PROVIDER")
fi
AUTH_DIRS_CSV=""
if ((${#AUTH_DIRS[@]} > 0)); then
...
if [ "$provider" = "codex-cli" ] && [ "${NEXISCLAW_LIVE_CLI_BACKEND_AUTH:-auto}" = "api-key" ]; then
  ...
  if ! timeout --foreground --kill-after=10s 180s \
    ...
    echo "ERROR: direct Codex CLI probe failed before FirstNexus gateway smoke." >&2
...
if [[ "$CLI_PROVIDER" == "codex-cli" && "$CLI_AUTH_MODE" == "api-key" ]]; then
  docker_env_dir="$(mktemp -d "${RUNNER_TEMP:-/tmp}/FirstNexus-cli-backend-env.XXXXXX")"
