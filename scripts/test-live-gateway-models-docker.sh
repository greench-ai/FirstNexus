...
elif [[ -n "${NEXISCLAW_LIVE_GATEWAY_PROVIDERS:-}" ]]; then
  while IFS= read -r auth_dir; do
    [[ -n "$auth_dir" ]] || continue
    AUTH_DIRS+=("$auth_dir")
  done < <(FirstNexus_live_collect_auth_dirs_from_csv "${NEXISCLAW_LIVE_GATEWAY_PROVIDERS:-}")
  while IFS= read -r auth_file; do
    [[ -n "$auth_file" ]] || continue
    AUTH_FILES+=("$auth_file")
  done < <(FirstNexus_live_collect_auth_files_from_csv "${NEXISCLAW_LIVE_GATEWAY_PROVIDERS:-}")
else
  while IFS= read -r auth_dir; do
    [[ -n "$auth_dir" ]] || continue
    AUTH_DIRS+=("$auth_dir")
...
CONTAINER_NODE_OPTIONS="${NEXISCLAW_DOCKER_NODE_OPTIONS:-${NODE_OPTIONS:-}}"
if [[ -z "$(FirstNexus_live_trim "$CONTAINER_NODE_OPTIONS")" ]]; then
...
DOCKER_RUN_ARGS=(docker run --rm -t \
  ...
  -e NEXISCLAW_LIVE_TEST_QUIET="${NEXISCLAW_LIVE_TEST_QUIET:-}" \
  -e NEXISCLAW_LIVE_WRAPPER_HEARTBEAT_MS="${NEXISCLAW_LIVE_WRAPPER_HEARTBEAT_MS:-}" \
  -e NEXISCLAW_LIVE_REQUIRE_PROFILE_KEYS="${NEXISCLAW_LIVE_REQUIRE_PROFILE_KEYS:-}" \
  -e NEXISCLAW_LIVE_GATEWAY_MODELS="${NEXISCLAW_LIVE_GATEWAY_MODELS:-modern}" \
  -e NEXISCLAW_LIVE_GATEWAY_PROVIDERS="${NEXISCLAW_LIVE_GATEWAY_PROVIDERS:-}" \
  -e NEXISCLAW_LIVE_GATEWAY_SMOKE="${NEXISCLAW_LIVE_GATEWAY_SMOKE:-1}" \
  -e NEXISCLAW_LIVE_GATEWAY_MAX_MODELS="${NEXISCLAW_LIVE_GATEWAY_MAX_MODELS:-8}" \
  -e NEXISCLAW_LIVE_GATEWAY_HEARTBEAT_MS="${NEXISCLAW_LIVE_GATEWAY_HEARTBEAT_MS:-}" \
  -e NEXISCLAW_LIVE_GATEWAY_STEP_TIMEOUT_MS="${NEXISCLAW_LIVE_GATEWAY_STEP_TIMEOUT_MS:-45000}" \
  -e NEXISCLAW_LIVE_GATEWAY_MODEL_TIMEOUT_MS="${NEXISCLAW_LIVE_GATEWAY_MODEL_TIMEOUT_MS:-90000}" \
