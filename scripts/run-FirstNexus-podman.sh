#!/usr/bin/env bash
# Rootless FirstNexus in Podman: run after one-time setup.
#
# One-time setup (from repo root): ./scripts/podman/setup.sh
# Then:
#   ./scripts/run-FirstNexus-podman.sh launch        # Start gateway
#   ./scripts/run-FirstNexus-podman.sh launch setup  # Onboarding wizard
#
# Manage the running container from the host CLI:
#   FirstNexus --container FirstNexus dashboard --no-open
#   FirstNexus --container FirstNexus channels login
#
...
load_podman_env_file() {
  ...
  while IFS= read -r line <&9 || [[ -n "$line" ]]; do
    ...
    case "$key" in
      NEXISCLAW_GATEWAY_TOKEN|NEXISCLAW_PODMAN_CONTAINER|NEXISCLAW_PODMAN_IMAGE|NEXISCLAW_IMAGE|NEXISCLAW_PODMAN_PULL|NEXISCLAW_PODMAN_GATEWAY_HOST_PORT|NEXISCLAW_GATEWAY_PORT|NEXISCLAW_PODMAN_BRIDGE_HOST_PORT|NEXISCLAW_BRIDGE_PORT|NEXISCLAW_GATEWAY_BIND|NEXISCLAW_PODMAN_USERNS|NEXISCLAW_BIND_MOUNT_OPTIONS|NEXISCLAW_PODMAN_PUBLISH_HOST)
        ;;
      *)
        continue
        ;;
...
if [[ "$(id -u)" -eq 0 ]]; then
  fail "Run run-FirstNexus-podman.sh as your normal user so Podman stays rootless."
fi
...
CONFIG_DIR="${NEXISCLAW_CONFIG_DIR:-$EFFECTIVE_HOME/.FirstNexus}"
ENV_FILE="${NEXISCLAW_PODMAN_ENV:-$CONFIG_DIR/.env}"
...
CONFIG_DIR="${NEXISCLAW_CONFIG_DIR:-$EFFECTIVE_HOME/.FirstNexus}"
ENV_FILE="${NEXISCLAW_PODMAN_ENV:-$CONFIG_DIR/.env}"
WORKSPACE_DIR="${NEXISCLAW_WORKSPACE_DIR:-$CONFIG_DIR/workspace}"
CONTAINER_NAME="${NEXISCLAW_PODMAN_CONTAINER:-FirstNexus}"
...
validate_single_line_value "image name" "$NEXISCLAW_IMAGE"
...
resolve_config_gateway_bind() {
  ...
  NEXISCLAW_CONTAINER="" NEXISCLAW_CONFIG_DIR="$config_dir" \
    FirstNexus config get gateway.bind 2>/dev/null || true
}
...
# NEXISCLAW_GATEWAY_BIND first, then gateway.bind in local config.
CONFIG_GATEWAY_BIND="$(resolve_config_gateway_bind "$CONFIG_DIR")"
GATEWAY_BIND="${NEXISCLAW_GATEWAY_BIND:-${CONFIG_GATEWAY_BIND:-lan}}"
...
create_token_env_file() {
  ...
  chmod 600 "$tmp"
  printf 'NEXISCLAW_GATEWAY_TOKEN=%s\n' "$token" >"$tmp"
  printf '%s' "$tmp"
}
...
sync_local_control_ui_origins_via_cli() {
  ...
  if ! command -v FirstNexus >/dev/null 2>&1; then
    echo "Warning: FirstNexus not found; unable to sync gateway.controlUi.allowedOrigins in $file." >&2
    return 0
  fi
  if ! command -v python3 >/dev/null 2>&1; then
    NEXISCLAW_CONTAINER="" NEXISCLAW_CONFIG_DIR="$config_dir" \
      FirstNexus config set gateway.controlUi.allowedOrigins \
  ...
  allowed_json="$(
    NEXISCLAW_CONTAINER="" NEXISCLAW_CONFIG_DIR="$config_dir" \
      FirstNexus config get gateway.controlUi.allowedOrigins --json 2>/dev/null || true
  )"
...
PY
  ...
  NEXISCLAW_CONTAINER="" NEXISCLAW_CONFIG_DIR="$config_dir" \
    FirstNexus config set gateway.controlUi.allowedOrigins "$merged_json" --strict-json >/dev/null
}
...
if [[ -z "${NEXISCLAW_GATEWAY_TOKEN:-}" ]]; then
  export NEXISCLAW_GATEWAY_TOKEN="$(generate_token_hex_32)"
  mkdir -p "$(dirname "$ENV_FILE")"
  ensure_safe_existing_dir "env file directory" "$(dirname "$ENV_FILE")"
  upsert_env_var "$ENV_FILE" "NEXISCLAW_GATEWAY_TOKEN" "$NEXISCLAW_GATEWAY_TOKEN"
  echo "Generated NEXISCLAW_GATEWAY_TOKEN and wrote it to $ENV_FILE." >&2
fi

CONFIG_JSON="$CONFIG_DIR/FirstNexus.json"
...
PODMAN_USERNS="${NEXISCLAW_PODMAN_USERNS:-keep-id}"
USERNS_ARGS=()
RUN_USER_ARGS=()
case "$PODMAN_USERNS" in
  ...
  *)
    echo "Unsupported NEXISCLAW_PODMAN_USERNS=$PODMAN_USERNS (expected: keep-id, auto, host)." >&2
    exit 2
    ;;
esac
...
if [[ "$PODMAN_USERNS" == "keep-id" ]]; then
  ...
  echo "Starting container without --user (NEXISCLAW_PODMAN_USERNS=$PODMAN_USERNS), mounts may require ownership fixes." >&2
fi

SELINUX_MOUNT_OPTS=""
if [[ -z "${NEXISCLAW_BIND_MOUNT_OPTIONS:-}" ]]; then
  ...
  SELINUX_MOUNT_OPTS="${NEXISCLAW_BIND_MOUNT_OPTIONS#:}"
  [[ -n "$SELINUX_MOUNT_OPTS" ]] && SELINUX_MOUNT_OPTS=",$SELINUX_MOUNT_OPTS"
fi

if [[ "$RUN_SETUP" == true ]]; then
  TOKEN_ENV_FILE="$(create_token_env_file "$ENV_FILE" "$NEXISCLAW_GATEWAY_TOKEN")"
  podman run --pull="$PODMAN_PULL" --rm -it \
    --init \
    "${USERNS_ARGS[@]}" "${RUN_USER_ARGS[@]}" \
    -e HOME=/home/node -e TERM=xterm-256color -e BROWSER=echo \
    -e NPM_CONFIG_CACHE=/home/node/.FirstNexus/.npm \
    -e NEXISCLAW_NO_RESPAWN=1 \
    --env-file "$TOKEN_ENV_FILE" \
    -v "$CONFIG_DIR:/home/node/.FirstNexus:rw${SELINUX_MOUNT_OPTS}" \
    -v "$WORKSPACE_DIR:/home/node/.FirstNexus/workspace:rw${SELINUX_MOUNT_OPTS}" \
    "$NEXISCLAW_IMAGE" \
...
echo "podman exec -it $CONTAINER_NAME FirstNexus dashboard --no-open"
echo "podman exec -it $CONTAINER_NAME FirstNexus devices approve --latest  # if pairing required"
