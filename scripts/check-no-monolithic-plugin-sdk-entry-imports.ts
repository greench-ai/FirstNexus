...
import { discoverFirstNexusPlugins } from "../src/plugins/discovery.js";
...
const ROOT_IMPORT_PATTERN = /["']FirstNexus\/plugin-sdk["']/;
const LEGACY_COMPAT_IMPORT_PATTERN = /["']FirstNexus\/plugin-sdk\/compat["']/;
const LEGACY_BROAD_SUBPATH_PATTERNS = [
  {
    pattern: /["']FirstNexus\/plugin-sdk\/channel-runtime["']/,
    label: "FirstNexus/plugin-sdk/channel-runtime",
  },
  {
    pattern: /["']FirstNexus\/plugin-sdk\/config-runtime["']/,
    label: "FirstNexus/plugin-sdk/config-runtime",
  },
  {
    pattern: /["']FirstNexus\/plugin-sdk\/infra-runtime["']/,
    label: "FirstNexus/plugin-sdk/infra-runtime",
  },
] as const;
...
function main() {
  ...
  if (
    ...
    if (monolithicOffenders.length > 0) {
      console.error(
        "Bundled plugin source files must not import monolithic FirstNexus/plugin-sdk.",
      );
    ...
    if (legacyCompatOffenders.length > 0) {
      console.error(
        "Bundled plugin source files must not import legacy FirstNexus/plugin-sdk/compat.",
      );
    ...
    if (
      ...
      console.error(
        "Use focused FirstNexus/plugin-sdk/<domain> subpaths for bundled plugins; root, compat, and broad runtime barrels are legacy surfaces only.",
      );
    }
    process.exit(1);
  }
