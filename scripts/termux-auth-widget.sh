...
# FirstNexus Auth Widget for Termux
...
# Check auth status
termux-toast "Checking FirstNexus auth..."

STATUS=$(ssh "$SERVER" '$HOME/FirstNexus/scripts/claude-auth-status.sh simple' 2>&1)
EXIT_CODE=$?

case "$STATUS" in
    OK)
        # Get remaining time
        DETAILS=$(ssh "$SERVER" '$HOME/FirstNexus/scripts/claude-auth-status.sh json' 2>&1)
...
3. SSH to server and paste key"
                ...
                termux-toast "Run: ssh $SERVER '$HOME/FirstNexus/scripts/mobile-reauth.sh'"
                ;;
            *)
    ...
    CLAUDE_EXPIRED|NEXISCLAW_EXPIRED)
...
2. Return here and tap OK to SSH"
                ...
                ;;
            *)
                termux-toast "Warning: FirstNexus won't work until re-auth"
                ;;
        esac
        ;;
