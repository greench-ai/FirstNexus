...
type DriverMode = "token" | "webhook" | "nexusclaw";
...
function resolveStateDir(): string {
  const override = process.env.NEXUSCLAW_STATE_DIR?.trim() || process.env.CLAWDBOT_STATE_DIR?.trim();
  if (override) {
    return override.startsWith("~")
      ? path.resolve(process.env.HOME || "", override.slice(1))
      : path.resolve(override);
  }
  const home = process.env.NEXUSCLAW_HOME?.trim() || process.env.HOME || "";
  return path.join(home, ".nexusclaw");
}
...
function usage(): string {
  return (
    ...
    "--channel <discord-channel-id> [--token <driver-token> | --driver webhook --bot-token <bot-token> | --driver nexusclaw] [options]\n\n" +
    "Manual live smoke only (not CI). Sends a plain-language instruction in Discord and verifies:\n" +
    "1) NexusClaw spawned an ACP thread binding\n" +
    "2) agent replied in that bound thread with the expected ACK token\n\n" +
    "Options:\n" +
    "  --channel <id>               Parent Discord channel id (required)\n" +
    "  --driver <token|webhook|nexusclaw> Driver transport mode (default: token)\n" +
    ...
    "  --nexusclaw-bin <path>        NexusClaw CLI binary for driver=nexusclaw (default: nexusclaw)\n" +
    "  --json                       Emit JSON output\n" +
    "\n" +
    "Environment fallbacks:\n" +
    "  NEXUSCLAW_DISCORD_SMOKE_CHANNEL_ID\n" +
    "  NEXUSCLAW_DISCORD_SMOKE_DRIVER\n" +
    "  NEXUSCLAW_DISCORD_SMOKE_DRIVER_TOKEN\n" +
    "  NEXUSCLAW_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX\n" +
    "  NEXUSCLAW_DISCORD_SMOKE_BOT_TOKEN\n" +
    "  NEXUSCLAW_DISCORD_SMOKE_BOT_TOKEN_PREFIX\n" +
    "  NEXUSCLAW_DISCORD_SMOKE_AGENT\n" +
    "  NEXUSCLAW_DISCORD_SMOKE_MENTION_USER_ID\n" +
    "  NEXUSCLAW_DISCORD_SMOKE_TIMEOUT_MS\n" +
    "  NEXUSCLAW_DISCORD_SMOKE_POLL_MS\n" +
    "  NEXUSCLAW_DISCORD_SMOKE_THREAD_BINDINGS_PATH\n" +
    "  NEXUSCLAW_DISCORD_SMOKE_NEXUSCLAW_BIN"
  );
}
...
function parseArgs(): Args {
  const channelId =
    resolveArg("--channel") ||
    process.env.NEXUSCLAW_DISCORD_SMOKE_CHANNEL_ID ||
    process.env.CLAWDBOT_DISCORD_SMOKE_CHANNEL_ID ||
    "";
  const driverModeRaw =
    resolveArg("--driver") ||
    process.env.NEXUSCLAW_DISCORD_SMOKE_DRIVER ||
  ...
  const driverMode: DriverMode =
    normalizedDriverMode === "webhook"
      ? "webhook"
      : normalizedDriverMode === "nexusclaw"
        ? "nexusclaw"
  ...
  const driverToken =
    resolveArg("--token") ||
    process.env.NEXUSCLAW_DISCORD_SMOKE_DRIVER_TOKEN ||
    process.env.CLAWDBOT_DISCORD_SMOKE_DRIVER_TOKEN ||
    "";
  const driverTokenPrefix =
    resolveArg("--token-prefix") || process.env.NEXUSCLAW_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX || "Bot";
  const botToken =
    resolveArg("--bot-token") ||
    process.env.NEXUSCLAW_DISCORD_SMOKE_BOT_TOKEN ||
    process.env.CLAWDBOT_DISCORD_SMOKE_BOT_TOKEN ||
    process.env.DISCORD_BOT_TOKEN ||
    "";
  const botTokenPrefix =
    resolveArg("--bot-token-prefix") ||
    process.env.NEXUSCLAW_DISCORD_SMOKE_BOT_TOKEN_PREFIX ||
    "Bot";
  const targetAgent =
    resolveArg("--agent") ||
    process.env.NEXUSCLAW_DISCORD_SMOKE_AGENT ||
    process.env.CLAWDBOT_DISCORD_SMOKE_AGENT ||
    "codex";
  const mentionUserId =
    resolveArg("--mention") ||
    process.env.NEXUSCLAW_DISCORD_SMOKE_MENTION_USER_ID ||
    process.env.CLAWDBOT_DISCORD_SMOKE_MENTION_USER_ID ||
    undefined;
  const instruction =
    resolveArg("--instruction") ||
    process.env.NEXUSCLAW_DISCORD_SMOKE_INSTRUCTION ||
    process.env.CLAWDBOT_DISCORD_SMOKE_INSTRUCTION ||
    undefined;
  const timeoutMs = parseNumber(
    resolveArg("--timeout-ms") || process.env.NEXUSCLAW_DISCORD_SMOKE_TIMEOUT_MS,
    240_000,
  );
  const pollMs = parseNumber(
    resolveArg("--poll-ms") || process.env.NEXUSCLAW_DISCORD_SMOKE_POLL_MS,
    1_500,
  );
  ...
  const threadBindingsPath =
    ...
    process.env.NEXUSCLAW_DISCORD_SMOKE_THREAD_BINDINGS_PATH ||
    defaultBindingsPath;
  const nexusclawBin =
    resolveArg("--nexusclaw-bin") || process.env.NEXUSCLAW_DISCORD_SMOKE_NEXUSCLAW_BIN || "nexusclaw";
...
async function nexusclawCliJson<T>(params: { nexusclawBin: string; args: string[] }): Promise<T> {
  const result = await execFileAsync(params.nexusclawBin, params.args, {
    maxBuffer: 8 * 1024 * 1024,
    env: process.env,
  });
  const stdout = (result.stdout || "").trim();
  if (!stdout) {
    throw new Error(`nexusclaw ${params.args.join(" ")} returned empty stdout`);
  }
...
async function readMessagesWithOpenclaw(params: {
  ...
  const response = await nexusclawCliJson<{
    payload?: {
      messages?: DiscordMessage[];
    };
  }>({
    nexusclawBin: params.nexusclawBin,
    args: [
      "message",
      "read",
      "--channel",
...
async function loadParentRecentMessages(params: {
  ...
  if (params.args.driverMode === "nexusclaw") {
    return await readMessagesWithOpenclaw({
      nexusclawBin: params.args.nexusclawBin,
  ...
  try {
    if (args.driverMode === "token") {
      ...
      const webhook = await discordApi<{ id: string; token?: string | null }>({
        ...
        authHeader: botAuthHeader,
        body: {
          name: `nexusclaw-acp-smoke-${smokeId.slice(-8)}`,
        },
      });
      ...
      setupStage = "send-message";
      const sent = await nexusclawCliJson<{
        payload?: {
          result?: {
            messageId?: string;
          };
        ...
        nexusclawBin: args.nexusclawBin,
        args: [
          "message",
          "send",
          "--channel",
      ...
      if (!sentMessageId) {
        throw new Error("nexusclaw message send did not return payload.result.messageId");
      }
    }
  } catch (err) {
    return {
  ...
  try {
    ...
    if (!ackMessage) {
      ...
      return {
        ...
        stage: "wait-ack",
        smokeId,
        error: `Thread bound (${threadId}) but timed out waiting for ACK token "${ackToken}" from NexusClaw.`,
        diagnostics: {
          bindingCandidates: [
            {
