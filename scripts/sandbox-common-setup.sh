...
BASE_IMAGE="${BASE_IMAGE:-nexusclaw-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-nexusclaw-sandbox-common:bookworm-slim}"
...
NEXUSCLAW_DOCKER_BUILD_USE_BUILDX="${NEXUSCLAW_DOCKER_BUILD_USE_BUILDX:-0}"
NEXUSCLAW_DOCKER_BUILD_CACHE_FROM="${NEXUSCLAW_DOCKER_BUILD_CACHE_FROM:-}"
NEXUSCLAW_DOCKER_BUILD_CACHE_TO="${NEXUSCLAW_DOCKER_BUILD_CACHE_TO:-}"
...
if [ "${NEXUSCLAW_DOCKER_BUILD_USE_BUILDX}" = "1" ]; then
  build_cmd=(docker buildx build --load)
  if [ -n "${NEXUSCLAW_DOCKER_BUILD_CACHE_FROM}" ]; then
    build_cmd+=(--cache-from "${NEXUSCLAW_DOCKER_BUILD_CACHE_FROM}")
  fi
  if [ -n "${NEXUSCLAW_DOCKER_BUILD_CACHE_TO}" ]; then
    build_cmd+=(--cache-to "${NEXUSCLAW_DOCKER_BUILD_CACHE_TO}")
  fi
fi
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=nexusclaw.sandbox=1)
NOTE
