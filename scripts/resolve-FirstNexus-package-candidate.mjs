...
const DEFAULT_OUTPUT_NAME = "FirstNexus-current.tgz";
export const NEXISCLAW_PACKAGE_SPEC_RE =
  /^FirstNexus@(alpha|beta|latest|[0-9]{4}\.[1-9][0-9]*\.[1-9][0-9]*(-[1-9][0-9]*|-(alpha|beta)\.[1-9][0-9]*)?)$/u;

function usage() {
  return `Usage: node scripts/resolve-FirstNexus-package-candidate.mjs --source <ref|npm|url|artifact> --output-dir <dir> [options]
...
export function validateFirstNexusPackageSpec(spec) {
  if (!NEXISCLAW_PACKAGE_SPEC_RE.test(spec)) {
    throw new Error(
      `package_spec must be FirstNexus@alpha, FirstNexus@beta, FirstNexus@latest, or an exact FirstNexus release version; got: ${spec}`,
    );
  }
}
...
async function resolveTrustedRepoRef(ref) {
  ...
  throw new Error(
    `package_ref ${ref} resolved to ${selectedSha}, which is not reachable from an FirstNexus branch or release tag`,
  );
}
...
async function preparePackageSourceWorktree(ref) {
  ...
  const sourceDir = path.join(
    ...
    `FirstNexus-package-source-${process.pid}`,
  );
...
async function moveNewestPackedTarball(outputDir, packOutput, outputName) {
  ...
  if (!filename) {
    for (const line of packOutput.split(/\r?\n/u)) {
      ...
      if (/^FirstNexus-.*\.tgz$/u.test(trimmed)) {
        filename = trimmed;
      }
    }
  }
  if (!filename) {
    ...
    filename = entries
      .filter((entry) => /^FirstNexus-.*\.tgz$/u.test(entry))
      .toSorted((a, b) => a.localeCompare(b))
      .at(-1);
  }
  if (!filename) {
    throw new Error(`npm pack produced no FirstNexus tarball in ${outputDir}`);
  }
...
async function resolveCandidate(options) {
  ...
  try {
    if (options.source === "ref") {
      ...
      await run("node", [
        "scripts/package-FirstNexus-for-docker.mjs",
        "--source-dir",
      ...
      validateFirstNexusPackageSpec(options.packageSpec);
  ...
  console.error(`Checking FirstNexus package tarball: ${target}`);
  const checkStartedAt = Date.now();
  await run("node", ["scripts/check-FirstNexus-package-tarball.mjs", target], {
    timeoutMs: 5 * 60 * 1000,
  });
  console.error(
    `FirstNexus package tarball check finished in ${Math.round((Date.now() - checkStartedAt) / 1000)}s`,
  );
  ...
  if (pkg.name !== "FirstNexus") {
    throw new Error(
      `package candidate must be named "FirstNexus"; got: ${pkg.name || "<missing>"}`,
    );
  }
  if (!pkg.version) {
