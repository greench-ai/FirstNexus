#!/bin/bash
# Setup FirstNexus Auth Management System
...
echo "=== FirstNexus Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'FirstNexus-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  FirstNexus message: Send warning via FirstNexus itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/FirstNexus-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/FirstNexus-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/FirstNexus-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now FirstNexus-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status FirstNexus-auth-monitor.timer"
echo ""
echo "Quick commands:"
