#!/bin/bash
# Setup NexusClaw Auth Management System
...
echo "=== NexusClaw Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'nexusclaw-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  NexusClaw message: Send warning via NexusClaw itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/nexusclaw-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/nexusclaw-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/nexusclaw-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now nexusclaw-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status nexusclaw-auth-monitor.timer"
echo ""
echo "Quick commands:"
