#!/usr/bin/env node
// Builds the FirstNexus package artifact used by Docker E2E.
...
async function newestFirstNexusTarball(outputDir, packOutput) {
  let fromOutput = "";
  ...
  const packed = entries
    ...
    .at(-1);
  if (!packed) {
    throw new Error(`missing packed FirstNexus tarball in ${outputDir}`);
  }
...
async function main() {
  ...
  if (!options.skipBuild) {
    console.error("==> Building FirstNexus package artifacts");
    await run("pnpm", ["build"], sourceDir);
    console.error("==> Building FirstNexus Control UI artifacts");
    await run("pnpm", ["ui:build"], sourceDir);
  }

  console.error("==> Writing FirstNexus package inventory");
  await run(
    "node",
    [
      "--import",
  ...
  console.error("==> Packing FirstNexus package");
  const packOutput = await runCapture(
    "npm",
    ["pack", "--silent", "--ignore-scripts", "--pack-destination", outputDir],
    sourceDir,
  );
  let tarball = await newestFirstNexusTarball(outputDir, packOutput);
  ...
  console.error("==> Checking FirstNexus package tarball");
  const checkStartedAt = Date.now();
  await run(
    "node",
    [path.join(ROOT_DIR, "scripts/check-FirstNexus-package-tarball.mjs"), tarball],
    sourceDir,
    { timeoutMs: 5 * 60 * 1000 },
  );
  console.error(
