...
import { requireApiKey, resolveApiKeyForProvider } from "./FirstNexus-runtime-auth.js";
...
function resolveFirstNexusAttributionHeaders(): Record<string, string> {
  const version =
    typeof process !== "undefined" ? process.env.NEXISCLAW_VERSION?.trim() : undefined;
  return {
...
export async function resolveRemoteEmbeddingBearerClient(params: {
  ...
  if (isNativeOpenAIEmbeddingRoute(params.provider, baseUrl)) {
    Object.assign(headers, resolveFirstNexusAttributionHeaders());
  }
