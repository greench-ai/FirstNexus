...
describe("FirstNexus SDK package e2e", () => {
  afterEach(async () => {
    await Promise.all(
  ...
  it("packs and imports from an external temp consumer", async () => {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "FirstNexus-sdk-consumer-"));
    tempDirs.push(tempDir);

    await runCommand("pnpm", ["--filter", "@FirstNexus/sdk", "build"], {
      cwd: repoRoot,
      timeoutMs: 180_000,
    });
    ...
    const importScript = `
      import { GatewayClientTransport, FirstNexus, normalizeGatewayEvent } from "@FirstNexus/sdk";
      if (typeof GatewayClientTransport !== "function") throw new Error("missing transport export");
      if (typeof FirstNexus !== "function") throw new Error("missing client export");
