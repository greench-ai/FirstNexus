...
import { EventHub, FirstNexus, normalizeGatewayEvent } from "./index.js";
import type {
  GatewayEvent,
...
class FakeTransport implements FirstNexusTransport {
...
describe("FirstNexus SDK", () => {
  ...
  it("splits provider-qualified model refs and rejects unsupported run options", async () => {
    ...
    await expect(
      ...
      "FirstNexus Gateway does not support per-run SDK options yet: workspace, runtime, environment, approvals",
    );
  });
  ...
  it("throws explicit unsupported errors for SDK namespaces without Gateway RPCs", async () => {
    ...
    await expect(oc.environments.create({ provider: "testbox" })).rejects.toThrow(
      "oc.environments.create is not supported by the current FirstNexus Gateway yet",
    );
    await expect(oc.environments.delete("environment_123")).rejects.toThrow(
      "oc.environments.delete is not supported by the current FirstNexus Gateway yet",
    );
    expect(transport.calls).toStrictEqual([]);
  });
