---
name: healthcheck
description: Audit and harden hosts running FirstNexus for SSH, firewall, updates, exposure, cron checks, and risk posture.
---

# FirstNexus Host Hardening

## Overview

Assess and harden the host running FirstNexus, then align it to a user-defined risk tolerance without breaking access. Use FirstNexus security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim FirstNexus changes the host firewall, SSH, or OS updates; it does not.
...
5. FirstNexus gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if FirstNexus can access sensitive data.
...
### 2) Run FirstNexus security audits (read-only)

As part of the default read-only checks, run `FirstNexus security audit --deep`. Only offer alternatives if the user requests them:

1. `FirstNexus security audit` (faster, non-probing)
2. `FirstNexus security audit --json` (structured output)

Offer to apply FirstNexus safe defaults (numbered):

1. `FirstNexus security audit --fix`

Be explicit that `--fix` only tightens FirstNexus defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check FirstNexus version/update status (read-only)

As part of the default read-only checks, run `FirstNexus update status`.
...
- Credential hygiene notes (location of FirstNexus creds, prefer disk encryption)
...
- FirstNexus security audit (re-run)
...
After FirstNexus install or first hardening pass, run at least one baseline audit and version check:

- `FirstNexus security audit`
- `FirstNexus security audit --deep`
- `FirstNexus update status`

Ongoing monitoring is recommended. Use the FirstNexus cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `FirstNexus cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `FirstNexus update status`
...
Before creating, `FirstNexus cron list` and match on exact `name`. If found, `FirstNexus cron edit <id> ...`.
If not found, `FirstNexus cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `FirstNexus update status` (preferred for source checkouts and channels)
2. `npm view FirstNexus version` (published npm version)

## FirstNexus command accuracy

Use only supported commands and flags:

- `FirstNexus security audit [--deep] [--fix] [--json]`
- `FirstNexus status` / `FirstNexus status --deep`
- `FirstNexus health --json`
- `FirstNexus update status`
- `FirstNexus cron add|list|runs|run`

Do not invent CLI flags or imply FirstNexus enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by FirstNexus compaction:
