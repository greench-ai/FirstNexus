...
- Re-open anytime: `FirstNexus dashboard` (copies link, opens browser if possible, shows SSH hint if headless).
- If clipboard and browser delivery fail, `FirstNexus dashboard` still prints the
  clean URL and tells you to use the token from `NEXISCLAW_GATEWAY_TOKEN` or
...
- **Shared-secret token source**: `gateway.auth.token` (or
  `NEXISCLAW_GATEWAY_TOKEN`); `FirstNexus dashboard` can pass it via URL fragment
...
- If `gateway.auth.token` is SecretRef-managed, `FirstNexus dashboard`
...
- If `gateway.auth.token` is configured as a SecretRef and is unresolved in your
  current shell, `FirstNexus dashboard` still prints a non-tokenized URL plus
  actionable auth setup guidance.
- **Shared-secret password**: use the configured `gateway.auth.password` (or
  `NEXISCLAW_GATEWAY_PASSWORD`). The dashboard does not persist passwords across
  reloads.
...
- Ensure the gateway is reachable (local: `FirstNexus status`; remote: SSH tunnel `ssh -N -L 18789:127.0.0.1:18789 user@host` then open `http://127.0.0.1:18789/`).
...
- Retrieve or supply the shared secret from the gateway host:
  - Token: `FirstNexus config get gateway.auth.token`
  ...
  - SecretRef-managed token: resolve the external secret provider or export
    `NEXISCLAW_GATEWAY_TOKEN` in this shell, then rerun `FirstNexus dashboard`
  - No shared secret configured: `FirstNexus doctor --generate-gateway-token`
