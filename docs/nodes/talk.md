...
## Config (`~/.FirstNexus/FirstNexus.json`)
...
- `consultThinkingLevel`: optional thinking level override for the full FirstNexus agent run behind realtime `FirstNexus_agent_consult` calls.
- `consultFastMode`: optional fast-mode override for realtime `FirstNexus_agent_consult` calls.
...
- `realtime.instructions`: appends provider-facing system instructions to FirstNexus's built-in realtime prompt. Use it for voice style and tone; FirstNexus keeps the default `FirstNexus_agent_consult` guidance.
...
- Browser realtime Talk uses `talk.client.toolCall` for `FirstNexus_agent_consult` instead of exposing `chat.send` to provider-owned browser sessions.
...
- macOS local MLX playback uses the bundled `FirstNexus-mlx-tts` helper when present, or an executable on `PATH`. Set `NEXISCLAW_MLX_TTS_BIN` to point at a custom helper binary during development.
