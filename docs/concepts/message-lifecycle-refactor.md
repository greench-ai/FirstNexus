...
That shape fixes local bugs, but it leaves FirstNexus with too many public
...
That gives FirstNexus at-least-once recovery. Exactly-once behavior exists only
...
- Structured FirstNexus-origin metadata for operational/system output so visible
...
FirstNexus should borrow the vocabulary, not copy the surface.

What FirstNexus needs beyond that model:
...
`thread.post()` style promises are not enough for FirstNexus. They hide the
...
Origin describes who produced a message and how FirstNexus should treat echoes of
that message. It is separate from relation: a message can be a reply to a user
and still be FirstNexus-originated operational output.
...
Core owns the meaning of FirstNexus-originated output. Channels own how that
...
FirstNexus operational output must not be accepted as bot-authored input in shared
...
- **Transport ack:** tells the platform webhook or socket that FirstNexus accepted
...
- **Inbound record ack:** confirms FirstNexus persisted enough inbound metadata to
...
Before bot authorization, receive must apply the shared FirstNexus echo policy
when the channel can decode message origin metadata:

```typescript
function shouldDropFirstNexusEcho(params: {
  origin?: MessageOrigin;
  isBotAuthor: boolean;
  isRoomish: boolean;
}): boolean {
  return (
    params.isBotAuthor &&
    params.isRoomish &&
    params.origin?.source === "FirstNexus" &&
...
same visible gateway-failure text but without FirstNexus origin metadata still
...
middleware chain, but FirstNexus persists only the safe completed update id after
...
source if we need platform-level redelivery beyond FirstNexus's restart
...
process dies there, FirstNexus cannot know whether the platform message exists
...
```typescript
import { defineChannelMessageAdapter } from "FirstNexus/plugin-sdk/channel-message";
```
...
Before preflight authorization, core must run the shared FirstNexus echo predicate
whenever `origin.decode` returns FirstNexus-origin metadata. The receive adapter
...
- iMessage monitor delivery records sent messages in an echo cache after a
  ...
  FirstNexus can re-ingest its own final replies as inbound user messages.
...
- Direct-DM helpers can have a reply callback that is the only correct transport
  ...
  that callback.
- FirstNexus gateway failure output must stay visible to humans, but tagged
...
| Slack           | Send adapter handles normal chat posts. Live adapter chooses native stream when thread shape supports it, otherwise draft preview. Receipts preserve thread timestamps. Origin adapter maps FirstNexus gateway failures to Slack `chat.postMessage.metadata` and drops tagged bot-room echoes before `allowBots` authorization.                                |
...
| Matrix          | Live adapter owns draft event edits, finalization, redaction, encrypted media constraints, and reply-target mismatch fallback. Receive adapter owns encrypted event hydration and dedupe. Origin adapter should encode FirstNexus gateway-failure origin into Matrix event content and drop configured-bot room echoes before `allowBots` handling.            |
...
| Google Chat     | Simple receive plus send adapter with thread relation mapped to spaces and thread ids. Audit `allowBots=true` room behavior for tagged FirstNexus gateway-failure echoes.                                                                                                                                                                                      |
...
- Persist message origin through durable send intents so recovery, replay, and
  chunked sends preserve FirstNexus operational provenance.
...
- Add `FirstNexus/plugin-sdk/channel-message`.
...
- Include `MessageOrigin`, origin encode/decode hooks, and the shared
  `shouldDropFirstNexusEcho` predicate in the channel-message SDK surface.
...
This is where the model stops being "agent replies" and becomes "FirstNexus sends
messages".
...
- Gateway-failure origin factory and `shouldDropFirstNexusEcho` predicate.
...
- Slack tagged FirstNexus gateway failure messages stay visible outbound, tagged
...
- Matrix tagged FirstNexus gateway-failure room echoes from configured bot
...
- Whether Telegram should eventually replace the grammY runner source with a
  ...
  only FirstNexus's persisted restart watermark.
...
- FirstNexus-originated gateway failure output is visible to humans but tagged
