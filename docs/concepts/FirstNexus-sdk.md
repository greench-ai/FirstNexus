---
summary: "Public FirstNexus App SDK for external apps, scripts, dashboards, CI jobs, and IDE extensions"
title: "FirstNexus App SDK"
sidebarTitle: "App SDK"
read_when:
  - You are building an external app, script, dashboard, CI job, or IDE extension that talks to FirstNexus
...
The **FirstNexus App SDK** is the public client API for apps outside the
FirstNexus process. Use `@FirstNexus/sdk` when a script, dashboard, CI job, IDE
...
<Note>
  ...
  `@FirstNexus/sdk` talks to the Gateway from outside FirstNexus.
  `FirstNexus/plugin-sdk/*` is only for plugins that run inside FirstNexus and
  register providers, channels, tools, hooks, or trusted runtimes.
</Note>

## What ships today

`@FirstNexus/sdk` ships with:

| Surface                   | Status  | What it does                                                                      |
| ------------------------- | ------- | --------------------------------------------------------------------------------- |
| `FirstNexus`              | Ready   | Main client entry point. Owns transport, connection, requests, and events.        |
...
`AgentRunParams`, `RunResult`, `RunStatus`, `FirstNexusEvent`,
`FirstNexusEventType`, `GatewayEvent`, `FirstNexusTransport`,
...
```typescript
import { FirstNexus } from "@FirstNexus/sdk";
...
`new FirstNexus({ gateway: "ws://..." })` is equivalent to `url`. The
...
For tests, pass an object that implements `FirstNexusTransport`:
...
The SDK normalizes raw Gateway events into a stable `FirstNexusEvent` envelope:

```typescript
type FirstNexusEvent = {
  version: 1;
  id: string;
  ts: number;
  type: FirstNexusEventType;
  runId?: string;
  sessionId?: string;
  sessionKey?: string;
  taskId?: string;
...
Task helpers use the durable task ledger that also backs `FirstNexus tasks`:
...
Use the App SDK when code lives outside FirstNexus:
...
Use the Plugin SDK when code runs inside FirstNexus:
...
App SDK code should import from `@FirstNexus/sdk`. Plugin code should import from
documented `FirstNexus/plugin-sdk/*` subpaths. Do not mix the two contracts.

## Related

- [FirstNexus App SDK API design](/reference/FirstNexus-sdk-api-design)
