...
FirstNexus handles inbound messages through a pipeline of session resolution, queueing, streaming, tool execution, and reasoning visibility. This page maps the path from inbound message to reply.
...
Channels can redeliver the same message after reconnects. FirstNexus keeps a
...
FirstNexus keeps that boundary explicit:
...
FirstNexus separates the **prompt body** from the **command body**:
...
FirstNexus can expose or hide model reasoning:
...
When a turn also has pending tool media, such as generated TTS audio, FirstNexus
strips the silent text but still delivers the media attachment.
FirstNexus resolves that behavior by conversation type:
...
FirstNexus also uses silent replies for internal runner failures that happen
