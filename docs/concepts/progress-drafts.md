...
read_when:
  ...
  - Explaining how FirstNexus updates one channel message while work is in progress
...
When progress drafts are enabled, FirstNexus creates one visible work-in-progress
...
That is usually enough. FirstNexus will pick an automatic one-word label, wait
...
FirstNexus sends the final answer normally and cleans up or stops updating the
...
The default label is `auto`, which chooses from FirstNexus's built-in
...
FirstNexus uses the same formatter for progress drafts and `/verbose`:
...
FirstNexus truncates long progress lines by default so repeated draft edits do not
...
With `toolProgress: false`, FirstNexus still suppresses the older standalone
...
When the final answer is ready, FirstNexus tries to keep the chat clean:

- If the draft can safely become the final answer, FirstNexus edits it in place.
- If the channel uses native progress streaming, FirstNexus finalizes that stream
...
- If the final answer has media, an approval prompt, an explicit reply target,
  too many chunks, or a failed edit/send, FirstNexus sends the final answer through
...
Check `streaming.progress.toolProgress`. If it is `false`, FirstNexus keeps the
