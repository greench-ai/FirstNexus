...
- `/usage cost` in chats: local cost summary aggregated from FirstNexus session logs.
- CLI: `FirstNexus status --usage` prints a full per-provider breakdown.
- CLI: `FirstNexus channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
...
- **MiniMax**: API key or MiniMax OAuth auth profile. FirstNexus treats
  ...
  quota, so FirstNexus inverts them before display; count-based fields win when
  present.
  ...
  - If the coding-plan endpoint returns `model_remains`, FirstNexus prefers the
...
can supply plugin-specific usage auth logic; otherwise FirstNexus falls back to
