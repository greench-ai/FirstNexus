...
FirstNexus gives agents tools to work across sessions, inspect status, and
...
After the target responds, FirstNexus can run a **reply-back loop** where the
...
the caller's current session; visible client labels such as `FirstNexus-tui` are
not session keys.
...
`subagents` is the control-plane helper for already spawned FirstNexus
...
the completion origin only identifies a channel FirstNexus can still reuse the
