...
Paste this into `FirstNexus.json` for a safe-default setup — plugin on, scoped to
...
With those enabled, FirstNexus can show:
...
case-insensitive. If `allowedChatIds` is non-empty and FirstNexus cannot resolve a
...
`config.promptOverride` replaces the default Active Memory prompt. FirstNexus
...
5. Verify memory search itself works with `FirstNexus memory status --deep`.
...
<AccordionGroup>
  <Accordion title="Embedding provider switched or stopped working">
    If `memorySearch.provider` is unset, FirstNexus auto-detects the first
  ...
  <Accordion title="Recall feels slow, empty, or inconsistent">
    ...
    - Run `FirstNexus memory status --deep` to inspect the memory-search backend
      and index health.
