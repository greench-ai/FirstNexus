---
summary: "How FirstNexus separates model providers, models, channels, and agent runtimes"
...
the finished turn to FirstNexus.
...
| ------------- | ------------------------------------- | --------------------------------------------------------------------- |
| Provider      | `openai`, `anthropic`, `openai-codex` | How FirstNexus authenticates, discovers models, and names model refs. |
| Model         | `gpt-5.5`, `claude-opus-4-6`          | The model selected for the agent turn.                                |
| Agent runtime | `pi`, `codex`, `claude-cli`           | The low level loop or backend that executes the prepared turn.        |
| Channel       | Telegram, Discord, Slack, WhatsApp    | Where messages enter and leave FirstNexus.                            |
...
`FirstNexus doctor --fix` removes old whole-agent runtime pins and rewrites
...
- **Embedded harnesses** run inside FirstNexus's prepared agent loop. Today this
...
| Surface                                          | FirstNexus name/config               | What it does                                                                                                   |
| ------------------------------------------------ | ------------------------------------ | -------------------------------------------------------------------------------------------------------------- |
...
the native app-server features available; `FirstNexus doctor --fix` owns legacy
...
That means FirstNexus selects an OpenAI model ref, then asks the Codex app-server
runtime to run the embedded agent turn. It does not mean "use API billing," and
it does not mean the channel, model provider catalog, or FirstNexus session store
becomes Codex.
...
4. If legacy config still contains **`openai-codex/*` model refs**, repair it to
   `openai/<model>` with `FirstNexus doctor --fix`; doctor keeps the Codex auth
...
| Surface                     | FirstNexus PI embedded                    | Codex app-server                                                              |
| --------------------------- | ----------------------------------------- | ----------------------------------------------------------------------------- |
| Model loop owner            | FirstNexus through the PI embedded runner | Codex app-server                                                              |
| Canonical thread state      | FirstNexus transcript                     | Codex thread, plus FirstNexus transcript mirror                               |
| FirstNexus dynamic tools    | Native FirstNexus tool loop               | Bridged through the Codex adapter                                             |
| Native shell and file tools | PI/FirstNexus path                        | Codex-native tools, bridged through native hooks where supported              |
| Context engine              | Native FirstNexus context assembly        | FirstNexus projects assembled context into the Codex turn                     |
| Compaction                  | FirstNexus or selected context engine     | Codex-native compaction, with FirstNexus notifications and mirror maintenance |
| Channel delivery            | FirstNexus                                | FirstNexus                                                                    |

This ownership split is the main design rule:

- If FirstNexus owns the surface, FirstNexus can provide normal plugin hook behavior.
- If the native runtime owns the surface, FirstNexus needs runtime events or native hooks.
- If the native runtime owns canonical thread state, FirstNexus should mirror and project context, not rewrite unsupported internals.

## Runtime selection

FirstNexus chooses an embedded runtime after provider and model resolution:
...
3. In `auto` mode, registered plugin runtimes can claim supported provider/model
   pairs.
4. If no runtime claims a turn in `auto` mode, FirstNexus uses PI as the
...
`FirstNexus doctor --fix` to remove stale whole-agent runtime config and convert
legacy runtime model refs where FirstNexus can preserve the intent.
...
FirstNexus routes PI internally through the legacy Codex-auth transport while
keeping the public model ref as `openai/*`. Stale OpenAI PI session pins are
ignored by runtime selection and can be cleaned with `FirstNexus doctor --fix`.

If `FirstNexus doctor` warns that the `codex` plugin is enabled while
`openai-codex/*` remains in config, treat that as legacy route state. Run
`FirstNexus doctor --fix` to rewrite it to `openai/*` with the Codex runtime.

## Compatibility contract

When a runtime is not PI, it should document what FirstNexus surfaces it supports.
...
| Who owns canonical thread history?     | Determines whether FirstNexus can edit history or only mirror it.                                   |
| Do FirstNexus dynamic tools work?      | Messaging, sessions, cron, and FirstNexus-owned tools rely on this.                                 |
| Do dynamic tool hooks work?            | Plugins expect `before_tool_call`, `after_tool_call`, and middleware around FirstNexus-owned tools. |
