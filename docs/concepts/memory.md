---
summary: "How FirstNexus remembers things across sessions"
...
FirstNexus remembers things by writing **plain Markdown files** in your agent's
...
These files live in the agent workspace (default `~/.FirstNexus/workspace`).
...
If `MEMORY.md` grows past the bootstrap file budget, FirstNexus keeps the file on
...
`FirstNexus doctor` to see raw vs injected sizes and truncation status.
...
for that case. FirstNexus infers them in a hidden background pass, scopes them to
...
<Info>
FirstNexus auto-detects your embedding provider from available API keys. If you
...
Before [compaction](/concepts/compaction) summarizes your conversation, FirstNexus
...
```bash
FirstNexus memory rem-backfill --rollback
FirstNexus memory rem-backfill --rollback-short-term
```

## CLI

```bash
FirstNexus memory status          # Check index status and provider
FirstNexus memory search "query"  # Search from the command line
FirstNexus memory index --force   # Rebuild the index
```
