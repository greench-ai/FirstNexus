...
read_when:
  - You want FirstNexus to remember natural follow-ups
  - You want to understand how inferred check-ins differ from reminders
  - You want to review or dismiss follow-up commitments
---

Commitments are short-lived follow-up memories. When enabled, FirstNexus can
notice that a conversation created a future check-in opportunity and remember
to bring it back later.

Examples:

- You mention an interview tomorrow. FirstNexus may check in afterward.
- You say you are exhausted. FirstNexus may ask later whether you slept.
- The agent says it will follow up after something changes. FirstNexus may track
  that open loop.

Commitments are not durable facts like `MEMORY.md`, and they are not exact
reminders. They sit between memory and automation: FirstNexus remembers a
...
```bash
FirstNexus config set commitments.enabled true
FirstNexus config set commitments.maxPerDay 3
```

Equivalent `FirstNexus.json`:
...
After an agent reply, FirstNexus may run a hidden background extraction pass in a
...
When it finds a high-confidence candidate, FirstNexus stores a commitment with:
...
without FirstNexus tools.

FirstNexus never delivers an inferred commitment immediately after writing it.
...
FirstNexus commitments dismiss cm_abc123
```

See [`FirstNexus commitments`](/cli/commitments) for the command reference.
...
Stored commitments are local FirstNexus state. They are operational memory, not
long-term memory. Disable the feature with:

```bash
FirstNexus config set commitments.enabled false
```
...
- Check `FirstNexus commitments --all` for pending, dismissed, snoozed, or expired
  records.
...
- [`FirstNexus commitments`](/cli/commitments)
