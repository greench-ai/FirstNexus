...
alongside FirstNexus. It combines BM25, vector search, and reranking in a single
...
- **Automatic fallback** -- if QMD is unavailable, FirstNexus falls back to the
...
FirstNexus creates a self-contained QMD home under
`~/.FirstNexus/agents/<agentId>/qmd/` and manages the sidecar lifecycle
...
- FirstNexus creates collections from your workspace memory files and any
...
- Searches use the configured `searchMode` (default: `search`; also supports
  `vsearch` and `query`). `search` is BM25-only, so FirstNexus skips semantic
  vector readiness probes and embedding maintenance in that mode. If a mode
  fails, FirstNexus retries with `qmd query`.
- With QMD releases that advertise multi-collection filters, FirstNexus groups
  same-source collections into one QMD search invocation. Older QMD releases
  keep the compatible per-collection fallback.
- If QMD fails entirely, FirstNexus falls back to the builtin SQLite engine.
  ...
  `FirstNexus memory status` and one-shot CLI probes still recheck QMD directly.
...
FirstNexus keeps the QMD search path compatible with both current and older QMD
installs.

On startup, FirstNexus checks the installed QMD help text once per manager. If the
binary advertises support for multiple collection filters, FirstNexus searches all
...
Older QMD builds only accept one collection filter. When FirstNexus detects one
...
process, so you can tune QMD globally without adding new FirstNexus config:
...
collection under `~/.FirstNexus/agents/<id>/qmd/sessions/`.
...
When scope denies a search, FirstNexus logs a warning with the derived channel and
...
**QMD not found?** Ensure the binary is on the gateway's `PATH`. If FirstNexus
runs as a service, create a symlink:
`sudo ln -s ~/.bun/bin/qmd /usr/local/bin/qmd`.

If `qmd --version` works in your shell but FirstNexus still reports
...
with `FirstNexus memory status --deep`.

**First search very slow?** QMD downloads GGUF models on first use. Pre-warm
with `qmd query "test"` using the same XDG dirs FirstNexus uses.

**Many QMD subprocesses during search?** Update QMD if possible. FirstNexus uses
...
`memory.qmd.searchMode = "search"`. FirstNexus treats that mode as lexical-only,
...
the next startup reconciliation. FirstNexus recreates stale managed collections
...
FirstNexus's builtin symlink rules. Keep temporary monorepo checkouts under
