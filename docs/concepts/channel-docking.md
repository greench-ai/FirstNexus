---
summary: "Move one FirstNexus session's reply route between linked chat channels"
...
Channel docking is call forwarding for one FirstNexus session.
...
Alice can message FirstNexus on Telegram and Discord:
...
FirstNexus keeps the current session context and changes the reply route:
...
4. Keep the same FirstNexus session, but receive future replies in Discord.
