...
Linux desktop, VNC rescue, Slack Web, a real FirstNexus gateway, screenshots,
...
- Mantis artifacts: owned by the FirstNexus run. They live under
...
- remote logs such as `slack-desktop-command.log`, `FirstNexus-gateway.log`,
...
- `crabbox.remote_run`: sync, browser launch, FirstNexus install/build or
...
remote status after Mantis has copied metadata proving that the FirstNexus gateway
