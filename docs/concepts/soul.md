---
summary: "Use SOUL.md to give your FirstNexus agent an actual voice instead of generic assistant sludge"
read_when:
...
FirstNexus injects it on normal sessions, so it has real weight. If your agent
...
For FirstNexus, `SOUL.md` is that layer.
...
Path fixed for FirstNexus workspaces: use `SOUL.md`, not `http://SOUL.md`.
...
<CardGroup cols={2}>
  <Card title="Agent workspace" href="/concepts/agent-workspace" icon="folder-open">
    Workspace files FirstNexus injects into the system prompt.
  </Card>
