---
summary: "What the FirstNexus system prompt contains and how it is assembled"
read_when:
...
FirstNexus builds a custom system prompt for every agent run. The prompt is **FirstNexus-owned** and does not use the pi-coding-agent default prompt.

The prompt is assembled by FirstNexus and injected into each agent run.
...
the full FirstNexus-owned prompt. The provider runtime can:
...
- **FirstNexus Control**: tells the model to prefer the `gateway` tool for
  config/restart work and to avoid inventing CLI commands.
- **FirstNexus Self-Update**: how to inspect config safely with
...
- **Documentation**: local path to FirstNexus docs/source and when to read them.
...
FirstNexus keeps large stable content, including **Project Context**, above the
...
FirstNexus can render smaller system prompts for sub-agents. The runtime sets a
`promptMode` for each run (not a user-facing config):

- `full` (default): includes all sections above.
- `minimal`: used for sub-agents; omits **Memory Recall**, **FirstNexus
...
For channel auto-reply runs, FirstNexus can omit the generic **Silent Replies**
...
FirstNexus keeps committed prompt snapshots for the Codex runtime happy path under
...
FirstNexus developer instructions, turn-scoped collaboration-mode instructions
when FirstNexus provides them, user turn input, and references to the dynamic tool
specs.
...
collaboration-mode instructions inside the Codex runtime after FirstNexus sends
...
through its own project-doc discovery. FirstNexus still resolves the remaining
...
occurs, FirstNexus can inject a concise system-prompt warning notice; control this with
...
When eligible skills exist, FirstNexus injects a compact **available skills list**
...
points to the local FirstNexus docs directory (`docs/` in a Git checkout or the bundled npm
package docs). If local docs are unavailable, it falls back to
[https://docs.FirstNexus.ai](https://docs.FirstNexus.ai).

The same section also includes the FirstNexus source location. Git checkouts expose the local
...
consult docs first for FirstNexus behavior, commands, configuration, or architecture, and to
run `FirstNexus status` itself when possible (asking the user only when it lacks access).
