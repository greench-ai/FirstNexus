...
`node-llama-cpp` runtime package next to FirstNexus, then point `local.modelPath`
at a GGUF file:
...
FirstNexus indexes `MEMORY.md` and `memory/*.md` into chunks (~400 tokens with
80-token overlap) and stores them in a per-agent SQLite database.

- **Index location:** `~/.FirstNexus/memory/<agentId>.sqlite`
...
- **Reindex on demand:** `FirstNexus memory index --force`
...
**Memory search disabled?** Check `FirstNexus memory status`. If no provider is
...
**Stale results?** Run `FirstNexus memory index --force` to rebuild. The watcher
may miss changes in rare edge cases.

**sqlite-vec not loading?** FirstNexus falls back to in-process cosine similarity
automatically. `FirstNexus memory status --deep` reports the local vector store
