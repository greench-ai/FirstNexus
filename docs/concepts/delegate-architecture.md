---
summary: "Delegate architecture: running FirstNexus as a named agent on behalf of an organization"
title: Delegate architecture
read_when: "You want an agent with its own identity that acts on behalf of humans in an organization."
status: active
---

Goal: run FirstNexus as a **named delegate** - an agent with its own identity that acts "on behalf of" people in an organization. The agent never impersonates a human. It sends, reads, and schedules under its own account with explicit delegation permissions.

This extends [Multi-Agent Routing](/concepts/multi-agent) from personal use into organizational deployments.

## What is a delegate?

A **delegate** is an FirstNexus agent that:
...
FirstNexus's default mode is a **personal assistant** - one human, one agent. Delegates extend this to organizations:
...
2. **Scope control**: the identity provider enforces what the delegate can access, independent of FirstNexus's own tool policy.
...
{
  id: "delegate",
  workspace: "~/.FirstNexus/workspace-delegate",
  tools: {
...
- Cron run history: `~/.FirstNexus/cron/runs/<jobId>.jsonl`
- Session transcripts: `~/.FirstNexus/agents/delegate/sessions`
- Identity provider audit logs (Exchange, Google Workspace)

All delegate actions flow through FirstNexus's session store. For compliance, ensure these logs are retained and reviewed.
...
```bash
FirstNexus agents add delegate
```

This creates:

- Workspace: `~/.FirstNexus/workspace-delegate`
- State: `~/.FirstNexus/agents/delegate/agent`
- Sessions: `~/.FirstNexus/agents/delegate/sessions`
...
```json5
{
  agents: {
    list: [
      { id: "main", workspace: "~/.FirstNexus/workspace" },
      {
        id: "delegate",
        workspace: "~/.FirstNexus/workspace-delegate",
        tools: {
...
~/.FirstNexus/agents/delegate/agent/auth-profiles.json
```
...
```json5
{
  agents: {
    list: [
      { id: "main", default: true, workspace: "~/.FirstNexus/workspace" },
      {
        id: "org-assistant",
        name: "[Organization] Assistant",
        workspace: "~/.FirstNexus/workspace-org",
        agentDir: "~/.FirstNexus/agents/org-assistant/agent",
...
recall view. FirstNexus redacts credential/token-like text, truncates long
