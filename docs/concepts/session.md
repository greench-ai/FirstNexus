---
summary: "How FirstNexus manages conversation sessions"
read_when:
...
FirstNexus organizes conversations into **sessions**. Each message is routed to a
...
Verify your setup with `FirstNexus security audit`.
...
FirstNexus automatically bounds session storage over time. By default, it runs
...
`FirstNexus sessions cleanup --dry-run --fix-dm-scope`. Applying the same flag
retires those old direct-DM rows and keeps their transcripts as deleted
archives.

Preview with `FirstNexus sessions cleanup --dry-run`.

## Inspecting sessions

- `FirstNexus status` -- session store path and recent activity.
- `FirstNexus sessions --json` -- all sessions (filter with `--active <minutes>`).
