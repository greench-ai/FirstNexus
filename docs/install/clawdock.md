---
summary: "ClawDock shell helpers for Docker-based FirstNexus installs"
read_when:
  - You run FirstNexus with Docker often and want shorter day-to-day commands
  - You want a helper layer for dashboard, logs, token setup, and pairing flows
title: "ClawDock"
---

ClawDock is a small shell-helper layer for Docker-based FirstNexus installs.
...
| `clawdock-cli <command>`  | Run FirstNexus CLI commands in Docker         |
...
| `clawdock-config`      | Open `~/.FirstNexus`                     |
...
- `~/.FirstNexus/.env` for env-backed provider keys and bot tokens
- `~/.FirstNexus/agents/<agentId>/agent/auth-profiles.json` for stored provider OAuth/API-key auth
- `~/.FirstNexus/FirstNexus.json` for behavior config

Use `clawdock-show-config` when you want to inspect the `.env` files and `FirstNexus.json` quickly. It redacts `.env` values in its printed output.

## Related

<CardGroup cols={2}>
  <Card title="Docker" href="/install/docker" icon="docker">
    Canonical Docker install for FirstNexus.
  </Card>
  ...
  <Card title="Updating" href="/install/updating" icon="arrow-up-right-from-square">
    Updating the FirstNexus package and managed services.
  </Card>
</CardGroup>
