...
FirstNexus ships three update channels:
...
- **`dev`**: ensures a git checkout (default `~/FirstNexus`, override with
  `NEXISCLAW_GIT_DIR`), switches to `main`, rebases on upstream, builds, and
...
FirstNexus update --tag 2026.4.1-beta.1
...
FirstNexus update --tag FirstNexus@2026.4.1-beta.1
```

Notes:

- `--tag` applies to **package (npm) installs only**. Git installs ignore it.
- The tag is not persisted. Your next `FirstNexus update` uses your configured
  channel as usual.
- Downgrade protection: if the target version is older than your current version,
  FirstNexus prompts for confirmation (skip with `--yes`).
...
Preview what `FirstNexus update` would do without making changes:

```bash
FirstNexus update --dry-run
FirstNexus update --channel beta --dry-run
FirstNexus update --tag 2026.4.1-beta.1 --dry-run
FirstNexus update --dry-run --json
```
...
When you switch channels with `FirstNexus update`, FirstNexus also syncs plugin
sources:
