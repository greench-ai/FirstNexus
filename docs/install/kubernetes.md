---
summary: "Deploy FirstNexus Gateway to a Kubernetes cluster with Kustomize"
read_when:
  - You want to run FirstNexus on a Kubernetes cluster
  - You want to test FirstNexus in a Kubernetes environment
title: "Kubernetes"
---

A minimal starting point for running FirstNexus on Kubernetes — not a production-ready deployment. It covers the core resources and is meant to be adapted to your environment.

## Why not Helm?

FirstNexus is a single container with some config files. The interesting customization is in agent content (markdown files, skills, config overrides), not infrastructure templating. Kustomize handles overlays without the overhead of a Helm chart. If your deployment grows more complex, a Helm chart can be layered on top of these manifests.
...
kubectl port-forward svc/FirstNexus 18789:18789 -n FirstNexus
...
```bash
kubectl get secret FirstNexus-secrets -n FirstNexus -o jsonpath='{.data.NEXISCLAW_GATEWAY_TOKEN}' | base64 -d
```
...
```
Namespace: FirstNexus (configurable via NEXISCLAW_NAMESPACE)
├── Deployment/FirstNexus        # Single pod, init container + gateway
├── Service/FirstNexus           # ClusterIP on port 18789
├── PersistentVolumeClaim      # 10Gi for agent state and config
├── ConfigMap/FirstNexus-config  # FirstNexus.json + AGENTS.md
└── Secret/FirstNexus-secrets    # Gateway token + API keys
```
...
Edit `FirstNexus.json` in `scripts/k8s/manifests/configmap.yaml`. See [Gateway configuration](/gateway/configuration) for the full reference.
...
```bash
kubectl patch secret FirstNexus-secrets -n FirstNexus \
  -p '{"stringData":{"<PROVIDER>_API_KEY":"..."}}'
kubectl rollout restart deployment/FirstNexus -n FirstNexus
```
...
```yaml
image: ghcr.io/FirstNexus/FirstNexus:latest # or pin to a specific version from https://github.com/FirstNexus/FirstNexus/releases
```
...
└── manifests/
    ...
    ├── configmap.yaml          # FirstNexus.json + AGENTS.md
