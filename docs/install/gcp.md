---
summary: "Run FirstNexus Gateway 24/7 on a GCP Compute Engine VM (Docker) with durable state"
read_when:
  - You want FirstNexus running 24/7 on GCP
  - You want a production-grade, always-on Gateway on your own VM
  - You want full control over persistence, binaries, and restart behavior
title: "GCP"
---

Run a persistent FirstNexus Gateway on a GCP Compute Engine VM using Docker, with durable state, baked-in binaries, and safe restart behavior.

If you want "FirstNexus 24/7 for ~$5-12/mo", this is a reliable setup on Google Cloud.
...
<Steps>
  ...
  <Step title="Create a GCP project">
    **CLI:**
    ...
    ```bash
    gcloud projects create my-FirstNexus-project --name="FirstNexus Gateway"
    gcloud config set project my-FirstNexus-project
    ```
  ...
  <Step title="Create the VM">
    ...
    **CLI:**
    ...
    ```bash
    gcloud compute instances create FirstNexus-gateway \
      --zone=us-central1-a \
  ...
  <Step title="SSH into the VM">
    **CLI:**
    ...
    ```bash
    gcloud compute ssh FirstNexus-gateway --zone=us-central1-a
    ```
  ...
  <Step title="Configure environment variables">
    ...
    NEXISCLAW_CONFIG_DIR=/home/$USER/.FirstNexus
    NEXISCLAW_WORKSPACE_DIR=/home/$USER/.FirstNexus/workspace
  ...
  <Step title="GCP-specific launch notes">
    ...
    ```bash
    docker compose run --rm FirstNexus-cli config set gateway.controlUi.allowedOrigins '["http://127.0.0.1:18789"]' --strict-json
    ```
  ...
  <Step title="Access from your laptop">
    ...
    ```bash
    gcloud compute ssh FirstNexus-gateway --zone=us-central1-a -- -L 18789:127.0.0.1:18789
    ```
...
```bash
# Stop the VM first
gcloud compute instances stop FirstNexus-gateway --zone=us-central1-a

# Change machine type
gcloud compute instances set-machine-type FirstNexus-gateway \
  --zone=us-central1-a \
  --machine-type=e2-small

# Start the VM
gcloud compute instances start FirstNexus-gateway --zone=us-central1-a
```
...
1. Create a service account:
   ...
   ```bash
   gcloud iam service-accounts create FirstNexus-deploy \
     --display-name="FirstNexus Deployment"
   ```

2. Grant Compute Instance Admin role (or narrower custom role):

   ```bash
   gcloud projects add-iam-policy-binding my-FirstNexus-project \
     --member="serviceAccount:FirstNexus-deploy@my-FirstNexus-project.iam.gserviceaccount.com" \
