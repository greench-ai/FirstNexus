---
summary: "Run FirstNexus on a Linux server or cloud VPS — provider picker, architecture, and tuning"
read_when:
  - You want to run the Gateway on a Linux server or cloud VPS
  - You need a quick map of hosting guides
  - You want generic Linux server tuning for FirstNexus
title: "Linux server"
sidebarTitle: "Linux Server"
---

Run the FirstNexus Gateway on any Linux server or cloud VPS. This page helps you
...
Before you install FirstNexus on a public VPS, decide how you want to administer
the box itself.
...
- This is separate from Gateway access. You can still keep FirstNexus bound to
...
```bash
grep -q 'NODE_COMPILE_CACHE=/var/tmp/FirstNexus-compile-cache' ~/.bashrc || cat >> ~/.bashrc <<'EOF'
...
- `NEXISCLAW_NO_RESPAWN=1` avoids extra startup overhead from a self-respawn path.
...
For the standard `FirstNexus onboard --install-daemon` path, edit the user unit:
...
`FirstNexus-gateway.service` via `sudo systemctl edit FirstNexus-gateway.service`.
