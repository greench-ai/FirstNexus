...
<Steps>
  <Step title="Create the skill directory">
    ...
    ```bash
    mkdir -p ~/.FirstNexus/workspace/skills/hello-world
    ```
  ...
  <Step title="Load the skill">
    Start a new session so FirstNexus picks up the skill:
  ...
  <Step title="Test it">
    ...
    ```bash
    FirstNexus agent --message "give me a greeting"
    ```
...
| `metadata.FirstNexus.os`              | No       | OS filter (`["darwin"]`, `["linux"]`, etc.)                    |
| `metadata.FirstNexus.requires.bins`   | No       | Required binaries on PATH                                      |
| `metadata.FirstNexus.requires.config` | No       | Required config keys                                           |
...
- **Test locally** — use `FirstNexus agent --message "..."` to test before sharing
...
| `~/.FirstNexus/skills/`           | Medium     | Shared (all agents)   |
| Bundled (shipped with FirstNexus) | Low        | Global                |
