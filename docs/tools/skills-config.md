...
`~/.FirstNexus/FirstNexus.json`. Agent-specific skill visibility lives under
...
- Built-in skill roots always include `~/.FirstNexus/skills`, `~/.agents/skills`,
...
- `install.nodeManager`: node installer preference (`npm` | `pnpm` | `yarn` | `bun`, default: npm).
  ...
  - `FirstNexus setup --node-manager` is narrower and currently accepts `npm`,
...
FirstNexus skips it and logs `Skipping escaped skill path outside its configured
root`.
...
- Keys under `entries` map to the skill name by default. If a skill defines
  `metadata.FirstNexus.skillKey`, use that key instead.
- Load precedence is `<workspace>/skills` → `<workspace>/.agents/skills` →
  `~/.agents/skills` → `~/.FirstNexus/skills` → bundled skills →
