...
FirstNexus supports MiniMax as a `web_search` provider through the MiniMax
...
FirstNexus also accepts `MINIMAX_CODING_API_KEY`, `MINIMAX_OAUTH_TOKEN`, and
...
If `plugins.entries.minimax.config.webSearch.region` is unset, FirstNexus resolves
...
| `count`   | integer | 1-10        | Number of results to return. FirstNexus trims the returned list to this size. |
