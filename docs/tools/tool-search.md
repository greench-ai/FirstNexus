...
read_when:
  ...
  - You want FirstNexus tools, MCP tools, and client tools exposed through one compact PI surface
  - You are implementing or debugging tool discovery for PI runs
---

Tool Search is an experimental FirstNexus PI-agent feature. It gives PI agents one
...
This page documents FirstNexus PI Tool Search. It is not the Codex-native tool
...
`FirstNexus.tools` bridge:

```js
const hits = await FirstNexus.tools.search("create a GitHub issue");
const tool = await FirstNexus.tools.describe(hits[0].id);
return await FirstNexus.tools.call(tool.id, {
  title: "Crash on startup",
  body: "Steps to reproduce...",
});
```

The catalog can include FirstNexus tools, plugin tools, MCP tools, and
...
needs the exact schema, and calls that tool through FirstNexus.

Codex harness runs do not receive these experimental FirstNexus Tool Search
controls. FirstNexus passes product capabilities to Codex as dynamic tools, and
...
2. List eligible FirstNexus and plugin tools.
...
At execution time every real tool call returns to FirstNexus. The isolated Node
runtime does not hold plugin implementations, MCP client objects, or secrets.
`FirstNexus.tools.call(...)` crosses the bridge back into the Gateway, where the
...
catalog includes eligible FirstNexus, MCP, and client tools after normal policy
filtering.
...
`FirstNexus.tools.search(query, options?)`

Searches the effective catalog for the current run. Results are compact and safe
to put back into prompt context.

```js
const hits = await FirstNexus.tools.search("calendar event", { limit: 5 });
```

`FirstNexus.tools.describe(id)`

Loads full metadata for one search result, including the exact input schema.

```js
const calendarCreate = await FirstNexus.tools.describe("mcp:calendar:create_event");
```

`FirstNexus.tools.call(id, args)`

Calls a selected tool through FirstNexus.

```js
await FirstNexus.tools.call(calendarCreate.id, {
  summary: "Planning",
...
network grants, and no child-process or worker grants. FirstNexus enforces a
...
- `FirstNexus.tools.search`
- `FirstNexus.tools.describe`
- `FirstNexus.tools.call`

Normal FirstNexus behavior still applies to final calls:
...
```bash
FirstNexus config set tools.toolSearch true
```
...
- final tool calls executed through FirstNexus
...
- whether the result came from FirstNexus, MCP, or a client tool
