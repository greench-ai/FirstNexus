...
FirstNexus supports Gemini models with built-in
...
For a gateway install, put env keys in `~/.FirstNexus/.env`.
...
`pd`, `pw`, `pm`, and `py`. FirstNexus converts these values, or an explicit
