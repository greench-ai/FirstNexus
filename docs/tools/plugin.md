---
summary: "Install, configure, and manage FirstNexus plugins"
read_when:
...
Plugins extend FirstNexus with new capabilities: channels, model providers,
...
search, and more. Some plugins are **core** (shipped with FirstNexus), others
...
temporary set of FirstNexus-owned plugin packages while that migration finishes.
...
<Steps>
  ...
  <Step title="Install a plugin">
    ...
    FirstNexus plugins install npm-pack:./FirstNexus-plugin-1.2.3.tgz
    ...
    FirstNexus plugins install ./my-plugin.tgz
    ```
  ...
  <Step title="Verify the plugin">
    ...
    FirstNexus <plugin-command> --help
    ```
...
`FirstNexus doctor --fix`. The only recovery exception is a narrow bundled-plugin
reinstall path for plugins that opt into
`FirstNexus.install.allowInvalidConfigRecovery`.
During Gateway startup, invalid plugin config fails closed like any other invalid
config. Run `FirstNexus doctor --fix` to quarantine the bad plugin config by
...
Run `FirstNexus doctor --fix` to remove the stale channel/plugin entries; unknown
...
Gateway startup skips plugin discovery/load work and `FirstNexus doctor` preserves
...
installed under FirstNexus's managed plugin roots. npm dependencies may be hoisted
within FirstNexus's managed npm root; install/update scans that managed root before
trust and uninstall removes npm-managed packages through npm. External plugins
and custom load paths must still be installed through `FirstNexus plugins install`.
Use `FirstNexus plugins list --json` to see the static `dependencyStatus` for each
...
and config validation follows with `plugin present but blocked`, FirstNexus found
...
FirstNexus as the same user that owns the state directory.

For Docker installs, the official image runs as `node` (uid `1000`), so the
host bind-mounted FirstNexus config and workspace directories should normally be
owned by uid `1000`:

```bash
sudo chown -R 1000:1000 /path/to/FirstNexus-config /path/to/FirstNexus-workspace
```

If you intentionally run FirstNexus as root, repair the managed plugin root to
root ownership instead:

```bash
sudo chown -R root:root /path/to/FirstNexus-config/npm
```

After fixing ownership, rerun `FirstNexus doctor --fix` or
`FirstNexus plugins registry --refresh` so the persisted plugin registry matches
the repaired files.

For npm installs, mutable selectors such as `latest` or a dist-tag are resolved
before installation and then pinned to the exact verified version in FirstNexus's
managed npm root. After npm finishes, FirstNexus verifies the installed
...
Managed npm roots also inherit FirstNexus's package-level npm `overrides`, so
security pins that protect the packaged host also apply to hoisted external
plugin dependencies.

Source checkouts are pnpm workspaces. If you clone FirstNexus to hack on bundled
plugins, run `pnpm install`; FirstNexus then loads bundled plugins from
`extensions/<id>` so edits and package-local dependencies are used directly.
Plain npm root installs are for packaged FirstNexus, not source checkout
development.

## Plugin types

FirstNexus recognizes two plugin formats:

| Format     | How it works                                                         | Examples                                               |
| ---------- | -------------------------------------------------------------------- | ------------------------------------------------------ |
| **Native** | `FirstNexus.plugin.json` + runtime module; executes in-process       | Official plugins, community npm packages               |
| **Bundle** | Codex/Claude/Cursor-compatible layout; mapped to FirstNexus features | `.codex-plugin/`, `.claude-plugin/`, `.cursor-plugin/` |

Both show up under `FirstNexus plugins list`. See [Plugin Bundles](/plugins/bundles) for bundle details.
...
Native plugin npm packages must declare `FirstNexus.extensions` in `package.json`.
...
npm packages installed into FirstNexus's managed plugin root.
...
FirstNexus needs at runtime. That is a plugin packaging issue, not a local config
...
Use `FirstNexus.runtimeExtensions` when published runtime files do not live at the
...
publish `FirstNexus.setupEntry`, use `FirstNexus.runtimeSetupEntry` for its built
...
### FirstNexus-owned npm packages during migration

ClawHub is the primary distribution path for most plugins. Current packaged
FirstNexus releases already bundle many official plugins, so those do not need
separate npm installs in normal setups. Until every FirstNexus-owned plugin has
migrated to ClawHub, FirstNexus still ships some `@FirstNexus/*` plugin packages on
npm for older/custom installs and direct npm workflows.

If npm reports an `@FirstNexus/*` plugin package as deprecated, that package
version is from an older external package train. Use the bundled plugin from
current FirstNexus or a local checkout until a newer npm package is published.

| Plugin          | Package                      | Docs                                       |
| --------------- | ---------------------------- | ------------------------------------------ |
| Discord         | `@FirstNexus/discord`        | [Discord](/channels/discord)               |
| Feishu          | `@FirstNexus/feishu`         | [Feishu](/channels/feishu)                 |
| Matrix          | `@FirstNexus/matrix`         | [Matrix](/channels/matrix)                 |
| Mattermost      | `@FirstNexus/mattermost`     | [Mattermost](/channels/mattermost)         |
| Microsoft Teams | `@FirstNexus/msteams`        | [Microsoft Teams](/channels/msteams)       |
| Nextcloud Talk  | `@FirstNexus/nextcloud-talk` | [Nextcloud Talk](/channels/nextcloud-talk) |
| Nostr           | `@FirstNexus/nostr`          | [Nostr](/channels/nostr)                   |
| Synology Chat   | `@FirstNexus/synology-chat`  | [Synology Chat](/channels/synology-chat)   |
| Tlon            | `@FirstNexus/tlon`           | [Tlon](/channels/tlon)                     |
| WhatsApp        | `@FirstNexus/whatsapp`       | [WhatsApp](/channels/whatsapp)             |
| Zalo            | `@FirstNexus/zalo`           | [Zalo](/channels/zalo)                     |
| Zalo Personal   | `@FirstNexus/zalouser`       | [Zalo Personal](/plugins/zalouser)         |

### Core (shipped with FirstNexus)
...
<AccordionGroup>
  ...
  <Accordion title="Other">
    - `browser` - bundled browser plugin for the browser tool, `FirstNexus browser` CLI, `browser.request` gateway method, browser runtime, and default browser control service (enabled by default; disable before replacing it)
...
to `plugins.allow` or remove `plugins.allow`; `FirstNexus doctor` warns about this
shape.
...
`FirstNexus plugins list` is a local plugin registry/config snapshot. An
...
`FirstNexus gateway run` process, or use `FirstNexus gateway restart` against the
running Gateway when the reload reports a failure.

<Accordion title="Plugin states: disabled vs missing vs invalid">
  - **Disabled**: plugin exists but enablement rules turned it off. Config is preserved.
  - **Missing**: config references a plugin id that discovery did not find.
  - **Invalid**: plugin exists but its config does not match the declared schema. Gateway startup skips only that plugin; `FirstNexus doctor --fix` can quarantine the invalid entry by disabling it and removing its config payload.

</Accordion>

## Discovery and precedence

FirstNexus scans for plugins in this order (first match wins):

<Steps>
  <Step title="Config paths">
    `plugins.load.paths` - explicit file or directory paths. Paths that point
    back at FirstNexus's own packaged bundled plugin directories are ignored;
    run `FirstNexus doctor --fix` to remove those stale aliases.
  </Step>

  <Step title="Workspace plugins">
    `\<workspace\>/.FirstNexus/<plugin-root>/*.ts` and `\<workspace\>/.FirstNexus/<plugin-root>/*/index.ts`.
  </Step>

  <Step title="Global plugins">
    `~/.FirstNexus/<plugin-root>/*.ts` and `~/.FirstNexus/<plugin-root>/*/index.ts`.
  </Step>

  <Step title="Bundled plugins">
    Shipped with FirstNexus. Many are enabled by default (model providers, speech).
...
`/app/extensions/synology-chat`, FirstNexus treats that mounted source directory
...
- Run `FirstNexus gateway status --deep --require-rpc` and confirm the active
...
- Restart the live Gateway after plugin install/config/code changes. In wrapper
  ...
  `FirstNexus gateway run` process.
- Use `FirstNexus plugins inspect <id> --runtime --json` to confirm hook registrations and
...
- For proof of the effective session model, use `FirstNexus sessions` or the
...
```bash
FirstNexus config set logging.level trace
...
FirstNexus caches successful plugin tool factory results for repeated resolutions
...
- Run `FirstNexus plugins list --enabled --verbose` to see every enabled plugin
  and origin.
- Run `FirstNexus plugins inspect <id> --runtime --json` for each suspected plugin and
  compare `channels`, `channelConfigs`, `tools`, and diagnostics.
- Run `FirstNexus plugins registry --refresh` after installing or removing
...
- If the duplicate is accidental, disable one side with
  ...
  install.
- If you explicitly enabled both plugins, FirstNexus keeps that request and
...
```bash
FirstNexus plugins list                       # compact inventory
FirstNexus plugins list --enabled            # only enabled plugins
FirstNexus plugins list --verbose            # per-plugin detail lines
FirstNexus plugins list --json               # machine-readable inventory
FirstNexus plugins search <query>            # search ClawHub plugin catalog
FirstNexus plugins inspect <id>              # static detail
FirstNexus plugins inspect <id> --runtime    # registered hooks/tools/CLI/gateway methods
FirstNexus plugins inspect <id> --json       # machine-readable
FirstNexus plugins inspect --all             # fleet-wide table
FirstNexus plugins info <id>                 # inspect alias
FirstNexus plugins doctor                    # diagnostics
FirstNexus plugins registry                  # inspect persisted registry state
FirstNexus plugins registry --refresh        # rebuild persisted registry
FirstNexus doctor --fix                      # repair plugin registry state

FirstNexus plugins install <package>         # install from npm by default
FirstNexus plugins install clawhub:<pkg>     # install from ClawHub only
FirstNexus plugins install npm:<pkg>         # install from npm only
FirstNexus plugins install git:<repo>        # install from git
FirstNexus plugins install git:<repo>@<ref>  # install from git ref
FirstNexus plugins install <spec> --force    # overwrite existing install
FirstNexus plugins install <path>            # install from local path
FirstNexus plugins install -l <path>         # link (no copy) for dev
FirstNexus plugins install <plugin> --marketplace <source>
FirstNexus plugins install <plugin> --marketplace https://github.com/<owner>/<repo>
FirstNexus plugins install <spec> --pin      # record exact resolved npm spec
FirstNexus plugins install <spec> --dangerously-force-unsafe-install
FirstNexus plugins update <id-or-npm-spec> # update one plugin
FirstNexus plugins update <id-or-npm-spec> --dangerously-force-unsafe-install
FirstNexus plugins update --all            # update all
FirstNexus plugins uninstall <id>          # remove config and plugin index records
...
FirstNexus plugins inspect <id> --runtime --json

# Run plugin-owned CLI commands directly from the FirstNexus root CLI.
FirstNexus <plugin-command> --help
...
Bundled plugins ship with FirstNexus. Many are enabled by default (for example
bundled model providers, bundled speech providers, and the bundled browser
plugin). Other bundled plugins still need `FirstNexus plugins enable <id>`.

`--force` overwrites an existing installed plugin or hook pack in place. Use
`FirstNexus plugins update <id-or-npm-spec>` for routine upgrades of tracked npm
plugins. It is not supported with `--link`, which reuses the source path instead
of copying over a managed install target.

When `plugins.allow` is already set, `FirstNexus plugins install` adds the
...
FirstNexus keeps a persisted local plugin registry as the cold read model for
...
the registry is missing, stale, or invalid, `FirstNexus plugins registry
...
install instead; for nix-FirstNexus, start with the agent-first
[Quick Start](https://github.com/FirstNexus/nix-FirstNexus#quick-start).
`FirstNexus plugins update <id-or-npm-spec>` applies to tracked installs. Passing
...
the resolved version and recorded artifact identity, FirstNexus skips the update
...
override instead, while `FirstNexus skills install` remains the separate ClawHub
...
`FirstNexus plugins inspect <id>` also reports detected bundle capabilities plus
...
See [`FirstNexus plugins` CLI reference](/cli/plugins) for full details.
...
FirstNexus loads the entry object and calls `register(api)` during plugin
...
FirstNexus may evaluate the trusted plugin entry or channel plugin module to build
