...
read_when:
  ...
  - Debugging why FirstNexus is interfering with your own Chrome
  - Implementing browser settings + lifecycle in the macOS app
title: "Browser (FirstNexus-managed)"
---

FirstNexus can run a **dedicated Chrome/Brave/Edge/Chromium profile** that the agent controls.
...
- The `FirstNexus` profile does **not** touch your personal browser profile.
- The agent can **open tabs, read pages, click, and type** in a safe lane.
- The built-in `user` profile attaches to your real signed-in Chrome session via Chrome MCP.

## What you get

- A separate browser profile named **FirstNexus** (orange accent by default).
...
- A bundled `browser-automation` skill that teaches agents the snapshot,
  ...
  plugin is enabled.
- Optional multi-profile support (`FirstNexus`, `work`, `remote`, ...).
...
FirstNexus browser --browser-profile FirstNexus doctor --deep
FirstNexus browser --browser-profile FirstNexus status
...
If `FirstNexus browser` is missing entirely, or the agent says the browser tool
...
Defaults need both `plugins.entries.browser.enabled` **and** `browser.enabled=true`. Disabling only the plugin removes the `FirstNexus browser` CLI, `browser.request` gateway method, agent tool, and control service as one unit; your `browser.*` config stays intact for a replacement.
...
If `FirstNexus browser` is unknown after an upgrade, `browser.request` is missing, or the agent reports the browser tool as unavailable, the usual cause is a `plugins.allow` list that omits `browser` and no root `browser` config block exists. Add it:
...
## Profiles: `FirstNexus` vs `user`

- `FirstNexus`: managed, isolated browser (no extension required).
- `user`: built-in Chrome MCP attach profile for your **real signed-in Chrome**
  session.

For agent browser tool calls:

- Default: use the isolated `FirstNexus` browser.
...
Set `browser.defaultProfile: "FirstNexus"` if you want managed mode by default.

## Configuration

Browser settings live in `~/.FirstNexus/FirstNexus.json`.
...
- Control service binds to loopback on a port derived from `gateway.port` (default `18791` = gateway + 2). Overriding `gateway.port` or `NEXISCLAW_GATEWAY_PORT` shifts the derived ports in the same family.
- Local `FirstNexus` profiles auto-assign `cdpPort`/`cdpUrl`; set those only for remote CDP. `cdpUrl` defaults to the managed local CDP port when unset.
...
- Repeated managed Chrome launch/readiness failures are circuit-broken per
  profile. After several consecutive failures, FirstNexus pauses new launch
...
- Gateway/provider `HTTP_PROXY`, `HTTPS_PROXY`, `ALL_PROXY`, and `NO_PROXY` environment variables do not automatically proxy the FirstNexus-managed browser. Managed Chrome launches direct by default so provider proxy settings do not weaken browser SSRF checks.
...
- `POST /start?headless=true` and `FirstNexus browser start --headless` request a
  ...
  remote CDP profiles reject the override because FirstNexus does not launch those
  browser processes.
- On Linux hosts without `DISPLAY` or `WAYLAND_DISPLAY`, local managed profiles
  default to headless automatically when neither the environment nor profile/global
  config explicitly chooses headed mode. `FirstNexus browser status --json`
  reports `headlessSource` as `env`, `profile`, `config`,
  `request`, `linux-display-fallback`, or `default`.
- `NEXISCLAW_BROWSER_HEADLESS=1` forces local managed launches headless for the
  current process. `NEXISCLAW_BROWSER_HEADLESS=0` forces headed mode for ordinary
...
- Default profile is `FirstNexus` (managed standalone). Use `defaultProfile: "user"` to opt into the signed-in user browser.
...
FirstNexus uses it automatically. Set `browser.executablePath` to override
...
Per-profile `executablePath` only affects local managed profiles that FirstNexus
...
- **Remote CDP:** set `browser.profiles.<name>.cdpUrl` (or `browser.cdpUrl`) to
  attach to a remote Chromium-based browser. In this case, FirstNexus will not launch a local browser.
- For externally managed CDP services on loopback (for example Browserless in
  ...
  without `attachOnly` is treated as a local FirstNexus-managed browser profile.
- `headless` only affects local managed profiles that FirstNexus launches. It does not restart or change existing-session or remote CDP browsers.
...
- local managed profiles: `FirstNexus browser stop` stops the browser process that
  FirstNexus launched
- attach-only and remote CDP profiles: `FirstNexus browser stop` closes the active
  ...
  though no browser process was launched by FirstNexus
...
FirstNexus preserves the auth when calling `/json/*` endpoints and when connecting
to the CDP WebSocket. Prefer environment variables or secrets managers for
tokens instead of committing them to config files.

## Node browser proxy (zero-config default)

If you run a **node host** on the machine that has your browser, FirstNexus can
...
- If you set `nodeHost.browserProxy.allowProfiles`, FirstNexus treats it as a least-privilege boundary: only allowlisted profiles can be targeted, and persistent profile create/delete routes are blocked on the proxy surface.
...
CDP connection URLs over HTTPS and WebSocket. FirstNexus can use either form, but
...
- If Browserless gives you an HTTPS base URL, you can either convert it to
  `wss://` for a direct CDP connection or keep the HTTPS URL and let FirstNexus
  discover `/json/version`.

### Browserless Docker on the same host

When Browserless is self-hosted in Docker and FirstNexus runs on the host, treat
...
FirstNexus process. Browserless must also advertise a matching reachable endpoint;
set Browserless `EXTERNAL` to that same public-to-FirstNexus WebSocket base, such
...
an address FirstNexus cannot reach, CDP HTTP can look healthy while the WebSocket
attach still fails.

Do not leave `attachOnly` unset for a loopback Browserless profile. Without
`attachOnly`, FirstNexus treats the loopback port as a local managed browser
profile and may report that the port is in use but not owned by FirstNexus.

## Direct WebSocket CDP providers

Some hosted browser services expose a **direct WebSocket** endpoint rather than
the standard HTTP-based CDP discovery (`/json/version`). FirstNexus accepts three
CDP URL shapes and picks the right connection strategy automatically:

- **HTTP(S) discovery** - `http://host[:port]` or `https://host[:port]`.
  FirstNexus calls `/json/version` to discover the WebSocket debugger URL, then
...
- **Direct WebSocket endpoints** - `ws://host[:port]/devtools/<kind>/<id>` or
  ...
  path. FirstNexus connects directly via a WebSocket handshake and skips
...
- **Bare WebSocket roots** - `ws://host[:port]` or `wss://host[:port]` with no
  ...
  [Browserbase](https://www.browserbase.com)). FirstNexus tries HTTP
  `/json/version` discovery first (normalising the scheme to `http`/`https`);
  if discovery returns a `webSocketDebuggerUrl` it is used, otherwise FirstNexus
  ...
  accepts it, FirstNexus falls back to that root as well. This lets a bare `ws://`
...
- The standalone loopback browser HTTP API uses **shared-secret auth only**:
  gateway token bearer auth, `x-FirstNexus-password`, or HTTP Basic auth with the
...
- If browser control is enabled and no shared-secret auth is configured, FirstNexus
  generates a runtime-only gateway token for that startup. Configure
  `gateway.auth.token`, `gateway.auth.password`, `NEXISCLAW_GATEWAY_TOKEN`, or
  `NEXISCLAW_GATEWAY_PASSWORD` explicitly if clients need a stable secret across
  restarts.
- FirstNexus does **not** auto-generate that token when `gateway.auth.mode` is
...
FirstNexus supports multiple named profiles (routing configs). Profiles can be:

- **FirstNexus-managed**: a dedicated Chromium-based browser instance with its own user data directory + CDP port
- **remote**: an explicit CDP URL (Chromium-based browser running elsewhere)
- **existing session**: your existing Chrome profile via Chrome DevTools MCP auto-connect

Defaults:

- The `FirstNexus` profile is auto-created if missing.
...
FirstNexus can also attach to a running Chromium-based browser profile through the
...
3. Keep the browser running and approve the connection prompt when FirstNexus attaches.
...
```bash
FirstNexus browser --browser-profile user start
FirstNexus browser --browser-profile user status
FirstNexus browser --browser-profile user tabs
FirstNexus browser --browser-profile user snapshot --format ai
```
...
- `FirstNexus doctor` migrates old extension-based browser config and checks that
...
- This path is higher-risk than the isolated `FirstNexus` profile because it can
  act inside your signed-in browser session.
- FirstNexus does not launch the browser for this driver; it only attaches.
- FirstNexus uses the official Chrome DevTools MCP `--autoConnect` flow here. If
...
When `cdpUrl` is set on an existing-session profile, FirstNexus skips
...
Compared to the managed `FirstNexus` profile, existing-session drivers are more constrained:
...
When launching locally, FirstNexus picks the first available:
...
control API** plus a matching `FirstNexus browser` CLI (snapshots, refs, wait
...
- **CDP startup or readiness failure** means FirstNexus cannot confirm that the browser control plane is healthy.
...
- CDP startup or readiness failure:
  - `Chrome CDP websocket for profile "FirstNexus" is not reachable after start`
  - `Remote CDP for profile "<name>" is not reachable at <cdpUrl>`
  - `Port <port> is in use for profile "<name>" but not by FirstNexus` when a
...
- For the local loopback `FirstNexus` managed profile, CDP health checks intentionally skip browser SSRF reachability enforcement for FirstNexus's own local control plane.
...
- `browser` accepts:
  - `profile` to choose a named browser profile (FirstNexus, chrome, or remote CDP).
