...
read_when:
  - Running FirstNexus Gateway in WSL2 while Chrome lives on Windows
...
In the common split-host setup, FirstNexus Gateway runs inside WSL2, Chrome runs on Windows, and browser control must cross the WSL2 and Windows boundary. The layered failure pattern from [issue #39369](https://github.com/FirstNexus/FirstNexus/issues/39369) means several independent problems can show up at once, which makes the wrong layer look broken first.
...
- FirstNexus and Chrome are on the same machine
...
- FirstNexus points a browser profile at the address that is reachable from WSL2
...
If this fails on Windows, FirstNexus is not the problem yet.
...
Fix that before touching FirstNexus config.

### Layer 3: Configure the correct browser profile

For raw remote CDP, point FirstNexus at the address that is reachable from WSL2:
...
- use HTTP(S) when you want FirstNexus to discover `/json/version`
- use WS(S) only when the browser provider gives you a direct DevTools socket URL
- test the same URL with `curl` before expecting FirstNexus to succeed
...
```bash
FirstNexus browser open https://example.com --browser-profile remote
FirstNexus browser tabs --browser-profile remote
```

Good result:

- the tab opens in Windows Chrome
- `FirstNexus browser tabs` returns the target
...
- stale viewport / dark-mode / locale / offline overrides after a remote session
  - run `FirstNexus browser stop --browser-profile remote`
...
3. FirstNexus config: does `browser.profiles.<name>.cdpUrl` use that exact WSL2-reachable address?
...
- only then debug FirstNexus config or Control UI auth
