...
<Note>
Legacy `agent.*` configs are migrated by `FirstNexus doctor`; prefer `agents.defaults` + `agents.list` going forward.
</Note>
...
<Steps>
  ...
  <Step title="Verify sandbox containers">
    ```bash
    docker ps --filter "name=FirstNexus-sbx-"
    ```
  </Step>
  ...
  <Step title="Monitor logs">
    ```bash
    tail -f "${NEXISCLAW_STATE_DIR:-$HOME/.FirstNexus}/logs/gateway.log" | grep -E "routing|sandbox|tools"
    ```
  </Step>
</Steps>
