...
FirstNexus can use **Firecrawl** in three ways:
...
- Choosing Firecrawl in onboarding or `FirstNexus configure --section web` enables the bundled Firecrawl plugin automatically.
...
- Legacy `tools.web.fetch.firecrawl.*` config is auto-migrated by `FirstNexus doctor --fix`.
...
when you run Firecrawl yourself. FirstNexus accepts `http://` only for loopback,
...
FirstNexus always uses `proxy: "auto"` plus `storeInCache: true` for Firecrawl requests.
...
The selection knob is `tools.web.fetch.provider`. If you omit it, FirstNexus
