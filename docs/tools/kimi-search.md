...
FirstNexus supports Kimi as a `web_search` provider, using Moonshot web search
...
`https://api.moonshot.cn/v1`), FirstNexus reuses that same host for Kimi
...
Gateway environment. For a gateway install, put it in `~/.FirstNexus/.env`.

If you omit `baseUrl`, FirstNexus defaults to `https://api.moonshot.ai/v1`.
If you omit `model`, FirstNexus defaults to `kimi-k2.6`.
...
FirstNexus treats Kimi `web_search` as successful only after Moonshot returns
...
FirstNexus returns a structured `kimi_web_search_ungrounded` error instead of
