...
<Steps>
  ...
  <Step title="Pick a default model (optional)">
    ...
    `openai-codex` OAuth profile is configured, FirstNexus routes image
...
nearby geometry option instead of the exact requested one, FirstNexus remaps to
...
FirstNexus tries providers in this order:
...
<AccordionGroup>
  ...
  <Accordion title="Auto-detection is auth-aware">
    A provider default only enters the candidate list when FirstNexus can
  ...
  <Accordion title="Timeouts">
    ...
    by FirstNexus's 600000 ms dynamic-tool bridge maximum.
  </Accordion>
...
<AccordionGroup>
  <Accordion title="OpenAI gpt-image-2 (and gpt-image-1.5)">
    ...
    `openai-codex` OAuth profile is configured, FirstNexus reuses the same
    ...
    `https://chatgpt.com/backend-api/codex` for image requests. FirstNexus
    ...
    FirstNexus forwards `prompt`, `count`, `size`, `quality`, `outputFormat`,
    and reference images to OpenAI. OpenAI does **not** receive
    `aspectRatio` or `resolution` directly; when possible FirstNexus maps
    ...
    transparency-capable OpenAI image model. FirstNexus routes default
  ...
  <Accordion title="OpenRouter image models">
    ...
    FirstNexus forwards `prompt`, `count`, reference images, and
  ...
  <Accordion title="xAI grok-imagine-image">
    ...
    - Resolutions: `1K`, `2K`
    - Outputs: returned as FirstNexus-managed image attachments

    FirstNexus intentionally does not expose xAI-native `quality`, `mask`,
...
```text
/tool image_generate action=generate model=openai/gpt-image-2 prompt="A clean editorial poster for FirstNexus image generation" size=3840x2160 count=1
```
  </Tab>
...
`FirstNexus infer image edit`; `--openai-background` remains as an
