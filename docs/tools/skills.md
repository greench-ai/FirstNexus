...
FirstNexus uses **[AgentSkills](https://agentskills.io)-compatible** skill
folders to teach the agent how to use tools. Each skill is a directory
containing a `SKILL.md` with YAML frontmatter and instructions. FirstNexus
...
FirstNexus loads skills from these sources, **highest precedence first**:
...
| 4   | Managed/local skills  | `~/.FirstNexus/skills`           |
...
Codex CLI's native `$CODEX_HOME/skills` directory is not one of these FirstNexus
...
Use `FirstNexus migrate codex --dry-run` to inventory them and
`FirstNexus migrate codex` to choose skill directories with an interactive
checkbox prompt before copying them into the current FirstNexus agent workspace.
...
| Shared managed/local | `~/.FirstNexus/skills`                      | All agents on that machine  |
...
`FirstNexus.plugin.json` (paths relative to the plugin root). Plugin skills
...
`metadata.FirstNexus.requires.config` on the plugin's config entry.
...
[ClawHub](https://clawhub.ai) is the public skills registry for FirstNexus.
Use native `FirstNexus skills` commands for discover/install/update, or the
separate `clawhub` CLI for publish/sync workflows. Full guide:
[ClawHub](/clawhub).

| Action                             | Command                                  |
| ---------------------------------- | ---------------------------------------- |
| Install a skill into the workspace | `FirstNexus skills install <skill-slug>` |
| Update all installed skills        | `FirstNexus skills update --all`         |
| Sync (scan + publish updates)      | `clawhub sync --all`                     |

Native `FirstNexus skills install` installs into the active workspace
...
configured FirstNexus workspace). FirstNexus picks that up as
...
`FirstNexus skills install <slug>` or ClawHub install flow. It is off by default
and only works when `skills.install.allowUploadedArchives: true` is set in
`FirstNexus.json`. Upload mode still installs into the default agent workspace
...
`FirstNexus skills install <slug>` remains only the install path; publishers
...
- `FirstNexus skills install <slug>` is different - it downloads a ClawHub skill folder into the workspace and does not use the installer-metadata path above.
...
FirstNexus follows the AgentSkills spec for layout/intent. The parser used
...
<ParamField path="homepage" type="string">
  URL surfaced as "Website" in the macOS Skills UI. Also supported via `metadata.FirstNexus.homepage`.
</ParamField>
<ParamField path="user-invocable" type="boolean" default="true">
  When `true`, the skill is exposed as a user slash command.
</ParamField>
<ParamField path="disable-model-invocation" type="boolean" default="false">
  When `true`, FirstNexus keeps the skill's instructions out of the agent's normal
...
FirstNexus filters skills at load time using `metadata` (single-line JSON):
...
Fields under `metadata.FirstNexus`:
...
<ParamField path="requires.config" type="string[]">
  List of `FirstNexus.json` paths that must be truthy.
</ParamField>
...
If no `metadata.FirstNexus` is present, the skill is always eligible (unless
disabled in config or blocked by `skills.allowBundled` for bundled skills).

<Note>
Legacy `metadata.clawdbot` blocks are still accepted when
`metadata.FirstNexus` is absent, so older installed skills keep their
...
<AccordionGroup>
  <Accordion title="Installer selection rules">
    ...
    - If all installers are `download`, FirstNexus lists each entry so you can see the available artifacts.
    - Installer specs can include `os: ["darwin"|"linux"|"win32"]` to filter options by platform.
    - Node installs honor `skills.install.nodeManager` in `FirstNexus.json` (default: npm; options: npm/pnpm/yarn/bun). This only affects skill installs; the Gateway runtime should still be Node - Bun is not recommended for WhatsApp/Telegram.
    - Gateway-backed installer selection is preference-driven: when install specs mix kinds, FirstNexus prefers Homebrew when `skills.install.preferBrew` is enabled and `brew` exists, then `uv`, then the configured node manager, then other fallbacks like `go` or `download`.
    - If every install spec is `download`, FirstNexus surfaces all download options instead of collapsing to one preferred installer.

  </Accordion>
  <Accordion title="Per-installer details">
    - **Go installs:** if `go` is missing and `brew` is available, the gateway installs Go via Homebrew first and sets `GOBIN` to Homebrew's `bin` when possible.
    - **Download installs:** `url` (required), `archive` (`tar.gz` | `tar.bz2` | `zip`), `extract` (default: auto when archive detected), `stripComponents`, `targetDir` (default: `~/.FirstNexus/tools/<skillKey>`).
...
under `skills.entries` in `~/.FirstNexus/FirstNexus.json`:
...
<ParamField path="apiKey" type='string | { source, provider, id }'>
  Convenience for skills that declare `metadata.FirstNexus.primaryEnv`. Supports plaintext or SecretRef.
</ParamField>
...
defines `metadata.FirstNexus.skillKey`, use that key under `skills.entries`.

<Note>
For stock image generation/editing inside FirstNexus, use the core
...
When an agent run starts, FirstNexus:
...
For the bundled `claude-cli` backend, FirstNexus also materializes the same
...
FirstNexus still owns precedence, per-agent allowlists, gating, and
`skills.entries.*` env/API key injection. Other CLI backends use the
prompt catalog only.

## Snapshots and refresh

FirstNexus snapshots the eligible skills **when a session starts** and
...
session, FirstNexus refreshes the snapshot so visible skills stay aligned
with the current agent.

### Skills watcher

By default, FirstNexus watches skill folders and bumps the skills snapshot
...
FirstNexus can treat macOS-only skills as eligible when the required
...
probes, FirstNexus clears its cached bin matches so agents no longer see
skills that cannot currently run there.

## Token impact

When skills are eligible, FirstNexus injects a compact XML list of available
...
FirstNexus ships a baseline set of skills as **bundled skills** with the
install (npm package or FirstNexus.app). `~/.FirstNexus/skills` exists for
