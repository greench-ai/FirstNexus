...
FirstNexus supports Perplexity Search API as a `web_search` provider.
It returns structured results with `title`, `url`, and `snippet` fields.

For compatibility, FirstNexus also supports legacy Perplexity Sonar/OpenRouter setups.
...
**Via config:** run `FirstNexus configure --section web`. It stores the key in
`~/.FirstNexus/FirstNexus.json` under `plugins.entries.perplexity.config.webSearch.apiKey`.
...
`~/.FirstNexus/.env` (or your service environment). See [Env vars](/help/faq#env-vars-and-env-loading).
