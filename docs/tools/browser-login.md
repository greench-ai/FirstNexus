...
When a site requires login, **sign in manually** in the **host** browser profile (the FirstNexus browser).
...
FirstNexus controls a **dedicated Chrome profile** (named `FirstNexus`, orange-tinted UI). This is separate from your daily browser profile.

For agent browser tool calls:

- Default choice: the agent should use its isolated `FirstNexus` browser.
...
FirstNexus browser open https://x.com
```

If you have multiple profiles, pass `--browser-profile <name>` (the default is `FirstNexus`).
...
```bash
FirstNexus browser open https://x.com --browser-profile FirstNexus
```
