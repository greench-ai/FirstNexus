...
title: "Tokenjuice"
read_when:
  - You want shorter `exec` or `bash` tool results in FirstNexus
...
Today this applies to PI embedded runs and FirstNexus dynamic tools in the Codex
app-server harness. Tokenjuice hooks FirstNexus's tool-result middleware and
...
```bash
FirstNexus config set plugins.entries.tokenjuice.enabled true
```

Equivalent:

```bash
FirstNexus plugins enable tokenjuice
```

FirstNexus already ships the plugin. There is no separate `plugins install`
or `tokenjuice install FirstNexus` step.
...
```bash
FirstNexus config set plugins.entries.tokenjuice.enabled false
```

Or:

```bash
FirstNexus plugins disable tokenjuice
```
