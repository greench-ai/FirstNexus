...
host-local `ask: "always"` in `~/.FirstNexus/exec-approvals.json` keeps
...
| ------------------------------------------------------------------ | -------------------------------------------------------------------------------------- |
| `FirstNexus approvals get` / `--gateway` / `--node <id\|name\|ip>` | Requested policy, host policy sources, and the effective result.                       |
| `FirstNexus exec-policy show`                                      | Local-machine merged view.                                                             |
| `FirstNexus exec-policy set` / `preset`                            | Synchronize the local requested policy with the local host approvals file in one step. |
...
- **Gateway host** → `FirstNexus` process on the gateway machine.
...
- For shell scripts and direct interpreter/runtime file invocations, FirstNexus also tries to bind one concrete local file operand. If that bound file changes after approval but before execution, the run is denied instead of executing drifted content.
...
```text
~/.FirstNexus/exec-approvals.json
```
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.FirstNexus/exec-approvals.sock",
...
<ParamField path="strictInlineEval" type="boolean">
  When `true`, FirstNexus treats inline code-eval forms as approval-only
...
<ParamField path="commandHighlighting" type="boolean" default="false">
  ...
  FirstNexus may attach parser-derived command spans so Web approval
...
**both** policy layers - requested exec policy in FirstNexus config
...
- In YOLO mode, FirstNexus does **not** add a separate heuristic command-obfuscation approval gate or script-preflight rejection layer on top of the configured host exec policy.
...
`--permission-mode bypassPermissions` when FirstNexus's requested exec
...
<Steps>
  ...
  <Step title="Match the host approvals file">
    ```bash
    FirstNexus approvals set --stdin <<'EOF'
    {
      version: 1,
      defaults: {
        security: "full",
...
```bash
FirstNexus exec-policy preset yolo
```

That local shortcut updates both:

- Local `tools.exec.host/security/ask`.
- Local `~/.FirstNexus/exec-approvals.json` defaults.

It is intentionally local-only. To change gateway-host or node-host
approvals remotely, use `FirstNexus approvals set --gateway` or
`FirstNexus approvals set --node <id|name|ip>`.
...
```bash
FirstNexus approvals set --node <id|name|ip> --stdin <<'EOF'
{
  version: 1,
  defaults: {
    security: "full",
...
- `FirstNexus exec-policy` does not synchronize node approvals.
- `FirstNexus exec-policy set --host node` is rejected.
- Node exec approvals are fetched from the node at runtime, so node-targeted updates must use `FirstNexus approvals --node ...`.
...
specific argument shape. FirstNexus evaluates the regular expression
...
entries instead of hand-editing the encoded value. If FirstNexus cannot
...
edit its local `~/.FirstNexus/exec-approvals.json` directly.

CLI: `FirstNexus approvals` supports gateway or node editing - see
...
When an async exec approval is denied, FirstNexus prevents the agent from
