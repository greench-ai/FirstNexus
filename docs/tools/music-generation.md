...
For session-backed agent runs, FirstNexus starts music generation as a
...
completion agent writes only a private final reply, FirstNexus falls back to a
...
<ParamField path="timeoutMs" type="number">Optional provider request timeout in milliseconds. When omitted, FirstNexus uses `agents.defaults.musicGenerationModel.timeoutMs` if configured. Values below 10000ms are raised to 10000ms and reported in the tool result.</ParamField>

<Note>
Not all providers support all parameters. FirstNexus still validates hard
limits such as input counts before submission. When a provider supports
duration but uses a shorter maximum than the requested value, FirstNexus
...
- **Status lookup:** `FirstNexus tasks list` or `FirstNexus tasks show <taskId>`
  inspects queued, running, and terminal status.
- **Completion wake:** FirstNexus injects an internal completion event back
