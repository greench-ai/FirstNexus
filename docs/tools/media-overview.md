...
read_when:
  - Looking for an overview of FirstNexus's media capabilities
...
FirstNexus generates images, videos, and music, understands inbound media
...
For async tools, FirstNexus submits the request to the provider, returns a task
...
FirstNexus wakes the agent with the generated media paths so it can tell the
user and, when required by source-delivery policy, relay the result through
the message tool. For message-tool-only group/channel routes, FirstNexus treats
...
<AccordionGroup>
  ...
  <Accordion title="DeepInfra">
    ...
    registered until FirstNexus has dedicated provider contracts for those
    categories.
  </Accordion>
  <Accordion title="xAI">
    ...
    not registered in FirstNexus until the shared realtime-voice contract can
    represent it.
  </Accordion>
</AccordionGroup>
