...
- `FirstNexus tasks list` shows all tasks; `FirstNexus tasks audit` surfaces issues.
...
| CLI operations         | `cli`        | `FirstNexus agent` commands that run through the gateway | `silent`              |
...
<AccordionGroup>
  <Accordion title="Notify defaults for cron and media">
    ...
    Session-backed `music_generate` and `video_generate` runs also use `silent` notify policy. They still create task records, but completion is handed back to the original agent session as an internal wake so the agent can write the follow-up message and attach the finished media itself. Group/channel completions follow the normal visible-reply policy, so the agent uses the message tool when source delivery requires it. If the completion agent fails to produce message-tool delivery evidence in a tool-only route, FirstNexus sends the completion fallback directly to the original channel instead of leaving the media private.
...
| `cancelled` | Stopped by the operator via `FirstNexus tasks cancel`                      |
...
- CLI tasks: tasks with a run id/source id use the live run context, so
  ...
  back to the child session. Gateway-backed `FirstNexus agent` runs also finalize
  from their run result, so completed runs do not sit active until the sweeper
  marks them `lost`.

## Delivery and notifications

When a task reaches a terminal state, FirstNexus notifies you. There are two delivery paths:

**Direct delivery** - if the task has a channel target (the `requesterOrigin`), the completion message goes straight to that channel (Telegram, Discord, Slack, etc.). Group and channel task completions are instead routed through the requester session so the parent agent can write the visible reply. For subagent completions, FirstNexus also preserves bound thread/topic routing when available and can fill a missing `to` / account from the requester session's stored route (`lastChannel` / `lastTo` / `lastAccountId`) before giving up on direct delivery.
...
<AccordionGroup>
  <Accordion title="tasks list">
    ```bash
    FirstNexus tasks list [--runtime <acp|subagent|cron|cli>] [--status <status>] [--json]
    ```
  ...
  <Accordion title="tasks audit">
    ```bash
    FirstNexus tasks audit [--json]
    ```

    Surfaces operational issues. Findings also appear in `FirstNexus status` when issues are detected.
  ...
  <Accordion title="tasks maintenance">
    ```bash
    FirstNexus tasks maintenance [--json]
    ...
    When applying maintenance, FirstNexus also removes stale `cron:<jobId>:run:<uuid>` session registry rows older than 7 days, while preserving rows for currently running cron jobs and leaving non-cron session rows untouched.

  </Accordion>
  <Accordion title="tasks flow list | show | cancel">
    ```bash
    FirstNexus tasks flow list [--status <status>] [--json]
...
For the full operator ledger, use the CLI: `FirstNexus tasks list`.

## Status integration (task pressure)

`FirstNexus status` includes an at-a-glance task summary:
...
<AccordionGroup>
  <Accordion title="Tasks and Task Flow">
    [Task Flow](/automation/taskflow) is the flow orchestration layer above background tasks. A single flow may coordinate multiple tasks over its lifetime using managed or mirrored sync modes. Use `FirstNexus tasks` to inspect individual task records and `FirstNexus tasks flow` to inspect the orchestrating flow.

    See [Task Flow](/automation/taskflow) for details.

  </Accordion>
  <Accordion title="Tasks and cron">
    A cron job **definition** lives in `~/.FirstNexus/cron/jobs.json`; runtime execution state lives beside it in `~/.FirstNexus/cron/jobs-state.json`. **Every** cron execution creates a task record - both main-session and isolated. Main-session cron tasks default to `silent` notify policy so they track without generating notifications.
