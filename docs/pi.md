---
summary: "Architecture of FirstNexus's embedded Pi agent integration and session lifecycle"
title: "Pi integration architecture"
read_when:
  - Understanding Pi SDK integration design in FirstNexus
  - Modifying agent session lifecycle, tooling, or provider wiring for Pi
---

FirstNexus integrates with [pi-coding-agent](https://github.com/badlogic/pi-mono/tree/main/packages/coding-agent) and its sibling packages (`pi-ai`, `pi-agent-core`, `pi-tui`) to power its AI agent capabilities.

## Overview

FirstNexus uses the pi SDK to embed an AI coding agent into its messaging gateway architecture. Instead of spawning pi as a subprocess or using RPC mode, FirstNexus directly imports and instantiates pi's `AgentSession` via `createAgentSession()`. This embedded approach provides:
...
| `pi-tui`          | Terminal UI components (used in FirstNexus's local TUI mode)                                           |
...
├── pi-tools.ts                    # createFirstNexusCodingTools()
...
├── FirstNexus-tools.ts              # FirstNexus-specific tools
...
Image injection is prompt-local: FirstNexus loads image refs from the current prompt and
...
2. **Custom Replacements**: FirstNexus replaces bash with `exec`/`process`, customizes read/edit/write for sandbox
3. **FirstNexus Tools**: messaging, browser, canvas, sessions, cron, gateway, etc.
...
FirstNexus wraps this with `guardSessionManager()` for tool result safety.
...
FirstNexus maintains an auth profile store with multiple API keys per provider:
...
FirstNexus loads custom pi extensions for specialized behavior:
...
FirstNexus also has a local TUI mode that uses pi-tui components directly:
...
| Aspect          | Pi CLI                  | FirstNexus Embedded                                                                               |
| --------------- | ----------------------- | ------------------------------------------------------------------------------------------------- |
| Invocation      | `pi` command / RPC      | SDK via `createAgentSession()`                                                                    |
| Tools           | Default coding tools    | Custom FirstNexus tool suite                                                                      |
| System prompt   | AGENTS.md + prompts     | Dynamic per-channel/context                                                                       |
| Session storage | `~/.pi/agent/sessions/` | `~/.FirstNexus/agents/<agentId>/sessions/` (or `$NEXISCLAW_STATE_DIR/agents/<agentId>/sessions/`) |
