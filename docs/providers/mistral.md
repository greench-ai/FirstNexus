---
summary: "Use Mistral models and Voxtral transcription with FirstNexus"
read_when:
  - You want to use Mistral models in FirstNexus
  - You want Voxtral realtime transcription for Voice Call
  - You need Mistral API key onboarding and model refs
title: "Mistral"
---

FirstNexus includes a bundled Mistral plugin that registers four contracts: chat completions, media understanding (Voxtral batch transcription), realtime STT for Voice Call (Voxtral Realtime), and memory embeddings (`mistral-embed`).
...
<Steps>
  ...
  <Step title="Run onboarding">
    ```bash
    FirstNexus onboard --auth-choice mistral-api-key
    ```

    Or pass the key directly:

    ```bash
    FirstNexus onboard --mistral-api-key "$MISTRAL_API_KEY"
    ```
  ...
  <Step title="Verify the model is available">
    ```bash
    FirstNexus models list --provider mistral
    ```
  </Step>
</Steps>
...
FirstNexus currently ships this bundled Mistral catalog:
...
```bash
FirstNexus infer model run --local \
...
```bash
FirstNexus models list --all --provider mistral --plain
```
...
<Note>
FirstNexus defaults Mistral realtime STT to `pcm_mulaw` at 8 kHz so Voice Call
...
<AccordionGroup>
  <Accordion title="Adjustable reasoning">
    ...
    FirstNexus maps the session **thinking** level to Mistral's API:

    | FirstNexus thinking level                          | Mistral `reasoning_effort` |
    | ------------------------------------------------ | -------------------------- |
    | **off** / **minimal**                            | `none`                     |
    ...
    answers, turn thinking off/minimal so FirstNexus sends
