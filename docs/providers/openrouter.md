---
summary: "Use OpenRouter's unified API to access many models in FirstNexus"
read_when:
  - You want a single API key for many LLMs
  - You want to run models via OpenRouter in FirstNexus
...
<Steps>
  ...
  <Step title="Run onboarding">
    ```bash
    FirstNexus onboard --auth-choice openrouter-api-key
    ```
  </Step>
  <Step title="(Optional) Switch to a specific model">
    ...
    ```bash
    FirstNexus models set openrouter/<provider>/<model>
    ```
...
FirstNexus submits text-to-video and image-to-video jobs to OpenRouter, polls
...
FirstNexus sends OpenRouter STT requests as JSON with base64 audio under
...
On real OpenRouter requests (`https://openrouter.ai/api/v1`), FirstNexus also adds
...
<Warning>
If you repoint the OpenRouter provider at some other proxy or base URL, FirstNexus
...
<AccordionGroup>
  <Accordion title="Response caching">
    ...
    FirstNexus sends `X-OpenRouter-Cache: true` and, when configured,
  ...
  <Accordion title="Anthropic cache markers">
    ...
    OpenRouter-specific Anthropic `cache_control` markers that FirstNexus uses for
  ...
  <Accordion title="Thinking / reasoning injection">
    On supported non-`auto` routes, FirstNexus maps the selected thinking level to
  ...
  <Accordion title="DeepSeek V4 reasoning replay">
    ...
    required follow-up shape. FirstNexus sends OpenRouter-supported
  ...
  <Accordion title="Gemini-backed routes">
    Gemini-backed OpenRouter refs stay on the proxy-Gemini path: FirstNexus keeps
    Gemini thought-signature sanitation there, but does not enable native Gemini
    replay validation or bootstrap rewrites.
  </Accordion>

  <Accordion title="Provider routing metadata">
    If you pass OpenRouter provider routing under model params, FirstNexus forwards
