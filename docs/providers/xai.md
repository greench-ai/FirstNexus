---
summary: "Use xAI Grok models in FirstNexus"
read_when:
  - You want to use Grok models in FirstNexus
  - You are configuring xAI auth or model ids
title: "xAI"
---

FirstNexus ships a bundled `xai` provider plugin for Grok models.
...
<Steps>
  ...
  <Step title="Set your API key">
    ...
    ```bash
    FirstNexus onboard --auth-choice xai-api-key
    ```
...
<Note>
FirstNexus uses the xAI Responses API as the bundled xAI transport. The same
API key from `FirstNexus onboard --auth-choice xai-api-key` can also power
...
FirstNexus includes these xAI model families out of the box:
...
The bundled plugin maps xAI's current public API surface onto FirstNexus's shared
...
| xAI capability             | FirstNexus surface                        | Status                                                                |
| -------------------------- | ----------------------------------------- | --------------------------------------------------------------------- |
...
| Streaming TTS              | -                                         | Not exposed; FirstNexus's TTS contract returns complete audio buffers |
| Batch speech-to-text       | `tools.media.audio` / media understanding | Yes                                                                   |
| Streaming speech-to-text   | Voice Call `streaming.provider: "xai"`    | Yes                                                                   |
| Realtime voice             | -                                         | Not exposed yet; different session/WebSocket contract                 |
| Files / batches            | Generic model API compatibility only      | Not a first-class FirstNexus tool                                     |

<Note>
FirstNexus uses xAI's REST image/video/TTS/STT APIs for media generation,
...
code-execution tools. Features that need different FirstNexus contracts, such as
...
<AccordionGroup>
  <Accordion title="Web search">
    ...
    ```bash
    FirstNexus config set tools.web.search.provider grok
    ```
  ...
  <Accordion title="Video generation">
    ...
    FirstNexus can encode those as data URLs for xAI.
    </Warning>
  ...
  <Accordion title="Image generation">
    ...
    FirstNexus asks xAI for `b64_json` image responses so generated media can be
    ...
    such as `1:2`, `2:1`, `9:20`, and `20:9`. FirstNexus forwards only the
  ...
  <Accordion title="Text-to-speech">
    ...
    <Note>
    FirstNexus uses xAI's batch `/v1/tts` endpoint. xAI also offers streaming TTS
    over WebSocket, but the FirstNexus speech provider contract currently expects
  ...
  <Accordion title="Speech-to-text">
    The bundled `xai` plugin registers batch speech-to-text through FirstNexus's
    ...
    transcription request. Prompt hints are accepted by the shared FirstNexus
  ...
  <Accordion title="x_search configuration">
    The bundled xAI plugin exposes `x_search` as an FirstNexus tool for searching
  ...
  <Accordion title="Code execution configuration">
    The bundled xAI plugin exposes `code_execution` as an FirstNexus tool for
  ...
  <Accordion title="Known limits">
    - Auth is API-key only today. The API key may be stored in an xAI auth
      ...
      device-code flow in FirstNexus yet.
    - `grok-4.20-multi-agent-experimental-beta-0304` is not supported on the
      normal xAI provider path because it requires a different upstream API
      surface than the standard FirstNexus xAI transport.
    - xAI Realtime voice is not registered as an FirstNexus provider yet. It
  ...
  <Accordion title="Advanced notes">
    - FirstNexus applies xAI-specific tool-schema and tool-call compatibility fixes
    ...
    - `web_search`, `x_search`, and `code_execution` are exposed as FirstNexus
      tools. FirstNexus enables the specific xAI built-in it needs inside each tool
...
NEXISCLAW_LIVE_TEST=1 NEXISCLAW_LIVE_TEST_QUIET=1 pnpm test:live -- extensions/xai/xai.live.test.ts
NEXISCLAW_LIVE_TEST=1 NEXISCLAW_LIVE_TEST_QUIET=1 NEXISCLAW_LIVE_IMAGE_GENERATION_PROVIDERS=xai pnpm test:live -- test/image-generation.runtime.live.test.ts
```
...
shared image live file verifies the same xAI provider through FirstNexus's
