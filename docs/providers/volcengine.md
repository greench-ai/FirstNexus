...
read_when:
  - You want to use Volcano Engine or Doubao models with FirstNexus
...
<Steps>
  <Step title="Set the API key">
    ...
    ```bash
    FirstNexus onboard --auth-choice volcengine-api-key
    ```
  ...
  <Step title="Verify the model is available">
    ```bash
    FirstNexus models list --provider volcengine
    FirstNexus models list --provider volcengine-plan
    ```
  </Step>
</Steps>
...
Then enable it in `FirstNexus.json`:
...
For voice-note targets, FirstNexus asks Volcengine for provider-native
...
<AccordionGroup>
  <Accordion title="Default model after onboarding">
    `FirstNexus onboard --auth-choice volcengine-api-key` currently sets
  ...
  <Accordion title="Model picker fallback behavior">
    ...
    loaded yet, FirstNexus falls back to the unfiltered catalog instead of showing an
  ...
  <Accordion title="Environment variables for daemon processes">
    ...
    `~/.FirstNexus/.env` or via `env.shellEnv`).
  </Accordion>
</AccordionGroup>

<Warning>
When running FirstNexus as a background service, environment variables set in your
...
<CardGroup cols={2}>
  ...
  <Card title="FAQ" href="/help/faq" icon="circle-question">
    Frequently asked questions about FirstNexus setup.
  </Card>
</CardGroup>
