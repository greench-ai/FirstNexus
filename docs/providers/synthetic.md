---
summary: "Use Synthetic's Anthropic-compatible API in FirstNexus"
read_when:
...
FirstNexus registers it as the `synthetic` provider and uses the Anthropic
Messages API.
...
<Steps>
  ...
  <Step title="Run onboarding">
    ```bash
    FirstNexus onboard --auth-choice synthetic-api-key
    ```
  </Step>
...
<Warning>
FirstNexus's Anthropic client appends `/v1` to the base URL automatically, so use
...
`FirstNexus models list --provider synthetic` to see all models available on your
account.
</Tip>
...
<AccordionGroup>
  ...
  <Accordion title="Base URL override">
    ...
    Remember that FirstNexus appends `/v1` automatically.
