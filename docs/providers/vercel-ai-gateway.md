...
read_when:
  - You want to use Vercel AI Gateway with FirstNexus
...
<Tip>
FirstNexus auto-discovers the Gateway `/v1/models` catalog, so
...
<Steps>
  <Step title="Set the API key">
    ...
    ```bash
    FirstNexus onboard --auth-choice ai-gateway-api-key
    ```
  ...
  <Step title="Verify the model is available">
    ```bash
    FirstNexus models list --provider vercel-ai-gateway
    ```
  </Step>
</Steps>
...
FirstNexus accepts Vercel Claude shorthand model refs and normalizes them at
runtime:
...
configuration. FirstNexus resolves the canonical form automatically.
</Tip>
...
<AccordionGroup>
  <Accordion title="Environment variable for daemon processes">
    If the FirstNexus Gateway runs as a daemon (launchd/systemd), make sure
    ...
    `~/.FirstNexus/.env` or via `env.shellEnv` to ensure the gateway process can
    read it.
    </Warning>
  ...
  <Accordion title="Thinking levels">
    `/think` options follow trusted upstream model prefixes when FirstNexus knows
