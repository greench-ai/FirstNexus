---
summary: "Use NVIDIA's OpenAI-compatible API in FirstNexus"
read_when:
  - You want to use open models in FirstNexus for free
...
<Steps>
  ...
  <Step title="Export the key and run onboarding">
    ...
    FirstNexus onboard --auth-choice nvidia-api-key
    ```
  </Step>
  <Step title="Set an NVIDIA model">
    ```bash
    FirstNexus models set nvidia/nvidia/nemotron-3-super-120b-a12b
    ```
  </Step>
</Steps>
...
```bash
FirstNexus onboard --auth-choice nvidia-api-key --nvidia-api-key "nvapi-..."
```
