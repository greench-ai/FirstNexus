---
summary: "Use DeepInfra's unified API to access the most popular open source and frontier models in FirstNexus"
read_when:
  - You want a single API key for the top open source LLMs
  - You want to run models via DeepInfra's API in FirstNexus
title: "DeepInfra"
---
...
```bash
FirstNexus onboard --deepinfra-api-key <key>
```
...
## Supported FirstNexus surfaces

The bundled plugin registers all DeepInfra surfaces that match current
FirstNexus provider contracts:

| Surface                  | Default model                      | FirstNexus config/tool                                   |
| ------------------------ | ---------------------------------- | -------------------------------------------------------- |
...
native model types. FirstNexus does not currently have first-class provider
contracts for those categories, so this plugin does not register them yet.

## Available models

FirstNexus dynamically discovers available DeepInfra models at startup. Use
