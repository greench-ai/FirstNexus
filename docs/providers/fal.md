---
summary: "fal image and video generation setup in FirstNexus"
title: "Fal"
read_when:
  - You want to use fal image generation in FirstNexus
  - You need the FAL_KEY auth flow
  - You want fal defaults for image_generate or video_generate
---

FirstNexus ships a bundled `fal` provider for hosted image and video generation.
...
<Steps>
  <Step title="Set the API key">
    ```bash
    FirstNexus onboard --auth-choice fal-api-key
    ```
  </Step>
...
explicit transparent-background control in FirstNexus, so `background:
...
<Tip>
Use `FirstNexus models list --provider fal` to see the full list of available fal
