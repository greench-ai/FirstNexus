---
summary: "Symptom first troubleshooting hub for FirstNexus"
read_when:
  - FirstNexus is not working and you need the fastest path to a fix
...
- `FirstNexus status` → shows configured channels and no obvious auth errors.
- `FirstNexus status --all` → full report is present and shareable.
- `FirstNexus gateway probe` → expected gateway target is reachable (`Reachable: yes`). `Capability: ...` tells you what auth level the probe could prove, and `Read probe: limited - missing scope: operator.read` is degraded diagnostics, not a connect failure.
- `FirstNexus gateway status` → `Runtime: running`, `Connectivity probe: ok`, and a plausible `Capability: ...` line. Use `--require-rpc` if you need read-scope RPC proof too.
- `FirstNexus doctor` → no blocking config/service errors.
- `FirstNexus channels status --probe` → reachable gateway returns live per-account
...
- `FirstNexus logs --follow` → steady activity, no repeating fatal errors.
...
## Local OpenAI-compatible backend works directly but fails in FirstNexus

If your local or self-hosted `/v1` backend answers small direct
`/v1/chat/completions` probes but fails on `FirstNexus infer model run` or normal
agent turns:

1. If the error mentions `messages[].content` expecting a string, set
   `models.providers.<provider>.models[].compat.requiresStringContent: true`.
2. If the backend still fails only on FirstNexus agent turns, set
   `models.providers.<provider>.models[].compat.supportsTools: false` and retry.
3. If tiny direct calls still work but larger FirstNexus prompts crash the
...
## Plugin install fails with missing FirstNexus extensions

If install fails with `package.json missing FirstNexus.extensions`, the plugin package
is using an old shape that FirstNexus no longer accepts.

Fix in the plugin package:

1. Add `FirstNexus.extensions` to `package.json`.
2. Point entries at built runtime files (usually `./dist/index.js`).
3. Republish the plugin and run `FirstNexus plugins install <package>` again.

Example:

```json
{
  "name": "@FirstNexus/my-plugin",
  "version": "1.2.3",
  "FirstNexus": {
...
If `FirstNexus doctor`, setup, or startup warnings show:
...
them. Do not remove the plugin config. Fix the file ownership or run FirstNexus as
...
```mermaid
flowchart TD
  A[FirstNexus is not working] --> B{What breaks first}
  B --> C[No replies]
...
<AccordionGroup>
  ...
  <Accordion title="Dashboard or Control UI will not connect">
    ...
    - `Dashboard: http://...` is shown in `FirstNexus gateway status`
    - `Connectivity probe: ok`
    - `Capability: read-only`, `write-capable`, or `admin-capable`
  ...
  <Accordion title="Exec suddenly asks for approval">
    ```bash
    FirstNexus config get tools.exec.host
    FirstNexus config get tools.exec.security
    FirstNexus config get tools.exec.ask
    ...
    ```bash
    FirstNexus config set tools.exec.host gateway
    FirstNexus config set tools.exec.security full
    FirstNexus config set tools.exec.ask off
    FirstNexus gateway restart
    ```
  ...
  <Accordion title="Browser tool fails">
    ...
    - `FirstNexus` starts, or `user` can see local Chrome tabs.
    ...
    - stale viewport / dark-mode / locale / offline overrides on attach-only or remote CDP profiles → run `FirstNexus browser stop --browser-profile <name>` to close the active control session and release emulation state without restarting the gateway.
