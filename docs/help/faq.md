---
summary: "Frequently asked questions about FirstNexus setup, configuration, and usage"
read_when:
...
5. **Tail the latest log**
   ...
   ```bash
   tail -f "$(ls -t /tmp/FirstNexus/FirstNexus-*.log | head -1)"
   ```
...
7. **Gateway snapshot**
   ...
   ```bash
   FirstNexus health --json
   FirstNexus health --verbose   # shows the target URL + config path on errors
   ```
...
<AccordionGroup>
  ...
  <Accordion title="What are the top five everyday use cases for FirstNexus?">
  ...
  <Accordion title="Can FirstNexus help with lead gen, outreach, ads, and blogs for a SaaS?">
    Yes for **research, qualification, and drafting**. It can scan sites, build shortlists,
    summarize prospects, and write outreach or ad copy drafts.

    For **outreach or ad runs**, keep a human in the loop. Avoid spam, follow local laws and
    platform policies, and review anything before it is sent. The safest pattern is to let
    FirstNexus draft and you approve.
  ...
  <Accordion title="What are the advantages vs Claude Code for web development?">
    FirstNexus is a **personal assistant** and coordination layer, not an IDE replacement. Use
    Claude Code or Codex for the fastest direct coding loop inside a repo. Use FirstNexus when you
    ...
    Showcase: [https://FirstNexus.ai/showcase](https://FirstNexus.ai/showcase)
...
<AccordionGroup>
  <Accordion title="How do I customize skills without keeping the repo dirty?">
    Use managed overrides instead of editing the repo copy. Put your changes in `~/.FirstNexus/skills/<name>/SKILL.md` (or add a folder via `skills.load.extraDirs` in `~/.FirstNexus/FirstNexus.json`). Precedence is `<workspace>/skills` → `<workspace>/.agents/skills` → `~/.agents/skills` → `~/.FirstNexus/skills` → bundled → `skills.load.extraDirs`, so managed overrides still win over bundled skills without touching git. If you need the skill installed globally but only visible to some agents, keep the shared copy in `~/.FirstNexus/skills` and control visibility with `agents.defaults.skills` and `agents.list[].skills`. Only upstream-worthy edits should live in the repo and go out as PRs.
  </Accordion>

  <Accordion title="Can I load skills from a custom folder?">
    Yes. Add extra directories via `skills.load.extraDirs` in `~/.FirstNexus/FirstNexus.json` (lowest precedence). Default precedence is `<workspace>/skills` → `<workspace>/.agents/skills` → `~/.agents/skills` → `~/.FirstNexus/skills` → bundled → `skills.load.extraDirs`. `clawhub` installs into `./skills` by default, which FirstNexus treats as `<workspace>/skills` on the next session. If the skill should only be visible to certain agents, pair that with `agents.defaults.skills` or `agents.list[].skills`.
  </Accordion>
  ...
  <Accordion title="A subagent finished, but the completion update went to the wrong place or never posted. What should I check?">
    ...
    - If the completion origin only carries a channel, FirstNexus falls back to the requester session's stored route (`lastChannel` / `lastTo` / `lastAccountId`) so direct delivery can still succeed.
    ...
    - If the child's last visible assistant reply is the exact silent token `NO_REPLY` / `no_reply`, or exactly `ANNOUNCE_SKIP`, FirstNexus intentionally suppresses the announce instead of posting stale earlier progress.
    ...
    ```bash
    FirstNexus tasks show <runId-or-sessionKey>
    ```
  ...
  <Accordion title="Cron or reminders do not fire. What should I check?">
    ...
    ```bash
    FirstNexus cron run <jobId>
    FirstNexus cron runs --id <jobId> --limit 50
    ```
  ...
  <Accordion title="How do I install skills on Linux?">
    Use native `FirstNexus skills` commands or drop skills into your workspace. The macOS Skills UI isn't available on Linux.
    Browse skills at [https://clawhub.ai](https://clawhub.ai).

    ```bash
    FirstNexus skills search "calendar"
    FirstNexus skills search --limit 20
    FirstNexus skills install <skill-slug>
    FirstNexus skills install <skill-slug> --version <version>
    FirstNexus skills install <skill-slug> --force
    FirstNexus skills update --all
    FirstNexus skills list --eligible
    FirstNexus skills check
    ```

    Native `FirstNexus skills install` writes into the active workspace `skills/`
    ...
    `~/.FirstNexus/skills` and use `agents.defaults.skills` or
    `agents.list[].skills` if you want to narrow which agents can see it.

  </Accordion>

  <Accordion title="Can FirstNexus run tasks on a schedule or continuously in the background?">
  ...
  <Accordion title="Can I run Apple macOS-only skills from Linux?">
    Not directly. macOS skills are gated by `metadata.FirstNexus.os` plus required binaries, and skills only appear in the system prompt when they are eligible on the **Gateway host**. On Linux, `darwin`-only skills (like `apple-notes`, `apple-reminders`, `things-mac`) will not load unless you override the gating.
    ...
    **Option B - use a macOS node (no SSH).**
    Run the Gateway on Linux, pair a macOS node (menubar app), and set **Node Run Commands** to "Always Ask" or "Always Allow" on the Mac. FirstNexus can treat macOS-only skills as eligible when the required binaries exist on the node. The agent runs those skills via the `nodes` tool. If you choose "Always Ask", approving "Always Allow" in the prompt adds that command to the allowlist.
    ...
    **Option C - proxy macOS binaries over SSH (advanced).**
    ...
    3. Override the skill metadata (workspace or `~/.FirstNexus/skills`) to allow Linux:

       ```markdown
       ---
       name: apple-notes
       description: Manage Apple Notes via the memo CLI on macOS.
       metadata: { "FirstNexus": { "os": ["darwin", "linux"], "requires": { "bins": ["memo"] } } }
       ---
       ```
  ...
  <Accordion title="Do you have a Notion or HeyGen integration?">
    ...
    Native installs land in the active workspace `skills/` directory. For shared skills across agents, place them in `~/.FirstNexus/skills/<name>/SKILL.md`. If only some agents should see a shared install, configure `agents.defaults.skills` or `agents.list[].skills`. Some skills expect binaries installed via Homebrew; on Linux that means Linuxbrew (see the Homebrew Linux FAQ entry above). See [Skills](/tools/skills), [Skills config](/tools/skills-config), and [ClawHub](/clawhub).

  </Accordion>

  <Accordion title="How do I use my existing signed-in Chrome with FirstNexus?">
    ...
    FirstNexus browser --browser-profile user snapshot
    ```
...
<AccordionGroup>
  ...
  <Accordion title="How do I bind a host folder into the sandbox?">
    ...
    FirstNexus validates bind sources against both the normalized path and the canonical path resolved through the deepest existing ancestor. That means symlink-parent escapes still fail closed even when the last path segment does not exist yet, and allowed-root checks still apply after symlink resolution.
  ...
  <Accordion title="How does memory work?">
    FirstNexus memory is just Markdown files in the agent workspace:

    - Daily notes in `memory/YYYY-MM-DD.md`
    - Curated long-term notes in `MEMORY.md` (main/private sessions only)

    FirstNexus also runs a **silent pre-compaction memory flush** to remind the model
  ...
  <Accordion title="Does semantic memory search require an OpenAI API key?">
    ...
    If you don't set a provider explicitly, FirstNexus auto-selects a provider when it
    ...
    configured and present, FirstNexus
...
<AccordionGroup>
  <Accordion title="Is all data used with FirstNexus saved locally?">
    No - **FirstNexus's state is local**, but **external services still see what you send them**.

    - **Local by default:** sessions, memory files, config, and workspace live on the Gateway host
      (`~/.FirstNexus` + your workspace directory).
  ...
  <Accordion title="Where does FirstNexus store its data?">
    Everything lives under `$NEXISCLAW_STATE_DIR` (default: `~/.FirstNexus`):

    | Path                                                            | Purpose                                                            |
    | --------------------------------------------------------------- | ------------------------------------------------------------------ |
    | `$NEXISCLAW_STATE_DIR/FirstNexus.json`                             | Main config (JSON5)                                                |
    ...
    Legacy single-agent path: `~/.FirstNexus/agent/*` (migrated by `FirstNexus doctor`).

    Your **workspace** (AGENTS.md, memory files, skills, etc.) is separate and configured via `agents.defaults.workspace` (default: `~/.FirstNexus/workspace`).

  </Accordion>

  <Accordion title="Where should AGENTS.md / SOUL.md / USER.md / MEMORY.md live?">
    These files live in the **agent workspace**, not `~/.FirstNexus`.

    - **Workspace (per agent)**: `AGENTS.md`, `SOUL.md`, `IDENTITY.md`, `USER.md`,
      `MEMORY.md`, `memory/YYYY-MM-DD.md`, optional `HEARTBEAT.md`.
      Lowercase root `memory.md` is legacy repair input only; `FirstNexus doctor --fix`
      can merge it into `MEMORY.md` when both files exist.
    - **State dir (`~/.FirstNexus`)**: config, channel/provider state, auth profiles, sessions, logs,
      and shared skills (`~/.FirstNexus/skills`).

    Default workspace is `~/.FirstNexus/workspace`, configurable via:

    ```json5
    {
      agents: { defaults: { workspace: "~/.FirstNexus/workspace" } },
    }
    ```
  ...
  <Accordion title="Recommended backup strategy">
    ...
    Do **not** commit anything under `~/.FirstNexus` (credentials, sessions, tokens, or encrypted secrets payloads).
  ...
  <Accordion title="How do I completely uninstall FirstNexus?">
  ...
  <Accordion title="Can agents work outside the workspace?">
    ...
    `workspace` to the repo root. The FirstNexus repo is just source code; keep the
...
<AccordionGroup>
  <Accordion title="What format is the config? Where is it?">
    FirstNexus reads an optional **JSON5** config from `$NEXISCLAW_CONFIG_PATH` (default: `~/.FirstNexus/FirstNexus.json`):

    ```
    $NEXISCLAW_CONFIG_PATH
    ```

    If the file is missing, it uses safe-ish defaults (including a default workspace of `~/.FirstNexus/workspace`).
  ...
  <Accordion title="Why do I need a token on localhost now?">
    FirstNexus enforces gateway auth by default, including loopback. In the normal default path that means token auth: if no explicit auth path is configured, gateway startup resolves to token mode and generates a runtime-only token for that startup, so **local WS clients must authenticate**. Configure `gateway.auth.token`, `gateway.auth.password`, `NEXISCLAW_GATEWAY_TOKEN`, or `NEXISCLAW_GATEWAY_PASSWORD` explicitly when clients need a stable secret across restarts. This blocks other local processes from calling the Gateway.

    If you prefer a different auth path, you can explicitly choose password mode (or, for identity-aware reverse proxies, `trusted-proxy`). If you **really** want open loopback, set `gateway.auth.mode: "none"` explicitly in your config. Doctor can generate a token for you any time: `FirstNexus doctor --generate-gateway-token`.
  ...
  <Accordion title="How do I disable funny CLI taglines?">
    ...
    - `default`: uses `All your chats, one FirstNexus.` every time.
  ...
  <Accordion title="How do I enable web search (and web fetch)?">
    ...
    **Recommended:** run `FirstNexus configure --section web` and choose a provider.
    ...
    - If `tools.web.fetch.provider` is omitted, FirstNexus auto-detects the first ready fetch fallback provider from available credentials. Today the bundled provider is Firecrawl.
    - Daemons read env vars from `~/.FirstNexus/.env` (or the service environment).
  ...
  <Accordion title="config.apply wiped my config. How do I recover and avoid this?">
    ...
    Current FirstNexus protects many accidental clobbers:

    - FirstNexus-owned config writes validate the full post-change config before writing.
    - Invalid or destructive FirstNexus-owned writes are rejected and saved as `FirstNexus.json.rejected.*`.
    - If a direct edit breaks startup or hot reload, Gateway fails closed or skips the reload; it does not rewrite `FirstNexus.json`.
    - `FirstNexus doctor --fix` owns repair and can restore last-known-good while saving the rejected file as `FirstNexus.json.clobbered.*`.

    Recover:

    - Check `FirstNexus logs --follow` for `Invalid config at`, `Config write rejected:`, or `config reload skipped (invalid config)`.
    - Inspect the newest `FirstNexus.json.clobbered.*` or `FirstNexus.json.rejected.*` beside the active config.
    - Run `FirstNexus config validate` and `FirstNexus doctor --fix`.
    - Copy only the intended keys back with `FirstNexus config set` or `config.patch`.
    - If you have no last-known-good or rejected payload, restore from backup, or re-run `FirstNexus doctor` and reconfigure channels/models.
    - If this was unexpected, file a bug and include your last known config or any backup.
    - A local coding agent can often reconstruct a working config from logs or history.

    Avoid it:

    - Use `FirstNexus config set` for small changes.
    - Use `FirstNexus configure` for interactive edits.
  ...
  <Accordion title="Can the FirstNexus browser run headless?">
...
<AccordionGroup>
  ...
  <Accordion title="Tailscale is connected but I get no replies. What now?">
    ...
    - Gateway is running: `FirstNexus gateway status`
    - Gateway health: `FirstNexus status`
    - Channel health: `FirstNexus channels status`
  ...
  <Accordion title="Can two FirstNexus instances talk to each other (local + VPS)?">
    ...
    **CLI bridge (generic):** run a script that calls the other Gateway with
    `FirstNexus agent --message ... --deliver`, targeting a chat where the other bot
    ...
    ```bash
    FirstNexus agent --message "Hello from local bot" --deliver --channel telegram --reply-to <chat-id>
    ```
  ...
  <Accordion title="How do I set up Tailscale on a VPS and connect from my Mac?">
    ...
    ```bash
    FirstNexus gateway --tailscale serve
    ```
...
<AccordionGroup>
  <Accordion title="How does FirstNexus load environment variables?">
    FirstNexus reads env vars from the parent process (shell, launchd/systemd, CI, etc.) and additionally loads:

    - `.env` from the current working directory
    - a global fallback `.env` from `~/.FirstNexus/.env` (aka `$NEXISCLAW_STATE_DIR/.env`)
  ...
  <Accordion title="I started the Gateway via the service and my env vars disappeared. What now?">
    ...
    1. Put the missing keys in `~/.FirstNexus/.env` so they're picked up even when the service doesn't inherit your shell env.
  ...
  <Accordion title='I set COPILOT_GITHUB_TOKEN, but models status shows "Shell env: off." Why?'>
    `FirstNexus models status` reports whether **shell env import** is enabled. "Shell env: off"
    does **not** mean your env vars are missing - it just means FirstNexus won't load
    your login shell automatically.

    If the Gateway runs as a service (launchd/systemd), it won't inherit your shell
    environment. Fix by doing one of these:

    1. Put the token in `~/.FirstNexus/.env`:
...
<AccordionGroup>
  ...
  <Accordion title="Is there a way to make a team of FirstNexus instances (one CEO and many agents)?">
  ...
  <Accordion title="How do I completely reset FirstNexus but keep it installed?">
    ...
    - If you used profiles (`--profile` / `NEXISCLAW_PROFILE`), reset each state dir (defaults are `~/.FirstNexus-<profile>`).
    - Dev reset: `FirstNexus gateway --dev --reset` (dev-only; wipes dev config + credentials + sessions + workspace).
  ...
  <Accordion title="Why am I getting heartbeat messages every 30 minutes?">
    ...
    headers like `# Heading`), FirstNexus skips the heartbeat run to save API calls.
  ...
  <Accordion title='Do I need to add a "bot account" to a WhatsApp group?'>
    No. FirstNexus runs on **your own account**, so if you're in the group, FirstNexus can see it.
  ...
  <Accordion title="How do I get the JID of a WhatsApp group?">
    ...
    ```bash
    FirstNexus directory groups list --channel whatsapp
    ```
  ...
  <Accordion title="Why does FirstNexus not reply in a group?">
    Two common causes:
  ...
  <Accordion title="How many workspaces and agents can I create?">
    ...
    - **Disk growth:** sessions + transcripts live under `~/.FirstNexus/agents/<agentId>/sessions/`.
    ...
    - Use `FirstNexus doctor` to spot stray workspaces and profile mismatches.
...
<AccordionGroup>
  ...
  <Accordion title='Why does FirstNexus gateway status say "Runtime: running" but "Connectivity probe: failed"?'>
    ...
    Use `FirstNexus gateway status` and trust these lines:
  ...
  <Accordion title='Why does FirstNexus gateway status show "Config (cli)" and "Config (service)" different?'>
    You're editing one config file while the service is running another (often a `--profile` / `NEXISCLAW_STATE_DIR` mismatch).

    Fix:

    ```bash
    FirstNexus gateway install --force
    ```
  ...
  <Accordion title='What does "another gateway instance is already listening" mean?'>
    FirstNexus enforces a runtime lock by binding the WebSocket listener immediately on startup (default `ws://127.0.0.1:18789`). If the bind fails with `EADDRINUSE`, it throws `GatewayLockError` indicating another instance is already listening.

    Fix: stop the other instance, free the port, or run with `FirstNexus gateway --port <port>`.

  </Accordion>

  <Accordion title="How do I run FirstNexus in remote mode (client connects to a Gateway elsewhere)?">
    ...
    - `FirstNexus gateway` only starts when `gateway.mode` is `local` (or you pass the override flag).
  ...
  <Accordion title='The Control UI says "unauthorized" (or keeps reconnecting). What now?'>
    ...
    - Fastest: `FirstNexus dashboard` (prints + copies the dashboard URL, tries to open; shows SSH hint if headless).
    - If you don't have a token yet: `FirstNexus doctor --generate-gateway-token`.
    ...
    - If mismatch persists after the one retry, rotate/re-approve the paired device token:
      - `FirstNexus devices list`
      - `FirstNexus devices rotate --device <id> --role operator`
    ...
    - Still stuck? Run `FirstNexus status --all` and follow [Troubleshooting](/gateway/troubleshooting). See [Dashboard](/web/dashboard) for auth details.
  ...
  <Accordion title="Can I run multiple Gateways on the same host?">
    ...
    - Use `FirstNexus --profile <name> ...` per instance (auto-creates `~/.FirstNexus-<name>`).
    - Set a unique `gateway.port` in each profile config (or pass `--port` for manual runs).
    - Install a per-profile service: `FirstNexus --profile <name> gateway install`.

    Profiles also suffix service names (`ai.FirstNexus.<profile>`; legacy `com.FirstNexus.*`, `FirstNexus-gateway-<profile>.service`, `FirstNexus Gateway (<profile>)`).
  ...
  <Accordion title='What does "invalid handshake" / code 1008 mean?'>
    ...
    ```
    FirstNexus tui --url ws://<host>:18789 --token <token>
    ```
...
<AccordionGroup>
  <Accordion title="Where are logs?">
    ...
    - macOS: `$NEXISCLAW_STATE_DIR/logs/gateway.log` and `gateway.err.log` (default: `~/.FirstNexus/logs/...`; profiles use `~/.FirstNexus-<profile>/logs/...`)
    - Linux: `journalctl --user -u FirstNexus-gateway[-<profile>].service -n 200 --no-pager`
    - Windows: `schtasks /Query /TN "FirstNexus Gateway (<profile>)" /V /FO LIST`
  ...
  <Accordion title="How do I start/stop/restart the Gateway service?">
    ...
    If you run the gateway manually, `FirstNexus gateway --force` can reclaim the port. See [Gateway](/gateway).

  </Accordion>

  <Accordion title="I closed my terminal on Windows - how do I restart FirstNexus?">
  ...
  <Accordion title='"Disconnected from gateway: no reason" - what now?'>
    ...
    1. Is the Gateway running? `FirstNexus gateway status`
    2. Is the Gateway healthy? `FirstNexus status`
    3. Does the UI have the right token? `FirstNexus dashboard`
  ...
  <Accordion title="Telegram setMyCommands fails. What should I check?">
    ...
    ```bash
    FirstNexus channels status
    FirstNexus channels logs --channel telegram
    ```

    Then match the error:

    - `BOT_COMMANDS_TOO_MUCH`: the Telegram menu has too many entries. FirstNexus already trims to the Telegram limit and retries with fewer commands, but some menu entries still need to be dropped. Reduce plugin/skill/custom commands, or disable `channels.telegram.commands.native` if you do not need the menu.
  ...
  <Accordion title="ELI5: FirstNexus gateway restart vs FirstNexus gateway">
    - `FirstNexus gateway restart`: restarts the **background service** (launchd/systemd).
    - `FirstNexus gateway`: runs the gateway **in the foreground** for this terminal session.

    If you installed the service, use the gateway commands. Use `FirstNexus gateway` when
...
<AccordionGroup>
  <Accordion title="My skill generated an image/PDF, but nothing was sent">
    Outbound attachments from the agent must include a `MEDIA:<path-or-url>` line (on its own line). See [FirstNexus assistant setup](/start/FirstNexus) and [Agent send](/tools/agent-send).

    CLI sending:

    ```bash
    FirstNexus message send --target +15555550123 --message "Here you go" --media /path/to/file.png
    ```
...
<AccordionGroup>
  <Accordion title="Is it safe to expose FirstNexus to inbound DMs?">
    Treat inbound DMs as untrusted input. Defaults are designed to reduce risk:

    - Default behavior on DM-capable channels is **pairing**:
      - Unknown senders receive a pairing code; the bot does not process their message.
      - Approve with: `FirstNexus pairing approve --channel <channel> [--account <id>] <code>`
      - Pending requests are capped at **3 per channel**; check `FirstNexus pairing list --channel <channel> [--account <id>]` if a code didn't arrive.
    - Opening DMs publicly requires explicit opt-in (`dmPolicy: "open"` and allowlist `"*"`).

    Run `FirstNexus doctor` to surface risky DM policies.
  ...
  <Accordion title="I ran /start in Telegram but did not get a pairing code">
    ...
    ```bash
    FirstNexus pairing list telegram
    ```
  ...
  <Accordion title="WhatsApp: will it message my contacts? How does pairing work?">
    No. Default WhatsApp DM policy is **pairing**. Unknown senders only get a pairing code and their message is **not processed**. FirstNexus only replies to chats it receives or to explicit sends you trigger.

    Approve pairing with:

    ```bash
    FirstNexus pairing approve whatsapp <code>
    ```

    List pending requests:

    ```bash
    FirstNexus pairing list whatsapp
    ```
...
<AccordionGroup>
  ...
  <Accordion title='How do I send a Discord message from Telegram? ("Cross-context messaging denied")'>
    FirstNexus blocks **cross-provider** messaging by default. If a tool call is bound
