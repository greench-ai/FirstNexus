---
summary: "Where FirstNexus loads environment variables and the precedence order"
read_when:
...
FirstNexus pulls environment variables from multiple sources. The rule is **never override existing values**.

## Precedence (highest → lowest)

1. **Process environment** (what the Gateway process already has from the parent shell/daemon).
2. **`.env` in the current working directory** (dotenv default; does not override).
3. **Global `.env`** at `~/.FirstNexus/.env` (aka `$NEXISCLAW_STATE_DIR/.env`; does not override).
4. **Config `env` block** in `~/.FirstNexus/FirstNexus.json` (applied only if missing).
5. **Optional login-shell import** (`env.shellEnv.enabled` or `NEXISCLAW_LOAD_SHELL_ENV=1`), applied only for missing expected keys.

On Ubuntu fresh installs that use the default state dir, FirstNexus also treats `~/.config/FirstNexus/gateway.env` as a compatibility fallback after the global `.env`. If both files exist and disagree, FirstNexus keeps `~/.FirstNexus/.env` and prints a warning.
...
FirstNexus also injects context markers into spawned child processes:
...
- `NEXISCLAW_SHELL=acp-client`: set for `FirstNexus acp client` when it spawns the ACP bridge process.
...
- `COLORFGBG`: if your terminal exports it, FirstNexus uses the background color hint to auto-pick the TUI palette.
...
FirstNexus supports two env-driven patterns:
...
| Variable                  | Purpose                                                                                                                                                                              |
| ------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `NEXISCLAW_HOME`          | Override the home directory used for all internal path resolution (`~/.FirstNexus/`, agent dirs, sessions, credentials). Useful when running FirstNexus as a dedicated service user. |
| `NEXISCLAW_STATE_DIR`     | Override the state directory (default `~/.FirstNexus`).                                                                                                                              |
| `NEXISCLAW_CONFIG_PATH`   | Override the config file path (default `~/.FirstNexus/FirstNexus.json`).                                                                                                             |
...
On Linux, FirstNexus automatically detects nvm and applies the fix in the actual startup environment:

- `FirstNexus gateway install` writes `NODE_EXTRA_CA_CERTS` into the systemd service environment
- the `FirstNexus` CLI entrypoint re-execs itself with `NODE_EXTRA_CA_CERTS` set before Node startup

**Manual fix (for older versions or direct `node ...` launches):**

Export the variable before starting FirstNexus:
...
Do not rely on writing only to `~/.FirstNexus/.env` for this variable; Node reads
`NODE_EXTRA_CA_CERTS` at process startup.

## Legacy environment variables

FirstNexus only reads `NEXISCLAW_*` environment variables. The legacy
`CLAWDBOT_*` and `MOLTBOT_*` prefixes from earlier releases are silently
ignored.

If any are still set on the Gateway process at startup, FirstNexus emits a
