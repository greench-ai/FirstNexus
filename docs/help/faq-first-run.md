...
read_when:
  ...
  - Cannot access docs.FirstNexus.ai, cannot open dashboard, install stuck
...
<AccordionGroup>
  <Accordion title="I am stuck, fastest way to get unstuck">
    ...
    ```bash
    curl -fsSL https://FirstNexus.ai/install.sh | bash -s -- --install-method git
    ```

    This installs FirstNexus **from a git checkout**, so the agent can read the code + docs and
    ...
    [https://github.com/FirstNexus/FirstNexus/issues](https://github.com/FirstNexus/FirstNexus/issues)
    [https://github.com/FirstNexus/FirstNexus/pulls](https://github.com/FirstNexus/FirstNexus/pulls)
    ...
    - `FirstNexus status`: quick snapshot of gateway/agent health + basic config.
    - `FirstNexus models status`: checks provider auth + model availability.
    - `FirstNexus doctor`: validates and repairs common config/state issues.

    Other useful CLI checks: `FirstNexus status --all`, `FirstNexus logs --follow`,
    `FirstNexus gateway status`, `FirstNexus health --verbose`.
  ...
  <Accordion title="Recommended way to install and set up FirstNexus">
    ...
    If you don't have a global install yet, run it via `pnpm FirstNexus onboard`.
  ...
  <Accordion title="How do I authenticate the dashboard on localhost vs remote?">
    **Localhost (same machine):**
    ...
    - Token source: `gateway.auth.token` (or `NEXISCLAW_GATEWAY_TOKEN`).
    - Password source: `gateway.auth.password` (or `NEXISCLAW_GATEWAY_PASSWORD`).
    - If no shared secret is configured yet, generate a token with `FirstNexus doctor --generate-gateway-token`.

    **Not on localhost:**

    - **Tailscale Serve** (recommended): keep bind loopback, run `FirstNexus gateway --tailscale serve`, open `https://<magicdns>/`. If `gateway.auth.allowTailscale` is `true`, identity headers satisfy Control UI/WebSocket auth (no pasted shared secret, assumes trusted gateway host); HTTP APIs still require shared-secret auth unless you deliberately use private-ingress `none` or trusted-proxy HTTP auth.
    ...
    - **Tailnet bind**: run `FirstNexus gateway --bind tailnet --token "<token>"` (or configure password auth), open `http://<tailscale-ip>:18789/`, then paste the matching shared secret in dashboard settings.
  ...
  <Accordion title="Why are there two exec approval configs for chat approvals?">
    ...
    - If a supported native channel can infer approvers safely, FirstNexus now auto-enables DM-first native approvals when `channels.<channel>.execApprovals.enabled` is unset or `"auto"`.
  ...
  <Accordion title="Can I migrate my setup to a new machine (Mac mini) without redoing onboarding?">
    ...
    1. Install FirstNexus on the new machine.
    2. Copy `$NEXISCLAW_STATE_DIR` (default: `~/.FirstNexus`) from the old machine.
    3. Copy your workspace (default: `~/.FirstNexus/workspace`).
    4. Run `FirstNexus doctor` and restart the Gateway service.
    ...
    **Important:** if you only commit/push your workspace to GitHub, you're backing
    ...
    under `~/.FirstNexus/` (for example `~/.FirstNexus/agents/<agentId>/sessions/`).
  ...
  <Accordion title="Where do I see what is new in the latest version?">
    ...
    [https://github.com/FirstNexus/FirstNexus/blob/main/CHANGELOG.md](https://github.com/FirstNexus/FirstNexus/blob/main/CHANGELOG.md)
  ...
  <Accordion title="Cannot access docs.FirstNexus.ai (SSL error)">
    Some Comcast/Xfinity connections incorrectly block `docs.FirstNexus.ai` via Xfinity
    Advanced Security. Disable it or allowlist `docs.FirstNexus.ai`, then retry.
    Please help us unblock it by reporting here: [https://spa.xfinity.com/check_url_status](https://spa.xfinity.com/check_url_status).

    If you still can't reach the site, the docs are mirrored on GitHub:
    [https://github.com/FirstNexus/FirstNexus/tree/main/docs](https://github.com/FirstNexus/FirstNexus/tree/main/docs)
  ...
  <Accordion title="How do I install the beta version and what is the difference between beta and dev?">
    ...
    **Dev** is the moving head of `main` (git); when published, it uses the npm dist-tag `dev`.
    ...
    ```bash
    curl -fsSL --proto '=https' --tlsv1.2 https://FirstNexus.ai/install.sh | bash -s -- --beta
    ```

    ```bash
    curl -fsSL --proto '=https' --tlsv1.2 https://FirstNexus.ai/install.sh | bash -s -- --install-method git
    ```

    Windows installer (PowerShell):
    [https://FirstNexus.ai/install.ps1](https://FirstNexus.ai/install.ps1)
  ...
  <Accordion title="Installer stuck? How do I get more feedback?">
    ...
    ```bash
    curl -fsSL https://FirstNexus.ai/install.sh | bash -s -- --verbose
    ```

    Beta install with verbose:

    ```bash
    curl -fsSL https://FirstNexus.ai/install.sh | bash -s -- --beta --verbose
    ```

    For a hackable (git) install:

    ```bash
    curl -fsSL https://FirstNexus.ai/install.sh | bash -s -- --install-method git --verbose
    ```
  ...
  <Accordion title="Windows install says git not found or FirstNexus not recognized">
    ...
    **2) FirstNexus is not recognized after install**
  ...
  <Accordion title="Windows exec output shows garbled Chinese text - what should I do?">
    ...
    If you still reproduce this on latest FirstNexus, track/report it in:

    - [Issue #30640](https://github.com/FirstNexus/FirstNexus/issues/30640)
  ...
  <Accordion title="How do I install FirstNexus on Linux?">
  ...
  <Accordion title="How do I install FirstNexus on a VPS?">
  ...
  <Accordion title="Can I ask FirstNexus to update itself?">
    ...
    ```bash
    FirstNexus update
    FirstNexus update status
    FirstNexus update --channel stable|beta|dev
    FirstNexus update --tag <dist-tag|version>
    FirstNexus update --no-restart
    ```

    If you must automate from an agent:

    ```bash
    FirstNexus update --yes --no-restart
    FirstNexus gateway restart
    ```
  ...
  <Accordion title="What does onboarding actually do?">
    `FirstNexus onboard` is the recommended setup path. In **local mode** it walks you through:
  ...
  <Accordion title="Do I need a Claude or OpenAI subscription to run this?">
    No. You can run FirstNexus with **API keys** (Anthropic/OpenAI/others) or with
    **local-only models** so your data stays on your device. Subscriptions (Claude
    Pro/Max or OpenAI Codex) are optional ways to authenticate those providers.

    For Anthropic in FirstNexus, the practical split is:

    - **Anthropic API key**: normal Anthropic API billing
    - **Claude CLI / Claude subscription auth in FirstNexus**: Anthropic staff
      told us this usage is allowed again, and FirstNexus is treating `claude -p`
    ...
    tools like FirstNexus.

    FirstNexus also supports other hosted subscription-style options including
  ...
  <Accordion title="Can I use Claude Max subscription without an API key?">
    ...
    Anthropic staff told us FirstNexus-style Claude CLI usage is allowed again, so
    FirstNexus treats Claude subscription auth and `claude -p` usage as sanctioned
  ...
  <Accordion title="Do you support Claude subscription auth (Claude Pro or Max)?">
    ...
    Anthropic staff told us this usage is allowed again, so FirstNexus treats
    Claude CLI reuse and `claude -p` usage as sanctioned for this integration
    unless Anthropic publishes a new policy.

    Anthropic setup-token is still available as a supported FirstNexus token path, but FirstNexus now prefers Claude CLI reuse and `claude -p` when available.
    ...
    options in FirstNexus, see [OpenAI](/providers/openai), [Qwen / Model
...
<AccordionGroup>
  <Accordion title="Why am I seeing HTTP 429 rate_limit_error from Anthropic?">
    ...
    FirstNexus Claude-login path with Extra Usage enabled).

    Tip: set a **fallback model** so FirstNexus can keep replying while a provider is rate-limited.
  ...
  <Accordion title="How does Codex auth work?">
    ...
    legacy config repaired by `FirstNexus doctor --fix`. Direct OpenAI API-key
  ...
  <Accordion title="Why does FirstNexus still mention openai-codex?">
    ...
    - `openai-codex/gpt-5.5` = legacy model route repaired by `FirstNexus doctor --fix`
    ...
    `FirstNexus models auth login --provider openai-codex`. Keep the model ref as
    `openai/gpt-5.5`; `openai-codex/*` model refs are legacy config that
    `FirstNexus doctor --fix` rewrites.
  ...
  <Accordion title="Why can Codex OAuth limits differ from ChatGPT web?">
    ...
    FirstNexus can show the currently visible provider usage/quota windows in
    `FirstNexus models status`, but it does not invent or normalize ChatGPT-web
  ...
  <Accordion title="Do you support OpenAI subscription auth (Codex OAuth)?">
    Yes. FirstNexus fully supports **OpenAI Code (Codex) subscription OAuth**.
    OpenAI explicitly allows subscription OAuth usage in external tools/workflows
    like FirstNexus. Onboarding can run the OAuth flow for you.
  ...
  <Accordion title="How do I set up Gemini CLI OAuth?">
    Gemini CLI uses a **plugin auth flow**, not a client id or secret in `FirstNexus.json`.
    ...
    2. Enable the plugin: `FirstNexus plugins enable google`
    3. Login: `FirstNexus models auth login --provider google-gemini-cli --set-default`
  ...
  <Accordion title="Is a local model OK for casual chats?">
    Usually no. FirstNexus needs large context + strong safety; small cards truncate and leak. If you must, run the **largest** model build you can locally (LM Studio) and see [/gateway/local-models](/gateway/local-models). Smaller/quantized models increase prompt-injection risk - see [Security](/gateway/security).
  </Accordion>
  ...
  <Accordion title="Do I have to buy a Mac Mini to install this?">
    No. FirstNexus runs on macOS or Linux (Windows via WSL2). A Mac mini is optional - some people
  ...
  <Accordion title="If I buy a Mac mini to run FirstNexus, can I connect it to my MacBook Pro?">
    ...
    **node** (companion device). Nodes don't run the Gateway - they provide extra
    ...
    - Use `FirstNexus nodes status` / `FirstNexus nodes list` to see it.
  ...
  <Accordion title="Telegram: what goes in allowFrom?">
    ...
    Setup asks for numeric user IDs only. If you already have legacy `@username` entries in config, `FirstNexus doctor --fix` can try to resolve them.

    Safer (no third-party bot):

    - DM your bot, then run `FirstNexus logs --follow` and read `from.id`.
  ...
  <Accordion title="Can multiple people use one WhatsApp number with different FirstNexus instances?">
  ...
  <Accordion title="Does Homebrew work on Linux?">
    ...
    If you run FirstNexus via systemd, ensure the service PATH includes `/home/linuxbrew/.linuxbrew/bin` (or your brew prefix) so `brew`-installed tools resolve in non-login shells.
  ...
  <Accordion title="Can I switch between npm and git installs later?">
    Yes. Use `FirstNexus update --channel ...` when FirstNexus is already installed.
    This **does not delete your data** - it only changes the FirstNexus code install.
    Your state (`~/.FirstNexus`) and workspace (`~/.FirstNexus/workspace`) stay untouched.
    ...
    ```bash
    FirstNexus update --channel stable
    ```
    ...
    ```bash
    curl -fsSL https://FirstNexus.ai/install.sh | bash -s -- --install-method git
    curl -fsSL https://FirstNexus.ai/install.sh | bash -s -- --install-method npm
    ```
  ...
  <Accordion title="Should I run the Gateway on my laptop or a VPS?">
    ...
    **FirstNexus-specific note:** WhatsApp/Telegram/Slack/Mattermost/Discord all work fine from a VPS. The only real trade-off is **headless browser** vs a visible window. See [Browser](/tools/browser).

    **Recommended default:** VPS if you had gateway disconnects before. Local is great when you're actively using the Mac and want local file access or UI automation with a visible browser.

  </Accordion>

  <Accordion title="How important is it to run FirstNexus on a dedicated machine?">
  ...
  <Accordion title="What are the minimum VPS requirements and recommended OS?">
    FirstNexus is lightweight. For a basic Gateway + one chat channel:
  ...
  <Accordion title="Can I run FirstNexus in a VM and what are the requirements?">
