---
summary: "How FirstNexus validates update paths, package migrations, and plugin install/update behavior"
read_when:
  - Changing FirstNexus update, doctor, package acceptance, or plugin install behavior
...
- A user can move from an older published package to the candidate package
  ...
  channel config.
- `FirstNexus doctor --fix --non-interactive` owns legacy cleanup and repair
...
- `test:docker:plugins` validates plugin install smoke, local folder installs,
  ...
  `NEXISCLAW_PLUGINS_E2E_CLAWHUB=0` to keep the ClawHub block hermetic/offline.
...
- `test:docker:plugin-update` validates that an unchanged installed plugin does
  not reinstall or lose install metadata during `FirstNexus plugins update`.
...
- `test:docker:published-upgrade-survivor` first installs a published baseline,
  configures it through a baked `FirstNexus config set` recipe, updates it to the
...
- `test:docker:update-restart-auth` installs the candidate package, starts a
  ...
  `FirstNexus update --yes --json`, and requires the candidate update command to
...
```bash
NEXISCLAW_UPGRADE_SURVIVOR_BASELINE_SPEC=FirstNexus@2026.4.23 \
NEXISCLAW_UPGRADE_SURVIVOR_SCENARIO=versioned-runtime-deps \
pnpm test:docker:published-upgrade-survivor

NEXISCLAW_UPGRADE_SURVIVOR_BASELINE_SPEC=FirstNexus@latest \
NEXISCLAW_UPGRADE_SURVIVOR_SCENARIO=bootstrap-persona \
pnpm test:docker:published-upgrade-survivor
```
...
`NEXISCLAW_UPGRADE_SURVIVOR_SCENARIOS=reported-issues` expands to all reported
...
- `source=npm`: validate `FirstNexus@beta`, `FirstNexus@latest`, or an exact
  published version.
...
`package_acceptance_package_spec=FirstNexus@YYYY.M.D` so the same upgrade matrix
...
`last-stable-4` resolves to the four latest stable npm-published FirstNexus
