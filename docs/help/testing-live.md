...
- Optional target overrides:
  - `NEXISCLAW_ANDROID_NODE_ID` or `NEXISCLAW_ANDROID_NODE_NAME`.
  - `NEXISCLAW_ANDROID_GATEWAY_URL` / `NEXISCLAW_ANDROID_GATEWAY_TOKEN` / `NEXISCLAW_ANDROID_GATEWAY_PASSWORD`.
...
- How to enable:
  - `pnpm test:live` (or `NEXISCLAW_LIVE_TEST=1` if invoking Vitest directly)
- Set `NEXISCLAW_LIVE_MODELS=modern` (or `all`, alias for modern) to actually run this suite; otherwise it skips to keep `pnpm test:live` focused on gateway smoke
- How to select models:
  - `NEXISCLAW_LIVE_MODELS=modern` to run the modern allowlist (Opus/Sonnet 4.6+, GPT-5.2 + Codex, Gemini 3, DeepSeek V4, GLM 4.7, MiniMax M2.7, Grok 4.3)
  - `NEXISCLAW_LIVE_MODELS=all` is an alias for the modern allowlist
  - or `NEXISCLAW_LIVE_MODELS="openai/gpt-5.5,openai-codex/gpt-5.5,anthropic/claude-opus-4-6,..."` (comma allowlist)
  - Modern/all sweeps default to a curated high-signal cap; set `NEXISCLAW_LIVE_MAX_MODELS=0` for an exhaustive modern sweep or a positive number for a smaller cap.
  - Exhaustive sweeps use `NEXISCLAW_LIVE_TEST_TIMEOUT_MS` for the whole direct-model test timeout. Default: 60 minutes.
  - Direct-model probes run with 20-way parallelism by default; set `NEXISCLAW_LIVE_MODEL_CONCURRENCY` to override.
- How to select providers:
  - `NEXISCLAW_LIVE_PROVIDERS="google,google-antigravity,google-gemini-cli"` (comma allowlist)
- Where keys come from:
  - By default: profile store and env fallbacks
  - Set `NEXISCLAW_LIVE_REQUIRE_PROFILE_KEYS=1` to enforce **profile store** only
- Why this exists:
...
- How to select models:
  ...
  - `NEXISCLAW_LIVE_GATEWAY_MODELS=all` is an alias for the modern allowlist
  - Or set `NEXISCLAW_LIVE_GATEWAY_MODELS="provider/model"` (or comma list) to narrow
  - Modern/all gateway sweeps default to a curated high-signal cap; set `NEXISCLAW_LIVE_GATEWAY_MAX_MODELS=0` for an exhaustive modern sweep or a positive number for a smaller cap.
- How to select providers (avoid "OpenRouter everything"):
  - `NEXISCLAW_LIVE_GATEWAY_PROVIDERS="google,google-antigravity,google-gemini-cli,openai,anthropic,zai,minimax"` (comma allowlist)
...
- Overrides (optional):
  - `NEXISCLAW_LIVE_CLI_BACKEND_MODEL="codex-cli/gpt-5.5"`
  - `NEXISCLAW_LIVE_CLI_BACKEND_COMMAND="/full/path/to/codex"`
  - `NEXISCLAW_LIVE_CLI_BACKEND_ARGS='["exec","--json","--color","never","--sandbox","read-only","--skip-git-repo-check"]'`
  - `NEXISCLAW_LIVE_CLI_BACKEND_IMAGE_PROBE=1` to send a real image attachment (paths are injected into the prompt). Docker recipes default this off unless explicitly requested.
  - `NEXISCLAW_LIVE_CLI_BACKEND_IMAGE_ARG="--image"` to pass image file paths as CLI args instead of prompt injection.
  - `NEXISCLAW_LIVE_CLI_BACKEND_IMAGE_MODE="repeat"` (or `"list"`) to control how image args are passed when `IMAGE_ARG` is set.
  - `NEXISCLAW_LIVE_CLI_BACKEND_RESUME_PROBE=1` to send a second turn and validate resume flow.
  - `NEXISCLAW_LIVE_CLI_BACKEND_MODEL_SWITCH_PROBE=1` to opt into the Claude Sonnet -> Opus same-session continuity probe when the selected model supports a switch target. Docker recipes default this off for aggregate reliability.
  - `NEXISCLAW_LIVE_CLI_BACKEND_MCP_PROBE=1` to opt into the MCP/tool loopback probe. Docker recipes default this off unless explicitly requested.
...
- Enable:
  - `NEXISCLAW_LIVE_TEST=1 NEXISCLAW_LIVE_APNS_REACHABILITY=1 pnpm test:live src/infra/push-apns-http2.live.test.ts`
- Optional timeout:
  - `NEXISCLAW_LIVE_APNS_TIMEOUT_MS=30000`
...
- Defaults:
  ...
  - ACP backend: `acpx`
- Overrides:
  - `NEXISCLAW_LIVE_ACP_BIND_AGENT=claude`
  - `NEXISCLAW_LIVE_ACP_BIND_AGENT=codex`
  - `NEXISCLAW_LIVE_ACP_BIND_AGENT=droid`
  - `NEXISCLAW_LIVE_ACP_BIND_AGENT=gemini`
  - `NEXISCLAW_LIVE_ACP_BIND_AGENT=opencode`
  - `NEXISCLAW_LIVE_ACP_BIND_AGENTS=claude,codex,gemini`
  - `NEXISCLAW_LIVE_ACP_BIND_AGENT_COMMAND='npx -y @agentclientprotocol/claude-agent-acp@<version>'`
  - `NEXISCLAW_LIVE_ACP_BIND_CODEX_MODEL=gpt-5.5`
  - `NEXISCLAW_LIVE_ACP_BIND_OPENCODE_MODEL=opencode/kimi-k2.6`
  - `NEXISCLAW_LIVE_ACP_BIND_REQUIRE_TRANSCRIPT=1`
  - `NEXISCLAW_LIVE_ACP_BIND_REQUIRE_CRON=1`
  - `NEXISCLAW_LIVE_ACP_BIND_PARENT_MODEL=openai/gpt-5.5`
- Notes:
  - This lane uses the gateway `chat.send` surface with admin-only synthetic originating-route fields so tests can attach message-channel context without pretending to deliver externally.
  - When `NEXISCLAW_LIVE_ACP_BIND_AGENT_COMMAND` is unset, the test uses the embedded `acpx` plugin's built-in agent registry for the selected ACP harness agent.
  - Bound-session cron MCP creation is best-effort by default because external ACP harnesses can cancel MCP calls after the bind/image proof has passed; set `NEXISCLAW_LIVE_ACP_BIND_REQUIRE_CRON=1` to make that post-bind cron probe strict.
...
- Use `NEXISCLAW_LIVE_ACP_BIND_AGENTS=claude`, `NEXISCLAW_LIVE_ACP_BIND_AGENTS=codex`, `NEXISCLAW_LIVE_ACP_BIND_AGENTS=droid`, `NEXISCLAW_LIVE_ACP_BIND_AGENTS=gemini`, or `NEXISCLAW_LIVE_ACP_BIND_AGENTS=opencode` to narrow the matrix.
...
- Enable: `NEXISCLAW_LIVE_CODEX_HARNESS=1`
- Default model: `openai/gpt-5.5`
- Optional image probe: `NEXISCLAW_LIVE_CODEX_HARNESS_IMAGE_PROBE=1`
- Optional MCP/tool probe: `NEXISCLAW_LIVE_CODEX_HARNESS_MCP_PROBE=1`
- Optional Guardian probe: `NEXISCLAW_LIVE_CODEX_HARNESS_GUARDIAN_PROBE=1`
...
```bash
source ~/.profile
NEXISCLAW_LIVE_CODEX_HARNESS=1 \
  NEXISCLAW_LIVE_CODEX_HARNESS_IMAGE_PROBE=1 \
  NEXISCLAW_LIVE_CODEX_HARNESS_MCP_PROBE=1 \
  NEXISCLAW_LIVE_CODEX_HARNESS_GUARDIAN_PROBE=1 \
  NEXISCLAW_LIVE_CODEX_HARNESS_MODEL=openai/gpt-5.5 \
...
- Docker enables the image, MCP/tool, and Guardian probes by default. Set
  `NEXISCLAW_LIVE_CODEX_HARNESS_IMAGE_PROBE=0` or
  `NEXISCLAW_LIVE_CODEX_HARNESS_MCP_PROBE=0` or
  `NEXISCLAW_LIVE_CODEX_HARNESS_GUARDIAN_PROBE=0` when you need a narrower debug
  run.
...
- Single model, direct (no gateway):
  - `NEXISCLAW_LIVE_MODELS="openai/gpt-5.5" pnpm test:live src/agents/models.profiles.live.test.ts`

- Single model, gateway smoke:
  - `NEXISCLAW_LIVE_GATEWAY_MODELS="openai/gpt-5.5" pnpm test:live src/gateway/gateway-models.profiles.live.test.ts`

- Tool calling across several providers:
  - `NEXISCLAW_LIVE_GATEWAY_MODELS="openai/gpt-5.5,openai-codex/gpt-5.5,anthropic/claude-opus-4-6,google/gemini-3-flash-preview,deepseek/deepseek-v4-flash,zai/glm-5.1,minimax/MiniMax-M2.7" pnpm test:live src/gateway/gateway-models.profiles.live.test.ts`

- Google focus (Gemini API key + Antigravity):
  - Gemini (API key): `NEXISCLAW_LIVE_GATEWAY_MODELS="google/gemini-3-flash-preview" pnpm test:live src/gateway/gateway-models.profiles.live.test.ts`
  - Antigravity (OAuth): `NEXISCLAW_LIVE_GATEWAY_MODELS="google-antigravity/claude-opus-4-6-thinking,google-antigravity/gemini-3-pro-high" pnpm test:live src/gateway/gateway-models.profiles.live.test.ts`
...
`NEXISCLAW_LIVE_GATEWAY_MODELS="openai/gpt-5.5,openai-codex/gpt-5.5,anthropic/claude-opus-4-6,google/gemini-3.1-pro-preview,google/gemini-3-flash-preview,google-antigravity/claude-opus-4-6-thinking,google-antigravity/gemini-3-flash,deepseek/deepseek-v4-flash,zai/glm-5.1,minimax/MiniMax-M2.7" pnpm test:live src/gateway/gateway-models.profiles.live.test.ts`
...
Include at least one image-capable model in `NEXISCLAW_LIVE_GATEWAY_MODELS` (Claude/Gemini/OpenAI vision-capable variants, etc.) to exercise the image probe.
...
- Enable: `NEXISCLAW_LIVE_TEST=1 COMFY_LIVE_TEST=1 pnpm test:live -- extensions/comfy/comfy.live.test.ts`
- Scope:
...
- Current bundled providers covered:
  ...
  - `xai`
- Optional narrowing:
  - `NEXISCLAW_LIVE_IMAGE_GENERATION_PROVIDERS="openai,google,openrouter,xai"`
  - `NEXISCLAW_LIVE_IMAGE_GENERATION_PROVIDERS="deepinfra"`
  - `NEXISCLAW_LIVE_IMAGE_GENERATION_MODELS="openai/gpt-image-2,google/gemini-3.1-flash-image-preview,openrouter/google/gemini-3.1-flash-image-preview,xai/grok-imagine-image"`
  - `NEXISCLAW_LIVE_IMAGE_GENERATION_CASES="google:flash-generate,google:pro-edit,openrouter:generate,xai:default-generate,xai:default-edit"`
- Optional auth behavior:
  - `NEXISCLAW_LIVE_REQUIRE_PROFILE_KEYS=1` to force profile-store auth and ignore env-only overrides
...
- Enable: `NEXISCLAW_LIVE_TEST=1 pnpm test:live -- extensions/music-generation-providers.live.test.ts`
...
- Optional narrowing:
  - `NEXISCLAW_LIVE_MUSIC_GENERATION_PROVIDERS="google,minimax"`
  - `NEXISCLAW_LIVE_MUSIC_GENERATION_MODELS="google/lyria-3-clip-preview,minimax/music-2.6"`
- Optional auth behavior:
  - `NEXISCLAW_LIVE_REQUIRE_PROFILE_KEYS=1` to force profile-store auth and ignore env-only overrides
...
- Enable: `NEXISCLAW_LIVE_TEST=1 pnpm test:live -- extensions/video-generation-providers.live.test.ts`
- Harness: `pnpm test:live:media video`
- Scope:
  - Exercises the shared bundled video-generation provider path
  - Defaults to the release-safe smoke path: non-FAL providers, one text-to-video request per provider, one-second lobster prompt, and a per-provider operation cap from `NEXISCLAW_LIVE_VIDEO_GENERATION_TIMEOUT_MS` (`180000` by default)
  - Skips FAL by default because provider-side queue latency can dominate release time; pass `--video-providers fal` or `NEXISCLAW_LIVE_VIDEO_GENERATION_PROVIDERS="fal"` to run it explicitly
  - Loads provider env vars from your login shell (`~/.profile`) before probing
  - Uses live/env API keys ahead of stored auth profiles by default, so stale test keys in `auth-profiles.json` do not mask real shell credentials
  - Skips providers with no usable auth/profile/model
  - Runs only `generate` by default
  - Set `NEXISCLAW_LIVE_VIDEO_GENERATION_FULL_MODES=1` to also run declared transform modes when available:
  ...
  - Provider-specific Vydra coverage:
    - `NEXISCLAW_LIVE_TEST=1 NEXISCLAW_LIVE_VYDRA_VIDEO=1 pnpm test:live -- extensions/vydra/vydra.live.test.ts`
...
- Optional narrowing:
  - `NEXISCLAW_LIVE_VIDEO_GENERATION_PROVIDERS="deepinfra,google,openai,runway"`
  - `NEXISCLAW_LIVE_VIDEO_GENERATION_MODELS="google/veo-3.1-fast-generate-preview,openai/sora-2,runway/gen4_aleph"`
  - `NEXISCLAW_LIVE_VIDEO_GENERATION_SKIP_PROVIDERS=""` to include every provider in the default sweep, including FAL
  - `NEXISCLAW_LIVE_VIDEO_GENERATION_TIMEOUT_MS=60000` to reduce each provider operation cap for an aggressive smoke run
- Optional auth behavior:
  - `NEXISCLAW_LIVE_REQUIRE_PROFILE_KEYS=1` to force profile-store auth and ignore env-only overrides
