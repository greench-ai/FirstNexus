...
CLI onboarding is the **recommended** way to set up FirstNexus on macOS,
...
`FirstNexus dashboard` and chat in the browser. Docs: [Dashboard](/web/dashboard).
</Info>
...
`FirstNexus configure --section web`. Docs: [Web tools](/tools/web).
</Tip>
...
2. **Workspace** — Location for agent files (default `~/.FirstNexus/workspace`). Seeds bootstrap files.
...
If the config is invalid or contains legacy keys, onboarding asks you to run `FirstNexus doctor` first.
</Note>
...
- CLI command reference: [`FirstNexus onboard`](/cli/onboard)
