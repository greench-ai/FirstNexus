---
summary: "Get FirstNexus installed and run your first chat in minutes."
read_when:
...
Install FirstNexus, run onboarding, and chat with your AI assistant — all in
...
```bash
mkdir -p "$HOME/.FirstNexus/control-ui-custom"
...
```json
{
  "gateway": {
    "controlUi": {
      "enabled": true,
      "root": "$HOME/.FirstNexus/control-ui-custom"
    }
  }
}
```
...
<Accordion title="Advanced: environment variables">
  If you run FirstNexus as a service account or want custom paths:
