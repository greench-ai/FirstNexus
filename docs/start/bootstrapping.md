...
On the first agent run, FirstNexus bootstraps the workspace (default
...
For embedded/local model runs, FirstNexus keeps `BOOTSTRAP.md` out of the
...
To skip this for a pre-seeded workspace, run `FirstNexus onboard --skip-bootstrap`.
...
host (for example, `user@gateway-host:~/.FirstNexus/workspace`).
</Note>
