---
summary: "Use OpenShell as a managed sandbox backend for FirstNexus agents"
title: OpenShell
read_when:
...
OpenShell is a managed sandbox backend for FirstNexus. Instead of running Docker
containers locally, FirstNexus delegates sandbox lifecycle to the `openshell` CLI,
...
- FirstNexus Gateway running on the host
...
2. Restart the Gateway. On the next agent turn, FirstNexus creates an OpenShell
...
- You edit files locally outside FirstNexus and want those changes visible in the
...
- When the sandbox is first created, FirstNexus seeds the remote workspace from
...
- FirstNexus does **not** sync remote changes back into the local workspace.
...
<Warning>
If you edit files on the host outside FirstNexus after the initial seed, the remote sandbox does **not** see those changes. Use `FirstNexus sandbox recreate` to re-seed.
</Warning>
...
| `from`                    | `string`                 | `"FirstNexus"` | Sandbox source for first-time create                  |
...
1. FirstNexus calls `openshell sandbox create` (with `--from`, `--gateway`,
   `--policy`, `--providers`, `--gpu` flags as configured).
2. FirstNexus calls `openshell sandbox ssh-config <name>` to get SSH connection
...
- [Sandbox CLI](/cli/sandbox) -- `FirstNexus sandbox` commands
