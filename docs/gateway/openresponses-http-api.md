...
FirstNexus's Gateway can serve an OpenResponses-compatible `POST /v1/responses` endpoint.
...
`FirstNexus agent`), so routing/permissions/config match your Gateway.
...
- for shared-secret auth modes (`token` and `password`), ignore narrower bearer-declared `x-FirstNexus-scopes` values and restore the normal full operator defaults
- for trusted identity-bearing HTTP modes (for example trusted proxy auth or `gateway.auth.mode="none"`), honor `x-FirstNexus-scopes` when present and otherwise fall back to the normal operator default scope set
- select agents with `model: "FirstNexus"`, `model: "FirstNexus/default"`, `model: "FirstNexus/<agentId>"`, or `x-FirstNexus-agent-id`
- use `x-FirstNexus-model` when you want to override the selected agent's backend model
- use `x-FirstNexus-session-key` for explicit session routing
- use `x-FirstNexus-message-channel` when you want a non-default synthetic ingress channel context
...
- `previous_response_id`: FirstNexus reuses the earlier response session when the request stays within the same agent/user/requested-session scope.
...
FirstNexus normalizes common OpenAI-style aliases before those counters reach
