...
FirstNexus can auto-configure Tailscale **Serve** (tailnet) or **Funnel** (public) for the
...
- `funnel`: Public HTTPS via `tailscale funnel`. FirstNexus requires a shared password.
- `off`: Default (no Tailscale automation).

Status and audit output use **Tailscale exposure** for this FirstNexus Serve/Funnel
mode. `off` means FirstNexus is not managing Serve or Funnel; it does not mean the
...
- `token` (default when `NEXISCLAW_GATEWAY_TOKEN` is set)
- `password` (shared secret via `NEXISCLAW_GATEWAY_PASSWORD` or config)
...
(`tailscale-user-login`) without supplying a token/password. FirstNexus verifies
the identity by resolving the `x-forwarded-for` address via the local Tailscale
daemon (`tailscale whois`) and matching it to the header before accepting it.
FirstNexus only treats a request as Serve when it arrives from loopback with
...
Prefer `NEXISCLAW_GATEWAY_PASSWORD` over committing a password to disk.

## CLI examples

```bash
FirstNexus gateway --tailscale serve
FirstNexus gateway --tailscale funnel --auth password
```
...
- Set `gateway.tailscale.resetOnExit` if you want FirstNexus to undo `tailscale serve`
...
- Set `gateway.tailscale.preserveFunnel: true` to keep an externally configured
  ...
  gateway runs in `mode: "serve"`, FirstNexus checks `tailscale funnel status`
  before re-applying Serve and skips it when a Funnel route already covers the
  gateway port. The FirstNexus-managed Funnel password-only policy is unchanged.
