---
summary: "SSH tunnel setup for FirstNexus.app connecting to a remote gateway"
...
# Running FirstNexus.app with a Remote Gateway

FirstNexus.app uses SSH tunneling to connect to a remote gateway. This guide shows you how to set it up.
...
### Step 5: Restart FirstNexus.app

```bash
# Quit FirstNexus.app (⌘Q), then reopen:
open /path/to/FirstNexus.app
```
...
Legacy note: remove any leftover `com.FirstNexus.ssh-tunnel` LaunchAgent if present.
...
FirstNexus.app connects to `ws://127.0.0.1:18789` on your client machine. The SSH tunnel forwards that connection to port 18789 on the remote machine where the Gateway is running.
