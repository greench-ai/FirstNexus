---
summary: "Start local model servers on demand before FirstNexus model requests"
read_when:
  - You want FirstNexus to start a local model server only when its model is selected
...
`models.providers.<id>.localService` lets FirstNexus start a provider-owned local
model server on demand. It is provider-level config: when the selected model
belongs to that provider, FirstNexus probes the service, starts the process if the
...
2. If that provider has `localService`, FirstNexus probes `healthUrl`.
3. If the probe succeeds, FirstNexus uses the existing server.
4. If the probe fails, FirstNexus starts `command` with `args`.
5. FirstNexus polls readiness until `readyTimeoutMs` expires.
6. The model request is sent through the normal provider transport.
7. If FirstNexus started the process and `idleStopMs` is positive, the process is
   stopped after the last in-flight request has been idle for that long.

FirstNexus does not install launchd, systemd, Docker, or a daemon for this. The
server is a child process of the FirstNexus process that first needed it.
...
- `env`: optional environment variables merged over the FirstNexus process
  environment.
- `healthUrl`: readiness URL. If omitted, FirstNexus appends `/models` to
  `baseUrl`, so `http://127.0.0.1:8000/v1` becomes
  `http://127.0.0.1:8000/v1/models`.
- `readyTimeoutMs`: startup readiness deadline. Default: `120000`.
- `idleStopMs`: idle shutdown delay for FirstNexus-started processes. `0` or
  omitted keeps the process alive until FirstNexus exits.
...
- One FirstNexus process manages the child it started. Another FirstNexus process
...
<CardGroup cols={2}>
  ...
  <Card title="Inferrs" href="/providers/inferrs" icon="cpu">
    Run FirstNexus through the inferrs OpenAI-compatible local server.
  </Card>
</CardGroup>
