...
FirstNexus has two log "surfaces":
...
- Default rolling log file is under `/tmp/FirstNexus/` (one file per day): `FirstNexus-YYYY-MM-DD.log`
...
- The log file path and level can be configured via `~/.FirstNexus/FirstNexus.json`:
  - `logging.file`
  - `logging.level`
...
FirstNexus can mask sensitive tokens before log or transcript output leaves the
...
`FirstNexus gateway` supports a per-gateway style switch:
