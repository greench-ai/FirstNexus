...
- `FirstNexus gateway status` shows `Runtime: running`, `Connectivity probe: ok`, and a `Capability: ...` line.
- `FirstNexus doctor` reports no blocking config/service issues.
- `FirstNexus channels status --probe` shows live per-account transport status and, where supported, probe/audit results such as `works` or `audit ok`.

## Split brain installs and newer config guard

Use this when a gateway service unexpectedly stops after an update, or logs show that one `FirstNexus` binary is older than the version that last wrote `FirstNexus.json`.

FirstNexus stamps config writes with `meta.lastTouchedVersion`. Read-only commands can still inspect a config written by a newer FirstNexus, but process and service mutations refuse to continue from an older binary. Blocked actions include gateway service start, stop, restart, uninstall, forced service reinstall, service-mode gateway startup, and `gateway --force` port cleanup.

```bash
which FirstNexus
FirstNexus --version
FirstNexus gateway status --deep
FirstNexus config get meta.lastTouchedVersion
```

<Steps>
  <Step title="Fix PATH">
    Fix `PATH` so `FirstNexus` resolves to the newer install, then rerun the action.
  </Step>
  ...
  <Step title="Remove stale wrappers">
    Remove stale system package or old wrapper entries that still point at an old `FirstNexus` binary.
  </Step>
</Steps>
...
FirstNexus treats every skill root as a containment boundary. A symlink under
`~/.agents/skills`, `<workspace>/.agents/skills`, `<workspace>/skills`, or
`~/.FirstNexus/skills` is skipped when its real target resolves outside that root
...
FirstNexus config get skills.load
```
...
FirstNexus config get agents.defaults.models
```
...
- FirstNexus model runs fail only on normal agent turns
...
FirstNexus infer model run --model <provider/model> --prompt "hi" --json
FirstNexus logs --follow
```

Look for:

- direct tiny calls succeed, but FirstNexus runs fail only on larger prompts
...
<AccordionGroup>
  <Accordion title="Common signatures">
    ...
    - `incomplete turn detected ... stopReason=stop payloads=0` → the backend completed the Chat Completions request but returned no user-visible assistant text for that turn. FirstNexus retries replay-safe empty OpenAI-compatible turns once; persistent failures usually mean the backend is emitting empty/non-text content or suppressing final-answer text.
    - direct tiny requests succeed, but FirstNexus agent runs fail with backend/model crashes (for example Gemma on some `inferrs` builds) → FirstNexus transport is likely already correct; the backend is failing on the larger agent-runtime prompt shape.
  ...
  <Accordion title="Fix options">
    ...
    3. Set `compat.supportsTools: false` for models/backends that cannot handle FirstNexus's tool schema surface reliably.
    ...
    5. If tiny direct requests keep passing while FirstNexus agent turns still crash inside the backend, treat it as an upstream server/model limitation and file a repro there with the accepted payload shape.
  </Accordion>
</AccordionGroup>
...
FirstNexus status
FirstNexus channels status --probe
FirstNexus pairing list --channel <channel> [--account <id>]
FirstNexus config get channels
FirstNexus logs --follow
```
...
| ---------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `AUTH_TOKEN_MISSING`         | Client did not send a required shared token.                                                                                                                                                 | Paste/set token in the client and retry. For dashboard paths: `FirstNexus config get gateway.auth.token` then paste into Control UI settings.                                                                                                                                            |
...
If `FirstNexus devices rotate` / `revoke` / `remove` is denied unexpectedly:

- paired-device token sessions can manage only **their own** device unless the caller also has `operator.admin`
- `FirstNexus devices rotate --scope ...` can only request operator scopes that the caller session already holds
...
FirstNexus logs --follow
FirstNexus doctor
FirstNexus gateway status --deep   # also scan system-level services
```
...
<AccordionGroup>
  <Accordion title="Common signatures">
    - `Gateway start blocked: set gateway.mode=local` or `existing config is missing gateway.mode` → local gateway mode is not enabled, or the config file was clobbered and lost `gateway.mode`. Fix: set `gateway.mode="local"` in your config, or re-run `FirstNexus onboard --mode local` / `FirstNexus setup` to restamp the expected local-mode config. If you are running FirstNexus via Podman, the default config path is `~/.FirstNexus/FirstNexus.json`.
    ...
    - `System-level FirstNexus gateway service detected` from doctor → a systemd system unit exists while the user-level service is missing. Remove or disable the duplicate before allowing doctor to install a user service, or set `NEXISCLAW_SERVICE_REPAIR_POLICY=external` if the system unit is the intended supervisor.
    - `Gateway service port does not match current gateway config` → the installed supervisor still pins the old `--port`. Run `FirstNexus doctor --fix` or `FirstNexus gateway install --force`, then restart the gateway service.
...
- A timestamped `FirstNexus.json.rejected.*` file beside the active config
- A timestamped `FirstNexus.json.clobbered.*` file if `doctor --fix` repaired a broken direct edit

<AccordionGroup>
  <Accordion title="What happened">
    - The config did not validate during startup, hot reload, or an FirstNexus-owned write.
    - Gateway startup fails closed instead of rewriting `FirstNexus.json`.
    - Hot reload skips invalid external edits and keeps the current runtime config active.
    - FirstNexus-owned writes reject invalid/destructive payloads before commit and save `.rejected.*`.
    - `FirstNexus doctor --fix` owns repair. It can remove non-JSON prefixes or restore the last-known-good copy while preserving the rejected payload as `.clobbered.*`.
  ...
  <Accordion title="Common signatures">
    ...
    - `.rejected.*` exists → an FirstNexus-owned config write failed schema or clobber checks before commit.
    ...
    - `missing-meta-vs-last-good`, `gateway-mode-missing-vs-last-good`, or `size-drop-vs-last-good:*` → an FirstNexus-owned write was rejected because it lost fields or size compared with the last-known-good backup.
    - `Config last-known-good promotion skipped` → the candidate contained redacted secret placeholders such as `***`.

  </Accordion>
  <Accordion title="Fix options">
    1. Run `FirstNexus doctor --fix` to let doctor repair prefixed/clobbered config or restore last-known-good.
    2. Copy only the intended keys from `.clobbered.*` or `.rejected.*`, then apply them with `FirstNexus config set` or `config.patch`.
    3. Run `FirstNexus config validate` before restarting.
...
Use this when `FirstNexus gateway probe` reaches something, but still prints a warning block.
...
```bash
FirstNexus cron status
FirstNexus cron list
FirstNexus cron runs --id <jobId> --limit 20
FirstNexus system heartbeat last
FirstNexus logs --follow
```
...
<AccordionGroup>
  <Accordion title="Common signatures">
    ...
    - `heartbeat skipped` with `reason=empty-heartbeat-file` → `HEARTBEAT.md` exists but only contains blank lines / markdown headers, so FirstNexus skips the model call.
...
FirstNexus approvals get --node <idOrNameOrIp>
FirstNexus logs --follow
FirstNexus status
```
...
FirstNexus browser start --browser-profile FirstNexus
...
<AccordionGroup>
  ...
  <Accordion title="Element / screenshot / upload signatures">
    ...
    - stale viewport / dark-mode / locale / offline overrides on attach-only or remote CDP profiles → run `FirstNexus browser stop --browser-profile <name>` to close the active control session and release Playwright/CDP emulation state without restarting the whole gateway.
...
- [Browser (FirstNexus-managed)](/tools/browser)
...
<AccordionGroup>
  ...
  <Accordion title="2. Bind and auth guardrails are stricter">
    ...
    FirstNexus config get gateway.auth.mode
    FirstNexus config get gateway.auth.token
    FirstNexus gateway status
    FirstNexus logs --follow
    ```
