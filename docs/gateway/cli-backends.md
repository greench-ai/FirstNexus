...
FirstNexus can run **local AI CLIs** as a **text-only fallback** when API providers are down,
rate-limited, or temporarily misbehaving. This is intentionally conservative:

- **FirstNexus tools are not injected directly**, but backends with `bundleMcp: true`
...
```bash
FirstNexus agent --message "hi" --model codex-cli/gpt-5.5
```
...
gateway host, FirstNexus now auto-loads the owning bundled plugin when your config
...
- If the primary provider fails (auth, rate limits, timeouts), FirstNexus will
    ...
    defaults: {
      cliBackends: {
        ...
        "my-cli": {
          ...
          // from bounded raw FirstNexus transcript history before compaction.
...
2. **Builds a system prompt** using the same FirstNexus prompt + workspace context.
3. **Executes the CLI** with a session id (if supported) so history stays consistent.
   The bundled `claude-cli` backend keeps a Claude stdio process alive per
   FirstNexus session and sends follow-up turns over stream-json stdin.
4. **Parses output** (JSON or plain text) and returns the final text.
5. **Persists session ids** per backend, so follow-ups reuse the same CLI session.

<Note>
The bundled Anthropic `claude-cli` backend is supported again. Anthropic staff
told us FirstNexus-style Claude CLI usage is allowed again, so FirstNexus treats
`claude -p` usage as sanctioned for this integration unless Anthropic publishes
a new policy.
</Note>

The bundled OpenAI `codex-cli` backend passes FirstNexus's system prompt through
Codex's `model_instructions_file` config override (`-c
model_instructions_file="..."`). Codex does not expose a Claude-style
`--append-system-prompt` flag, so FirstNexus writes the assembled prompt to a
temporary file for each fresh Codex CLI session.

The bundled Anthropic `claude-cli` backend receives the FirstNexus skills snapshot
two ways: the compact FirstNexus skills catalog in the appended system prompt, and
...
resolver sees the same filtered set that FirstNexus would otherwise advertise in
the prompt. Skill env/API key overrides are still applied by FirstNexus to the
child process environment for the run.

Claude CLI also has its own noninteractive permission mode. FirstNexus maps that
...
`tools.exec.ask: "off"`), FirstNexus adds `--permission-mode bypassPermissions`.
...
The bundled Anthropic `claude-cli` backend also maps FirstNexus `/think` levels
...
Before FirstNexus can use the bundled `claude-cli` backend, Claude Code itself
must already be logged in on the same host:

```bash
claude auth login
claude auth status --text
FirstNexus models auth login --provider anthropic --method cli --set-default
```
...
- `claude-cli` defaults to `liveSession: "claude-stdio"`, `output: "jsonl"`,
  ...
  exits, FirstNexus resumes from the stored Claude session id. Stored session
...
- Claude live sessions keep bounded JSONL output guards. Defaults allow up to
  ...
  and `maxTurnLines`; FirstNexus clamps those settings to 64 MiB and 100,000
  lines.
- Stored CLI sessions are provider-owned continuity. The implicit daily session
  ...
  do.
- Fresh CLI sessions normally reseed only from FirstNexus's compaction summary
  plus post-compaction tail. To recover short sessions that are invalidated
  before compaction, a backend can opt in with
  `reseedFromRawTranscriptWhenUncompacted: true`. FirstNexus still keeps raw
...
- FirstNexus drops stored CLI session reuse when the selected auth identity changes,
  ...
  stable OAuth account id, FirstNexus lets that CLI enforce resume permissions.

## Fallback prelude from claude-cli sessions

When a `claude-cli` attempt fails over to a non-CLI candidate in
[`agents.defaults.model.fallbacks`](/concepts/model-failover), FirstNexus seeds
...
provider would start cold because FirstNexus's own session transcript is empty
for `claude-cli` runs.
...
FirstNexus will write base64 images to temp files. If `imageArg` is set, those
paths are passed as CLI args. If `imageArg` is missing, FirstNexus appends the
...
- For Gemini CLI JSON output, FirstNexus reads reply text from `response` and
...
- `stats.cached` is normalized into FirstNexus `cacheRead`.
- If `stats.input` is missing, FirstNexus derives input tokens from
...
rewrites streamed assistant deltas and parsed final text before FirstNexus handles
...
CLI backends do **not** receive FirstNexus tool calls directly, but a backend can
...
- `codex-cli`: inline config overrides for `mcp_servers`; the generated
  FirstNexus loopback server is marked with Codex's per-server tool approval mode
  so MCP calls cannot stall on local approval prompts
- `google-gemini-cli`: generated Gemini system settings file

When bundle MCP is enabled, FirstNexus:
...
If no MCP servers are enabled, FirstNexus still injects a strict config when a
...
- **No direct FirstNexus tool calls.** FirstNexus does not inject tool calls into
...
- **Codex CLI sessions** resume via text output (no JSONL), which is less
  structured than the initial `--json` run. FirstNexus sessions still work
  normally.
