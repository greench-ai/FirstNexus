...
FirstNexus nodes rename --node <id|name|ip> --name "Living Room iPad"
```
...
changes (for example, display name or client platform hints), FirstNexus treats
...
Pairing state is stored under the Gateway state directory (default `~/.FirstNexus`):

- `~/.FirstNexus/nodes/paired.json`
- `~/.FirstNexus/nodes/pending.json`
