---
summary: "Schema-accurate configuration examples for common FirstNexus setups"
read_when:
  - Learning how to configure FirstNexus
...
Save to `~/.FirstNexus/FirstNexus.json` and you can DM the bot from that number.
...
```json5
{
  ...
  session: {
    ...
    store: "~/.FirstNexus/agents/default/sessions/sessions.json",
    maintenance: {
      mode: "warn",
      pruneAfter: "30d",
      maxEntries: 500,
  ...
  cron: {
    enabled: true,
    store: "~/.FirstNexus/cron/cron.json",
  ...
  gateway: {
    ...
    port: 18789,
    bind: "loopback",
    controlUi: { enabled: true, basePath: "/FirstNexus" },
    auth: {
      mode: "token",
      token: "gateway-token",
      allowTailscale: true,
...
```json5
{
  agents: {
    ...
    list: [
      ...
      { id: "docs", workspace: "~/.FirstNexus/workspace-docs", skills: ["docs-search"] },
    ],
  },
}
```
