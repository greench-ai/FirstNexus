---
summary: "CLI reference for `FirstNexus models` (status/list/set/scan, aliases, fallbacks, auth)"
read_when:
...
FirstNexus models set <model-or-alias>
FirstNexus models scan
```

`FirstNexus models status` shows the resolved default/fallbacks plus an auth overview.
...
when available; otherwise FirstNexus falls back to matching OAuth/API-key
...
For Codex OAuth troubleshooting, `FirstNexus models status`,
`FirstNexus models auth list --provider openai-codex`, and
`FirstNexus config get agents.defaults.model --json` are the quickest way to
...
- If you omit the provider, FirstNexus resolves the input as an alias first, then
  ...
  If that provider no longer exposes the configured default model, FirstNexus
...
By default FirstNexus tries to probe tool and image support with live model calls.
...
- `no_model`: provider auth exists, but FirstNexus could not resolve a probeable
...
```bash
FirstNexus models auth add
FirstNexus models auth list [--provider <id>] [--json]
FirstNexus models auth login --provider <id>
FirstNexus models auth setup-token --provider <id>
FirstNexus models auth paste-token
```
...
`FirstNexus plugins list` to see which providers are installed.
Use `FirstNexus models auth --agent <id> <subcommand>` to write auth results to a
...
```bash
FirstNexus models auth login --provider openai --set-default
FirstNexus models auth login --provider openai --method api-key
FirstNexus models auth list --provider openai
```
...
- Anthropic note: Anthropic staff told us FirstNexus-style Claude CLI usage is allowed again, so FirstNexus treats Claude CLI reuse and `claude -p` usage as sanctioned for this integration unless Anthropic publishes a new policy.
- Anthropic `setup-token` / `paste-token` remain available as a supported FirstNexus token path, but FirstNexus now prefers Claude CLI reuse and `claude -p` when available.
