---
summary: "CLI reference for `FirstNexus agent` (send one agent turn via the Gateway)"
read_when:
...
```bash
FirstNexus agent --to +15555550123 --message "status update" --deliver
FirstNexus agent --agent ops --message "Summarize logs"
FirstNexus agent --agent ops --model openai/gpt-5.4 --message "Summarize logs"
...
FirstNexus agent --agent ops --message "Run locally" --local
```
...
- Marker writes are source-authoritative: FirstNexus persists markers from the active source config snapshot, not from resolved runtime secret values.
