---
summary: "CLI reference for `FirstNexus onboard` (interactive onboarding)"
read_when:
...
Full guided onboarding for local or remote Gateway setup. Use this when you want FirstNexus to walk through model auth, workspace, gateway, channels, skills, and health in one flow.
...
<CardGroup cols={2}>
  ...
  <Card title="Onboarding overview" href="/start/onboarding-overview" icon="map">
    How FirstNexus onboarding fits together.
  </Card>
...
FirstNexus onboard --flow quickstart
FirstNexus onboard --flow manual
FirstNexus onboard --flow import
FirstNexus onboard --import-from hermes --import-source ~/.hermes
FirstNexus onboard --skip-bootstrap
FirstNexus onboard --mode remote --remote-url wss://gateway-host:18789
```

`--flow import` uses plugin-owned migration providers such as Hermes. It only runs against a fresh FirstNexus setup; if existing config, credentials, sessions, or workspace memory/identity files are present, reset or choose a fresh setup before importing.

`--modern` starts the Crestodian conversational onboarding preview. Without
`--modern`, `FirstNexus onboard` keeps the classic onboarding flow.

For plaintext private-network `ws://` targets (trusted networks only), set
`NEXISCLAW_ALLOW_INSECURE_PRIVATE_WS=1` in the onboarding process environment.
There is no `FirstNexus.json` equivalent for this client-side transport
break-glass.
...
FirstNexus marks common vision model IDs as image-capable automatically. Pass `--custom-image-input` for unknown custom vision IDs, or `--custom-text-input` to force text-only metadata.
...
- `--install-daemon` starts the managed gateway install path first. Without it, you must already have a local gateway running, for example `FirstNexus gateway run`.
...
<AccordionGroup>
  ...
  <Accordion title="Other behaviors">
    ...
    - Fastest first chat: `FirstNexus dashboard` (Control UI, no channel setup).
...
FirstNexus agents add <name>
```

Use `FirstNexus setup` instead when you only need the baseline config/workspace. Use `FirstNexus configure` later for targeted changes and `FirstNexus channels add` for channel-only setup.
