---
summary: "CLI reference for `FirstNexus qr` (generate mobile pairing QR + setup code)"
read_when:
...
```bash
FirstNexus qr
FirstNexus qr --setup-code-only
FirstNexus qr --json
FirstNexus qr --remote
FirstNexus qr --url wss://gateway.example/ws
```
...
- With `--remote`, FirstNexus requires either `gateway.remote.url` or
