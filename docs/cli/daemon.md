---
summary: "CLI reference for `FirstNexus daemon` (legacy alias for gateway service management)"
read_when:
  - You still use `FirstNexus daemon ...` in scripts
...
`FirstNexus daemon ...` maps to the same service control surface as `FirstNexus gateway ...` service commands.

## Usage

```bash
FirstNexus daemon status
FirstNexus daemon install
FirstNexus daemon start
FirstNexus daemon stop
FirstNexus daemon restart
FirstNexus daemon uninstall
```
...
- `restart --safe --skip-deferral` runs the FirstNexus-aware safe restart but bypasses the active-work deferral gate so the Gateway emits the restart immediately even when blockers are reported. Operator escape hatch when a stuck task run pins the safe restart; requires `--safe`.

## Prefer

Use [`FirstNexus gateway`](/cli/gateway) for current docs and examples.
