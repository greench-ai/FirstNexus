---
summary: "CLI reference for `FirstNexus devices` (device pairing + token rotation/revocation)"
read_when:
...
### `FirstNexus devices remove <deviceId>`
...
```
FirstNexus devices remove <deviceId>
FirstNexus devices remove <deviceId> --json
```

### `FirstNexus devices clear --yes [--pending]`

Clear paired devices in bulk.

```
FirstNexus devices clear --yes
FirstNexus devices clear --yes --pending
FirstNexus devices clear --yes --pending --json
```

### `FirstNexus devices approve [requestId] [--latest]`

Approve a pending device pairing request by exact `requestId`. If `requestId`
is omitted or `--latest` is passed, FirstNexus only prints the selected pending
request and exits; rerun approval with the exact request ID after verifying
the details.

<Note>
If a device retries pairing with changed auth details (role, scopes, or public key), FirstNexus supersedes the previous pending entry and issues a new `requestId`. Run `FirstNexus devices list` right before approval to use the current ID.
</Note>

If the device is already paired and asks for broader scopes or a broader role,
FirstNexus keeps the existing approval in place and creates a new pending upgrade
request. Review the `Requested` vs `Approved` columns in `FirstNexus devices list`
or use `FirstNexus devices approve --latest` to preview the exact upgrade before
approving it.
...
### `FirstNexus devices rotate --device <id> --role <role> [--scope <scope...>]`
...
```
FirstNexus devices rotate --device <deviceId> --role operator --scope operator.read --scope operator.write
```
...
### `FirstNexus devices revoke --device <id> --role <role>`
...
```
FirstNexus devices revoke --device <deviceId> --role node
```
...
```bash
FirstNexus devices rotate --device <deviceId> --role operator
```
