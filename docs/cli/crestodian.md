...
read_when:
  - You run FirstNexus with no command and want to understand Crestodian
  - You need a configless-safe way to inspect or repair FirstNexus
  - You are designing or enabling message-channel rescue mode
title: "Crestodian"
---

# `FirstNexus crestodian`

Crestodian is FirstNexus's local setup, repair, and configuration helper. It is
designed to stay reachable when the normal agent path is broken.

Running `FirstNexus` with no command starts Crestodian in an interactive terminal.
Running `FirstNexus crestodian` starts the same helper explicitly.

## What Crestodian shows

On startup, interactive Crestodian opens the same TUI shell used by
`FirstNexus tui`, with a Crestodian chat backend. The chat log starts with a short
greeting:
...
Crestodian uses the same FirstNexus reference discovery as regular agents. In a Git checkout,
...
[https://github.com/FirstNexus/FirstNexus](https://github.com/FirstNexus/FirstNexus), with explicit
...
FirstNexus
FirstNexus crestodian
FirstNexus crestodian --json
FirstNexus crestodian --message "models"
FirstNexus crestodian --message "validate config"
FirstNexus crestodian --message "setup workspace ~/Projects/work model openai/gpt-5.5" --yes
FirstNexus crestodian --message "set default model openai/gpt-5.5" --yes
FirstNexus onboard --modern
```
...
config set-ref gateway.auth.token env NEXISCLAW_GATEWAY_TOKEN
gateway status
restart gateway
agents
...
plugin install clawhub:FirstNexus-codex-app-server
plugin uninstall FirstNexus-codex-app-server
talk to work agent
...
- `FirstNexus.json` is missing
- `FirstNexus.json` is invalid
- the Gateway is down
- plugin command registration is unavailable
- no agent has been configured yet

`FirstNexus --help` and `FirstNexus --version` still use the normal fast paths.
Noninteractive `FirstNexus` exits with a short message instead of printing root
...
```text
~/.FirstNexus/audit/crestodian.jsonl
```

Discovery is not audited. Only applied operations and writes are logged.

`FirstNexus onboard --modern` starts Crestodian as the modern onboarding preview.
Plain `FirstNexus onboard` still runs classic onboarding.
...
planner turn through FirstNexus's normal runtime paths. It first uses the
configured FirstNexus model. If no configured model is usable yet, it can fall
...
`FirstNexus tui`, `FirstNexus chat`, and `FirstNexus terminal` still open the normal
...
FirstNexus: Crestodian rescue mode. Gateway reachable: no. Config valid: no.
You: /crestodian restart gateway
FirstNexus: Plan: restart the Gateway. Reply /crestodian yes to apply.
You: /crestodian yes
FirstNexus: Applied. Audit entry written.
```
...
- Remote rescue cannot open the local TUI or switch into an interactive agent
  session. Use local `FirstNexus` for agent handoff.
...
That lane starts with an empty state dir, routes bare `FirstNexus` to Crestodian,
