---
summary: "CLI reference for `FirstNexus memory` (status/index/search/promote/promote-explain/rem-harness)"
read_when:
...
```bash
FirstNexus memory status
FirstNexus memory status --deep
FirstNexus memory status --fix
FirstNexus memory index --force
FirstNexus memory search "meeting notes"
FirstNexus memory search --query "deployment" --max-results 20
FirstNexus memory promote --limit 10 --min-score 0.75
FirstNexus memory promote --apply
FirstNexus memory promote --json --min-recall-count 0 --min-unique-queries 0
FirstNexus memory promote-explain "router vlan"
FirstNexus memory promote-explain "router vlan" --json
FirstNexus memory rem-harness
FirstNexus memory rem-harness --json
FirstNexus memory status --json
FirstNexus memory status --deep --index
FirstNexus memory status --deep --index --verbose
FirstNexus memory status --agent main
FirstNexus memory index --agent main --verbose
```
...
```bash
FirstNexus memory promote [--apply] [--limit <n>] [--include-promoted]
```
...
- When dreaming is enabled, `memory-core` auto-manages one cron job that runs a full sweep (`light -> REM -> deep`) in the background (no manual `FirstNexus cron add` required).
- `--agent <id>`: scope to a single agent (default: the default agent).
...
```bash
FirstNexus memory promote-explain <selector> [--agent <id>] [--include-promoted] [--json]
```
...
```bash
FirstNexus memory rem-harness [--agent <id>] [--include-promoted] [--json]
```
