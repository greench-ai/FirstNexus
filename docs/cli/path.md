---
summary: "CLI reference for `FirstNexus path` (inspect and edit workspace files via the `oc://` addressing scheme)"
read_when:
...
```bash
FirstNexus plugins enable oc-path
```

## Why use it

FirstNexus state is spread across human-edited markdown, commented JSONC config,
...
`FirstNexus path` gives those callers a stable address instead of a one-off grep,
...
You probably do not need `FirstNexus path` for ordinary whole-file edits, rich
...
```bash
FirstNexus path resolve 'oc://config.jsonc/plugins/github/enabled'
```

Preview a write without touching disk:

```bash
FirstNexus path set 'oc://config.jsonc/plugins/github/enabled' 'true' --dry-run
```

Find matching records in an append-only JSONL log:

```bash
FirstNexus path find 'oc://session.jsonl/[event=tool_call]/name'
```
...
```bash
FirstNexus path resolve 'oc://AGENTS.md/runtime-safety/FirstNexus-gateway'
```

Validate a path in CI or a preflight script before the script reads or writes:

```bash
FirstNexus path validate 'oc://AGENTS.md/tools/$last/risk'
```
...
`FirstNexus path` does four things:
...
# Read a leaf
FirstNexus path resolve 'oc://gateway.jsonc/version'

# Wildcard search
FirstNexus path find 'oc://session.jsonl/*/event' --file ./logs/session.jsonl

# Dry-run a write
FirstNexus path set 'oc://gateway.jsonc/version' '2.0' --dry-run

# Apply the write
FirstNexus path set 'oc://gateway.jsonc/version' '2.0'

# Byte-fidelity round-trip (diagnostic)
FirstNexus path emit ./AGENTS.md
```
...
FirstNexus path resolve 'oc://config.jsonc/agents.defaults.models/"anthropic/claude-opus-4-7"/alias'

# Predicate search over JSONC children
FirstNexus path find 'oc://config.jsonc/plugins/[enabled=true]/id'

# Insert into a JSONC array
FirstNexus path set 'oc://config.jsonc/items/+1' '{"id":"new","enabled":true}' --dry-run

# Insert a JSONC object key
FirstNexus path set 'oc://config.jsonc/plugins/+github' '{"enabled":true}' --dry-run

# Append a JSONL event
FirstNexus path set 'oc://session.jsonl/+' '{"event":"checkpoint","ok":true}' --file ./logs/session.jsonl

# Resolve the last JSONL value line
FirstNexus path resolve 'oc://session.jsonl/$last/event' --file ./logs/session.jsonl

# Address markdown frontmatter
FirstNexus path resolve 'oc://AGENTS.md/[frontmatter]/name'

# Insert markdown frontmatter
FirstNexus path set 'oc://AGENTS.md/[frontmatter]/+description' 'Agent instructions' --dry-run

# Find markdown item fields
FirstNexus path find 'oc://SKILL.md/Tools/*/send_email'

# Validate a session-scoped path
FirstNexus path validate 'oc://AGENTS.md/Tools/$last/risk?session=cron-daily'
```
...
```bash
$ FirstNexus path resolve 'oc://x.md/[frontmatter]/tier' --file frontmatter.md --human
leaf @ L4: "core" (string)

$ FirstNexus path resolve 'oc://x.md/tools/gh/gh' --file frontmatter.md --human
leaf @ L9: "GitHub CLI" (string)

$ FirstNexus path find 'oc://x.md/tools/*' --file frontmatter.md --human
...
```bash
$ FirstNexus path resolve 'oc://config.jsonc/plugins/github/enabled' --file config.jsonc --human
leaf @ L4: "true" (boolean)

$ FirstNexus path set 'oc://config.jsonc/plugins/slack/enabled' 'true' --file config.jsonc --dry-run
...
```bash
$ FirstNexus path find 'oc://session.jsonl/[event=action]/userId' --file session.jsonl --human
1 match for oc://session.jsonl/[event=action]/userId:
  oc://session.jsonl/L2/userId  →  leaf @ L2: "u1" (string)

$ FirstNexus path resolve 'oc://session.jsonl/L2/ts' --file session.jsonl --human
leaf @ L2: "2" (number)
```
...
```bash
FirstNexus path resolve 'oc://AGENTS.md/tools/gh/risk' --human
FirstNexus path resolve 'oc://gateway.jsonc/server/port' --json
```
...
```bash
FirstNexus path find 'oc://AGENTS.md/tools/**/risk'
FirstNexus path find 'oc://session.jsonl/[event=action]/userId'
FirstNexus path find 'oc://config.jsonc/plugins/{github,slack}/enabled'
```
...
FirstNexus path set 'oc://gateway.jsonc/version' '2.0' --dry-run
FirstNexus path set 'oc://gateway.jsonc/version' '2.0'
FirstNexus path set 'oc://AGENTS.md/Tools/+gh/risk' 'low'
```
...
```bash
$ FirstNexus path validate 'oc://AGENTS.md/tools/gh' --human
...
```bash
FirstNexus path emit ./AGENTS.md
FirstNexus path emit ./gateway.jsonc --json
```
...
`FirstNexus path` is TTY-aware: human-readable output on a terminal, JSON when
