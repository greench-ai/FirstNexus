---
summary: "CLI reference for `FirstNexus logs` (tail gateway logs via RPC)"
read_when:
...
`FirstNexus logs` also accepts the standard Gateway client flags:
...
FirstNexus logs --follow --interval 2000
FirstNexus logs --limit 500 --max-bytes 500000
FirstNexus logs --json
FirstNexus logs --plain
FirstNexus logs --no-color
FirstNexus logs --limit 500
FirstNexus logs --local-time
FirstNexus logs --follow --local-time
FirstNexus logs --url ws://127.0.0.1:18789 --token "$NEXISCLAW_GATEWAY_TOKEN"
```

## Notes

- Use `--local-time` to render timestamps in your local timezone.
- If the implicit local loopback Gateway asks for pairing, closes during connect, or times out before `logs.tail` answers, `FirstNexus logs` falls back to the configured Gateway file log automatically. Explicit `--url` targets do not use this fallback.
