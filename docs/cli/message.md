---
summary: "CLI reference for `FirstNexus message` (send + channel actions)"
read_when:
...
```
FirstNexus message <subcommand> [flags]
```
...
- `FirstNexus message` resolves the selected channel to its owning plugin when `--channel` or a channel-prefixed target is present; otherwise it loads configured channel plugins for default-channel inference.
...
- On cache miss, FirstNexus will attempt a live directory lookup when the provider supports it.
...
- `FirstNexus message` resolves supported channel SecretRefs before running the selected action.
- Resolution is scoped to the active action target when possible:
  ...
  - when `--account` is omitted, FirstNexus does not force a `default` account SecretRef scope
...
```
FirstNexus message send --channel discord \
...
```bash
FirstNexus message send --channel googlechat --target spaces/AAA... \
  --message "Choose:" \
...
```
FirstNexus message poll --channel discord \
  --target channel:123 \
  --poll-question "Snack?" \
...
```
FirstNexus message poll --channel telegram \
  --target @mychat \
  --poll-question "Lunch?" \
  --poll-option Pizza --poll-option Sushi \
...
```
FirstNexus message poll --channel msteams \
...
```
FirstNexus message react --channel slack \
...
```
FirstNexus message react --channel signal \
...
```
FirstNexus message send --channel telegram --target @mychat --message "Choose:" \
...
```bash
FirstNexus message send --channel telegram --target @mychat \
