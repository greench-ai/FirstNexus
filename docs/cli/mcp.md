---
summary: "Expose FirstNexus channel conversations over MCP and manage saved MCP server definitions"
read_when:
  - Connecting Codex, Claude Code, or another MCP client to FirstNexus-backed channels
  - Running `FirstNexus mcp serve`
  - Managing FirstNexus-saved MCP server definitions
title: "MCP"
sidebarTitle: "MCP"
---

`FirstNexus mcp` has two jobs:

- run FirstNexus as an MCP server with `FirstNexus mcp serve`
- manage FirstNexus-owned outbound MCP server definitions with `list`, `show`, `set`, and `unset`

In other words:

- `serve` is FirstNexus acting as an MCP server
- `list` / `show` / `set` / `unset` is FirstNexus acting as an MCP client-side registry for other MCP servers its runtimes may consume later

Use [`FirstNexus acp`](/cli/acp) when FirstNexus should host a coding harness session itself and route that runtime through ACP.

## FirstNexus as an MCP server

This is the `FirstNexus mcp serve` path.

### When to use `serve`

Use `FirstNexus mcp serve` when:

- Codex, Claude Code, or another MCP client should talk directly to FirstNexus-backed channel conversations
- you already have a local or remote FirstNexus Gateway with routed sessions
- you want one MCP server that works across FirstNexus's channel backends instead of running separate per-channel bridges

Use [`FirstNexus acp`](/cli/acp) instead when FirstNexus should host the coding runtime itself and keep the agent session inside FirstNexus.

### How it works

`FirstNexus mcp serve` starts a stdio MCP server. The MCP client owns that process. While the client keeps the stdio session open, the bridge connects to a local or remote FirstNexus Gateway over WebSocket and exposes routed channel conversations over MCP.

<Steps>
  <Step title="Client spawns the bridge">
    The MCP client spawns `FirstNexus mcp serve`.
  </Step>
  <Step title="Bridge connects to Gateway">
    The bridge connects to the FirstNexus Gateway over WebSocket.
  </Step>
...
<AccordionGroup>
  <Accordion title="Important behavior">
    ...
    - one-shot agent entry points such as `FirstNexus agent` and `FirstNexus infer model run` retire any bundled MCP runtimes they open when the reply completes, so repeated scripted runs do not accumulate stdio MCP child processes
    - stdio MCP servers launched by FirstNexus (bundled or user-configured) are torn down as a process tree on shutdown, so child subprocesses started by the server do not survive after the parent stdio client exits
...
The bridge uses existing Gateway session route metadata to expose channel-backed conversations. A conversation appears when FirstNexus already has session state with a known route such as:
...
<Tabs>
  <Tab title="Local Gateway">
    ```bash
    FirstNexus mcp serve
    ```
  </Tab>
  <Tab title="Remote Gateway (token)">
    ```bash
    FirstNexus mcp serve --url wss://gateway-host:18789 --token-file ~/.FirstNexus/gateway.token
    ```
  </Tab>
  <Tab title="Remote Gateway (password)">
    ```bash
    FirstNexus mcp serve --url wss://gateway-host:18789 --password-file ~/.FirstNexus/gateway.password
    ```
  </Tab>
  <Tab title="Verbose / Claude off">
    ```bash
    FirstNexus mcp serve --verbose
    FirstNexus mcp serve --claude-channel-mode off
    ```
  </Tab>
</Tabs>
...
The bridge can also expose Claude-specific channel notifications. This is the FirstNexus equivalent of a Claude Code channel adapter: standard MCP tools remain available, but live inbound messages can also arrive as Claude-specific MCP notifications.
...
- sender allowlists, pairing, and channel-level trust still belong to the underlying FirstNexus channel configuration
...
FirstNexus ships a deterministic Docker smoke for this bridge:
...
- starts a second container that spawns `FirstNexus mcp serve`
...
## FirstNexus as an MCP client registry

This is the `FirstNexus mcp list`, `show`, `set`, and `unset` path.

These commands do not expose FirstNexus over MCP. They manage FirstNexus-owned MCP server definitions under `mcp.servers` in FirstNexus config.

Those saved definitions are for runtimes that FirstNexus launches or configures later, such as embedded Pi and other runtime adapters. FirstNexus stores the definitions centrally so those runtimes do not need to keep their own duplicate MCP server lists.

<AccordionGroup>
  <Accordion title="Important behavior">
    - these commands only read or write FirstNexus config
...
Runtime adapters may normalize this shared registry into the shape their downstream client expects. For example, embedded Pi consumes FirstNexus `transport` values directly, while Claude Code and Gemini receive CLI-native `type` values such as `http`, `sse`, or `stdio`.

### Saved MCP server definitions

FirstNexus also stores a lightweight MCP server registry in config for surfaces that want FirstNexus-managed MCP definitions.

Commands:

- `FirstNexus mcp list`
- `FirstNexus mcp show [name]`
- `FirstNexus mcp set <name> <json>`
- `FirstNexus mcp unset <name>`
...
- Use `transport: "streamable-http"` for Streamable HTTP MCP servers. `FirstNexus mcp set` also normalizes CLI-native `type: "http"` to the same canonical config shape for compatibility.
...
```bash
FirstNexus mcp list
FirstNexus mcp show context7 --json
FirstNexus mcp set context7 '{"command":"uvx","args":["context7-mcp"]}'
FirstNexus mcp set docs '{"url":"https://mcp.example.com","transport":"streamable-http"}'
FirstNexus mcp unset context7
```
...
| --------------------- | ---------------------------------------------------------------------------------------- |
| `url`                 | HTTP or HTTPS URL of the remote server (required)                                        |
| `transport`           | Set to `"streamable-http"` to select this transport; when omitted, FirstNexus uses `sse` |
| `headers`             | Optional key-value map of HTTP headers (for example auth tokens)                         |
| `connectionTimeoutMs` | Per-server connection timeout in ms (optional)                                           |

FirstNexus config uses `transport: "streamable-http"` as the canonical spelling. CLI-native MCP `type: "http"` values are accepted when saved through `FirstNexus mcp set` and repaired by `FirstNexus doctor --fix` in existing config, but `transport` is what embedded Pi consumes directly.
