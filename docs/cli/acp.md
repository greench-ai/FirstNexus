...
Run the [Agent Client Protocol (ACP)](https://agentclientprotocol.com/) bridge that talks to an FirstNexus Gateway.

This command speaks ACP over stdio for IDEs and forwards prompts to the Gateway
over WebSocket. It keeps ACP sessions mapped to Gateway session keys.

`FirstNexus acp` is a Gateway-backed ACP bridge, not a full ACP-native editor
runtime. It focuses on session routing, prompt delivery, and basic streaming
updates.

If you want an external MCP client to talk directly to FirstNexus channel
conversations instead of hosting an ACP harness session, use
[`FirstNexus mcp serve`](/cli/mcp) instead.

## What this is not

This page is often confused with ACP harness sessions.

`FirstNexus acp` means:

- FirstNexus acts as an ACP server
- an IDE or ACP client connects to FirstNexus
- FirstNexus forwards that work into a Gateway session

This is different from [ACP Agents](/tools/acp-agents), where FirstNexus runs an
external harness such as Codex or Claude Code through `acpx`.

Quick rule:

- editor/client wants to talk ACP to FirstNexus: use `FirstNexus acp`
- FirstNexus should launch Codex/Claude/Gemini as an ACP harness: use `/acp spawn` and [ACP Agents](/tools/acp-agents)
...
| Session lineage metadata                                              | Implemented | Session listings and session info snapshots include FirstNexus parent and child lineage in `_meta` so ACP clients can render subagent graphs without private Gateway side channels.                                                              |
...
| Per-session MCP servers (`mcpServers`)                                | Unsupported | Bridge mode rejects per-session MCP server requests. Configure MCP on the FirstNexus gateway or agent instead.                                                                                                                                   |
...
# Remote Gateway
FirstNexus acp --url wss://gateway-host:18789 --token <token>

# Remote Gateway (token from file)
FirstNexus acp --url wss://gateway-host:18789 --token-file ~/.FirstNexus/gateway.token

# Attach to an existing session key
FirstNexus acp --session agent:main:main

# Attach by label (must already exist)
FirstNexus acp --session-label "support inbox"

# Reset the session key before the first prompt
FirstNexus acp --session agent:main:main --reset-session
```
...
```bash
FirstNexus acp client

# Point the spawned bridge at a remote Gateway
FirstNexus acp client --server-args --url wss://gateway-host:18789 --token-file ~/.FirstNexus/gateway.token

# Override the server command (default: FirstNexus)
FirstNexus acp client --server "node" --server-args FirstNexus.mjs acp --url ws://127.0.0.1:19001
```
...
- This ACP bridge policy is separate from ACPX harness permissions. If you run FirstNexus through the `acpx` backend, `plugins.entries.acpx.config.permissionMode=approve-all` is the break-glass "yolo" switch for that harness session.

## Protocol smoke testing

For protocol-level debugging, start a Gateway with isolated state and drive
`FirstNexus acp` over stdio with an ACP JSON-RPC client. Cover `initialize`,
...
Avoid using `FirstNexus gateway call sessions.list` as the only ACP proof. That
...
it to drive an FirstNexus Gateway session.

1. Ensure the Gateway is running (local or remote).
2. Configure the Gateway target (config or flags).
3. Point your IDE to run `FirstNexus acp` over stdio.

Example config (persisted):

```bash
FirstNexus config set gateway.remote.url wss://gateway-host:18789
FirstNexus config set gateway.remote.token <token>
```
...
```bash
FirstNexus acp --session agent:main:main
FirstNexus acp --session agent:design:main
FirstNexus acp --session agent:qa:bug-123
```
...
If you want ACPX-backed sessions to see FirstNexus plugin tools or selected
...
[FirstNexus tools MCP bridge](/tools/acp-agents-setup#FirstNexus-tools-mcp-bridge).

## Use from `acpx` (Codex, Claude, other ACP clients)

If you want a coding agent such as Codex or Claude Code to talk to your
FirstNexus bot over ACP, use `acpx` with its built-in `FirstNexus` target.

Typical flow:

1. Run the Gateway and make sure the ACP bridge can reach it.
2. Point `acpx FirstNexus` at `FirstNexus acp`.
3. Target the FirstNexus session key you want the coding agent to use.

Examples:

```bash
# One-shot request into your default FirstNexus ACP session
acpx FirstNexus exec "Summarize the active FirstNexus session state."

# Persistent named session for follow-up turns
acpx FirstNexus sessions ensure --name codex-bridge
acpx FirstNexus -s codex-bridge --cwd /path/to/repo \
  "Ask my FirstNexus work agent for recent context relevant to this repo."
```

If you want `acpx FirstNexus` to target a specific Gateway and session key every
time, override the `FirstNexus` agent command in `~/.acpx/config.json`:

```json
{
  "agents": {
    "FirstNexus": {
      "command": "env NEXISCLAW_HIDE_BANNER=1 NEXISCLAW_SUPPRESS_NOTES=1 FirstNexus acp --url ws://127.0.0.1:18789 --token-file ~/.FirstNexus/gateway.token --session agent:main:main"
    }
  }
}
```

For a repo-local FirstNexus checkout, use the direct CLI entrypoint instead of the
dev runner so the ACP stream stays clean. For example:

```bash
env NEXISCLAW_HIDE_BANNER=1 NEXISCLAW_SUPPRESS_NOTES=1 node FirstNexus.mjs acp ...
```

This is the easiest way to let Codex, Claude Code, or another ACP-aware client
pull contextual information from an FirstNexus agent without scraping a terminal.
...
In Zed, open the Agent panel and select "FirstNexus ACP" to start a thread.
...
- Prefer `--token-file`/`--password-file` or environment variables (`NEXISCLAW_GATEWAY_TOKEN`, `NEXISCLAW_GATEWAY_PASSWORD`).
- Gateway auth resolution follows the shared contract used by other Gateway clients:
  - local mode: env (`NEXISCLAW_GATEWAY_*`) -> `gateway.auth.*` -> `gateway.remote.*` fallback only when `gateway.auth.*` is unset (configured-but-unresolved local SecretRefs fail closed)
...
- ACP runtime backend child processes receive `NEXISCLAW_SHELL=acp`, which can be used for context-specific shell/profile rules.
- `FirstNexus acp client` sets `NEXISCLAW_SHELL=acp-client` on the spawned bridge process.

### `acp client` options

- `--cwd <dir>`: working directory for the ACP session.
- `--server <command>`: ACP server command (default: `FirstNexus`).
