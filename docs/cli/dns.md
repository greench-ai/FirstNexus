---
summary: "CLI reference for `FirstNexus dns` (wide-area discovery helpers)"
read_when:
  - You want wide-area discovery (DNS-SD) via Tailscale + CoreDNS
  - You're setting up split DNS for a custom discovery domain (example: FirstNexus.internal)
title: "DNS"
---
...
```bash
FirstNexus dns setup
FirstNexus dns setup --domain FirstNexus.internal
...
- `--domain <domain>`: wide-area discovery domain (for example `FirstNexus.internal`)
...
- zone file path
- current tailnet IPs
- recommended `FirstNexus.json` discovery config
...
- If `--domain` is omitted, FirstNexus uses `discovery.wideArea.domain` from config.
