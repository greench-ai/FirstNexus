---
summary: "CLI reference for `FirstNexus configure` (interactive configuration prompts)"
read_when:
...
Use `FirstNexus onboard` for the full guided first-run journey, `FirstNexus setup` for the baseline config/workspace only, and `FirstNexus channels add` when you only need channel account setup.
...
<Tip>
`FirstNexus config` without a subcommand opens the same wizard. Use `FirstNexus config get|set|unset` for non-interactive edits.
</Tip>

For web search, `FirstNexus configure --section web` lets you choose a provider
...
FirstNexus configure --section model --section channels
FirstNexus configure --section gateway --section daemon
```
