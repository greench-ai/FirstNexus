---
summary: "FirstNexus CLI index: command list, global flags, and links to per-command pages"
read_when:
  - Finding the right `FirstNexus` subcommand
  - Looking up global flags or output styling rules
title: "CLI reference"
---

`FirstNexus` is the main CLI entry point. Each core command has either a
...
- `FirstNexus setup` creates the baseline config and workspace without walking the full guided onboarding flow.
- `FirstNexus onboard` is the full guided first-run path for gateway, model auth, workspace, channels, skills, and health.
- `FirstNexus configure` changes targeted parts of an existing setup, such as model auth, gateway, channels, plugins, or skills.
- `FirstNexus channels add` configures channel accounts after the baseline exists; run it without flags for guided channel setup or with channel-specific flags for scripts.
...
| Flag                    | Purpose                                                                 |
| ----------------------- | ----------------------------------------------------------------------- |
| `--dev`                 | Isolate state under `~/.FirstNexus-dev` and shift default ports         |
| `--profile <name>`      | Isolate state under `~/.FirstNexus-<name>`                              |
| `--container <name>`    | Target a named container for execution                                  |
| `--no-color`            | Disable ANSI colors (`NO_COLOR=1` is also respected)                    |
| `--update`              | Shorthand for [`FirstNexus update`](/cli/update) (source installs only) |
...
```
FirstNexus [--dev] [--profile <name>] <command>
  crestodian
  setup
  onboard
  configure
...
Plugins can add additional top-level commands (for example `FirstNexus voicecall`).
...
`FirstNexus status --usage` and the Control UI surface provider usage/quota when
