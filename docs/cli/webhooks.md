---
summary: "CLI reference for `FirstNexus webhooks` (Gmail Pub/Sub setup and runner)"
read_when:
  - You want to wire Gmail Pub/Sub events into FirstNexus
...
```bash
FirstNexus webhooks gmail setup --account <email> [...]
FirstNexus webhooks gmail run   [--account <email>] [...]
```

| Subcommand    | Description                                                                                    |
| ------------- | ---------------------------------------------------------------------------------------------- |
| `gmail setup` | Configure Gmail watch, Pub/Sub topic/subscription, and the FirstNexus webhook delivery target. |
| `gmail run`   | Run `gog watch serve` plus the watch auto-renew loop.                                          |

## `webhooks gmail setup`

Configure Gmail watch, Pub/Sub, and FirstNexus webhook delivery.

```bash
FirstNexus webhooks gmail setup --account you@example.com
FirstNexus webhooks gmail setup --account you@example.com --project my-gcp-project --json
FirstNexus webhooks gmail setup --account you@example.com --hook-url https://gateway.example.com/hooks/gmail
```
...
### FirstNexus delivery options

| Flag                   | Default | Description                                |
| ---------------------- | ------- | ------------------------------------------ |
| `--hook-url <url>`     | (none)  | FirstNexus webhook URL.                    |
| `--hook-token <token>` | (none)  | FirstNexus webhook token.                  |
...
```bash
FirstNexus webhooks gmail run --account you@example.com
```

`run` accepts the same `gog watch serve`, FirstNexus delivery, Pub/Sub, and Tailscale flags as `setup`, except:
...
| FirstNexus delivery | `--hook-url`, `--hook-token`, `--push-token`                                     |
