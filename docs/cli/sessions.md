---
summary: "CLI reference for `FirstNexus sessions` (list stored sessions + usage)"
read_when:
...
until a message is processed. Use `FirstNexus channels status --probe`,
`FirstNexus status --deep`, or `FirstNexus health --verbose` when you need live
channel connectivity.

`FirstNexus sessions` and Gateway `sessions.list` responses are bounded by
...
```bash
FirstNexus sessions
FirstNexus sessions --agent work
FirstNexus sessions --all-agents
FirstNexus sessions --active 120
FirstNexus sessions --limit 25
FirstNexus sessions --verbose
FirstNexus sessions --json
```
...
FirstNexus sessions export-trajectory --session-key "agent:main:telegram:direct:123" --output bug-123 --json
```
...
inside `.FirstNexus/trajectory-exports/` under the selected workspace.

`FirstNexus sessions --all-agents` reads configured agent stores. Gateway and ACP
...
`FirstNexus sessions --all-agents --json`:

```json
{
  "path": null,
  "stores": [
    { "agentId": "main", "path": "/home/user/.FirstNexus/agents/main/sessions/sessions.json" },
    { "agentId": "work", "path": "/home/user/.FirstNexus/agents/work/sessions/sessions.json" }
  ],
  "allAgents": true,
  "count": 2,
  "totalCount": 2,
...
FirstNexus sessions cleanup --agent work --dry-run
FirstNexus sessions cleanup --all-agents --dry-run
FirstNexus sessions cleanup --enforce
FirstNexus sessions cleanup --enforce --active-key "agent:main:telegram:direct:123"
FirstNexus sessions cleanup --dry-run --fix-dm-scope
FirstNexus sessions cleanup --json
```

`FirstNexus sessions cleanup` uses `session.maintenance` settings from config:

- Scope note: `FirstNexus sessions cleanup` maintains session stores, transcripts, and trajectory sidecars. It does not prune cron run logs (`cron/runs/<jobId>.jsonl`), which are managed by `cron.runLog.maxBytes` and `cron.runLog.keepLines` in [Cron configuration](/automation/cron-jobs#configuration) and explained in [Cron maintenance](/automation/cron-jobs#maintenance).
...
`FirstNexus sessions cleanup --all-agents --dry-run --json`:
...
```json
{
  ...
  "stores": [
    {
      "agentId": "main",
      "storePath": "/home/user/.FirstNexus/agents/main/sessions/sessions.json",
      "beforeCount": 120,
      "afterCount": 80,
      "missing": 0,
      "dmScopeRetired": 0,
    ...
    {
      "agentId": "work",
      "storePath": "/home/user/.FirstNexus/agents/work/sessions/sessions.json",
      "beforeCount": 18,
      "afterCount": 18,
      "missing": 0,
      "dmScopeRetired": 0,
