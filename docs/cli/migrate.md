---
summary: "CLI reference for `FirstNexus migrate` (import state from another agent system)"
read_when:
  - You want to migrate from Hermes or another agent system into FirstNexus
...
```bash
FirstNexus migrate list
FirstNexus migrate claude --dry-run
FirstNexus migrate codex --dry-run
FirstNexus migrate codex --skill gog-vault77-google-workspace
FirstNexus migrate codex --plugin google-calendar --dry-run
FirstNexus migrate hermes --dry-run
FirstNexus migrate hermes
FirstNexus migrate apply codex --yes --skill gog-vault77-google-workspace
FirstNexus migrate apply codex --yes --plugin google-calendar
FirstNexus migrate apply codex --yes
FirstNexus migrate apply claude --yes
FirstNexus migrate apply hermes --yes
FirstNexus migrate apply hermes --include-secrets --yes
FirstNexus onboard --flow import
FirstNexus onboard --import-from claude --import-source ~/.claude
FirstNexus onboard --import-from hermes --import-source ~/.hermes
```

<ParamField path="<provider>" type="string">
  Name of a registered migration provider, for example `hermes`. Run `FirstNexus migrate list` to see installed providers.
</ParamField>
...
<ParamField path="--no-backup" type="boolean">
  Skip the pre-apply backup. Requires `--force` when local FirstNexus state exists.
</ParamField>
...
`FirstNexus migrate` is preview-first.
...
<AccordionGroup>
  <Accordion title="Preview before apply">
    ...
    `FirstNexus migrate apply <provider>` previews the plan and prompts before changing state unless `--yes` is set. In non-interactive mode, apply requires `--yes`.

  </Accordion>
  <Accordion title="Backups">
    Apply creates and verifies an FirstNexus backup before applying the migration. If no local FirstNexus state exists yet, the backup step is skipped and the migration can continue. To skip a backup when state exists, pass both `--no-backup` and `--force`.
  </Accordion>
...
- Project `CLAUDE.md` and `.claude/CLAUDE.md` into the FirstNexus agent workspace.
...
- Claude command Markdown files converted into FirstNexus skills with manual invocation only.

### Archive and manual-review state

Claude hooks, permissions, environment defaults, local memory, path-scoped rules, subagents, caches, plans, and project history are preserved in the migration report or reported as manual-review items. FirstNexus does not execute hooks, copy broad allowlists, or import OAuth/Desktop credential state automatically.
...
Use this provider when moving to the FirstNexus Codex harness and you want to
...
Running `FirstNexus migrate codex` in an interactive terminal previews the full
...
start checked unless the target FirstNexus Codex plugin config already has that
...
```bash
FirstNexus migrate codex --dry-run --skill gog-vault77-google-workspace
FirstNexus migrate apply codex --yes --skill gog-vault77-google-workspace
```
...
```bash
FirstNexus migrate codex --dry-run --plugin google-calendar
FirstNexus migrate apply codex --yes --plugin google-calendar
```
...
- Personal AgentSkills under `$HOME/.agents/skills`, copied into the current
  FirstNexus agent workspace when you want per-agent ownership.
...
- `SOUL.md` and `AGENTS.md` into the FirstNexus agent workspace.
- `memories/MEMORY.md` and `memories/USER.md` appended to workspace memory files.
- Memory config defaults for FirstNexus file memory, plus archive or manual-review items for external memory providers such as Honcho.
...
Hermes state that FirstNexus cannot safely interpret is copied into the migration report for manual review, but it is not loaded into live FirstNexus config or credentials. This preserves opaque or unsafe state without pretending FirstNexus can execute or trust it automatically:
...
Migration sources are plugins. A plugin declares its provider ids in `FirstNexus.plugin.json`:
...
Provider plugins can use `FirstNexus/plugin-sdk/migration` for item construction and summary counts, plus `FirstNexus/plugin-sdk/migration-runtime` for conflict-aware file copies, archive-only report copies, cached config-runtime wrappers, and migration reports.

## Onboarding integration

Onboarding can offer migration when a provider detects a known source. Both `FirstNexus onboard --flow import` and `FirstNexus setup --wizard --import-from hermes` use the same plugin migration provider and still show a preview before applying.

<Note>
Onboarding imports require a fresh FirstNexus setup. Reset config, credentials, sessions, and the workspace first if you already have local state. Backup-plus-overwrite or merge imports are feature-gated for existing setups.
</Note>
...
- [Migrating](/install/migrating): move FirstNexus to a new machine.
