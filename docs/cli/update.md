---
summary: "CLI reference for `FirstNexus update` (safe-ish source update + gateway auto-restart)"
read_when:
  - You want to update a source checkout safely
  - You are debugging `FirstNexus update` output or options
...
Safely update FirstNexus and switch between stable/beta/dev channels.
...
FirstNexus update wizard
FirstNexus update --channel beta
FirstNexus update --channel dev
FirstNexus update --tag beta
FirstNexus update --tag main
FirstNexus update --dry-run
FirstNexus update --no-restart
FirstNexus update --yes
FirstNexus update --json
FirstNexus --update
```
...
- `--tag <dist-tag|version|spec>`: override the package target for this update only. For package installs, `main` maps to `github:FirstNexus/FirstNexus#main`.
...
`FirstNexus update` does not have a `--verbose` flag. Use `--dry-run` to preview
the planned channel/tag/install/restart actions, `--json` for machine-readable
results, and `FirstNexus update status --json` when you only need channel and
...
<Note>
In Nix mode (`NEXISCLAW_NIX_MODE=1`), mutating `FirstNexus update` runs are disabled. Update the Nix source or flake input for this install instead; for nix-FirstNexus, use the agent-first [Quick Start](https://github.com/FirstNexus/nix-FirstNexus#quick-start). `FirstNexus update status` and `FirstNexus update --dry-run` remain read-only.
</Note>
...
```bash
FirstNexus update status
FirstNexus update status --json
FirstNexus update status --timeout 10
```
...
When you switch channels explicitly (`--channel ...`), FirstNexus also keeps the
install method aligned:

- `dev` → ensures a git checkout (default: `~/FirstNexus`, override with `NEXISCLAW_GIT_DIR`),
...
For package-manager installs, `FirstNexus update` resolves the target package
version before invoking the package manager. npm global installs use a staged
install: FirstNexus installs the new package into a temporary npm prefix, verifies
...
installed FirstNexus build while leaving full plugin-command completion rebuilds to
explicit `FirstNexus completion --write-state` runs.
...
healthy. If the plist is installed but launchd is not supervising it, FirstNexus
...
<Steps>
  ...
  <Step title="Run doctor">
    `FirstNexus doctor` runs as the final safe-update check.
  </Step>
...
beta release, FirstNexus falls back to the recorded default/latest spec and reports
that as a warning. For npm plugins, FirstNexus also falls back when the beta
...
<Warning>
If an exact pinned npm plugin update resolves to an artifact whose integrity differs from the stored install record, `FirstNexus update` aborts that plugin artifact update instead of installing it. Reinstall or update the plugin explicitly only after verifying that you trust the new artifact.
</Warning>

<Note>
Post-update plugin sync failures that are scoped to a managed plugin and that the sync path can route around (e.g. an unreachable npm registry for a non-essential plugin) are reported as warnings after the core update succeeds. The JSON result keeps the top-level update `status: "ok"` and reports `postUpdate.plugins.status: "warning"` with `FirstNexus doctor --fix` and `FirstNexus plugins inspect <id> --runtime --json` guidance. Unexpected updater or sync exceptions still fail the update result. Fix the plugin install or update error, then rerun `FirstNexus doctor --fix` or `FirstNexus update`.

After the per-plugin sync step, `FirstNexus update` runs a mandatory **post-core convergence** pass before the gateway is restarted: it repairs missing configured plugin payloads, validates each _active_ tracked install record on disk, and statically verifies its `package.json` is parseable (and any explicitly-declared `main` exists). Failures from this pass — and an invalid FirstNexus config snapshot — return `postUpdate.plugins.status: "error"` and flip the top-level update `status` to `"error"`, so `FirstNexus update` exits non-zero and the gateway is _not_ restarted with an unverified plugin set. The error includes structured `postUpdate.plugins.warnings[].guidance` lines pointing at `FirstNexus doctor --fix` and `FirstNexus plugins inspect <id> --runtime --json` for follow-up. Disabled plugin entries and records that are not trusted-source-linked official sync targets are skipped here, mirroring the `skipDisabledPlugins` policy used by the missing-payload check, so a stale disabled plugin record cannot block an otherwise valid update.
...
`FirstNexus --update` rewrites to `FirstNexus update` (useful for shells and launcher scripts).

## Related

- `FirstNexus doctor` (offers to run update first on git checkouts)
