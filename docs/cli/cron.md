---
summary: "CLI reference for `FirstNexus cron` (schedule and run background jobs)"
read_when:
...
<Tip>
Run `FirstNexus cron --help` for the full command surface. See [Cron jobs](/automation/cron-jobs) for the conceptual guide.
</Tip>
...
`FirstNexus cron list` and `FirstNexus cron show <job-id>` preview the resolved delivery route. For `channel: "last"`, the preview shows whether the route resolved from the main or current session, or will fail closed.
...
If an isolated run times out before the first model request, `FirstNexus cron show`
and `FirstNexus cron runs` include a phase-specific error such as
...
Skipped runs are tracked separately from execution errors. They do not affect retry backoff, but `FirstNexus cron edit <job-id> --failure-alert-include-skipped` can opt failure alerts into repeated skipped-run notifications.
...
`FirstNexus cron run` returns as soon as the manual run is queued. Successful responses include `{ ok: true, enqueued: true, runId }`. Use `FirstNexus cron runs --id <job-id>` to follow the eventual outcome.

<Note>
`FirstNexus cron run <job-id>` force-runs by default. Use `--due` to keep the older "only run if due" behavior.
</Note>
...
- When a job has `--model` but no fallback list is configured, FirstNexus passes an explicit empty fallback override so the agent primary is not appended as a hidden retry target.
...
- `cron.runLog.maxBytes` and `cron.runLog.keepLines` prune `~/.FirstNexus/cron/runs/<jobId>.jsonl`.

## Migrating older jobs

<Note>
If you have cron jobs from before the current delivery and store format, run `FirstNexus doctor --fix`. Doctor normalizes legacy cron fields (`jobId`, `schedule.cron`, top-level delivery fields including legacy `threadId`, payload `provider` delivery aliases) and migrates simple `notify: true` webhook fallback jobs to explicit webhook delivery when `cron.webhook` is configured.
</Note>
...
```bash
FirstNexus cron edit <job-id> --announce --channel telegram --to "123456789"
```

Disable delivery for an isolated job:

```bash
FirstNexus cron edit <job-id> --no-deliver
```

Enable lightweight bootstrap context for an isolated job:

```bash
FirstNexus cron edit <job-id> --light-context
```

Announce to a specific channel:

```bash
FirstNexus cron edit <job-id> --announce --channel slack --to "channel:C1234567890"
```

Announce to a Telegram forum topic:

```bash
FirstNexus cron edit <job-id> --announce --channel telegram --to "-1001234567890" --thread-id 42
```
...
FirstNexus cron list --agent ops
FirstNexus cron get <job-id>
FirstNexus cron show <job-id>
FirstNexus cron run <job-id>
FirstNexus cron run <job-id> --due
FirstNexus cron runs --id <job-id> --limit 50
```

`FirstNexus cron list` shows all matching jobs by default. Pass `--agent <id>` to show only jobs whose effective normalized agent id matches; jobs without a stored agent id count as the configured default agent.

`FirstNexus cron get <job-id>` returns the stored job JSON directly. Use `cron show <job-id>` when you want the human-readable view with delivery-route preview.
...
```bash
FirstNexus cron edit <job-id> --agent ops
FirstNexus cron edit <job-id> --clear-agent
FirstNexus cron edit <job-id> --session current
FirstNexus cron edit <job-id> --session "session:daily-brief"
```

`FirstNexus cron add` warns when `--agent` is omitted on agent-turn jobs and falls back to the default agent (`main`). Pass `--agent <id>` at create time to pin a specific agent.

Delivery tweaks:

```bash
FirstNexus cron edit <job-id> --announce --channel slack --to "channel:C1234567890"
FirstNexus cron edit <job-id> --best-effort-deliver
FirstNexus cron edit <job-id> --no-best-effort-deliver
FirstNexus cron edit <job-id> --no-deliver
```
