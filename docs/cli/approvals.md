---
summary: "CLI reference for `FirstNexus approvals` and `FirstNexus exec-policy`"
read_when:
...
# `FirstNexus approvals`

Manage exec approvals for the **local host**, **gateway host**, or a **node host**.
By default, commands target the local approvals file on disk. Use `--gateway` to target the gateway, or `--node` to target a specific node.

Alias: `FirstNexus exec-approvals`
...
## `FirstNexus exec-policy`

`FirstNexus exec-policy` is the local convenience command for keeping the requested
...
- synchronize local `tools.exec.*` and local `~/.FirstNexus/exec-approvals.json`

Examples:

```bash
FirstNexus exec-policy show
FirstNexus exec-policy show --json

FirstNexus exec-policy preset yolo
FirstNexus exec-policy preset cautious --json

FirstNexus exec-policy set --host gateway --security full --ask off --ask-fallback full
```
...
- `FirstNexus exec-policy show` marks `host=node` scopes as node-managed at runtime instead of deriving an effective policy from the local approvals file

If you need to edit remote host approvals directly, keep using `FirstNexus approvals set --gateway`
or `FirstNexus approvals set --node <id|name|ip>`.

## Common commands

```bash
FirstNexus approvals get
FirstNexus approvals get --node <id|name|ip>
FirstNexus approvals get --gateway
```

`FirstNexus approvals get` now shows the effective exec policy for local, gateway, and node targets:
...
```bash
FirstNexus approvals set --file ./exec-approvals.json
FirstNexus approvals set --stdin <<'EOF'
{ version: 1, defaults: { security: "full", ask: "off" } }
EOF
FirstNexus approvals set --node <id|name|ip> --file ./exec-approvals.json
FirstNexus approvals set --gateway --file ./exec-approvals.json
```
...
This changes the **host approvals file** only. To keep the requested FirstNexus policy aligned, also set:
...
```bash
FirstNexus approvals allowlist add "~/Projects/**/bin/rg"
FirstNexus approvals allowlist add --agent main --node <id|name|ip> "/usr/bin/uptime"
FirstNexus approvals allowlist add --agent "*" "/usr/bin/uname"

FirstNexus approvals allowlist remove "~/Projects/**/bin/rg"
```
...
- `--node` uses the same resolver as `FirstNexus nodes` (id, name, ip, or id prefix).
- `--agent` defaults to `"*"`, which applies to all agents.
- The node host must advertise `system.execApprovals.get/set` (macOS app or headless node host).
- Approvals files are stored per host at `~/.FirstNexus/exec-approvals.json`.
