...
- repo: `FirstNexus/docs`
- local checkout: `~/Projects/FirstNexus-docs`

## Source of truth

- English docs are authored in `FirstNexus/FirstNexus`.
...
1. Edit English docs in `FirstNexus/FirstNexus`.
2. Push to `main`.
3. `FirstNexus/FirstNexus/.github/workflows/docs-sync-publish.yml` mirrors the docs tree into `FirstNexus/docs`.
...
5. `FirstNexus/docs/.github/workflows/translate-all.yml` waits for `main` to settle, translates only stale or missing locale pages, and uploads per-locale artifacts.
...
- Docs translation workflows generate the same non-English locale set in `FirstNexus/docs`.
...
- Do not treat missing `th` or `fa` entries in generated `docs/docs.json` as a pipeline failure. Verify their generated folders in `FirstNexus/docs` instead.
...
- Sync metadata is written to `.FirstNexus-sync/source.json` in the publish repo.
- Source repo secret: `NEXISCLAW_DOCS_SYNC_TOKEN`
- Publish repo secret: `NEXISCLAW_DOCS_I18N_OPENAI_API_KEY`
- If locale output looks stale, check the `Translate All` workflow in `FirstNexus/docs` first.
