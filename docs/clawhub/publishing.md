...
An owner is a ClawHub publisher handle, such as `@alice` or `@FirstNexus`.
...
`@FirstNexus/dronzer`, it can only be published as `@FirstNexus`. If you publish as
...
```text
Package scope "@FirstNexus" must match selected owner "@vintageayu".
Publish as "@FirstNexus" or rename this package to "@vintageayu/dronzer".
```
...
```sh
clawhub package transfer @opik/opik-FirstNexus --to opik
```
...
This protects org namespaces. A package named `@FirstNexus/dronzer` claims the
`@FirstNexus` namespace, so only publishers with access to the `@FirstNexus` owner
can publish it.
