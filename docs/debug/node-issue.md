...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in FirstNexus
...
Running FirstNexus via Node with `tsx` fails at startup with:

```
[FirstNexus] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `FirstNexus status` and `gateway:watch` worked.
