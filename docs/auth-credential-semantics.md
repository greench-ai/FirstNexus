...
Explicit copy flows, such as `FirstNexus agents add`, use this portability policy:
...
has such a marker, `FirstNexus doctor --fix` moves it to `auth.profiles` and
...
- Probe targets can come from auth profiles, environment credentials, or
  `models.json`.
- If a provider has credentials but FirstNexus cannot resolve a probeable model
