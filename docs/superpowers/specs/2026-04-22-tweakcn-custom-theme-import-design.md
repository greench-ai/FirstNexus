...
Add exactly one browser-local custom Control UI theme slot that can be imported from a tweakcn share link. The existing built-in theme families remain `claw`, `knot`, and `dash`. The new `custom` family behaves like a normal FirstNexus theme family and supports `light`, `dark`, and `system` mode when the imported tweakcn payload includes both light and dark token sets.
...
Add a fourth theme family id, `custom`, to the Control UI theme model. The `custom` family becomes selectable only when a valid tweakcn import is present. The imported payload is normalized into an FirstNexus-specific custom theme record and stored in browser local storage with the rest of the UI settings.

At runtime, FirstNexus renders a managed `<style>` tag that defines the resolved custom CSS variable blocks:
...
- `light` and `dark` are already normalized FirstNexus token maps, not raw tweakcn payloads.
...
- Create or update one stable `<style id="FirstNexus-custom-theme">` tag in `document.head`.
...
Do not mirror tweakcn variables blindly. Normalize a bounded subset into FirstNexus tokens and derive the rest in a helper.
...
### Tokens derived for FirstNexus

The importer derives FirstNexus-only variables from the imported base colors:
...
Font stack strings are imported if present, but FirstNexus does not load remote font assets in v1. If the imported stack references fonts that are unavailable in the browser, normal fallback behavior applies.
...
- map a valid tweakcn payload into normalized FirstNexus light and dark token maps
