---
summary: "Reference design for the public FirstNexus App SDK API, event taxonomy, artifacts, approvals, and package structure"
title: "FirstNexus App SDK API design"
sidebarTitle: "App SDK API design"
read_when:
  - You are implementing the proposed public FirstNexus app SDK
  - You need the draft namespace, event, result, artifact, approval, or security contract for the app SDK
  - You are comparing Gateway protocol resources with the high-level FirstNexus App SDK wrapper
---

This page is the detailed API reference design for the public
[FirstNexus App SDK](/concepts/FirstNexus-sdk). It is intentionally separate from
the [Plugin SDK](/plugins/sdk-overview).

<Note>
  `@FirstNexus/sdk` is the external app/client package for talking to the
  Gateway. `FirstNexus/plugin-sdk/*` is the in-process plugin authoring contract.
...
2. A high-level ergonomic wrapper with `FirstNexus`, `Agent`, `Session`, `Run`,
...
- source: FirstNexus, MCP, plugin, channel, runtime, or app
- schema summary
- approval policy
...
| Package                   | Purpose                                                       |
| ------------------------- | ------------------------------------------------------------- |
| `@FirstNexus/sdk`         | Public high-level SDK and generated low-level Gateway client. |
| `@FirstNexus/sdk-react`   | Optional React hooks for dashboards and app builders.         |
| `@FirstNexus/sdk-testing` | Test helpers and fake Gateway server for app integrations.    |

The repo already has `FirstNexus/plugin-sdk/*` for plugins. Keep that namespace
...
4. High-level `FirstNexus`, `Agent`, `Session`, `Run`, `Task`, and `Artifact`
   wrappers.
...
- [FirstNexus App SDK](/concepts/FirstNexus-sdk)
