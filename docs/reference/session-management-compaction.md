...
FirstNexus manages sessions end-to-end across these areas:
...
FirstNexus is designed around a single **Gateway process** that owns session state.
...
FirstNexus persists sessions in two layers:
...
- Transcripts: `~/.FirstNexus/agents/<agentId>/sessions/<sessionId>.jsonl`
  - Telegram topic sessions: `.../<sessionId>-topic-<threadId>.jsonl`

FirstNexus resolves these via `src/config/sessions.ts`.
...
FirstNexus no longer creates automatic `sessions.json.bak.*` rotation backups during Gateway writes. The legacy `session.maintenance.rotateBytes` key is ignored and `FirstNexus doctor --fix` removes it from older configs.
...
In `mode: "warn"`, FirstNexus reports potential evictions but does not mutate the store/files.

Run maintenance on demand:

```bash
FirstNexus sessions cleanup --dry-run
FirstNexus sessions cleanup --enforce
```
...
- `cron.runLog.maxBytes` + `cron.runLog.keepLines` prune `~/.FirstNexus/cron/runs/<jobId>.jsonl` files (defaults: `2_000_000` bytes and `2000` lines).
...
- **Parent fork policy** uses PI's active branch when creating a thread or subagent fork. If that branch is too large, FirstNexus starts the child with isolated context instead of failing or inheriting unusable history. The sizing policy is automatic; legacy `session.parentForkMaxTokens` config is removed by `FirstNexus doctor --fix`.
...
FirstNexus intentionally does **not** "fix up" transcripts; the Gateway uses `SessionManager` to read/write them.
...
When FirstNexus splits a long transcript into compaction chunks, it keeps
assistant tool calls paired with their matching `toolResult` entries.

- If the token-share split lands between a tool call and its result, FirstNexus
...
- If a trailing tool-result block would otherwise push the chunk over target,
  FirstNexus preserves that pending tool block and keeps the unsummarized tail
  intact.
...
These are Pi runtime semantics (FirstNexus consumes the events, but Pi decides when to compact).

FirstNexus can also trigger a preflight local compaction before opening the next
...
reopen cost, not raw archival: FirstNexus still runs normal semantic compaction,
...
next model call, FirstNexus estimates the prompt pressure using the same preflight
...
FirstNexus also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, FirstNexus bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it's already higher, FirstNexus leaves it alone.
...
- When `agents.defaults.compaction.truncateAfterCompaction` is enabled,
  FirstNexus rotates the active transcript to a compacted successor JSONL after
...
- If the provider fails or returns an empty result, FirstNexus falls back to built-in LLM summarization automatically.
...
- `FirstNexus status` (CLI)
- `FirstNexus sessions` / `sessions --json`
- Gateway logs (`pnpm gateway:watch` or `FirstNexus logs --follow`): `embedded run auto-compaction start` + `complete`
...
FirstNexus supports "silent" turns for background tasks where the user should not see intermediate output.
...
- FirstNexus strips/suppresses this in the delivery layer.
...
As of `2026.1.10`, FirstNexus also suppresses **draft/typing streaming** when a
...
FirstNexus uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but FirstNexus's
...
- Store vs transcript mismatch? Confirm the Gateway host and the store path from `FirstNexus status`.
