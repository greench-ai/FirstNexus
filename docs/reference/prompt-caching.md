...
Prompt caching means the model provider can reuse unchanged prompt prefixes (usually system/developer instructions and other stable context) across turns instead of re-processing them every time. FirstNexus normalizes provider usage into `cacheRead` and `cacheWrite` where the upstream API exposes those counters directly.
...
- With Anthropic API-key auth profiles, FirstNexus seeds `cacheRetention: "short"` for Anthropic model refs when unset.
- Anthropic native Messages responses expose both `cache_read_input_tokens` and `cache_creation_input_tokens`, so FirstNexus can show both `cacheRead` and `cacheWrite`.
...
- Prompt caching is automatic on supported recent models. FirstNexus does not need to inject block-level cache markers.
- FirstNexus uses `prompt_cache_key` to keep cache routing stable across turns and uses `prompt_cache_retention: "24h"` only when `cacheRetention: "long"` is selected on direct OpenAI hosts.
...
- OpenAI responses expose cached prompt tokens via `usage.prompt_tokens_details.cached_tokens` (or `input_tokens_details.cached_tokens` on Responses API events). FirstNexus maps that to `cacheRead`.
...
For `openrouter/anthropic/*` model refs, FirstNexus injects Anthropic
...
handles provider-side prompt caching automatically. FirstNexus does not inject
...
If you repoint the model at an arbitrary OpenAI-compatible proxy URL, FirstNexus
...
- Direct Gemini transport (`api: "google-generative-ai"`) reports cache hits
  through upstream `cachedContentTokenCount`; FirstNexus maps that to `cacheRead`.
- When `cacheRetention` is set on a direct Gemini model, FirstNexus automatically
...
- This is separate from Anthropic/OpenAI prompt-prefix caching. For Gemini,
  FirstNexus manages a provider-native `cachedContents` resource rather than
...
- Gemini CLI JSON output can also surface cache hits through `stats.cached`;
  FirstNexus maps that to `cacheRead`.
- If the CLI omits a direct `stats.input` value, FirstNexus derives input tokens
  from `stats.input_tokens - stats.cached`.
- This is usage normalization only. It does not mean FirstNexus is creating
  Anthropic/OpenAI-style prompt-cache markers for Gemini CLI.

## System-prompt cache boundary

FirstNexus splits the system prompt into a **stable prefix** and a **volatile
...
## FirstNexus cache-stability guards

FirstNexus also keeps several cache-sensitive payload shapes deterministic before
...
FirstNexus exposes dedicated cache-trace diagnostics for embedded agent runs.
...
FirstNexus keeps one combined live cache regression gate for repeated prefixes, tool turns, image turns, MCP-style tool transcripts, and an Anthropic no-cache control.
...
diagnostics:
  cacheTrace:
    enabled: true
    filePath: "~/.FirstNexus/logs/cache-trace.jsonl" # optional
...
- If you need request tracing, log request IDs and rate-limit headers separately from cache metrics. FirstNexus's current cache-trace output is focused on prompt/session shape and normalized token usage rather than raw provider response headers.
