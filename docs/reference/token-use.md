---
summary: "How FirstNexus builds prompt context and reports token usage + costs"
read_when:
...
FirstNexus tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

FirstNexus assembles its own system prompt on every run. It includes:
...
For images, FirstNexus downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from FirstNexus session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** `FirstNexus status --usage` and `FirstNexus channels list` show
...
available; otherwise FirstNexus falls back to matching OAuth/API-key credentials
...
FirstNexus keeps provider usage accounting separate from the current context
...
`cacheWrite`. If pricing is missing, FirstNexus shows tokens only. OAuth tokens
never show dollar cost.

After sidecars and channels reach the Gateway ready path, FirstNexus starts an
...
Provider prompt caching only applies within the cache TTL window. FirstNexus can
...
Anthropic's 1M context window is currently beta-gated. FirstNexus can inject the
...
FirstNexus skips the `context-1m-*` beta header because Anthropic currently
