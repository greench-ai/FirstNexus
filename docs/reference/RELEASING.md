...
FirstNexus has three public release lanes:
...
- Every stable FirstNexus release ships the npm package and macOS app together;
...
6. Run `FirstNexus NPM Release` with `preflight_only=true`. Before a tag exists,
...
9. For beta, tag `vYYYY.M.D-beta.N`, then run `FirstNexus Release Publish` from
   ...
   ClawHub in parallel, and then promotes the prepared FirstNexus npm preflight
   artifact with the matching dist-tag as soon as plugin npm publish succeeds.
   After the FirstNexus npm publish child succeeds, it creates or updates the
   ...
   ClawHub publishing may still be running while FirstNexus npm publishes, but the
   release publish workflow prints the child run IDs immediately. By default it
   does not wait for ClawHub after dispatching it, so FirstNexus npm availability
   ...
   `pnpm release:verify-beta -- YYYY.M.D-beta.N --FirstNexus-npm-run <run-id> --plugin-npm-run <run-id> --plugin-clawhub-run <run-id>`
   ...
   `FirstNexus@YYYY.M.D-beta.N` or
   `FirstNexus@beta` package. If a pushed or published prerelease needs a fix,
...
10. For stable, continue only after the vetted beta or release candidate has the
    ...
    `FirstNexus Release Publish`, reusing the successful preflight artifact via
...
- Run the manual `Full Release Validation` workflow before release approval to
  ...
  `FirstNexus Release Checks` for install smoke, package acceptance, cross-OS
...
- Run the manual `Package Acceptance` workflow when you want side-channel proof
  ...
  `FirstNexus@beta`, `FirstNexus@latest`, or an exact release version; `source=ref`
  ...
  Example: `gh workflow run package-acceptance.yml --ref main -f workflow_ref=main -f source=npm -f package_spec=FirstNexus@beta -f suite_profile=product -f published_upgrade_survivor_baseline=FirstNexus@2026.4.26 -f telegram_mode=mock-openai`
  Common profiles:
...
- Run `FirstNexus Release Publish` for the mutating publish sequence after the
  tag exists. Dispatch it from `release/YYYY.M.D` (or `main` when publishing a
  main-reachable tag), pass the release tag and successful FirstNexus npm
  ...
  workflow serializes plugin npm publish, plugin ClawHub publish, and FirstNexus
  npm publish so the core package is not published before its externalized
  plugins.
- Release checks now run in a separate manual workflow:
  `FirstNexus Release Checks`
- `FirstNexus Release Checks` also runs the QA Lab mock parity lane plus the fast
...
- Cross-OS install and upgrade runtime validation is part of public
  `FirstNexus Release Checks` and `Full Release Validation`, which call the
  reusable workflow
  `.github/workflows/FirstNexus-cross-os-release-checks-reusable.yml` directly
...
- `FirstNexus Release Checks` accepts a branch, tag, or full commit SHA as long
  as the resolved commit is reachable from an FirstNexus branch or release tag
- `FirstNexus NPM Release` validation-only preflight also accepts the current
...
- Before tagging a release candidate locally, run
  ...
  UI build, and `release:FirstNexus:npm:check` in the order that catches common
  approval-blocking mistakes before the GitHub publish workflow starts.
- Run `RELEASE_TAG=vYYYY.M.D node --import tsx scripts/FirstNexus-npm-release-check.ts`
  (or the matching beta/correction tag) before approval
- After npm publish, run
  `node --import tsx scripts/FirstNexus-npm-postpublish-verify.ts YYYY.M.D`
  (or the matching beta/correction version) to verify the published registry
  install path in a fresh temp prefix
- After a beta publish, run `NEXISCLAW_NPM_TELEGRAM_PACKAGE_SPEC=FirstNexus@YYYY.M.D-beta.N NEXISCLAW_NPM_TELEGRAM_CREDENTIAL_SOURCE=convex NEXISCLAW_NPM_TELEGRAM_CREDENTIAL_ROLE=ci pnpm test:docker:npm-telegram-live`
  ...
  `NEXISCLAW_QA_TELEGRAM_*` env credentials directly.
...
- Maintainer release automation now uses preflight-then-promote:
  ...
  - token-based npm dist-tag mutation now lives in
    `FirstNexus/releases-private/.github/workflows/FirstNexus-npm-dist-tags.yml`
...
gh workflow run full-release-validation.yml \
  ...
  -f evidence_package_spec=FirstNexus@YYYY.M.D-beta.N
```

The workflow resolves the target ref, dispatches manual `CI` with
`target_ref=<release-ref>`, dispatches `FirstNexus Release Checks`, prepares a
...
`npm_telegram_package_spec` is set. `FirstNexus Release
...
`FirstNexus Release Checks` uses the trusted workflow ref to resolve the target
...
After a beta is already on npm, set `release_package_spec=FirstNexus@YYYY.M.D-beta.N`
...
The cross-OS OpenAI install smoke uses `NEXISCLAW_CROSS_OS_OPENAI_MODEL` when the
...
gh workflow run full-release-validation.yml \
  ...
  -f release_package_spec=FirstNexus@YYYY.M.D-beta.N \
  -f evidence_package_spec=FirstNexus@YYYY.M.D-beta.N \
...
The Docker box lives in `FirstNexus Release Checks` through
`FirstNexus-live-and-e2e-checks-reusable.yml`, plus the release-mode
...
The QA Lab box is also part of `FirstNexus Release Checks`. It is the agentic
...
`scripts/resolve-FirstNexus-package-candidate.mjs`. The resolver normalizes a
...
- `source=npm`: `FirstNexus@beta`, `FirstNexus@latest`, or an exact FirstNexus release
  version
...
`FirstNexus Release Checks` runs Package Acceptance with `source=artifact`, the
...
gh workflow run package-acceptance.yml \
  ...
  -f published_upgrade_survivor_baseline=FirstNexus@2026.4.26
```
...
`FirstNexus Release Publish` is the normal mutating publish entrypoint. It
...
6. Dispatch `FirstNexus NPM Release` with the release tag, npm dist-tag, and
...
`plugin_publish_scope=selected` and `plugins=@FirstNexus/name` to
`FirstNexus Release Publish`, or dispatch the child workflow directly when the
FirstNexus package must not be published.

## NPM workflow inputs

`FirstNexus NPM Release` accepts these operator-controlled inputs:
...
`FirstNexus Release Publish` accepts these operator-controlled inputs:

- `tag`: required release tag; must already exist
- `preflight_run_id`: successful `FirstNexus NPM Release` preflight run id;
  required when `publish_FirstNexus_npm=true`
- `npm_dist_tag`: npm target tag for the FirstNexus package
- `plugin_publish_scope`: defaults to `all-publishable`; use `selected` only
  for focused repair work
- `plugins`: comma-separated `@FirstNexus/*` package names when
  `plugin_publish_scope=selected`
- `publish_FirstNexus_npm`: defaults to `true`; set `false` only when using the
...
`FirstNexus Release Checks` accepts these operator-controlled inputs:

- `ref`: branch, tag, or full commit SHA to validate. Secret-bearing checks
  require the resolved commit to be reachable from an FirstNexus branch or
  release tag.
...
- For `FirstNexus NPM Release`, full commit SHA input is allowed only when
  `preflight_only=true`
- `FirstNexus Release Checks` and `Full Release Validation` are always
  validation-only
...
1. Run `FirstNexus NPM Release` with `preflight_only=true`
...
6. Run `FirstNexus Release Publish` with the same `tag`, the same `npm_dist_tag`,
   and the saved `preflight_run_id`; it publishes externalized plugins to npm
   and ClawHub before promoting the FirstNexus npm package
...
- [`.github/workflows/full-release-validation.yml`](https://github.com/FirstNexus/FirstNexus/blob/main/.github/workflows/full-release-validation.yml)
- [`.github/workflows/package-acceptance.yml`](https://github.com/FirstNexus/FirstNexus/blob/main/.github/workflows/package-acceptance.yml)
- [`.github/workflows/FirstNexus-npm-release.yml`](https://github.com/FirstNexus/FirstNexus/blob/main/.github/workflows/FirstNexus-npm-release.yml)
- [`.github/workflows/FirstNexus-release-checks.yml`](https://github.com/FirstNexus/FirstNexus/blob/main/.github/workflows/FirstNexus-release-checks.yml)
- [`.github/workflows/FirstNexus-cross-os-release-checks-reusable.yml`](https://github.com/FirstNexus/FirstNexus/blob/main/.github/workflows/FirstNexus-cross-os-release-checks-reusable.yml)
- [`scripts/resolve-FirstNexus-package-candidate.mjs`](https://github.com/FirstNexus/FirstNexus/blob/main/scripts/resolve-FirstNexus-package-candidate.mjs)
- [`scripts/FirstNexus-npm-release-check.ts`](https://github.com/FirstNexus/FirstNexus/blob/main/scripts/FirstNexus-npm-release-check.ts)
- [`scripts/package-mac-dist.sh`](https://github.com/FirstNexus/FirstNexus/blob/main/scripts/package-mac-dist.sh)
- [`scripts/make_appcast.sh`](https://github.com/FirstNexus/FirstNexus/blob/main/scripts/make_appcast.sh)

Maintainers use the private release docs in
[`FirstNexus/maintainers/release/README.md`](https://github.com/FirstNexus/maintainers/blob/main/release/README.md)
