...
- `NEXISCLAW_DEBUG_MODEL_TRANSPORT=1`: emit request start, fetch response, SDK
  headers, first streaming event, stream completion, and transport errors at
  `info` level.
- `NEXISCLAW_DEBUG_MODEL_PAYLOAD=summary`: include a bounded request payload
  summary in model request logs.
- `NEXISCLAW_DEBUG_MODEL_PAYLOAD=tools`: include all model-facing tool names in
  the payload summary.
- `NEXISCLAW_DEBUG_MODEL_PAYLOAD=full-redacted`: include a redacted, capped JSON
  payload snapshot. Use only while debugging; secrets are redacted but prompts
  and message text may still be present.
- `NEXISCLAW_DEBUG_SSE=events`: emit first-event and stream-completion timing.
- `NEXISCLAW_DEBUG_SSE=peek`: also emit the first five redacted SSE event
  payloads, capped per event.
- `NEXISCLAW_DEBUG_CODE_MODE=1`: emit code-mode model-surface diagnostics,
...
- **Diagnostics flags** — targeted debug-log flags that route extra logs to
  ...
  or via the `NEXISCLAW_DIAGNOSTICS=...` env override. Full guide:
