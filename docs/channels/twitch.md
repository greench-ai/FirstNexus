...
read_when:
  - Setting up Twitch chat integration for FirstNexus
title: "Twitch"
sidebarTitle: "Twitch"
---

Twitch chat support via IRC connection. FirstNexus connects as a Twitch user (bot account) to receive and send messages in channels.

## Bundled plugin

<Note>
Twitch ships as a bundled plugin in current FirstNexus releases, so normal packaged builds do not need a separate install.
</Note>
...
<Tabs>
  <Tab title="npm registry">
    ```bash
    FirstNexus plugins install @FirstNexus/twitch
    ```
  </Tab>
  <Tab title="Local checkout">
    ```bash
    FirstNexus plugins install ./path/to/local/twitch-plugin
    ```
  </Tab>
</Tabs>
...
<Steps>
  ...
  <Step title="Configure the token">
    - Env: `NEXISCLAW_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "FirstNexus", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use NEXISCLAW_TWITCH_ACCESS_TOKEN env var)
...
<Tabs>
  <Tab title="Env var (default account only)">
    ```bash
    NEXISCLAW_TWITCH_ACCESS_TOKEN=oauth:abc123...
    ```
  </Tab>
  <Tab title="Config">
    ```json5
