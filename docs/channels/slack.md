...
| Request signing              | Not used (auth is the App-Level Token)                                               | Slack signs every request; FirstNexus verifies with `signingSecret`                                            |
...
```json Recommended
{
  "display_information": {
    "name": "FirstNexus",
    "description": "Slack connector for FirstNexus"
  },
  "features": {
    "bot_user": { "display_name": "FirstNexus", "always_online": true },
    "app_home": {
...
```
      ...
      <Step title="Configure FirstNexus">
...
JSON5
FirstNexus config patch --file ./slack.socket.patch.json5 --dry-run
FirstNexus config patch --file ./slack.socket.patch.json5
```
...
```
        ...
        <Info>
          The three URL fields (`slash_commands[].url`, `event_subscriptions.request_url`, and `interactivity.request_url` / `message_menu_options_url`) all point at the same FirstNexus endpoint. Slack's manifest schema requires them named separately, but FirstNexus routes by payload type so a single `webhookPath` (default `/slack/events`) is enough. Slash commands without `slash_commands[].url` will silently no-op in HTTP mode.
        </Info>
...
JSON5
FirstNexus config patch --file ./slack.http.patch.json5 --dry-run
FirstNexus config patch --file ./slack.http.patch.json5
```
...
FirstNexus sets the Slack SDK client pong timeout to 15 seconds by default for Socket Mode. Override the transport settings only when you need workspace- or host-specific tuning:
...
The default manifest enables the Slack App Home **Home** tab and subscribes to `app_home_opened`. When a workspace member opens the Home tab, FirstNexus publishes a safe default Home view with `views.publish`; no conversation payload or private configuration is included. The **Messages** tab remains enabled for Slack DMs.
...
<Tabs>
  <Tab title="DM policy">
    ...
    Legacy `channels.slack.dm.policy` and `channels.slack.dm.allowFrom` still read for compatibility. `FirstNexus doctor --fix` migrates them to `dmPolicy` and `allowFrom` when it can do so without changing access.

    Pairing in DMs uses `FirstNexus pairing approve slack <code>`.
...
- In channels where FirstNexus handles top-level messages without requiring an explicit mention, non-`off` `replyToMode` routes each handled root into `agent:<agentId>:slack:channel:<channelId>:thread:<rootTs>` so the visible Slack thread maps to one FirstNexus session from the first turn.
...
When a `message` tool call runs inside a Slack thread and targets the same channel, FirstNexus normally inherits the current Slack thread according to `replyToMode`. Set `topLevel: true` on `action: "send"` or `action: "upload-file"` to force a new parent-channel message instead. `threadId: null` is accepted as the same top-level opt-out.
...
- Top-level Slack DMs stay off-thread by default, so they do not show Slack's thread-style native stream/status preview; FirstNexus posts and edits a draft preview in the DM instead.
...
- If streaming fails mid-reply, FirstNexus falls back to normal delivery for remaining payloads.
...
- Run `FirstNexus doctor --fix` to rewrite persisted Slack streaming config to the canonical keys.

## Typing reaction fallback

`typingReaction` adds a temporary reaction to the inbound Slack message while FirstNexus is processing a reply, then removes it when the run finishes. This is most useful outside of thread replies, which use a default "is typing..." status indicator.
...
<AccordionGroup>
  <Accordion title="Inbound attachments">
    ...
    Downloads use bounded idle and total timeouts. If Slack file retrieval stalls or fails, FirstNexus keeps processing the message and falls back to the file placeholder.
...
- The interactive callback values are FirstNexus-generated opaque tokens, not raw agent-authored values.
- If generated interactive blocks would exceed Slack Block Kit limits, FirstNexus falls back to the original text reply instead of sending an invalid blocks payload.
...
```
  ...
  <Accordion title="Socket mode not connecting">
    ...
    If `FirstNexus channels status --probe --json` shows `botTokenStatus` or
...
1. FirstNexus downloads the file from Slack's private URL using the bot token (`xoxb-...`).
...
| Forwarded/shared attachments           | Text and Slack-hosted image/file media are best-effort                       | Re-share directly in the FirstNexus thread                                 |
...
- [PDF tool](/tools/pdf)
- Epic: [#51349](https://github.com/FirstNexus/FirstNexus/issues/51349) — Slack attachment vision enablement
- Regression tests: [#51353](https://github.com/FirstNexus/FirstNexus/issues/51353)
- Live verification: [#51354](https://github.com/FirstNexus/FirstNexus/issues/51354)
