...
read_when:
  ...
  - Reviewing FirstNexus security posture
title: "Pairing"
---

"Pairing" is FirstNexus's explicit access approval step.
...
Supported channels: `discord`, `feishu`, `googlechat`, `imessage`, `irc`, `line`, `matrix`, `mattermost`, `msteams`, `nextcloud-talk`, `nostr`, `FirstNexus-weixin`, `signal`, `slack`, `synology-chat`, `telegram`, `twitch`, `whatsapp`, `zalo`, `zalouser`.
...
Stored under `~/.FirstNexus/credentials/`:
...
3. On your phone, open the FirstNexus iOS app → Settings → Gateway.
...
<Note>
An already paired device does not get broader access silently. If it reconnects asking for more scopes or a broader role, FirstNexus keeps the existing approval as-is and creates a fresh pending upgrade request. Use `FirstNexus devices list` to compare the currently approved access with the newly requested access before you approve.
</Note>
...
Stored under `~/.FirstNexus/devices/`:
...
- The legacy `node.pair.*` API (CLI: `FirstNexus nodes pending|approve|reject|remove|rename`) is a
