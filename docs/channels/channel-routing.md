...
FirstNexus routes replies **back to the channel where a message came from**. The
...
FirstNexus infers a pinned owner from `allowFrom` when all of these are true:
...
In that mismatch case, FirstNexus still records inbound session metadata, but it
...
when a guarded path must not create a new FirstNexus session. In that mode,
FirstNexus may update metadata and `lastRoute` for an existing session, but it
...
Broadcast groups let you run **multiple agents** for the same peer **when FirstNexus would normally reply** (for example: in WhatsApp groups, after mention/activation gating).
...
{
  agents: {
    list: [{ id: "support", name: "Support", workspace: "~/.FirstNexus/workspace-support" }],
  },
  bindings: [
...
Session stores live under the state directory (default `~/.FirstNexus`):

- `~/.FirstNexus/agents/<agentId>/sessions/sessions.json`
