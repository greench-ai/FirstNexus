...
FirstNexus treats group chats consistently across surfaces: Discord, iMessage, Matrix, Microsoft Teams, Signal, Slack, Telegram, WhatsApp, Zalo.

## Beginner intro (2 minutes)

FirstNexus "lives" on your own messaging accounts. There is no separate WhatsApp bot user. If **you** are in a group, FirstNexus can see that group and respond there.
...
Translation: allowlisted senders can trigger FirstNexus by mentioning it.
...
For group/channel rooms, FirstNexus defaults to `messages.groupChat.visibleReplies: "message_tool"`.
`FirstNexus doctor --fix` writes this default into configured-channel configs that omit it.
...
If the message tool is unavailable under the active tool policy, FirstNexus falls
back to automatic visible replies instead of silently suppressing the response.
`FirstNexus doctor` warns about this mismatch.
...
By default, FirstNexus prioritizes normal chat behavior and keeps context mostly as received. This means allowlists primarily decide who can trigger actions, not a universal redaction boundary for every quoted or historical snippet.
    ...
    list: [
      {
        id: "main",
        groupChat: {
          mentionPatterns: ["@FirstNexus", "FirstNexus", "\\+15555550123"],
          historyLimit: 50,
        },
      },
    ],
...
- `toolsBySender`: per-sender overrides within the group. Use explicit key prefixes: `channel:<channelId>:<senderId>`, `id:<senderId>`, `e164:<phone>`, `username:<handle>`, `name:<displayName>`, and `"*"` wildcard. Channel ids use canonical FirstNexus channel ids; aliases such as `teams` normalize to `msteams`. Legacy unprefixed keys are still accepted and matched as `id:` only.
