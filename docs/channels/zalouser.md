...
read_when:
  - Setting up Zalo Personal for FirstNexus
  - Debugging Zalo Personal login or message flow
title: "Zalo personal"
---

Status: experimental. This integration automates a **personal Zalo account** via native `zca-js` inside FirstNexus.
...
Zalo Personal ships as a bundled plugin in current FirstNexus releases, so normal
packaged builds do not need a separate install.

If you are on an older build or a custom install that excludes Zalo Personal,
install the npm package directly:

- Install via CLI: `FirstNexus plugins install @FirstNexus/zalouser`
- Pinned version: `FirstNexus plugins install @FirstNexus/zalouser@2026.5.2`
- Or from a source checkout: `FirstNexus plugins install ./path/to/local/zalouser-plugin`
...
1. Ensure the Zalo Personal plugin is available.
   - Current packaged FirstNexus releases already bundle it.
   - Older/custom installs can add it manually with the commands above.
2. Login (QR, on the Gateway machine):
   - `FirstNexus channels login --channel zalouser`
...
```bash
FirstNexus directory self --channel zalouser
FirstNexus directory peers list --channel zalouser --query "name"
FirstNexus directory groups list --channel zalouser --query "work"
```
...
- `FirstNexus pairing list zalouser`
- `FirstNexus pairing approve zalouser <code>`
...
- On startup, FirstNexus resolves group/user names in allowlists to IDs and logs the mapping only when `channels.zalouser.dangerouslyAllowNameMatching: true` is enabled.
...
- When a group message is skipped because mention is required, FirstNexus stores it as pending group history and includes it on the next processed group message.
...
Accounts map to `zalouser` profiles in FirstNexus state. Example:
...
- FirstNexus sends a typing event before dispatching a reply (best-effort).
...
- For inbound messages that include event metadata, FirstNexus sends delivered + seen acknowledgements (best-effort).
...
- Re-login: `FirstNexus channels logout --channel zalouser && FirstNexus channels login --channel zalouser`
...
- The channel now runs fully in FirstNexus without external CLI binaries.
