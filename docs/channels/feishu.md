...
<Note>
Requires FirstNexus 2026.4.25 or above. Run `FirstNexus --version` to check. Upgrade with `FirstNexus update`.
</Note>

<Steps>
  <Step title="Run the channel setup wizard">
  ```bash
  FirstNexus channels login --channel feishu
  ```
...
```bash
FirstNexus pairing list feishu
FirstNexus pairing approve feishu <CODE>
```
...
5. Ensure the gateway is running: `FirstNexus gateway status`
6. Check logs: `FirstNexus logs --follow`

### QR setup does not react in the Feishu mobile app

1. Rerun setup: `FirstNexus channels login --channel feishu`
...
of raw `file_key` JSON. When `tools.media.audio` is configured, FirstNexus
...
conversion fails, FirstNexus falls back to a file attachment and logs the reason.
...
topic session key. If a native topic starter event omits `thread_id`, FirstNexus
hydrates it from Feishu before routing the turn. Normal group replies that
FirstNexus turns into threads keep using the reply root message ID (`om_*`) so the
