...
read_when:
  - You want to connect FirstNexus to LINE
  - You need LINE webhook + credential setup
  - You want LINE-specific message options
title: LINE
---

LINE connects to FirstNexus via the LINE Messaging API. The plugin runs as a webhook
...
```bash
FirstNexus plugins install @FirstNexus/line
```

Local checkout (when running from a git repo):

```bash
FirstNexus plugins install ./path/to/local/line-plugin
```
...
- LINE signature verification is body-dependent (HMAC over the raw body), so FirstNexus applies strict pre-auth body limits and timeout before verification.
- FirstNexus processes webhook events from the verified raw request bytes. Upstream middleware-transformed `req.body` values are ignored for signature-integrity safety.
...
```bash
FirstNexus pairing list line
FirstNexus pairing approve line <CODE>
```
...
- Inbound media is saved under `~/.FirstNexus/media/inbound/` before it is passed
...
Outbound media URLs must be public HTTPS URLs. FirstNexus validates the target hostname before handing the URL to LINE and rejects loopback, link-local, and private-network targets.
