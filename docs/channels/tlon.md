...
Tlon is a decentralized messenger built on Urbit. FirstNexus connects to your Urbit ship and can
...
Tlon ships as a bundled plugin in current FirstNexus releases, so normal packaged
...
```bash
FirstNexus plugins install @FirstNexus/tlon
```
...
```bash
FirstNexus plugins install ./path/to/local/tlon-plugin
```
...
By default, FirstNexus blocks private/internal hostnames and IP ranges for SSRF protection.
...
Use these with `FirstNexus message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, FirstNexus replies in-thread.
