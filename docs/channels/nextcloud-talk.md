...
Nextcloud Talk ships as a bundled plugin in current FirstNexus releases, so
...
```bash
FirstNexus plugins install @FirstNexus/nextcloud-talk
```
...
```bash
FirstNexus plugins install ./path/to/local/nextcloud-talk-plugin
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "FirstNexus" "<shared-secret>" "<webhook-url>" --feature webhook --feature response --feature reaction
   ```
...
4. Configure FirstNexus:
   ...
   ```bash
   FirstNexus channels add --channel nextcloud-talk \
...
- Approve via:
  - `FirstNexus pairing list nextcloud-talk`
  - `FirstNexus pairing approve nextcloud-talk <CODE>`
