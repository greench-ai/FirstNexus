...
This hub links the core docs for how FirstNexus connects, pairs, and secures
devices across localhost, LAN, and tailnet.

## Core model

Most operations flow through the Gateway (`FirstNexus gateway`), a single long-running process that owns channel connections and the WebSocket control plane.
...
- **Canvas host** is served on the same port as the Gateway (`/__FirstNexus__/canvas/`, `/__FirstNexus__/a2ui/`), protected by Gateway auth when bound beyond loopback.
