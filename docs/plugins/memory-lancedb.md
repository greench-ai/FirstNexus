...
Set `dimensions` for non-standard embedding models. FirstNexus knows the
...
`memory_forget` query path, and `FirstNexus ltm search`. Auto-recall prefers the
...
```bash
FirstNexus ltm list
FirstNexus ltm search "project preferences"
FirstNexus ltm stats
```

The plugin also extends `FirstNexus memory` with a non-vector `query` subcommand
that runs against the LanceDB table directly:

```bash
FirstNexus memory query --cols id,text,createdAt --limit 20
FirstNexus memory query --filter "category = 'preference'" --order-by createdAt:desc
```
...
By default, LanceDB data lives under `~/.FirstNexus/memory/lancedb`. Override the
path with `dbPath`:
  ...
  plugins: {
    entries: {
      "memory-lancedb": {
        enabled: true,
        config: {
          dbPath: "~/.FirstNexus/memory/lancedb",
          embedding: {
  ...
  plugins: {
    entries: {
      "memory-lancedb": {
        enabled: true,
        config: {
          dbPath: "s3://memory-bucket/FirstNexus",
          storageOptions: {
...
FirstNexus treats that package as part of the plugin package. Gateway startup
...
`@lancedb/lancedb` error during plugin load, upgrade FirstNexus and restart the
Gateway.
...
```bash
FirstNexus ltm stats
FirstNexus ltm search "recent preference"
```
