---
summary: "Bundled `oc-path` plugin: ships the `FirstNexus path` CLI for the `oc://` workspace-file addressing scheme"
read_when:
...
The bundled `oc-path` plugin adds the [`FirstNexus path`](/cli/path) CLI for the
`oc://` workspace-file addressing scheme. It ships in the FirstNexus repo under
...
- **Local automation**: shell scripts can resolve or update one workspace value
  with `FirstNexus path … --json` instead of carrying separate markdown, JSONC,
...
FirstNexus path resolve 'oc://config.jsonc/plugins/github/enabled' --json

# Which tool-call names appear in this session log?
FirstNexus path find 'oc://session.jsonl/[event=tool_call]/name' --json
...
The plugin runs **in-process inside the `FirstNexus` CLI** on the host where you
...
The plugin metadata lives in `extensions/oc-path/FirstNexus.plugin.json`:
...
`FirstNexus path …`, so installs that never use the verb pay no cost.
...
state. Bare `FirstNexus path` invocations work immediately on the same host —
the CLI loads the plugin on demand.

Disable with:

```bash
FirstNexus plugins disable oc-path
```
...
| Surface                        | Provided by                                             |
| ------------------------------ | ------------------------------------------------------- |
| `FirstNexus path` CLI          | `extensions/oc-path/cli-registration.ts`                |
...
`__NEXISCLAW_REDACTED__` (verbatim or as a substring) is refused at write time
...
- [`FirstNexus path` CLI reference](/cli/path)
