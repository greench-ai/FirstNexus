...
At startup, FirstNexus does roughly this:
...
The manifest is the control-plane source of truth. FirstNexus uses it to:
...
channels, slots, and auto-enable rules. If that derived set is empty, FirstNexus
...
FirstNexus does not cache plugin discovery results or direct manifest registry
...
FirstNexus still owns the generic agent loop, failover, transcript handling, and
...
For model/provider plugins, FirstNexus calls hooks in this rough order.
...
| --  | _(built-in model lookup)_         | FirstNexus tries the normal registry/catalog path first                                                        | _(not a plugin hook)_                                                                                                                         |
...
that still runs on FirstNexus's normal inference loop.
...
<AccordionGroup>
  <Accordion title="Pass-through catalog providers">
    ...
    model ids ahead of FirstNexus's static catalog.
  </Accordion>
...
- The preferred ownership model is company-oriented: one vendor plugin can own
  text, speech, image, and future media providers as FirstNexus adds those
...
- `extractStructuredWithModel(...)` is the plugin-facing seam for bounded
  ...
  product plugins own their routes and schemas while FirstNexus owns the
...
- FirstNexus only honors those override fields for trusted callers.
...
const result = await api.runtime.webSearch.search({
  config: api.config,
  args: {
    query: "FirstNexus plugin runtime helpers",
    count: 5,
  },
});
```
...
- `auth: "gateway"` routes run inside a Gateway request runtime scope, but that scope is intentionally conservative:
  - shared-secret bearer auth (`gateway.auth.mode = "token"` / `"password"`) keeps plugin-route runtime scopes pinned to `operator.write`, even if the caller sends `x-FirstNexus-scopes`
  - trusted identity-bearing HTTP modes (for example `trusted-proxy` or `gateway.auth.mode = "none"` on a private ingress) honor `x-FirstNexus-scopes` only when the header is explicitly present
  - if `x-FirstNexus-scopes` is absent on those identity-bearing plugin-route requests, runtime scope falls back to `operator.write`
- Practical rule: do not assume a gateway-auth plugin route is an implicit admin surface. If your route needs admin-only behavior, require an identity-bearing auth mode and document the explicit `x-FirstNexus-scopes` header contract.

## Plugin SDK import paths

Use narrow SDK subpaths instead of the monolithic `FirstNexus/plugin-sdk` root
barrel when authoring new plugins. Core subpaths:

| Subpath                               | Purpose                                                |
| ------------------------------------- | ------------------------------------------------------ |
| `FirstNexus/plugin-sdk/plugin-entry`  | Plugin registration primitives                         |
| `FirstNexus/plugin-sdk/channel-core`  | Channel entry/build helpers                            |
| `FirstNexus/plugin-sdk/core`          | Generic shared helpers and umbrella contract           |
| `FirstNexus/plugin-sdk/config-schema` | Root `FirstNexus.json` Zod schema (`FirstNexusSchema`) |
...
<Info>
`FirstNexus/plugin-sdk/channel-runtime`, `FirstNexus/plugin-sdk/config-runtime`,
and `FirstNexus/plugin-sdk/infra-runtime` are deprecated compatibility shims for
...
External plugins should only import `FirstNexus/plugin-sdk/*` subpaths. Never
...
`FirstNexus/plugin-sdk/directory-runtime`.
...
`catalog.run(...)` returns the same shape FirstNexus writes into
`models.providers`:
...
`catalog.order` controls when a plugin's catalog merges relative to FirstNexus's
...
- if both `catalog` and `discovery` are registered, FirstNexus uses `catalog`
...
- Read-only command paths such as `FirstNexus status`, `FirstNexus status --all`,
  `FirstNexus channels status`, `FirstNexus channels resolve`, and doctor/config
...
A plugin directory may include a `package.json` with `FirstNexus.extensions`:
...
Security guardrail: every `FirstNexus.extensions` entry must stay inside the plugin
directory after symlink resolution. Entries that escape the package directory are
rejected.

Security note: `FirstNexus plugins install` installs plugin dependencies with a
...
Optional: `FirstNexus.setupEntry` can point at a lightweight setup-only module.
When FirstNexus needs setup surfaces for a disabled channel plugin, or
...
Optional: `FirstNexus.startup.deferConfiguredChannelFullLoadUntilAfterListen`
...
this flag. Keep the plugin on the default behavior and let FirstNexus load the
...
Channel plugins can advertise setup/discovery metadata via `FirstNexus.channel` and
install hints via `FirstNexus.install`. This keeps the core catalog data-free.

Example:

```json
{
  "name": "@FirstNexus/nextcloud-talk",
  "FirstNexus": {
    "extensions": ["./index.ts"],
    "channel": {
      "id": "nextcloud-talk",
    ...
    "install": {
      "npmSpec": "@FirstNexus/nextcloud-talk",
...
Useful `FirstNexus.channel` fields beyond the minimal example:
...
FirstNexus can also merge **external channel catalogs** (for example, an MPM
registry export). Drop a JSON file at one of:

- `~/.FirstNexus/mpm/plugins.json`
- `~/.FirstNexus/mpm/catalog.json`
- `~/.FirstNexus/plugins/catalog.json`

Or point `NEXISCLAW_PLUGIN_CATALOG_PATHS` (or `NEXISCLAW_MPM_CATALOG_PATHS`) at
one or more JSON files (comma/semicolon/`PATH`-delimited). Each file should
contain `{ "entries": [ { "name": "@scope/pkg", "FirstNexus": { "channel": {...}, "install": {...} } } ] }`. The parser also accepts `"packages"` or `"plugins"` as legacy aliases for the `"entries"` key.

Generated channel catalog entries and provider install catalog entries expose
normalized install-source facts next to the raw `FirstNexus.install` block. The
...
```ts
import { buildMemorySystemPromptAddition } from "FirstNexus/plugin-sdk/core";
...
2. add typed plugin registration/runtime surfaces
   Extend `FirstNexusPluginApi` and/or `api.runtime` with the smallest useful
...
This is how FirstNexus stays opinionated without becoming hardcoded to one
