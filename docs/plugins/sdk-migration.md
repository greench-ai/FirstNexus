...
read_when:
  - You see the NEXISCLAW_PLUGIN_SDK_COMPAT_DEPRECATED warning
  - You see the NEXISCLAW_EXTENSION_API_DEPRECATED warning
  - You used api.registerEmbeddedExtensionFactory before FirstNexus 2026.4.25
  - You are updating a plugin to the modern plugin architecture
  - You maintain an external FirstNexus plugin
---

FirstNexus has moved from a broad backwards-compatibility layer to a modern plugin
...
- **`FirstNexus/plugin-sdk/compat`** - a single import that re-exported dozens of
  helpers. It was introduced to keep older hook-based plugins working while the
  new plugin architecture was being built.
- **`FirstNexus/plugin-sdk/infra-runtime`** - a broad runtime helper barrel that
...
- **`FirstNexus/plugin-sdk/config-runtime`** - a broad config compatibility barrel
  that still carries deprecated direct load/write helpers during the migration
  window.
- **`FirstNexus/extension-api`** - a bridge that gave plugins direct access to
...
FirstNexus does not remove or reinterpret documented plugin behavior in the same
...
The modern plugin SDK fixes this: each import path (`FirstNexus/plugin-sdk/\<subpath\>`)
...
`FirstNexus/plugin-sdk/realtime-voice`. The new controller owns the common Talk
...
`talk.providers` should be repaired with `FirstNexus doctor --fix`; runtime Talk
...
<Steps>
  <Step title="Migrate runtime config load/write helpers">
    ...
    `FirstNexus/plugin-sdk/config-runtime` compatibility barrel. Use the narrow
    SDK subpath that matches the job:

    | Need | Import |
    | --- | --- |
    | Config types such as `FirstNexusConfig` | `FirstNexus/plugin-sdk/config-contracts` |
    | Already-loaded config assertions and plugin-entry config lookup | `FirstNexus/plugin-sdk/plugin-config-runtime` |
    | Current runtime snapshot reads | `FirstNexus/plugin-sdk/runtime-config-snapshot` |
    | Config writes | `FirstNexus/plugin-sdk/config-mutation` |
    | Session store helpers | `FirstNexus/plugin-sdk/session-store-runtime` |
    | Markdown table config | `FirstNexus/plugin-sdk/markdown-table-runtime` |
    | Group policy runtime helpers | `FirstNexus/plugin-sdk/runtime-group-policy` |
    | Secret input resolution | `FirstNexus/plugin-sdk/secret-input-runtime` |
    | Model/session overrides | `FirstNexus/plugin-sdk/model-session-runtime` |
  ...
  <Step title="Migrate approval-native handlers to capability facts">
    ...
    - Register channel-owned runtime objects such as clients, tokens, or Bolt
      apps through `FirstNexus/plugin-sdk/channel-runtime-context`
  ...
  <Step title="Audit Windows wrapper fallback behavior">
    If your plugin uses `FirstNexus/plugin-sdk/windows-spawn`, unresolved Windows
  ...
  <Step title="Find deprecated imports">
    ...
    grep -r "FirstNexus/extension-api" my-plugin/
    ```
  ...
  <Step title="Replace with focused imports">
    ...
    } from "FirstNexus/plugin-sdk/compat";

    // After (modern focused imports)
    import { createChannelReplyPipeline } from "FirstNexus/plugin-sdk/channel-reply-pipeline";
    import { createPluginRuntimeStore } from "FirstNexus/plugin-sdk/runtime-store";
    import { resolveControlCommandGate } from "FirstNexus/plugin-sdk/command-auth";
    ```
    ...
    import { runEmbeddedPiAgent } from "FirstNexus/extension-api";
  ...
  <Step title="Replace broad infra-runtime imports">
    `FirstNexus/plugin-sdk/infra-runtime` still exists for external
    compatibility, but new code should import the focused helper surface it
    actually needs:

    | Need | Import |
    | --- | --- |
    | System event queue helpers | `FirstNexus/plugin-sdk/system-event-runtime` |
    | Heartbeat wake, event, and visibility helpers | `FirstNexus/plugin-sdk/heartbeat-runtime` |
    | Pending delivery queue drain | `FirstNexus/plugin-sdk/delivery-queue-runtime` |
    | Channel activity telemetry | `FirstNexus/plugin-sdk/channel-activity-runtime` |
    | In-memory dedupe caches | `FirstNexus/plugin-sdk/dedupe-runtime` |
    | Safe local-file/media path helpers | `FirstNexus/plugin-sdk/file-access-runtime` |
    | Dispatcher-aware fetch | `FirstNexus/plugin-sdk/runtime-fetch` |
    | Proxy and guarded fetch helpers | `FirstNexus/plugin-sdk/fetch-runtime` |
    | SSRF dispatcher policy types | `FirstNexus/plugin-sdk/ssrf-dispatcher` |
    | Approval request/resolution types | `FirstNexus/plugin-sdk/approval-runtime` |
    | Approval reply payload and command helpers | `FirstNexus/plugin-sdk/approval-reply-runtime` |
    | Error formatting helpers | `FirstNexus/plugin-sdk/error-runtime` |
    | Transport readiness waits | `FirstNexus/plugin-sdk/transport-ready-runtime` |
    | Secure token helpers | `FirstNexus/plugin-sdk/secure-random-runtime` |
    | Bounded async task concurrency | `FirstNexus/plugin-sdk/concurrency-runtime` |
    | Numeric coercion | `FirstNexus/plugin-sdk/number-runtime` |
    | Process-local async lock | `FirstNexus/plugin-sdk/async-lock-runtime` |
    | File locks | `FirstNexus/plugin-sdk/file-lock` |
  ...
  <Step title="Migrate channel route helpers">
    New channel route code should use `FirstNexus/plugin-sdk/channel-route`.
...
<Accordion title="Common import path table">
  ...
  | `plugin-sdk/config-schema` | Root config schema export | `FirstNexusSchema` |
  ...
  | `plugin-sdk/bundled-channel-config-schema` | Bundled config schemas | FirstNexus-maintained bundled plugins only; new plugins must define plugin-local schemas |
...
`@FirstNexus/discord@2026.3.13` package. Owner-specific helpers live inside the
...
<AccordionGroup>
  <Accordion title="command-auth help builders → command-status">
    **Old (`FirstNexus/plugin-sdk/command-auth`)**: `buildCommandsMessage`,
    `buildCommandsMessagePaginated`, `buildHelpMessage`.

    **New (`FirstNexus/plugin-sdk/command-status`)**: same signatures, same
    exports - just imported from the narrower subpath. `command-auth`
    re-exports them as compat stubs.

    ```typescript
    // Before
    import { buildHelpMessage } from "FirstNexus/plugin-sdk/command-auth";

    // After
    import { buildHelpMessage } from "FirstNexus/plugin-sdk/command-status";
    ```
  ...
  <Accordion title="Mention gating helpers → resolveInboundMentionDecision">
    **Old**: `resolveInboundMentionRequirement({ facts, policy })` and
    ...
    `FirstNexus/plugin-sdk/channel-inbound` or
    `FirstNexus/plugin-sdk/channel-mention-gating`.
  ...
  <Accordion title="Channel runtime shim and channel actions helpers">
    `FirstNexus/plugin-sdk/channel-runtime` is a compatibility shim for older
    channel plugins. Do not import it from new code; use
    `FirstNexus/plugin-sdk/channel-runtime-context` for registering runtime
    objects.

    `channelActions*` helpers in `FirstNexus/plugin-sdk/channel-actions` are
  ...
  <Accordion title="Web search provider tool() helper → createTool() on the plugin">
    **Old**: `tool()` factory from `FirstNexus/plugin-sdk/provider-web-search`.

    **New**: implement `createTool(...)` directly on the provider plugin.
    FirstNexus no longer needs the SDK helper to register the tool wrapper.
  ...
  <Accordion title="Thinking policy hooks → resolveThinkingProfile">
    ...
    **New**: a single `resolveThinkingProfile(ctx)` that returns a
    ...
    ranked level list. FirstNexus downgrades stale stored values by profile
    rank automatically.
  ...
  <Accordion title="FirstNexusSchemaType alias → FirstNexusConfig">
    `FirstNexusSchemaType` re-exported from `FirstNexus/plugin-sdk` is now a
    one-line alias for `FirstNexusConfig`. Prefer the canonical name.

    ```typescript
    // Before
    import type { FirstNexusSchemaType } from "FirstNexus/plugin-sdk";
    // After
    import type { FirstNexusConfig } from "FirstNexus/plugin-sdk/config-schema";
    ```
...
```bash
NEXISCLAW_SUPPRESS_PLUGIN_SDK_COMPAT_WARNING=1 FirstNexus gateway run
NEXISCLAW_SUPPRESS_EXTENSION_API_WARNING=1 FirstNexus gateway run
```
