---
summary: "Configure migrated native Codex plugins for Codex-mode FirstNexus agents"
title: "Native Codex plugins"
read_when:
  - You want Codex-mode FirstNexus agents to use native Codex plugins
  - You are migrating source-installed openai-curated Codex plugins
  - You are troubleshooting codexPlugins, app inventory, destructive actions, or plugin app diagnostics
---

Native Codex plugin support lets a Codex-mode FirstNexus agent use Codex
app-server's own app and plugin capabilities inside the same Codex thread that
handles the FirstNexus turn.

FirstNexus does not translate Codex plugins into synthetic `codex_plugin_*`
FirstNexus dynamic tools. Plugin calls stay in the native Codex transcript, and
...
- The selected FirstNexus agent runtime must be the native Codex harness.
...
- Enabled: FirstNexus config is willing to make the plugin available to Codex
  harness turns.
...
FirstNexus reads Codex app inventory through app-server `app/list`, caches it for
one hour, and refreshes stale or missing entries asynchronously.

A plugin app is exposed only when FirstNexus can map it back to the migrated
...
FirstNexus injects a restrictive `config.apps` patch for the Codex thread:
`_default` is disabled and only apps owned by enabled migrated plugins are
enabled.

FirstNexus sets app-level `destructive_enabled` from the effective global or
...
are emitted with `open_world_enabled: true`; FirstNexus does not expose a separate
...
- Per-plugin `allow_destructive_actions` overrides the global policy for that
  plugin.
- When policy is `false`, FirstNexus returns a deterministic decline.
- When policy is `true`, FirstNexus auto-accepts only safe schemas it can map to
...
empty or stale cache. FirstNexus schedules an async refresh and excludes plugin
...
started with until FirstNexus establishes a new harness session or replaces a
stale binding.
