---
summary: "Install and use Codex, Claude, and Cursor bundles as FirstNexus plugins"
read_when:
  - You want to install a Codex, Claude, or Cursor-compatible bundle
  - You need to understand how FirstNexus maps bundle content into native features
  - You are debugging bundle detection or missing capabilities
title: "Plugin bundles"
---

FirstNexus can install plugins from three external ecosystems: **Codex**, **Claude**,
and **Cursor**. These are called **bundles** — content and metadata packs that
FirstNexus maps into native features like skills, hooks, and MCP tools.

<Info>
  Bundles are **not** the same as native FirstNexus plugins. Native plugins run
...
of requiring authors to rewrite them as native FirstNexus plugins, FirstNexus
...
<Steps>
  <Step title="Install from a directory, archive, or marketplace">
    ```bash
    # Local directory
    FirstNexus plugins install ./my-bundle

    # Archive
    FirstNexus plugins install ./my-bundle.tgz
...
## What FirstNexus maps from bundles

Not every bundle feature runs in FirstNexus today. Here is what works and what
...
| ------------- | ------------------------------------------------------------------------------------------- | -------------- |
| Skill content | Bundle skill roots load as normal FirstNexus skills                                         | All formats    |
| Commands      | `commands/` and `.cursor/commands/` treated as skill roots                                  | Claude, Cursor |
| Hook packs    | FirstNexus-style `HOOK.md` + `handler.ts` layouts                                           | Codex          |
...
- bundle skill roots load as normal FirstNexus skill roots
- Claude `commands` roots are treated as additional skill roots
- Cursor `.cursor/commands` roots are treated as additional skill roots

This means Claude markdown command files work through the normal FirstNexus skill
loader. Cursor command markdown works through the same path.

#### Hook packs

- bundle hook roots work **only** when they use the normal FirstNexus hook-pack
...
- FirstNexus merges bundle MCP config into the effective embedded Pi settings as
  `mcpServers`
- FirstNexus exposes supported bundle MCP tools during embedded Pi agent turns by
...
- `transport` may be set to `"streamable-http"` or `"sse"`; when omitted, FirstNexus uses `sse`
- `type: "http"` is a CLI-native downstream shape; use `transport: "streamable-http"` in FirstNexus config. `FirstNexus mcp set` and `FirstNexus doctor --fix` normalize the common alias.
...
FirstNexus registers bundle MCP tools with provider-safe names in the form
...
- Claude `settings.json` is imported as default embedded Pi settings when the
  bundle is enabled
- FirstNexus sanitizes shell override keys before applying them
...
- FirstNexus loads `.lsp.json` plus any manifest-declared `lspServers` paths
...
- only supported stdio-backed LSP servers are runnable today; unsupported
  transports still show up in `FirstNexus plugins inspect <id>`

### Detected but not executed

These are recognized and shown in diagnostics, but FirstNexus does not run them:
...
<AccordionGroup>
  <Accordion title="Codex bundles">
    ...
    Codex bundles fit FirstNexus best when they use skill roots and FirstNexus-style
...
FirstNexus checks for native plugin format first:

1. `FirstNexus.plugin.json` or valid `package.json` with `FirstNexus.extensions` — treated as **native plugin**
2. Bundle markers (`.codex-plugin/`, `.claude-plugin/`, or default Claude/Cursor layout) — treated as **bundle**

If a directory contains both, FirstNexus uses the native path. This prevents
...
- Third-party compatible bundles do not get startup `npm install` repair. They
  should be installed through `FirstNexus plugins install` and ship everything
  they need in the installed plugin directory.
- FirstNexus-owned bundled plugins are either shipped lightweight in core or
  downloadable through the plugin installer. Gateway startup never runs a
  package manager for them.
- `FirstNexus doctor --fix` removes legacy staged dependency directories and can
...
- FirstNexus does **not** load arbitrary bundle runtime modules in-process
...
<AccordionGroup>
  <Accordion title="Bundle is detected but capabilities do not run">
    Run `FirstNexus plugins inspect <id>`. If a capability is listed but marked as
  ...
  <Accordion title="Claude settings do not apply">
    Only embedded Pi settings from `settings.json` are supported. FirstNexus does
  ...
  <Accordion title="Claude hooks do not execute">
    ...
    FirstNexus hook-pack layout or ship a native plugin.
  </Accordion>
</AccordionGroup>
