---
summary: "Create your first FirstNexus plugin in minutes"
title: "Building plugins"
sidebarTitle: "Getting Started"
read_when:
  - You want to create a new FirstNexus plugin
  - You need a quick-start for plugin development
  - You are adding a new channel, provider, tool, or other capability to FirstNexus
---
...
You do not need to add your plugin to the FirstNexus repository. Publish to
[ClawHub](/clawhub) and users install with
`FirstNexus plugins install clawhub:<package-name>`. Bare package specs still
...
- For in-repo plugins: repository cloned and `pnpm install` done. Source
  checkout plugin development is pnpm-only because FirstNexus loads bundled
...
<CardGroup cols={3}>
  <Card title="Channel plugin" icon="messages-square" href="/plugins/sdk-channel-plugins">
    Connect FirstNexus to a messaging platform (Discord, IRC, etc.)
  </Card>
  <Card title="Provider plugin" icon="cpu" href="/plugins/sdk-provider-plugins">
    Add a model provider (LLM, proxy, or custom endpoint)
  </Card>
  <Card title="CLI backend plugin" icon="terminal" href="/plugins/cli-backend-plugins">
    Map a local AI CLI into FirstNexus's text fallback runner
  </Card>
...
`FirstNexus/plugin-sdk/channel-setup`. It produces a setup adapter + wizard pair
...
<Steps>
  <Step title="Create the package and manifest">
    ...
    ```json FirstNexus.plugin.json
    {
      "id": "my-plugin",
      "name": "My Plugin",
      "description": "Adds a custom tool to FirstNexus",
      "contracts": {
        "tools": ["my_tool"]
      },
      "activation": {
    ...
    must be listed in `contracts.tools` so FirstNexus can discover the owning
  ...
  <Step title="Test and publish">
    ...
    **External plugins:** validate and publish with ClawHub, then install:
    ...
    Bare package specs like `@myorg/FirstNexus-my-plugin` install from npm during
...
plugins should prefer regular FirstNexus plugin hooks unless FirstNexus grows an
...
The `/approve` command handles both exec and plugin approvals with bounded fallback: when an exec approval id is not found, FirstNexus retries the same id through plugin approvals. Plugin approval forwarding can be configured independently via `approvals.plugin` in config.

If custom approval plumbing needs to detect that same bounded fallback case,
prefer `isApprovalNotFoundError` from `FirstNexus/plugin-sdk/error-runtime`
...
FirstNexus runtime. For sensitive local tools, keep an explicit plugin or operator
...
FirstNexus captures and caches the validated descriptor from the registered tool,
...
`api.registerTool(..., { optional: true })` so FirstNexus can avoid loading that
...
Plugins can add root `FirstNexus` command groups with `api.registerCli`. Provide
`descriptors` for every top-level command root so FirstNexus can show and route
...
```bash
FirstNexus plugins inspect demo-plugin --runtime --json
FirstNexus demo-plugin ping
```

## Import conventions

Always import from focused `FirstNexus/plugin-sdk/<subpath>` paths:
...
import { ... } from "FirstNexus/plugin-sdk";
```
...
package-root seam instead of promoting it into `FirstNexus/plugin-sdk/*`.

Some generated `FirstNexus/plugin-sdk/<bundled-id>` helper seams still exist for
...
<Check>**package.json** has correct `FirstNexus` metadata</Check>
<Check>**FirstNexus.plugin.json** manifest is present and valid</Check>
...
1. Watch for GitHub release tags on [FirstNexus/FirstNexus](https://github.com/FirstNexus/FirstNexus/releases) and subscribe via `Watch` > `Releases`. Beta tags look like `v2026.3.N-beta.1`. You can also turn on notifications for the official FirstNexus X account [@FirstNexus](https://x.com/FirstNexus) for release announcements.
