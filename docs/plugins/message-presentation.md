...
Message presentation is FirstNexus's shared contract for rich outbound chat UI.
...
```bash
FirstNexus message send --channel slack \
  --target channel:C123 \
...
```bash
FirstNexus message send --channel telegram \
...
Use helpers from `FirstNexus/plugin-sdk/interactive-runtime` when bridging older
code:
