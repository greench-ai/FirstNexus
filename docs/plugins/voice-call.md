...
read_when:
  - You want to place an outbound voice call from FirstNexus
...
Voice calls for FirstNexus via a plugin. Supports outbound notifications,
...
<Steps>
  <Step title="Install the plugin">
    <Tabs>
      <Tab title="From npm">
        ```bash
        FirstNexus plugins install @FirstNexus/voice-call
        ```
      </Tab>
      <Tab title="From a local folder (dev)">
        ```bash
        PLUGIN_SRC=./path/to/local/voice-call-plugin
        FirstNexus plugins install "$PLUGIN_SRC"
...
<AccordionGroup>
  ...
  <Accordion title="Legacy config migrations">
    ...
    `streaming.*` OpenAI keys are rewritten by `FirstNexus doctor --fix`.
    Runtime fallback still accepts the old voice-call keys for now, but
    the rewrite path is `FirstNexus doctor --fix` and the compat shim is
    temporary.
...
- Voice Call exposes the shared `FirstNexus_agent_consult` realtime tool by default. The realtime model can call it when the caller asks for deeper reasoning, current information, or normal FirstNexus tools.
- `realtime.consultPolicy` optionally adds guidance for when the realtime model should call `FirstNexus_agent_consult`.
...
configured FirstNexus agent without paying a full agent-consult round trip on
...
`FirstNexus_agent_consult` still run the full FirstNexus agent and should be used
          ...
          "voice-call": {
            config: {
              ...
              realtime: {
                enabled: true,
                provider: "google",
                instructions: "Speak briefly. Call FirstNexus_agent_consult before using deeper tools.",
                toolPolicy: "safe-read-only",
                consultPolicy: "substantive",
...
- Legacy `tts.<provider>` keys inside plugin config (`openai`, `elevenlabs`, `microsoft`, `edge`) are repaired by `FirstNexus doctor --fix`; committed config should use `tts.providers.<provider>`.
...
```bash
FirstNexus voicecall call --to "+15555550123" --message "Hello from FirstNexus"
FirstNexus voicecall start --to "+15555550123"   # alias for call
FirstNexus voicecall continue --call-id <id> --message "Any questions?"
FirstNexus voicecall speak --call-id <id> --message "One moment"
FirstNexus voicecall dtmf --call-id <id> --digits "ww123456#"
FirstNexus voicecall end --call-id <id>
FirstNexus voicecall status --call-id <id>
FirstNexus voicecall tail
FirstNexus voicecall latency                      # summarize turn latency from logs
FirstNexus voicecall expose --mode funnel
```
...
```bash
FirstNexus voicecall setup
FirstNexus voicecall setup --json
```
...
Provider signatures are checked against the public URL FirstNexus reconstructs
...
Use `FirstNexus logs --follow` for the live phase trace. A healthy Twilio Meet
...
`FirstNexus voicecall tail` still shows persisted call records; it is useful for
...
- `FirstNexus logs --follow` shows realtime TwiML served, the realtime bridge
