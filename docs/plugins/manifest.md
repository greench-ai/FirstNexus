...
read_when:
  - You are building an FirstNexus plugin
  - You need to ship a plugin config schema or debug plugin validation errors
title: "Plugin manifest"
---

This page is for the **native FirstNexus plugin manifest** only.
...
FirstNexus auto-detects those bundle layouts too, but they are not validated
against the `FirstNexus.plugin.json` schema described here.

For compatible bundles, FirstNexus currently reads bundle metadata plus declared
...
FirstNexus runtime expectations.

Every native FirstNexus plugin **must** ship a `FirstNexus.plugin.json` file in the
**plugin root**. FirstNexus uses this manifest to validate configuration
...
`FirstNexus.plugin.json` is the metadata FirstNexus reads **before it loads your
...
- QA runner metadata the shared `FirstNexus qa` host can inspect
...
| `providerAuthEnvVars`                | No       | `Record<string, string[]>`       | Deprecated compatibility env metadata for provider auth/status lookup. Prefer `setup.providers[].envVars` for new plugins; FirstNexus still reads this during the deprecation window.                                               |
| `providerAuthAliases`                | No       | `Record<string, string>`         | Provider ids that should reuse another provider id for auth lookup, for example a coding provider that shares the base provider API key and auth profiles.                                                                          |
| `channelEnvVars`                     | No       | `Record<string, string[]>`       | Cheap channel env metadata that FirstNexus can inspect without loading plugin code. Use this for env-driven channel setup or auth surfaces that generic startup/config helpers should see.                                          |
...
| `qaRunners`                          | No       | `object[]`                       | Cheap QA runner descriptors used by the shared `FirstNexus qa` host before plugin runtime loads.                                                                                                                                    |
...
FirstNexus reads these fields before provider runtime loads so core tools can
...
ownership. `toolMetadata` declares cheap availability evidence so FirstNexus can
...
If a tool has no `toolMetadata`, FirstNexus preserves the existing behavior and
...
FirstNexus reads this before provider runtime loads.
...
| `choiceLabel`         | No       | `string`                                        | User-facing label. If omitted, FirstNexus falls back to `choiceId`.                                      |
...
mistakenly put in `plugins.allow` or try to run as a root CLI command. FirstNexus
...
the shared `FirstNexus qa` root. Keep this metadata cheap and static; the plugin
...
| Field         | Required | Type     | What it means                                                      |
| ------------- | -------- | -------- | ------------------------------------------------------------------ |
| `commandName` | Yes      | `string` | Subcommand mounted beneath `FirstNexus qa`, for example `matrix`.  |
...
FirstNexus also includes `setup.providers[].envVars` in generic provider auth and
...
FirstNexus can also derive simple setup choices from `setup.providers[].authMethods`
...
setup surface. FirstNexus treats explicit `false` as a descriptor-only contract
and will not execute `setup-api` or `FirstNexus.setupEntry` for setup lookup. If
a descriptor-only plugin still ships one of those setup runtime entries,
FirstNexus reports an additive diagnostic and continues ignoring it. Omitted
...
Use `contracts` only for static capability ownership metadata that FirstNexus can
...
| `migrationProviders`             | `string[]` | Import provider ids this plugin owns for `FirstNexus migrate`.        |
...
FirstNexus reads manifest metadata to decide which plugin owns that configured
...
`channelConfigs` entries. Without them, FirstNexus can still load the plugin, but
...
the same defaults through `package.json#FirstNexus.channel.commands` alongside
...
When `channels.chat` is configured, FirstNexus considers both the channel id and
the preferred plugin id. If the lower-priority plugin was only selected because
it is bundled or enabled by default, FirstNexus disables it in the effective
runtime config so one plugin owns the channel and its tools. Explicit user
selection still wins: if the user explicitly enables both plugins, FirstNexus
...
Use `modelSupport` when FirstNexus should infer your provider plugin from
...
FirstNexus applies this precedence:
...
Use `modelCatalog` when FirstNexus should know provider model metadata before
...
provider-filtered list uses an alias, FirstNexus can read the owning manifest and
...
| `compat`        | `object`                                                       | Optional compatibility flags matching FirstNexus model config compatibility. |
...
refreshable rows are not authoritative by themselves. Use `runtime` when FirstNexus
...
| Field                      | Type               | What it means                                                                                                          |
| -------------------------- | ------------------ | ---------------------------------------------------------------------------------------------------------------------- |
| `provider`                 | `string`           | External catalog provider id when it differs from the FirstNexus provider id, for example `z-ai` for a `zai` provider. |
...
### FirstNexus Provider Index

The FirstNexus Provider Index is FirstNexus-owned preview metadata for providers
...
4. FirstNexus Provider Index preview rows.
...
Legacy top-level capability keys are deprecated. Use `FirstNexus doctor --fix` to
...
| File                     | Use it for                                                                                                                         |
| ------------------------ | ---------------------------------------------------------------------------------------------------------------------------------- |
| `FirstNexus.plugin.json` | Discovery, config validation, auth-choice metadata, and UI hints that must exist before plugin code runs                           |
| `package.json`           | npm metadata, dependency installation, and the `FirstNexus` block used for entrypoints, install gating, setup, or catalog metadata |

If you are unsure where a piece of metadata belongs, use this rule:

- if FirstNexus must know it before loading plugin code, put it in `FirstNexus.plugin.json`
...
`FirstNexus` block instead of `FirstNexus.plugin.json`.
`FirstNexus.bundle` and `FirstNexus.bundle.json` are not FirstNexus plugin contracts;
native plugins must use `FirstNexus.plugin.json` plus the supported
`package.json#FirstNexus` fields below.

Important examples:

| Field                                                                                            | What it means                                                                                                                                                                        |
| ------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `FirstNexus.extensions`                                                                          | Declares native plugin entrypoints. Must stay inside the plugin package directory.                                                                                                   |
| `FirstNexus.runtimeExtensions`                                                                   | Declares built JavaScript runtime entrypoints for installed packages. Must stay inside the plugin package directory.                                                                 |
| `FirstNexus.setupEntry`                                                                          | Lightweight setup-only entrypoint used during onboarding, deferred channel startup, and read-only channel status/SecretRef discovery. Must stay inside the plugin package directory. |
| `FirstNexus.runtimeSetupEntry`                                                                   | Declares the built JavaScript setup entrypoint for installed packages. Requires `setupEntry`, must exist, and must stay inside the plugin package directory.                         |
| `FirstNexus.channel`                                                                             | Cheap channel catalog metadata like labels, docs paths, aliases, and selection copy.                                                                                                 |
| `FirstNexus.channel.commands`                                                                    | Static native command and native skill auto-default metadata used by config, audit, and command-list surfaces before channel runtime loads.                                          |
| `FirstNexus.channel.configuredState`                                                             | Lightweight configured-state checker metadata that can answer "does env-only setup already exist?" without loading the full channel runtime.                                         |
| `FirstNexus.channel.persistedAuthState`                                                          | Lightweight persisted-auth checker metadata that can answer "is anything already signed in?" without loading the full channel runtime.                                               |
| `FirstNexus.install.clawhubSpec` / `FirstNexus.install.npmSpec` / `FirstNexus.install.localPath` | Install/update hints for bundled and externally published plugins.                                                                                                                   |
| `FirstNexus.install.defaultChoice`                                                               | Preferred install path when multiple install sources are available.                                                                                                                  |
| `FirstNexus.install.minHostVersion`                                                              | Minimum supported FirstNexus host version, using a semver floor like `>=2026.3.22` or `>=2026.5.1-beta.1`.                                                                           |
| `FirstNexus.install.expectedIntegrity`                                                           | Expected npm dist integrity string such as `sha512-...`; install and update flows verify the fetched artifact against it.                                                            |
| `FirstNexus.install.allowInvalidConfigRecovery`                                                  | Allows a narrow bundled-plugin reinstall recovery path when config is invalid.                                                                                                       |
| `FirstNexus.startup.deferConfiguredChannelFullLoadUntilAfterListen`                              | Lets setup-only channel surfaces load before the full channel plugin during startup.                                                                                                 |

Manifest metadata decides which provider/channel/setup choices appear in
onboarding before runtime loads. `package.json#FirstNexus.install` tells
onboarding how to fetch or enable that plugin when the user picks one of those
choices. Do not move install hints into `FirstNexus.plugin.json`.

`FirstNexus.install.minHostVersion` is enforced during install and manifest
...
`"npmSpec": "@wecom/wecom-FirstNexus-plugin@1.2.3"`. Official external catalog
...
Channel plugins should provide `FirstNexus.setupEntry` when status, channel list,
...
entrypoint fields. For example, `FirstNexus.runtimeExtensions` cannot make an
escaping `FirstNexus.extensions` path loadable.

`FirstNexus.install.allowInvalidConfigRecovery` is intentionally narrow. It does
...
to `FirstNexus doctor --fix`.

`FirstNexus.channel.persistedAuthState` is package metadata for a tiny checker
module:
...
`FirstNexus.channel.configuredState` follows the same shape for cheap env-only
configured checks:
...
FirstNexus discovers plugins from several roots (bundled, global install, workspace, explicit config-selected paths). If two discoveries share the same `id`, only the **highest-precedence** manifest is kept; lower-precedence duplicates are dropped instead of loading beside it.

Precedence, highest to lowest:

1. **Config-selected** — a path explicitly pinned in `plugins.entries.<id>`
2. **Bundled** — plugins shipped with FirstNexus
3. **Global install** — plugins installed into the global FirstNexus plugin root
...
- When extending or forking a bundled plugin with new config keys, update that plugin's `FirstNexus.plugin.json` `configSchema` at the same time. Bundled plugin schemas are strict, so adding `plugins.entries.<id>.config.myNewKey` in user config without adding `myNewKey` to `configSchema.properties` will be rejected before the plugin runtime loads.
...
- The manifest is **required for native FirstNexus plugins**, including local filesystem loads. Runtime still loads the plugin module separately; the manifest is only for discovery + validation.
...
- Declare exclusive plugin kind in this manifest. Runtime-entry `FirstNexusPluginDefinition.kind` is deprecated and remains only as a compatibility fallback for older plugins.
