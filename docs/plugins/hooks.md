...
Plugin hooks are in-process extension points for FirstNexus plugins. Use them
...
current plugin options; FirstNexus injects it per handler without mutating the
...
- FirstNexus strips `toolResult.details` before provider replay and compaction
...
When a run is blocked, FirstNexus stores only the replacement text in
...
`before_agent_start` and `agent_end` include `event.runId` when FirstNexus can
...
identifier when FirstNexus can derive one from the session key or delivery
metadata.
...
FirstNexus continues; it does not cancel plugin-owned network work unless the
...
`durationMs`/`outcome`, and `upstreamRequestIdHash` when FirstNexus can derive a
...
Codex native `Stop` hooks are relayed into this hook as FirstNexus
...
reach the next model turn exactly once. FirstNexus drains queued injections before
...
context when syncing external wake schedulers, and keep FirstNexus as the
