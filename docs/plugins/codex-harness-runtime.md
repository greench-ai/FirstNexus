...
the native model loop, and FirstNexus adapts its plugin, tool, session, and
diagnostic surfaces around that boundary.

FirstNexus still owns channel routing, session files, visible message delivery,
FirstNexus dynamic tools, approvals, media delivery, and a transcript mirror.
...
When an FirstNexus session is attached to an existing Codex thread, the next turn
...
to the FirstNexus `message` tool if the deployment has not explicitly configured
...
Codex heartbeat turns also get `heartbeat_respond` in the searchable FirstNexus
...
| Layer                                 | Owner                      | Purpose                                                             |
| ------------------------------------- | -------------------------- | ------------------------------------------------------------------- |
| FirstNexus plugin hooks               | FirstNexus                 | Product/plugin compatibility across PI and Codex harnesses.         |
| Codex app-server extension middleware | FirstNexus bundled plugins | Per-turn adapter behavior around FirstNexus dynamic tools.          |
| Codex native hooks                    | Codex                      | Low-level Codex lifecycle and native tool policy from Codex config. |

FirstNexus does not use project or global Codex `hooks.json` files to route
FirstNexus plugin behavior. For the supported native tool and permission bridge,
FirstNexus injects per-thread Codex config for `PreToolUse`, `PostToolUse`,
...
Codex's app-server reviewer and FirstNexus's approval bridge handle real
...
Codex-level controls. They are not exposed as FirstNexus plugin hooks in the v1
contract.

For FirstNexus dynamic tools, FirstNexus executes the tool after Codex asks for the
call, so FirstNexus fires the plugin and middleware behavior it owns in the
harness adapter. For Codex-native tools, Codex owns the canonical tool record.
FirstNexus can mirror selected events, but it cannot rewrite the native Codex
...
notifications and FirstNexus adapter state, not native Codex hook commands.
FirstNexus's `before_compaction`, `after_compaction`, `llm_input`, and
...
They do not invoke FirstNexus plugin hooks.
...
| FirstNexus channel routing and delivery       | Supported                                                                        | Telegram, Discord, Slack, WhatsApp, iMessage, and other channels stay outside the model runtime.                                                                                                             |
| FirstNexus dynamic tools                      | Supported                                                                        | Codex asks FirstNexus to execute these tools, so FirstNexus stays in the execution path.                                                                                                                     |
| Prompt and context plugins                    | Supported                                                                        | FirstNexus builds prompt overlays and projects context into the Codex turn before starting or resuming the thread.                                                                                           |
...
| Dynamic tool hooks                            | Supported                                                                        | `before_tool_call`, `after_tool_call`, and tool-result middleware run around FirstNexus-owned dynamic tools.                                                                                                 |
...
| Native permission policy                      | Supported through Codex app-server approvals and compatibility native hook relay | Codex app-server approval requests route through FirstNexus after Codex review. The `PermissionRequest` native hook relay is opt-in for native approval modes because Codex emits it before guardian review. |
| App-server trajectory capture                 | Supported                                                                        | FirstNexus records the request it sent to app-server and the app-server notifications it receives.                                                                                                           |

Not supported in Codex runtime v1:

| Surface                                             | V1 boundary                                                                                                                                       | Future path                                                                               |
| --------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------- |
| Native tool argument mutation                       | Codex native pre-tool hooks can block, but FirstNexus does not rewrite Codex-native tool arguments.                                               | Requires Codex hook/schema support for replacement tool input.                            |
| Editable Codex-native transcript history            | Codex owns canonical native thread history. FirstNexus owns a mirror and can project future context, but should not mutate unsupported internals. | Add explicit Codex app-server APIs if native thread surgery is needed.                    |
| `tool_result_persist` for Codex-native tool records | That hook transforms FirstNexus-owned transcript writes, not Codex-native tool records.                                                           | Could mirror transformed records, but canonical rewrite needs Codex support.              |
| Rich native compaction metadata                     | FirstNexus observes compaction start and completion, but does not receive a stable kept/dropped list, token delta, or summary payload.            | Needs richer Codex compaction events.                                                     |
| Compaction intervention                             | Current FirstNexus compaction hooks are notification-level in Codex mode.                                                                         | Add Codex pre/post compaction hooks if plugins need to veto or rewrite native compaction. |
| Byte-for-byte model API request capture             | FirstNexus can capture app-server requests and notifications, but Codex core builds the final OpenAI API request internally.                      | Needs a Codex model-request tracing event or debug API.                                   |

## Native permissions and MCP elicitations

For `PermissionRequest`, FirstNexus only returns explicit allow or deny decisions
...
FirstNexus remembers that exact provider/session/tool input/cwd fingerprint for a
...
Codex MCP tool approval elicitations are routed through FirstNexus's plugin
...
default `messages.queue.mode: "steer"`, FirstNexus batches queued chat messages
...
case, FirstNexus uses the follow-up queue when the selected mode allows fallback.
...
harness, FirstNexus also calls Codex app-server `feedback/upload` for relevant
...
error. The completed diagnostics reply lists the channels, FirstNexus session ids,
Codex thread ids, and local `codex resume <thread-id>` commands for the threads
that were sent.

If you deny or ignore the approval, FirstNexus does not print those Codex ids and
...
delegated to Codex app-server. FirstNexus keeps a transcript mirror for channel
...
reasoning or plan records when the app-server emits them. Today, FirstNexus only
...
FirstNexus is writing an FirstNexus-owned session transcript tool result.

## Media and delivery

FirstNexus continues to own media delivery and media provider selection. Image,
...
through the normal FirstNexus delivery path. Media generation does not require PI.
When Codex emits a native image-generation item with a `savedPath`, FirstNexus
