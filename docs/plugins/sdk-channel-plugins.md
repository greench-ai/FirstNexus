---
summary: "Step-by-step guide to building a messaging channel plugin for FirstNexus"
title: "Building channel plugins"
sidebarTitle: "Channel Plugins"
read_when:
  - You are building a new messaging channel plugin
  - You want to connect FirstNexus to a messaging platform
  - You need to understand the ChannelPlugin adapter surface
---

This guide walks through building a channel plugin that connects FirstNexus to a
...
Channel plugins do not need their own send/edit/react tools. FirstNexus keeps one
...
`defineChannelMessageAdapter` from `FirstNexus/plugin-sdk/channel-message`. The
...
receive/send lifecycle helpers on `FirstNexus/plugin-sdk/channel-message`.

Channels migrating inbound authorization can use the experimental
`FirstNexus/plugin-sdk/channel-ingress-runtime` subpath from runtime receive
...
Use `FirstNexus/plugin-sdk/channel-route` when plugin code needs to normalize
...
- If a channel can infer stable owner-like DM identities from existing config, use `createResolvedApproverActionAuthAdapter` from `FirstNexus/plugin-sdk/approval-runtime` to restrict same-chat `/approve` without adding approval-specific core logic.
- If a channel needs native approval delivery, keep channel code focused on target normalization plus transport/presentation facts. Use `createChannelExecApprovalProfile`, `createChannelNativeOriginTargetResolver`, `createChannelApproverDmTargetResolver`, and `createApproverRestrictedNativeApprovalCapability` from `FirstNexus/plugin-sdk/approval-runtime`. Put the channel-specific facts behind `approvalCapability.nativeRuntime`, ideally via `createChannelApprovalNativeRuntimeAdapter(...)` or `createLazyChannelApprovalNativeRuntimeAdapter(...)`, so core can assemble the handler and own request filtering, routing, dedupe, expiry, gateway subscription, and routed-elsewhere notices. `nativeRuntime` is split into a few smaller seams:
...
- `FirstNexus/plugin-sdk/approval-auth-runtime`
- `FirstNexus/plugin-sdk/approval-client-runtime`
- `FirstNexus/plugin-sdk/approval-delivery-runtime`
- `FirstNexus/plugin-sdk/approval-gateway-runtime`
- `FirstNexus/plugin-sdk/approval-handler-adapter-runtime`
- `FirstNexus/plugin-sdk/approval-handler-runtime`
- `FirstNexus/plugin-sdk/approval-native-runtime`
- `FirstNexus/plugin-sdk/approval-reply-runtime`
- `FirstNexus/plugin-sdk/channel-runtime-context`

Likewise, prefer `FirstNexus/plugin-sdk/setup-runtime`,
`FirstNexus/plugin-sdk/setup-runtime`,
`FirstNexus/plugin-sdk/reply-runtime`,
`FirstNexus/plugin-sdk/reply-dispatch-runtime`,
`FirstNexus/plugin-sdk/reply-reference`, and
`FirstNexus/plugin-sdk/reply-chunking` when you do not need the broader umbrella
surface.

For setup specifically:

- `FirstNexus/plugin-sdk/setup-runtime` covers the runtime-safe setup helpers:
...
- `FirstNexus/plugin-sdk/setup-runtime` includes the env-aware adapter seam for
  `createEnvPatchedAccountSetupAdapter`
- `FirstNexus/plugin-sdk/channel-setup` covers the optional-install setup
...
SecretRef scans before the plugin runtime starts, add `FirstNexus.setupEntry` in
...
- use the broader `FirstNexus/plugin-sdk/setup` seam only when you also need the
...
- `FirstNexus/plugin-sdk/account-core`,
  `FirstNexus/plugin-sdk/account-id`,
  `FirstNexus/plugin-sdk/account-resolution`, and
  `FirstNexus/plugin-sdk/account-helpers` for multi-account config and
  default-account fallback
- `FirstNexus/plugin-sdk/inbound-envelope` and
  `FirstNexus/plugin-sdk/inbound-reply-dispatch` for inbound route/envelope and
  record-and-dispatch wiring
- `FirstNexus/plugin-sdk/messaging-targets` for target parsing/matching
- `FirstNexus/plugin-sdk/outbound-media` and
  `FirstNexus/plugin-sdk/outbound-runtime` for media loading plus outbound
  identity/send delegates and payload planning
- `buildThreadAwareOutboundSessionRoute(...)` from
  `FirstNexus/plugin-sdk/channel-core` when an outbound route should preserve an
...
- `FirstNexus/plugin-sdk/thread-bindings-runtime` for thread-binding lifecycle
  and adapter registration
- `FirstNexus/plugin-sdk/agent-media-payload` only when a legacy agent/media
  payload field layout is still required
- `FirstNexus/plugin-sdk/telegram-command-config` for Telegram custom-command
...
Use `FirstNexus/plugin-sdk/channel-mention-gating` for mention-policy decisions.
Use `FirstNexus/plugin-sdk/channel-inbound` only when you need the broader inbound
helper barrel.
...
`FirstNexus/plugin-sdk/channel-mention-gating` to avoid loading unrelated inbound
runtime helpers.

The older `resolveMentionGating*` helpers remain on
`FirstNexus/plugin-sdk/channel-inbound` as compatibility exports only. New code
...
<Steps>
  ...
  <Step title="Package and manifest">
    ...
    see [Plugin Setup and Config](/plugins/sdk-setup#FirstNexus-channel):

    <CodeGroup>
    ```json package.json
    {
      "name": "@myorg/FirstNexus-acme-chat",
      "version": "1.0.0",
      "type": "module",
      "FirstNexus": {
        "extensions": ["./index.ts"],
        ...
        "channel": {
          "id": "acme-chat",
          "label": "Acme Chat",
          "blurb": "Connect FirstNexus to Acme Chat."
        }
      }
    }
    ```
  ...
  <Step title="Build the channel plugin object">
    ...
    import type { FirstNexusConfig } from "FirstNexus/plugin-sdk/channel-core";
  ...
  <Step title="Wire the entry point">
    ...
    ```typescript index.ts
    import { defineChannelPluginEntry } from "FirstNexus/plugin-sdk/channel-core";
    ...
    Put channel-owned CLI descriptors in `registerCliMetadata(...)` so FirstNexus
  ...
  <Step title="Add a setup entry">
    ...
    import { defineSetupPluginEntry } from "FirstNexus/plugin-sdk/channel-core";
    import { acmeChatPlugin } from "./src/channel.js";

    export default defineSetupPluginEntry(acmeChatPlugin);
    ```

    FirstNexus loads this instead of the full entry when the channel is disabled
    ...
    `FirstNexus/plugin-sdk/channel-entry-contract` when they also need an
  ...
  <Step title="Handle inbound messages">
    ...
    FirstNexus. The typical pattern is a webhook that verifies the request and
    ...
    registerFull(api) {
      api.registerHttpRoute({
        ...
        handler: async (req, res) => {
          ...
          // Your inbound handler dispatches the message to FirstNexus.
...
├── package.json              # FirstNexus.channel metadata
├── FirstNexus.plugin.json      # Manifest with config schema
