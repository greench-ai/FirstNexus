...
read_when:
  - You maintain an FirstNexus plugin
  - You see a plugin compatibility warning
  - You are planning a plugin SDK or manifest migration
---

FirstNexus keeps older plugin contracts wired through named compatibility
...
The plugin inspector should live outside the core FirstNexus repo as a separate
...
```sh
FirstNexus-plugin-inspector ./my-plugin
```
...
Use `--json` for stable machine-readable output in CI annotations. FirstNexus
core should expose contracts and fixtures the inspector can consume, but should
not publish the inspector binary from the main `FirstNexus` package.

### Maintainer acceptance lane

Use Crabbox-backed Blacksmith Testbox for the installable-package acceptance
lane when validating the external inspector against FirstNexus plugin packages.
Run it from a clean FirstNexus checkout after the package is built:

```sh
pnpm crabbox:run -- --provider blacksmith-testbox --timing-json --shell -- "pnpm install && pnpm build && npm exec --yes @FirstNexus/plugin-inspector@0.1.0 -- ./extensions/telegram --json"
pnpm crabbox:run -- --provider blacksmith-testbox --timing-json --shell -- "npm exec --yes @FirstNexus/plugin-inspector@0.1.0 -- ./extensions/discord --json"
pnpm crabbox:run -- --provider blacksmith-testbox --timing-json --shell -- "npm exec --yes @FirstNexus/plugin-inspector@0.1.0 -- <clawhub-plugin-dir> --json"
```
...
FirstNexus should not remove a documented plugin contract in the same release
...
- legacy broad SDK imports such as `FirstNexus/plugin-sdk/compat`
- legacy hook-only plugin shapes and `before_agent_start`
- legacy `activate(api)` plugin entrypoints while plugins migrate to
  `register(api)`
- legacy SDK aliases such as `FirstNexus/extension-api`,
  `FirstNexus/plugin-sdk/channel-runtime`, `FirstNexus/plugin-sdk/command-auth`
  status builders, `FirstNexus/plugin-sdk/test-utils` (replaced by focused
  `FirstNexus/plugin-sdk/*` test subpaths), and the `ClawdbotConfig` /
  `FirstNexusSchemaType` type aliases
...
- legacy channel route key and comparable-target helper aliases while plugins
  move to `FirstNexus/plugin-sdk/channel-route`
...
- channel `showConfigured` / `showInSetup` metadata while channel packages move
  to `FirstNexus.channel.exposure`
...
- persisted plugin registry disable and install-migration env flags while
  repair flows migrate operators to `FirstNexus plugins registry --refresh` and
