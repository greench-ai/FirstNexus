...
read_when:
  ...
  - You need to map a third-party CLI into FirstNexus's text fallback runner
---

CLI backend plugins let FirstNexus call a local AI CLI as a text inference
...
| -------------------- | ------------------------ | --------------------------------------------------------- |
| Package entry        | `package.json`           | Points FirstNexus at the plugin runtime module            |
| Manifest ownership   | `FirstNexus.plugin.json` | Declares the backend id before runtime loads              |
...
<Steps>
  <Step title="Create package metadata">
    ...
    entry is `./src/index.ts`, add `FirstNexus.runtimeExtensions` that points at
  ...
  <Step title="Declare backend ownership">
    ```json FirstNexus.plugin.json
    {
      "id": "acme-cli",
      "name": "Acme CLI",
      "description": "Run Acme's local AI CLI through FirstNexus",
    ...
    `cliBackends` is the runtime ownership list. It lets FirstNexus auto-load the
...
`CliBackendConfig` describes how FirstNexus should launch and parse the CLI:
...
| `modelAliases`                            | Map FirstNexus model ids to CLI-native ids                  |
| `sessionArg` / `sessionArgs`              | How to pass a session id                                    |
| `sessionMode`                             | `always`, `existing`, or `none`                             |
| `sessionIdFields`                         | JSON fields FirstNexus reads from CLI output                |
...
| `defaultAuthProfileId`             | Prefer a specific FirstNexus auth profile              |
| `authEpochMode`                    | Decide how auth changes invalidate stored CLI sessions |
| `nativeToolMode`                   | Declare whether the CLI has always-on native tools     |
| `bundleMcp` / `bundleMcpMode`      | Opt into FirstNexus's loopback MCP tool bridge         |
...
CLI backends do not receive FirstNexus tools by default. If the CLI can consume an
...
"always-on"` so FirstNexus can fail closed when a caller requires no native tools.
...
```bash
FirstNexus plugins inspect acme-cli --runtime --json
FirstNexus agent --message "reply exactly: backend ok" --model acme-cli/acme-large
```
...
<Check>`package.json` has `FirstNexus.extensions` and built runtime entries for published packages</Check>
<Check>`FirstNexus.plugin.json` declares `cliBackends` and intentional `activation.onStartup`</Check>
