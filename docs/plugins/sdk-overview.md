...
read_when:
  ...
  - You want a reference for all registration methods on FirstNexusPluginApi
...
<Note>
  This page is for plugin authors using `FirstNexus/plugin-sdk/*` inside
  FirstNexus. For external apps, scripts, dashboards, CI jobs, and IDE extensions
  that want to run agents through the Gateway, use the
  [FirstNexus App SDK](/concepts/FirstNexus-sdk) and the `@FirstNexus/sdk` package
  instead.
</Note>
...
prefer `FirstNexus/plugin-sdk/channel-core`; keep `FirstNexus/plugin-sdk/core` for
...
`FirstNexus.plugin.json#channelConfigs`. The `plugin-sdk/channel-config-schema`
subpath is for shared schema primitives and the generic builder. FirstNexus's
...
<Warning>
  ...
  `FirstNexus/plugin-sdk/slack`, `.../discord`, `.../signal`, `.../whatsapp`).
...
`FirstNexus/plugin-sdk/discord` and `FirstNexus/plugin-sdk/telegram-account` are
...
The `register(api)` callback receives an `FirstNexusPluginApi` object with these
methods:
...
| `api.registerNodeCliFeature(registrar, opts?)` | Node feature CLI under `FirstNexus nodes` |
...
cannot register this middleware; keep normal FirstNexus plugin hooks for work
...
Gateway on a local discovery transport such as mDNS/Bonjour. FirstNexus calls the
...
`FirstNexus nodes canvas` explicit plugin-owned node features.
...
- User config still wins. FirstNexus merges `agents.defaults.cliBackends.<id>` over the
...
- Use `resolveExecutionArgs` for request-scoped argv rewrites that belong to
  the CLI dialect, such as mapping FirstNexus thinking levels to a native effort
  flag.
...
- `registerMemoryCapability` may also expose `publicArtifacts.listArtifacts(...)`
  ...
  `FirstNexus/plugin-sdk/memory-host-core` instead of reaching into a specific
...
- `cron_changed`: observe gateway-owned cron lifecycle changes. Use `event.job?.state?.nextRunAtMs` and `ctx.getCron?.()` when syncing external wake schedulers, and keep FirstNexus as the source of truth for due checks and execution.
...
| `api.config`             | `FirstNexusConfig`        | Current config snapshot (active in-memory runtime snapshot when available)                  |
...
<Warning>
  Never import your own plugin through `FirstNexus/plugin-sdk/<your-plugin>`
...
active runtime config snapshot when FirstNexus is already running. If no runtime
snapshot exists yet, they fall back to the resolved config file on disk.
Packaged bundled plugin facades should be loaded through FirstNexus's plugin
...
- **`@FirstNexus/openai-provider`**: `api.ts` exports provider builders,
  default-model helpers, and realtime provider builders.
- **`@FirstNexus/openrouter-provider`**: `api.ts` exports the provider builder
  plus onboarding/config helpers.

<Warning>
  Extension production code should also avoid `FirstNexus/plugin-sdk/<other-plugin>`
  imports. If a helper is truly shared, promote it to a neutral SDK subpath
  such as `FirstNexus/plugin-sdk/speech`, `.../provider-model-shared`, or another
