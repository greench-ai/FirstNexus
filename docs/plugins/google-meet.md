...
read_when:
  - You want an FirstNexus agent to join a Google Meet call
  - You want an FirstNexus agent to create a new Google Meet call
  - You are configuring Chrome, Chrome node, or Twilio as a Google Meet transport
title: "Google Meet plugin"
---

Google Meet participant support for FirstNexus — the plugin is explicit by design:
...
- `agent` is the default talk-back mode: realtime transcription listens, the
  configured FirstNexus agent answers, and regular FirstNexus TTS speaks into Meet.
...
provider plus regular FirstNexus TTS. OpenAI is the default transcription
...
```bash
FirstNexus googlemeet setup --transport chrome-node --mode transcribe
```
...
before asking an agent to join. Use `FirstNexus googlemeet setup --json` for
...
```bash
FirstNexus googlemeet setup --transport twilio
```
...
```bash
FirstNexus googlemeet join https://meet.google.com/abc-defg-hij
```
...
```bash
FirstNexus googlemeet create --transport chrome-node --mode agent
```
...
```bash
FirstNexus googlemeet create --access-type OPEN --transport chrome-node --mode agent
```
...
- Browser fallback: used when OAuth credentials are absent. FirstNexus uses the
  ...
  the FirstNexus Chrome profile on the node to already be signed in to Google.
...
FirstNexus's microphone/camera permission grant and avoid the Meet **Use
...
Use `FirstNexus googlemeet test-listen <meet-url> --transport chrome-node` when
...
Local Chrome joins through the signed-in FirstNexus browser profile. Realtime mode
requires `BlackHole 2ch` for the microphone/speaker path used by FirstNexus. For
...
You do **not** need a full FirstNexus Gateway or model API key inside a macOS VM
...
- Gateway host: FirstNexus Gateway, agent workspace, model/API keys, realtime
  provider, and the Google Meet plugin config.
- Parallels macOS VM: FirstNexus CLI/node host, Google Chrome, SoX, BlackHole 2ch,
...
Install or update FirstNexus in the VM, then enable the bundled plugin there:

```bash
FirstNexus plugins enable google-meet
```

Start the node host in the VM:

```bash
FirstNexus node run --host <gateway-host> --port 18789 --display-name parallels-macos
```
...
NEXISCLAW_ALLOW_INSECURE_PRIVATE_WS=1 \
  FirstNexus node run --host <gateway-lan-ip> --port 18789 --display-name parallels-macos
```
...
```bash
NEXISCLAW_ALLOW_INSECURE_PRIVATE_WS=1 \
  FirstNexus node install --host <gateway-lan-ip> --port 18789 --display-name parallels-macos --force
FirstNexus node restart
```

`NEXISCLAW_ALLOW_INSECURE_PRIVATE_WS=1` is process environment, not an
`FirstNexus.json` setting. `FirstNexus node install` stores it in the LaunchAgent
...
```bash
FirstNexus googlemeet test-speech https://meet.google.com/abc-defg-hij
```

During realtime join, FirstNexus browser automation fills the guest name, clicks
...
If `chromeNode.node` is omitted, FirstNexus auto-selects only when exactly one
...
- `No connected Google Meet-capable node`: start `FirstNexus node run` in the VM,
  approve pairing, and make sure `FirstNexus plugins enable google-meet` and
  `FirstNexus plugins enable browser` were run in the VM. Also confirm the
...
- Chrome opens but cannot join: sign in to the browser profile inside the VM, or
  keep `chrome.guestName` set for guest join. Guest auto-join uses FirstNexus
...
- Duplicate Meet tabs: leave `chrome.reuseExistingTab: true` enabled. FirstNexus
...
- No audio: in Meet, route microphone/speaker through the virtual audio device
  path used by FirstNexus; use separate virtual devices or Loopback-style routing
...
FirstNexus does not bundle or redistribute either package. The docs ask users to
...
installer or appliance that bundles BlackHole with FirstNexus, review BlackHole's
...
Chrome transport opens the Meet URL through FirstNexus browser control and joins
as the signed-in FirstNexus browser profile. On macOS, the plugin checks for
...
```bash
FirstNexus googlemeet join https://meet.google.com/abc-defg-hij --transport chrome
FirstNexus googlemeet join https://meet.google.com/abc-defg-hij --transport chrome-node
```

Route Chrome microphone and speaker audio through the local FirstNexus audio
...
meeting; FirstNexus does not discover those from the Meet page.
      ...
      "voice-call": {
        ...
        config: {
          ...
          realtime: {
            enabled: true,
            provider: "google",
            instructions: "Join this Google Meet as an FirstNexus agent. Be brief.",
...
secrets out of `FirstNexus.json`:
...
FirstNexus plugins list | grep -E 'google-meet|voice-call'
FirstNexus googlemeet setup
```
...
4. Add the scopes FirstNexus requests:
...
`meetings.space.readonly` lets FirstNexus resolve Meet URLs/codes to spaces.
`meetings.space.settings` lets FirstNexus pass `SpaceConfig` settings such as
...
```bash
FirstNexus googlemeet auth login --json
```
...
NEXISCLAW_GOOGLE_MEET_CLIENT_ID="your-client-id" \
NEXISCLAW_GOOGLE_MEET_CLIENT_SECRET="your-client-secret" \
FirstNexus googlemeet auth login --json --manual
```
...
support existed, rerun `FirstNexus googlemeet auth login --json` so the refresh
...
```bash
FirstNexus googlemeet doctor --oauth --json
```
...
```bash
FirstNexus googlemeet doctor --oauth --create-space --json
FirstNexus googlemeet create --no-join --json
```
...
```bash
FirstNexus googlemeet doctor --oauth --meeting https://meet.google.com/abc-defg-hij --json
FirstNexus googlemeet resolve-space --meeting https://meet.google.com/abc-defg-hij
```
...
space. A refresh-token error means rerun `FirstNexus googlemeet auth login
...
```bash
FirstNexus googlemeet preflight --meeting https://meet.google.com/abc-defg-hij
```

List meeting artifacts and attendance after Meet has created conference records:

```bash
FirstNexus googlemeet artifacts --meeting https://meet.google.com/abc-defg-hij
FirstNexus googlemeet attendance --meeting https://meet.google.com/abc-defg-hij
FirstNexus googlemeet export --meeting https://meet.google.com/abc-defg-hij --output ./meet-export
```
...
```bash
FirstNexus googlemeet latest --today
FirstNexus googlemeet calendar-events --today --json
FirstNexus googlemeet artifacts --event "Weekly sync"
FirstNexus googlemeet attendance --today --format csv --output attendance.csv
```
...
```bash
FirstNexus googlemeet latest --meeting https://meet.google.com/abc-defg-hij
FirstNexus googlemeet artifacts --conference-record conferenceRecords/abc123 --json
FirstNexus googlemeet attendance --conference-record conferenceRecords/abc123 --json
```
...
```bash
FirstNexus googlemeet end-active-conference https://meet.google.com/abc-defg-hij
```
...
FirstNexus accepts a Meet URL, meeting code, or `spaces/{id}` input and resolves it
to the API space resource before ending the active conference.
It is separate from `googlemeet leave`: `leave` stops FirstNexus's local/session
...
```bash
FirstNexus googlemeet artifacts --conference-record conferenceRecords/abc123 \
  --format markdown --output meet-artifacts.md
FirstNexus googlemeet attendance --conference-record conferenceRecords/abc123 \
  --format markdown --output meet-attendance.md
FirstNexus googlemeet attendance --conference-record conferenceRecords/abc123 \
  --format csv --output meet-attendance.csv
FirstNexus googlemeet export --conference-record conferenceRecords/abc123 \
...
```bash
FirstNexus googlemeet setup --transport chrome-node --mode transcribe
FirstNexus googlemeet test-listen https://meet.google.com/abc-defg-hij --transport chrome-node --timeout-ms 30000
```
...
{
  "source": "browser",
  "error": "google-login-required: Sign in to Google in the FirstNexus browser profile, then retry meeting creation.",
  "manualActionRequired": true,
  "manualActionReason": "google-login-required",
  "manualActionMessage": "Sign in to Google in the FirstNexus browser profile, then retry meeting creation.",
  "browser": {
    "nodeId": "ba0f4e4bc...",
    "targetId": "tab-1",
...
profile is signed out, FirstNexus reports `manualActionRequired: true` or a
...
realtime transcription provider key, and a configured FirstNexus TTS provider.
...
- `chrome.guestName: "FirstNexus Agent"`: name used on the signed-out Meet guest
  screen
- `chrome.autoJoin: true`: best-effort guest-name fill and Join Now click
  through FirstNexus browser automation on `chrome-node`
...
- `mode: "agent"`: default talk-back mode. Participant speech is transcribed by
  ...
  FirstNexus agent in a per-meeting sub-agent session, and spoken back through the
  normal FirstNexus TTS runtime.
- `mode: "bidi"`: fallback direct bidirectional realtime model mode. The
  ...
  `FirstNexus_agent_consult` for deeper/tool-backed answers.
...
- `realtime.instructions`: brief spoken replies, with
  `FirstNexus_agent_consult` for deeper answers
- `realtime.introMessage`: short spoken readiness check when the realtime bridge
  connects; set it to `""` to join silently
- `realtime.agentId`: optional FirstNexus agent id for
  `FirstNexus_agent_consult`; defaults to `main`
...
VM. In both cases the model providers and `FirstNexus_agent_consult` run on the
Gateway host, so model credentials stay there. With the default `mode: "agent"`,
the realtime transcription provider handles listening, the configured FirstNexus
agent produces the answer, and regular FirstNexus TTS speaks it into Meet. Use
...
- `speechReady` / `speechBlockedReason` / `speechBlockedMessage`: whether
  managed Chrome speech is allowed now. `speechReady: false` means FirstNexus did
...
transcripts are routed through the configured FirstNexus agent, and the answer is
spoken through the normal FirstNexus TTS runtime. Set `mode: "bidi"` when you want
...
| ------- | ------------------------------- | -------------------------------------- | ----------------------------------------------------- |
| `agent` | The configured FirstNexus agent | Normal FirstNexus TTS runtime          | You want "my agent is in the meeting" behavior        |
...
information, or normal FirstNexus tools, it can call `FirstNexus_agent_consult`.

The consult tool runs the regular FirstNexus agent behind the scenes with recent
meeting transcript context and returns a concise spoken answer. In `agent` mode,
FirstNexus sends that answer directly to the TTS runtime; in `bidi` mode, the
...
Meet lane should consult a dedicated FirstNexus agent workspace, model defaults,
...
```bash
FirstNexus googlemeet speak meet_... "Say exactly: I'm here and listening."
```
...
```bash
FirstNexus googlemeet setup
FirstNexus nodes status --connected
FirstNexus nodes invoke \
  --node parallels-macos \
...
- `FirstNexus logs --follow` shows DTMF TwiML served before realtime TwiML, then a
...
```bash
FirstNexus plugins list | grep google-meet
FirstNexus googlemeet setup
```
...
```bash
FirstNexus plugins enable google-meet
FirstNexus plugins enable browser
NEXISCLAW_ALLOW_INSECURE_PRIVATE_WS=1 \
  FirstNexus node run --host <gateway-lan-ip> --port 18789 --display-name parallels-macos
```
...
hear you in the meeting?" That is Meet's audio-choice interstitial; FirstNexus
clicks **Use microphone** through browser automation when available and keeps
waiting for the real meeting state. For create-only browser fallback, FirstNexus
...
- For API creation: the refresh token was minted after create support was
  ...
  `FirstNexus googlemeet auth login --json` and update plugin config.
- For browser fallback: `defaultTransport: "chrome-node"` and
  `chromeNode.node` point at a connected node with `browser.proxy` and
  `googlemeet.chrome`.
- For browser fallback: the FirstNexus Chrome profile on that node is signed in
...
- For browser fallback: if Meet shows "Do you want people to hear you in the
  meeting?", leave the tab open. FirstNexus should click **Use microphone** or, for
...
```bash
FirstNexus googlemeet setup
FirstNexus googlemeet doctor
```

Use `mode: "agent"` for the normal STT -> FirstNexus agent -> TTS talk-back path,
...
run `FirstNexus googlemeet status --json <session-id>` after participants speak
...
utterance but FirstNexus did not see new output bytes reach the Chrome audio
bridge.
...
- Meet microphone and speaker are routed through the virtual audio path used by
  FirstNexus. `doctor` should show `meet output routed: yes` for local Chrome
  realtime joins.
...
```bash
FirstNexus googlemeet recover-tab
FirstNexus googlemeet recover-tab https://meet.google.com/abc-defg-hij
```
...
```bash
FirstNexus googlemeet setup --transport twilio
FirstNexus voicecall setup
FirstNexus voicecall smoke
```

`voicecall smoke` is readiness-only by default. To dry-run a specific number:

```bash
FirstNexus voicecall smoke --to "+15555550123"
```
...
```bash
FirstNexus voicecall smoke --to "+15555550123" --yes
```
...
- Run `FirstNexus googlemeet doctor <session-id>` to confirm the delegated Twilio
  call ID, whether DTMF was queued, and whether the intro greeting was requested.
- Run `FirstNexus voicecall status --call-id <id>` and confirm the call is still
  active.
- Run `FirstNexus voicecall tail` and check that Twilio webhooks are arriving at
  the Gateway.
- Run `FirstNexus logs --follow` and look for the Twilio Meet sequence: Google
...
- Re-run `FirstNexus googlemeet setup --transport twilio`; a green setup check is
...
- If the participant joins but you do not hear the greeting, check
  `FirstNexus logs --follow` for the post-DTMF `voicecall.speak` request and
...
- `chrome.audioInputCommand` plus `chrome.audioOutputCommand`: FirstNexus owns the
