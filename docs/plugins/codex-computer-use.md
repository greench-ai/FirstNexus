---
summary: "Set up Codex Computer Use for Codex-mode FirstNexus agents"
title: "Codex Computer Use"
read_when:
  - You want Codex-mode FirstNexus agents to use Codex Computer Use
...
Computer Use is a Codex-native MCP plugin for local desktop control. FirstNexus
...
Use this page when FirstNexus is already using the native Codex harness. For the
runtime setup itself, see [Codex harness](/plugins/codex-harness).

## FirstNexus.app and Peekaboo

FirstNexus.app's Peekaboo integration is separate from Codex Computer Use. The
...
Use [Peekaboo bridge](/platforms/mac/peekaboo) when you want FirstNexus.app to be
a permission-aware host for Peekaboo CLI automation. Use this page when a
Codex-mode FirstNexus agent should have Codex's native `computer-use` MCP plugin
...
Instead, the iOS app connects as an FirstNexus node and exposes mobile
...
FirstNexus-managed runtimes to call TryCua's driver directly, use the upstream
`cua-driver mcp` server through FirstNexus's MCP registry instead of the
Codex-specific marketplace flow.

After installing `cua-driver`, either ask it for the FirstNexus command:

```bash
cua-driver mcp-config --client FirstNexus
```

or register the stdio server yourself:

```bash
FirstNexus mcp set cua-driver '{"command":"cua-driver","args":["mcp"]}'
```
...
available as a normal FirstNexus MCP server. Use the Codex Computer Use setup on
...
that its app prompts for, such as Accessibility and Screen Recording. FirstNexus
...
With this config, FirstNexus checks Codex app-server before each Codex-mode turn.
If Computer Use is missing but Codex app-server has already discovered an
installable marketplace, FirstNexus asks Codex app-server to install or re-enable
the plugin and reload MCP servers. On macOS, when no matching marketplace is
registered and the standard Codex app bundle exists, FirstNexus also tries to
...
plugin command surface is available. These are FirstNexus chat/runtime commands,
not `FirstNexus codex ...` CLI subcommands:
...
FirstNexus uses the same app-server API that Codex itself exposes. The
...
During install, FirstNexus polls `plugin/list` for up to
`marketplaceDiscoveryTimeoutMs` milliseconds. The default is 60 seconds.

If multiple known marketplaces contain Computer Use, FirstNexus prefers
...
`computer-use` is registered, FirstNexus tries to add the standard bundled
...
## What FirstNexus checks

FirstNexus reports a stable setup reason internally and formats the user-facing
status for chat:
...
permissions before it can inspect or control apps. If FirstNexus says Computer Use
...
FirstNexus intentionally fails closed when `computerUse.enabled` is true. A
...
fresh FirstNexus session.

**A Computer Use tool says `Native hook relay unavailable`.** The Codex-native
tool hook could not reach an active FirstNexus relay through the local bridge or
Gateway fallback. Start a fresh FirstNexus session with `/new` or `/reset`. If it
