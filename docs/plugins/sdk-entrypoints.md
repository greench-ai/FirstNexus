...
when FirstNexus loads an installed package and let npm packages avoid runtime
...
an installed package only declares a TypeScript source entry, FirstNexus will use a
...
**Import:** `FirstNexus/plugin-sdk/plugin-entry`
...
| `kind`         | `string`                                                             | No       | -                   |
| `configSchema` | `FirstNexusPluginConfigSchema \| () => FirstNexusPluginConfigSchema` | No       | Empty object schema |
| `register`     | `(api: FirstNexusPluginApi) => void`                                 | Yes      | -                   |

- `id` must match your `FirstNexus.plugin.json` manifest.
- `kind` is for exclusive slots: `"memory"` or `"context-engine"`.
- `configSchema` can be a function for lazy evaluation.
- FirstNexus resolves and memoizes that schema on first access, so expensive schema
  builders only run once.

## `defineChannelPluginEntry`

**Import:** `FirstNexus/plugin-sdk/channel-core`
...
| `registerCliMetadata` | `(api: FirstNexusPluginApi) => void`                                 | No       | -                   |
| `registerFull`        | `(api: FirstNexusPluginApi) => void`                                 | No       | -                   |
...
- Discovery registration is non-activating, not import-free. FirstNexus may
...
- Like `definePluginEntry`, `configSchema` can be a lazy factory and FirstNexus
...
- For plugin-owned root CLI commands, prefer `api.registerCli(..., { descriptors: [...] })`
  ...
  `api.registerNodeCliFeature(...)` so the command lands under `FirstNexus nodes`.
  For other nested plugin commands, add `parentPath` and register commands on
  the `program` object passed to the registrar; FirstNexus resolves it to the
...
FirstNexus loads this instead of the full entry when a channel is disabled,
...
- `FirstNexus/plugin-sdk/setup-runtime` for runtime-safe setup helpers such as
  import-safe setup patch adapters, lookup-note output,
  `promptResolvedAllowFrom`, `splitSetupEntries`, and delegated setup proxies
- `FirstNexus/plugin-sdk/channel-setup` for optional-install setup surfaces
- `FirstNexus/plugin-sdk/setup-tools` for setup/install CLI/archive/docs helpers
...
`FirstNexus/plugin-sdk/channel-entry-contract` instead. That contract lets the
setup entry keep setup-safe plugin/secrets exports while still exposing a
runtime setter:

```typescript
import { defineBundledChannelSetupEntry } from "FirstNexus/plugin-sdk/channel-entry-contract";
...
the plugin entry and the channel plugin object so FirstNexus can register channel
...
- use `descriptors` when the registrar owns one or more root commands and you
  want FirstNexus to lazy-load the real CLI module on first invocation
...
- keep descriptor command names to letters, numbers, hyphen, and underscore,
  starting with a letter or number; FirstNexus rejects descriptor names outside
...
FirstNexus classifies loaded plugins by their registration behavior:
...
Use `FirstNexus plugins inspect <id>` to see a plugin's shape.
