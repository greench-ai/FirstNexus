---
summary: "Run FirstNexus embedded agent turns through the bundled Codex app-server harness"
title: "Codex harness"
read_when:
...
The bundled `codex` plugin lets FirstNexus run embedded OpenAI agent turns
...
app-server execution. FirstNexus still owns chat channels, session files, model
selection, FirstNexus dynamic tools, approvals, media delivery, and the visible
transcript mirror.
...
FirstNexus starts Codex app-server threads with Codex native code mode and
code-mode-only enabled. That keeps deferred/searchable FirstNexus dynamic tools
...
- FirstNexus with the bundled `codex` plugin available.
...
- Codex auth available through `FirstNexus models auth login --provider openai-codex`,
...
Most users who want Codex in FirstNexus want this path: sign in with a
...
harness options in FirstNexus config, and use the CLI only for Codex auth:

| Need                                   | Set                                                                              | Where                                |
| -------------------------------------- | -------------------------------------------------------------------------------- | ------------------------------------ |
| Enable the harness                     | `plugins.entries.codex.enabled: true`                                            | FirstNexus config                    |
| Keep an allowlisted plugin install     | Include `codex` in `plugins.allow`                                               | FirstNexus config                    |
| Route OpenAI agent turns through Codex | `agents.defaults.model` or `agents.list[].model` as `openai/gpt-*`               | FirstNexus agent config              |
| Sign in with Codex OAuth               | `FirstNexus models auth login --provider openai-codex`                           | CLI auth profile                     |
| Add API-key backup for Codex runs      | `openai:*` API-key profile listed after subscription auth in `auth.order.openai` | CLI auth profile + FirstNexus config |
| Fail closed when Codex is unavailable  | Provider or model `agentRuntime.id: "codex"`                                     | FirstNexus model/provider config     |
| Use direct OpenAI API traffic          | Provider or model `agentRuntime.id: "pi"` with normal OpenAI auth                | FirstNexus model/provider config     |
...
- Do not use `openai-codex/gpt-*` in config. Run `FirstNexus doctor --fix` to
...
| Legacy config                                        | `openai-codex/gpt-*`                                             | `FirstNexus doctor --fix` rewrites it   | Do not write new config this way   |
...
With Codex forced, FirstNexus fails early if the Codex plugin is disabled, the
app-server is too old, or the app-server cannot start.

## App-server policy

By default, the plugin starts FirstNexus's managed Codex binary locally with stdio
...
implicit YOLO posture, FirstNexus selects allowed guardian permissions instead.
When an FirstNexus sandbox is active for the session, FirstNexus narrows Codex
...
supports FirstNexus text commands.
...
- `/codex resume <thread-id>` attaches the current FirstNexus session to an
...
When FirstNexus sees a ChatGPT subscription-style Codex auth profile, it removes
...
If a subscription profile hits a Codex usage limit, FirstNexus records the reset
...
Codex dynamic tools default to `searchable` loading. FirstNexus does not expose
dynamic tools that duplicate Codex-native workspace operations: `read`, `write`,
`edit`, `apply_patch`, `exec`, `process`, and `update_plan`. Remaining FirstNexus
...
search under the `FirstNexus` namespace, keeping the initial model context
smaller.
...
| -------------------------- | -------------- | ------------------------------------------------------------------------------------------ |
| `codexDynamicToolsLoading` | `"searchable"` | Use `"direct"` to put FirstNexus dynamic tools directly in the initial Codex tool context. |
| `codexDynamicToolsExclude` | `[]`           | Additional FirstNexus dynamic tool names to omit from Codex app-server turns.              |
...
| `clearEnv`                    | `[]`                                                   | Extra environment variable names removed from the spawned stdio app-server process after FirstNexus builds its inherited environment. `CODEX_HOME` and `HOME` are reserved for FirstNexus's per-agent Codex isolation on local launches.  |
| `requestTimeoutMs`            | `60000`                                                | Timeout for app-server control-plane calls.                                                                                                                                                                                               |
| `turnCompletionIdleTimeoutMs` | `60000`                                                | Quiet window after a turn-scoped Codex app-server request while FirstNexus waits for `turn/completed`. Raise this for slow post-tool or status-only synthesis phases.                                                                     |
...
| `sandbox`                     | `"danger-full-access"` or an allowed guardian sandbox  | Native Codex sandbox mode sent to thread start/resume. Guardian defaults prefer `"workspace-write"` when allowed, otherwise `"read-only"`. When an FirstNexus sandbox is active, `danger-full-access` is narrowed to `"workspace-write"`. |
| `approvalsReviewer`           | `"user"` or an allowed guardian reviewer               | Use `"auto_review"` to let Codex review native approval prompts when allowed, otherwise `guardian_subagent` or `user`. `guardian_subagent` remains a legacy alias.                                                                        |
| `serviceTier`                 | unset                                                  | Optional Codex app-server service tier. `"priority"` enables fast-mode routing, `"flex"` requests flex processing, `null` clears the override, and legacy `"fast"` is accepted as `"priority"`.                                           |

FirstNexus-owned dynamic tool calls are bounded independently from
`appServer.requestTimeoutMs`: Codex `item/tool/call` requests use a 30 second
FirstNexus watchdog by default. A positive per-call `timeoutMs` argument extends
...
budgets are capped at 600000 ms. On timeout, FirstNexus aborts the tool signal
...
After FirstNexus responds to a Codex turn-scoped app-server request, the harness
...
response, FirstNexus best-effort interrupts the Codex turn, records a diagnostic
timeout, and releases the FirstNexus session lane so follow-up chat messages are
...
FirstNexus treats the assistant output as effectively complete, best-effort
...
capabilities in the same Codex thread as the FirstNexus harness turn. FirstNexus
does not translate Codex plugins into synthetic `codex_plugin_*` FirstNexus
dynamic tools.
...
Thread app config is computed when FirstNexus establishes a Codex harness session
...
The short version: FirstNexus does not vendor the desktop-control app or execute
...
- FirstNexus dynamic tools are supported. Codex asks FirstNexus to execute those
  tools, so FirstNexus remains in the execution path.
- Codex-native shell, patch, MCP, and native app tools are owned by Codex.
  FirstNexus can observe or block selected native events through the supported
  relay, but it does not rewrite native tool arguments.
- Codex owns native compaction. FirstNexus keeps a transcript mirror for channel
...
- Media generation, media understanding, TTS, approvals, and messaging-tool
  output continue through the matching FirstNexus provider/model settings.
- `tool_result_persist` applies to FirstNexus-owned transcript tool results, not
...
**FirstNexus uses PI instead of Codex:** make sure the model ref is
...
**Legacy `openai-codex/*` config remains:** run `FirstNexus doctor --fix`.
...
`0.125.0-alpha.2` or `0.125.0+custom` are rejected because FirstNexus tests the
