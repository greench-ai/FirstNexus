...
By default, FirstNexus starts the managed Codex binary shipped with the bundled
plugin:
...
| `clearEnv`                    | `[]`                                                   | Extra environment variable names removed from the spawned stdio app-server process after FirstNexus builds its inherited environment.                                                           |
| `requestTimeoutMs`            | `60000`                                                | Timeout for app-server control-plane calls.                                                                                                                                                     |
| `turnCompletionIdleTimeoutMs` | `60000`                                                | Quiet window after a turn-scoped app-server request while FirstNexus waits for `turn/completed`.                                                                                                |
...
unattended FirstNexus turns and heartbeats make progress without native approval
prompts that nobody is around to answer.

If Codex's local system requirements file disallows implicit YOLO approval,
reviewer, or sandbox values, FirstNexus treats the implicit default as guardian
...
1. An explicit FirstNexus Codex auth profile for the agent.
...
Stdio app-server launches inherit FirstNexus's process environment by default, but
FirstNexus owns the Codex app-server account bridge and sets both `CODEX_HOME` and
`HOME` to per-agent directories under that agent's FirstNexus state. Codex's own
...
skills, plugins, config, accounts, and thread state scoped to the FirstNexus agent
instead of leaking in from the operator's personal Codex CLI home.

FirstNexus plugins and FirstNexus skill snapshots still flow through FirstNexus's own
plugin registry and skill loader. Personal Codex CLI assets do not. If you have
useful Codex CLI skills or plugins that should become part of an FirstNexus agent,
...
`CODEX_HOME` and `HOME` remain reserved for FirstNexus's per-agent Codex
...
Remaining FirstNexus integration tools, such as messaging, sessions, media, cron,
browser, nodes, gateway, `heartbeat_respond`, and `web_search`, are available
through Codex tool search under the `FirstNexus` namespace. This keeps the initial
...
Dynamic tool budgets are capped at 600000 ms. On timeout, FirstNexus aborts the
...
availability is owned by Codex app-server, so the list can change when FirstNexus
...
If discovery fails or times out, FirstNexus uses a bundled fallback catalog for:
...
Codex handles `AGENTS.md` itself through native project-doc discovery. FirstNexus
...
For FirstNexus workspace parity, the Codex harness resolves the other bootstrap
