...
read_when:
  ...
  - You are defining plugin config schemas or package.json FirstNexus metadata
---

Reference for plugin packaging (`package.json` metadata), manifests (`FirstNexus.plugin.json`), setup entries, and config schemas.
...
Your `package.json` needs an `FirstNexus` field that tells the plugin system what your plugin provides:

<Tabs>
  <Tab title="Channel plugin">
    ```json
    {
      "name": "@myorg/FirstNexus-my-channel",
      "version": "1.0.0",
      "type": "module",
      "FirstNexus": {
        "extensions": ["./index.ts"],
  ...
  <Tab title="Provider plugin / ClawHub baseline">
    ```json FirstNexus-clawhub-package.json
    {
      "name": "@myorg/FirstNexus-my-plugin",
      "version": "1.0.0",
      "type": "module",
      "FirstNexus": {
        "extensions": ["./index.ts"],
...
`FirstNexus.channel` is cheap package metadata for channel discovery and setup surfaces before runtime loads.
...
`FirstNexus.install` is package metadata, not manifest metadata.
...
| `minHostVersion`             | `string`                            | Minimum supported FirstNexus version in the form `>=x.y.z` or `>=x.y.z-prerelease`. |
...
<AccordionGroup>
  ...
  <Accordion title="Pinned npm installs">
    ...
    ```json
    {
      "FirstNexus": {
        "install": {
          "npmSpec": "@wecom/wecom-FirstNexus-plugin@1.2.3",
...
When enabled, FirstNexus loads only `setupEntry` during the pre-listen startup phase, even for already-configured channels. The full entry loads after the gateway starts listening.
...
Every native plugin must ship an `FirstNexus.plugin.json` in the package root. FirstNexus uses this to validate config without executing plugin code.

```json
{
  "id": "my-plugin",
  "name": "My Plugin",
  "description": "Adds My Plugin capabilities to FirstNexus",
  "configSchema": {
    "type": "object",
...
The `setup-entry.ts` file is a lightweight alternative to `index.ts` that FirstNexus loads when it only needs setup surfaces (onboarding, config repair, disabled channel inspection).
...
If you already author the contract as JSON Schema or TypeBox, use the direct helper so FirstNexus can skip Zod-to-JSON-Schema conversion on metadata paths:

```typescript
import { Type } from "typebox";
import { buildJsonChannelConfigSchema } from "FirstNexus/plugin-sdk/channel-config-schema";
...
For third-party plugins, the cold-path contract is still the plugin manifest: mirror the generated JSON Schema into `FirstNexus.plugin.json#channelConfigs` so config schema, setup, and UI surfaces can inspect `channels.<id>` without loading runtime code.

## Setup wizards

Channel plugins can provide interactive setup wizards for `FirstNexus onboard`. The wizard is a `ChannelSetupWizard` object on the `ChannelPlugin`:

```typescript
import type { ChannelSetupWizard } from "FirstNexus/plugin-sdk/channel-setup";
...
<AccordionGroup>
  <Accordion title="Shared allowFrom prompts">
    For DM allowlist prompts that only need the standard `note -> prompt -> parse -> merge -> patch` flow, prefer the shared setup helpers from `FirstNexus/plugin-sdk/setup`: `createPromptParsedAllowFromForAccount(...)`, `createTopLevelChannelParsedAllowFromPrompt(...)`, and `createNestedChannelParsedAllowFromPrompt(...)`.
  </Accordion>
  <Accordion title="Standard channel setup status">
    For channel setup status blocks that only vary by labels, scores, and optional extra lines, prefer `createStandardChannelSetupStatus(...)` from `FirstNexus/plugin-sdk/setup` instead of hand-rolling the same `status` object in each plugin.
  </Accordion>
  <Accordion title="Optional channel setup surface">
    For optional setup surfaces that should only appear in certain contexts, use `createOptionalChannelSetupSurface` from `FirstNexus/plugin-sdk/channel-setup`:

    ```typescript
    import { createOptionalChannelSetupSurface } from "FirstNexus/plugin-sdk/channel-setup";

    const setupSurface = createOptionalChannelSetupSurface({
      channel: "my-channel",
      label: "My Channel",
      npmSpec: "@myorg/FirstNexus-my-channel",
      docsPath: "/channels/my-channel",
    });
...
<Tabs>
  <Tab title="npm">
    ```bash
    FirstNexus plugins install @myorg/FirstNexus-my-plugin
    ```
  ...
  <Tab title="ClawHub only">
    ```bash
    FirstNexus plugins install clawhub:@myorg/FirstNexus-my-plugin
    ```
  </Tab>
  <Tab title="npm package spec">
    ...
    ```bash
    FirstNexus plugins install npm:@myorg/FirstNexus-my-plugin
    ```
...
```bash
FirstNexus plugins install <package-name>
```

<Info>
For npm-sourced installs, `FirstNexus plugins install` installs the package under `~/.FirstNexus/npm` with lifecycle scripts disabled. Keep plugin dependency trees pure JS/TS and avoid packages that require `postinstall` builds.
</Info>
...
Bundled package metadata is explicit, not inferred from built JavaScript at gateway startup. Runtime dependencies belong in the plugin package that owns them; packaged FirstNexus startup never repairs or mirrors plugin dependencies.
