---
summary: "How FirstNexus installs plugin packages and resolves plugin dependencies"
read_when:
  - You are debugging plugin package installs
  - You are changing plugin startup, doctor, or package-manager install behavior
  - You are maintaining packaged FirstNexus installs or bundled plugin manifests
title: "Plugin dependency resolution"
sidebarTitle: "Dependencies"
---

FirstNexus keeps plugin dependency work at install/update time. Runtime loading
does not run package managers, repair dependency trees, or mutate the FirstNexus
package directory.
...
- SDK/core imports are peer or supplied FirstNexus imports
- local development plugins bring their own already-installed dependencies
- npm and git plugins are installed into FirstNexus-owned package roots

FirstNexus owns only the plugin lifecycle:
...
FirstNexus uses stable per-source roots:

- npm packages install under `~/.FirstNexus/npm`
- git packages clone under `~/.FirstNexus/git`
...
`FirstNexus plugins install npm-pack:<path.tgz>` uses that same managed npm root
for a local npm-pack tarball. FirstNexus reads the tarball's npm metadata, adds it
...
npm may hoist transitive dependencies to `~/.FirstNexus/npm/node_modules` beside
the plugin package. FirstNexus scans the managed npm root before trusting the
...
Plugins that import `FirstNexus/plugin-sdk/*` declare `FirstNexus` as a peer
dependency. FirstNexus does not let npm install a separate registry copy of the
...
resolution/materialization for the shared root and FirstNexus reasserts
plugin-local `node_modules/FirstNexus` links for installed packages that declare
...
Local plugins are treated as developer-controlled directories. FirstNexus does not
...
```bash
FirstNexus plugins update <id>
FirstNexus plugins install <source>
FirstNexus doctor --fix
```

`doctor --fix` can clean legacy FirstNexus-generated dependency state and recover
...
Lightweight and core-critical bundled plugins are shipped as part of FirstNexus.
...
In source checkouts, FirstNexus treats the repository as a pnpm monorepo. After
...
| -------------------------------- | ------------------------------------- | -------------------------------------------------------------------- |
| `npm install -g FirstNexus`      | Built runtime tree inside the package | FirstNexus package and explicit plugin install/update/doctor flows   |
...
| `FirstNexus plugins install ...` | Managed npm/git/ClawHub plugin root   | The plugin install/update flow                                       |

## Legacy cleanup

Older FirstNexus versions generated bundled-plugin dependency roots at startup or
...
`.FirstNexus-runtime-deps*` manifests, generated plugin `node_modules`, install
