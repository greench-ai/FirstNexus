...
- Network path:
  ...
  - Tailnet via unicast DNS-SD (example domain: `FirstNexus.internal.`), **or**
...
Run `FirstNexus devices list` again before approval.
...
FirstNexus gateway call node.list --params "{}"
```
...
- `NEXISCLAW_APNS_RELAY_BASE_URL` still works as a temporary env override for the gateway.
...
- Only genuine FirstNexus iOS builds distributed through Apple can use the hosted relay.
...
- To allow hosted relay usage only for official/TestFlight FirstNexus builds.
...
```bash
export NEXISCLAW_APNS_TEAM_ID="TEAMID"
export NEXISCLAW_APNS_KEY_ID="KEYID"
export NEXISCLAW_APNS_PRIVATE_KEY_P8="$(cat /path/to/AuthKey_KEYID.p8)"
```
...
```bash
mkdir -p ~/.FirstNexus/credentials/apns
chmod 700 ~/.FirstNexus/credentials/apns
mv /path/to/AuthKey_KEYID.p8 ~/.FirstNexus/credentials/apns/AuthKey_KEYID.p8
chmod 600 ~/.FirstNexus/credentials/apns/AuthKey_KEYID.p8
export NEXISCLAW_APNS_PRIVATE_KEY_PATH="$HOME/.FirstNexus/credentials/apns/AuthKey_KEYID.p8"
```
...
The iOS app browses `_FirstNexus-gw._tcp` on `local.` and, when configured, the same
...
`FirstNexus.internal.`) and Tailscale split DNS.
...
```bash
FirstNexus nodes invoke --node "iOS Node" --command canvas.navigate --params '{"url":"http://<gateway-host>:18789/__FirstNexus__/canvas/"}'
```

Notes:

- The Gateway canvas host serves `/__FirstNexus__/canvas/` and `/__FirstNexus__/a2ui/`.
...
tools; the iOS app exposes iPhone capabilities through FirstNexus node commands
such as `canvas.*`, `camera.*`, `screen.*`, `location.*`, and `talk.*`.

Agents can still operate the iOS app through FirstNexus by invoking node
...
```bash
FirstNexus nodes invoke --node "iOS Node" --command canvas.eval --params '{"javaScript":"(() => { const {ctx} = window.__FirstNexus; ctx.clearRect(0,0,innerWidth,innerHeight); ctx.lineWidth=6; ctx.strokeStyle=\"#ff2d55\"; ctx.beginPath(); ctx.moveTo(40,40); ctx.lineTo(innerWidth-40, innerHeight-40); ctx.stroke(); return \"ok\"; })()"}'
```

```bash
FirstNexus nodes invoke --node "iOS Node" --command canvas.snapshot --params '{"maxWidth":900,"format":"jpeg"}'
```
...
- Pairing prompt never appears: run `FirstNexus devices list` and approve manually.
