...
2. `npm i -g FirstNexus@latest`
3. `FirstNexus onboard --install-daemon`
...
FirstNexus installs a systemd **user** service by default. Use a **system**
service for shared or always-on servers. `FirstNexus gateway install` and
`FirstNexus onboard --install-daemon` already render the current canonical unit
...
Create `~/.config/systemd/user/FirstNexus-gateway[-<profile>].service`:

```
[Unit]
Description=FirstNexus Gateway (profile: <profile>, v<version>)
...
sessions and channel connections. FirstNexus therefore biases transient child
processes to be killed before the Gateway when possible.

For eligible Linux child spawns, FirstNexus starts the child through a short
...
- FirstNexus-launched browser/Chrome processes.

The wrapper is Linux-only and is skipped when `/bin/sh` is unavailable. It is
also skipped if the child env sets `NEXISCLAW_CHILD_OOM_SCORE_ADJ=0`, `false`,
`no`, or `off`.
