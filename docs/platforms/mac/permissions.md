...
- Same path: run the app from a fixed location (for FirstNexus, `dist/FirstNexus.app`).
...
```bash
sudo tccutil reset Accessibility ai.FirstNexus.mac
sudo tccutil reset ScreenCapture ai.FirstNexus.mac
...
Workaround: move files into the FirstNexus workspace (`~/.FirstNexus/workspace`) if you want to avoid per-folder grants.
