...
service via the external `FirstNexus` CLI (no embedded runtime). This gives you
...
- The app installs a per-user LaunchAgent labeled `ai.FirstNexus.gateway`
  (or `ai.FirstNexus.<profile>` when using `--profile`/`NEXISCLAW_PROFILE`; legacy `com.FirstNexus.*` is supported).
