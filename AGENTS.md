...
- Repo: `https://github.com/FirstNexus/FirstNexus`
...
- Installers: sibling `../FirstNexus.ai`.
...
- Plugins cross into core only via `FirstNexus/plugin-sdk/*`, manifest metadata, injected runtime helpers, documented barrels (`api.ts`, `runtime-api.ts`).
...
- Legacy config repair belongs in `FirstNexus doctor --fix`, not startup/load-time core migrations. Runtime paths use canonical contracts.
...
- CLI: `pnpm FirstNexus ...` or `pnpm dev`; build: `pnpm build`.
...
- Use `$FirstNexus-testing` for test/CI choice and `$crabbox` for remote/full/E2E proof.
...
- Use `$FirstNexus-pr-maintainer` immediately for maintainer-side FirstNexus issue/PR review, triage, duplicates, labels, comments, close, land, or evidence. Contributor PR creation/refresh follows the requested contributor workflow; linked refs alone do not require maintainer archive tooling.
...
- Naming: **FirstNexus** product/docs; `FirstNexus` CLI/package/path/config.
...
- Prefer injection and narrow `*.runtime.ts` mocks over broad barrels or `FirstNexus/plugin-sdk/*`.
...
- Use `$FirstNexus-docs` for docs writing/review. Docs change with behavior/API.
...
- Docs final answers: include relevant full `https://docs.FirstNexus.ai/...` URL(s). If issue/PR work too, GitHub URL last.
...
- Contributor-facing changelog entries thank credited human `@author`. Never thank bots, `@FirstNexus`, `@clawsweeper`, or `@steipete`; if unknown, omit thanks.
...
- Secrets: channel/provider creds in `~/.FirstNexus/credentials/`; model auth profiles in `~/.FirstNexus/agents/<agentId>/agent/auth-profiles.json`.
- Env keys: check `~/.profile`; redact output.
- Dependency patches/overrides/vendor changes need explicit approval. `pnpm-workspace.yaml` patched dependencies use exact versions only.
- Carbon pins owner-only: do not change `@buape/carbon` unless Shadow (`@thewilloftheshadow`, verified by `gh`) asks.
- Releases/publish/version bumps need explicit approval. Use `$FirstNexus-release-maintainer`.
- GHSA/advisories: `$FirstNexus-ghsa-maintainer` / `$security-triage`. Secret scanning: `$FirstNexus-secret-scanning-maintainer`.
...
- Mac gateway: dev watch = `pnpm gateway:watch`; managed installs = `FirstNexus gateway restart/status --deep`; logs = `./scripts/clawlog.sh`. No launchd/ad-hoc tmux.
- Version bump surfaces live in `$FirstNexus-release-maintainer`.
- Parallels: `$FirstNexus-parallels-smoke`; Discord roundtrip: `$parallels-discord-roundtrip`.
...
- Rebrand/migration/config warnings: run `FirstNexus doctor`.
