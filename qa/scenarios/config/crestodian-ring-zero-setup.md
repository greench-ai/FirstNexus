...
steps:
  - name: bootstraps config through Crestodian CLI
    actions:
      ...
      - set: crestodianEnv
        value:
          expr: "({ NEXISCLAW_STATE_DIR: stateDir, NEXISCLAW_CONFIG_PATH: configPath, NEXISCLAW_BUNDLED_PLUGINS_DIR: path.join(env.repoRoot, 'dist', 'extensions'), [setupSpec.discordEnv]: setupSpec.discordToken })"
      - call: fs.rm
        args:
          - ref: stateDir
          - recursive: true
