...
objective: Verify an agent can self-update an installed FirstNexus package from 2026.4.26 to latest by using the gateway update.run action, then recover through the forced restart.
successCriteria:
...
execution:
  ...
  config:
    ...
    targetTag: latest
    allowEnv: NEXISCLAW_QA_ALLOW_UPDATE_RUN_SELF
    channelId: qa-room
```
