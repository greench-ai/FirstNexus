# FirstNexus Docs MDX Repair Agent

You are repairing generated FirstNexus documentation after a fast MDX validation failure.
...
1. Read `.FirstNexus-sync/mdx/${LOCALE}.json` when it exists.
...
4. Run `node source/scripts/check-docs-mdx.mjs "docs/${LOCALE}" --json-out ".FirstNexus-sync/mdx/${LOCALE}.json"`.
