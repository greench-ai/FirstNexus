...
async function withTempDir<T>(run: (dir: string) => Promise<T>): Promise<T> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "nexusclaw-version-"));
  try {
...
describe("version resolution", () => {
  it("resolves package version from nested dist/plugin-sdk module URL", async () => {
    await withTempDir(async (root) => {
      await writeJsonFixture(root, "package.json", { name: "nexusclaw", version: "1.2.3" });
  ...
  it("ignores unrelated nearby package.json files", async () => {
    await withTempDir(async (root) => {
      await writeJsonFixture(root, "package.json", { name: "nexusclaw", version: "2.3.4" });
  ...
  it("ignores non-nexusclaw package and blank build-info versions", async () => {
    await withTempDir(async (root) => {
  ...
  it("prefers NEXUSCLAW_VERSION over service and package versions", () => {
    expect(
      resolveRuntimeServiceVersion({
        NEXUSCLAW_VERSION: "9.9.9",
        NEXUSCLAW_SERVICE_VERSION: "2.2.2",
  ...
  it("prefers runtime VERSION over service/package markers and ignores blank env values", () => {
    expect(
      resolveRuntimeServiceVersion({
        NEXUSCLAW_VERSION: "   ",
        NEXUSCLAW_SERVICE_VERSION: "  2.0.0  ",
