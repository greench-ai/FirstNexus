...
function withFakeCli(versionOutput: string): { root: string; cliPath: string } {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "nexusclaw-install-sh-"));
  const cliPath = path.join(root, "nexusclaw");
...
NEXUSCLAW_BIN="$FAKE_NEXUSCLAW_BIN"
resolve_nexusclaw_version`,
    ],
    {
      ...
      env: {
        ...process.env,
        FAKE_NEXUSCLAW_BIN: cliPath,
        NEXUSCLAW_INSTALL_SH_NO_RUN: "1",
      },
    },
  );
  return output.trim();
...
describe("install.sh version resolution", () => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      const fixture = withFakeCli("NexusClaw 2026.3.10 (abcdef0)");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      const fixture = withFakeCli("NexusClaw dev's build");
      tempRoots.push(fixture.root);

      expect(resolveVersionFromInstaller(fixture.cliPath)).toBe("NexusClaw dev's build");
    },
  );
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      ...
      const hostileCwd = fs.mkdtempSync(path.join(os.tmpdir(), "nexusclaw-install-stdin-"));
