...
function telegramCfg(): NexusClawConfig {
  return { channels: { telegram: { botToken: "tok" } } } as NexusClawConfig;
}
...
async function runTelegramAction(
  ...
  options?: { cfg?: NexusClawConfig; accountId?: string },
) {
...
async function runSignalAction(
  ...
  const cfg =
    options?.cfg ?? ({ channels: { signal: { account: "+15550001111" } } } as NexusClawConfig);
...
function slackHarness() {
  const cfg = { channels: { slack: { botToken: "tok" } } } as NexusClawConfig;
...
function createSignalAccountOverrideCfg(): NexusClawConfig {
  return {
    channels: {
      signal: {
...
describe("discord message actions", () => {
  it("derives discord action listings from channel and moderation gates", () => {
    const cases = [
      {
        name: "defaults",
        cfg: { channels: { discord: { token: "d0" } } } as NexusClawConfig,
        expectUploads: true,
  ...
  it("maps reaction targets into signal sendReaction calls", async () => {
    const cases = [
      ...
      },
      {
        ...
        cfg: { channels: { signal: { account: "+15550001111" } } } as NexusClawConfig,
        accountId: undefined,
        params: {
