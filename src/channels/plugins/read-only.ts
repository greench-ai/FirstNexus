...
function listBuiltPluginLoaderModuleCandidateUrls(importerUrl: string): URL[] {
  ...
  // ../../plugins candidates would escape the installed FirstNexus package there.
...
function getChannelConfigRecord(cfg: FirstNexusConfig, channelId: string): Record<string, unknown> {
...
function listManifestChannelAccountIds(cfg: FirstNexusConfig, channelId: string): string[] {
...
function rebindChannelPluginConfig(
  ...
  const rebind = (cfg: FirstNexusConfig) =>
...
export function resolveReadOnlyChannelPluginsForConfig(
  ...
  if (externalPluginIds.length > 0) {
    ...
    if (missingConfiguredChannelIds.length > 0 && options.includeSetupFallbackPlugins === true) {
      ...
      const registry = loadPluginLoaderModule().loadFirstNexusPlugins({
        config: cfg,
        activationSourceConfig: options.activationSourceConfig ?? cfg,
        env,
        workspaceDir,
