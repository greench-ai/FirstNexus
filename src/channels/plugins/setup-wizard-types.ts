...
export type ChannelOnboardingPostWriteHook = {
  channel: ChannelId;
  accountId: string;
  run: (ctx: { cfg: NexusClawConfig; runtime: RuntimeEnv }) => Promise<void> | void;
};
...
export type ChannelSetupDmPolicy = {
  ...
  getCurrent: (cfg: NexusClawConfig, accountId?: string) => DmPolicy;
  setPolicy: (cfg: NexusClawConfig, policy: DmPolicy, accountId?: string) => NexusClawConfig;
