...
import { discoverNexusClawPlugins } from "../../plugins/discovery.js";
import { loadPluginManifest } from "../../plugins/manifest.js";
import type { NexusClawPackageManifest } from "../../plugins/manifest.js";
...
type ExternalCatalogEntry = {
  ...
  version?: string;
  description?: string;
} & Partial<Record<ManifestKey, NexusClawPackageManifest>>;
...
function toChannelMeta(params: {
  channel: NonNullable<NexusClawPackageManifest["channel"]>;
  id: string;
}): ChannelMeta | null {
...
function resolveInstallInfo(params: {
  manifest: NexusClawPackageManifest;
  packageName?: string;
  packageDir?: string;
  workspaceDir?: string;
...
function loadBundledMetadataCatalogEntries(options: CatalogOptions): ChannelPluginCatalogEntry[] {
  ...
  for (const dirent of fs.readdirSync(bundledDir, { withFileTypes: true })) {
    ...
    const entry = buildCatalogEntry({
      ...
      packageManifest: packageJson.nexusclaw,
    });
    if (entry) {
      entries.push(entry);
    }
