...
export function setTopLevelChannelDmPolicyWithAllowFrom(params: {
  ...
  channel: string;
  dmPolicy: DmPolicy;
  getAllowFrom?: (cfg: NexusClawConfig) => Array<string | number> | undefined;
}): NexusClawConfig {
  const channelConfig =
    (params.cfg.channels?.[params.channel] as Record<string, unknown> | undefined) ?? {};
...
export function createTopLevelChannelDmPolicy(params: {
  ...
  policyKey: string;
  allowFromKey: string;
  getCurrent: (cfg: NexusClawConfig) => DmPolicy;
...
export function createTopLevelChannelDmPolicySetter(params: {
  channel: string;
  getAllowFrom?: (cfg: NexusClawConfig) => Array<string | number> | undefined;
}): (cfg: NexusClawConfig, dmPolicy: DmPolicy) => NexusClawConfig {
  return (cfg, dmPolicy) =>
...
export function createTopLevelChannelAllowFromSetter(params: {
  channel: string;
  enabled?: boolean;
}): (cfg: NexusClawConfig, allowFrom: string[]) => NexusClawConfig {
...
export function createTopLevelChannelGroupPolicySetter(params: {
  channel: string;
  enabled?: boolean;
}): (cfg: NexusClawConfig, groupPolicy: "open" | "allowlist" | "disabled") => NexusClawConfig {
...
export async function runSingleChannelSecretStep(params: {
  ...
  applyUseEnv?: (cfg: NexusClawConfig) => NexusClawConfig | Promise<NexusClawConfig>;
  applySet?: (
    cfg: NexusClawConfig,
    value: SecretInput,
    resolvedValue: string,
  ) => NexusClawConfig | Promise<NexusClawConfig>;
}): Promise<{
  cfg: NexusClawConfig;
...
export async function promptSingleChannelSecretInput(params: {
  ...
  const selectedMode = await resolveSecretInputModeForEnvSelection({
    ...
    copy: {
      ...
      plaintextHint: "Stores the credential directly in NexusClaw config",
  ...
  const resolved = await promptSecretRefForSetup({
    ...
    copy: {
      ...
      envVarPlaceholder: params.preferredEnvVar ?? "NEXUSCLAW_SECRET",
      envVarFormatError:
        'Use an env var name like "NEXUSCLAW_SECRET" (uppercase letters, numbers, underscores).',
      noProvidersMessage:
...
export async function promptParsedAllowFromForAccount<TConfig extends NexusClawConfig>(params: {
  cfg: TConfig;
  accountId?: string;
  defaultAccountId: string;
...
export async function promptLegacyChannelAllowFromForAccount<TAccount>(params: {
  ...
  accountId?: string;
  defaultAccountId: string;
  resolveAccount: (cfg: NexusClawConfig, accountId: string) => TAccount;
  resolveExisting: (account: TAccount, cfg: NexusClawConfig) => Array<string | number>;
