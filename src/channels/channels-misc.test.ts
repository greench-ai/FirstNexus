...
describe("WA_WEB_AUTH_DIR", () => {
  ...
  it("resolves lazily and caches across the legacy and channels/web entrypoints", async () => {
    const resolveWaWebAuthDir = vi.fn(() => "/tmp/nexusclaw-whatsapp-auth");
    ...
    expect(String(channelWeb.WA_WEB_AUTH_DIR)).toBe("/tmp/nexusclaw-whatsapp-auth");
    expect(String(webEntry.WA_WEB_AUTH_DIR)).toBe("/tmp/nexusclaw-whatsapp-auth");
