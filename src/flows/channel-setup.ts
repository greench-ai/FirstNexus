      ...
      if (adapter) {
        await prompter.note(
          `${channel} plugin not available (continuing with setup). If the channel still doesn't work after setup, run \`${formatCliCommand(
            "FirstNexus plugins list",
          )}\` and \`${formatCliCommand("FirstNexus plugins enable " + channel)}\`, then restart the gateway.`,
          "Channel setup",
        );
        await refreshStatus(channel);
        return true;
    ...
    if (!adapter) {
      await prompter.note(
        `${channel} does not have an interactive setup screen yet. Run ${formatCliCommand(
          `FirstNexus channels add --channel ${channel} --help`,
        )} for supported flags.`,
        "Channel setup",
      );
      return;
      ...
      const choice = await prompter.select({
        ...
        options: [
          ...
          }).map((contribution) => contribution.option),
          {
            value: "__skip__",
            label: "Skip for now",
            hint: `You can add channels later via \`${formatCliCommand("FirstNexus channels add")}\``,
          },
        ],
