...
type SearchConfig = NonNullable<
  NonNullable<NonNullable<FirstNexusConfig["tools"]>["web"]>["search"]
>;
...
function rawKeyValue(config: FirstNexusConfig, provider: SearchProvider): unknown {
...
export function hasExistingKey(config: FirstNexusConfig, provider: SearchProvider): boolean {
  return hasConfiguredSecretInput(rawKeyValue(config, provider));
}

function buildSearchEnvRef(config: FirstNexusConfig, provider: SearchProvider): SecretRef {
  const entry =
...
export function applySearchKey(
  ...
  const nextBase: FirstNexusConfig = {
    ...config,
    tools: {
      ...config.tools,
...
function preserveDisabledState(
  original: FirstNexusConfig,
  result: FirstNexusConfig,
): FirstNexusConfig {
...
export async function runSearchSetupFlow(
  ...
  const choice = await prompter.select({
    ...
    options: [
      ...options,
      {
        ...
        label: "Skip for now",
        hint: "Configure later with FirstNexus configure --section web",
      },
    ],
  ...
  if (!needsCredential) {
    await prompter.note(
      [
        ...
        "FirstNexus will enable the plugin and use it as your web_search provider.",
        `Docs: ${entry.docsUrl ?? "https://docs.FirstNexus.ai/tools/web"}`,
      ].join("\n"),
      "Web search",
    );
    return await finalizeSearchProviderSetup({
  ...
  if (useSecretRefMode) {
    ...
    await prompter.note(
      [
        "Secret references enabled — FirstNexus will store a reference instead of the API key.",
