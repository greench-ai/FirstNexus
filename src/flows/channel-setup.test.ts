...
const resolveAgentWorkspaceDir = vi.hoisted(() =>
  vi.fn((_cfg?: unknown, _agentId?: unknown) => "/tmp/FirstNexus-workspace"),
);
...
describe("setupChannels workspace shadow exclusion", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    resolveAgentWorkspaceDir.mockReturnValue("/tmp/FirstNexus-workspace");
  ...
  it("preloads configured external plugins from the trusted catalog boundary", async () => {
    ...
    expect(trustedInput.workspaceDir).toBe("/tmp/FirstNexus-workspace");
    ...
    expect(registryInput.workspaceDir).toBe("/tmp/FirstNexus-workspace");
  });
  ...
  it("loads the selected bundled catalog plugin without writing explicit plugin enablement", async () => {
    ...
    expect(firstRegistryInput.workspaceDir).toBe("/tmp/FirstNexus-workspace");
    ...
    expect(secondRegistryInput.workspaceDir).toBe("/tmp/FirstNexus-workspace");
  ...
  it(
    ...
    async () => {
      ...
      expect(catalogLookupCall[1]?.workspaceDir).toBe("/tmp/FirstNexus-workspace");
      expect(ensureChannelSetupPluginInstalled).toHaveBeenCalledTimes(1);
      expectExternalCatalogInstallCall();
      expect(note).not.toHaveBeenCalledWith("external-chat plugin not available.", "Channel setup");
      expect(configure).toHaveBeenCalledTimes(1);
