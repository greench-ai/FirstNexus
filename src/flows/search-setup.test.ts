        ...
        plugins: {
          ...
          installs: {
            ...
            [entry.pluginId]: {
              ...
              installPath: `/tmp/FirstNexus-plugins/${entry.pluginId}`,
            },
          },
        },
      },
...
describe("runSearchSetupFlow", () => {
  ...
  it("installs an external catalog search provider before enabling it", async () => {
    ...
    expect(installRequest.entry?.install?.npmSpec).toBe("@FirstNexus/brave-plugin");
    ...
    expect(next.plugins?.installs?.brave?.spec).toBe("@FirstNexus/brave-plugin");
  });
