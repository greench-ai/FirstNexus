...
describe("resolveTaskScriptPath", () => {
  it.each([
    {
      name: "uses default path when NEXUSCLAW_PROFILE is unset",
      env: { USERPROFILE: "C:\\Users\\test" },
      expected: path.join("C:\\Users\\test", ".nexusclaw", "gateway.cmd"),
    },
    {
      name: "uses profile-specific path when NEXUSCLAW_PROFILE is set to a custom value",
      env: { USERPROFILE: "C:\\Users\\test", NEXUSCLAW_PROFILE: "jbphoenix" },
      expected: path.join("C:\\Users\\test", ".nexusclaw-jbphoenix", "gateway.cmd"),
    },
    {
      name: "prefers NEXUSCLAW_STATE_DIR over profile-derived defaults",
      env: {
        USERPROFILE: "C:\\Users\\test",
        NEXUSCLAW_PROFILE: "rescue",
        NEXUSCLAW_STATE_DIR: "C:\\State\\nexusclaw",
      },
      expected: path.join("C:\\State\\nexusclaw", "gateway.cmd"),
    },
    {
      name: "falls back to HOME when USERPROFILE is not set",
      env: { HOME: "/home/test", NEXUSCLAW_PROFILE: "default" },
      expected: path.join("/home/test", ".nexusclaw", "gateway.cmd"),
    },
  ])("$name", ({ env, expected }) => {
...
describe("readScheduledTaskCommand", () => {
  async function withScheduledTaskScript(
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "nexusclaw-schtasks-test-"));
    try {
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          "rem NexusClaw Gateway",
          "cd /d C:\\Projects\\nexusclaw",
          "set NODE_ENV=production",
          "set NEXUSCLAW_PORT=18789",
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"C:\\Program Files\\nodejs\\node.exe" C:\\Users\\test\\AppData\\Roaming\\npm\\node_modules\\nexusclaw\\dist\\index.js gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"\\\\fileserver\\NexusClaw Share\\node.exe" "\\\\fileserver\\NexusClaw Share\\dist\\index.js" gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "\\\\fileserver\\NexusClaw Share\\node.exe",
            "\\\\fileserver\\NexusClaw Share\\dist\\index.js",
            "gateway",
            "--port",
            "18789",
          ],
  ...
  it("reads script from NEXUSCLAW_STATE_DIR override", async () => {
    await withScheduledTaskScript(
      {
        env: (tmpDir) => ({ NEXUSCLAW_STATE_DIR: path.join(tmpDir, "custom-state") }),
