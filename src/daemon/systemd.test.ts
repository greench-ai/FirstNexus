...
const TEST_MANAGED_HOME = "/tmp/nexusclaw-test-home";
const GATEWAY_SERVICE = "nexusclaw-gateway.service";
...
async function expectExecStartWithoutEnvironment(envFileLine: string) {
  mockReadGatewayServiceFile(["[Service]", "ExecStart=/usr/bin/nexusclaw gateway run", envFileLine]);

  const command = await readSystemdServiceExecStart({ HOME: TEST_SERVICE_HOME });
  expect(command?.programArguments).toEqual(["/usr/bin/nexusclaw", "gateway", "run"]);
...
describe("isSystemdServiceEnabled", () => {
  ...
  it("returns false without calling systemctl when the managed unit file is missing", async () => {
    ...
    const result = await isSystemdServiceEnabled({ env: { HOME: "/tmp/nexusclaw-test-home" } });
  ...
  it("throws when systemctl is-enabled fails for non-state errors", async () => {
    ...
    execFileMock
      .mockImplementationOnce((_cmd, args, _opts, cb) => {
        expect(args).toEqual(["--user", "is-enabled", "nexusclaw-gateway.service"]);
      ...
      .mockImplementationOnce((_cmd, args, _opts, cb) => {
        ...
        expect(args.slice(2)).toEqual(["--user", "is-enabled", "nexusclaw-gateway.service"]);
        const err = new Error("permission denied") as Error & { code?: number };
        err.code = 1;
        cb(err, "", "permission denied");
      });
    await expect(
      isSystemdServiceEnabled({ env: { HOME: "/tmp/nexusclaw-test-home" } }),
  ...
  it("returns false when systemctl is-enabled exits with code 4 (not-found)", async () => {
    ...
    execFileMock.mockImplementationOnce((_cmd, _args, _opts, cb) => {
      ...
      const err = new Error(
        "Command failed: systemctl --user is-enabled nexusclaw-gateway.service",
      ) as Error & { code?: number };
      err.code = 4;
      cb(err, "not-found\n", "");
    });
...
describe("isNonFatalSystemdInstallProbeError", () => {
  it("matches wrapper-only WSL install probe failures", () => {
    expect(
      isNonFatalSystemdInstallProbeError(
        new Error("Command failed: systemctl --user is-enabled nexusclaw-gateway.service"),
      ),
    ).toBe(true);
  });
...
describe("resolveSystemdUserUnitPath", () => {
  it.each([
    {
      name: "uses default service name when NEXUSCLAW_PROFILE is unset",
      env: { HOME: "/home/test" },
      expected: "/home/test/.config/systemd/user/nexusclaw-gateway.service",
    },
    {
      name: "uses profile-specific service name when NEXUSCLAW_PROFILE is set to a custom value",
      env: { HOME: "/home/test", NEXUSCLAW_PROFILE: "jbphoenix" },
      expected: "/home/test/.config/systemd/user/nexusclaw-gateway-jbphoenix.service",
    },
    {
      name: "prefers NEXUSCLAW_SYSTEMD_UNIT over NEXUSCLAW_PROFILE",
      env: {
        HOME: "/home/test",
        NEXUSCLAW_PROFILE: "jbphoenix",
        NEXUSCLAW_SYSTEMD_UNIT: "custom-unit",
      },
      expected: "/home/test/.config/systemd/user/custom-unit.service",
    },
    {
      name: "handles NEXUSCLAW_SYSTEMD_UNIT with .service suffix",
      env: {
        HOME: "/home/test",
        NEXUSCLAW_SYSTEMD_UNIT: "custom-unit.service",
      },
      expected: "/home/test/.config/systemd/user/custom-unit.service",
    },
    {
      name: "trims whitespace from NEXUSCLAW_SYSTEMD_UNIT",
      env: {
        HOME: "/home/test",
        NEXUSCLAW_SYSTEMD_UNIT: "  custom-unit  ",
      },
...
describe("splitArgsPreservingQuotes", () => {
  it("splits on whitespace outside quotes", () => {
    expect(splitArgsPreservingQuotes('/usr/bin/nexusclaw gateway start --name "My Bot"')).toEqual([
      "/usr/bin/nexusclaw",
      "gateway",
      "start",
      "--name",
  ...
  it("supports systemd-style backslash escaping", () => {
    expect(
      splitArgsPreservingQuotes('nexusclaw --name "My \\"Bot\\"" --foo bar', {
        escapeMode: "backslash",
      }),
    ).toEqual(["nexusclaw", "--name", 'My "Bot"', "--foo", "bar"]);
  });

  it("supports schtasks-style escaped quotes while preserving other backslashes", () => {
    expect(
      splitArgsPreservingQuotes('nexusclaw --path "C:\\\\Program Files\\\\NexusClaw"', {
        escapeMode: "backslash-quote-only",
      }),
    ).toEqual(["nexusclaw", "--path", "C:\\\\Program Files\\\\NexusClaw"]);

    expect(
      splitArgsPreservingQuotes('nexusclaw --label "My \\"Quoted\\" Name"', {
        escapeMode: "backslash-quote-only",
      }),
    ).toEqual(["nexusclaw", "--label", 'My "Quoted" Name']);
  });
});

describe("parseSystemdExecStart", () => {
  it("preserves quoted arguments", () => {
    const execStart = '/usr/bin/nexusclaw gateway start --name "My Bot"';
...
describe("readSystemdServiceExecStart", () => {
  ...
  it("loads NEXUSCLAW_GATEWAY_TOKEN from EnvironmentFile", async () => {
    const readFileSpy = mockReadGatewayServiceFile(
      ["[Service]", "ExecStart=/usr/bin/nexusclaw gateway run", "EnvironmentFile=%h/.nexusclaw/.env"],
      { [`${TEST_SERVICE_HOME}/.nexusclaw/.env`]: "NEXUSCLAW_GATEWAY_TOKEN=env-file-token\n" },
    );

    const command = await readSystemdServiceExecStart({ HOME: TEST_SERVICE_HOME });
    expect(command?.environment?.NEXUSCLAW_GATEWAY_TOKEN).toBe("env-file-token");
  ...
  it("lets EnvironmentFile override inline Environment values", async () => {
    mockReadGatewayServiceFile(
      [
        "[Service]",
        "ExecStart=/usr/bin/nexusclaw gateway run",
        "EnvironmentFile=%h/.nexusclaw/.env",
        'Environment="NEXUSCLAW_GATEWAY_TOKEN=inline-token"',
      ],
      { [`${TEST_SERVICE_HOME}/.nexusclaw/.env`]: "NEXUSCLAW_GATEWAY_TOKEN=env-file-token\n" },
    );
    ...
    const command = await readSystemdServiceExecStart({ HOME: TEST_SERVICE_HOME });
    expect(command?.environment?.NEXUSCLAW_GATEWAY_TOKEN).toBe("env-file-token");
    expect(command?.environmentValueSources?.NEXUSCLAW_GATEWAY_TOKEN).toBe("file");
  });

  it("ignores missing optional EnvironmentFile entries", async () => {
    await expectExecStartWithoutEnvironment("EnvironmentFile=-%h/.nexusclaw/missing.env");
  });
  ...
  it("supports multiple EnvironmentFile entries and quoted paths", async () => {
    vi.spyOn(fs, "readFile").mockImplementation(async (pathname) => {
      const pathValue = pathLikeToString(pathname);
      if (pathValue.endsWith("/nexusclaw-gateway.service")) {
        return [
          "[Service]",
          "ExecStart=/usr/bin/nexusclaw gateway run",
          'EnvironmentFile=%h/.nexusclaw/first.env "%h/.nexusclaw/second env.env"',
        ].join("\n");
      }
      if (pathValue === "/home/test/.nexusclaw/first.env") {
        return "NEXUSCLAW_GATEWAY_TOKEN=first-token\n"; // pragma: allowlist secret
      }
      if (pathValue === "/home/test/.nexusclaw/second env.env") {
        return 'NEXUSCLAW_GATEWAY_PASSWORD="second password"\n'; // pragma: allowlist secret
      }
      throw new Error(`unexpected readFile path: ${pathValue}`);
    });
    ...
    expect(command?.environment).toEqual({
      NEXUSCLAW_GATEWAY_TOKEN: "first-token",
      NEXUSCLAW_GATEWAY_PASSWORD: "second password", // pragma: allowlist secret
    });
  });
  ...
  it("resolves relative EnvironmentFile paths from the unit directory", async () => {
    vi.spyOn(fs, "readFile").mockImplementation(async (pathname) => {
      ...
      if (pathValue.endsWith("/.config/systemd/user/gateway.env")) {
        return [
          "NEXUSCLAW_GATEWAY_TOKEN=relative-token", // pragma: allowlist secret
          "NEXUSCLAW_GATEWAY_PASSWORD=relative-password", // pragma: allowlist secret
        ].join("\n");
      }
      if (pathValue.endsWith("/.config/systemd/user/override.env")) {
        return "NEXUSCLAW_GATEWAY_TOKEN=override-token\n"; // pragma: allowlist secret
      }
      throw new Error(`unexpected readFile path: ${pathValue}`);
    });
    ...
    const command = await readSystemdServiceExecStart({ HOME: "/home/test" });
    expect(command?.environment).toEqual({
      NEXUSCLAW_GATEWAY_TOKEN: "override-token",
      NEXUSCLAW_GATEWAY_PASSWORD: "relative-password", // pragma: allowlist secret
    });
  });
  ...
  it("parses EnvironmentFile content with comments and quoted values", async () => {
    vi.spyOn(fs, "readFile").mockImplementation(async (pathname) => {
      ...
      if (pathValue.endsWith("/nexusclaw-gateway.service")) {
        return [
          "[Service]",
          "ExecStart=/usr/bin/nexusclaw gateway run",
          "EnvironmentFile=%h/.nexusclaw/gateway.env",
        ].join("\n");
      }
      if (pathValue === "/home/test/.nexusclaw/gateway.env") {
        return [
          "# comment",
          "; another comment",
          'NEXUSCLAW_GATEWAY_TOKEN="quoted token"', // pragma: allowlist secret
          "NEXUSCLAW_GATEWAY_PASSWORD=quoted-password", // pragma: allowlist secret
        ].join("\n");
      }
      throw new Error(`unexpected readFile path: ${pathValue}`);
    });
    ...
    const command = await readSystemdServiceExecStart({ HOME: "/home/test" });
    expect(command?.environment).toEqual({
      NEXUSCLAW_GATEWAY_TOKEN: "quoted token",
      NEXUSCLAW_GATEWAY_PASSWORD: "quoted-password", // pragma: allowlist secret
    });
    expect(command?.environmentValueSources).toEqual({
      NEXUSCLAW_GATEWAY_TOKEN: "file",
      NEXUSCLAW_GATEWAY_PASSWORD: "file", // pragma: allowlist secret
    });
  });
});
...
describe("systemd service control", () => {
  ...
  it("restarts a profile-specific user unit", async () => {
    execFileMock
      .mockImplementationOnce((_cmd, _args, _opts, cb) => cb(null, "", ""))
      .mockImplementationOnce((_cmd, args, _opts, cb) => {
        assertUserSystemctlArgs(args, "restart", "nexusclaw-gateway-work.service");
        cb(null, "", "");
      });
    await assertRestartSuccess({ NEXUSCLAW_PROFILE: "work" });
  });
