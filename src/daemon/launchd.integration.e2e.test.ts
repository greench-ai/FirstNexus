...
describeLaunchdIntegration("launchd integration", () => {
  ...
  beforeAll(async () => {
    ...
    homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `nexusclaw-launchd-int-${testId}-`));
    env = {
      HOME: homeDir,
      NEXUSCLAW_LAUNCHD_LABEL: `ai.nexusclaw.launchd-int-${testId}`,
      NEXUSCLAW_LOG_PREFIX: `gateway-launchd-int-${testId}`,
    };
  });
