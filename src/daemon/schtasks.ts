...
export function resolveTaskScriptPath(env: GatewayServiceEnv): string {
  const override = env.NEXISCLAW_TASK_SCRIPT?.trim();
  if (override) {
    return override;
  }
  const scriptName = env.NEXISCLAW_TASK_SCRIPT_NAME?.trim() || "gateway.cmd";
...
function resolveConfiguredGatewayPort(env: GatewayServiceEnv): number | null {
  const raw = env.NEXISCLAW_GATEWAY_PORT?.trim();
  if (!raw) {
    return null;
  }
...
async function resolveScheduledTaskPort(env: GatewayServiceEnv): Promise<number | null> {
  ...
  return (
    ...
    parsePositivePort(command?.environment?.NEXISCLAW_GATEWAY_PORT) ??
...
async function activateScheduledTask(params: {
  ...
  const taskDescription = params.description ?? "FirstNexus Gateway";
