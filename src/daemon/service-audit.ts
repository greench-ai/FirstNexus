...
function auditGatewayToken(
  ...
  issues.push({
    ...
    message: "Gateway service embeds NEXUSCLAW_GATEWAY_TOKEN and should be reinstalled.",
    detail: "Run `nexusclaw gateway install --force` to remove embedded service token.",
    level: "recommended",
  });
  ...
  issues.push({
    ...
    message:
      "Gateway service NEXUSCLAW_GATEWAY_TOKEN does not match gateway.auth.token in nexusclaw.json",
...
export function readEmbeddedGatewayToken(command: GatewayServiceCommand): string | undefined {
  ...
  if (command.environmentValueSources?.NEXUSCLAW_GATEWAY_TOKEN === "file") {
    return undefined;
  }
  return command.environment?.NEXUSCLAW_GATEWAY_TOKEN?.trim() || undefined;
}
...
export function checkTokenDrift(params: {
  ...
  if (configToken && serviceToken !== configToken) {
    return {
      ...
      detail: "Run `nexusclaw gateway install --force` to sync the token.",
      level: "recommended",
    };
  }
