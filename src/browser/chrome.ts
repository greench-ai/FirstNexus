...
export function resolveNexusClawUserDataDir(profileName = DEFAULT_NEXUSCLAW_BROWSER_PROFILE_NAME) {
...
export async function launchNexusClawChrome(
  ...
  const userDataDir = resolveNexusClawUserDataDir(profile.name);
  fs.mkdirSync(userDataDir, { recursive: true });

  const needsDecorate = !isProfileDecorated(
    userDataDir,
    profile.name,
    (profile.color ?? DEFAULT_NEXUSCLAW_BROWSER_COLOR).toUpperCase(),
  );
  ...
  if (needsDecorate) {
    try {
      decorateNexusClawProfile(userDataDir, {
        name: profile.name,
        color: profile.color,
      });
      log.info(`🦞 nexusclaw browser profile decorated (${profile.color})`);
    } catch (err) {
      log.warn(`nexusclaw browser profile decoration failed: ${String(err)}`);
    }
  }
  ...
  try {
    ...
    log.warn(`nexusclaw browser clean-exit prefs failed: ${String(err)}`);
  }
  ...
  log.info(
    `🦞 nexusclaw browser started (${exe.kind}) profile "${profile.name}" on 127.0.0.1:${profile.cdpPort} (pid ${pid})`,
  );
...
export async function stopNexusClawChrome(
