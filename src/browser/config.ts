import type { BrowserConfig, BrowserProfileConfig, NexusClawConfig } from "../config/config.js";
...
import {
  ...
  DEFAULT_NEXUSCLAW_BROWSER_ENABLED,
...
function normalizeHexColor(raw: string | undefined) {
  ...
  if (!value) {
    return DEFAULT_NEXUSCLAW_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "nexusclaw" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_NEXUSCLAW_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_NEXUSCLAW_BROWSER_PROFILE_NAME] = {
...
export function resolveBrowserConfig(
  ...
  rootConfig?: NexusClawConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_NEXUSCLAW_BROWSER_ENABLED;
  ...
  const defaultProfile =
    ...
    (profiles[DEFAULT_BROWSER_DEFAULT_PROFILE_NAME]
      ? DEFAULT_BROWSER_DEFAULT_PROFILE_NAME
      : profiles[DEFAULT_NEXUSCLAW_BROWSER_PROFILE_NAME]
        ? DEFAULT_NEXUSCLAW_BROWSER_PROFILE_NAME
        : "user");
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "existing-session" ? "existing-session" : "nexusclaw";
