...
function localNexusClawProfile(): Parameters<typeof createProfileResetOps>[0]["profile"] {
  return {
    name: "nexusclaw",
...
function createLocalNexusClawResetOps(
  params: Omit<Parameters<typeof createProfileResetOps>[0], "profile">,
) {
  return createProfileResetOps({ profile: localNexusClawProfile(), ...params });
}
...
function createStatelessResetOps(profile: Parameters<typeof createProfileResetOps>[0]["profile"]) {
  return createProfileResetOps({
    ...
    resolveNexusClawUserDataDir: (name: string) => `/tmp/${name}`,
  });
}
...
describe("createProfileResetOps", () => {
  ...
  it("stops local browser, closes playwright connection, and trashes profile dir", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "nexusclaw-reset-"));
    const profileDir = path.join(tempRoot, "nexusclaw");
    ...
    const ops = createLocalNexusClawResetOps({
      getProfileState,
      stopRunningBrowser,
      isHttpReachable,
      resolveNexusClawUserDataDir: () => profileDir,
    });
  ...
  it("forces playwright disconnect when loopback cdp is occupied by non-owned process", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "nexusclaw-reset-no-own-"));
    const profileDir = path.join(tempRoot, "nexusclaw");
    fs.mkdirSync(profileDir, { recursive: true });
