...
const { stopNexusClawChromeMock } = vi.hoisted(() => ({
  stopNexusClawChromeMock: vi.fn(async () => {}),
}));
...
vi.mock("./chrome.js", () => ({
  stopNexusClawChrome: stopNexusClawChromeMock,
}));
...
describe("stopKnownBrowserProfiles", () => {
  beforeEach(() => {
    ...
    stopNexusClawChromeMock.mockClear();
  });

  it("stops all known profiles and ignores per-profile failures", async () => {
    listKnownProfileNamesMock.mockReturnValue(["nexusclaw", "user"]);
    const stopMap: Record<string, ReturnType<typeof vi.fn>> = {
      nexusclaw: vi.fn(async () => {}),
      user: vi.fn(async () => {
    ...
    expect(stopMap.nexusclaw).toHaveBeenCalledTimes(1);
  ...
  it("stops tracked runtime browsers even when the profile no longer resolves", async () => {
    ...
    expect(stopNexusClawChromeMock).toHaveBeenCalledWith(launchedBrowser);
  ...
  it("warns when profile enumeration fails", async () => {
    ...
    expect(onWarn).toHaveBeenCalledWith("nexusclaw browser stop failed: Error: oops");
  });
});
