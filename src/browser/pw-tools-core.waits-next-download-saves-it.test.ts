...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredNexusClawTmpDir: vi.fn(() => "/tmp/nexusclaw"),
}));
vi.mock("../infra/tmp-nexusclaw-dir.js", () => tmpDirMocks);
...
describe("pw-tools-core", () => {
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredNexusClawTmpDir.mockReturnValue("/tmp/nexusclaw");
  });

  async function withTempDir<T>(run: (tempDir: string) => Promise<T>): Promise<T> {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "nexusclaw-browser-download-test-"));
    try {
  ...
  async function expectAtomicDownloadSave(params: {
    ...
    expect(path.basename(String(savedPath))).toContain(".nexusclaw-output-");
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredNexusClawTmpDir.mockReturnValue("/tmp/nexusclaw-preferred");
    ...
    const expectedRootedDownloadsDir = path.resolve(
      path.join(path.sep, "tmp", "nexusclaw-preferred", "downloads"),
    );
    const expectedDownloadsTail = `${path.join("tmp", "nexusclaw-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredNexusClawTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(String(outPath))).toBe(
      path.resolve(path.join(path.sep, "tmp", "nexusclaw-preferred", "downloads")),
    );
    expect(path.basename(String(outPath))).toMatch(/-passwd$/);
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "nexusclaw-preferred", "downloads")}${path.sep}`),
    );
  });
