...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".nexusclaw-profile-decorated");
}
 ...
 */
export function decorateNexusClawProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_NEXUSCLAW_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_NEXUSCLAW_BROWSER_COLOR).toUpperCase();
