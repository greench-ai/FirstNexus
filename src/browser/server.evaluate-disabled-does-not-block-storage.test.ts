  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        profiles: {
          nexusclaw: { cdpPort: testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
describe("browser control evaluate gating", () => {
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.NEXUSCLAW_GATEWAY_PORT;
    process.env.NEXUSCLAW_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.NEXUSCLAW_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.NEXUSCLAW_GATEWAY_PASSWORD;
