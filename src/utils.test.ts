...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    await withTempDir("nexusclaw-test-", async (tmp) => {
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", async () => {
    await withTempDir("nexusclaw-auth-", (authDir) => {
  ...
  it("falls back through lidMappingDirs in order", async () => {
    await withTempDir("nexusclaw-lid-a-", async (first) => {
      await withTempDir("nexusclaw-lid-b-", (second) => {
...
describe("resolveConfigDir", () => {
  it("prefers ~/.nexusclaw when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "nexusclaw-config-dir-"));
    try {
  ...
  it("expands NEXUSCLAW_STATE_DIR using the provided env", () => {
    const env = {
      HOME: "/tmp/nexusclaw-home",
      NEXUSCLAW_STATE_DIR: "~/state",
    } as NodeJS.ProcessEnv;

    expect(resolveConfigDir(env)).toBe(path.resolve("/tmp/nexusclaw-home", "state"));
  });
});

describe("resolveHomeDir", () => {
  it("prefers NEXUSCLAW_HOME over HOME", () => {
    vi.stubEnv("NEXUSCLAW_HOME", "/srv/nexusclaw-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveHomeDir()).toBe(path.resolve("/srv/nexusclaw-home"));
...
describe("shortenHomePath", () => {
  it("uses $NEXUSCLAW_HOME prefix when NEXUSCLAW_HOME is set", () => {
    vi.stubEnv("NEXUSCLAW_HOME", "/srv/nexusclaw-home");
    vi.stubEnv("HOME", "/home/other");

    expect(shortenHomePath(`${path.resolve("/srv/nexusclaw-home")}/.nexusclaw/nexusclaw.json`)).toBe(
      "$NEXUSCLAW_HOME/.nexusclaw/nexusclaw.json",
    );
...
describe("shortenHomeInString", () => {
  it("uses $NEXUSCLAW_HOME replacement when NEXUSCLAW_HOME is set", () => {
    vi.stubEnv("NEXUSCLAW_HOME", "/srv/nexusclaw-home");
    vi.stubEnv("HOME", "/home/other");

    expect(
      shortenHomeInString(`config: ${path.resolve("/srv/nexusclaw-home")}/.nexusclaw/nexusclaw.json`),
    ).toBe("config: $NEXUSCLAW_HOME/.nexusclaw/nexusclaw.json");
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/nexusclaw", {}, () => "/Users/thoffman")).toBe(
      path.resolve("/Users/thoffman", "nexusclaw"),
    );
  });
  ...
  it("prefers NEXUSCLAW_HOME for tilde expansion", () => {
    vi.stubEnv("NEXUSCLAW_HOME", "/srv/nexusclaw-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveUserPath("~/nexusclaw")).toBe(path.resolve("/srv/nexusclaw-home", "nexusclaw"));
  ...
  it("uses the provided env for tilde expansion", () => {
    ...
    expect(resolveUserPath("~/nexusclaw", env)).toBe(path.resolve("/srv/nexusclaw-home", "nexusclaw"));
  });
