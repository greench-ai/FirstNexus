...
describe("canvas host state dir defaults", () => {
  it("uses NEXUSCLAW_STATE_DIR for the default canvas root", async () => {
    await withStateDirEnv("nexusclaw-canvas-state-", async ({ stateDir }) => {
      ...
      try {
        ...
        expect(indexContents).toContain("NexusClaw Canvas");
      } finally {
        await handler.close();
      }
    });
