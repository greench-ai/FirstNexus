...
import { createFirstNexusChannelMcpServer, FirstNexusChannelBridge } from "./channel-server.js";
...
async function connectMcpWithoutGateway(params?: { claudeChannelMode?: "auto" | "on" | "off" }) {
  const serverHarness = await createFirstNexusChannelMcpServer({
...
describe("FirstNexus channel mcp server", () => {
  ...
  describe("gateway-backed flows", () => {
    describe("gateway integration", () => {
      ...
      test("lists conversations and reads messages", async () => {
        ...
        const bridge = new FirstNexusChannelBridge({} as never, {
        ...
        expect((messages[1]?.__FirstNexus as { id?: string } | undefined)?.id).toBe(
          "msg-attachment",
        );
        expect(
