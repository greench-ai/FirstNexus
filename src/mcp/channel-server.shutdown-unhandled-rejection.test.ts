...
vi.mock("./channel-bridge.js", () => ({
  FirstNexusChannelBridge: class MockFirstNexusChannelBridge {
...
describe("serveFirstNexusChannelMcp shutdown", () => {
  ...
  it("does not leak unhandled rejections when shutdown close fails", async () => {
    ...
    const { serveFirstNexusChannelMcp } = await import("./channel-server.js");

    const servePromise = serveFirstNexusChannelMcp({ verbose: false });
