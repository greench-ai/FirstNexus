...
import { z } from "zod";
import type { FirstNexusChannelBridge } from "./channel-bridge.js";
import {
...
export function registerChannelMcpTools(server: McpServer, bridge: FirstNexusChannelBridge): void {
  server.tool(
    "conversations_list",
    "List FirstNexus channel-backed conversations available through session routes.",
    {
  ...
  server.tool(
    "conversation_get",
    "Get one FirstNexus conversation by session key.",
  ...
  server.tool(
    "messages_read",
    "Read recent messages for one FirstNexus conversation.",
    {
      session_key: z.string().min(1),
  ...
  server.tool(
    "attachments_fetch",
    "List non-text attachments for a message in one FirstNexus conversation.",
    {
      session_key: z.string().min(1),
  ...
  server.tool(
    "events_poll",
    "Poll queued FirstNexus conversation events since a cursor.",
    {
  ...
  server.tool(
    "events_wait",
    "Wait for the next queued FirstNexus conversation event.",
    {
      after_cursor: z.number().int().min(0).optional(),
      session_key: z.string().optional(),
  ...
  server.tool(
    "messages_send",
    "Send a message back through the same FirstNexus conversation route.",
    {
      session_key: z.string().min(1),
      text: z.string().min(1),
    },
  ...
  server.tool(
    ...
    "List open FirstNexus exec or plugin approval requests visible through the Gateway.",
    {},
    async () => {
  ...
  server.tool(
    "permissions_respond",
    "Allow or deny one pending FirstNexus exec or plugin approval request.",
    {
