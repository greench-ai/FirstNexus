/**
 * Standalone MCP server that exposes FirstNexus plugin-registered tools
...
function resolvePluginToolPolicy(config: FirstNexusConfig): {
...
function resolveTools(config: FirstNexusConfig): AnyAgentTool[] {
...
export function createPluginToolsMcpServer(
  ...
  return createToolsMcpServer({ name: "FirstNexus-plugin-tools", tools });
}
