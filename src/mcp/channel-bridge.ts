...
export class FirstNexusChannelBridge {
  ...
  constructor(
    private readonly cfg: FirstNexusConfig,
  ...
  async start(): Promise<void> {
    ...
    this.gateway = new GatewayClientCtor({
      ...
      clientDisplayName: "FirstNexus MCP",
  ...
  async handleClaudePermissionRequest(params: {
    ...
    if (this.verbose) {
      process.stderr.write(`FirstNexus mcp: pending Claude permission ${params.requestId}\n`);
    }
  }
    ...
    try {
      ...
      if (this.verbose && !this.closed) {
        process.stderr.write(
          `FirstNexus mcp: notification ${notification.method} failed: ${String(error)}\n`,
        );
      }
    }
  }
