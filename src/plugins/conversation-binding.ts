...
const APPROVALS_PATH = "~/.FirstNexus/plugin-binding-approvals.json";
...
const LEGACY_CODEX_PLUGIN_SESSION_PREFIXES = [
  "FirstNexus-app-server:thread:",
  "FirstNexus-codex-app-server:thread:",
] as const;
...
const PLUGIN_BINDING_PENDING_REQUESTS_KEY = Symbol.for("FirstNexus.pluginBindingPendingRequests");
...
const pluginBindingGlobalStateKey = Symbol.for("FirstNexus.plugins.binding.global-state");
...
export function buildPluginBindingUnavailableText(binding: PluginConversationBinding): string {
  return `The bound plugin ${resolvePluginBindingDisplayName(binding)} is not currently loaded. Routing this message to FirstNexus instead. If this started after an update, run "FirstNexus doctor --fix"; otherwise reinstall or enable the plugin.${buildDetachHintSuffix(binding.detachHint)}`;
}
