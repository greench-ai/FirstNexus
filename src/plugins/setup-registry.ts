...
function collectConfiguredPluginEntryIds(config: FirstNexusConfig): string[] {
...
function resolveRegister(mod: FirstNexusPluginModule): {
...
function resolveSetupRegistration(record: PluginManifestRecord): {
  ...
  let mod: FirstNexusPluginModule;
  try {
    mod = getModuleLoader(setupSource)(setupSource) as FirstNexusPluginModule;
  } catch {
    return null;
  }

  const resolved = resolveRegister((mod as { default?: FirstNexusPluginModule }).default ?? mod);
...
function pushDescriptorRuntimeDisabledDiagnostic(params: {
  ...
  params.diagnostics.push({
    ...
    message:
      "setup.requiresRuntime is false, so FirstNexus ignored the plugin setup runtime entry. Remove setup-api/FirstNexus.setupEntry or set requiresRuntime true if setup lookup still needs plugin code.",
  });
}
