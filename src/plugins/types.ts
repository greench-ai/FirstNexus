 ...
 */
export type NexusClawPluginConfigSchema = {
...
export type NexusClawPluginToolContext = {
...
export type NexusClawPluginToolFactory = (
  ctx: NexusClawPluginToolContext,
) => AnyAgentTool | AnyAgentTool[] | null | undefined;

export type NexusClawPluginToolOptions = {
  name?: string;
  names?: string[];
  optional?: boolean;
};

export type NexusClawPluginHookOptions = {
  entry?: HookEntry;
  name?: string;
  description?: string;
  register?: boolean;
...
export type ProviderAuthResult = {
   ...
   */
  configPatch?: Partial<NexusClawConfig>;
  defaultModel?: string;
  notes?: string[];
};
...
export type ProviderAuthMethod = {
   ...
   * key + OAuth, or region-specific login flows). NexusClaw uses this to expose
 ...
 *
 * Runs after NexusClaw resolves an explicit/discovered/dynamic model and before
 ...
 *
 * The helper methods cover the common NexusClaw auth resolution paths:
 *
 ...
 *
 * Called when OAuth refresh fails and NexusClaw wants a provider-specific repair
...
/**
 * Provider-owned extra-param normalization before NexusClaw builds its generic
...
/**
 * Provider-owned stream wrapper hook after NexusClaw applies its generic
 ...
 *
 * Return `true` or `false` to override NexusClaw's built-in provider cache TTL
 ...
 *
 * Runs only after NexusClaw exhausts normal env/profile/config auth resolution
 ...
 *
 * Runs after NexusClaw loads the discovered model catalog and merges configured
...
export type ProviderPlugin = {
   ...
   *
   * NexusClaw calls this only from async model resolution paths. After it
  ...
  /**
   * Provider-owned stream wrapper applied after generic NexusClaw wrappers.
   *
   ...
   *
   * Called after NexusClaw resolves the raw configured credential but before the
   ...
   * hint than NexusClaw's generic auth-store guidance.
   */
   ...
   * `errorMessage` when NexusClaw should surface a provider-specific hint for
   ...
   * merging. NexusClaw deduplicates by `provider/id`, so plugins only need to
   ...
   *
   * NexusClaw uses this when a stored auth profile is already valid and needs to
  ...
  /**
   * Legacy auth-profile ids that should be retired by `nexusclaw doctor`.
   *
   ...
   *
   * NexusClaw calls this before falling back to the shared `pi-ai` OAuth
   ...
   * path. Return nothing to keep NexusClaw's generic error text.
   */
  buildAuthDoctorHint?: (
...
export type WebSearchProviderPlugin = {
  ...
  getConfiguredCredentialValue?: (config?: NexusClawConfig) => unknown;
  setConfiguredCredentialValue?: (configTarget: NexusClawConfig, value: unknown) => void;
  applySelectionConfig?: (config: NexusClawConfig) => NexusClawConfig;
...
export type NexusClawPluginGatewayMethod = {
  method: string;
...
export type PluginCommandContext = {
  ...
  commandBody: string;
  /** Current NexusClaw configuration */
  config: NexusClawConfig;
 ...
 */
export type NexusClawPluginCommandDefinition = {
...
export type NexusClawPluginHttpRouteAuth = "gateway" | "plugin";
export type NexusClawPluginHttpRouteMatch = "exact" | "prefix";

export type NexusClawPluginHttpRouteHandler = (
  req: IncomingMessage,
  res: ServerResponse,
) => Promise<boolean | void> | boolean | void;

export type NexusClawPluginHttpRouteParams = {
  path: string;
  handler: NexusClawPluginHttpRouteHandler;
  auth: NexusClawPluginHttpRouteAuth;
  match?: NexusClawPluginHttpRouteMatch;
  replaceExisting?: boolean;
};

export type NexusClawPluginCliContext = {
  program: Command;
  config: NexusClawConfig;
  workspaceDir?: string;
  logger: PluginLogger;
...
export type NexusClawPluginCliRegistrar = (ctx: NexusClawPluginCliContext) => void | Promise<void>;

/** Context passed to long-lived plugin services. */
export type NexusClawPluginServiceContext = {
  config: NexusClawConfig;
  workspaceDir?: string;
  stateDir: string;
  logger: PluginLogger;
...
export type NexusClawPluginService = {
  id: string;
  start: (ctx: NexusClawPluginServiceContext) => void | Promise<void>;
  stop?: (ctx: NexusClawPluginServiceContext) => void | Promise<void>;
};

export type NexusClawPluginChannelRegistration = {
  plugin: ChannelPlugin;
};

/** Module-level plugin definition loaded from a native plugin entry file. */
export type NexusClawPluginDefinition = {
  id?: string;
  name?: string;
  description?: string;
  version?: string;
  ...
  register?: (api: NexusClawPluginApi) => void | Promise<void>;
  activate?: (api: NexusClawPluginApi) => void | Promise<void>;
};

export type NexusClawPluginModule =
  | NexusClawPluginDefinition
  | ((api: NexusClawPluginApi) => void | Promise<void>);

export type PluginRegistrationMode = "full" | "setup-only" | "setup-runtime";

/** Main registration API injected into native plugin entry files. */
export type NexusClawPluginApi = {
  id: string;
  name: string;
  version?: string;
  description?: string;
  ...
  logger: PluginLogger;
  registerTool: (
    tool: AnyAgentTool | NexusClawPluginToolFactory,
    opts?: NexusClawPluginToolOptions,
  ) => void;
  registerHook: (
    events: string | string[],
    handler: InternalHookHandler,
    opts?: NexusClawPluginHookOptions,
  ) => void;
  registerHttpRoute: (params: NexusClawPluginHttpRouteParams) => void;
  /** Register a native messaging channel plugin (channel capability). */
  registerChannel: (registration: NexusClawPluginChannelRegistration | ChannelPlugin) => void;
  registerGatewayMethod: (method: string, handler: GatewayRequestHandler) => void;
  registerCli: (registrar: NexusClawPluginCliRegistrar, opts?: { commands?: string[] }) => void;
  registerService: (service: NexusClawPluginService) => void;
   ...
   */
  registerCommand: (command: NexusClawPluginCommandDefinition) => void;
...
export type PluginFormat = "nexusclaw" | "bundle";
