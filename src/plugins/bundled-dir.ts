...
export function resolveBundledPluginsDir(env: NodeJS.ProcessEnv = process.env): string | undefined {
  const override = env.NEXUSCLAW_BUNDLED_PLUGINS_DIR?.trim();
  if (override) {
  ...
  try {
    const packageRoots = [
      resolveNexusClawPackageRootSync({ cwd: process.cwd() }),
      resolveNexusClawPackageRootSync({ moduleUrl: import.meta.url }),
    ].filter(
