...
import {
  ...
  readFirstNexusManagedNpmRootOverrides,
  repairManagedNpmRootFirstNexusPeer,
...
import { installedPackageNeedsFirstNexusPeerLinkRepair } from "../infra/package-update-utils.js";
import {
...
import {
  linkFirstNexusPeerDependencies,
  relinkFirstNexusPeerDependenciesInManagedNpmRoot,
...
function formatUnresolvedFirstNexusPeerLinkError(packageName: string): string {
  return `Installed plugin ${packageName} declares FirstNexus as a peer dependency, but FirstNexus could not create a plugin-local node_modules/FirstNexus link. Run from a packaged FirstNexus install or reinstall FirstNexus, then retry.`;
}

const MISSING_EXTENSIONS_ERROR =
  'package.json missing FirstNexus.extensions; update the plugin package to include FirstNexus.extensions (for example ["./dist/index.js"]). See https://docs.FirstNexus.ai/help/troubleshooting#plugin-install-fails-with-missing-FirstNexus-extensions';
...
const MANAGED_NPM_PACK_ARCHIVE_DIR = "_FirstNexus-pack-archives";
...
export const PLUGIN_INSTALL_ERROR_CODE = {
  ...
  MISSING_NEXISCLAW_EXTENSIONS: "missing_FirstNexus_extensions",
  MISSING_PLUGIN_MANIFEST: "missing_plugin_manifest",
  EMPTY_NEXISCLAW_EXTENSIONS: "empty_FirstNexus_extensions",
  INVALID_NEXISCLAW_EXTENSIONS: "invalid_FirstNexus_extensions",
...
function ensureFirstNexusExtensions(params: { manifest: PackageManifest }):
  | {
      ok: true;
      entries: string[];
    }
  ...
  if (resolved.status === "missing") {
    return {
      ...
      code: PLUGIN_INSTALL_ERROR_CODE.MISSING_NEXISCLAW_EXTENSIONS,
    };
  }
  if (resolved.status === "empty") {
    return {
      ok: false,
      error: "package.json FirstNexus.extensions is empty",
      code: PLUGIN_INSTALL_ERROR_CODE.EMPTY_NEXISCLAW_EXTENSIONS,
    };
  }
  return {
    ok: true,
...
function compareNpmSemver(a: string, b: string): number {
  const releaseCmp = compareFirstNexusReleaseVersions(a, b);
  if (releaseCmp !== null) {
    return releaseCmp;
  }
...
async function resolveTrustedOfficialPrereleaseResolution(params: {
  ...
  if (!params.spec.name.startsWith("@FirstNexus/")) {
    return null;
  }
  ...
  if (!stableVersion) {
    ...
    if (prereleaseVersion && semverVersions.every(isPrereleaseSemverVersion)) {
      if (prereleaseVersion !== params.resolvedPrereleaseVersion) {
        ...
        params.logger.warn?.(
          `Resolved ${params.spec.raw} to prerelease version ${params.resolvedPrereleaseVersion}; using newest prerelease ${prereleaseSpec} because this trusted official FirstNexus package has no stable npm versions yet.`,
        );
        return { kind: "prerelease-only", resolution: metadataResult.metadata };
      }
      params.logger.warn?.(
        `Resolved ${params.spec.raw} to prerelease version ${params.resolvedPrereleaseVersion}; allowing it because this trusted official FirstNexus package has no stable npm versions yet.`,
      );
  ...
  params.logger.warn?.(
    `Resolved ${params.spec.raw} to prerelease version ${params.resolvedPrereleaseVersion}; falling back to stable ${stableSpec} for this trusted official FirstNexus install.`,
  );
...
async function rollbackManagedNpmPluginInstall(params: {
  ...
  try {
    await relinkFirstNexusPeerDependenciesInManagedNpmRoot({
      npmRoot: params.npmRoot,
...
async function installPluginFromManagedNpmRoot(
  ...
  if (params.packageName !== "FirstNexus") {
    const repairedFirstNexusPeer = await repairManagedNpmRootFirstNexusPeer({
      npmRoot,
      timeoutMs,
      logger,
    });
    if (repairedFirstNexusPeer) {
      logger.info?.(`Repaired stale FirstNexus peer dependency in ${npmRoot}`);
    }
  }
  await upsertManagedNpmRootDependency({
    ...
    managedOverrides: await readFirstNexusManagedNpmRootOverrides(),
  });
  ...
  if (params.packageName !== "FirstNexus") {
    ...
    if (repairedFirstNexusPeer) {
      logger.info?.(`Repaired stale FirstNexus peer dependency in ${npmRoot} after npm install`);
    }
  }
  try {
    await relinkFirstNexusPeerDependenciesInManagedNpmRoot({
    ...
    return {
      ok: false,
      error: `Failed to repair FirstNexus peer links after npm install: ${String(error)}`,
    };
  }
  if (installedPackageNeedsFirstNexusPeerLinkRepair(installRoot)) {
    await rollbackManagedNpmPluginInstall({
      npmRoot,
      packageName: params.packageName,
      targetDir: installRoot,
    ...
    return {
      ok: false,
      error: formatUnresolvedFirstNexusPeerLinkError(params.packageName),
    };
  }
...
async function runInstallSourceScan(params: {
  ...
  try {
    ...
    return {
      ok: false,
      error: `${params.subject} installation blocked: code safety scan failed (${String(err)}). Run "FirstNexus security audit --deep" for details.`,
      code: PLUGIN_INSTALL_ERROR_CODE.SECURITY_SCAN_FAILED,
    };
  }
}
...
async function detectNativePackageInstallSource(packageDir: string): Promise<boolean> {
  ...
  try {
    ...
    return ensureFirstNexusExtensions({ manifest }).ok;
  } catch {
    return false;
  }
}
...
> {
  ...
  const extensionsResult = ensureFirstNexusExtensions({
    manifest,
  });
  ...
  if (!ocManifestResult.ok && params.requirePluginManifest) {
    return {
      ok: false,
      error: `package missing valid FirstNexus.plugin.json: ${ocManifestResult.error}`,
  ...
  if (!minHostVersionCheck.ok) {
    if (minHostVersionCheck.kind === "invalid") {
      return {
        ok: false,
        error: `invalid package.json FirstNexus.install.minHostVersion: ${minHostVersionCheck.error}`,
    ...
    if (minHostVersionCheck.kind === "unknown_host_version") {
      return {
        ok: false,
        error: `plugin "${pluginId}" requires FirstNexus >=${minHostVersionCheck.requirement.minimumLabel}, but this host version could not be determined. Re-run from a released build or set NEXISCLAW_VERSION and retry.`,
        code: PLUGIN_INSTALL_ERROR_CODE.UNKNOWN_HOST_VERSION,
      };
    }
    return {
      ok: false,
      error: `plugin "${pluginId}" requires FirstNexus >=${minHostVersionCheck.requirement.minimumLabel}, but this host is ${minHostVersionCheck.currentVersion}. Upgrade FirstNexus and retry.`,
  ...
  if (!extensionValidation.ok) {
    return {
      ...
      code: PLUGIN_INSTALL_ERROR_CODE.INVALID_NEXISCLAW_EXTENSIONS,
    };
  }
...
async function scanAndLinkInstalledPackage(params: {
  ...
  if (peerLinkRepair.skipped > 0) {
    return {
      ok: false,
      error: formatUnresolvedFirstNexusPeerLinkError(params.pluginId),
    };
  }
  return null;
}
...
export async function installPluginFromArchive(
  ...
  return await runtime.withExtractedArchiveRoot({
    archivePath,
    tempDirPrefix: "FirstNexus-plugin-",
    timeoutMs,
    logger,
