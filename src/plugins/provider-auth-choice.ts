...
function restoreConfiguredPrimaryModel(
  ...
  originalConfig: NexusClawConfig,
): NexusClawConfig {
...
export async function runProviderPluginAuthMethod(params: {
  ...
  opts?: Partial<ProviderAuthOptionBag>;
}): Promise<{ config: NexusClawConfig; defaultModel?: string }> {
...
export async function applyAuthChoicePluginProvider(
  ...
  const agentDir =
    params.agentDir ??
    (agentId === defaultAgentId ? resolveNexusClawAgentDir() : resolveAgentDir(nextConfig, agentId));
  const workspaceDir =
