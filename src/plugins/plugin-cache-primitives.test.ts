...
describe("createConfigScopedPromiseLoader", () => {
  ...
  it("caches loads by config object", async () => {
    const firstConfig = { plugins: { load: { disabled: true } } } as FirstNexusConfig;
    const secondConfig = { plugins: { load: { disabled: false } } } as FirstNexusConfig;
    const load = vi.fn(async (config?: FirstNexusConfig) =>
  ...
  it("clears default and config-scoped entries", async () => {
    ...
    const loader = createConfigScopedPromiseLoader(
      async (owner?: FirstNexusConfig) => `${owner ? "config" : "default"}-${++calls}`,
    );
