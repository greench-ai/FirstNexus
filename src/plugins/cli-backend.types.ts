...
export type PluginTextTransforms = {
  ...
  /** Rewrites applied to inbound assistant text before FirstNexus consumes it. */
...
export type CliBackendPlugin = {
  ...
  /**
   * Whether FirstNexus should inject bundle MCP config for this backend.
   *
   * Keep this opt-in. Only backends that explicitly consume FirstNexus's bundle
   ...
   *
   * Use this when the backend should consume a canonical FirstNexus auth profile
   ...
   * `profile-only` treats the selected FirstNexus auth profile as the sole auth
   ...
   * as static config, such as mapping FirstNexus thinking levels to a backend's
   * native effort flag.
   */
  resolveExecutionArgs?: CliBackendResolveExecutionArgs;
  /**
   * Whether this CLI backend can expose native tools outside FirstNexus's tool
