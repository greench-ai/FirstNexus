...
type LoadFirstNexusPlugins = typeof import("./loader.js").loadFirstNexusPlugins;
...
const loadFirstNexusPluginsMock = vi.fn<LoadFirstNexusPlugins>();
...
function expectLastSetupRegistryCall(params: {
  ...
  const call = loadFirstNexusPluginsMock.mock.calls.at(-1)?.[0];
...
function getLastSetupLoadedPluginConfig() {
  const call = loadFirstNexusPluginsMock.mock.calls.at(-1)?.[0];
  if (!call) {
    throw new Error("expected FirstNexus plugin setup loader to be called");
  }
...
function createAutoEnabledProviderConfig() {
  const rawConfig: FirstNexusConfig = {
    plugins: {},
  };
  const autoEnabledConfig: FirstNexusConfig = {
    ...rawConfig,
    plugins: {
      entries: {
        google: { enabled: true },
...
describe("resolvePluginProviders", () => {
  beforeAll(async () => {
    ...
    vi.doMock("./loader.js", () => ({
      loadFirstNexusPlugins: (...args: Parameters<LoadFirstNexusPlugins>) =>
        loadFirstNexusPluginsMock(...args),
  ...
  beforeEach(() => {
    ...
    applyPluginAutoEnableMock.mockImplementation(
      (params): PluginAutoEnableResult => ({
        config: params.config ?? ({} as FirstNexusConfig),
        changes: [],
        autoEnabledReasons: {},
      }),
    );
