...
describe("loadEnabledBundleMcpConfig", () => {
  it("loads enabled Claude bundle MCP config and absolutizes relative args", async () => {
    ...
    try {
      const homeDir = await tempHarness.createTempDir("nexusclaw-bundle-mcp-home-");
      const workspaceDir = await tempHarness.createTempDir("nexusclaw-bundle-mcp-workspace-");
  ...
  it("merges inline bundle MCP servers and skips disabled bundles", async () => {
    const env = captureEnv(["HOME", "USERPROFILE", "NEXUSCLAW_HOME", "NEXUSCLAW_STATE_DIR"]);
    try {
      const homeDir = await tempHarness.createTempDir("nexusclaw-bundle-inline-home-");
      const workspaceDir = await tempHarness.createTempDir("nexusclaw-bundle-inline-workspace-");
      process.env.HOME = homeDir;
      process.env.USERPROFILE = homeDir;
      delete process.env.NEXUSCLAW_HOME;
      delete process.env.NEXUSCLAW_STATE_DIR;

      const enabledRoot = path.join(homeDir, ".nexusclaw", "extensions", "inline-enabled");
      const disabledRoot = path.join(homeDir, ".nexusclaw", "extensions", "inline-disabled");
  ...
  it("resolves inline Claude MCP paths from the plugin root and expands CLAUDE_PLUGIN_ROOT", async () => {
    const env = captureEnv(["HOME", "USERPROFILE", "NEXUSCLAW_HOME", "NEXUSCLAW_STATE_DIR"]);
    try {
      const homeDir = await tempHarness.createTempDir("nexusclaw-bundle-inline-placeholder-home-");
      const workspaceDir = await tempHarness.createTempDir(
        "nexusclaw-bundle-inline-placeholder-workspace-",
      );
      process.env.HOME = homeDir;
      process.env.USERPROFILE = homeDir;
      delete process.env.NEXUSCLAW_HOME;
      ...
      const pluginRoot = path.join(homeDir, ".nexusclaw", "extensions", "inline-claude");
