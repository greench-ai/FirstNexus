...
describe("updateNpmInstalledPlugins", () => {
  ...
  it("skips integrity drift checks for unpinned npm specs during dry-run updates", async () => {
    installPluginFromNpmSpecMock.mockResolvedValue({
      ok: true,
      pluginId: "opik-nexusclaw",
      targetDir: "/tmp/opik-nexusclaw",
      version: "0.2.6",
      extensions: ["index.ts"],
    });
      ...
      config: {
        plugins: {
          installs: {
            "opik-nexusclaw": {
              source: "npm",
              spec: "@opik/opik-nexusclaw",
              integrity: "sha512-old",
              installPath: "/tmp/opik-nexusclaw",
            },
          },
        },
      },
      pluginIds: ["opik-nexusclaw"],
      dryRun: true,
    });
      ...
      config: {
        plugins: {
          installs: {
            "opik-nexusclaw": {
              source: "npm",
              spec: "@opik/opik-nexusclaw@0.2.5",
              integrity: "sha512-old",
              installPath: "/tmp/opik-nexusclaw",
            },
          },
  ...
  it("formats package-not-found updates with a stable message", async () => {
    installPluginFromNpmSpecMock.mockResolvedValue({
      ...
      error: "Package not found on npm: @nexusclaw/missing.",
    });
    ...
    expect(result.outcomes).toEqual([
      {
        pluginId: "missing",
        status: "error",
        message: "Failed to check missing: npm package not found for @nexusclaw/missing.",
      },
    ]);
  });
  ...
  it("reuses a recorded npm dist-tag spec for id-based updates", async () => {
    installPluginFromNpmSpecMock.mockResolvedValue({
      ok: true,
      pluginId: "nexusclaw-codex-app-server",
      targetDir: "/tmp/nexusclaw-codex-app-server",
        ...
        plugins: {
          installs: {
            "nexusclaw-codex-app-server": {
              ...
              resolvedSpec: "nexusclaw-codex-app-server@0.2.0-beta.3",
            },
          },
        },
      },
    ...
    expect(installPluginFromNpmSpecMock).toHaveBeenCalledWith(
      expect.objectContaining({
        spec: "nexusclaw-codex-app-server@beta",
        expectedPluginId: "nexusclaw-codex-app-server",
      }),
    );
    expect(result.config.plugins?.installs?.["nexusclaw-codex-app-server"]).toMatchObject({
      source: "npm",
      spec: "nexusclaw-codex-app-server@beta",
      installPath: "/tmp/nexusclaw-codex-app-server",
      version: "0.2.0-beta.4",
  ...
  it("uses and persists an explicit npm spec override during updates", async () => {
    installPluginFromNpmSpecMock.mockResolvedValue({
      ...
      extensions: ["index.ts"],
      npmResolution: {
        name: "nexusclaw-codex-app-server",
        version: "0.2.0-beta.4",
        resolvedSpec: "nexusclaw-codex-app-server@0.2.0-beta.4",
      },
    });
  ...
  it("skips recorded integrity checks when an explicit npm version override changes the spec", async () => {
      ...
      config: {
        plugins: {
          installs: {
            "nexusclaw-codex-app-server": {
              source: "npm",
              spec: "nexusclaw-codex-app-server@0.2.0-beta.3",
              integrity: "sha512-old",
              installPath: "/tmp/nexusclaw-codex-app-server",
            },
          },
    ...
    expect(installPluginFromNpmSpecMock).toHaveBeenCalledWith(
      expect.objectContaining({
        spec: "nexusclaw-codex-app-server@0.2.0-beta.4",
        expectedIntegrity: undefined,
      }),
    );
  });
  ...
  it("migrates legacy unscoped install keys when a scoped npm package updates", async () => {
    installPluginFromNpmSpecMock.mockResolvedValue({
      ok: true,
      pluginId: "@nexusclaw/voice-call",
      targetDir: "/tmp/nexusclaw-voice-call",
      version: "0.0.2",
      extensions: ["index.ts"],
    });
    ...
    expect(result.config.plugins?.allow).toEqual(["@nexusclaw/voice-call"]);
    expect(result.config.plugins?.deny).toEqual(["@nexusclaw/voice-call"]);
    expect(result.config.plugins?.slots?.memory).toBe("@nexusclaw/voice-call");
    expect(result.config.plugins?.entries?.["@nexusclaw/voice-call"]).toEqual({
      enabled: false,
      hooks: { allowPromptInjection: false },
    });
    expect(result.config.plugins?.entries?.["voice-call"]).toBeUndefined();
    expect(result.config.plugins?.installs?.["@nexusclaw/voice-call"]).toMatchObject({
      source: "npm",
      spec: "@nexusclaw/voice-call",
      installPath: "/tmp/nexusclaw-voice-call",
      version: "0.0.2",
    });
...
describe("syncPluginsForUpdateChannel", () => {
  ...
  it("uses the provided env when matching bundled load and install paths", async () => {
    const bundledHome = "/tmp/nexusclaw-home";
    resolveBundledPluginSourcesMock.mockReturnValue(
      new Map([
        [
          "feishu",
