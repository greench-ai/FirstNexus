...
import { normalizeFirstNexusVersionBase } from "../config/version.js";
...
function resolveReportedPluginVersion(
  ...
  return (
    normalizeFirstNexusVersionBase(resolveCompatibilityHostVersion(env)) ??
    normalizeFirstNexusVersionBase(plugin.version) ??
    plugin.version
  );
}
...
function buildPluginRecordFromInstalledIndex(
  ...
  const format = plugin.format ?? manifest?.format ?? "FirstNexus";
