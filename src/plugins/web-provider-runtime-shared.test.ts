...
vi.mock("./loader.js", () => ({
  ...
  loadFirstNexusPlugins: mocks.loadFirstNexusPlugins,
...
describe("web-provider-runtime-shared", () => {
  ...
  it("caches setup web provider plugin loads by default", () => {
    ...
    const mapRegistryProviders = vi.fn(() => ["provider"]);
    mocks.loadFirstNexusPlugins.mockReturnValue(loadedRegistry as never);
    ...
    expect(providers).toEqual(["provider"]);
    expect(mockArg(mocks.loadFirstNexusPlugins).cache).toBe(true);
    expect(mockArg(mocks.loadFirstNexusPlugins).onlyPluginIds).toEqual(["brave"]);
  });
  ...
  it("falls back to a scoped provider load when the active runtime registry has no web providers", () => {
    ...
    mocks.loadFirstNexusPlugins.mockReturnValue(fallbackRegistry as never);
  ...
  it("keeps explicit setup web provider cache opt-outs", () => {
    ...
    expect(mockArg(mocks.loadFirstNexusPlugins).cache).toBe(false);
    expect(mockArg(mocks.loadFirstNexusPlugins).onlyPluginIds).toEqual(["brave"]);
  });
});
