...
import { discoverNexusClawPlugins } from "./discovery.js";
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: NexusClawPluginDefinition;
  register?: NexusClawPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as NexusClawPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as NexusClawPluginDefinition;
...
function createPluginRecord(params: {
  ...
  return {
    ...
    format: params.format ?? "nexusclaw",
...
function recordPluginError(params: {
  ...
  const errorText =
    process.env.NEXUSCLAW_PLUGIN_LOADER_DEBUG_STACKS === "1" &&
...
function resolveCandidateDuplicateRank(params: {
  candidate: ReturnType<typeof discoverNexusClawPlugins>["candidates"][number];
...
function compareDuplicateCandidateOrder(params: {
  left: ReturnType<typeof discoverNexusClawPlugins>["candidates"][number];
  right: ReturnType<typeof discoverNexusClawPlugins>["candidates"][number];
  manifestByRoot: Map<string, ReturnType<typeof loadPluginManifestRegistry>["plugins"][number]>;
  provenance: PluginProvenanceIndex;
  env: NodeJS.ProcessEnv;
}): number {
...
export function loadNexusClawPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  if (options.activate === false && options.cache !== false) {
    throw new Error(
      "loadNexusClawPlugins: activate:false requires cache:false to prevent command registry divergence",
    );
  }
  ...
  const discovery = discoverNexusClawPlugins({
    workspaceDir: options.workspaceDir,
  ...
  for (const candidate of orderedCandidates) {
    ...
    if (record.format === "bundle") {
      ...
      for (const capability of unsupportedCapabilities) {
        registry.diagnostics.push({
          ...
          pluginId: record.id,
          source: record.source,
          message: `bundle capability detected but not wired into NexusClaw yet: ${capability}`,
        });
      }
      if (
        enableState.enabled &&
    ...
    let mod: NexusClawPluginModule | null = null;
    try {
      mod = getJiti(safeSource)(safeSource) as NexusClawPluginModule;
    } catch (err) {
      recordPluginError({
        logger,
        registry,
