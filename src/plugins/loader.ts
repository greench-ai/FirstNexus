...
import { discoverFirstNexusPlugins, type PluginCandidate } from "./discovery.js";
...
function createGuardedPluginRegistrationApi(api: FirstNexusPluginApi): {
...
function runPluginRegisterSync(
  ...
  api: Parameters<NonNullable<FirstNexusPluginDefinition["register"]>>[0],
): void {
...
export const __testing = {
  ...
  resolvePluginSdkAliasFile,
  resolvePluginRuntimeModulePath,
  ensureFirstNexusPluginSdkAlias,
  shouldLoadChannelPluginInSetupRuntime,
  shouldPreferNativeModuleLoad,
  toSafeImportPath,
...
export function resolveRuntimePluginRegistry(
  ...
  // plugin registration is still in flight. Let direct loadFirstNexusPlugins(...)
  // callers surface the hard error instead.
  if (isPluginRegistryLoadInFlight(options)) {
    return undefined;
  }
  return loadFirstNexusPlugins(options);
}
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: FirstNexusPluginDefinition;
  register?: FirstNexusPluginDefinition["register"];
} {
  ...
  for (let index = 0; index < candidates.length && index < 12; index += 1) {
    ...
    if (typeof resolved === "function") {
      return {
        register: resolved as FirstNexusPluginDefinition["register"],
      };
    }
    if (resolved && typeof resolved === "object") {
      const def = resolved as FirstNexusPluginDefinition;
...
export function loadFirstNexusPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  try {
    ...
    for (const candidate of orderedCandidates) {
      ...
      if (record.format === "bundle") {
        ...
        for (const capability of unsupportedCapabilities) {
          registry.diagnostics.push({
            ...
            pluginId: record.id,
            source: record.source,
            message: `bundle capability detected but not wired into FirstNexus yet: ${capability}`,
          });
        }
        if (
          enableState.enabled &&
      ...
      let mod: FirstNexusPluginModule | null = null;
      try {
        ...
        mod = withProfile(
          ...
          registrationMode,
          () => loadPluginModule(safeSource) as FirstNexusPluginModule,
        );
      } catch (err) {
        recordPluginError({
          logger,
      ...
      if (registrationPlan.loadSetupEntry && manifestRecord.setupSource) {
        ...
        if (setupRegistration.plugin) {
          ...
          if (
            ...
            let runtimeMod: FirstNexusPluginModule | null = null;
            try {
              runtimeMod = withProfile(
                { pluginId: record.id, source: safeRuntimeSource },
                "load-setup-runtime-entry",
                () => loadPluginModule(safeRuntimeSource) as FirstNexusPluginModule,
              );
            } catch (err) {
              recordPluginError({
                logger,
      ...
      if (failedPlugins.length > 0) {
        logger.warn(
          `[plugins] ${failedPlugins.length} plugin(s) failed to initialize (${formatPluginFailureSummary(
            failedPlugins,
          )}). Run 'FirstNexus plugins list' for details.`,
        );
      }
    }
...
export async function loadFirstNexusPluginCliRegistry(
  options: PluginLoadOptions = {},
