...
function createRepoBundledManifestRegistry(): PluginManifestRegistry {
  return {
    plugins: listRepoBundledPluginManifests().map(({ manifest, dirName }) => ({
      ...
      manifestPath: path.join(repoRoot, "extensions", dirName, "FirstNexus.plugin.json"),
...
describe("bundled plugin metadata", () => {
  ...
  it("prefers built generated paths when present and falls back to source paths", () => {
    const tempRoot = createGeneratedPluginTempRoot("FirstNexus-bundled-plugin-metadata-");
  ...
  it("resolves plugin-local generated entry paths when the plugin dir is provided", () => {
    const tempRoot = createGeneratedPluginTempRoot("FirstNexus-bundled-plugin-metadata-local-");
  ...
  it("scans direct plugin-tree overrides and resolves generated paths from that scan dir", () => {
    const tempRoot = createGeneratedPluginTempRoot("FirstNexus-bundled-plugin-direct-tree-");
    const pluginsDir = path.join(tempRoot, "bundled-plugins");
    const pluginRoot = path.join(pluginsDir, "alpha");

    writeJson(path.join(pluginRoot, "package.json"), {
      name: "@FirstNexus/alpha",
      version: "0.0.1",
      FirstNexus: {
        extensions: ["./index.ts"],
      },
    });
    writeJson(path.join(pluginRoot, "FirstNexus.plugin.json"), {
      id: "alpha",
      channels: ["alpha"],
  ...
  it("reflects bundled manifest edits on the next metadata read", () => {
    const tempRoot = createGeneratedPluginTempRoot("FirstNexus-bundled-plugin-fresh-");
    const pluginRoot = path.join(tempRoot, "extensions", "alpha");
  ...
  it("prefers direct scan-dir overrides over nested dist artifacts within the same override root", () => {
    const pluginsDir = createGeneratedPluginTempRoot("FirstNexus-bundled-plugin-direct-priority-");
    const pluginRoot = path.join(pluginsDir, "alpha");
  ...
  it("resolves bundled repo entry paths from dist before workspace source", () => {
    const tempRoot = createGeneratedPluginTempRoot("FirstNexus-bundled-plugin-repo-entry-");
    const pluginRoot = path.join(tempRoot, "extensions", "alpha");
    const distPluginRoot = path.join(tempRoot, "dist", "extensions", "alpha");
  ...
  it("merges runtime channel schema metadata with manifest-owned channel config fields", () => {
    const tempRoot = createGeneratedPluginTempRoot("FirstNexus-bundled-plugin-channel-configs-");
    ...
    writeJson(path.join(tempRoot, "extensions", "alpha", "FirstNexus.plugin.json"), {
      id: "alpha",
      channels: ["alpha"],
      configSchema: { type: "object" },
      channelConfigs: {
  ...
  it("captures top-level public surface artifacts without duplicating the primary entrypoints", () => {
    const tempRoot = createGeneratedPluginTempRoot("FirstNexus-bundled-plugin-public-artifacts-");
  ...
  it("loads channel config metadata from built public surfaces in dist-only roots", () => {
    const tempRoot = createGeneratedPluginTempRoot("FirstNexus-bundled-plugin-dist-config-");
    ...
    writeJson(path.join(distRoot, "extensions", "alpha", "FirstNexus.plugin.json"), {
      id: "alpha",
      configSchema: {
        type: "object",
        properties: {},
  ...
  it("does not probe broad runtime public surfaces for channel config metadata", () => {
    const tempRoot = createGeneratedPluginTempRoot(
      "FirstNexus-bundled-plugin-dist-config-runtime-",
    );
    const distRoot = path.join(tempRoot, "dist");
