...
type PluginConfig = NonNullable<FirstNexusConfig["plugins"]>;
...
function createPluginConfig(params: {
  ...
  loadPaths?: string[];
  channels?: FirstNexusConfig["channels"];
}): FirstNexusConfig {
...
function createSinglePluginWithEmptySlotsConfig(): FirstNexusConfig {
...
function createSingleNpmInstallConfig(installPath: string): FirstNexusConfig {
  return createPluginConfig({
    entries: createSinglePluginEntries(),
    installs: {
...
describe("removePluginFromConfig", () => {
  ...
  it("removes absolute load path for a workspace-relative install source path", async () => {
    ...
    const tempDir = await fs.mkdtemp(path.join(tempRoot, "FirstNexus-uninstall-portable-source-"));
    try {
    ...
    },
    {
      ...
      config: createPluginConfig({
        ...
        channels: {
          ...
          timbot: { sdkAppId: "123" },
        } as unknown as FirstNexusConfig["channels"],
      }),
      pluginId: "timbot",
      expectedChannels: {
        defaults: { groupPolicy: "opt-in" },
...
describe("uninstallPlugin", () => {
      ...
      config: createPluginConfig({
        installs: {
          "missing-linked-plugin": createPathInstallRecord(
            "/missing/FirstNexus/plugin",
            "/missing/FirstNexus/plugin",
          ),
        },
        loadPaths: ["/missing/FirstNexus/plugin", "/keep/this/plugin"],
      }),
      expectedActions: {
        entry: false,
        install: true,
  ...
  it("uninstalls npm-managed packages through npm before deleting the package directory", async () => {
    ...
    const pluginDir = path.join(npmRoot, "node_modules", "@FirstNexus", "kitchen-sink");
    ...
    const plan = planPluginUninstall({
      config: createPluginConfig({
        entries: createSinglePluginEntries("FirstNexus-kitchen-sink-fixture"),
        installs: {
          "FirstNexus-kitchen-sink-fixture": {
            source: "npm",
            spec: "@FirstNexus/kitchen-sink@1.0.0",
            installPath: pluginDir,
          },
        },
      }),
    ...
    expect(plan.directoryRemoval).toEqual({
      ...
      cleanup: {
        kind: "npm",
        npmRoot,
        packageName: "@FirstNexus/kitchen-sink",
      },
    });
    ...
    expect(applied).toEqual({ directoryRemoved: true, warnings: [] });
    expectNpmUninstallCommand({ packageName: "@FirstNexus/kitchen-sink", npmRoot });
    await expectPathAccessState(pluginDir, "missing");
  });

  it("repairs remaining npm plugin FirstNexus peer links after npm uninstall prunes them", async () => {
    const stateDir = path.join(tempDir, "state");
    const npmRoot = path.join(stateDir, "npm");
    ...
    runCommandWithTimeoutMock.mockImplementationOnce(async (argv: string[]) => {
      ...
      if (!argv.includes("--legacy-peer-deps")) {
        await fs.mkdir(path.join(npmRoot, "node_modules", "FirstNexus"), { recursive: true });
      }
      return {
        code: 0,
        stdout: "",
    ...
    await expectPathAccessState(path.join(npmRoot, "node_modules", "FirstNexus"), "missing");
...
describe("resolveUninstallDirectoryTarget", () => {
  ...
  it("returns managed install path for copied path installs", () => {
    const extensionsDir = path.join(os.tmpdir(), "FirstNexus-uninstall-safe");
  ...
  it("falls back to default path when configured installPath is untrusted", () => {
    ...
    const target = resolveUninstallDirectoryTarget({
      ...
      installRecord: {
        ...
        installPath: "/tmp/not-FirstNexus-plugin-install/my-plugin",
      },
      extensionsDir,
    });
  ...
  it("uses configured installPath when npm installed it under the managed npm root", () => {
    const stateDir = path.join(os.tmpdir(), "FirstNexus-uninstall-safe");
    const extensionsDir = path.join(stateDir, "extensions");
    const installPath = path.join(stateDir, "npm", "node_modules", "@FirstNexus", "kitchen-sink");
    ...
    expect(
      resolveUninstallDirectoryTarget({
        ...
        installRecord: {
          source: "npm",
          spec: "@FirstNexus/kitchen-sink@latest",
          installPath,
        },
        extensionsDir,
      }),
    ...
    expect(
      resolveUninstallDirectoryTarget({
        ...
        installRecord: {
          source: "npm",
          spec: "@FirstNexus/kitchen-sink@latest",
          installPath: path.join(os.tmpdir(), "npm", "node_modules", "@FirstNexus", "kitchen-sink"),
        },
        extensionsDir,
      }),
    ).toBe(resolvePluginInstallDir("FirstNexus-kitchen-sink-fixture", extensionsDir));
  });

  it("uses configured installPath when it is under the recorded managed extensions root", () => {
    const currentExtensionsDir = path.join(
      os.tmpdir(),
      "FirstNexus-uninstall-current",
      "extensions",
    );
    const recordedExtensionsDir = path.join(
      os.tmpdir(),
      "FirstNexus-uninstall-recorded",
      "extensions",
    );
    const installPath = resolvePluginInstallDir("my-plugin", recordedExtensionsDir);
