...
import { resolveNexusClawPackageRootSync } from "../infra/nexusclaw-root.js";
...
function hasTrustedNexusClawRootIndicator(params: {
  packageRoot: string;
  ...
  const hasNexusClawBin =
    (typeof params.packageJson.bin === "string" &&
      params.packageJson.bin.toLowerCase().includes("nexusclaw")) ||
    (typeof params.packageJson.bin === "object" &&
      params.packageJson.bin !== null &&
      typeof params.packageJson.bin.nexusclaw === "string");
  const hasNexusClawEntrypoint = fs.existsSync(path.join(params.packageRoot, "nexusclaw.mjs"));
  return hasCliEntryExport || hasNexusClawBin || hasNexusClawEntrypoint;
}
...
function readPluginSdkSubpathsFromPackageRoot(packageRoot: string): string[] | null {
  ...
  if (!hasTrustedNexusClawRootIndicator({ packageRoot, packageJson: pkg })) {
    return null;
  }
...
function resolveLoaderPluginSdkPackageRoot(
  ...
  const cwd = params.cwd ?? path.dirname(params.modulePath);
  const fromCwd = resolveNexusClawPackageRootSync({ cwd });
  const fromExplicitHints =
    params.argv1 || params.moduleUrl
      ? resolveNexusClawPackageRootSync({
          cwd,
...
export function resolvePluginSdkScopedAliasMap(
  ...
  for (const subpath of listPluginSdkExportedSubpaths({ modulePath })) {
    ...
    for (const kind of orderedKinds) {
      ...
      if (fs.existsSync(candidate)) {
        aliasMap[`nexusclaw/plugin-sdk/${subpath}`] = candidate;
        break;
      }
    }
  }
...
export function buildPluginLoaderAliasMap(modulePath: string): Record<string, string> {
  ...
  return {
    ...(extensionApiAlias ? { "nexusclaw/extension-api": extensionApiAlias } : {}),
    ...(pluginSdkAlias ? { "nexusclaw/plugin-sdk": pluginSdkAlias } : {}),
