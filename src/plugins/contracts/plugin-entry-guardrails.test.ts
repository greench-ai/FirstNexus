...
const FORBIDDEN_CONTRACT_MODULE_SPECIFIER_PATTERNS = [
  /^vitest$/u,
  /^FirstNexus\/plugin-sdk\/testing$/u,
  /(^|\/)test-api(?:\.[cm]?[jt]s)?$/u,
  /(^|\/)__tests__(\/|$)/u,
  /(^|\/)test-support(\/|$)/u,
  /(^|\/)[^/]*\.test(?:[-.][^/]*)?(?:\.[cm]?[jt]s)?$/u,
...
function analyzeSourceModule(params: { filePath: string; source: string }): {
  ...
  for (const match of source.matchAll(STATIC_FROM_IMPORT_RE)) {
    ...
    if (specifier === "FirstNexus/plugin-sdk/core" && importsDefinePluginEntry(importClause)) {
    ...
    for (const plugin of listBundledPluginRoots()) {
      ...
      try {
        const pkg = JSON.parse(readFileSync(packageJsonPath, "utf8")) as {
          FirstNexus?: { extensions?: unknown };
        };
        const extensions = Array.isArray(pkg.FirstNexus?.extensions)
          ? pkg.FirstNexus.extensions
          : [];
        if (
          extensions.some(
  ...
  it("follows relative import edges while scanning guarded contract graphs", () => {
    expect(
      analyzeSourceModule({
        ...
        import { y } from "FirstNexus/plugin-sdk/testing";
      `,
  ...
  it("detects aliased definePluginEntry imports from core", () => {
    expect(
      analyzeSourceModule({
        ...
        source: `
          import { definePluginEntry as dpe } from "FirstNexus/plugin-sdk/core";
          import { somethingElse } from "FirstNexus/plugin-sdk/core";
        `,
      }).importsDefinePluginEntryFromCore,
    ).toBe(true);
  });
