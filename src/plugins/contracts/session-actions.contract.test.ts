...
describe("plugin session actions", () => {
  ...
  it("emits plugin-attributed agent events through the plugin API", () => {
    ...
    const { config, registry } = createPluginRegistryFixture();
    let bundledApi: FirstNexusPluginApi | undefined;
    let workspaceApi: FirstNexusPluginApi | undefined;
    registerTestPlugin({
      registry,
      config,
      record: createPluginRecord({
  ...
  it("blocks agent events from stale and non-activating plugin API closures", () => {
    ...
    const unsubscribe = onAgentEvent((event) => observed.push(event));
    const { config, registry } = createPluginRegistryFixture();
    let capturedApi: FirstNexusPluginApi | undefined;
    registerTestPlugin({
      registry,
      config,
      record: createPluginRecord({
    ...
    try {
      ...
      let neverActiveApi: FirstNexusPluginApi | undefined;
      registerTestPlugin({
      ...
      let inactiveApi: FirstNexusPluginApi | undefined;
      registerTestPlugin({
