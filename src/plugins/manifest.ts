...
export const PLUGIN_MANIFEST_FILENAME = "FirstNexus.plugin.json";
...
export type PluginManifestQaRunner = {
  /** Subcommand mounted beneath `FirstNexus qa`, for example `matrix`. */
  commandName: string;
...
// package.json "FirstNexus" metadata (used for setup/catalog)
...
export type FirstNexusPackageStartup = {
  /**
...
export type FirstNexusPackageSetupFeatures = {
  configPromotion?: boolean;
  legacyStateMigrations?: boolean;
  legacySessionSurfaces?: boolean;
};

export type FirstNexusPackageManifest = {
  extensions?: string[];
  runtimeExtensions?: string[];
  setupEntry?: string;
  runtimeSetupEntry?: string;
  setupFeatures?: FirstNexusPackageSetupFeatures;
  plugin?: {
    id?: string;
    label?: string;
  };
  ...
  startup?: FirstNexusPackageStartup;
};
...
} & Partial<Record<ManifestKey, FirstNexusPackageManifest>>;

export function getPackageManifestMetadata(
  manifest: PackageManifest | undefined,
): FirstNexusPackageManifest | undefined {
  if (!manifest) {
    return undefined;
  }
