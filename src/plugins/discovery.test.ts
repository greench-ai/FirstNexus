...
import { clearPluginDiscoveryCache, discoverNexusClawPlugins } from "./discovery.js";
import {
...
function makeTempDir() {
  return makeTrackedTempDir("nexusclaw-plugins", tempDirs);
}
...
async function discoverWithStateDir(
  stateDir: string,
  params: Parameters<typeof discoverNexusClawPlugins>[0],
) {
  return discoverNexusClawPlugins({ ...params, env: buildDiscoveryEnv(stateDir) });
}
...
function writePluginPackageManifest(params: {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      ...
      nexusclaw: { extensions: params.extensions },
    }),
    "utf-8",
  );
}
...
describe("discoverNexusClawPlugins", () => {
  it("discovers global and workspace extensions", async () => {
    ...
    const workspaceExt = path.join(workspaceDir, ".nexusclaw", "extensions");
  ...
  it("resolves tilde workspace dirs against the provided env", () => {
    ...
    const workspaceExt = path.join(workspaceRoot, ".nexusclaw", "extensions");
    mkdirSafe(workspaceExt);
    fs.writeFileSync(path.join(workspaceExt, "tilde-workspace.ts"), "export default {}", "utf-8");

    const result = discoverNexusClawPlugins({
  ...
  it("derives unscoped ids for scoped packages", async () => {
    ...
    writePluginPackageManifest({
      packageDir: globalExt,
      packageName: "@nexusclaw/voice-call",
  ...
  it("strips provider suffixes from package-derived ids", async () => {
    ...
    writePluginPackageManifest({
      packageDir: globalExt,
      packageName: "@nexusclaw/ollama-provider",
      extensions: ["./src/index.ts"],
    });
    fs.writeFileSync(
      path.join(globalExt, "src", "index.ts"),
  ...
  it("normalizes bundled speech package ids to canonical plugin ids", async () => {
    ...
    writePluginPackageManifest({
      ...
      packageName: "@nexusclaw/elevenlabs-speech",
      extensions: ["./src/index.ts"],
    });
    writePluginPackageManifest({
      packageDir: microsoftDir,
      packageName: "@nexusclaw/microsoft-speech",
      extensions: ["./src/index.ts"],
    });
  ...
  it("treats configured directory paths as plugin packages", async () => {
    ...
    writePluginPackageManifest({
      packageDir: packDir,
      packageName: "@nexusclaw/demo-plugin-dir",
      extensions: ["./index.js"],
    });
  ...
  it("falls back to legacy index discovery when a scanned bundle sidecar is malformed", async () => {
    ...
    expect(legacy?.format).toBe("nexusclaw");
  ...
  it("blocks extension entries that escape package directory", async () => {
    ...
    writePluginPackageManifest({
      packageDir: globalExt,
      packageName: "@nexusclaw/escape-pack",
  ...
  it("reuses discovery results from cache until cleared", async () => {
    ...
    const first = discoverNexusClawPlugins({
      env: {
        ...buildDiscoveryEnv(stateDir),
        NEXUSCLAW_PLUGIN_DISCOVERY_CACHE_MS: "5000",
      },
    });
    ...
    const second = discoverNexusClawPlugins({
      env: {
        ...buildDiscoveryEnv(stateDir),
        NEXUSCLAW_PLUGIN_DISCOVERY_CACHE_MS: "5000",
      },
    ...
    const third = discoverNexusClawPlugins({
      env: {
        ...buildDiscoveryEnv(stateDir),
        NEXUSCLAW_PLUGIN_DISCOVERY_CACHE_MS: "5000",
      },
