...
export type PluginCliCommandGroupEntry = {
  ...
  placeholders: readonly FirstNexusPluginCliCommandDescriptor[];
  names: readonly string[];
  register: (program: FirstNexusPluginCliContext["program"]) => Promise<void>;
};
...
export async function loadPluginCliMetadataRegistryWithContext(
  ...
  return {
    ...context,
    registry: await loadFirstNexusPluginCliRegistry(
...
export async function loadPluginCliCommandRegistryWithContext(params: {
  ...
  return {
    ...params.context,
    registry: loadFirstNexusPlugins(
...
): Promise<FirstNexusPluginCliCommandDescriptor[]> {
  try {
