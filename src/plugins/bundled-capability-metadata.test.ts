  ...
  return fs
    ...
    .filter((pluginDir) => {
      ...
      const packageJson = JSON.parse(fs.readFileSync(packagePath, "utf-8")) as {
        FirstNexus?: FirstNexusPackageManifest;
      };
      return normalizeBundledPluginStringList(packageJson.FirstNexus?.extensions).length > 0;
    })
    .map(
      (pluginDir) =>
        JSON.parse(
          fs.readFileSync(path.join(pluginDir, "FirstNexus.plugin.json"), "utf-8"),
        ) as PluginManifest,
    )
