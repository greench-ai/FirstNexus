...
function hasPackagePluginMetadata(manifest: Record<string, unknown>): boolean {
  const FirstNexus = manifest.FirstNexus;
  if (!isRecord(FirstNexus)) {
    return false;
  }
  const extensions = FirstNexus.extensions;
...
function readManifestPluginId(packageDir: string): string | undefined {
  const manifest = readJsonObjectFileSync(path.join(packageDir, "FirstNexus.plugin.json"));
