...
describe("bundle plugin hooks", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fsp.mkdtemp(path.join(os.tmpdir(), "nexusclaw-plugin-hooks-"));
  });
  ...
  beforeEach(async () => {
    ...
    previousBundledHooksDir = process.env.NEXUSCLAW_BUNDLED_HOOKS_DIR;
    process.env.NEXUSCLAW_BUNDLED_HOOKS_DIR = "/nonexistent/bundled/hooks";
  });

  afterEach(() => {
    clearInternalHooks();
    if (previousBundledHooksDir === undefined) {
      delete process.env.NEXUSCLAW_BUNDLED_HOOKS_DIR;
    } else {
      process.env.NEXUSCLAW_BUNDLED_HOOKS_DIR = previousBundledHooksDir;
    }
  });
  ...
  async function writeBundleHookFixture(): Promise<string> {
    ...
    await fsp.writeFile(
      ...
      [
        ...
        'metadata: {"nexusclaw":{"events":["command:new"]}}',
        "---",
        "",
        "# Bundle hook",
        "",
  ...
  function createConfig(enabled: boolean): NexusClawConfig {
    return {
      hooks: {
        internal: {
          enabled: true,
  ...
  it("exposes enabled bundle hook dirs as plugin-managed hook entries", async () => {
    ...
    expect(entries[0]?.hook.source).toBe("nexusclaw-plugin");
  ...
  it("does not treat Claude hooks.json bundles as NexusClaw hook packs", async () => {
    const bundleRoot = path.join(workspaceDir, ".nexusclaw", "extensions", "claude-bundle");
