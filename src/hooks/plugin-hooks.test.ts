...
describe("bundle plugin hooks", () => {
  ...
  beforeEach(async () => {
    ...
    previousBundledHooksDir = process.env.NEXISCLAW_BUNDLED_HOOKS_DIR;
  ...
  afterEach(() => {
    ...
    if (previousBundledHooksDir === undefined) {
      delete process.env.NEXISCLAW_BUNDLED_HOOKS_DIR;
    } else {
      process.env.NEXISCLAW_BUNDLED_HOOKS_DIR = previousBundledHooksDir;
    }
  });
