...
describe("hook policy", () => {
  describe("resolveHookEnableState", () => {
    it("keeps workspace hooks disabled by default", () => {
      const entry = makeHookEntry("workspace-hook", "nexusclaw-workspace");
    ...
    it("keeps plugin hooks enabled without local hook toggles", () => {
      const entry = makeHookEntry("plugin-hook", "nexusclaw-plugin");
  ...
  describe("resolveHookEntries", () => {
    it("lets managed hooks override bundled and plugin hooks", () => {
      const bundled = makeHookEntry("shared", "nexusclaw-bundled");
      const plugin = makeHookEntry("shared", "nexusclaw-plugin");
      const managed = makeHookEntry("shared", "nexusclaw-managed");

      const resolved = resolveHookEntries([bundled, plugin, managed]);
      expect(resolved).toHaveLength(1);
      expect(resolved[0]?.hook.source).toBe("nexusclaw-managed");
    });

    it("prevents workspace hooks from overriding non-workspace hooks", () => {
      const managed = makeHookEntry("shared", "nexusclaw-managed");
      const workspace = makeHookEntry("shared", "nexusclaw-workspace");
    ...
    it("keeps later workspace entries for the same source/name", () => {
      const first = makeHookEntry("shared", "nexusclaw-workspace");
      const second = makeHookEntry("shared", "nexusclaw-workspace");
      second.hook.handlerPath = "/tmp/nexusclaw-workspace/shared/handler-2.js";
