...
describe("hook policy", () => {
  describe("resolveHookEnableState", () => {
    it("keeps workspace hooks disabled by default", () => {
      const entry = makeHookEntry("workspace-hook", "FirstNexus-workspace");
    ...
    it("keeps plugin hooks enabled without local hook toggles", () => {
      const entry = makeHookEntry("plugin-hook", "FirstNexus-plugin");
  ...
  describe("resolveHookEntries", () => {
    it("lets managed hooks override bundled and plugin hooks", () => {
      const bundled = makeHookEntry("shared", "FirstNexus-bundled");
      const plugin = makeHookEntry("shared", "FirstNexus-plugin");
      const managed = makeHookEntry("shared", "FirstNexus-managed");

      const resolved = resolveHookEntries([bundled, plugin, managed]);
      expect(resolved).toHaveLength(1);
      expect(resolved[0]?.hook.source).toBe("FirstNexus-managed");
    });

    it("prevents workspace hooks from overriding non-workspace hooks", () => {
      const managed = makeHookEntry("shared", "FirstNexus-managed");
      const workspace = makeHookEntry("shared", "FirstNexus-workspace");
    ...
    it("keeps later workspace entries for the same source/name", () => {
      const first = makeHookEntry("shared", "FirstNexus-workspace");
      const second = makeHookEntry("shared", "FirstNexus-workspace");
      second.hook.handlerPath = "/tmp/FirstNexus-workspace/shared/handler-2.js";
