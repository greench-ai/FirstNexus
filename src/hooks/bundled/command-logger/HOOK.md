...
homepage: https://docs.nexusclaw.ai/automation/hooks#command-logger
metadata:
  {
    "nexusclaw":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.nexusclaw/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.nexusclaw/logs/commands.log ~/.nexusclaw/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/nexusclaw`:
   ```
   /home/username/.nexusclaw/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
...
```bash
tail -n 20 ~/.nexusclaw/logs/commands.log
```

Pretty-print with jq:

```bash
cat ~/.nexusclaw/logs/commands.log | jq .
```

Filter by action:

```bash
grep '"action":"new"' ~/.nexusclaw/logs/commands.log | jq .
```
