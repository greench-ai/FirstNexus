...
homepage: https://docs.FirstNexus.ai/automation/hooks#command-logger
metadata:
  {
    "FirstNexus":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.FirstNexus/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.FirstNexus/logs/commands.log ~/.FirstNexus/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/FirstNexus`:
   ```
   /home/username/.FirstNexus/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
