...
This directory contains hooks that ship with NexusClaw. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
**Output**: `<workspace>/memory/YYYY-MM-DD-slug.md` (defaults to `~/.nexusclaw/workspace`)

**Enable**:

```bash
nexusclaw hooks enable session-memory
```
...
```bash
nexusclaw hooks enable bootstrap-extra-files
```
...
**Output**: `~/.nexusclaw/logs/commands.log`

**Enable**:

```bash
nexusclaw hooks enable command-logger
```
...
```bash
nexusclaw hooks enable boot-md
```
...
homepage: https://docs.nexusclaw.ai/automation/hooks#my-hook
metadata:
  { "nexusclaw": { "emoji": "🔗", "events": ["command:new"], "requires": { "bins": ["node"] } } }
---
# Hook Title
...
- **Managed hooks**: `~/.nexusclaw/hooks/` (shared across workspaces)
...
```bash
nexusclaw hooks list
```

Show hook details:

```bash
nexusclaw hooks info session-memory
```

Check hook status:

```bash
nexusclaw hooks check
```
...
```bash
nexusclaw hooks enable session-memory
nexusclaw hooks disable command-logger
```

## Configuration

Hooks can be configured in `~/.nexusclaw/nexusclaw.json`:
...
2. Restart gateway: `pkill -9 -f 'nexusclaw.*gateway' && pnpm nexusclaw gateway`
3. Enable the hook: `nexusclaw hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.nexusclaw.ai/automation/hooks
