...
This directory contains hooks that ship with FirstNexus. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
**Output**: `<workspace>/memory/YYYY-MM-DD-slug.md` (defaults to `~/.FirstNexus/workspace`)
...
homepage: https://docs.FirstNexus.ai/automation/hooks#my-hook
metadata:
...
- **Managed hooks**: `~/.FirstNexus/hooks/` (shared across workspaces)
...
Hooks can be configured in `~/.FirstNexus/FirstNexus.json`:
...
2. Restart gateway: `pkill -9 -f 'FirstNexus.*gateway' && pnpm FirstNexus gateway`
3. Enable the hook: `FirstNexus hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.FirstNexus.ai/automation/hooks
