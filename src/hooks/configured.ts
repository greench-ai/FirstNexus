...
export function hasConfiguredInternalHooks(config: FirstNexusConfig): boolean {
...
export function resolveConfiguredInternalHookNames(config: FirstNexusConfig): Set<string> | null {
  const internal = config.hooks?.internal;
  if (!internal || internal.enabled === false) {
    return new Set();
  }
