...
const fixtureRoot = path.join(process.cwd(), ".tmp", `FirstNexus-hook-install-${randomUUID()}`);
...
const zipHooksBuffer = await createZipHookPackBuffer({
  packageName: "@FirstNexus/zip-hooks",
  hookName: "zip-hook",
...
const npmPackHooksBuffer = await createTarGzHookPackBuffer({
  packageName: "@FirstNexus/test-hooks",
  hookName: "one-hook",
...
function writeHookPackManifest(params: {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      name: "@FirstNexus/test-hooks",
      version: "0.0.1",
      FirstNexus: { hooks: params.hooks },
...
async function createZipHookPackBuffer(params: {
  ...
  const packageJson = JSON.stringify({
    ...
    version: "0.0.1",
    FirstNexus: { hooks: [`./hooks/${params.hookName}`] },
  });
    ...
    const cases = [
      {
        hooks: ["../outside"],
        setupLink: false,
        expected: "FirstNexus.hooks entry escapes package directory",
      },
      {
        hooks: ["./linked"],
        setupLink: true,
        expected: "FirstNexus.hooks entry resolves outside package directory",
      },
    ] as const;
...
describe("installHooksFromNpmSpec", () => {
  it("does not expose dangerous force unsafe install through npm-spec archive params", async () => {
    ...
    try {
      const result = await installHooksFromNpmSpec({
        spec: "@FirstNexus/test-hooks@0.0.1",
      });
  ...
  it("uses --ignore-scripts for npm pack and cleans up temp dir", async () => {
      ...
      if (argv[0] === "npm" && argv[1] === "pack") {
        ...
        return {
          ...
          stdout: JSON.stringify([
            {
              id: "@FirstNexus/test-hooks@0.0.1",
              name: "@FirstNexus/test-hooks",
              version: "0.0.1",
              filename: packedName,
    ...
    expect(result.hookPackId).toBe("test-hooks");
    expect(result.npmResolution?.resolvedSpec).toBe("@FirstNexus/test-hooks@0.0.1");
    ...
    expectSingleNpmPackIgnoreScriptsCall({
      ...
      expectedSpec: "@FirstNexus/test-hooks@0.0.1",
    });
  ...
  it("rejects invalid npm spec shapes", async () => {
    ...
    const run = runCommandWithTimeoutMock;
    mockNpmPackMetadataResult(run, {
      id: "@FirstNexus/test-hooks@0.0.2-beta.1",
      name: "@FirstNexus/test-hooks",
    ...
    const result = await installHooksFromNpmSpec({
      spec: "@FirstNexus/test-hooks",
      logger: { info: () => {}, warn: () => {} },
    });
    expect(result.ok).toBe(false);
    if (!result.ok) {
      expect(result.error).toContain("prerelease version 0.0.2-beta.1");
      expect(result.error).toContain('"@FirstNexus/test-hooks@beta"');
    }
  });
});
