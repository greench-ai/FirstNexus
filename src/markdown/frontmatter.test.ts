...
`;
    const result = parseFrontmatterBlock(content);
    expect(result.metadata).toBe('{"FirstNexus":{"emoji":"disk","events":["command:new"]}}');

    const parsed = JSON5.parse(result.metadata);
    expect(parsed.FirstNexus?.emoji).toBe("disk");
  });
...
---`;
    const result = parseFrontmatterBlock(content);
    expect(result.metadata).toBe('{"FirstNexus":true}');
  });
