...
describe("shared/assistant-identity-values", () => {
  ...
  it("trims values and preserves strings within the limit", () => {
    expect(coerceIdentityValue("  NexusClaw  ", 20)).toBe("NexusClaw");
    expect(coerceIdentityValue("  NexusClaw  ", 8)).toBe("NexusClaw");
  });

  it("truncates overlong trimmed values at the exact limit", () => {
    expect(coerceIdentityValue("  NexusClaw Assistant  ", 8)).toBe("NexusClaw");
  });

  it("returns an empty string when truncating to a zero-length limit", () => {
    expect(coerceIdentityValue("  NexusClaw  ", 0)).toBe("");
    expect(coerceIdentityValue("  NexusClaw  ", -1)).toBe("OpenCla");
  });
});
