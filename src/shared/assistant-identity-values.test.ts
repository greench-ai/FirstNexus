...
describe("shared/assistant-identity-values", () => {
  ...
  it("trims values and preserves strings within the limit", () => {
    expect(coerceIdentityValue("  FirstNexus  ", 20)).toBe("FirstNexus");
    expect(coerceIdentityValue("  FirstNexus  ", 8)).toBe("FirstNexus");
  });

  it("truncates overlong trimmed values at the exact limit", () => {
    expect(coerceIdentityValue("  FirstNexus Assistant  ", 8)).toBe("FirstNexus");
  });

  it("returns an empty string when truncating to a zero-length limit", () => {
    expect(coerceIdentityValue("  FirstNexus  ", 0)).toBe("");
    expect(coerceIdentityValue("  FirstNexus  ", -1)).toBe("OpenCla");
  });
});
