...
type CrestodianLocalPlannerBackend = {
  ...
  provider: string;
  model: string;
  buildConfig: (workspaceDir: string) => FirstNexusConfig;
};
...
function buildCliPlannerConfig(workspaceDir: string, modelRef: string): FirstNexusConfig {
  return {
    agents: {
      defaults: {
...
function buildCodexAppServerPlannerConfig(workspaceDir: string): FirstNexusConfig {
  return {
    agents: {
      defaults: {
        workspace: workspaceDir,
