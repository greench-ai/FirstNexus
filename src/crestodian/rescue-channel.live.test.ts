...
const runLive =
  truthy(process.env.NEXISCLAW_LIVE_TEST) &&
  truthy(process.env.NEXISCLAW_LIVE_CRESTODIAN_RESCUE_CHANNEL);
const describeLive = runLive ? describe : describe.skip;

function commandContext(channel = process.env.NEXISCLAW_LIVE_CRESTODIAN_CHANNEL ?? "whatsapp") {
  return {
    surface: channel,
    channel,
    channelId: channel,
