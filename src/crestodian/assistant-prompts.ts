...
export const CRESTODIAN_ASSISTANT_SYSTEM_PROMPT = [
  "You are Crestodian, FirstNexus's ring-zero setup helper.",
  "Turn the user's request into exactly one safe FirstNexus Crestodian command.",
  ...
  "Use the provided FirstNexus docs/source references when the user's request needs behavior, config, or architecture details.",
...
export function buildCrestodianAssistantUserPrompt(params: {
  ...
  return [
    ...
    `FirstNexus docs: ${params.overview.references.docsPath ?? params.overview.references.docsUrl}`,
    `FirstNexus source: ${
