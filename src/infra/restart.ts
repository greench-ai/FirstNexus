...
export function triggerNexusClawRestart(): RestartAttempt {
  ...
  if (process.platform === "linux") {
    const unit = normalizeSystemdUnit(
      process.env.NEXUSCLAW_SYSTEMD_UNIT,
      process.env.NEXUSCLAW_PROFILE,
    );
  ...
  const label =
    process.env.NEXUSCLAW_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.NEXUSCLAW_PROFILE);
