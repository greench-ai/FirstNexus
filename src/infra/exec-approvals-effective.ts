...
const DEFAULT_HOST_PATH = "~/.FirstNexus/exec-approvals.json";
...
function formatRequestedSource(params: {
  ...
  return params.sourcePath === "__default__"
    ? `FirstNexus default (${params.defaultValue})`
...
function formatHostFieldSource(params: {
  ...
  if (params.field === "askFallback") {
    return `FirstNexus default (${DEFAULT_EXEC_APPROVAL_ASK_FALLBACK})`;
  }
