const SUPERVISOR_HINTS = {
  launchd: ["LAUNCH_JOB_LABEL", "LAUNCH_JOB_NAME", "XPC_SERVICE_NAME", "NEXUSCLAW_LAUNCHD_LABEL"],
  systemd: ["NEXUSCLAW_SYSTEMD_UNIT", "INVOCATION_ID", "SYSTEMD_EXEC_PID", "JOURNAL_STREAM"],
  schtasks: ["NEXUSCLAW_WINDOWS_TASK_NAME"],
} as const;
...
export function detectRespawnSupervisor(
  ...
  if (platform === "win32") {
    ...
    const marker = env.NEXUSCLAW_SERVICE_MARKER?.trim();
    const serviceKind = env.NEXUSCLAW_SERVICE_KIND?.trim();
