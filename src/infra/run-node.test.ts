...
async function withTempDir<T>(run: (dir: string) => Promise<T>): Promise<T> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "nexusclaw-run-node-"));
  try {
...
describe("run-node script", () => {
    ...
    async () => {
      await withTempDir(async (tmp) => {
        ...
        const exitCode = await runNodeMain({
          ...
          env: {
            ...process.env,
            NEXUSCLAW_FORCE_BUILD: "1",
            NEXUSCLAW_RUNNER_LOG: "0",
          },
          spawn,
        ...
        expect(nodeCalls).toEqual([
          ...
          [process.execPath, "nexusclaw.mjs", "--version"],
        ]);
      });
    },
  );
  ...
  it("copies bundled plugin metadata after rebuilding from a clean dist", async () => {
    await withTempDir(async (tmp) => {
      const extensionManifestPath = path.join(tmp, "extensions", "demo", "nexusclaw.plugin.json");
      ...
      expect(spawnCalls).toEqual([
        expectedBuildSpawn(),
        [process.execPath, "nexusclaw.mjs", "status"],
      ]);
      ...
      await expect(
        fs
          .readFile(path.join(tmp, "dist", "extensions", "demo", "nexusclaw.plugin.json"), "utf-8")
  ...
  it("skips rebuilding when dist is current and the source tree is clean", async () => {
    await withTempDir(async (tmp) => {
      ...
      await fs.writeFile(packageJsonPath, '{"name":"nexusclaw-test"}\n', "utf-8");
      ...
      expect(exitCode).toBe(0);
      expect(spawnCalls).toEqual([[process.execPath, "nexusclaw.mjs", "status"]]);
    });
  });
  ...
  it("skips rebuilding when extension package metadata is newer than the build stamp", async () => {
    await withTempDir(async (tmp) => {
      const manifestPath = path.join(tmp, "extensions", "demo", "nexusclaw.plugin.json");
      ...
      await fs.writeFile(
        packagePath,
        '{"name":"demo","nexusclaw":{"extensions":["./index.ts"]}}\n',
        "utf-8",
      );
      await fs.writeFile(tsconfigPath, "{}\n", "utf-8");
      await fs.writeFile(packageJsonPath, '{"name":"nexusclaw-test"}\n', "utf-8");
      ...
      await fs.writeFile(
        distPackagePath,
        '{"name":"demo","nexusclaw":{"extensions":["./stale.js"]}}\n',
        "utf-8",
      );
      await fs.writeFile(buildStampPath, '{"head":"abc123"}\n', "utf-8");
  ...
  it("skips rebuilding for dirty extension manifests that only affect runtime reload", async () => {
    await withTempDir(async (tmp) => {
      const srcPath = path.join(tmp, "src", "index.ts");
      const manifestPath = path.join(tmp, "extensions", "demo", "nexusclaw.plugin.json");
      const distManifestPath = path.join(tmp, "dist", "extensions", "demo", "nexusclaw.plugin.json");
      const distEntryPath = path.join(tmp, "dist", "entry.js");
      const buildStampPath = path.join(tmp, "dist", ".buildstamp");
      const tsconfigPath = path.join(tmp, "tsconfig.json");
      const packageJsonPath = path.join(tmp, "package.json");
      ...
      const spawnSync = (cmd: string, args: string[]) => {
        ...
        if (cmd === "git" && args[0] === "status") {
          return { status: 0, stdout: " M extensions/demo/nexusclaw.plugin.json\n" };
        }
        return { status: 1, stdout: "" };
      };
