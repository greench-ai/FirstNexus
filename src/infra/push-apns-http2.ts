  ...
  try {
    return await new Promise<ProbeApnsHttp2ReachabilityViaProxyResult>((resolve, reject) => {
      ...
      const request = session.request({
        ...
        authorization: "bearer intentionally.invalid.FirstNexus.proxy.validation",
        "apns-topic": "ai.FirstNexus.ios",
      ...
      request.end(JSON.stringify({ aps: { alert: "FirstNexus APNs proxy validation" } }));
    });
  } finally {
