...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.NEXUSCLAW_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*nexusclaw-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "nexusclaw";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "nexusclaw");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "nexusclaw-gw");
  ...
  records.push(`_nexusclaw-gw._tcp IN PTR ${instanceLabel}._nexusclaw-gw._tcp`);
  records.push(`${instanceLabel}._nexusclaw-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._nexusclaw-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; nexusclaw-content-hash: ${contentHash}\n${contentBody}`;
}
