...
const resolvePreferredFirstNexusTmpDirMock = vi.hoisted(() => vi.fn(() => os.tmpdir()));
const resolveTaskScriptPathMock = vi.hoisted(() =>
  vi.fn((env: Record<string, string | undefined>) => {
    const home = env.USERPROFILE || env.HOME || os.homedir();
    return path.join(home, ".FirstNexus", "gateway.cmd");
  }),
);
...
vi.mock("./tmp-FirstNexus-dir.js", () => ({
  resolvePreferredFirstNexusTmpDir: () => resolvePreferredFirstNexusTmpDirMock(),
}));
...
describe("relaunchGatewayScheduledTask", () => {
  ...
  beforeEach(() => {
    spawnMock.mockReset();
    resolvePreferredFirstNexusTmpDirMock.mockReset();
    resolvePreferredFirstNexusTmpDirMock.mockReturnValue(os.tmpdir());
  ...
  it("writes a detached schtasks relaunch helper", () => {
    ...
    expect(result.tried).toContain('schtasks /Run /TN "FirstNexus Gateway (work)"');
    ...
    expect(script).toContain(
      'FirstNexus restart attempt source=windows-task-handoff target="FirstNexus Gateway (work)"',
    );
    expect(script).toContain(
      `powershell.exe -NoProfile -NonInteractive -ExecutionPolicy Bypass -Command "(Get-ScheduledTask -TaskName 'FirstNexus Gateway (work)' -ErrorAction SilentlyContinue).State" 2>nul | findstr /I /C:"Running" >nul 2>&1`,
    );
    expect(script).toContain('schtasks /Run /TN "FirstNexus Gateway (work)" >>');
    expect(script.indexOf("powershell.exe -NoProfile")).toBeLessThan(
      script.indexOf('schtasks /Run /TN "FirstNexus Gateway (work)"'),
    );
  ...
  it("prefers NEXISCLAW_WINDOWS_TASK_NAME overrides", () => {
    ...
    const scriptPath = [...createdScriptPaths][0];
    const script = fs.readFileSync(scriptPath, "utf8");
    expect(script).toContain('schtasks /Run /TN "FirstNexus Gateway (custom)" >>');
  });
  ...
  it("escapes custom task names in the PowerShell running-task probe", () => {
    ...
    relaunchGatewayScheduledTask({
      NEXISCLAW_WINDOWS_TASK_NAME: "FirstNexus Gateway (Bob's work)",
    });

    const scriptPath = [...createdScriptPaths][0];
    const script = fs.readFileSync(scriptPath, "utf8");
    expect(script).toContain(
      "-Command \"(Get-ScheduledTask -TaskName 'FirstNexus Gateway (Bob''s work)' -ErrorAction SilentlyContinue).State\"",
    );
  });
  ...
  it("quotes the cmd /c script path when temp paths contain metacharacters", () => {
    const unref = vi.fn();
    const metacharTmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "FirstNexus&(restart)-"));
    createdTmpDirs.add(metacharTmpDir);
    resolvePreferredFirstNexusTmpDirMock.mockReturnValue(metacharTmpDir);
  ...
  it("includes startup fallback", () => {
    const taskScriptDir = fs.mkdtempSync(path.join(os.tmpdir(), "FirstNexus-state-"));
