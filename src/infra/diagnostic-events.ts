...
function getDiagnosticEventsState(): DiagnosticEventsGlobalState {
  const globalStore = globalThis as typeof globalThis & {
    __nexusclawDiagnosticEventsState?: DiagnosticEventsGlobalState;
  };
  if (!globalStore.__nexusclawDiagnosticEventsState) {
    globalStore.__nexusclawDiagnosticEventsState = {
      seq: 0,
      listeners: new Set<(evt: DiagnosticEventPayload) => void>(),
      dispatchDepth: 0,
    };
  }
  return globalStore.__nexusclawDiagnosticEventsState;
}

export function isDiagnosticsEnabled(config?: NexusClawConfig): boolean {
