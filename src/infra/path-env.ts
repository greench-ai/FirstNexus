...
function candidateBinDirs(opts: EnsureNexusClawPathOpts): { prepend: string[]; append: string[] } {
  ...
  // Bundled macOS app: `nexusclaw` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "nexusclaw");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.NEXUSCLAW_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "nexusclaw"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `nexusclaw` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureNexusClawCliOnPath(opts: EnsureNexusClawPathOpts = {}) {
  if (isTruthyEnvValue(process.env.NEXUSCLAW_PATH_BOOTSTRAPPED)) {
    return;
  }
