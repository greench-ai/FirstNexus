...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: NexusClawConfig) => void;
};

function hasExplicitHeartbeatAgents(cfg: NexusClawConfig) {
...
function resolveHeartbeatAgents(cfg: NexusClawConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: NexusClawConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: NexusClawConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: NexusClawConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
