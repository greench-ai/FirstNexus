...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: FirstNexusConfig) => void;
};
...
function hasExplicitHeartbeatAgents(cfg: FirstNexusConfig) {
...
function resolveHeartbeatAgents(cfg: FirstNexusConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
function resolveHeartbeatPromptRaw(cfg: FirstNexusConfig, heartbeat?: HeartbeatConfig) {
  return heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt;
}

export function resolveHeartbeatPrompt(cfg: FirstNexusConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(resolveHeartbeatPromptRaw(cfg, heartbeat));
}

function resolveHeartbeatResponseToolPrompt(cfg: FirstNexusConfig, heartbeat?: HeartbeatConfig) {
...
function usesCodexHarness(params: {
  ...
  const runtimeId =
    ...
    normalizeHeartbeatRuntimeId(process.env.NEXISCLAW_AGENT_RUNTIME) ||
...
function resolveHeartbeatAckMaxChars(cfg: FirstNexusConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
function isHeartbeatTypingEnabled(params: { cfg: FirstNexusConfig; hasChatDelivery: boolean }) {
...
function resolveHeartbeatTypingIntervalSeconds(cfg: FirstNexusConfig) {
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: FirstNexusConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
