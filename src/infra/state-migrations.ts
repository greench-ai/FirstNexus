...
export async function autoMigrateLegacyStateDir(params: {
  ...
  if (env.NEXISCLAW_STATE_DIR?.trim()) {
    return { migrated: false, skipped: true, changes: [], warnings: [] };
  }
  ...
  try {
    ...
    try {
      ...
      try {
        ...
        warnings.push(
          `Rollback failed; set NEXISCLAW_STATE_DIR=${targetDir} to avoid split state: ${String(rollbackErr)}`,
        );
...
export async function autoMigrateLegacyState(params: {
  ...
  if (env.NEXISCLAW_AGENT_DIR?.trim() || env.PI_CODING_AGENT_DIR?.trim()) {
