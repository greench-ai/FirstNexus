...
describe("resolveProviderAuths key normalization", () => {
  ...
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "nexusclaw-provider-auth-suite-"));
  ...
  async function withSuiteHome<T>(
    ...
    await fs.mkdir(path.join(base, ".nexusclaw", "agents", "main", "sessions"), { recursive: true });
    ...
    process.env.NEXUSCLAW_STATE_DIR = path.join(base, ".nexusclaw");
  ...
  async function writeAuthProfiles(home: string, profiles: Record<string, unknown>) {
    const agentDir = path.join(home, ".nexusclaw", "agents", "main", "agent");
