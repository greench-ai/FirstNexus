...
async function makeTempDir(label: string): Promise<string> {
  return await tempDirs.make(`FirstNexus-${label}-`);
}
...
describe("git commit resolution", () => {
  ...
  it("does not walk out of the FirstNexus package into a host repo", async () => {
    ...
    const packageRoot = path.join(hostRepo, "node_modules", "FirstNexus");
    await fs.mkdir(path.join(packageRoot, "dist"), { recursive: true });
    await fs.writeFile(
      path.join(packageRoot, "package.json"),
      JSON.stringify({ name: "FirstNexus", version: "2026.3.10" }),
      "utf-8",
    );
