...
describe("resolveVapidKeys", () => {
  ...
  it("prefers env vars over persisted keys", async () => {
    ...
    // Set env overrides.
    process.env.NEXISCLAW_VAPID_PUBLIC_KEY = "env-public";
    process.env.NEXISCLAW_VAPID_PRIVATE_KEY = "env-private";
    process.env.NEXISCLAW_VAPID_SUBJECT = "mailto:env@test.com";
    try {
      const keys = await resolveVapidKeys(tmpDir);
      ...
      delete process.env.NEXISCLAW_VAPID_PUBLIC_KEY;
      delete process.env.NEXISCLAW_VAPID_PRIVATE_KEY;
      delete process.env.NEXISCLAW_VAPID_SUBJECT;
    }
  });
});
