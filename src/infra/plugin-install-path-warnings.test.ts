...
const MATRIX_REPO_INSTALL_COMMAND = `FirstNexus plugins install ${repoInstallSpec("matrix")}`;
...
describe("plugin install path warnings", () => {
  ...
  it("detects stale custom plugin install paths", async () => {
    const issue = await detectPluginInstallPathIssue({
      ...
      install: {
        source: "path",
        sourcePath: "/tmp/FirstNexus-matrix-missing",
        installPath: "/tmp/FirstNexus-matrix-missing",
      },
    });
    ...
    expect(issue).toEqual({
      kind: "missing-path",
      pluginId: "matrix",
      path: "/tmp/FirstNexus-matrix-missing",
    });
    expect(
      formatPluginInstallPathIssue({
        issue: issue!,
      ...
      "Matrix is installed from a custom path that no longer exists: /tmp/FirstNexus-matrix-missing",
      'Reinstall with "FirstNexus plugins install @FirstNexus/matrix".',
  ...
  it("applies custom command formatting in warning messages", () => {
    expect(
      ...
      'Reinstall with "<FirstNexus plugins install @FirstNexus/matrix>" when you want to return to the standard Matrix plugin.',
