...
const GATEWAY_DEVICE_ID_HEADER = "x-nexusclaw-gateway-device-id";
const GATEWAY_SIGNATURE_HEADER = "x-nexusclaw-gateway-signature";
const GATEWAY_SIGNED_AT_HEADER = "x-nexusclaw-gateway-signed-at-ms";
...
export function resolveApnsRelayConfigFromEnv(
  ...
  const envBaseUrl = normalizeNonEmptyString(env.NEXUSCLAW_APNS_RELAY_BASE_URL);
  ...
  try {
    ...
    if (parsed.protocol === "http:" && !readAllowHttp(env.NEXUSCLAW_APNS_RELAY_ALLOW_HTTP)) {
      throw new Error(
        "http relay URLs require NEXUSCLAW_APNS_RELAY_ALLOW_HTTP=true (development only)",
      );
    }
    ...
    return {
      ...
      value: {
        ...
        timeoutMs: normalizeTimeoutMs(
          env.NEXUSCLAW_APNS_RELAY_TIMEOUT_MS ?? configuredRelay?.timeoutMs,
        ),
      },
    };
  } catch (err) {
