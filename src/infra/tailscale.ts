...
export function getTestTailscaleBinaryOverride(
  ...
  const forcedBinary = env.NEXISCLAW_TEST_TAILSCALE_BINARY?.trim();
  if (!forcedBinary) {
    return null;
  }
...
export async function ensureFunnel(
  ...
  try {
    ...
    runtime.error(
      info(
        `Tip: Funnel is optional for FirstNexus. You can keep running the web gateway without it: \`${formatCliCommand("FirstNexus gateway")}\``,
      ),
    );
    if (shouldLogVerbose()) {
      const rich = isRich();
