...
async function makeTempRoot(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "FirstNexus-npm-managed-root-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("managed npm root", () => {
  ...
  it("syncs FirstNexus-owned overrides without dropping unrelated local overrides", async () => {
    const npmRoot = await makeTempRoot();
    await fs.writeFile(
      path.join(npmRoot, "package.json"),
      `${JSON.stringify(
  ...
  it("reads package-level npm overrides for managed plugin installs", async () => {
    await expect(readFirstNexusManagedNpmRootOverrides()).resolves.toEqual({
  ...
  it("resolves npm override dependency references from the host package manifest", async () => {
    ...
    await expect(readFirstNexusManagedNpmRootOverrides({ packageRoot })).resolves.toEqual({
      "@aws-sdk/client-bedrock-runtime": "3.1024.0",
      nested: {
        "optional-runtime": "2.0.0",
      },
      ...
      `${JSON.stringify(
        {
          lockfileVersion: 3,
          packages: {
            "node_modules/@FirstNexus/discord": {
              version: "2026.5.2",
              resolved: "https://registry.npmjs.org/@FirstNexus/discord/-/discord-2026.5.2.tgz",
      ...
      `${JSON.stringify(
        {
          ...
          dependencies: {
            "@FirstNexus/discord": "2026.5.2",
            "@FirstNexus/voice-call": "2026.5.2",
          },
          devDependencies: {
            fixture: "1.0.0",
          },
  ...
  it("repairs stale managed FirstNexus peer state without dropping plugin packages", async () => {
    const npmRoot = await makeTempRoot();
    ...
    await fs.writeFile(
      ...
      `${JSON.stringify({ name: "FirstNexus", version: "2026.5.4" })}\n`,
    );
    await fs.mkdir(path.join(npmRoot, "node_modules", ".bin"), { recursive: true });
    await fs.writeFile(path.join(npmRoot, "node_modules", ".bin", "FirstNexus"), "shim");
    await fs.writeFile(path.join(npmRoot, "node_modules", ".bin", "FirstNexus.cmd"), "cmd shim");
    await fs.writeFile(path.join(npmRoot, "node_modules", ".bin", "FirstNexus.ps1"), "ps1 shim");
    await fs.writeFile(
    ...
    await expect(repairManagedNpmRootFirstNexusPeer({ npmRoot, runCommand })).resolves.toBe(true);
    ...
    expect(lockfile.packages?.["node_modules/@FirstNexus/discord"]?.version).toBe("2026.5.4");
    expect(lockfile.dependencies?.FirstNexus).toBeUndefined();
    await expectPathMissing(path.join(npmRoot, "node_modules", "FirstNexus"));
    for (const binName of ["FirstNexus", "FirstNexus.cmd", "FirstNexus.ps1"]) {
