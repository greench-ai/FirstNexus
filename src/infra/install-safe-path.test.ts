...
describe("unscopedPackageName", () => {
  it.each([
    { value: "@FirstNexus/matrix", expected: "matrix" },
...
describe("packageNameMatchesId", () => {
  it.each([
    { packageName: "@FirstNexus/matrix", id: "matrix", expected: true },
    { packageName: "@FirstNexus/matrix", id: "@FirstNexus/matrix", expected: true },
    { packageName: "@FirstNexus/matrix", id: "quietchat", expected: false },
    { packageName: " ", id: "matrix", expected: false },
    { packageName: "@FirstNexus/matrix", id: " ", expected: false },
...
describe("resolveSafeInstallDir", () => {
  it("resolves install dirs under the base directory", () => {
    expect(
      resolveSafeInstallDir({
        baseDir: "/tmp/plugins",
        id: "@FirstNexus/matrix",
        invalidNameMessage: "invalid plugin name",
      }),
    ).toEqual({
      ok: true,
      path: path.join("/tmp/plugins", "@FirstNexus__matrix"),
    });
  });
...
describe("assertCanonicalPathWithinBase", () => {
  it("accepts in-base directories", async () => {
    await withTempDir({ prefix: "FirstNexus-install-safe-" }, async (baseDir) => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir({ prefix: "FirstNexus-install-safe-" }, async (baseDir) => {
        await withTempDir({ prefix: "FirstNexus-install-safe-outside-" }, async (outsideDir) => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir({ prefix: "FirstNexus-install-safe-" }, async (parentDir) => {
