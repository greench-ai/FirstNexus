...
function createRelayApnsSendFixture(params: {
  ...
  return {
    ...
    relayConfig: {
      baseUrl: "https://relay.FirstNexus.test",
      timeoutMs: 2_500,
    },
    gatewayIdentity: {
...
describe("push APNs send semantics", () => {
  it("sends alert pushes with alert headers and payload", async () => {
    ...
    const FirstNexusPayload = requireRecord(payload.FirstNexus, "FirstNexus payload");
    expectRecordFields(FirstNexusPayload, {
      kind: "push.test",
      nodeId: "ios-node-alert",
    });
    expect(typeof FirstNexusPayload.ts).toBe("number");
  ...
  it("routes direct APNs HTTP/2 requests through the active managed proxy", async () => {
    ...
    try {
      ...
      expect(request?.headers["apns-topic"]).toBe("ai.FirstNexus.ios");
  ...
  it("sends exec approval alert pushes with generic modal-only metadata", async () => {
    ...
    expect(payload.aps).toEqual({
      alert: {
        ...
        body: "Open FirstNexus to review this request.",
      },
      sound: "default",
      category: "FirstNexus.exec-approval",
      "content-available": 1,
    });
    const FirstNexusPayload = requireRecord(payload.FirstNexus, "FirstNexus payload");
    expectRecordFields(FirstNexusPayload, {
    ...
    expect(typeof FirstNexusPayload.ts).toBe("number");
    expectNoProperties(FirstNexusPayload, [
      "host",
      "nodeId",
      "agentId",
      "commandText",
  ...
  it("defaults background wake reason when not provided", async () => {
    ...
    expectRecordFields(requireRecord(payload.FirstNexus, "FirstNexus payload"), {
      kind: "node.wake",
      reason: "node.invoke",
      nodeId: "ios-node-wake-default-reason",
    });
