...
describe("matchesExecAllowlistPattern", () => {
  ...
  it("expands home-prefix patterns", () => {
    const prevNexusClawHome = process.env.NEXUSCLAW_HOME;
    const prevHome = process.env.HOME;
    process.env.NEXUSCLAW_HOME = "/srv/nexusclaw-home";
    process.env.HOME = "/home/other";
    const openClawHome = path.join(path.resolve("/srv/nexusclaw-home"), "bin", "tool");
    ...
    try {
      ...
      if (prevNexusClawHome === undefined) {
        delete process.env.NEXUSCLAW_HOME;
      } else {
        process.env.NEXUSCLAW_HOME = prevNexusClawHome;
      }
