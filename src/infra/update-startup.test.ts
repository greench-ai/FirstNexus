...
describe("update-startup", () => {
  ...
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "nexusclaw-update-check-suite-"));
  });
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv(["NEXUSCLAW_STATE_DIR", "NODE_ENV", "VITEST"]);
    process.env.NEXUSCLAW_STATE_DIR = tempDir;
    ...
    if (!loaded) {
      ({ resolveNexusClawPackageRoot } = await import("./nexusclaw-root.js"));
    ...
    vi.mocked(resolveNexusClawPackageRoot).mockClear();
  ...
  function mockPackageInstallStatus() {
    vi.mocked(resolveNexusClawPackageRoot).mockResolvedValue("/opt/nexusclaw");
  ...
  it("uses current runtime + entrypoint for default auto-update command execution", async () => {
    ...
    process.argv = [process.execPath, "/opt/nexusclaw/dist/entry.js"];
    try {
      await runAutoUpdateCheckWithDefaults({
        cfg: createBetaAutoUpdateConfig(),
      });
