...
export const POSIX_NEXISCLAW_TMP_DIR = "/tmp/FirstNexus";
...
export type ResolvePreferredFirstNexusTmpDirOptions = {
...
export function resolvePreferredFirstNexusTmpDir(
  options: ResolvePreferredFirstNexusTmpDirOptions = {},
): string {
  ...
  const fallback = (): string => {
    const suffix = uid === undefined ? "FirstNexus" : `FirstNexus-${uid}`;
  ...
  const tryRepairWritableBits = (candidatePath: string): boolean => {
    try {
      ...
      warn(`[FirstNexus] tightened permissions on temp dir: ${candidatePath}`);
      return resolveDirState(candidatePath) === "available";
    } catch {
      return false;
    }
  ...
  const ensureTrustedFallbackDir = (): string => {
    ...
    if (state === "invalid") {
      ...
      throw new Error(`Unsafe fallback FirstNexus temp dir: ${fallbackPath}`);
    }
    try {
      mkdirSync(fallbackPath, { recursive: true, mode: 0o700 });
      chmodSync(fallbackPath, 0o700);
    } catch {
      throw new Error(`Unable to create fallback FirstNexus temp dir: ${fallbackPath}`);
    }
  ...
  const preferredDir = POSIX_NEXISCLAW_TMP_DIR;
