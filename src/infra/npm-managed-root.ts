...
import { resolveFirstNexusPackageRootSync } from "./FirstNexus-root.js";
...
type ManagedNpmRootFirstNexusMetadata = {
...
function buildManagedFirstNexusMetadata(params: {
  current: unknown;
  managedOverrideKeys: string[];
}): ManagedNpmRootFirstNexusMetadata | undefined {
  const metadata: ManagedNpmRootFirstNexusMetadata = isRecord(params.current)
    ? { ...params.current }
    : {};
...
export async function readFirstNexusManagedNpmRootOverrides(params?: {
  argv1?: string;
  cwd?: string;
  moduleUrl?: string;
...
export async function upsertManagedNpmRootDependency(params: {
  ...
  for (const key of readManagedOverrideKeys(manifest.FirstNexus)) {
    delete overrides[key];
  }
  Object.assign(overrides, managedOverrides);
  const FirstNexusMetadata = buildManagedFirstNexusMetadata({
    current: manifest.FirstNexus,
    managedOverrideKeys,
  });
  ...
  if (FirstNexusMetadata) {
    next.FirstNexus = FirstNexusMetadata;
  } else {
    delete next.FirstNexus;
  }
  await writeJson(manifestPath, next, { trailingNewline: true });
}

export async function repairManagedNpmRootFirstNexusPeer(params: {
  npmRoot: string;
  timeoutMs?: number;
  ...
  const manifest = await readManagedNpmRootManifest(manifestPath);
  const dependencies = readDependencyRecord(manifest.dependencies);
  const hasManifestDependency = "FirstNexus" in dependencies;
  const hasLockDependency = await managedNpmRootLockfileHasFirstNexusPeer(params.npmRoot);
  const hasPackageDir = await pathExists(path.join(params.npmRoot, "node_modules", "FirstNexus"));
  ...
  try {
    ...
    if (result.code !== 0) {
      params.logger?.warn?.(
        `npm ${hasManifestDependency ? "uninstall FirstNexus" : "prune"} failed while repairing managed npm root; falling back to direct cleanup: ${result.stderr.trim() || result.stdout.trim()}`,
      );
    }
  } catch (error) {
    params.logger?.warn?.(
      `npm ${hasManifestDependency ? "uninstall FirstNexus" : "prune"} failed while repairing managed npm root; falling back to direct cleanup: ${String(error)}`,
    );
  }

  await scrubManagedNpmRootFirstNexusPeer({ npmRoot: params.npmRoot });
  return true;
}

async function managedNpmRootLockfileHasFirstNexusPeer(npmRoot: string): Promise<boolean> {
  ...
  try {
    ...
    if (isRecord(parsed.packages)) {
      ...
      if (
        ...
        "FirstNexus" in rootPackage.dependencies
      ) {
        return true;
      }
      if ("node_modules/FirstNexus" in parsed.packages) {
        return true;
      }
    }
    return isRecord(parsed.dependencies) && "FirstNexus" in parsed.dependencies;
  } catch (err) {
...
async function scrubManagedNpmRootFirstNexusPeer(params: { npmRoot: string }): Promise<void> {
  const manifestPath = path.join(params.npmRoot, "package.json");
  const manifest = await readManagedNpmRootManifest(manifestPath);
  const dependencies = readDependencyRecord(manifest.dependencies);
  if ("FirstNexus" in dependencies) {
    const { FirstNexus: _removed, ...nextDependencies } = dependencies;
    await fs.writeFile(
      manifestPath,
  ...
  try {
    ...
    if (isRecord(parsed.packages)) {
      ...
      if (isRecord(rootPackage) && isRecord(rootPackage.dependencies)) {
        ...
        if ("FirstNexus" in dependencies) {
          delete dependencies.FirstNexus;
          parsed.packages[""] = { ...rootPackage, dependencies };
          lockChanged = true;
        }
      }
      if ("node_modules/FirstNexus" in parsed.packages) {
        delete parsed.packages["node_modules/FirstNexus"];
        lockChanged = true;
      }
    }
    if (isRecord(parsed.dependencies) && "FirstNexus" in parsed.dependencies) {
  ...
  const FirstNexusPackageDir = path.join(params.npmRoot, "node_modules", "FirstNexus");
  if (await pathExists(FirstNexusPackageDir)) {
    await fs.rm(FirstNexusPackageDir, { recursive: true, force: true });
  }
  const binDir = path.join(params.npmRoot, "node_modules", ".bin");
  await Promise.all(
    ["FirstNexus", "FirstNexus.cmd", "FirstNexus.ps1"].map((binName) =>
