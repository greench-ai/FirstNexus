  ...
  it("includes the cross-platform supervisor hint env vars", () => {
    expect(SUPERVISOR_HINT_ENV_VARS).toEqual(
      expect.arrayContaining([
        "LAUNCH_JOB_LABEL",
        "INVOCATION_ID",
        "NEXUSCLAW_WINDOWS_TASK_NAME",
        "NEXUSCLAW_SERVICE_MARKER",
...
describe("detectRespawnSupervisor", () => {
  it("detects launchd and systemd only from non-blank platform-specific hints", () => {
    expect(detectRespawnSupervisor({ LAUNCH_JOB_LABEL: " ai.nexusclaw.gateway " }, "darwin")).toBe(
      "launchd",
    );
  ...
  it("detects scheduled-task supervision on Windows from either hint family", () => {
    expect(
      detectRespawnSupervisor({ NEXUSCLAW_WINDOWS_TASK_NAME: "NexusClaw Gateway" }, "win32"),
    ).toBe("schtasks");
    expect(
      detectRespawnSupervisor(
        {
          NEXUSCLAW_SERVICE_MARKER: "nexusclaw",
  ...
  it("ignores service markers on non-Windows platforms and unknown platforms", () => {
    ...
    expect(
      detectRespawnSupervisor({ LAUNCH_JOB_LABEL: "ai.nexusclaw.gateway" }, "freebsd"),
    ).toBeNull();
  });
});
