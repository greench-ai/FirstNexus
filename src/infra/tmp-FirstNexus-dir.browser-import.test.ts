...
describe("tmp-FirstNexus-dir browser-safe import", () => {
  it("loads when a browser fs shim omits constants", async () => {
    const resultKey = `__FirstNexusTmpDirBrowserImport_${crypto.randomUUID().replaceAll("-", "_")}`;
    ...
    const bundled = await build({
      ...
      stdin: {
        contents: `
          import { POSIX_NEXISCLAW_TMP_DIR, resolvePreferredFirstNexusTmpDir } from "./src/infra/tmp-FirstNexus-dir.ts";
          globalThis.${resultKey} = {
            posixTmpDir: POSIX_NEXISCLAW_TMP_DIR,
            resolverType: typeof resolvePreferredFirstNexusTmpDir,
          };
        `,
        loader: "ts",
        resolveDir: process.cwd(),
        sourcefile: "tmp-FirstNexus-dir-browser-entry.ts",
      },
      write: false,
    });
