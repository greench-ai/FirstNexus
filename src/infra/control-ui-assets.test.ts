...
vi.mock("./FirstNexus-root.js", () => ({
  resolveFirstNexusPackageRoot: vi.fn(async () => null),
  resolveFirstNexusPackageRootSync: vi.fn(() => null),
}));
...
let FirstNexusRoot: typeof import("./FirstNexus-root.js");
...
describe("control UI assets helpers (fs-mocked)", () => {
  beforeAll(async () => {
    ...
    FirstNexusRoot = await import("./FirstNexus-root.js");
  });
  ...
  it("resolves dist control-ui index path for symlinked argv1 via realpath", async () => {
    const pkgRoot = abs("fixtures/bun-global/FirstNexus");
    const wrapperArgv1 = abs("fixtures/bin/FirstNexus");
  ...
  it("uses resolveFirstNexusPackageRoot when available", async () => {
    const pkgRoot = abs("fixtures/FirstNexus");
    (
      FirstNexusRoot.resolveFirstNexusPackageRoot as unknown as ReturnType<typeof vi.fn>
    ).mockResolvedValueOnce(pkgRoot);

    await expect(resolveControlUiDistIndexPath(abs("fixtures/bin/FirstNexus"))).resolves.toBe(
      path.join(pkgRoot, "dist", "control-ui", "index.html"),
    );
  });
  ...
  it("falls back to package.json name matching when root resolution fails", async () => {
    ...
    setFile(path.join(root, "package.json"), JSON.stringify({ name: "FirstNexus" }));
    setFile(path.join(root, "dist", "control-ui", "index.html"), "<html></html>\n");

    await expect(resolveControlUiDistIndexPath(path.join(root, "FirstNexus.mjs"))).resolves.toBe(
      path.join(root, "dist", "control-ui", "index.html"),
    );
  });

  it("returns null when fallback package name does not match", async () => {
    const root = abs("fixtures/not-FirstNexus");
  ...
  it("resolves control-ui root for dist bundle argv1 and moduleUrl candidates", () => {
    const pkgRoot = abs("fixtures/FirstNexus-bundle");
    (
      FirstNexusRoot.resolveFirstNexusPackageRootSync as unknown as ReturnType<typeof vi.fn>
  ...
  it("prefers packaged app Control UI assets in Contents/Resources", () => {
    const execPath = abs("fixtures/FirstNexus.app/Contents/MacOS/FirstNexus");
    const bundledUiDir = abs("fixtures/FirstNexus.app/Contents/Resources/control-ui");
  ...
  it("detects package-proven control-ui roots", () => {
    const pkgRoot = abs("fixtures/FirstNexus-package-root");
    const uiDir = path.join(pkgRoot, "dist", "control-ui");
    setDir(uiDir);
    setFile(path.join(uiDir, "index.html"), "<html></html>\n");
    (
