 ...
 * returning control to the caller (typically `triggerNexusClawRestart` →
...
/**
 * Parse nexusclaw gateway PIDs from lsof -Fpc stdout.
...
function parsePidsFromLsofOutput(stdout: string): number[] {
  ...
  for (const line of stdout.split(/\r?\n/).filter(Boolean)) {
    if (line.startsWith("p")) {
      if (currentPid != null && currentCmd && currentCmd.toLowerCase().includes("nexusclaw")) {
 ...
 * Returns only PIDs that belong to nexusclaw gateway processes (not the current process).
 */
