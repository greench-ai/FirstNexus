...
import { resolveFirstNexusPackageRoot } from "./FirstNexus-root.js";
...
function resolveAutoUpdatePolicy(cfg: FirstNexusConfig): AutoUpdatePolicy {
...
function resolveCheckIntervalMs(cfg: FirstNexusConfig): number {
...
export async function runGatewayUpdateCheck(params: {
  ...
  const autoDisabledByEnv = isTruthyEnvValue(process.env.NEXISCLAW_NO_AUTO_UPDATE);
  ...
  if (cmp != null && cmp < 0) {
    ...
    if (shouldRunUpdateHints && shouldNotify) {
      params.log.info(
        `update available (${tag}): v${resolved.version} (current v${VERSION}). Run: ${formatCliCommand("FirstNexus update")}`,
      );
    ...
    if (auto.enabled && autoDisabledByEnv) {
      params.log.info("auto-update disabled by NEXISCLAW_NO_AUTO_UPDATE", {
