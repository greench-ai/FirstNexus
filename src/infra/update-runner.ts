...
const DEFAULT_PACKAGE_NAME = "nexusclaw";
...
function normalizeTag(tag?: string) {
  return normalizePackageTagInput(tag, ["nexusclaw", DEFAULT_PACKAGE_NAME]) ?? "latest";
}
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "nexusclaw-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "nexusclaw.mjs");
    ...
    const doctorStep = await runStep(
      step("nexusclaw doctor", doctorArgv, gitRoot, { NEXUSCLAW_UPDATE_IN_PROGRESS: "1" }),
    );
