...
export const POSIX_NEXUSCLAW_TMP_DIR = "/tmp/nexusclaw";
const TMP_DIR_ACCESS_MODE = fs.constants.W_OK | fs.constants.X_OK;

type ResolvePreferredNexusClawTmpDirOptions = {
...
export function resolvePreferredNexusClawTmpDir(
  options: ResolvePreferredNexusClawTmpDirOptions = {},
): string {
  ...
  const fallback = (): string => {
    const base = tmpdir();
    const suffix = uid === undefined ? "nexusclaw" : `nexusclaw-${uid}`;
  ...
  const tryRepairWritableBits = (candidatePath: string): boolean => {
    try {
      ...
      warn(`[nexusclaw] tightened permissions on temp dir: ${candidatePath}`);
  ...
  const ensureTrustedFallbackDir = (): string => {
    ...
    if (state === "invalid") {
      ...
      throw new Error(`Unsafe fallback NexusClaw temp dir: ${fallbackPath}`);
    }
    try {
      mkdirSync(fallbackPath, { recursive: true, mode: 0o700 });
      chmodSync(fallbackPath, 0o700);
    } catch {
      throw new Error(`Unable to create fallback NexusClaw temp dir: ${fallbackPath}`);
    }
  ...
  const existingPreferredState = resolveDirState(POSIX_NEXUSCLAW_TMP_DIR);
  if (existingPreferredState === "available") {
    return POSIX_NEXUSCLAW_TMP_DIR;
  }
  if (existingPreferredState === "invalid") {
    if (tryRepairWritableBits(POSIX_NEXUSCLAW_TMP_DIR)) {
      return POSIX_NEXUSCLAW_TMP_DIR;
    }
  ...
  try {
    ...
    mkdirSync(POSIX_NEXUSCLAW_TMP_DIR, { recursive: true, mode: 0o700 });
    chmodSync(POSIX_NEXUSCLAW_TMP_DIR, 0o700);
    if (
      resolveDirState(POSIX_NEXUSCLAW_TMP_DIR) !== "available" &&
      !tryRepairWritableBits(POSIX_NEXUSCLAW_TMP_DIR)
    ) {
      return ensureTrustedFallbackDir();
    }
    return POSIX_NEXUSCLAW_TMP_DIR;
