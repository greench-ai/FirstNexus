...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: NexusClawConfig | null = null;
  ...
  get(key: string, cfg: NexusClawConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: NexusClawConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  clear(cfg?: NexusClawConfig): void {
    this.cache.clear();
    if (cfg) {
  ...
  private resetIfConfigChanged(cfg: NexusClawConfig): void {
