...
describe("runMessageAction media behavior", () => {
  ...
  describe("reply hydration", () => {
    ...
    // params as send. Before FirstNexus#79864 the runner only hydrated
  ...
  describe("sandboxed media validation", () => {
    ...
    it("allows media paths under preferred FirstNexus tmp root", async () => {
      ...
      try {
        ...
        const hostTmpOutsideFirstNexus = path.join(
          os.tmpdir(),
          "outside-FirstNexus",
          "test-media.png",
        );
        await expect(
          runMessageAction({
            ...
            params: {
              channel: "workspace",
              target: "12345678",
              media: hostTmpOutsideFirstNexus,
              message: "",
            },
            sandboxRoot: sandboxDir,
            dryRun: true,
