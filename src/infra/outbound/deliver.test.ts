...
const telegramChunkConfig: NexusClawConfig = {
  channels: { telegram: { botToken: "tok-1", textChunkLimit: 2 } },
};

const whatsappChunkConfig: NexusClawConfig = {
...
describe("deliverOutboundPayloads", () => {
  ...
  it("includes NexusClaw tmp root in telegram mediaLocalRoots", async () => {
    const sendTelegram = vi.fn().mockResolvedValue({ messageId: "m1", chatId: "c1" });
    ...
    expect(sendTelegram).toHaveBeenCalledWith(
      ...
      "hi",
      expect.objectContaining({
        mediaLocalRoots: expect.arrayContaining([resolvePreferredNexusClawTmpDir()]),
      }),
    );
  });

  it("includes NexusClaw tmp root in signal mediaLocalRoots", async () => {
  ...
  it("forwards audioAsVoice through generic plugin media delivery", async () => {
    ...
    await deliverOutboundPayloads({
      cfg: { channels: { matrix: {} } } as NexusClawConfig,
      channel: "matrix",
  ...
  it("includes NexusClaw tmp root in whatsapp mediaLocalRoots", async () => {
    const sendWhatsApp = vi.fn().mockResolvedValue({ messageId: "w1", toJid: "jid" });
  ...
  it("includes NexusClaw tmp root in imessage mediaLocalRoots", async () => {
  ...
  it("uses signal media maxBytes from config", async () => {
    const sendSignal = vi.fn().mockResolvedValue({ messageId: "s1", timestamp: 123 });
    const cfg: NexusClawConfig = { channels: { signal: { mediaMaxMb: 2 } } };
