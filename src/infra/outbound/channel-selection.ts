...
function isConfiguredChannel(cfg: FirstNexusConfig, channelId: string): boolean {
...
function formatMissingOfficialExternalChannelsMessage(
  ...
  return `Configured official external channels ${labels} are missing their plugins. Run: FirstNexus doctor --fix, or install individually: ${installCommands}.`;
}

function formatNoConfiguredChannelsMessage(): string {
  return [
    "Channel is required (no configured channels detected).",
    "Run FirstNexus channels add to configure one, or pass --channel <channel> after enabling a channel.",
    "Use FirstNexus channels list --all to see available channel ids.",
  ].join(" ");
}
...
async function isPluginConfigured(plugin: ChannelPlugin, cfg: FirstNexusConfig): Promise<boolean> {
