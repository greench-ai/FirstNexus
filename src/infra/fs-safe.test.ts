...
async function withOutsideHardlinkAlias(params: {
  ...
  const outside = await tempDirs.make("nexusclaw-fs-safe-outside-");
...
async function setupSymlinkWriteRaceFixture(options?: { seedInsideTarget?: boolean }): Promise<{
  ...
  const root = await tempDirs.make("nexusclaw-fs-safe-root-");
...
describe("fs-safe", () => {
  it("reads a local file safely", async () => {
    const dir = await tempDirs.make("nexusclaw-fs-safe-");
  ...
  it("copies a file within root safely", async () => {
    const root = await tempDirs.make("nexusclaw-fs-safe-root-");
    const sourceDir = await tempDirs.make("nexusclaw-fs-safe-source-");
  ...
  it("writes a file within root from another local source path safely", async () => {
    const root = await tempDirs.make("nexusclaw-fs-safe-root-");
    const outside = await tempDirs.make("nexusclaw-fs-safe-src-");
...
describe("tilde expansion in file tools", () => {
  it("keeps tilde expansion behavior aligned", async () => {
    ...
    const root = await tempDirs.make("nexusclaw-tilde-test-");
    ...
    const outsideRoot = await tempDirs.make("nexusclaw-tilde-outside-");
    await expect(
      openFileWithinRoot({
        rootDir: outsideRoot,
        relativePath: "~/escape.txt",
