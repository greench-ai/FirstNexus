...
const stateDir = "/opt/FirstNexus/state";
...
describe("isVolatileBackupPath", () => {
  ...
  it("does not match paths that escape the anchor via `..`", () => {
    // `/opt/FirstNexus/state/sessions/../config.jsonl` resolves to
    // `/opt/FirstNexus/state/config.jsonl`, which is NOT inside sessions/.
  ...
  it("normalizes Windows-style separators before anchor checks", () => {
    const winStateDir = "C:\\FirstNexus\\state";
  ...
  it("matches tar filter paths when node-tar omits the leading slash", () => {
    expect(
      isVolatileBackupPath("opt/FirstNexus/state/agents/main/sessions/transcript.jsonl", plan),
    ).toBe(true);
  });
