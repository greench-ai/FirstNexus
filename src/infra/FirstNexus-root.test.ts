...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__FirstNexus_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "FirstNexus-root");
...
function setPackageRoot(root: string, name = "FirstNexus") {
  setFile(path.join(root, "package.json"), JSON.stringify({ name }));
}

function expectResolvedPackageRoot(
  syncResolver: typeof import("./FirstNexus-root.js").resolveFirstNexusPackageRootSync,
  asyncResolver: typeof import("./FirstNexus-root.js").resolveFirstNexusPackageRoot,
  opts: Parameters<typeof import("./FirstNexus-root.js").resolveFirstNexusPackageRootSync>[0],
...
vi.mock("./FirstNexus-root.fs.runtime.js", () => ({
  openClawRootFsSync: mockFsModule(),
  openClawRootFs: mockFsPromisesModule(),
}));

describe("resolveFirstNexusPackageRoot", () => {
  let resolveFirstNexusPackageRoot: typeof import("./FirstNexus-root.js").resolveFirstNexusPackageRoot;
  let resolveFirstNexusPackageRootSync: typeof import("./FirstNexus-root.js").resolveFirstNexusPackageRootSync;
  let clearFirstNexusPackageRootCaches: typeof import("./FirstNexus-root.js").__testing.clearFirstNexusPackageRootCaches;

  beforeAll(async () => {
    ({
      resolveFirstNexusPackageRoot,
      resolveFirstNexusPackageRootSync,
      __testing: { clearFirstNexusPackageRootCaches },
    } = await import("./FirstNexus-root.js"));
  });

  beforeEach(() => {
    clearFirstNexusPackageRootCaches();
    state.entries.clear();
  ...
  it.each([
    {
      ...
      setup: () => {
        ...
        const argv1 = path.join(project, "node_modules", ".bin", "FirstNexus");
        const pkgRoot = path.join(project, "node_modules", "FirstNexus");
    ...
    },
    {
      ...
      setup: () => {
        ...
        const bin = path.join(project, "bin", "FirstNexus");
        const realPkg = path.join(project, "real-pkg");
        state.realpaths.set(abs(bin), abs(path.join(realPkg, "FirstNexus.mjs")));
    ...
    {
      name: "falls through from a non-FirstNexus moduleUrl candidate to cwd",
      setup: () => {
        const wrongPkgRoot = fx("moduleurl-fallthrough", "wrong");
        const cwdPkgRoot = fx("moduleurl-fallthrough", "cwd");
        setPackageRoot(wrongPkgRoot, "not-FirstNexus");
    ...
    {
      name: "returns null for non-FirstNexus package roots",
      setup: () => {
        const pkgRoot = fx("not-FirstNexus");
        setPackageRoot(pkgRoot, "not-FirstNexus");
    ...
    },
    {
      ...
      setup: () => {
        ...
        state.realpaths.set(
          abs(argv1),
          abs(path.join(project, "versions", "current", "FirstNexus.mjs")),
        );
        const pkgRoot = path.join(project, "node_modules", "FirstNexus");
        setPackageRoot(pkgRoot);
        return { opts: { argv1 }, expected: pkgRoot };
