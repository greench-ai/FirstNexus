...
describe("normalizePackageTagInput", () => {
  const packageNames = ["nexusclaw", "@nexusclaw/plugin"] as const;
  ...
  it("strips known package-name prefixes before returning the tag", () => {
    expect(normalizePackageTagInput("nexusclaw@beta", packageNames)).toBe("beta");
    expect(normalizePackageTagInput("@nexusclaw/plugin@2026.2.24", packageNames)).toBe("2026.2.24");
    expect(normalizePackageTagInput("nexusclaw@   ", packageNames)).toBeNull();
  });

  it("treats exact known package names as an empty tag", () => {
    expect(normalizePackageTagInput("nexusclaw", packageNames)).toBeNull();
    expect(normalizePackageTagInput(" @nexusclaw/plugin ", packageNames)).toBeNull();
  });
  ...
  it("returns trimmed raw values when no package prefix matches", () => {
    ...
    expect(normalizePackageTagInput("nexusclawer@beta", packageNames)).toBe("nexusclawer@beta");
  });
});
