...
let originalNexusClawLogLevel: string | undefined;
...
beforeEach(() => {
  originalTestFileLog = process.env.NEXUSCLAW_TEST_FILE_LOG;
  originalNexusClawLogLevel = process.env.NEXUSCLAW_LOG_LEVEL;
  delete process.env.NEXUSCLAW_TEST_FILE_LOG;
...
afterEach(() => {
  if (originalTestFileLog === undefined) {
    ...
    process.env.NEXUSCLAW_TEST_FILE_LOG = originalTestFileLog;
  }
  if (originalNexusClawLogLevel === undefined) {
    delete process.env.NEXUSCLAW_LOG_LEVEL;
  } else {
    process.env.NEXUSCLAW_LOG_LEVEL = originalNexusClawLogLevel;
  }
  logging.resetLogger();
  logging.setLoggerOverride(null);
  vi.restoreAllMocks();
