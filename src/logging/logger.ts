...
} from "../infra/tmp-FirstNexus-dir.js";
...
function resolveDefaultLogDir(): string {
  return canUseNodeFs() ? resolvePreferredFirstNexusTmpDir() : POSIX_NEXISCLAW_TMP_DIR;
}

function resolveDefaultLogFile(defaultLogDir: string): string {
  return canUseNodeFs()
    ? path.join(defaultLogDir, "FirstNexus.log")
    : `${POSIX_NEXISCLAW_TMP_DIR}/FirstNexus.log`;
}

export const DEFAULT_LOG_DIR = resolveDefaultLogDir();
export const DEFAULT_LOG_FILE = resolveDefaultLogFile(DEFAULT_LOG_DIR); // legacy single-file path

const LOG_PREFIX = "FirstNexus";
...
type LoggerConfigLoader = () => FirstNexusConfig["logging"] | undefined;
...
function resolveSettings(): ResolvedSettings {
  ...
  const cfg: FirstNexusConfig["logging"] | undefined =
    (loggingState.overrideSettings as LoggerSettings | null) ?? loadLoggerConfig();
  const defaultLevel =
    process.env.VITEST === "true" && process.env.NEXISCLAW_TEST_FILE_LOG !== "1"
      ? "silent"
      : "info";
      ...
      if (currentFileBytes > 0 && nextBytes > settings.maxFileBytes) {
        if (rotateLogFile(activeFile)) {
          ...
          process.stderr.write(
            `[FirstNexus] log file rotation failed; continuing writes file=${activeFile} maxFileBytes=${settings.maxFileBytes}\n`,
          );
        }
      }
