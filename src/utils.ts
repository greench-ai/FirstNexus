...
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".nexusclaw");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.NEXUSCLAW_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$NEXUSCLAW_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via NEXUSCLAW_STATE_DIR.
