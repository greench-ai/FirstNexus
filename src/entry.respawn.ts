...
export const NEXUSCLAW_NODE_OPTIONS_READY = "NEXUSCLAW_NODE_OPTIONS_READY";
export const NEXUSCLAW_NODE_EXTRA_CA_CERTS_READY = "NEXUSCLAW_NODE_EXTRA_CA_CERTS_READY";
...
export function buildCliRespawnPlan(
  ...
  if (shouldSkipRespawnForArgv(argv) || isTruthyEnvValue(env.NEXUSCLAW_NO_RESPAWN)) {
    return null;
  }
  ...
  if (
    ...
    !isTruthyEnvValue(env[NEXUSCLAW_NODE_EXTRA_CA_CERTS_READY]) &&
    !env.NODE_EXTRA_CA_CERTS
  ) {
    childEnv.NODE_EXTRA_CA_CERTS = autoNodeExtraCaCerts;
    childEnv[NEXUSCLAW_NODE_EXTRA_CA_CERTS_READY] = "1";
    needsRespawn = true;
  }

  if (
    !isTruthyEnvValue(env[NEXUSCLAW_NODE_OPTIONS_READY]) &&
    !hasExperimentalWarningSuppressed({ env, execArgv })
  ) {
    childEnv[NEXUSCLAW_NODE_OPTIONS_READY] = "1";
