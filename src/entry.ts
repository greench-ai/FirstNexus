...
import {
  enableFirstNexusCompileCache,
  resolveEntryInstallRoot,
  respawnWithoutFirstNexusCompileCacheIfNeeded,
} from "./entry.compile-cache.js";
import { buildCliRespawnPlan, runCliRespawnPlan } from "./entry.respawn.js";
import { tryHandleRootVersionFastPath } from "./entry.version-fast-path.js";
import { isTruthyEnvValue, normalizeEnv } from "./infra/env.js";
import { ensureFirstNexusExecMarkerOnProcess } from "./infra/FirstNexus-exec-env.js";
import { isMainModule } from "./infra/is-main.js";
import { installProcessWarningFilter } from "./infra/warning-filter.js";

const ENTRY_WRAPPER_PAIRS = [
  { wrapperBasename: "FirstNexus.mjs", entryBasename: "entry.js" },
  { wrapperBasename: "FirstNexus.js", entryBasename: "entry.js" },
] as const;
...
if (
  ...
  const waitingForCompileCacheRespawn = respawnWithoutFirstNexusCompileCacheIfNeeded({
    currentFile: entryFile,
    installRoot,
  });
  if (!waitingForCompileCacheRespawn) {
    process.title = "FirstNexus";
    ...
    if (!ensureCliRespawnReady()) {
      ...
      if (!parsedContainer.ok) {
        console.error(`[FirstNexus] ${parsedContainer.error}`);
        process.exit(2);
      }
      ...
      if (!parsed.ok) {
        ...
        console.error(`[FirstNexus] ${parsed.error}`);
        process.exit(2);
      }

      const containerTargetName = resolveCliContainerTarget(process.argv);
      if (containerTargetName && parsed.profile) {
        console.error("[FirstNexus] --container cannot be combined with --profile/--dev");
        process.exit(2);
      }
...
export async function tryHandleRootHelpFastPath(
  ...
  const handleError =
    ...
    ((error: unknown) => {
      console.error(
        "[FirstNexus] Failed to display help:",
