...
describe("syncPluginVersions", () => {
  ...
  it("preserves workspace FirstNexus devDependencies and plugin host floors", () => {
    const rootDir = makeTempDir(tempDirs, "FirstNexus-sync-plugin-versions-");
    ...
    writeJson(path.join(rootDir, "extensions/imessage/package.json"), {
      name: "@FirstNexus/imessage",
      version: "2026.3.30",
      devDependencies: {
        FirstNexus: "workspace:*",
      },
      peerDependencies: {
        FirstNexus: ">=2026.3.30",
      },
      FirstNexus: {
        install: {
    ...
    expect(summary.updated).toContain("@FirstNexus/imessage");
    expect(updatedPackage.version).toBe("2026.4.1");
    expect(updatedPackage.devDependencies?.FirstNexus).toBe("workspace:*");
    expect(updatedPackage.peerDependencies?.FirstNexus).toBe(">=2026.4.1");
    expect(updatedPackage.FirstNexus?.install?.minHostVersion).toBe(">=2026.3.30");
    expect(updatedPackage.FirstNexus?.compat?.pluginApi).toBe(">=2026.4.1");
    expect(updatedPackage.FirstNexus?.build?.FirstNexusVersion).toBe("2026.4.1");
  });

  it("reports pending version sync without writing in check mode", () => {
    const rootDir = makeTempDir(tempDirs, "FirstNexus-sync-plugin-versions-check-");
    ...
    expect(summary.updated).toEqual(["@FirstNexus/discord"]);
    expect(unchangedPackage.version).toBe("2026.4.1");
    expect(unchangedPackage.peerDependencies?.FirstNexus).toBe(">=2026.4.1");
    expect(unchangedPackage.FirstNexus?.compat?.pluginApi).toBe(">=2026.4.1");
  });

  it("uses the base release version for beta changelog entries", () => {
    const rootDir = makeTempDir(tempDirs, "FirstNexus-sync-plugin-versions-beta-changelog-");
    ...
    expect(summary.changelogged).toEqual(["@FirstNexus/matrix"]);
