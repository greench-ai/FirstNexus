...
type FirstNexusTestStateLayout = "home" | "state-only" | "split";

type FirstNexusTestStateScenario =
  | "empty"
  | "minimal"
  | "update-stable"
  | "upgrade-survivor"
...
export type FirstNexusTestStateOptions = {
  prefix?: string;
  label?: string;
  layout?: FirstNexusTestStateLayout;
  scenario?: FirstNexusTestStateScenario;
  agentEnv?: "clear" | "main";
  applyEnv?: boolean;
...
export type FirstNexusTestState = {
  root: string;
  home: string;
  stateDir: string;
  configPath: string;
...
const DEFAULT_PREFIX = "FirstNexus-test-state-";
const ENV_KEYS = [
  "HOME",
  "USERPROFILE",
  "HOMEDRIVE",
  ...
  "NEXISCLAW_AGENT_DIR",
  "PI_CODING_AGENT_DIR",
  "NEXISCLAW_SERVICE_REPAIR_POLICY",
] as const;
...
function resolveLayout(
  ...
  if (layout === "split") {
    ...
    return {
      home,
      stateDir,
      configPath: path.join(root, "config", "FirstNexus.json"),
...
function scenarioConfig(options: FirstNexusTestStateOptions): Record<string, unknown> | undefined {
  ...
  if (scenario === "upgrade-survivor") {
    return {
      ...
      gateway: {
        ...
        auth: {
          mode: "token",
          token: options.gateway?.token ?? "FirstNexus-test-token",
        },
        controlUi: {
          enabled: false,
        },
...
function scenarioEnv(options: FirstNexusTestStateOptions): Record<string, string | undefined> {
...
export async function createFirstNexusTestState(
  options: FirstNexusTestStateOptions = {},
): Promise<FirstNexusTestState> {
  ...
  const state: FirstNexusTestState = {
    root,
    ...paths,
    env,
    envVars,
...
export async function withFirstNexusTestState<T>(
  options: FirstNexusTestStateOptions,
  fn: (state: FirstNexusTestState) => Promise<T>,
): Promise<T> {
  const state = await createFirstNexusTestState(options);
  try {
