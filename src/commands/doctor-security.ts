...
import type { NexusClawConfig, GatewayBindMode } from "../config/config.js";
...
function collectImplicitHeartbeatDirectPolicyWarnings(cfg: NexusClawConfig): string[] {
...
export async function noteSecurityWarnings(cfg: NexusClawConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("nexusclaw security audit --deep")}`;

  if (cfg.approvals?.exec?.enabled === false) {
    warnings.push(
      "- Note: approvals.exec.enabled=false disables approval forwarding only.",
      "  Host exec gating still comes from ~/.nexusclaw/exec-approvals.json.",
      `  Check local policy with: ${formatCliCommand("nexusclaw approvals get --gateway")}`,
    );
  }
  ...
  const saferRemoteAccessLines = [
    ...
    "  Docs: https://docs.nexusclaw.ai/gateway/remote",
  ];
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("nexusclaw configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("nexusclaw config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("nexusclaw doctor --fix")} to generate a token`,
              `  Or set token directly: ${formatCliCommand(
                "nexusclaw config set gateway.auth.mode token",
              )}`,
            ];
      warnings.push(
        `- CRITICAL: Gateway bound to ${bindDescriptor} without authentication.`,
        `  Anyone on your network (or internet if port-forwarded) can fully control your agent.`,
        `  Fix: ${formatCliCommand("nexusclaw config set gateway.bind loopback")}`,
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('nexusclaw config set session.dmScope "per-channel-peer"') +
