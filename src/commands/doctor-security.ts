...
import type { FirstNexusConfig, GatewayBindMode } from "../config/config.js";
...
function collectImplicitHeartbeatDirectPolicyWarnings(cfg: FirstNexusConfig): string[] {
...
function collectExecPolicyConflictWarnings(cfg: FirstNexusConfig): string[] {
  const warnings: string[] = [];
  const approvals = loadExecApprovals();
  const defaultRequestedSecuritySource = "FirstNexus default (full)";
  const defaultRequestedAskSource = "FirstNexus default (off)";
  ...
  const maybeWarn = (params: {
    ...
    warnings.push(
      [
        ...
        `  Inspect with: ${formatCliCommand("FirstNexus approvals get --gateway")}`,
      ].join("\n"),
    );
  };
...
function collectDurableExecApprovalWarnings(cfg: FirstNexusConfig): string[] {
  void cfg;
  return [];
}

function collectExecFilesystemPolicyWarnings(cfg: FirstNexusConfig): string[] {
...
export async function noteSecurityWarnings(cfg: FirstNexusConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("FirstNexus security audit --deep")}`;

  if (cfg.approvals?.exec?.enabled === false) {
    warnings.push(
      "- Note: approvals.exec.enabled=false disables approval forwarding only.",
      "  Host exec gating still comes from ~/.FirstNexus/exec-approvals.json.",
      `  Check local policy with: ${formatCliCommand("FirstNexus approvals get --gateway")}`,
    );
  }
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("FirstNexus configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("FirstNexus config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("FirstNexus doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("FirstNexus config set gateway.bind loopback")}`,
