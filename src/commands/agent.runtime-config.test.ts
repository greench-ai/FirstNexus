...
type ConfigSnapshotForWrite = {
  snapshot: { valid: boolean; resolved: FirstNexusConfig };
...
const loadConfigMock = vi.hoisted(() => vi.fn<() => FirstNexusConfig>());
...
const setRuntimeConfigSnapshotMock = vi.hoisted(() =>
  vi.fn<(cfg: FirstNexusConfig, sourceConfig: FirstNexusConfig) => void>(),
);
...
const resolveCommandConfigWithSecretsMock = vi.hoisted(() =>
  vi.fn<
    (params: ResolveCommandConfigParams) => Promise<{
      ...
      effectiveConfig: FirstNexusConfig;
      diagnostics: never[];
    }>
  >(),
);
...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "FirstNexus-agent-" });
}
...
function mockConfig(home: string, storePath: string): FirstNexusConfig {
  const cfg = {
    agents: {
      defaults: {
...
beforeEach(() => {
  ...
  readConfigFileSnapshotForWriteMock.mockResolvedValue({
    snapshot: { valid: false, resolved: {} as FirstNexusConfig },
    writeOptions: {},
  });
});
