...
function resolveProviderAuthLoginCommand(params: {
  ...
  return formatCliCommand(`FirstNexus models auth login --provider ${choice.providerId}`);
}
...
export function buildProviderAuthRecoveryHint(params: {
  ...
  if (params.includeConfigure !== false) {
    parts.push(`\`${formatCliCommand("FirstNexus configure")}\``);
  }
  if (params.includeEnvVar) {
    parts.push("set an API key env var");
  }
  if (parts.length === 0) {
    return `Run \`${formatCliCommand("FirstNexus configure")}\`.`;
  }
  if (parts.length === 1) {
    return `${parts[0]}.`;
  }
