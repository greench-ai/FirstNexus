...
type PromptDefaultModelResult = { model?: string; config?: NexusClawConfig };
...
function resolveConfiguredModelRaw(cfg: NexusClawConfig): string {
  return resolveAgentModelPrimaryValue(cfg.agents?.defaults?.model) ?? "";
}

function resolveConfiguredModelKeys(cfg: NexusClawConfig): string[] {
...
export function applyModelAllowlist(cfg: NexusClawConfig, models: string[]): NexusClawConfig {
