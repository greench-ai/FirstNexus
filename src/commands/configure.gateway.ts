...
export async function promptGatewayConfig(
    ...
    if (tokenInputMode === "ref") {
      const envVar = guardCancel(
        await text({
          ...
          initialValue: "NEXUSCLAW_GATEWAY_TOKEN",
          placeholder: "NEXUSCLAW_GATEWAY_TOKEN",
          validate: (value) => {
            const candidate = String(value ?? "").trim();
            if (!isValidEnvSecretRefId(candidate)) {
              return "Use an env var name like NEXUSCLAW_GATEWAY_TOKEN.";
            }
      ...
      note(`Validated ${envVarName}. NexusClaw will store a token SecretRef.`, "Gateway token");
    } else {
  ...
  if (authMode === "trusted-proxy") {
    note(
      [
        "Trusted proxy mode: NexusClaw trusts user identity from a reverse proxy.",
        ...
        "Docs: https://docs.nexusclaw.ai/gateway/trusted-proxy-auth",
      ].join("\n"),
      "Trusted Proxy Auth",
    );
