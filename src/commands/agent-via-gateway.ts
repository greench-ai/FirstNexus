...
function parseTimeoutSeconds(opts: { cfg: FirstNexusConfig; timeout?: string }) {
  const raw =
...
async function agentViaGatewayCommand(opts: AgentCliOpts, runtime: RuntimeEnv) {
  ...
  if (!body) {
    throw new Error(
      `Missing message. Use ${formatCliCommand('FirstNexus agent --message "..." --agent <id>')} or pass --to/--session-id for an existing conversation.`,
    );
  }
  if (!opts.to && !opts.sessionId && !opts.agent) {
    throw new Error(
      `No target session selected. Use --agent <id>, --session-id <id>, or --to <E.164>. Run ${formatCliCommand("FirstNexus agents list")} to see agents.`,
    );
  }
  ...
  if (agentId) {
    ...
    if (!knownAgents.includes(agentId)) {
      throw new Error(
        `Unknown agent id "${agentIdRaw}". Use "${formatCliCommand("FirstNexus agents list")}" to see configured agents.`,
      );
    }
  }
