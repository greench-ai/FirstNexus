...
import { FirstNexusSchema } from "../config/zod-schema.js";
...
export function stripUnknownConfigKeys(config: FirstNexusConfig): {
  config: FirstNexusConfig;
  removed: string[];
} {
  const parsed = FirstNexusSchema.safeParse(config);
  if (parsed.success) {
...
export function noteOpencodeProviderOverrides(cfg: FirstNexusConfig): void {
...
export function collectImplicitFallbackClobberWarnings(cfg: FirstNexusConfig): string[] {
...
export function noteImplicitFallbackClobberWarnings(cfg: FirstNexusConfig): void {
