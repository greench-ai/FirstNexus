...
import { NexusClawSchema } from "../config/zod-schema.js";
...
export function stripUnknownConfigKeys(config: NexusClawConfig): {
  config: NexusClawConfig;
  removed: string[];
} {
  const parsed = NexusClawSchema.safeParse(config);
  if (parsed.success) {
...
export function noteOpencodeProviderOverrides(cfg: NexusClawConfig): void {
