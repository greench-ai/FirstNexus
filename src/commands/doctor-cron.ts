...
export async function noteLegacyWhatsAppCrontabHealthCheck(
  ...
  note(
    [
      ...
      "`~/.FirstNexus/bin/ensure-whatsapp.sh` is not maintained by current FirstNexus and can misreport `Gateway inactive` from cron when the systemd user bus environment is missing.",
      `Remove the stale crontab entry with ${formatCliCommand("crontab -e")}; use ${formatCliCommand("FirstNexus channels status --probe")}, ${formatCliCommand("FirstNexus doctor")}, and ${formatCliCommand("FirstNexus gateway status")} for current health checks.`,
...
export async function maybeRepairLegacyCronStore(params: {
  ...
  note(
    [
      ...
      ...previewLines,
      `Repair with ${formatCliCommand("FirstNexus doctor --fix")} to normalize the store before the next scheduler run.`,
    ].join("\n"),
    "Cron",
  );
