...
function resolveAgentName(cfg: NexusClawConfig, agentId: string) {
...
function resolveAgentModel(cfg: NexusClawConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: NexusClawConfig): AgentSummary[] {
