...
import { resolveNexusClawPackageRoot } from "../infra/nexusclaw-root.js";
...
function resolveMode(cfg: NexusClawConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  intro("NexusClaw doctor");

  const root = await resolveNexusClawPackageRoot({
  ...
  let cfg: NexusClawConfig = configResult.cfg;
  ...
  if (!cfg.gateway?.mode) {
    const lines = [
      ...
      `Fix: run ${formatCliCommand("nexusclaw configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("nexusclaw config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("nexusclaw setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  if (resolveMode(cfg) === "local" && hasAmbiguousGatewayAuthModeConfig(cfg)) {
    note(
      [
        ...
        `Set token mode: ${formatCliCommand("nexusclaw config set gateway.auth.mode token")}`,
        `Set password mode: ${formatCliCommand("nexusclaw config set gateway.auth.mode password")}`,
      ].join("\n"),
      "Gateway auth",
    );
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("nexusclaw doctor --fix")}" to apply changes.`);
  }
