...
describe("detectMacCloudSyncedStateDir", () => {
  ...
  it("detects state dir under Library/CloudStorage", () => {
    const stateDir = path.join(home, "Library", "CloudStorage", "Dropbox", "NexusClaw", ".nexusclaw");
  ...
  it("detects cloud-synced target when state dir resolves via symlink", () => {
    const symlinkPath = "/tmp/nexusclaw-state";
  ...
  it("ignores cloud-synced symlink prefix when resolved target is local", () => {
    ...
    const resolvedLocalPath = path.join(home, ".nexusclaw");
  ...
  it("anchors cloud detection to OS homedir when NEXUSCLAW_HOME is overridden", () => {
    const stateDir = path.join(home, "Library", "CloudStorage", "iCloud Drive", ".nexusclaw");
    const originalNexusClawHome = process.env.NEXUSCLAW_HOME;
    process.env.NEXUSCLAW_HOME = "/tmp/nexusclaw-home-override";
