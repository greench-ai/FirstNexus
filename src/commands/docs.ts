...
const SEARCH_TOOL = "https://docs.nexusclaw.ai/mcp.SearchNexusClaw";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.nexusclaw.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('nexusclaw docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.nexusclaw.ai/");
      runtime.log(`Search: ${formatCliCommand('nexusclaw docs "your query"')}`);
    }
    return;
  }
