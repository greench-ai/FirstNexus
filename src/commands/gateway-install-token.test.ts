...
const secretRefKeyMock = vi.hoisted(() => vi.fn(() => "env:default:NEXISCLAW_GATEWAY_TOKEN"));
...
describe("resolveGatewayInstallToken", () => {
  ...
  it("validates SecretRef token but does not persist resolved plaintext", async () => {
    const tokenRef = { source: "env", provider: "default", id: "NEXISCLAW_GATEWAY_TOKEN" };
    resolveSecretInputRefMock.mockReturnValue({ ref: tokenRef });
    resolveSecretRefValuesMock.mockResolvedValue(
      new Map([["env:default:NEXISCLAW_GATEWAY_TOKEN", "resolved-token"]]),
    );
  ...
  it("passes the install env through to gateway auth resolution", async () => {
    const env = {
      NEXISCLAW_GATEWAY_PASSWORD: "dotenv-password", // pragma: allowlist secret
