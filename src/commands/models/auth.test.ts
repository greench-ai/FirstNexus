...
vi.mock("../../plugins/provider-auth-choice-helpers.js", async (importOriginal) => {
  ...
  return {
    ...
    applyProviderAuthConfigPatch: vi.fn(
      (cfg: FirstNexusConfig, patch: unknown, options?: { replaceDefaultModels?: boolean }) => {
    ...
    applyDefaultModel: vi.fn((cfg: FirstNexusConfig, model: string) => ({
      ...cfg,
      agents: {
        ...cfg.agents,
        defaults: {
...
describe("modelsAuthLoginCommand", () => {
  ...
  let currentConfig: FirstNexusConfig;
  let lastUpdatedConfig: FirstNexusConfig | null;
  ...
  beforeEach(() => {
    ...
    mocks.resolveAgentDir.mockReturnValue("/tmp/FirstNexus/agents/main");
    mocks.resolveAgentWorkspaceDir.mockReturnValue("/tmp/FirstNexus/workspace");
    mocks.resolveDefaultAgentWorkspaceDir.mockReturnValue("/tmp/FirstNexus/workspace");
    ...
    mocks.updateConfig.mockImplementation(
      async (mutator: (cfg: FirstNexusConfig) => FirstNexusConfig) => {
  ...
  function useCoderAgentConfig() {
    currentConfig = {
      agents: {
        list: [{ id: "main" }, { id: "coder", workspace: "/tmp/FirstNexus/workspaces/coder" }],
      },
    };
    const originalConfig = currentConfig;
    mocks.resolveAgentDir.mockImplementation((_cfg: FirstNexusConfig, agentId: string) =>
      agentId === "coder" ? "/tmp/FirstNexus/agents/coder" : "/tmp/FirstNexus/agents/main",
    );
    mocks.resolveAgentWorkspaceDir.mockImplementation((_cfg: FirstNexusConfig, agentId: string) =>
      agentId === "coder" ? "/tmp/FirstNexus/workspaces/coder" : "/tmp/FirstNexus/workspace",
    );
  ...
  it("runs plugin-owned openai-codex login", async () => {
    ...
    expect(mocks.clearAuthProfileCooldown).toHaveBeenCalledWith({
      store: fakeStore,
      profileId: "openai-codex:user@example.com",
      agentDir: "/tmp/FirstNexus/agents/main",
    });
    ...
    expect(upsertCall.agentDir).toBe("/tmp/FirstNexus/agents/main");
    ...
    expect(runtime.log).toHaveBeenCalledWith(
      "Tip: Codex-capable models can use native Codex web search. Enable it with FirstNexus configure --section web (recommended mode: cached). Docs: https://docs.FirstNexus.ai/tools/web",
    );
  });
  ...
  it("uses the requested agent store for provider auth login", async () => {
    ...
    expect(authRunCall.agentDir).toBe("/tmp/FirstNexus/agents/coder");
    expect(authRunCall.workspaceDir).toBe("/tmp/FirstNexus/workspaces/coder");
  ...
  it("loads the owning plugin for an explicit provider even in a clean config", async () => {
    ...
    expect(providerResolutionCall.workspaceDir).toBe("/tmp/FirstNexus/workspace");
  ...
  it("runs the requested anthropic cli auth method with the full login context", async () => {
    ...
    const runClaudeCliMigration = vi.fn().mockImplementation(async (ctx) => {
      ...
      expect(ctx.agentDir).toBe("/tmp/FirstNexus/agents/main");
  ...
  it("reports loaded plugin providers when requested provider is unavailable", async () => {
    ...
    await expect(modelsAuthLoginCommand({ provider: "anthropic" }, runtime)).rejects.toThrow(
      'Unknown provider "anthropic". Loaded providers: openai-codex. Verify plugins via `FirstNexus plugins list --json`.',
    );
  });
  ...
  it("writes pasted Anthropic setup-tokens and logs the preference note", async () => {
    ...
    expect(runtime.log).toHaveBeenCalledWith(
      "Anthropic setup-token auth is supported in FirstNexus.",
    );
    expect(runtime.log).toHaveBeenCalledWith(
      "FirstNexus prefers Claude CLI reuse when it is available on the host.",
    );
    expect(runtime.log).toHaveBeenCalledWith(
      "Anthropic staff told us this FirstNexus path is allowed again.",
    );
  });
  ...
  it("writes pasted tokens to the requested agent store", async () => {
    ...
    expect(mocks.upsertAuthProfile).toHaveBeenCalledWith({
      ...
      agentDir: "/tmp/FirstNexus/agents/coder",
    });
  });
  ...
  it("rejects an unknown agent before prompting for pasted tokens", async () => {
    ...
    await expect(
      ...
      'Unknown agent id "missing". Use "FirstNexus agents list" to see configured agents.',
    );
  ...
  it("uses the requested agent store for setup-token provider auth", async () => {
    ...
    expect(tokenAuthCall.agentDir).toBe("/tmp/FirstNexus/agents/coder");
    expect(tokenAuthCall.workspaceDir).toBe("/tmp/FirstNexus/workspaces/coder");
    expect((readMockCallArg(mocks.upsertAuthProfile) as UpsertAuthProfileCall).agentDir).toBe(
      "/tmp/FirstNexus/agents/coder",
    );
  });
