...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveNexusClawAgentDir: vi.fn().mockReturnValue("/tmp/nexusclaw-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/nexusclaw-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/nexusclaw-agent/auth-profiles.json"),
    ...
    createConfigIO: vi.fn().mockReturnValue({
      configPath: "/tmp/nexusclaw-dev/nexusclaw.json",
    }),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveNexusClawAgentDir: mocks.resolveNexusClawAgentDir,
}));
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/nexusclaw-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveNexusClawAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.configPath).toBe("/tmp/nexusclaw-dev/nexusclaw.json");
    expect(payload.auth.storePath).toBe("/tmp/nexusclaw-agent/auth-profiles.json");
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/nexusclaw-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/nexusclaw-agent-custom");
