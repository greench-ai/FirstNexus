...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/FirstNexus-agent"),
    resolveAgentWorkspaceDir: vi.fn().mockReturnValue("/tmp/FirstNexus-agent/workspace"),
    ...
    resolveAuthStorePathForDisplay: vi.fn(
      (agentDir?: string) => `${agentDir ?? "/tmp/FirstNexus-agent"}/auth-profiles.json`,
    ),
...
vi.mock("../../agents/workspace.js", () => ({
  resolveDefaultAgentWorkspaceDir: vi.fn().mockReturnValue("/tmp/FirstNexus-agent/workspace"),
}));
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/FirstNexus-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(payload.configPath).toBe("/tmp/FirstNexus-dev/FirstNexus.json");
    expect(payload.auth.storePath).toBe("/tmp/FirstNexus-agent/auth-profiles.json");
  ...
  it("honors NEXISCLAW_AGENT_DIR when no --agent override is provided", async () => {
    ...
    process.env.NEXISCLAW_AGENT_DIR = "/tmp/FirstNexus-isolated-agent";
    ...
    expect(mocks.ensureAuthProfileStore).toHaveBeenCalledWith("/tmp/FirstNexus-isolated-agent");
    const payload = parseFirstJsonLog(localRuntime);
    expect(payload.agentDir).toBe("/tmp/FirstNexus-isolated-agent");
    expect(payload.auth.storePath).toBe("/tmp/FirstNexus-isolated-agent/auth-profiles.json");
  });
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/FirstNexus-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/FirstNexus-agent-custom");
        ...
        expect(openAiCodex?.effective).toEqual({
          kind: "profiles",
          detail: "/tmp/FirstNexus-agent-custom/auth-profiles.json",
        });
      },
    );
  });
  ...
  it("includes auth-evidence-only providers in the auth overview", async () => {
    ...
    mocks.resolveEnvApiKey.mockImplementation(
      (provider: string, _env?: NodeJS.ProcessEnv, options?: { workspaceDir?: string }) =>
        ...
        options?.workspaceDir === "/tmp/FirstNexus-agent/workspace"
          ? {
