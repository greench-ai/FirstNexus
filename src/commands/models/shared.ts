...
export async function loadValidConfigOrThrow(): Promise<FirstNexusConfig> {
...
export async function updateConfig(
  mutator: (cfg: FirstNexusConfig) => FirstNexusConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: FirstNexusConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: FirstNexusConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("FirstNexus agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
