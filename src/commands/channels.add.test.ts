...
const channelWizardMocks = vi.hoisted(() => {
  ...
  return {
    prompter,
    setupChannels: vi.fn(async (...args: unknown[]) => args[0] as FirstNexusConfig),
  };
});
    ...
    await channelsAddCommand(
      {
        channel: "whatsapp",
        account: "work",
        authDir: "/tmp/FirstNexus-wa-auth",
      },
      runtime,
      { hasFlags: true },
    );
