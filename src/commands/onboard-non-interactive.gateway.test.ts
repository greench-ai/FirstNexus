...
describe("onboard (non-interactive): gateway and remote auth", () => {
  ...
  const initStateDir = async (prefix: string) => {
    ...
    process.env.NEXUSCLAW_STATE_DIR = stateDir;
  ...
  beforeAll(async () => {
    ...
    tempHome = await makeTempWorkspace("nexusclaw-onboard-");
  ...
  it("writes gateway token auth into config", async () => {
    await withStateDir("state-noninteractive-", async (stateDir) => {
      ...
      const workspace = path.join(stateDir, "nexusclaw");
  ...
  it("uses NEXUSCLAW_GATEWAY_TOKEN when --gateway-token is omitted", async () => {
    await withStateDir("state-env-token-", async (stateDir) => {
      const envToken = "tok_env_fallback_123";
      const workspace = path.join(stateDir, "nexusclaw");
      const prevToken = process.env.NEXUSCLAW_GATEWAY_TOKEN;
      process.env.NEXUSCLAW_GATEWAY_TOKEN = envToken;
      ...
      try {
        ...
        if (prevToken === undefined) {
          ...
          process.env.NEXUSCLAW_GATEWAY_TOKEN = prevToken;
        }
      }
    });
  }, 60_000);
        ...
        await runNonInteractiveSetup(
          {
            ...
            gatewayBind: "loopback",
            gatewayAuth: "token",
            gatewayTokenRefEnv: "NEXUSCLAW_GATEWAY_TOKEN",
          },
          runtime,
        );
        ...
        runNonInteractiveSetup(
          {
            nonInteractive: true,
            mode: "local",
            workspace: path.join(stateDir, "nexusclaw"),
            authChoice: "skip",
            skipSkills: true,
            skipHealth: false,
            installDaemon: false,
  ...
  it("emits structured JSON diagnostics when daemon health fails", async () => {
    await withStateDir("state-local-daemon-health-json-fail-", async (stateDir) => {
      ...
      expect(parsed.hints).toContain("Run `nexusclaw gateway status --deep` for more detail.");
  ...
  it("auto-generates token auth when binding LAN and persists the token", async () => {
    ...
    await withStateDir("state-lan-", async (stateDir) => {
      process.env.NEXUSCLAW_STATE_DIR = stateDir;
      process.env.NEXUSCLAW_CONFIG_PATH = path.join(stateDir, "nexusclaw.json");
