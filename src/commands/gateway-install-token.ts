...
import { readConfigFileSnapshot, writeConfigFile, type NexusClawConfig } from "../config/config.js";
...
function formatAmbiguousGatewayAuthModeReason(): string {
  return [
    ...
    `Set ${formatCliCommand("nexusclaw config set gateway.auth.mode token")} or ${formatCliCommand("nexusclaw config set gateway.auth.mode password")}.`,
  ].join(" ");
}
