...
const LOCAL_TUI_CMD_RE =
  /(?:^|\s)(?:FirstNexus-tui|FirstNexus\s+tui|FirstNexus\s+chat|FirstNexus\s+terminal)(?:\s|$)/;
...
function hasWhatsappEnabled(cfg: FirstNexusConfig): boolean {
...
export async function noteWhatsappResponsivenessHealth(params: {
  ...
  if (gatewayDegraded && tuiProcesses.length > 0) {
    ...
    if (params.shouldRepair) {
      ...
      warnings.push(
        `Fix: close those TUI sessions, or run ${formatCliCommand("FirstNexus doctor --fix")}.`,
      );
    }
  }
