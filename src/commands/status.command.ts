...
export async function statusCommand(
  ...
  const daemonValue = (() => {
    ...
    const installedPrefix = daemon.managedByNexusClaw ? "installed · " : "";
  ...
  const nodeDaemonValue = (() => {
    ...
    const installedPrefix = nodeDaemon.managedByNexusClaw ? "installed · " : "";
  ...
  runtime.log(theme.heading("NexusClaw status"));
  runtime.log("");
  ...
  if (pairingRecovery) {
    ...
    if (pairingRecovery.requestId) {
      runtime.log(
        theme.muted(
          `Recovery: ${formatCliCommand(`nexusclaw devices approve ${pairingRecovery.requestId}`)}`,
        ),
      );
    }
    runtime.log(theme.muted(`Fallback: ${formatCliCommand("nexusclaw devices approve --latest")}`));
    runtime.log(theme.muted(`Inspect: ${formatCliCommand("nexusclaw devices list")}`));
  }
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("nexusclaw security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("nexusclaw security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.nexusclaw.ai/faq");
  runtime.log("Troubleshooting: https://docs.nexusclaw.ai/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("nexusclaw status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("nexusclaw logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("nexusclaw status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("nexusclaw gateway probe")}`);
  }
}
