...
describe("status-all diagnosis port checks", () => {
  ...
  it("labels FirstNexus Tailscale exposure separately from daemon state", async () => {
  ...
  it("treats same-process dual-stack loopback listeners as healthy", async () => {
    const params = createBaseParams([
      { pid: 5001, commandLine: "FirstNexus-gateway", address: "127.0.0.1:18789" },
      { pid: 5001, commandLine: "FirstNexus-gateway", address: "[::1]:18789" },
    ]);
  ...
  it("treats a single wildcard Gateway listener as healthy", async () => {
    ...
    const output = params.lines.join("\n");
    expect(output).toContain("✓ Port 18789");
    expect(output).toContain("Detected FirstNexus Gateway listener on the configured port.");
    expect(output).not.toContain("Port 18789 is already in use.");
  });
  ...
  it("keeps warning for multi-process listener conflicts", async () => {
    const params = createBaseParams([
      { pid: 5001, commandLine: "FirstNexus-gateway", address: "127.0.0.1:18789" },
      { pid: 5002, commandLine: "FirstNexus-gateway", address: "[::1]:18789" },
    ]);
  ...
  it("does not read or display stale stderr tails on Darwin", async () => {
    ...
    try {
      restartLogMocks.resolveGatewayLogPaths.mockReturnValue({
        logDir: "/tmp/FirstNexus/logs",
        stdoutPath: "/tmp/FirstNexus/logs/gateway.log",
        stderrPath: "/tmp/FirstNexus/logs/gateway.err.log",
      });
      restartLogMocks.resolveGatewayRestartLogPath.mockReturnValue(
        "/tmp/FirstNexus/logs/gateway-restart.log",
      );
      ...
      expect(gatewayMocks.readFileTailLines).not.toHaveBeenCalledWith(
        "/tmp/FirstNexus/logs/gateway.err.log",
        40,
      );
      expect(output).toContain("# stdout: /tmp/FirstNexus/logs/gateway.log");
