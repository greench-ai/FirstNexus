...
export async function maybeRepairGatewayDaemon(params: {
  ...
  if (process.platform === "darwin") {
    const label = resolveGatewayLaunchAgentLabel(process.env.NEXUSCLAW_PROFILE);
    note(
      `LaunchAgent loaded; stopping requires "${formatCliCommand("nexusclaw gateway stop")}" or launchctl bootout gui/$UID/${label}.`,
      "Gateway",
    );
  }
