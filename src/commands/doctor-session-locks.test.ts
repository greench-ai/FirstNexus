...
describe("noteSessionLockHealth", () => {
  let state: FirstNexusTestState;

  beforeEach(async () => {
    note.mockClear();
    state = await createFirstNexusTestState({
      layout: "state-only",
      prefix: "FirstNexus-doctor-locks-",
    });
  });
  ...
  it("reports existing lock files with pid status and age", async () => {
    ...
    await noteSessionLockHealth({
      shouldRepair: false,
      staleMs: 60_000,
      readOwnerProcessArgs: () => ["node", "/opt/FirstNexus/FirstNexus.mjs", "doctor"],
    });
  ...
  it("removes fresh live locks when the owner is not an FirstNexus process", async () => {
    const sessionsDir = state.sessionsDir();
    await fs.mkdir(sessionsDir, { recursive: true });
    ...
    expect(note).toHaveBeenCalledTimes(1);
    const [message] = note.mock.calls.at(0) as [string, string];
    expect(message).toContain("stale=yes (non-FirstNexus-owner)");
    expect(message).toContain("[removed]");
    expect(message).toContain("Removed 1 stale session lock file");
