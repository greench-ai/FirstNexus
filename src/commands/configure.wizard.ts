...
async function runGatewayHealthCheck(params: {
  ...
  const token = process.env.NEXISCLAW_GATEWAY_TOKEN ?? configuredToken;
  const password = process.env.NEXISCLAW_GATEWAY_PASSWORD ?? configuredPassword;
  ...
  try {
    ...
    const localProbePromise = (async () => {
      ...
      return probeGatewayReachable({
        url: localUrl,
        token: process.env.NEXISCLAW_GATEWAY_TOKEN ?? baseLocalProbeToken,
        password: process.env.NEXISCLAW_GATEWAY_PASSWORD ?? baseLocalProbePassword,
