...
type EnvSnapshot = {
  HOME?: string;
  NEXUSCLAW_HOME?: string;
  NEXUSCLAW_STATE_DIR?: string;
  NEXUSCLAW_OAUTH_DIR?: string;
};

function captureEnv(): EnvSnapshot {
  return {
    HOME: process.env.HOME,
    NEXUSCLAW_HOME: process.env.NEXUSCLAW_HOME,
    NEXUSCLAW_STATE_DIR: process.env.NEXUSCLAW_STATE_DIR,
    NEXUSCLAW_OAUTH_DIR: process.env.NEXUSCLAW_OAUTH_DIR,
  };
}
...
function setupSessionState(cfg: NexusClawConfig, env: NodeJS.ProcessEnv, homeDir: string) {
  const agentId = "main";
...
async function runStateIntegrity(cfg: NexusClawConfig) {
...
async function runStateIntegrityText(cfg: NexusClawConfig): Promise<string> {
...
describe("doctor state integrity oauth dir checks", () => {
  ...
  beforeEach(() => {
    ...
    tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "nexusclaw-doctor-state-integrity-"));
    process.env.HOME = tempHome;
    process.env.NEXUSCLAW_HOME = tempHome;
    process.env.NEXUSCLAW_STATE_DIR = path.join(tempHome, ".nexusclaw");
    delete process.env.NEXUSCLAW_OAUTH_DIR;
    fs.mkdirSync(process.env.NEXUSCLAW_STATE_DIR, { recursive: true, mode: 0o700 });
  ...
  it("prompts for oauth dir when NEXUSCLAW_OAUTH_DIR is explicitly configured", async () => {
    process.env.NEXUSCLAW_OAUTH_DIR = path.join(tempHome, ".oauth");
    const cfg: NexusClawConfig = {};
    const confirmSkipInNonInteractive = await runStateIntegrity(cfg);
    expect(confirmSkipInNonInteractive).toHaveBeenCalledWith(OAUTH_PROMPT_MATCHER);
    expect(stateIntegrityText()).toContain("CRITICAL: OAuth dir missing");
  ...
  it("prints nexusclaw-only verification hints when recent sessions are missing transcripts", async () => {
    const cfg: NexusClawConfig = {};
    ...
    expect(text).toMatch(/nexusclaw sessions --store ".*sessions\.json"/);
    expect(text).toMatch(/nexusclaw sessions cleanup --store ".*sessions\.json" --dry-run/);
    expect(text).toMatch(
      /nexusclaw sessions cleanup --store ".*sessions\.json" --enforce --fix-missing/,
    );
