...
describe("setupWizardCommand", () => {
  ...
  it("fails fast for invalid secret-input-mode before setup starts", async () => {
    ...
    expect(runtime.error).toHaveBeenCalledWith(
      `Invalid --secret-input-mode. Use "plaintext" or "ref", or run ${formatCliCommand("FirstNexus onboard")} for the interactive setup.`,
    );
  ...
  it("logs ASCII-safe Windows guidance before setup", async () => {
    ...
    try {
      ...
      expect(runtime.log).toHaveBeenCalledWith(
        [
          "Windows detected - FirstNexus runs great on WSL2!",
          "Native Windows might be trickier.",
          "Quick setup: wsl --install (one command, one reboot)",
          "Guide: https://docs.FirstNexus.ai/windows",
        ].join("\n"),
      );
    } finally {
  ...
  it("uses configured default workspace for --reset when --workspace is not provided", async () => {
    ...
    mocks.readConfigFileSnapshot.mockResolvedValue({
      ...
      config: {
        agents: {
          defaults: {
            workspace: "/tmp/FirstNexus-custom-workspace",
          },
        },
      },
    });
    ...
    expect(mocks.handleReset).toHaveBeenCalledWith(
      ...
      path.resolve("/tmp/FirstNexus-custom-workspace"),
      runtime,
    );
  });
  ...
  it("fails fast for invalid --reset-scope", async () => {
    ...
    expect(runtime.error).toHaveBeenCalledOnce();
    expect(runtime.error).toHaveBeenCalledWith(
      `Invalid --reset-scope. Use "config", "config+creds+sessions", or "full". Run ${formatCliCommand("FirstNexus onboard --reset --reset-scope config")} for a config-only reset.`,
    );
    expect(runtime.exit).toHaveBeenCalledWith(1);
