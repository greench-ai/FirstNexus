...
describe("exportTrajectoryCommand", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mocks.resolveDefaultSessionStorePath.mockReturnValue("/tmp/FirstNexus/sessions.json");
  ...
  it("points missing session key users at the sessions command", async () => {
    ...
    expect(runtime.error).toHaveBeenCalledWith(
      "--session-key is required. Run FirstNexus sessions to choose a session.",
    );
  ...
  it("points missing session users at the sessions command", async () => {
    ...
    expect(runtime.error).toHaveBeenCalledWith(
      "Session not found: agent:main:telegram:direct:123. Run FirstNexus sessions to see available sessions.",
    );
    expect(runtime.exit).toHaveBeenCalledWith(1);
  });
});
