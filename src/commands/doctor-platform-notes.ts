...
export async function noteMacLaunchAgentOverrides() {
  ...
  const markerCandidates = [path.join(home, ".nexusclaw", "disable-launchagent")];
...
function hasConfigGatewayCreds(cfg: NexusClawConfig): boolean {
...
export async function noteMacLaunchctlGatewayEnvOverrides(
  ...
  if (deprecatedLaunchctlEntries.length > 0) {
    const lines = [
      ...
      ...deprecatedLaunchctlEntries.map(
        ([key]) =>
          `- \`${key}\` is set; use \`NEXUSCLAW_${key.slice(key.indexOf("_") + 1)}\` instead.`,
      ),
    ];
  ...
  const tokenEntries = [
    ["NEXUSCLAW_GATEWAY_TOKEN", await getenv("NEXUSCLAW_GATEWAY_TOKEN")],
  ] as const;
  const passwordEntries = [
    ["NEXUSCLAW_GATEWAY_PASSWORD", await getenv("NEXUSCLAW_GATEWAY_PASSWORD")],
  ] as const;
  ...
  const lines = [
    ...
    envToken && envTokenKey
      ...
      : undefined,
    envPassword
      ? `- \`${envPasswordKey ?? "NEXUSCLAW_GATEWAY_PASSWORD"}\` is set; it overrides config passwords.`
      : undefined,
...
export function noteDeprecatedLegacyEnvVars(
  ...
  const lines = [
    ...
    "- Use NEXUSCLAW_* equivalents instead:",
    ...entries.map((key) => {
      const suffix = key.slice(key.indexOf("_") + 1);
      return `  ${key} -> NEXUSCLAW_${suffix}`;
    }),
  ];
  (deps?.noteFn ?? note)(lines.join("\n"), "Environment");
}
...
export function noteStartupOptimizationHints(
  ...
  const noRespawn = env.NEXUSCLAW_NO_RESPAWN?.trim() ?? "";
  const lines: string[] = [];
  ...
  if (noRespawn !== "1") {
    lines.push(
      "- NEXUSCLAW_NO_RESPAWN is not set to 1; set it to avoid extra startup overhead from self-respawn.",
    );
  }
  ...
  const suggestions = [
    ...
    "  export NODE_COMPILE_CACHE=/var/tmp/nexusclaw-compile-cache",
    "  mkdir -p /var/tmp/nexusclaw-compile-cache",
    "  export NEXUSCLAW_NO_RESPAWN=1",
