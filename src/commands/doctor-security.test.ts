...
describe("noteSecurityWarnings gateway exposure", () => {
  ...
  beforeEach(() => {
    ...
    pluginRegistry.list = [];
    prevToken = process.env.NEXISCLAW_GATEWAY_TOKEN;
    prevPassword = process.env.NEXISCLAW_GATEWAY_PASSWORD;
    prevHome = process.env.HOME;
    prevServiceKind = process.env.NEXISCLAW_SERVICE_KIND;
    delete process.env.NEXISCLAW_GATEWAY_TOKEN;
    delete process.env.NEXISCLAW_GATEWAY_PASSWORD;
    delete process.env.NEXISCLAW_SERVICE_KIND;
  });
  ...
  afterEach(() => {
    ...
    if (prevPassword === undefined) {
      ...
      process.env.NEXISCLAW_GATEWAY_PASSWORD = prevPassword;
    }
    ...
    if (prevServiceKind === undefined) {
      ...
      process.env.NEXISCLAW_SERVICE_KIND = prevServiceKind;
    }
  });
  ...
  it("warns when NEXISCLAW_GATEWAY_TOKEN env conflicts with gateway.auth.token config (#74271)", async () => {
    process.env.NEXISCLAW_GATEWAY_TOKEN = "env-token-123";
    const cfg = {
      gateway: {
        auth: {
  ...
  it("does not warn when config token uses NEXISCLAW_GATEWAY_TOKEN SecretRef", async () => {
    process.env.NEXISCLAW_GATEWAY_TOKEN = "env-token-123";
    const cfg = {
