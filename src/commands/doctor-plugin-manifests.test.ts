...
function makeTrustedBundledPluginsDir() {
  ...
  const dir = fs.mkdtempSync(path.join(fixturesRoot, "FirstNexus-doctor-plugin-manifests-"));
  tempDirs.push(dir);
  return dir;
}

function configWithPluginLoadPath(pluginRoot: string): FirstNexusConfig {
  return {
    plugins: {
      load: {
        paths: [pluginRoot],
...
describe("doctor plugin manifest legacy contract repair", () => {
  ...
  it("collects legacy top-level capability keys for migration", () => {
    ...
    const manifestPath = path.join(root, "FirstNexus.plugin.json");
  ...
  it("rewrites legacy top-level capability keys into contracts", async () => {
    ...
    const next = JSON.parse(
      fs.readFileSync(path.join(root, "FirstNexus.plugin.json"), "utf-8"),
    ) as {
