...
const ensureNexusClawModelsJson = vi.fn().mockResolvedValue(undefined);
const resolveNexusClawAgentDir = vi.fn().mockReturnValue("/tmp/nexusclaw-agent");
...
beforeEach(() => {
  ...
  ensureNexusClawModelsJson.mockClear();
...
describe("models list/status", () => {
  ...
  it("modelsListCommand persists using the write snapshot config when provided", async () => {
    ...
    expect(ensureNexusClawModelsJson).toHaveBeenCalled();
    expect(ensureNexusClawModelsJson.mock.calls[0]?.[0]).toEqual(resolvedConfig);
  });
