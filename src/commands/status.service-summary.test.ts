...
describe("readServiceStatusSummary", () => {
  it("marks FirstNexus-managed services as installed", async () => {
    const summary = await readServiceStatusSummary(
      createService({
        isLoaded: vi.fn(async () => true),
        readCommand: vi.fn(async () => ({ programArguments: ["FirstNexus", "gateway", "run"] })),
    ...
    expect(summary.managedByFirstNexus).toBe(true);
  ...
  it("marks running unmanaged services as externally managed", async () => {
    ...
    expect(summary.installed).toBe(true);
    expect(summary.managedByFirstNexus).toBe(false);
    ...
    const summary = await readServiceStatusSummary(
      createService({
        ...
        readCommand: vi.fn(async () => ({
          programArguments: ["FirstNexus", "gateway", "run", "--port", "18789"],
  ...
  it("includes service layout diagnostics and flags source checkout entrypoints", async () => {
    await withTempDir({ prefix: "FirstNexus-status-service-layout-" }, async (root) => {
      ...
      const serviceFile = path.join(root, "FirstNexus-gateway.service");
      ...
      expect(layout.sourcePathReal).toBe(path.join(realRoot, "FirstNexus-gateway.service"));
