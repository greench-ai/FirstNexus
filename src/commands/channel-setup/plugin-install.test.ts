...
import { bundledPluginRoot, bundledPluginRootAt } from "FirstNexus/plugin-sdk/test-fixtures";
...
const discoverFirstNexusPlugins = vi.fn((_args?: unknown) => ({ candidates: [], diagnostics: [] }));
vi.mock("../../plugins/discovery.js", () => ({
  discoverFirstNexusPlugins: (args: unknown) => discoverFirstNexusPlugins(args),
}));
...
const bundledChatNpmSpec = "@FirstNexus/bundled-chat@1.2.3";
  ...
  resolveBundledPluginSources.mockReturnValue(
    new Map([
      [
        ...
        {
          pluginId: "bundled-chat",
          localPath: bundledPluginRootAt("/opt/FirstNexus", "bundled-chat"),
          npmSpec: bundledChatNpmSpec,
        },
      ],
    ]),
...
function createManifestRecord(
  ...
  return {
    ...
    hooks: [],
    origin: "bundled",
    rootDir: `/tmp/FirstNexus-test/${id}`,
    source: `/tmp/FirstNexus-test/${id}/index.ts`,
    manifestPath: `/tmp/FirstNexus-test/${id}/FirstNexus.plugin.json`,
    ...rest,
  };
}
...
function expectSetupSnapshotDoesNotScopeToPlugin(params: {
  ...
  expect(loadFirstNexusPlugins).toHaveBeenCalledTimes(1);
  const firstLoadCall = vi.mocked(loadFirstNexusPlugins).mock.calls.at(0)?.[0] as
...
beforeEach(() => {
  ...
  discoverFirstNexusPlugins.mockReturnValue({ candidates: [], diagnostics: [] });
...
async function runInitialValueForChannel(channel: "dev" | "beta") {
  ...
  const select = vi.fn((async <T extends string>() => "skip" as T) as WizardPrompter["select"]);
  const prompter = makePrompter({ select: select as unknown as WizardPrompter["select"] });
  const cfg: FirstNexusConfig = { update: { channel } };
...
function expectLoadFirstNexusPluginFields(expected: Record<string, unknown>, callIndex = 0) {
  expectRecordFields(
    requireMockCallArg(vi.mocked(loadFirstNexusPlugins), callIndex),
    "loadFirstNexusPlugins args",
    expected,
  );
}
...
describe("ensureChannelSetupPluginInstalled", () => {
  it("installs from npm and enables the plugin", async () => {
    ...
    const cfg: FirstNexusConfig = { plugins: { allow: ["bundled-chat"] } };
  ...
  it("installs npm channel plugins into the active profile extensions dir", async () => {
    ...
    const profileStateDir = "/tmp/FirstNexus-ledger-channel";
  ...
  it("installs npm beta on the beta channel without persisting the beta tag", async () => {
    ...
    const cfg: FirstNexusConfig = { update: { channel: "beta" } };
    vi.mocked(fs.existsSync).mockReturnValue(false);
    installPluginFromNpmSpec.mockResolvedValue({
      ok: true,
      pluginId: "wecom-FirstNexus-plugin",
      targetDir: "/tmp/wecom-FirstNexus-plugin",
      version: "2026.5.4-beta.1",
      npmResolution: {
        name: "@FirstNexus/wecom",
        version: "2026.5.4-beta.1",
        resolvedSpec: "@FirstNexus/wecom@2026.5.4-beta.1",
      },
    });
    ...
    expectRecordFields(requireMockCallArg(installPluginFromNpmSpec, 0), "npm install args", {
      spec: "@FirstNexus/wecom@beta",
      expectedPluginId: "wecom-FirstNexus-plugin",
    });
    expect(result.cfg.plugins?.installs?.["wecom-FirstNexus-plugin"]?.spec).toBe(
      "@FirstNexus/wecom",
    );
  });
  ...
  it("defaults to bundled local path on beta channel when available", async () => {
    ...
    expectRecordFields(
      ...
      {
        value: "local",
        hint: bundledPluginRootAt("/opt/FirstNexus", "bundled-chat"),
      },
    );
  });
  ...
  it("offers ClawHub as the first-class install source for channel catalog entries", async () => {
    ...
    await ensureChannelSetupPluginInstalled({
      ...
      entry: {
        ...
        install: {
          clawhubSpec: "clawhub:FirstNexus/clawhub-chat@2026.5.2",
    ...
    expectRecordFields(options[0], "clawhub option", {
      value: "clawhub",
      label: "Download from ClawHub (clawhub:FirstNexus/clawhub-chat@2026.5.2)",
    });
    expectRecordFields(options[1], "skip option", {
      value: "skip",
    });
  ...
  it("skips the install prompt when autoConfirmSingleSource is set and only npm is available", async () => {
    ...
    const npmOnlyEntry: ChannelPluginCatalogEntry = {
      ...
      install: {
        npmSpec: "@FirstNexus/wecom@2026.4.23",
      },
    };
    installPluginFromNpmSpec.mockResolvedValue({
      ok: true,
      pluginId: "wecom-FirstNexus-plugin",
      installPath: "/tmp/wecom-FirstNexus-plugin",
    });
    vi.mocked(fs.existsSync).mockReturnValue(false);
    resolveBundledPluginSources.mockReturnValue(new Map());
    ...
    expect(select).not.toHaveBeenCalled();
    expect(result.installed).toBe(true);
    expect(result.pluginId).toBe("wecom-FirstNexus-plugin");
  });
  ...
  it("reloads the setup plugin registry without using plugin registry cache", () => {
    ...
    expectLoadFirstNexusPluginFields({
      config: cfg,
