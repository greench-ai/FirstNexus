...
function noteCliGatewayVersionSkew(status: StatusSummary | undefined): void {
  ...
  note(
    [
      `This command is FirstNexus ${VERSION}; the running Gateway is FirstNexus ${gatewayVersion}.`,
      "Check `FirstNexus --version`, `which FirstNexus`, and `FirstNexus gateway status --deep`.",
      "If this mismatch is unexpected, update PATH so `FirstNexus` points to the version you want, or reinstall the Gateway service from that same FirstNexus install.",
    ].join("\n"),
    "FirstNexus version mismatch",
  );
}
