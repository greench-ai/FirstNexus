...
async function maybeMigrateLegacyConfig(): Promise<string[]> {
  ...
  const targetDir = path.join(home, ".FirstNexus");
  const targetPath = path.join(targetDir, "FirstNexus.json");
  try {
...
export async function runDoctorConfigPreflight(
  ...
  if (options.repairPrefixedConfig === true && snapshot.exists && !snapshot.valid) {
    if (await recoverConfigFromJsonRootSuffix(snapshot)) {
      note(
        "Removed non-JSON prefix from FirstNexus.json; original saved as .clobbered.*.",
        "Config",
      );
      ...
      note(
        "Restored FirstNexus.json from last-known-good; original saved as .clobbered.*.",
        "Config",
      );
      snapshot = addDoctorLegacyIssues(await readConfigFileSnapshot());
    }
