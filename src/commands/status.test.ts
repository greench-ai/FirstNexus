...
beforeAll(() => {
  envSnapshot = captureEnv(["NEXUSCLAW_PROFILE"]);
  process.env.NEXUSCLAW_PROFILE = "isolated";
});
...
vi.mock("../infra/nexusclaw-root.js", () => ({
  resolveNexusClawPackageRoot: vi.fn().mockResolvedValue("/tmp/nexusclaw"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/nexusclaw/pnpm-lock.yaml",
      markerPath: "/tmp/nexusclaw/node_modules/.modules.yaml",
    },
...
vi.mock("../daemon/service.js", () => ({
  resolveGatewayService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.nexusclaw.gateway.plist",
    }),
  }),
}));
vi.mock("../daemon/node-service.js", () => ({
  resolveNodeService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.nexusclaw.node.plist",
    }),
  }),
}));
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    expect(
      logs.some(
        (line) =>
          line.includes("nexusclaw status --all") ||
          line.includes("nexusclaw --profile isolated status --all"),
      ),
    ).toBe(true);
  });

  it("shows gateway auth when reachable", async () => {
    await withEnvVar("NEXUSCLAW_GATEWAY_TOKEN", "abcd1234", async () => {
