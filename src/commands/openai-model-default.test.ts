...
function expectPrimaryModelChanged(
  applied: { changed: boolean; next: NexusClawConfig },
  primary: string,
) {
...
type SharedDefaultModelCase = {
  apply: (cfg: NexusClawConfig) => { changed: boolean; next: NexusClawConfig };
  defaultModel: string;
  overrideConfig: NexusClawConfig;
  alreadyDefaultConfig: NexusClawConfig;
};
...
describe("applyDefaultModelChoice", () => {
  it("ensures allowlist entry exists when returning an agent override", async () => {
    ...
    const applied = await applyDefaultModelChoice({
      ...
      applyProviderConfig: (config: NexusClawConfig) => config,
      applyDefaultConfig: (config: NexusClawConfig) => config,
      noteAgentModel,
...
describe("shared default model behavior", () => {
  it("sets defaults when model is unset", () => {
    for (const testCase of SHARED_DEFAULT_MODEL_CASES) {
      const cfg: NexusClawConfig = { agents: { defaults: {} } };
