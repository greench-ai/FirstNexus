...
export function createStatusScanSharedMocks(configPathLabel: string): StatusScanSharedMocks {
  return {
    resolveConfigPath: vi.fn(
      () => `/tmp/FirstNexus-${configPathLabel}-missing-${process.pid}.json`,
    ),
...
export function createStatusScanConfig<T extends object = FirstNexusConfig>(
  overrides: T = {} as T,
): FirstNexusConfig & T {
  return {
    session: {},
...
export function createStatusMemorySearchConfig(): FirstNexusConfig {
