...
describe("writeOAuthCredentials", () => {
  ...
  it("writes auth-profiles.json under NEXUSCLAW_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("nexusclaw-oauth-");
  ...
  it("writes OAuth credentials to all sibling agent dirs when syncSiblingAgents=true", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "nexusclaw-oauth-sync-"));
    ...
    process.env.NEXUSCLAW_AGENT_DIR = kidAgentDir;
  ...
  it("writes OAuth credentials only to target dir by default", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "nexusclaw-oauth-nosync-"));
    process.env.NEXUSCLAW_STATE_DIR = tempStateDir;
  ...
  it("syncs siblings from explicit agentDir outside NEXUSCLAW_STATE_DIR", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "nexusclaw-oauth-external-"));
    process.env.NEXUSCLAW_STATE_DIR = tempStateDir;

    // Create standard-layout agents tree *outside* NEXUSCLAW_STATE_DIR
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to NEXUSCLAW_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("nexusclaw-minimax-", { agentSubdir: "custom-agent" });
    lifecycle.setStateDir(env.stateDir);
