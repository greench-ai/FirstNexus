...
describe("buildStatusCommandReportData", () => {
  it("builds report inputs from shared status surfaces", async () => {
    ...
    expect(result.taskMaintenanceHint).toBe(
      "Task maintenance: cmd:FirstNexus tasks maintenance --apply",
    );
    ...
    expect(result.footerLines.at(-1)).toBe("  Need to test channels? cmd:FirstNexus status --deep");
  });
  ...
  it("shows skipped audit text when fast status omits the security audit", async () => {
    ...
    expect(result.securityAuditLines).toEqual([
      "muted(Skipped in fast status. Full report: cmd:FirstNexus security audit)",
      "muted(Deep probe: cmd:FirstNexus status --deep)",
    ]);
  });
