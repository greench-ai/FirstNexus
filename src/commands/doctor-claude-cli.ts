...
function usesClaudeCliModelSelection(cfg: FirstNexusConfig): boolean {
...
function resolveClaudeCliCommand(cfg: FirstNexusConfig): string {
...
function formatWorkspaceHealthLine(
  ...
  if (health === "missing") {
    return `- ${label}: ${display} (missing; FirstNexus will create it on first run).`;
  }
...
function resolveClaudeCliAgentIds(cfg: FirstNexusConfig): string[] {
...
export function noteClaudeCliHealth(
  ...
  if (!storedProfile) {
    lines.push(
      `- FirstNexus auth profile: missing (${CLAUDE_CLI_PROFILE_ID}) in ${authStorePath}.`,
    );
    fixHints.push(
    ...
    lines.push(
      `- FirstNexus auth profile: ${CLAUDE_CLI_PROFILE_ID} is wired to provider "${storedProfile.provider}" instead of "${CLAUDE_CLI_PROVIDER}".`,
    );
    fixHints.push(
    ...
    lines.push(
      `- FirstNexus auth profile: ${CLAUDE_CLI_PROFILE_ID} (provider ${CLAUDE_CLI_PROVIDER}).`,
    );
  }
