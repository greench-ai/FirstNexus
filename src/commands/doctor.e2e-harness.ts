...
export const resolveFirstNexusPackageRoot = vi.fn().mockResolvedValue(null) as unknown as MockFn;
...
vi.mock("../plugins/loader.js", () => ({
  ...
  loadFirstNexusPlugins: () => createEmptyPluginRegistry(),
...
vi.mock("../config/config.js", async () => {
  ...
  return {
    ...actual,
    CONFIG_PATH: "/tmp/FirstNexus.json",
    createConfigIO,
...
vi.mock("FirstNexus/plugin-sdk/provider-model-shared", () => ({
  DEFAULT_CONTEXT_TOKENS: 32768,
  normalizeProviderId: (value: string) => normalizeLowercaseStringOrEmpty(value),
}));

vi.mock("FirstNexus/plugin-sdk/provider-stream-shared", () => ({
...
beforeEach(() => {
  ...
  resolveFirstNexusPackageRoot.mockReset().mockResolvedValue(null);
  ...
  originalUpdateInProgress = process.env.NEXISCLAW_UPDATE_IN_PROGRESS;
  process.env.NEXISCLAW_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "FirstNexus-doctor-state-"));
