...
async function resolveCatalogChannelEntry(raw: string, cfg: FirstNexusConfig | null) {
...
async function channelsAddCommandImpl(
  ...
  const cfg = (configSnapshot.sourceConfig ?? configSnapshot.config) as FirstNexusConfig;
  ...
  if (!channel) {
    const hint = catalogEntry
      ? `Plugin ${catalogEntry.meta.label} could not be loaded after install. Run FirstNexus doctor --fix, then retry FirstNexus channels add.`
  ...
  if (!plugin?.setup?.applyAccountConfig) {
    runtime.error(
      ...
      })} Run ${formatCliCommand("FirstNexus channels add")} with no flags for guided setup.`,
    );
    runtime.exit(1);
    return;
  }
