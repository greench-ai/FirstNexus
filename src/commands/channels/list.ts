...
export async function channelsListCommand(
    ...
    lines.push(
      theme.muted(
        showAll
          ...
          : "- no configured chat channels (run `FirstNexus channels list --all` to see installable channels)",
      ),
    );
  } else {
  ...
  runtime.log(
    theme.muted(
      "Model provider usage moved out of `channels list` — see `FirstNexus status` or `FirstNexus models list`.",
    ),
  );
