    ...
    findLegacyConfigIssues: (raw: unknown, sourceRaw?: unknown, extraRules: LegacyRule[] = []) => {
      ...
      if ("heartbeat" in root) {
        addIssue(
          issues,
          ["heartbeat"],
          'heartbeat is legacy; use agents.defaults.heartbeat and channels.defaults.heartbeat. Run "FirstNexus doctor --fix".',
        );
      }
      if ("memorySearch" in root) {
        addIssue(
          issues,
          ["memorySearch"],
          'memorySearch is legacy; use agents.defaults.memorySearch. Run "FirstNexus doctor --fix".',
        );
      }
      ...
      if (gateway && "bind" in gateway) {
        addIssue(
          issues,
          ["gateway", "bind"],
          'gateway.bind host aliases are legacy; use the canonical bind mode. Run "FirstNexus doctor --fix".',
        );
      }
      ...
      if (sessionThreadBindings && "ttlHours" in sessionThreadBindings) {
        addIssue(
          ...
          'session.threadBindings.ttlHours is legacy; use session.threadBindings.idleHours. Run "FirstNexus doctor --fix".',
        );
      }
      ...
      if (sessionMaintenance && "rotateBytes" in sessionMaintenance) {
        addIssue(
          issues,
          ["session", "maintenance"],
          'session.maintenance.rotateBytes is deprecated and ignored; run "FirstNexus doctor --fix" to remove it.',
        );
      }
      ...
      if (xSearch && "apiKey" in xSearch) {
        addIssue(
          ...
          'tools.web.x_search.apiKey is legacy; use plugins.entries.xai.config.webSearch.apiKey. Run "FirstNexus doctor --fix".',
        );
      }
      ...
      if (sandbox && "perSession" in sandbox) {
        addIssue(
          ...
          'agents.defaults.sandbox.perSession is legacy; use agents.defaults.sandbox.scope. Run "FirstNexus doctor --fix".',
        );
      }
      ...
      for (const [channelId, channelRaw] of Object.entries(channels ?? {})) {
        ...
        if (hasLegacyStreamingAlias(channel)) {
          addIssue(
            ...
            channelId === "googlechat"
              ? `channels.${channelId}.streamMode is legacy and no longer used. Run "FirstNexus doctor --fix".`
              : `channels.${channelId}.streamMode, channels.${channelId}.streaming aliases are legacy. Run "FirstNexus doctor --fix".`,
          );
        }
        ...
        if (threadBindings && "ttlHours" in threadBindings) {
          addIssue(
            ...
            'channels.<id>.threadBindings.ttlHours is legacy; use channels.<id>.threadBindings.idleHours. Run "FirstNexus doctor --fix".',
          );
        }
        if (channelId === "slack") {
          for (const roomRaw of Object.values(asRecord(channel.channels) ?? {})) {
            if ("allow" in (asRecord(roomRaw) ?? {})) {
              addIssue(
                issues,
                ["channels", "slack"],
                'channels.slack.channels.<id>.allow is legacy; use enabled. Run "FirstNexus doctor --fix".',
              );
            }
          }
        }
          ...
          for (const spaceRaw of Object.values(asRecord(channel.groups) ?? {})) {
            if ("allow" in (asRecord(spaceRaw) ?? {})) {
              addIssue(
                issues,
                ["channels", "googlechat"],
                'channels.googlechat.groups.<id>.allow is legacy; use enabled. Run "FirstNexus doctor --fix".',
              );
            }
          }
        }
            ...
            for (const roomRaw of Object.values(asRecord(guild?.channels) ?? {})) {
              if ("allow" in (asRecord(roomRaw) ?? {})) {
                addIssue(
                  issues,
                  ["channels", "discord"],
                  'channels.discord.guilds.<id>.channels.<id>.allow is legacy; use enabled. Run "FirstNexus doctor --fix".',
                );
              }
            }
          }
...
vi.mock("./doctor-config-preflight.js", async () => {
  ...
  function resolveConfigPath() {
    const stateDir =
      ...
      (process.env.HOME ? path.join(process.env.HOME, ".FirstNexus") : "");
    return process.env.NEXISCLAW_CONFIG_PATH || path.join(stateDir, "FirstNexus.json");
  }
...
describe("doctor config flow", () => {
  ...
  it("warns when hooks transformsDir points outside the hook transforms root", async () => {
    const doctorWarnings = await collectDoctorWarnings({
      hooks: {
        enabled: true,
        token: "hook-secret",
        transformsDir: "/virtual/.FirstNexus/workspace/skills/linear-webhook",
        mappings: [
          {
            match: { path: "linear" },
            action: "agent",
            messageTemplate: "Linear event",
            transform: { module: "./FirstNexus-linear-transform.js" },
          },
        ],
      },
    });
    ...
    expect(warning).toContain("/virtual/.FirstNexus/workspace/skills/linear-webhook");
    expect(warning).toContain("/virtual/.FirstNexus/hooks/transforms");
  ...
  it("keeps discord streaming aliases on disk during repair so downgrades stay recoverable", async () => {
    await withTempHome(
      async (home) => {
        const configDir = path.join(home, ".FirstNexus");
  ...
  it("converts numeric discord ids to strings on repair", async () => {
    await withTempHome(
      async (home) => {
        ...
        await fs.mkdir(configDir, { recursive: true });
        await fs.writeFile(
          path.join(configDir, "FirstNexus.json"),
          JSON.stringify(
            {
              channels: {
                discord: {
    ...
    try {
      ...
      expect(
        noteSpy.mock.calls.some(
          ([message, title]) =>
            title === "Doctor" &&
            message.includes('Run "FirstNexus doctor --fix" to migrate legacy config keys.'),
        ),
      ).toBe(true);
    } finally {
      noteSpy.mockClear();
