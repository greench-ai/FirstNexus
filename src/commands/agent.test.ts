...
vi.mock("../config/sessions/transcript-resolve.runtime.js", () => {
  ...
  const resolveSessionFile = (sessionId: string, agentId: string, sessionsDir?: string): string =>
    joinPath(sessionsDir ?? ".FirstNexus", "agents", agentId, "sessions", `${sessionId}.jsonl`);
...
function mockConfig(
  home: string,
  storePath: string,
  agentOverrides?: Partial<NonNullable<NonNullable<FirstNexusConfig["agents"]>["defaults"]>>,
  telegramOverrides?: Partial<NonNullable<NonNullable<FirstNexusConfig["channels"]>["telegram"]>>,
...
describe("agentCommand", () => {
  it("enables the Codex runtime plugin for one-shot OpenAI model overrides", async () => {
    await withTempHome(async (home) => {
      ...
      expect(registryLoad?.workspaceDir).toBe(path.join(home, "FirstNexus"));
