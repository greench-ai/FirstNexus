...
vi.mock("../agents/workspace.js", () => {
  const resolveDefaultAgentWorkspaceDir = () => "/tmp/nexusclaw-workspace";
  return {
    DEFAULT_AGENT_WORKSPACE_DIR: "/tmp/nexusclaw-workspace",
...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "nexusclaw-agent-" });
}

function mockConfig(
  home: string,
  storePath: string,
  agentOverrides?: Partial<NonNullable<NonNullable<NexusClawConfig["agents"]>["defaults"]>>,
  telegramOverrides?: Partial<NonNullable<NonNullable<NexusClawConfig["channels"]>["telegram"]>>,
...
beforeEach(() => {
  ...
  readConfigFileSnapshotForWriteSpy.mockResolvedValue({
    snapshot: { valid: false, resolved: {} as NexusClawConfig },
    writeOptions: {},
