  ...
  await withProgress({ label: "Scanning status --all…", total: 11 }, async (progress) => {
    ...
    const readServiceSummary = async (service: GatewayService) => {
      try {
        ...
        return {
          ...
          managedByNexusClaw: summary.managedByNexusClaw,
    ...
    const overviewRows = [
      ...
      ...(probeAuthResolution.warning
        ...
        : []),
      { Item: "Security", Value: `Run: ${formatCliCommand("nexusclaw security audit --deep")}` },
      gatewaySelfLine
      ...
      daemon
        ? {
            ...
            Value: !daemon.installed
              ...
              : `${daemon.label} ${daemon.managedByNexusClaw ? "installed · " : ""}${daemon.loadedText}${daemon.runtime?.status ? ` · ${daemon.runtime.status}` : ""}${daemon.runtime?.pid ? ` (pid ${daemon.runtime.pid})` : ""}`,
          }
      ...
      nodeService
        ? {
            ...
            Value: !nodeService.installed
              ...
              : `${nodeService.label} ${nodeService.managedByNexusClaw ? "installed · " : ""}${nodeService.loadedText}${nodeService.runtime?.status ? ` · ${nodeService.runtime.status}` : ""}${nodeService.runtime?.pid ? ` (pid ${nodeService.runtime.pid})` : ""}`,
          }
