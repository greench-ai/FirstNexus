...
function buildMemoryRecallIssueNote(audit: ShortTermAuditSummary): string | null {
  ...
  const guidance = hasFixableIssue
    ? `Fix: ${formatCliCommand("FirstNexus doctor --fix")} or ${formatCliCommand("FirstNexus memory status --fix")}`
    : `Verify: ${formatCliCommand("FirstNexus memory status --deep")}`;
  return [
...
export async function noteMemoryRecallHealth(cfg: FirstNexusConfig): Promise<void> {
  try {
 ...
 * Runs as part of `FirstNexus doctor` — config-only checks where possible;
...
export async function noteMemorySearchHealth(
    ...
    if (!qmdCheck.available) {
      note(
        [
          ...
          `- Set an explicit binary path: ${formatCliCommand("FirstNexus config set memory.qmd.command /absolute/path/to/qmd")}`,
          `- Or switch back to builtin memory: ${formatCliCommand("FirstNexus config set memory.backend builtin")}`,
          "",
          `Verify: ${formatCliCommand("FirstNexus memory status --deep")}`,
        ]
          .filter(Boolean)
  ...
  if (resolved.provider !== "auto") {
      ...
      note(
        [
          ...
          suggestedRemoteProvider
            ? `- Switch to a remote provider: ${formatCliCommand(`FirstNexus config set agents.defaults.memorySearch.provider ${suggestedRemoteProvider}`)}`
    ...
    if (isKeyOptionalMemoryProvider(resolved.provider)) {
      ...
      // not warn. A skipped probe means the user ran `FirstNexus doctor` without
    ...
    note(
      [
        ...
        `- Configure credentials: ${formatCliCommand("FirstNexus configure --section model")}`,
        `- To disable: ${formatCliCommand("FirstNexus config set agents.defaults.memorySearch.enabled false")}`,
        "",
        `Verify: ${formatCliCommand("FirstNexus memory status --deep")}`,
      ].join("\n"),
      "Memory search",
