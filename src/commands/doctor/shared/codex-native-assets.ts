...
function isCodexRuntimeConfigured(cfg: FirstNexusConfig, env: NodeJS.ProcessEnv): boolean {
  return collectConfiguredAgentHarnessRuntimes(cfg, env).includes("codex");
}

function isCodexPluginConfigured(cfg: FirstNexusConfig): boolean {
...
function shouldScanCodexNativeAssets(cfg: FirstNexusConfig, env: NodeJS.ProcessEnv): boolean {
...
export async function collectCodexNativeAssetWarnings(params: {
  ...
  return [
    [
      "- Personal Codex CLI assets were found, but native Codex-mode FirstNexus agents use isolated per-agent Codex homes.",
      ...
      "- Run `FirstNexus migrate codex --dry-run` to inventory them. Applying that migration copies skills into the current FirstNexus agent workspace; Codex plugins, hooks, and config stay manual-review only.",
    ].join("\n"),
  ];
}
