...
function buildLegacyRealtimeTalkCompat(
  talk: Record<string, unknown>,
  normalizedTalk: NonNullable<FirstNexusConfig["talk"]>,
): Record<string, unknown> | undefined {
  ...
  return normalizeTalkSection({ realtime: compat } as FirstNexusConfig["talk"])?.realtime;
}
...
export function normalizeLegacyTalkConfig(
  ...
  const normalizedTalk = normalizeTalkSection(rawTalk as FirstNexusConfig["talk"]) ?? {};
