...
function listAgentRecords(cfg: FirstNexusConfig): Record<string, unknown>[] {
  return Array.isArray(cfg.agents?.list) ? cfg.agents.list.filter(hasRecord) : [];
}

function hasChannels(cfg: FirstNexusConfig): boolean {
  return hasRecord(cfg.channels);
}

function hasPlugins(cfg: FirstNexusConfig): boolean {
  return hasRecord(cfg.plugins);
}

function hasPluginLoadPaths(cfg: FirstNexusConfig): boolean {
...
function hasConfiguredSafeBins(cfg: FirstNexusConfig): boolean {
...
function collectMessageToolUnavailableTargets(cfg: FirstNexusConfig): string[] {
...
function resolveGroupVisibleReplyProvenance(cfg: FirstNexusConfig): {
...
export function collectVisibleReplyToolPolicyWarnings(cfg: FirstNexusConfig): string[] {
  ...
  if (groupPolicy.value === "message_tool") {
    ...
    if (groupPolicy.provenance === "default") {
      warnings.push(
        `- messages.groupChat.visibleReplies defaults to "message_tool", but the message tool is unavailable for ${targetSummary}; FirstNexus falls back to automatic group/channel replies to avoid silent responses. Enable the message tool or set messages.groupChat.visibleReplies explicitly.`,
      );
    } else {
      warnings.push(
        `- ${groupPolicy.path} is set to "message_tool", but the message tool is unavailable for ${targetSummary}; FirstNexus falls back to automatic visible replies, so normal replies may post to the source chat. Enable the message tool or set ${groupPolicy.path} to "automatic".`,
      );
    }
  }
  ...
  if (globalVisibleReplies === "message_tool" && groupPolicy.path !== "messages.visibleReplies") {
    warnings.push(
      `- messages.visibleReplies is set to "message_tool", but the message tool is unavailable for ${formatTargets(
        targets,
      )}; FirstNexus falls back to automatic direct-chat replies, so normal replies may post to the source chat. Enable the message tool or set messages.visibleReplies to "automatic".`,
    );
  }
  return warnings;
}
...
export function collectChannelBoundMessageToolPolicyWarnings(cfg: FirstNexusConfig): string[] {
