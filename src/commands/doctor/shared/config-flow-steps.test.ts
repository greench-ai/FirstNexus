...
describe("doctor config flow steps", () => {
  beforeEach(() => {
    ...
    migrateLegacyConfigMock.mockImplementation((config: FirstNexusConfig) => ({
      config,
      changes: [],
    }));
  ...
  it("collects legacy compatibility issue lines and preview fix hints", () => {
    ...
    expect(result.state.fixHints).toStrictEqual([
      'Run "FirstNexus doctor --fix" to migrate legacy config keys.',
    ]);
  ...
  it("removes unknown keys and adds preview hint", () => {
    ...
    const result = applyUnknownConfigKeyStep({
      state: {
        cfg: {},
        candidate: { bogus: true } as unknown as FirstNexusConfig,
        pendingChanges: false,
        fixHints: [],
      },
      shouldRepair: false,
    ...
    expect(result.state.fixHints).toStrictEqual([
      'Run "FirstNexus doctor --fix" to remove these keys.',
    ]);
  });
