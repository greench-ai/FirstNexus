...
describe("doctor config flow steps", () => {
  it("collects legacy compatibility issue lines and preview fix hints", () => {
    ...
    expect(result.state.fixHints).toContain(
      'Run "nexusclaw doctor --fix" to apply compatibility migrations.',
    );
  });
  ...
  it("removes unknown keys and adds preview hint", () => {
    const result = applyUnknownConfigKeyStep({
      state: {
        cfg: {},
        candidate: { bogus: true } as unknown as NexusClawConfig,
        pendingChanges: false,
        fixHints: [],
      },
      shouldRepair: false,
    ...
    expect(result.state.fixHints).toContain('Run "nexusclaw doctor --fix" to remove these keys.');
  });
});
