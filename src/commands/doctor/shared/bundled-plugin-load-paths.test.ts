...
import { bundledDistPluginRootAt, bundledPluginRootAt } from "FirstNexus/plugin-sdk/test-fixtures";
...
describe("bundled plugin load path repair", () => {
  beforeEach(() => {
    const packageRoot = "/app/node_modules/FirstNexus";
  ...
  it("detects legacy bundled plugin paths that still point at source extensions", () => {
    const packageRoot = path.resolve("app-node-modules", "FirstNexus");
  ...
  it("formats a doctor hint for legacy bundled plugin paths", () => {
    ...
    expect(warnings).toEqual([
      `- plugins.load.paths: bundled plugin path "${legacyPath}" still aliases feishu; FirstNexus loads the packaged bundled plugin from "${bundledPath}".`,
      '- Run "FirstNexus doctor --fix" to remove these redundant bundled plugin paths.',
    ]);
  });

  it("ignores bundled plugins that already resolve to source extensions", () => {
    const sourcePath = path.resolve("repo", "FirstNexus", "extensions", "feishu");
    vi.spyOn(bundledSources, "resolveBundledPluginSources").mockReturnValue(
