...
const mocks = vi.hoisted(() => ({
  ...
  getOfficialExternalPluginCatalogManifest: vi.fn(
    (entry: { FirstNexus?: unknown }) => entry.FirstNexus,
  ),
  ...
  resolveDefaultPluginExtensionsDir: vi.fn(() => "/tmp/FirstNexus-plugins"),
...
describe("repairMissingConfiguredPluginInstalls", () => {
  beforeEach(() => {
    ...
    mocks.installPluginFromClawHub.mockResolvedValue({
      ok: true,
      pluginId: "matrix",
      targetDir: "/tmp/FirstNexus-plugins/matrix",
      version: "1.2.3",
      clawhub: {
        source: "clawhub",
        clawhubUrl: "https://clawhub.ai",
        clawhubPackage: "@FirstNexus/plugin-matrix",
    ...
    mocks.installPluginFromNpmSpec.mockResolvedValue({
      ...
      version: "1.2.3",
      npmResolution: {
        name: "@FirstNexus/plugin-matrix",
        version: "1.2.3",
        resolvedSpec: "@FirstNexus/plugin-matrix@1.2.3",
  ...
  it("installs a missing configured FirstNexus channel plugin from npm by default", async () => {
    mocks.listChannelPluginCatalogEntries.mockReturnValue([
      {
        id: "matrix",
        pluginId: "matrix",
        meta: { label: "Matrix" },
        install: {
          npmSpec: "@FirstNexus/plugin-matrix@1.2.3",
    ...
    expectRecordFields(mockCallArg(mocks.installPluginFromNpmSpec), {
      spec: "@FirstNexus/plugin-matrix@1.2.3",
      extensionsDir: "/tmp/FirstNexus-plugins",
    ...
    expectRecordFields((records as Record<string, unknown>).matrix, {
      source: "npm",
      spec: "@FirstNexus/plugin-matrix@1.2.3",
      installPath: "/tmp/FirstNexus-plugins/matrix",
      version: "1.2.3",
    });
    ...
    expect(result.changes).toEqual([
      'Installed missing configured plugin "matrix" from @FirstNexus/plugin-matrix@1.2.3.',
    ]);
  ...
  it("uses an explicit ClawHub install spec before npm", async () => {
    mocks.listChannelPluginCatalogEntries.mockReturnValue([
      {
        ...
        meta: { label: "Matrix" },
        install: {
          clawhubSpec: "clawhub:@FirstNexus/plugin-matrix@stable",
          npmSpec: "@FirstNexus/plugin-matrix@1.2.3",
          expectedIntegrity: "sha512-test",
        },
      },
    ...
    expectRecordFields(mockCallArg(mocks.installPluginFromClawHub), {
      spec: "clawhub:@FirstNexus/plugin-matrix@stable",
      expectedPluginId: "matrix",
    });
    expect(mocks.installPluginFromNpmSpec).not.toHaveBeenCalled();
    expect(result.changes).toEqual([
      'Installed missing configured plugin "matrix" from clawhub:@FirstNexus/plugin-matrix@stable.',
    ]);
    expect(result.warnings).toStrictEqual([]);
  });
  ...
  it("falls back to npm when an FirstNexus channel plugin is not on ClawHub", async () => {
    ...
    expect(result.changes).toEqual([
      'ClawHub clawhub:@FirstNexus/plugin-matrix@stable unavailable for "matrix"; falling back to npm @FirstNexus/plugin-matrix@1.2.3.',
      'Installed missing configured plugin "matrix" from @FirstNexus/plugin-matrix@1.2.3.',
    ]);
    expect(result.warnings).toStrictEqual([]);
  });

  it("honors npm-first catalog metadata for missing FirstNexus channel plugins", async () => {
    mocks.installPluginFromNpmSpec.mockResolvedValueOnce({
      ok: true,
      pluginId: "twitch",
      targetDir: "/tmp/FirstNexus-plugins/twitch",
      version: "2026.5.2",
      npmResolution: {
        name: "@FirstNexus/twitch",
        version: "2026.5.2",
        resolvedSpec: "@FirstNexus/twitch@2026.5.2",
    ...
    expect(mocks.installPluginFromClawHub).not.toHaveBeenCalled();
    expectRecordFields(mockCallArg(mocks.installPluginFromNpmSpec), {
      spec: expectedNpmInstallSpec("@FirstNexus/twitch"),
      expectedPluginId: "twitch",
      trustedSourceLinkedOfficialInstall: true,
    });
    expect(result.changes).toEqual([
      `Installed missing configured plugin "twitch" from ${expectedNpmInstallSpec("@FirstNexus/twitch")}.`,
    ]);
  });
  ...
  it("installs missing configured non-channel plugins from the official external catalog", async () => {
    mocks.installPluginFromNpmSpec.mockResolvedValueOnce({
      ...
      targetDir: "/tmp/FirstNexus-plugins/diagnostics-otel",
      version: "2026.5.2",
      npmResolution: {
        name: "@FirstNexus/diagnostics-otel",
        version: "2026.5.2",
        resolvedSpec: "@FirstNexus/diagnostics-otel@2026.5.2",
    ...
    expect(mocks.installPluginFromClawHub).not.toHaveBeenCalled();
    expectRecordFields(mockCallArg(mocks.installPluginFromNpmSpec), {
      spec: expectedNpmInstallSpec("@FirstNexus/diagnostics-otel"),
      expectedPluginId: "diagnostics-otel",
    });
    expect(result.changes).toEqual([
      `Installed missing configured plugin "diagnostics-otel" from ${expectedNpmInstallSpec("@FirstNexus/diagnostics-otel")}.`,
    ]);
  });
  ...
  it("does not let runtime fallback metadata override official catalog install specs", async () => {
    mocks.installPluginFromNpmSpec.mockResolvedValueOnce({
      ok: true,
      pluginId: "acpx",
      targetDir: "/tmp/FirstNexus-plugins/acpx",
    ...
    expectRecordFields(mockCallArg(mocks.installPluginFromNpmSpec), {
      spec: expectedNpmInstallSpec("@FirstNexus/acpx"),
      expectedPluginId: "acpx",
      trustedSourceLinkedOfficialInstall: true,
    });
    expect(result.changes).toEqual([
      `Installed missing configured plugin "acpx" from ${expectedNpmInstallSpec("@FirstNexus/acpx")}.`,
    ]);
  });
  ...
  it("uses current bundled discovery to remove records before stale snapshots can reinstall official plugins", async () => {
    const records = {
      "google-meet": {
        ...
        resolvedName: "@FirstNexus/google-meet",
      ...
      plugins: [
        {
          id: "google-meet",
          origin: "npm",
          packageName: "@FirstNexus/google-meet",
        },
      ],
      diagnostics: [],
    });
    ...
    mocks.listOfficialExternalPluginCatalogEntries.mockReturnValue([
      {
        id: "google-meet",
        label: "Google Meet",
        install: { npmSpec: "@FirstNexus/google-meet" },
        FirstNexus: {
          id: "google-meet",
          install: { npmSpec: "@FirstNexus/google-meet" },
        },
  ...
  it.each([
    [
      ...
      {
        source: "npm",
        spec: "@FirstNexus/matrix-fork",
        resolvedName: "@FirstNexus/matrix-fork",
        resolvedSpec: "@FirstNexus/matrix-fork@1.2.3",
    ...
    [
      ...
      {
        source: "clawhub",
        spec: "clawhub:@FirstNexus/matrix-fork@stable",
        clawhubPackage: "@FirstNexus/matrix-fork",
        installPath: "/missing/matrix-fork",
      },
    ],
  ])(
  ...
  it("defers missing external payload repair during the package update doctor pass", async () => {
    ...
    expect(result).toMatchObject({
      changes: [
        'Skipped package-manager repair for configured plugin "discord" during package update; rerun "FirstNexus doctor --fix" after the update completes.',
      ],
      warnings: [],
    });
  });
    ...
    mocks.listChannelPluginCatalogEntries.mockReturnValue([
      {
        ...
        pluginId: "discord",
        meta: { label: "Discord" },
        install: { npmSpec: "@FirstNexus/discord" },
      },
    ]);
  ...
  it("installs a missing third-party downloadable plugin from npm only", async () => {
    mocks.installPluginFromNpmSpec.mockResolvedValueOnce({
      ok: true,
      pluginId: "wecom",
      targetDir: "/tmp/FirstNexus-plugins/wecom",
      version: "2026.4.23",
      npmResolution: {
        name: "@wecom/wecom-FirstNexus-plugin",
        version: "2026.4.23",
        resolvedSpec: "@wecom/wecom-FirstNexus-plugin@2026.4.23",
    ...
    expectRecordFields(installArg, {
      spec: "@wecom/wecom-FirstNexus-plugin@2026.4.23",
      expectedPluginId: "wecom",
    });
    expect(installArg).not.toHaveProperty("trustedSourceLinkedOfficialInstall", true);
    expect(result.changes).toEqual([
      'Installed missing configured plugin "wecom" from @wecom/wecom-FirstNexus-plugin@2026.4.23.',
    ]);
  });
  ...
  it("installs a missing default Codex runtime plugin from the official external catalog", async () => {
    mocks.installPluginFromNpmSpec.mockResolvedValueOnce({
      ok: true,
      pluginId: "codex",
      targetDir: "/tmp/FirstNexus-plugins/codex",
      version: "2026.5.2",
      npmResolution: {
    ...
    expectRecordFields(mockCallArg(mocks.installPluginFromNpmSpec), {
      spec: expectedNpmInstallSpec("@FirstNexus/codex"),
    ...
    expectRecordFields((records as Record<string, unknown>).codex, {
      ...
      installPath: "/tmp/FirstNexus-plugins/codex",
      version: "2026.5.2",
    });
    expect(mockCallArg(mocks.writePersistedInstalledPluginIndexInstallRecords, 0, 1)).toEqual({
      env: {},
    ...
    expect(result.changes).toEqual([
      `Installed missing configured plugin "codex" from ${expectedNpmInstallSpec("@FirstNexus/codex")}.`,
    ]);
    expect(result.warnings).toStrictEqual([]);
  });
  ...
  it("still installs a channel catalog plugin when the configured owner is blocked by the allowlist", async () => {
    ...
    mocks.installPluginFromNpmSpec.mockResolvedValueOnce({
      ok: true,
      pluginId: "feishu",
      targetDir: "/tmp/FirstNexus-plugins/feishu",
      version: "2026.5.2",
      npmResolution: {
        name: "@FirstNexus/feishu",
        version: "2026.5.2",
        resolvedSpec: "@FirstNexus/feishu@2026.5.2",
      },
    });
    ...
    expectRecordFields(mockCallArg(mocks.installPluginFromNpmSpec), {
      spec: expectedNpmInstallSpec("@FirstNexus/feishu"),
      expectedPluginId: "feishu",
      trustedSourceLinkedOfficialInstall: true,
    });
    expect(result.changes).toEqual([
      `Installed missing configured plugin "feishu" from ${expectedNpmInstallSpec("@FirstNexus/feishu")}.`,
    ]);
  });
  ...
  it("reinstalls a missing configured plugin from its persisted install record", async () => {
    const records = {
      demo: {
        source: "npm",
        spec: "@FirstNexus/plugin-demo@1.0.0",
        ...
        plugins: {
          installs: {
            demo: {
              source: "npm",
              spec: "@FirstNexus/plugin-demo@1.0.0",
              installPath: "/tmp/FirstNexus-plugins/demo",
            },
          },
        },
      },
  ...
  it("repairs a broken managed package entry from its attributed registry diagnostic", async () => {
    const records = {
      demo: {
        source: "npm",
        spec: "@FirstNexus/plugin-demo@1.0.0",
        resolvedName: "@FirstNexus/plugin-demo",
        resolvedSpec: "@FirstNexus/plugin-demo@1.0.0",
  ...
  it("reinstalls a known configured plugin from the catalog when its recorded install path is missing", async () => {
    const records = {
      discord: {
        source: "npm",
        spec: "@FirstNexus/discord",
        installPath: "/tmp/FirstNexus-missing-discord-install-record",
      },
    };
    mocks.loadInstalledPluginIndexInstallRecords.mockResolvedValue(records);
    mocks.loadPluginMetadataSnapshot.mockReturnValue({
    ...
    mocks.installPluginFromNpmSpec.mockResolvedValueOnce({
      ok: true,
      pluginId: "discord",
      targetDir: "/tmp/FirstNexus-plugins/discord",
      version: "1.2.3",
      npmResolution: {
        name: "@FirstNexus/discord",
        version: "1.2.3",
        resolvedSpec: "@FirstNexus/discord@1.2.3",
    ...
    expectRecordFields(updateConfig.plugins, { installs: records });
    expectRecordFields(mockCallArg(mocks.installPluginFromNpmSpec), {
      spec: expectedNpmInstallSpec("@FirstNexus/discord"),
      expectedPluginId: "discord",
      trustedSourceLinkedOfficialInstall: true,
    });
    const persistedRecords = mockCallArg(mocks.writePersistedInstalledPluginIndexInstallRecords);
    expectRecordFields((persistedRecords as Record<string, unknown>).discord, {
      installPath: "/tmp/FirstNexus-plugins/discord",
    });
    expect(mockCallArg(mocks.writePersistedInstalledPluginIndexInstallRecords, 0, 1)).toEqual({
      env: {},
    });
    expect(result.changes).toEqual([
      `Installed missing configured plugin "discord" from ${expectedNpmInstallSpec("@FirstNexus/discord")}.`,
    ]);
  });
      ...
      diagnostics: [
        {
          ...
          pluginId: "discord",
          message:
            "channel plugin manifest declares discord without channelConfigs metadata; add FirstNexus.plugin.json#channelConfigs so config schema and setup surfaces work before runtime loads",
        },
      ],
    });
    mocks.updateNpmInstalledPlugins.mockResolvedValue({
  ...
  it("reinstalls a recorded external web search plugin from provider-only config", async () => {
    const records = {
      brave: {
        source: "npm",
        spec: "@FirstNexus/brave-plugin@beta",
    ...
    mocks.listOfficialExternalPluginCatalogEntries.mockReturnValue([
      {
        ...
        label: "Brave",
        install: {
          npmSpec: "@FirstNexus/brave-plugin",
          defaultChoice: "npm",
        },
        FirstNexus: {
  ...
  it("installs a configured external web search plugin from provider-only config", async () => {
    ...
    mocks.resolveOfficialExternalPluginId.mockImplementation(
      (entry: { id?: string; FirstNexus?: { plugin?: { id?: string } } }) =>
        entry.FirstNexus?.plugin?.id ?? entry.id,
    );
    mocks.resolveOfficialExternalPluginInstall.mockImplementation(
      (entry: { install?: unknown; FirstNexus?: { install?: unknown } }) =>
        entry.FirstNexus?.install ?? entry.install ?? null,
    );
    mocks.resolveOfficialExternalPluginLabel.mockImplementation(
      (entry: { label?: string; FirstNexus?: { plugin?: { label?: string } } }) =>
        entry.FirstNexus?.plugin?.label ?? entry.label ?? "plugin",
    );
    mocks.installPluginFromNpmSpec.mockResolvedValueOnce({
      ok: true,
      pluginId: "brave",
      targetDir: "/tmp/FirstNexus-plugins/brave",
      version: "2026.5.2",
      npmResolution: {
        name: "@FirstNexus/brave-plugin",
        version: "2026.5.2",
        resolvedSpec: "@FirstNexus/brave-plugin@2026.5.2",
      },
    });
    ...
    expectRecordFields(mockCallArg(mocks.installPluginFromNpmSpec), {
      spec: expectedNpmInstallSpec("@FirstNexus/brave-plugin"),
      expectedPluginId: "brave",
      trustedSourceLinkedOfficialInstall: true,
    });
    expect(result.changes).toEqual([
      `Installed missing configured plugin "brave" from ${expectedNpmInstallSpec("@FirstNexus/brave-plugin")}.`,
    ]);
  });
  ...
  it("installs configured external web search plugins from beta on the beta channel", async () => {
    ...
    mocks.installPluginFromNpmSpec.mockResolvedValueOnce({
      ...
      npmResolution: {
        name: "@FirstNexus/brave-plugin",
        version: "2026.5.4-beta.1",
        resolvedSpec: "@FirstNexus/brave-plugin@2026.5.4-beta.1",
      },
    });
    ...
    expectRecordFields(persistedRecords.brave, { spec: "@FirstNexus/brave-plugin" });
    expect(mockCallArg(mocks.writePersistedInstalledPluginIndexInstallRecords, 0, 1)).toEqual({
      env: {},
    });
    expect(result.changes).toEqual([
      'Installed missing configured plugin "brave" from @FirstNexus/brave-plugin@beta.',
    ]);
  });
