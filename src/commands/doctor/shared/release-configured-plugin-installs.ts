...
import { compareFirstNexusVersions } from "../../../config/version.js";
...
function isPluginsGloballyDisabled(cfg: FirstNexusConfig): boolean {
  return cfg.plugins?.enabled === false;
}

function isDenied(cfg: FirstNexusConfig, pluginId: string): boolean {
  const deny = cfg.plugins?.deny;
  return Array.isArray(deny) && deny.includes(pluginId);
}

function collectBlockedPluginIds(cfg: FirstNexusConfig): string[] {
...
function isPluginEntryDisabled(cfg: FirstNexusConfig, pluginId: string): boolean {
  return cfg.plugins?.entries?.[pluginId]?.enabled === false;
}

function isChannelDisabled(cfg: FirstNexusConfig, channelId: string): boolean {
  const channels = asObjectRecord(cfg.channels);
  const entry = asObjectRecord(channels?.[channelId]);
  return entry?.enabled === false;
}

function isDisabled(cfg: FirstNexusConfig, pluginId: string): boolean {
...
function collectMaterialPluginEntryIds(cfg: FirstNexusConfig): string[] {
  const entries = asObjectRecord(cfg.plugins?.entries);
  if (!entries) {
    return [];
  }
...
function collectSlotPluginIds(cfg: FirstNexusConfig): string[] {
...
function collectConfiguredProviderIds(cfg: FirstNexusConfig): Set<string> {
  const ids = new Set<string>();
...
function collectProviderPluginIds(cfg: FirstNexusConfig, env: NodeJS.ProcessEnv): string[] {
...
function collectWebSearchPluginIds(cfg: FirstNexusConfig): string[] {
...
function collectAcpRuntimePluginIds(cfg: FirstNexusConfig): string[] {
...
function collectAllowOnlyOfficialPluginIds(cfg: FirstNexusConfig): string[] {
...
export function shouldRunConfiguredPluginInstallReleaseStep(params: {
  ...
  const currentComparedToRelease = compareFirstNexusVersions(
  ...
  const touchedComparedToRelease = compareFirstNexusVersions(params.touchedVersion, releaseVersion);
