...
export function formatMatrixLegacyStatePreview(
  ...
  return [
    ...
    '- Run "nexusclaw doctor --fix" to migrate this Matrix state now.',
  ].join("\n");
}
  ...
  for (const plan of detection.plans) {
    notes.push(
      [
        ...
        '- Run "nexusclaw doctor --fix" to extract any saved backup key now. Backed-up room keys will restore automatically on next gateway start.',
      ].join("\n"),
    );
  }
  return notes;
...
export async function collectMatrixInstallPathWarnings(cfg: NexusClawConfig): Promise<string[]> {
...
export async function applyMatrixDoctorRepair(params: {
  ...
  if (actionableMatrixMigration) {
    try {
      ...
      warnings.push(
        '- Skipping Matrix migration changes for now. Resolve the snapshot failure, then rerun "nexusclaw doctor --fix".',
      );
    }
