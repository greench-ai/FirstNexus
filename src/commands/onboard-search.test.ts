...
function createPerplexityConfig(apiKey: string, enabled?: boolean): NexusClawConfig {
  return {
    tools: {
      web: {
        search: {
...
function pluginWebSearchApiKey(config: NexusClawConfig, pluginId: string): unknown {
  const entry = (
...
function createDisabledFirecrawlConfig(apiKey?: string): NexusClawConfig {
  return {
    tools: {
      web: {
        search: {
...
function readFirecrawlPluginApiKey(config: NexusClawConfig): string | undefined {
...
async function runQuickstartPerplexitySetup(
  apiKey: string,
  enabled?: boolean,
): Promise<{ result: NexusClawConfig; prompter: WizardPrompter }> {
  const cfg = createPerplexityConfig(apiKey, enabled);
