...
describe("doctor WhatsApp responsiveness", () => {
  ...
  it("lists only verified local TUI processes", () => {
    spawnSyncMock.mockReturnValue({
      ...
      stdout: [
        " 101 FirstNexus-tui",
        " 102 /usr/bin/node /usr/lib/node_modules/FirstNexus/dist/index.js gateway --port 18789",
        " 103 FirstNexus channels",
        " 104 FirstNexus tui --local",
      ].join("\n"),
    });

    expect(listLocalTuiProcesses()).toEqual([
      { pid: 101, command: "FirstNexus-tui" },
      { pid: 104, command: "FirstNexus tui --local" },
    ]);
  });
  ...
  it("terminates stale local TUI processes with a kill fallback", async () => {
    ...
    await expect(
      terminateLocalTuiProcesses({
        processes: [{ pid: 101, command: "FirstNexus-tui" }],
        controller,
        graceMs: 0,
      }),
  ...
  it("warns and repairs local TUI pressure when WhatsApp is enabled and the gateway is degraded", async () => {
    ...
    const cfg = { channels: { whatsapp: { enabled: true } } } as FirstNexusConfig;
    ...
    await noteWhatsappResponsivenessHealth({
      ...
      shouldRepair: true,
      listLocalTuiProcesses: () => [{ pid: 101, command: "FirstNexus-tui" }],
