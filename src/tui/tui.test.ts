...
describe("resolveInitialTuiAgentId", () => {
  const cfg: NexusClawConfig = {
    agents: {
      list: [
        { id: "main", workspace: "/tmp/nexusclaw" },
        { id: "ops", workspace: "/tmp/nexusclaw/projects/ops" },
      ],
    },
  };
  ...
  it("infers agent from cwd when session is not agent-prefixed", () => {
    expect(
      resolveInitialTuiAgentId({
        ...
        initialSessionInput: "",
        cwd: "/tmp/nexusclaw/projects/ops/src",
      }),
    ).toBe("ops");
  });
...
describe("resolveGatewayDisconnectState", () => {
  it("returns pairing recovery guidance when disconnect reason requires pairing", () => {
    ...
    expect(state.activityStatus).toBe("pairing required: run nexusclaw devices list");
    expect(state.pairingHint).toContain("nexusclaw devices list");
  });
