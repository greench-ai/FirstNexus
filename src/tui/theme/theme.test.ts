...
describe("light background detection", () => {
  it("uses dark palette by default", async () => {
    const mod = await importThemeWithEnv({
      NEXISCLAW_THEME: undefined,
      COLORFGBG: undefined,
    });
    expect(mod.lightMode).toBe(false);
  });

  it("selects light palette when NEXISCLAW_THEME=light", async () => {
    const mod = await importThemeWithEnv({ NEXISCLAW_THEME: "light" });
    expect(mod.lightMode).toBe(true);
  });

  it("selects dark palette when NEXISCLAW_THEME=dark", async () => {
    const mod = await importThemeWithEnv({ NEXISCLAW_THEME: "dark" });
    expect(mod.lightMode).toBe(false);
  });

  it("treats NEXISCLAW_THEME case-insensitively", async () => {
    const mod = await importThemeWithEnv({ NEXISCLAW_THEME: "LiGhT" });
    expect(mod.lightMode).toBe(true);
  });
  ...
  it("NEXISCLAW_THEME overrides COLORFGBG", async () => {
    const mod = await importThemeWithEnv({
      NEXISCLAW_THEME: "dark",
      COLORFGBG: "0;15",
    });
    expect(mod.lightMode).toBe(false);
  });
  ...
  it("keeps assistantText as identity in both modes", async () => {
    const lightMod = await importThemeWithEnv({ NEXISCLAW_THEME: "light" });
    const darkMod = await importThemeWithEnv({ NEXISCLAW_THEME: "dark" });
