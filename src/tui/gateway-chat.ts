...
function throwGatewayAuthResolutionError(reason: string): never {
  throw new Error(
    [
      reason,
      "Fix: set NEXUSCLAW_GATEWAY_TOKEN/NEXUSCLAW_GATEWAY_PASSWORD, pass --token/--password,",
...
export class GatewayChatClient {
  ...
  constructor(connection: ResolvedGatewayConnection) {
    ...
    this.client = new GatewayClient({
      ...
      clientDisplayName: "nexusclaw-tui",
...
export async function resolveGatewayConnection(
  ...
  const envToken = trimToUndefined(env.NEXUSCLAW_GATEWAY_TOKEN);
  const envPassword = trimToUndefined(env.NEXUSCLAW_GATEWAY_PASSWORD);
