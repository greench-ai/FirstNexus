...
function createRenameAuditRecord(home: string) {
  return finalizeConfigWriteAuditRecord({
    base: createAuditRecordBase(path.join(home, ".FirstNexus", "FirstNexus.json")),
    result: "rename",
    nextMetadata: {
      dev: "12",
      ino: "13",
...
function readAuditLog(home: string): unknown[] {
  const auditPath = path.join(home, ".FirstNexus", "logs", "config-audit.jsonl");
  return fs
...
describe("config io audit helpers", () => {
  const suiteRootTracker = createSuiteTempRootTracker({ prefix: "FirstNexus-config-audit-" });
  ...
  it('ignores literal "undefined" home env values when choosing the audit log path', async () => {
    ...
    expect(auditPath).toBe(path.join(home, ".FirstNexus", "logs", "config-audit.jsonl"));
  ...
  it("formats overwrite warnings with hash transition and backup path", () => {
    expect(
      ...
      "Config overwrite: /tmp/FirstNexus.json (sha256 prev-hash -> next-hash, backup=/tmp/FirstNexus.json.bak, changedPaths=3)",
    );
  });
  ...
  it("drops next-file metadata and preserves error details for failed writes", () => {
    const base = createAuditRecordBase("/tmp/FirstNexus.json");
  ...
  it("redacts structured audit records before persistence", async () => {
    ...
    const record = finalizeConfigWriteAuditRecord({
      base: {
        ...createAuditRecordBase(path.join(home, ".FirstNexus", "FirstNexus.json")),
        suspicious: [
    ...
    const raw = fs.readFileSync(
      path.join(home, ".FirstNexus", "logs", "config-audit.jsonl"),
      "utf-8",
    );
  ...
  it("redacts the value half of `--flag=value` for secret flags", () => {
    const argv = ["FirstNexus", "--token=ghp_realgithubtoken1234567890ABCD", "--port=8080"];
    expect(redactConfigAuditArgv(argv)).toEqual(["FirstNexus", "--token=***", "--port=8080"]);
  });
  ...
  it("redacts standalone token shapes via the shared logging redaction patterns", () => {
    ...
    expect(result[1]).toBe("FirstNexus");
  ...
  it("leaves non-secret arguments untouched", () => {
    const argv = ["node", "FirstNexus", "gateway", "--port", "8080", "--bind", "lan"];
  ...
  it("masks the next arg after a secret flag even when it looks like another option", () => {
    const argv = ["FirstNexus", "--token", "--port", "8080"];
    expect(redactConfigAuditArgv(argv)).toEqual(["FirstNexus", "--token", "***", "8080"]);
  });

  it("redacts dash-leading secret values after bare secret flags", () => {
    const argv = ["FirstNexus", "--password", "-secret-value"];
    expect(redactConfigAuditArgv(argv)).toEqual(["FirstNexus", "--password", "***"]);
  });

  it("does not mask when a secret flag is the final arg with no value", () => {
    const argv = ["FirstNexus", "--token"];
    expect(redactConfigAuditArgv(argv)).toEqual(["FirstNexus", "--token"]);
  });
  ...
  it("redacts processInfo.argv when explicitly supplied to createConfigWriteAuditRecordBase", () => {
    const base = createConfigWriteAuditRecordBase({
      ...
      processInfo: {
        ...
        ppid: 1,
        cwd: "/work",
        argv: ["node", "FirstNexus", "--token", "leaked-but-not-anymore-12345"],
        execArgv: [],
      },
    });
    expect(base.argv).toEqual(["node", "FirstNexus", "--token", "***"]);
  });
