...
function isProviderConfigured(cfg: FirstNexusConfig, providerId: string): boolean {
...
function hasPluginOwnedWebSearchConfig(cfg: FirstNexusConfig, pluginId: string): boolean {
  const pluginConfig = cfg.plugins?.entries?.[pluginId]?.config;
  return isRecord(pluginConfig) && isRecord(pluginConfig.webSearch);
}

function hasPluginOwnedWebFetchConfig(cfg: FirstNexusConfig, pluginId: string): boolean {
  const pluginConfig = cfg.plugins?.entries?.[pluginId]?.config;
...
function hasPluginOwnedToolConfig(cfg: FirstNexusConfig, plugin: PluginManifestRecord): boolean {
...
function collectConfiguredChannelIds(cfg: FirstNexusConfig, env: NodeJS.ProcessEnv): string[] {
...
function hasConfiguredWebSearchPluginEntry(cfg: FirstNexusConfig): boolean {
...
function hasConfiguredWebSearchProviderSelection(cfg: FirstNexusConfig): boolean {
...
function hasConfiguredWebFetchPluginEntry(cfg: FirstNexusConfig): boolean {
  const entries = cfg.plugins?.entries;
  return (
    !!entries &&
    typeof entries === "object" &&
...
function hasConfiguredPluginConfigEntry(cfg: FirstNexusConfig): boolean {
  const entries = cfg.plugins?.entries;
  return (
    !!entries &&
    typeof entries === "object" &&
...
function hasBrowserToolReference(cfg: FirstNexusConfig): boolean {
...
function collectConfiguredPluginEntryIds(cfg: FirstNexusConfig): string[] {
  const entries = cfg.plugins?.entries;
...
function hasOwnPluginEntry(cfg: FirstNexusConfig, pluginId: string): boolean {
  const entries = cfg.plugins?.entries;
  return !!entries && typeof entries === "object" && Object.hasOwn(entries, pluginId);
}

function isPluginEntryExplicitlyDisabled(cfg: FirstNexusConfig, pluginId: string): boolean {
  return cfg.plugins?.entries?.[pluginId]?.enabled === false;
}

function hasNonDisabledPluginEntry(cfg: FirstNexusConfig, pluginId: string): boolean {
...
function hasBrowserSetupAutoEnableRelevantConfig(cfg: FirstNexusConfig): boolean {
...
function hasAcpxSetupAutoEnableRelevantConfig(cfg: FirstNexusConfig): boolean {
...
function hasXaiSetupAutoEnableRelevantConfig(cfg: FirstNexusConfig): boolean {
...
function resolveRelevantSetupAutoEnablePluginIds(cfg: FirstNexusConfig): string[] {
...
function hasSetupAutoEnableRelevantConfig(cfg: FirstNexusConfig): boolean {
  return (
...
function hasPluginEntries(cfg: FirstNexusConfig): boolean {
  const entries = cfg.plugins?.entries;
  return !!entries && typeof entries === "object" && Object.keys(entries).length > 0;
}

function hasPluginAllowlistWithMaterialEntries(cfg: FirstNexusConfig): boolean {
  if (
...
function arePluginsGloballyDisabled(cfg: FirstNexusConfig): boolean {
...
function isPluginExplicitlyDisabled(cfg: FirstNexusConfig, pluginId: string): boolean {
...
function isPluginDenied(cfg: FirstNexusConfig, pluginId: string): boolean {
  const deny = cfg.plugins?.deny;
  return Array.isArray(deny) && deny.includes(pluginId);
}

function isPluginExplicitlySelected(cfg: FirstNexusConfig, pluginId: string): boolean {
...
function disableImplicitPreferredOverPlugin(params: {
  config: FirstNexusConfig;
  originalConfig: FirstNexusConfig;
  pluginId: string;
...
function isBuiltInChannelAlreadyEnabled(cfg: FirstNexusConfig, channelId: string): boolean {
  const channels = cfg.channels as Record<string, unknown> | undefined;
