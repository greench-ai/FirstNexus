...
describe("readBestEffortConfig", () => {
  it("does not restore suspicious direct edits from .bak during ordinary reads", async () => {
    await withTempHome(async (home) => {
      ...
      const entries = await fs.readdir(`${home}/.FirstNexus`);
      expect(entries.some((entry) => entry.startsWith("FirstNexus.json.clobbered."))).toBe(false);
    });
  });
