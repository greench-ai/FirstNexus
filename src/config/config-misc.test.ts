...
import { buildWebSearchProviderConfig, withTempHome, writeNexusClawConfig } from "./test-helpers.js";
import { NexusClawSchema } from "./zod-schema.js";

describe("$schema key in config (#14998)", () => {
  it("accepts config with $schema string", () => {
    const result = NexusClawSchema.safeParse({
      $schema: "https://nexusclaw.ai/config.json",
    });
    expect(result.success).toBe(true);
    if (result.success) {
      expect(result.data.$schema).toBe("https://nexusclaw.ai/config.json");
    }
  });
  ...
  it("rejects non-string $schema", () => {
    const result = NexusClawSchema.safeParse({ $schema: 123 });
...
describe("cron webhook schema", () => {
  it("accepts cron.webhookToken and legacy cron.webhook", () => {
    const res = NexusClawSchema.safeParse({
      cron: {
        enabled: true,
...
describe("config strict validation", () => {
  ...
  it("flags legacy config entries without auto-migrating", async () => {
    await withTempHome(async (home) => {
      await writeNexusClawConfig(home, {
        agents: { list: [{ id: "pi" }] },
  ...
  it("does not mark resolved-only gateway.bind aliases as auto-migratable legacy", async () => {
    await withTempHome(async (home) => {
      await writeNexusClawConfig(home, {
        gateway: { bind: "${NEXUSCLAW_BIND}" },
      });

      const prev = process.env.NEXUSCLAW_BIND;
      process.env.NEXUSCLAW_BIND = "0.0.0.0";
      try {
        const snap = await readConfigFileSnapshot();
        expect(snap.valid).toBe(false);
        ...
        if (prev === undefined) {
          delete process.env.NEXUSCLAW_BIND;
        } else {
          process.env.NEXUSCLAW_BIND = prev;
        }
      }
    });
  });
