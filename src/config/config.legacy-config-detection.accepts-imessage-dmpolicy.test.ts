...
function expectFirstNexusSchemaInvalidPreservesField(params: {
  config: unknown;
  ...
  const res = FirstNexusSchema.safeParse(params.config);
...
describe("legacy config detection", () => {
  ...
  it("rejects removed legacy provider sections", () => {
    expectFirstNexusSchemaInvalidPreservesField({
  ...
  it("preserves claude-cli auth profile mode during validation", () => {
    ...
    const res = FirstNexusSchema.safeParse(config);
