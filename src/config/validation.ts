...
import type { FirstNexusConfig, ConfigValidationIssue } from "./types.js";
...
const SECRETREF_POLICY_DOC_URL =
  "https://docs.FirstNexus.ai/reference/secretref-credential-surface";
...
function formatMissingOfficialExternalPluginWarning(pluginId: string): string | null {
  ...
  return `plugin not installed: ${pluginId} — install the official external plugin with: FirstNexus plugins install ${installSpec}`;
}
...
function collectRawBundledChannelConfigIssues(config: FirstNexusConfig): ConfigValidationIssue[] {
...
function validateIdentityAvatar(config: FirstNexusConfig): ConfigValidationIssue[] {
...
function validateGatewayTailscaleBind(config: FirstNexusConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
...
): { ok: true; config: FirstNexusConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  const normalizedRaw = stripDeprecatedValidationKeys(raw);
  const policyIssues = collectUnsupportedSecretRefPolicyIssues(normalizedRaw);
  const validated = FirstNexusSchema.safeParse(normalizedRaw);
  ...
  const validatedConfig = validated.data as FirstNexusConfig;
  const channelIssues =
...
function validateConfigObjectWithPluginsBase(
  ...
  let compatConfig: FirstNexusConfig | null | undefined;
  ...
  const ensureCompatConfig = (): FirstNexusConfig => {
  ...
  const validateWebSearchProvider = () => {
    ...
    if (installCatalogEntry) {
      issues.push({
        path,
        message: `web_search provider is not available: ${trimmed} (install or enable plugin "${installCatalogEntry.pluginId}", then run FirstNexus doctor --fix)`,
    ...
    if (hasStalePluginEvidenceForUnknownWebSearchProvider(trimmed)) {
      warnings.push({
        ...issue,
        message: `${issue.message} (stale web search plugin config ignored; run FirstNexus doctor --fix to remove stale config, or install the plugin)`,
      });
      return;
    }
    issues.push(issue);
    ...
    for (const key of Object.keys(config.channels)) {
      ...
      if (!allowedChannels.has(trimmed)) {
        ...
        if (hasStalePluginEvidenceForUnknownChannel(trimmed)) {
          warnings.push({
            ...issue,
            message: `${issue.message} (stale channel plugin config ignored; run FirstNexus doctor --fix to remove stale config, or install the plugin)`,
          });
        } else {
          issues.push(issue);
        }
  ...
  for (const record of registry.plugins) {
    ...
    if (shouldValidate) {
      if (record.configSchema) {
        ...
        // Compatible bundles currently expose no native FirstNexus config schema.
