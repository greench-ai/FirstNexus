...
import type { NexusClawConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: NexusClawConfig): ConfigValidationIssue[] {
...
function validateGatewayTailscaleBind(config: NexusClawConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
  raw: unknown,
): { ok: true; config: NexusClawConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = NexusClawSchema.safeParse(normalizedRaw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as NexusClawConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as NexusClawConfig);
  if (avatarIssues.length > 0) {
    return { ok: false, issues: avatarIssues };
  }
  const gatewayTailscaleBindIssues = validateGatewayTailscaleBind(validated.data as NexusClawConfig);
  if (gatewayTailscaleBindIssues.length > 0) {
    return { ok: false, issues: gatewayTailscaleBindIssues };
  }
  return {
    ok: true,
    config: validated.data as NexusClawConfig,
  };
}
...
function validateConfigObjectWithPluginsBase(
  ...
  const warnings: ConfigValidationIssue[] = listLegacyWebSearchConfigPaths(raw).map((path) => ({
    ...
    message:
      ...
      "Move it under plugins.entries.<plugin>.config.webSearch.*; NexusClaw mapped it automatically for compatibility.",
  }));
  ...
  let compatConfig: NexusClawConfig | null | undefined;

  const ensureCompatConfig = (): NexusClawConfig => {
  ...
  for (const record of registry.plugins) {
    ...
    if (shouldValidate) {
      if (record.configSchema) {
        ...
        // Compatible bundles currently expose no native NexusClaw config schema.
