...
async function writePluginFixture(params: {
  ...
  await fs.writeFile(
    path.join(params.dir, "nexusclaw.plugin.json"),
...
describe("config plugin validation", () => {
  ...
  const suiteEnv = () =>
    ({
      ...
      NEXUSCLAW_STATE_DIR: path.join(suiteHome, ".nexusclaw"),
      CLAWDBOT_STATE_DIR: undefined,
      NEXUSCLAW_PLUGIN_MANIFEST_CACHE_MS: "10000",
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "nexusclaw-config-plugin-validation-"));
  ...
  it("does not fail validation for the implicit default memory slot when plugins config is explicit", async () => {
    const res = validateConfigObjectWithPlugins(
      ...
      },
      {
        env: {
          ...suiteEnv(),
          NEXUSCLAW_BUNDLED_PLUGINS_DIR: path.join(suiteHome, "missing-bundled-plugins"),
        },
      },
    );
