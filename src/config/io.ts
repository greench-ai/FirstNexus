...
import type { NexusClawConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
  validateConfigObjectRawWithPlugins,
  validateConfigObjectWithPlugins,
} from "./validation.js";
import { compareNexusClawVersions } from "./version.js";
...
function formatConfigValidationFailure(pathLabel: string, issueMessage: string): string {
  ...
  return [
    ...
    "",
    "Fix with:",
    `  nexusclaw config set ${allowPath} '["*"]'`,
    "",
    "Or switch policy:",
    `  nexusclaw config set ${policyPath} "pairing"`,
  ].join("\n");
}
...
function unsetPathForWrite(
  root: NexusClawConfig,
  pathSegments: string[],
): { changed: boolean; next: NexusClawConfig } {
...
function coerceConfig(value: unknown): NexusClawConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as NexusClawConfig;
}
...
function stampConfigVersion(cfg: NexusClawConfig): NexusClawConfig {
...
function warnIfConfigFromFuture(cfg: NexusClawConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) {
    return;
  }
  const cmp = compareNexusClawVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer NexusClaw (${touched}); current version is ${VERSION}.`,
    );
  }
}
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as NexusClawConfig, env);
  }
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): NexusClawConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as NexusClawConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: NexusClawConfig, options: ConfigWriteOptions = {}) {
    clearConfigCache();
    ...
    const outputConfigBase =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) as NexusClawConfig)
        : cfgToWrite;
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.NEXUSCLAW_TEST_CONFIG_OVERWRITE_LOG === "1";
    ...
    const logConfigWriteAnomalies = () => {
      ...
      const isVitest = deps.env.VITEST === "true";
      const shouldLogInVitest = deps.env.NEXUSCLAW_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
    ...
    const auditRecordBase = {
      ...
      watchMode: deps.env.NEXUSCLAW_WATCH_MODE === "1",
      watchSession:
        typeof deps.env.NEXUSCLAW_WATCH_SESSION === "string" &&
        deps.env.NEXUSCLAW_WATCH_SESSION.trim().length > 0
          ? deps.env.NEXUSCLAW_WATCH_SESSION.trim()
          : null,
      watchCommand:
        typeof deps.env.NEXUSCLAW_WATCH_COMMAND === "string" &&
        deps.env.NEXUSCLAW_WATCH_COMMAND.trim().length > 0
          ? deps.env.NEXUSCLAW_WATCH_COMMAND.trim()
          : null,
      existsBefore: snapshot.exists,
...
// module scope. `NEXUSCLAW_CONFIG_PATH` (and friends) are expected to work even
...
let runtimeConfigSnapshot: NexusClawConfig | null = null;
let runtimeConfigSourceSnapshot: NexusClawConfig | null = null;
let runtimeConfigSnapshotRefreshHandler: RuntimeConfigSnapshotRefreshHandler | null = null;

function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.NEXUSCLAW_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.NEXUSCLAW_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function getRuntimeConfigSnapshot(): NexusClawConfig | null {
  return runtimeConfigSnapshot;
}

export function getRuntimeConfigSourceSnapshot(): NexusClawConfig | null {
  return runtimeConfigSourceSnapshot;
}

function isCompatibleTopLevelRuntimeProjectionShape(params: {
  runtimeSnapshot: NexusClawConfig;
  candidate: NexusClawConfig;
}): boolean {
...
export function projectConfigOntoRuntimeSourceSnapshot(config: NexusClawConfig): NexusClawConfig {
...
export function loadConfig(): NexusClawConfig {
...
export async function readBestEffortConfig(): Promise<NexusClawConfig> {
...
export async function writeConfigFile(
  ...
  // edits to nexusclaw.json remain visible (no stale snapshot).
}
