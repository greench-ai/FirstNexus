...
function collectConfigEnvVarsByTarget(cfg?: NexusClawConfig): Record<string, string> {
...
export function collectConfigRuntimeEnvVars(cfg?: NexusClawConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

export function collectConfigServiceEnvVars(cfg?: NexusClawConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

/** @deprecated Use `collectConfigRuntimeEnvVars` or `collectConfigServiceEnvVars`. */
export function collectConfigEnvVars(cfg?: NexusClawConfig): Record<string, string> {
...
export function applyConfigEnvVars(
  ...
  for (const [key, value] of Object.entries(entries)) {
    ...
    // (e.g. process.env.NEXUSCLAW_GATEWAY_TOKEN = "${VAULT_TOKEN}") which downstream auth
