...
export type ConfigSchema = ReturnType<typeof NexusClawSchema.toJSONSchema>;
...
function buildBaseConfigSchema(): ConfigSchemaResponse {
  ...
  const schema = NexusClawSchema.toJSONSchema({
    target: "draft-07",
    unrepresentable: "any",
  });
  schema.title = "NexusClawConfig";
  const hints = applyDerivedTags(mapSensitivePaths(NexusClawSchema, "", buildBaseHints()));
  const next = {
