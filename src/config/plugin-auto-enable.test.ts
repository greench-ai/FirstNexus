...
function makeTempDir() {
  const dir = mkdtempSafe(path.join(os.tmpdir(), "nexusclaw-plugin-auto-enable-"));
  tempDirs.push(dir);
  return dir;
}
...
function writePluginManifestFixture(params: { rootDir: string; id: string; channels: string[] }) {
  ...
  fs.writeFileSync(
    path.join(params.rootDir, "nexusclaw.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
function makeRegistry(plugins: Array<{ id: string; channels: string[] }>): PluginManifestRegistry {
  return {
    plugins: plugins.map((p) => ({
      ...
      manifestPath: `/fake/${p.id}/nexusclaw.plugin.json`,
    })),
    diagnostics: [],
  };
}
...
describe("applyPluginAutoEnable", () => {
        ...
        entries: [
          {
            ...
            nexusclaw: {
              ...
              install: {
                npmSpec: "@nexusclaw/env-secondary",
              },
            },
          },
        ],
  ...
  describe("third-party channel plugins (pluginId ≠ channelId)", () => {
    it("uses the plugin manifest id, not the channel id, for plugins.entries", () => {
      // Reproduces: https://github.com/nexusclaw/nexusclaw/issues/25261
