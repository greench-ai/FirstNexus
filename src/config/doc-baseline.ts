...
function logConfigDocBaselineDebug(message: string): void {
  if (process.env.NEXUSCLAW_CONFIG_DOC_BASELINE_DEBUG === "1") {
    console.error(`[config-doc-baseline] ${message}`);
  }
}

function resolveRepoRoot(): string {
  const fromPackage = resolveNexusClawPackageRootSync({
...
async function loadBundledConfigSchemaResponse(): Promise<ConfigSchemaResponse> {
  ...
  const env = {
    ...process.env,
    HOME: os.tmpdir(),
    NEXUSCLAW_STATE_DIR: path.join(os.tmpdir(), "nexusclaw-config-doc-baseline-state"),
    NEXUSCLAW_BUNDLED_PLUGINS_DIR: path.join(repoRoot, "extensions"),
  };
  ...
  const channelPlugins =
    process.env.NEXUSCLAW_CONFIG_DOC_BASELINE_DEBUG === "1"
