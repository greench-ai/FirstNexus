...
async function withTempHome(run: (home: string) => Promise<void>): Promise<void> {
  const home = await fs.mkdtemp(path.join(os.tmpdir(), "nexusclaw-config-"));
  try {
    await run(home);
  } finally {
...
async function writeConfig(
  ...
  dirname: ".nexusclaw",
  port: number,
  filename: string = "nexusclaw.json",
) {
...
describe("config io paths", () => {
  it("uses ~/.nexusclaw/nexusclaw.json when config exists", async () => {
    await withTempHome(async (home) => {
      const configPath = await writeConfig(home, ".nexusclaw", 19001);
  ...
  it("defaults to ~/.nexusclaw/nexusclaw.json when config is missing", async () => {
    await withTempHome(async (home) => {
      const io = createIoForHome(home);
      expect(io.configPath).toBe(path.join(home, ".nexusclaw", "nexusclaw.json"));
    });
  });

  it("uses NEXUSCLAW_HOME for default config path", async () => {
    await withTempHome(async (home) => {
      const io = createConfigIO({
        env: { NEXUSCLAW_HOME: path.join(home, "svc-home") } as NodeJS.ProcessEnv,
        homedir: () => path.join(home, "ignored-home"),
      });
      expect(io.configPath).toBe(path.join(home, "svc-home", ".nexusclaw", "nexusclaw.json"));
    });
  });

  it("honors explicit NEXUSCLAW_CONFIG_PATH override", async () => {
    await withTempHome(async (home) => {
      const customPath = await writeConfig(home, ".nexusclaw", 20002, "custom.json");
      const io = createIoForHome(home, { NEXUSCLAW_CONFIG_PATH: customPath } as NodeJS.ProcessEnv);
  ...
  it("honors legacy CLAWDBOT_CONFIG_PATH override", async () => {
    await withTempHome(async (home) => {
      const customPath = await writeConfig(home, ".nexusclaw", 20003, "legacy-custom.json");
  ...
  it("normalizes safe-bin config entries at config load time", async () => {
    await withTempHome(async (home) => {
      ...
      const configPath = path.join(configDir, "nexusclaw.json");
      await fs.writeFile(
        configPath,
        JSON.stringify(
          {
