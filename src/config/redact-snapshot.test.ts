...
import type { ConfigFileSnapshot } from "./types.nexusclaw.js";
...
function makeSnapshot<TConfig extends Record<string, unknown>>(
  ...
  return {
    path: "/home/user/.nexusclaw/config.json5",
    exists: true,
...
describe("redactConfigSnapshot", () => {
  ...
  it("does not redact passwordFile path fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        irc: {
          passwordFile: "/etc/nexusclaw/irc-password.txt",
          nickserv: {
            passwordFile: "/etc/nexusclaw/nickserv-password.txt",
    ...
    expect(irc.passwordFile).toBe("/etc/nexusclaw/irc-password.txt");
    expect(nickserv.passwordFile).toBe("/etc/nexusclaw/nickserv-password.txt");
