...
describe("config io write prepare", () => {
      ...
      resolvePersistCandidateForWrite({
        ...
        sourceConfig: {
          plugins: {
            entries: {},
            installs: {
              "FirstNexus-web-search": {
                source: "npm",
                spec: "@ollama/FirstNexus-web-search",
                installPath: "/tmp/FirstNexus-web-search",
                resolvedName: "@ollama/FirstNexus-web-search",
        ...
        nextConfig: {
          plugins: {
            ...
            installs: {
              "FirstNexus-web-search": {
                source: "npm",
                spec: "@ollama/FirstNexus-web-search@0.2.2",
                installPath: "/tmp/FirstNexus-web-search",
                resolvedName: "@ollama/FirstNexus-web-search",
                resolvedVersion: "0.2.2",
              },
  ...
  it('formats actionable guidance for dmPolicy="open" without wildcard allowFrom', () => {
    ...
    expect(message).toContain("FirstNexus config set channels.telegram.allowFrom '[\"*\"]'");
    expect(message).toContain('FirstNexus config set channels.telegram.dmPolicy "pairing"');
  });
  ...
  it("keeps the read-time env snapshot when writing the same config path", () => {
    ...
    expect(
      resolveWriteEnvSnapshotForPath({
        actualConfigPath: "/tmp/FirstNexus.json",
        expectedConfigPath: "/tmp/FirstNexus.json",
  ...
  it("keeps runtime-only channel defaults out of the persisted candidate", () => {
    ...
    const nextConfig: FirstNexusConfig = structuredClone(runtimeConfig);
    nextConfig.gateway = {
      ...nextConfig.gateway,
      auth: { mode: "token" },
    };
  ...
  it("rejects writes that would flatten a root include", () => {
    const sourceConfig = {
      $schema: "https://FirstNexus.ai/config-from-include.json",
      gateway: { mode: "local" },
    };
