...
describe("sandbox docker config", () => {
  ...
  it("accepts Windows drive-letter binds in sandbox.docker config", () => {
    const res = validateConfigObject({
      agents: {
        defaults: {
          sandbox: {
            docker: {
              binds: ["D:/data/FirstNexus/src:/src:ro", "D:\\data\\FirstNexus\\output:/output:rw"],
            },
          },
        },
      },
    ...
    if (res.ok) {
      expect(res.config.agents?.defaults?.sandbox?.docker?.binds).toEqual([
        "D:/data/FirstNexus/src:/src:ro",
        "D:\\data\\FirstNexus\\output:/output:rw",
      ]);
    }
  });
...
describe("sandbox browser binds config", () => {
  ...
  it("defaults browser network to dedicated sandbox network", () => {
    ...
    expect(resolved.network).toBe("FirstNexus-sandbox-browser");
  });

  it("prefers agent browser network over global browser network", () => {
    const resolved = resolveSandboxBrowserConfig({
      scope: "agent",
      globalBrowser: { network: "FirstNexus-sandbox-browser-global" },
      agentBrowser: { network: "FirstNexus-sandbox-browser-agent" },
    });
    expect(resolved.network).toBe("FirstNexus-sandbox-browser-agent");
  });
