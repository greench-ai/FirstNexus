...
const ENV_CATALOG_PATHS = ["NEXUSCLAW_PLUGIN_CATALOG_PATHS", "NEXUSCLAW_MPM_CATALOG_PATHS"];

function collectModelRefs(cfg: NexusClawConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: NexusClawConfig, providerId: string): boolean {
...
function parseExternalCatalogChannelEntries(raw: unknown): ExternalCatalogChannelEntry[] {
  ...
  for (const entry of list) {
    if (!isRecord(entry) || !isRecord(entry.nexusclaw) || !isRecord(entry.nexusclaw.channel)) {
      continue;
    }
    const channel = entry.nexusclaw.channel;
...
function collectCandidateChannelIds(cfg: NexusClawConfig): string[] {
...
function configMayNeedPluginManifestRegistry(cfg: NexusClawConfig): boolean {
  const configuredChannels = cfg.channels as Record<string, unknown> | undefined;
  if (!configuredChannels || typeof configuredChannels !== "object") {
    return false;
  }
...
function isPluginExplicitlyDisabled(cfg: NexusClawConfig, pluginId: string): boolean {
...
function isPluginDenied(cfg: NexusClawConfig, pluginId: string): boolean {
...
function registerPluginEntry(cfg: NexusClawConfig, pluginId: string): NexusClawConfig {
  const builtInChannelId = normalizeChatChannelId(pluginId);
  if (builtInChannelId) {
    const channels = cfg.channels as Record<string, unknown> | undefined;
