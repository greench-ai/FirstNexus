...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "nexusclaw-sessions-suite-"));
  });
  ...
  const withStateDir = <T>(stateDir: string, fn: () => T): T =>
    withEnv({ NEXUSCLAW_STATE_DIR: stateDir }, fn);
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-nexusclaw",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-nexusclaw#general");
  });
  ...
  it("derives session transcripts dir from NEXUSCLAW_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { NEXUSCLAW_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("resolves cross-agent paths when NEXUSCLAW_STATE_DIR differs from stored paths", () => {
