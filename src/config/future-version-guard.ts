...
type FutureConfigGuardParams = {
  ...
  config?: Pick<FirstNexusConfig, "meta"> | null;
  currentVersion?: string;
...
export function resolveFutureConfigActionBlock(
  ...
  return {
    ...
    currentVersion,
    touchedVersion,
    message: `Refusing to ${params.action} because this FirstNexus binary (${currentVersion}) is older than the config last written by FirstNexus ${touchedVersion}.`,
    hints: [
