...
function createCustomRootCfg(customRoot: string, defaultAgentId = "ops"): NexusClawConfig {
  return {
    session: {
...
const discoveryResolvers = [
  {
    label: "async",
    resolve: async (cfg: NexusClawConfig, env: NodeJS.ProcessEnv) =>
...
describe("resolveSessionStoreTargets", () => {
  it("resolves all configured agent stores", () => {
    const cfg: NexusClawConfig = {
      session: {
        store: "~/.nexusclaw/agents/{agentId}/sessions/sessions.json",
      },
      agents: {
    ...
    expect(targets).toEqual([
      {
        ...
        storePath: path.resolve(
          path.join(process.env.HOME ?? "", ".nexusclaw/agents/main/sessions/sessions.json"),
        ),
      },
      {
        agentId: "work",
        storePath: path.resolve(
          path.join(process.env.HOME ?? "", ".nexusclaw/agents/work/sessions/sessions.json"),
        ),
      },
    ]);
  });
...
describe("resolveAllAgentSessionStoreTargets", () => {
  ...
  it("respects the caller env when resolving configured and discovered store roots", async () => {
    await withTempHome(async (home) => {
      ...
      const env = {
        ...process.env,
        NEXUSCLAW_STATE_DIR: envStateDir,
      };
      const cfg: NexusClawConfig = {};
