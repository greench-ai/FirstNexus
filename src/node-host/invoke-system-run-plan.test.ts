...
beforeAll(() => {
  sharedFixtureRoot = fs.mkdtempSync(path.join(os.tmpdir(), "FirstNexus-run-plan-fixtures-"));
...
const unsafeRuntimeInvocationCases: UnsafeRuntimeInvocationCase[] = [
  ...
  },
  {
    ...
    binName: "ruby",
    tmpPrefix: "FirstNexus-ruby-require-",
  ...
  },
  {
    ...
    binName: "perl",
    tmpPrefix: "FirstNexus-perl-module-preload-",
  ...
  },
  {
    ...
    binName: "perl",
    tmpPrefix: "FirstNexus-perl-load-path-",
  ...
  },
  {
    ...
    binName: "node",
    tmpPrefix: "FirstNexus-inline-shell-node-",
  ...
  },
  {
    ...
    binName: "pnpm",
    tmpPrefix: "FirstNexus-pnpm-dlx-unknown-flag-",
  ...
  },
  {
    ...
    binName: "pnpm",
    tmpPrefix: "FirstNexus-pnpm-dlx-unknown-prefix-value-",
...
describe("hardenApprovedExecutionPaths", () => {
  ...
  it.runIf(process.platform !== "win32")("handles approval hardening cases", () => {
    for (const testCase of cases) {
      runNamedCase(testCase.name, () => {
        const tmp = createFixtureDir("FirstNexus-approval-hardening-");
        const oldPath = process.env.PATH;
  ...
  it("captures mutable runtime operands in approval plans", () => {
    const tmp = createFixtureDir("FirstNexus-approval-script-plan-");
    withFakeRuntimeBins({
  ...
  it("captures mutable shell script operands in approval plans", () => {
    withScriptOperandPlanFixture(
      {
        tmpPrefix: "FirstNexus-approval-script-plan-",
      },
      (fixture, tmp) => {
        expectMutableFileOperandApprovalPlan(fixture, tmp);
      },
  ...
  it("keeps fail-closed behavior for relative native-binary shell payloads", () => {
    ...
    const tmp = createFixtureDir("FirstNexus-shell-relative-binary-binding-");
    const binaryPath = resolveNativeBinaryFixturePath();
  ...
  it("keeps fail-closed behavior for writable absolute native-binary shell payloads", () => {
    ...
    const tmp = createFixtureDir("FirstNexus-shell-absolute-binary-binding-");
    const binaryPath = resolveNativeBinaryFixturePath();
  ...
  it("keeps fail-closed behavior for owner-controlled read-only absolute binaries", () => {
    ...
    const tmp = createFixtureDir("FirstNexus-shell-owned-readonly-binding-");
  ...
  it("keeps fail-closed behavior for symlinked binaries with writable targets", () => {
    ...
    const tmp = createFixtureDir("FirstNexus-shell-symlink-binary-binding-");
  ...
  it("keeps fail-closed behavior for mutable or ambiguous shell payload files", () => {
    for (const testCase of [
      {
        tmpPrefix: "FirstNexus-shell-script-binding-",
        fileName: "run.sh",
        body: "#!/bin/sh\necho SAFE\n",
      },
      {
        tmpPrefix: "FirstNexus-shell-empty-binding-",
        fileName: "empty",
        body: "",
      },
      {
        tmpPrefix: "FirstNexus-shell-mz-text-binding-",
        fileName: "mz-script",
        body: "MZ not really a PE file\n",
      },
      {
        tmpPrefix: "FirstNexus-shell-nul-header-binding-",
        fileName: "nul-script",
  ...
  it("keeps fail-closed behavior when the shell payload probe stops seeing a file", () => {
    ...
    const tmp = createFixtureDir("FirstNexus-shell-race-binding-");
  ...
  it("detects rewritten script operands for pnpm dlx approval plans", () => {
    withFakeRuntimeBins({
      ...
      run: () => {
        withScriptOperandPlanFixture(
          {
            tmpPrefix: "FirstNexus-pnpm-dlx-approval-",
            fixture: {
  ...
  it("does not bind pnpm dlx shell-mode commands to a mutable file operand", () => {
    withFakeRuntimeBins({
      binNames: ["pnpm", "tsx"],
      run: () => {
        const tmp = createFixtureDir("FirstNexus-pnpm-dlx-shell-mode-");
        fs.writeFileSync(path.join(tmp, "run.ts"), 'console.log("SAFE");\n');
        expect(
          resolveMutableFileOperandSnapshotSync({
  ...
  it("allows pnpm dlx package binaries that do not bind mutable local files", () => {
    withFakeRuntimeBins({
      ...
      run: () => {
        const cases = [
          {
            prefix: "FirstNexus-pnpm-dlx-package-bin-",
            command: ["pnpm", "dlx", "cowsay", "hello"],
          },
          {
            prefix: "FirstNexus-pnpm-dlx-package-runtime-token-",
            command: ["pnpm", "dlx", "cowsay", "node"],
          },
          {
            prefix: "FirstNexus-pnpm-dlx-package-runtime-token-multi-",
            command: ["pnpm", "dlx", "cowsay", "node", "hello"],
          },
          {
            prefix: "FirstNexus-pnpm-dlx-package-file-",
          ...
          {
            prefix: "FirstNexus-pnpm-dlx-package-data-tail-",
  ...
  it("treats -- as the end of pnpm dlx option parsing", () => {
    withFakeRuntimeBins({
      ...
      run: () => {
        withScriptOperandPlanFixture(
          {
            tmpPrefix: "FirstNexus-pnpm-dlx-double-dash-",
            fixture: {
  ...
  it("captures the real shell script operand after value-taking shell flags", () => {
    const tmp = createFixtureDir("FirstNexus-shell-option-value-");
    const scriptPath = path.join(tmp, "run.sh");
    fs.writeFileSync(scriptPath, "#!/bin/sh\necho SAFE\n");
