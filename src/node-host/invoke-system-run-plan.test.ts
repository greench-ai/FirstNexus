...
function withFakeRuntimeBin<T>(params: { binName: string; run: () => T }): T {
  return withFakeRuntimeBins({
    ...
    tmpPrefix: `nexusclaw-${params.binName}-bin-`,
    run: params.run,
  });
}
...
function withFakeRuntimeBins<T>(params: {
  ...
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), params.tmpPrefix ?? "nexusclaw-runtime-bins-"));
...
const unsafeRuntimeInvocationCases: UnsafeRuntimeInvocationCase[] = [
  ...
  },
  {
    ...
    binName: "ruby",
    tmpPrefix: "nexusclaw-ruby-require-",
  ...
  },
  {
    ...
    binName: "ruby",
    tmpPrefix: "nexusclaw-ruby-load-path-",
  ...
  },
  {
    ...
    binName: "perl",
    tmpPrefix: "nexusclaw-perl-module-preload-",
  ...
  },
  {
    ...
    binName: "perl",
    tmpPrefix: "nexusclaw-perl-load-path-",
  ...
  },
  {
    ...
    binName: "perl",
    tmpPrefix: "nexusclaw-perl-preload-load-path-",
  ...
  },
  {
    ...
    binName: "node",
    tmpPrefix: "nexusclaw-inline-shell-node-",
...
describe("hardenApprovedExecutionPaths", () => {
  ...
  for (const testCase of cases) {
    it.runIf(process.platform !== "win32")(testCase.name, () => {
      const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "nexusclaw-approval-hardening-"));
      const oldPath = process.env.PATH;
      ...
      withFakeRuntimeBins({
        ...
        run: () => {
          withScriptOperandPlanFixture(
            {
              tmpPrefix: "nexusclaw-approval-script-plan-",
              fixture: runtimeCase,
  ...
  it("captures the real shell script operand after value-taking shell flags", () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "nexusclaw-shell-option-value-"));
    try {
