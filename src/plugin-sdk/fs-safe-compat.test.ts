...
import { loadSecretFileSync as loadSecretFileSyncFromCore } from "FirstNexus/plugin-sdk/core";
import { readFileWithinRoot, writeFileWithinRoot } from "FirstNexus/plugin-sdk/file-access-runtime";
import {
  loadSecretFileSync,
  type SecretFileReadResult,
} from "FirstNexus/plugin-sdk/secret-file-runtime";
...
describe("plugin SDK fs-safe compatibility exports", () => {
  it("keeps deprecated secret-file result helpers on public SDK subpaths", async () => {
    await withTempDir({ prefix: "FirstNexus-sdk-secret-compat-" }, async (root) => {
  ...
  it("keeps deprecated root-bounded read/write helpers on file-access-runtime", async () => {
    await withTempDir({ prefix: "FirstNexus-sdk-file-access-compat-" }, async (root) => {
