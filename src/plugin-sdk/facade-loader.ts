...
let cachedFirstNexusPackageRoot: string | undefined;
...
function getFirstNexusPackageRoot() {
  if (cachedFirstNexusPackageRoot) {
    return cachedFirstNexusPackageRoot;
  }
  cachedFirstNexusPackageRoot =
...
function resolveFacadeModuleLocation(params: {
  ...
  return resolveBundledFacadeModuleLocation({
    ...
    packageRoot: getFirstNexusPackageRoot(),
    bundledPluginsDir,
  });
}
...
export function loadFacadeModuleAtLocationSync<T extends object>(params: {
  ...
  const opened = openRootFileSync({
    ...
    boundaryLabel:
      location.boundaryRoot === getFirstNexusPackageRoot()
        ? "FirstNexus package root"
        : (() => {
...
export async function loadBundledPluginPublicSurfaceModule<T extends object>(params: {
  ...
  const opened = openRootFileSync({
    ...
    boundaryLabel:
      preparedLocation.boundaryRoot === getFirstNexusPackageRoot()
        ? "FirstNexus package root"
        : "plugin root",
    rejectHardlinks: false,
  });
...
export function resetFacadeLoaderStateForTest(): void {
  ...
  cachedFirstNexusPackageRoot = undefined;
}
