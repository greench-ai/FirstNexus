...
import type { FirstNexusConfig } from "../testing.js";
import {
...
export function expectProviderOnboardAllowlistAlias(params: {
  applyProviderConfig: (config: FirstNexusConfig) => FirstNexusConfig;
  modelRef: string;
  alias: string;
}) {
...
export function expectProviderOnboardPrimaryAndFallbacks(params: {
  applyConfig: (config: FirstNexusConfig) => FirstNexusConfig;
  modelRef: string;
}) {
