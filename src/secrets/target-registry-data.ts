...
function createPluginFirstNexusConfigSecretTargetEntry(
  pluginId: string,
  configPath: string,
  ...
  for (const record of bundledPlugins) {
    for (const config of WEB_PROVIDER_SECRET_CONFIGS) {
      if (
        ...
        entries.push(createPluginFirstNexusConfigSecretTargetEntry(record.id, config.configPath));
      }
    }
  }
...
function listBundledPluginConfigSecretTargetRegistryEntries(
  ...
  for (const record of bundledPlugins) {
    ...
    for (const secretInput of secretInputs) {
      const entry = createPluginFirstNexusConfigSecretTargetEntry(record.id, secretInput.path);
...
export function getSourceSecretTargetRegistry(): SecretTargetRegistryEntry[] {
  return loadSecretTargetRegistryFromPluginMetadata({
    env: {
      ...process.env,
      NEXISCLAW_BUNDLED_PLUGINS_DIR: process.env.NEXISCLAW_BUNDLED_PLUGINS_DIR ?? "extensions",
    },
