...
export function buildConfigureCandidates(config: NexusClawConfig): ConfigureCandidate[] {
...
export function buildConfigureCandidatesForScope(params: {
  ...
  authoredNexusClawConfig?: NexusClawConfig;
  authProfiles?: {
    agentId: string;
    store: AuthProfileStore;
  };
}): ConfigureCandidate[] {
  const authoredConfig = params.authoredNexusClawConfig ?? params.config;

  const hasPathInAuthoredConfig = (pathSegments: string[]): boolean =>
    hasPath(authoredConfig, pathSegments);

  const nexusclawCandidates = discoverConfigSecretTargets(params.config)
    ...
    .map((entry) => {
      ...
      return {
        ...
        label: entry.path,
        configFile: "nexusclaw.json" as const,
  ...
  return [...nexusclawCandidates, ...authCandidates].toSorted((a, b) =>
...
export function collectConfigureProviderChanges(params: {
  original: NexusClawConfig;
  next: NexusClawConfig;
...
export function buildSecretsConfigurePlan(params: {
  ...
  return {
    ...
    generatedBy: "nexusclaw secrets configure",
