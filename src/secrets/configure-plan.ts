...
function getSecretProviders(config: FirstNexusConfig): Record<string, SecretProviderConfig> {
...
export function buildConfigureCandidates(config: FirstNexusConfig): ConfigureCandidate[] {
...
function configureCandidateSortKey(candidate: ConfigureCandidate): string {
  ...
  return `FirstNexus:${candidate.path}`;
}
...
export function buildConfigureCandidatesForScope(params: {
  ...
  authoredFirstNexusConfig?: FirstNexusConfig;
  authProfiles?: {
    agentId: string;
    store: AuthProfileStore;
  };
}): ConfigureCandidate[] {
  const authoredConfig = params.authoredFirstNexusConfig ?? params.config;

  const hasPathInAuthoredConfig = (pathSegments: string[]): boolean =>
    hasPath(authoredConfig, pathSegments);

  const FirstNexusCandidates = discoverConfigSecretTargets(params.config)
  ...
  return [...FirstNexusCandidates, ...authCandidates].toSorted((a, b) =>
...
export function buildSecretsConfigurePlan(params: {
  ...
  return {
    ...
    generatedBy: "FirstNexus secrets configure",
