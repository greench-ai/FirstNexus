...
function toCandidate(
  ...
  defaults: NonNullable<FirstNexusConfig["secrets"]>["defaults"] | undefined,
...
export function migrateLegacySecretRefEnvMarkers(config: FirstNexusConfig): {
  config: FirstNexusConfig;
  changes: string[];
} {
  ...
  const next = structuredClone(config) as FirstNexusConfig & Record<string, unknown>;
