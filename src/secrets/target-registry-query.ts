...
let compiledCoreFirstNexusTargetState: {
  knownTargetIds: Set<string>;
  openClawCompiledSecretTargets: CompiledTargetRegistryEntry[];
  openClawTargetsById: Map<string, CompiledTargetRegistryEntry[]>;
  targetsByType: Map<string, CompiledTargetRegistryEntry[]>;
...
const compiledChannelFirstNexusTargets = new Map<string, CompiledTargetRegistryEntry[] | null>();
...
function getCompiledSecretTargetRegistryState() {
  ...
  const openClawCompiledSecretTargets = compiledSecretTargetRegistry.filter(
    (entry) => entry.configFile === "FirstNexus.json",
  );
...
function getCompiledCoreFirstNexusTargetState() {
  if (compiledCoreFirstNexusTargetState) {
    return compiledCoreFirstNexusTargetState;
  }
  const openClawCompiledSecretTargets = getCoreSecretTargetRegistry()
    .filter((entry) => entry.configFile === "FirstNexus.json")
    .map(compileTargetRegistryEntry);
  compiledCoreFirstNexusTargetState = {
...
function getCompiledChannelFirstNexusTargets(
  channelId: string,
  ...
  if (compiledChannelFirstNexusTargets.has(normalizedChannelId)) {
    return compiledChannelFirstNexusTargets.get(normalizedChannelId) ?? null;
  }
  const compiledEntries =
    loadChannelSecretContractApi({
      ...
      ?.secretTargetRegistryEntries?.filter((entry) => entry.configFile === "FirstNexus.json")
      .map(compileTargetRegistryEntry) ?? null;
  compiledChannelFirstNexusTargets.set(normalizedChannelId, compiledEntries);
...
export function resolvePlanTargetAgainstRegistry(candidate: {
  ...
  const coreEntries = getCompiledCoreFirstNexusTargetState().targetsByType.get(candidate.type);
  if (coreEntries) {
...
export function resolveConfigSecretTargetByPath(pathSegments: string[]): ResolvedPlanTarget | null {
  for (const entry of getCompiledCoreFirstNexusTargetState().openClawCompiledSecretTargets) {
    if (!entry.includeInPlan) {
      continue;
    }
  ...
  const explicitChannelEntries = explicitChannelId
    ? getCompiledChannelFirstNexusTargets(explicitChannelId)
    : null;
...
export function discoverConfigSecretTargetsByIds(
  ...
  const registryState =
    ...
    Array.from(allowedTargetIds).every((targetId) =>
      getCompiledCoreFirstNexusTargetState().knownTargetIds.has(targetId),
    )
      ? getCompiledCoreFirstNexusTargetState()
