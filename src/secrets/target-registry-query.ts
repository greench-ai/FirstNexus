...
const NEXUSCLAW_COMPILED_SECRET_TARGETS = COMPILED_SECRET_TARGET_REGISTRY.filter(
...
function buildConfigTargetIdIndex(): Map<string, CompiledTargetRegistryEntry[]> {
  ...
  for (const entry of NEXUSCLAW_COMPILED_SECRET_TARGETS) {
...
const NEXUSCLAW_TARGETS_BY_ID = buildConfigTargetIdIndex();
...
export function discoverConfigSecretTargetsByIds(
  ...
  const discoveryEntries = resolveDiscoveryEntries({
    allowedTargetIds,
    defaultEntries: NEXUSCLAW_COMPILED_SECRET_TARGETS,
    entriesById: NEXUSCLAW_TARGETS_BY_ID,
  });
