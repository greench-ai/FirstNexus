...
function setConfiguredFetchProviderKey(configTarget: FirstNexusConfig, value: unknown): void {
  const plugins = ensureRecord(configTarget as Record<string, unknown>, "plugins");
  const entries = ensureRecord(plugins, "entries");
...
async function runRuntimeWebTools(params: { config: FirstNexusConfig; env?: NodeJS.ProcessEnv }) {
...
function readProviderKey(config: FirstNexusConfig, provider: ProviderUnderTest): unknown {
...
describe("runtime web tools resolution", () => {
  ...
  it("uses runtime web search discovery when the managed plugin index install records is populated", async () => {
    loadInstalledPluginIndexInstallRecordsSyncMock.mockReturnValue({
      "external-search": {
        source: "npm",
        spec: "@FirstNexus/external-search",
      },
    });
  ...
  it("uses runtime web fetch discovery when the managed plugin index install records is populated", async () => {
    loadInstalledPluginIndexInstallRecordsSyncMock.mockReturnValue({
      "external-fetch": {
        source: "npm",
        spec: "@FirstNexus/external-fetch",
      },
    });
  ...
  describe("when brave is installed as an external plugin and explicitly configured", () => {
    ...
    beforeEach(() => {
      loadInstalledPluginIndexInstallRecordsSyncMock.mockReturnValue({
        brave: { source: "npm", spec: "@FirstNexus/brave-search" },
      });
