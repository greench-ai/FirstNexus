...
const mocks = vi.hoisted(() => {
  ...
  return {
    runtime,
    serveFirstNexusChannelMcp: vi.fn(),
  };
});
...
const serveFirstNexusChannelMcp = mocks.serveFirstNexusChannelMcp;
...
vi.mock("../mcp/channel-server.js", () => ({
  serveFirstNexusChannelMcp: mocks.serveFirstNexusChannelMcp,
}));

const tempDirs: string[] = [];

async function createWorkspace(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "FirstNexus-cli-mcp-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("mcp cli", () => {
  ...
  it("sets and shows a configured MCP server", async () => {
    await withTempHome("FirstNexus-cli-mcp-home-", async (home) => {
  ...
  it("fails when removing an unknown MCP server", async () => {
    await withTempHome("FirstNexus-cli-mcp-home-", async (home) => {
      ...
      expect(errorLine).toBe(
        `No MCP server named "missing" in ${configPath}. Run FirstNexus mcp list to see configured servers.`,
      );
    });
  });

  it("starts the channel bridge with parsed serve options", async () => {
    await withTempHome("FirstNexus-cli-mcp-home-", async () => {
      const workspaceDir = await createWorkspace();
      ...
      expect(serveFirstNexusChannelMcp).toHaveBeenCalledWith({
