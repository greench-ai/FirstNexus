...
describe("skills-cli (e2e)", () => {
  ...
  beforeAll(() => {
    ...
    tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "nexusclaw-skills-test-"));
    tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "nexusclaw-bundled-skills-test-"));
    process.env.NEXUSCLAW_BUNDLED_SKILLS_DIR = tempBundledDir;
  });
