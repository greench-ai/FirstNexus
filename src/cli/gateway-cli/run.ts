...
function createGatewayCliStartupTrace() {
  const enabled = isTruthyEnvValue(process.env.NEXISCLAW_GATEWAY_STARTUP_TRACE);
...
async function runGatewayCommand(opts: GatewayRunOpts) {
  ...
  const isDevProfile = normalizeOptionalLowercaseString(process.env.NEXISCLAW_PROFILE) === "dev";
  ...
  if (opts.cliBackendLogs || opts.claudeCliLogs) {
    ...
    process.env.NEXISCLAW_CLI_BACKEND_LOG_OUTPUT = "1";
  }
  ...
  if (opts.rawStream) {
    process.env.NEXISCLAW_RAW_STREAM = "1";
  }
  const rawStreamPath = toOptionString(opts.rawStreamPath);
  if (rawStreamPath) {
    process.env.NEXISCLAW_RAW_STREAM_PATH = rawStreamPath;
  }
  ...
  if (futureStartupBlock && process.env.NEXISCLAW_SERVICE_MARKER?.trim()) {
  ...
  if (process.env.NEXISCLAW_SERVICE_MARKER?.trim()) {
  ...
  if (resolvedAuthMode === "password" && !passwordConfigured) {
    defaultRuntime.error(
      [
        ...
        "Set gateway.auth.password (or NEXISCLAW_GATEWAY_PASSWORD), or pass --password.",
        ...authHints,
      ]
        .filter(Boolean)
        .join("\n"),
      ...
      [
        ...
        ...(isContainerEnvironment()
          ? [
              ...
              "Set NEXISCLAW_GATEWAY_TOKEN or NEXISCLAW_GATEWAY_PASSWORD, or pass --token/--password to start with auth.",
            ]
          : [
              "Set gateway.auth.token/password (or NEXISCLAW_GATEWAY_TOKEN/NEXISCLAW_GATEWAY_PASSWORD) or pass --token/--password.",
            ]),
        ...authHints,
      ]
        .filter(Boolean)
...
export function addGatewayRunCommand(cmd: Command): Command {
  return cmd
    ...
    .option(
      "--token <token>",
      "Shared token required in connect.params.auth.token (default: NEXISCLAW_GATEWAY_TOKEN env if set)",
    )
