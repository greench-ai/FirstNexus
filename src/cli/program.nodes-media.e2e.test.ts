...
describe("cli program (nodes media)", () => {
  ...
  it("runs nodes camera snap and prints two MEDIA paths", async () => {
    ...
    expect(mediaPaths[0]).toContain("FirstNexus-camera-snap-");
    expect(mediaPaths[1]).toContain("FirstNexus-camera-snap-");
  ...
  it("runs nodes camera clip and prints one MEDIA path", async () => {
    ...
    await expectLoggedSingleMediaFile({
      expectedPathPattern: /FirstNexus-camera-clip-front-.*\.mp4$/,
    });
  });
    ...
    it.each([
      {
        ...
        expectedPathPattern: /FirstNexus-camera-snap-front-.*\.jpg$/,
      },
      {
