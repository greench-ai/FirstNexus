...
describe("completion-cli", () => {
  it("generates zsh functions for nested subcommands", () => {
    ...
    expect(script).toContain("_FirstNexus_gateway()");
    expect(script).toContain("(status) _FirstNexus_gateway_status ;;");
    expect(script).toContain("(restart) _FirstNexus_gateway_restart ;;");
  ...
  it("defers zsh registration until compinit is available", async () => {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "FirstNexus-zsh-completion-"));
    try {
      const scriptPath = path.join(tempDir, "FirstNexus.zsh");
      ...
      const result = spawnSync(
        ...
        [
          ...
          `
            ...
            [[ -z "\${_comps[FirstNexus]-}" ]] || exit 10
            [[ "\${precmd_functions[(r)_FirstNexus_register_completion]}" = "_FirstNexus_register_completion" ]] || exit 11
            autoload -Uz compinit
            compinit -C
            _FirstNexus_register_completion
            [[ -z "\${precmd_functions[(r)_FirstNexus_register_completion]}" ]] || exit 12
            [[ "\${_comps[FirstNexus]-}" = "_FirstNexus_root_completion" ]]
          `,
        ],
        {
          encoding: "utf8",
  ...
  it("generates PowerShell command paths without the executable prefix", () => {
    ...
    expect(script).not.toContain("if ($commandPath -eq 'FirstNexus gateway') {");
  ...
  it("generates fish completions for root and nested command contexts", () => {
    ...
    expect(script).toContain(
      'complete -c FirstNexus -n "__fish_use_subcommand" -a "gateway" -d \'Gateway commands\'',
    );
    expect(script).toContain(
      'complete -c FirstNexus -n "__fish_seen_subcommand_from gateway" -a "status" -d \'Show gateway status\'',
    );
    expect(script).toContain(
      "complete -c FirstNexus -n \"__fish_seen_subcommand_from gateway\" -l force -d 'Force the action'",
    );
  });
});
