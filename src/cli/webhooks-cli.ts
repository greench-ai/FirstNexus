...
export function registerWebhooksCli(program: Command) {
  const webhooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/webhooks", "docs.nexusclaw.ai/cli/webhooks")}\n`,
    );
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + NexusClaw hooks")
    ...
    .option("--hook-url <url>", "NexusClaw hook URL")
    .option("--hook-token <token>", "NexusClaw hook token")
