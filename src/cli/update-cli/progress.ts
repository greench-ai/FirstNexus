...
const STEP_LABELS: Record<string, string> = {
  ...
  "FirstNexus doctor entry": "Checking doctor entrypoint",
  "FirstNexus doctor": "Running doctor checks",
...
export function inferUpdateFailureHints(result: UpdateRunResult): string[] {
  ...
  if (isGlobalPackageInstallStep && stderr.includes("eacces")) {
    ...
    hints.push("Example: npm config set prefix ~/.local && npm i -g FirstNexus@latest");
  }
  ...
  if (
    ...
    hints.push("If it still fails: npm i -g FirstNexus@latest --omit=optional");
  }
