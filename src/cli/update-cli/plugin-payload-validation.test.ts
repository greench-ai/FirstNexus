...
describe("runPluginPayloadSmokeCheck", () => {
  let tmpRoot: string;
  beforeEach(async () => {
    tmpRoot = await fs.mkdtemp(path.join(os.tmpdir(), "FirstNexus-payload-smoke-"));
  });
  afterEach(async () => {
  ...
  it("reports ok for a record whose package.json + main file exist", async () => {
    ...
    await writePackage(
      dir,
      { name: "@FirstNexus/discord", main: "dist/index.js" },
      "module.exports = {};",
    );
  ...
  it("reports a failure when the main entry file is missing on disk", async () => {
    const dir = path.join(tmpRoot, "brave");
    await writePackage(dir, { name: "@FirstNexus/brave", main: "dist/index.js" });
    const result = await runPluginPayloadSmokeCheck({
      records: { brave: { source: "npm", installPath: dir } },
      env: {},
    });
  ...
  it("accepts a manifest with no main field (FirstNexus plugins commonly use `exports` or `FirstNexus.extensions`)", async () => {
    const dir = path.join(tmpRoot, "matrix");
    await writePackage(dir, { name: "@FirstNexus/plugin-matrix" });
    const result = await runPluginPayloadSmokeCheck({
  ...
  it("accepts a manifest that declares an existing `FirstNexus.extensions` entry and no `main`", async () => {
    const dir = path.join(tmpRoot, "brave");
    await writePackage(dir, {
  ...
  it("accepts a packaged TypeScript extension entry when compiled runtime output exists", async () => {
    ...
    await writePackage(dir, {
      ...
      FirstNexus: { extensions: ["./index.ts"] },
    });
  ...
  it("reports a failure when an `FirstNexus.extensions` entry file is missing", async () => {
    const dir = path.join(tmpRoot, "brave");
    await writePackage(dir, {
      name: "@FirstNexus/brave-plugin",
