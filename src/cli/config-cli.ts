...
function normalizeConfigMutationModelRefs(cfg: FirstNexusConfig): FirstNexusConfig {
...
const CONFIG_SET_EXAMPLE_PROVIDER = formatCliCommand(
  "FirstNexus config set secrets.providers.vault --provider-source file --provider-path /etc/FirstNexus/secrets.json --provider-mode json",
);
...
const CONFIG_PATCH_EXAMPLE_STDIN = formatCliCommand("FirstNexus config patch --stdin");
...
function formatDoctorHint(message: string): string {
  return `Run \`${formatCliCommand("FirstNexus doctor --fix")}\` ${message}`;
}
...
async function loadValidConfig(runtime: RuntimeEnv = defaultRuntime) {
  ...
  runtime.error(`FirstNexus config is invalid: ${shortenHomePath(snapshot.path)}`);
...
function formatPluginInstallConfigSetError(): string {
  return [
    ...
    `  ${formatCliCommand("FirstNexus plugins install <spec>")}`,
    `  ${formatCliCommand("FirstNexus plugins update <plugin-id>")}`,
    `  ${formatCliCommand("FirstNexus plugins uninstall <plugin-id>")}`,
  ].join("\n");
}
...
async function runConfigOperations(params: {
  ...
  const nextConfig = normalizeConfigMutationModelRefs(next as FirstNexusConfig);
...
export async function runConfigGet(opts: { path: string; json?: boolean; runtime?: RuntimeEnv }) {
  ...
  try {
    ...
    if (!res.found) {
      runtime.error(
        danger(
          `Config path not found: ${opts.path}. Run ${formatCliCommand("FirstNexus config validate")} to inspect config shape.`,
        ),
      );
      runtime.exit(1);
      return;
...
export async function runConfigUnset(opts: { path: string; runtime?: RuntimeEnv }) {
  ...
  try {
    ...
    if (!unsetResult.removed) {
      runtime.error(
        danger(
          `Config path not found: ${opts.path}. Nothing was changed. Run ${formatCliCommand("FirstNexus config get <path>")} first if you are unsure of the path.`,
        ),
      );
      runtime.exit(1);
      return;
...
export async function runConfigValidate(opts: { json?: boolean; runtime?: RuntimeEnv } = {}) {
  const runtime = opts.runtime ?? defaultRuntime;
  let outputPath = CONFIG_PATH ?? "FirstNexus.json";
  ...
  try {
    ...
    if (!snapshot.exists) {
      if (opts.json) {
        ...
        runtime.error(
          `Create one with ${formatCliCommand("FirstNexus onboard")} or run ${formatCliCommand("FirstNexus doctor --fix")}.`,
        );
      }
      runtime.exit(1);
      return;
    ...
    if (!snapshot.valid) {
      ...
      if (opts.json) {
        ...
        runtime.error(danger(`FirstNexus config is invalid: ${shortPath}`));
        ...
        runtime.error(`Inspect with ${formatCliCommand("FirstNexus config validate")}.`);
      }
      runtime.exit(1);
      return;
    }
...
export function registerConfigCli(program: Command) {
  const cmd = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/config", "docs.FirstNexus.ai/cli/config")}\n`,
    )
    .option(
      "--section <section>",
  ...
  setCommandJsonMode(cmd.command("set"), "parse-only")
    ...
    .option(
      "--dry-run",
      "Validate changes without writing FirstNexus.json (checks run in builder/json/batch modes; exec SecretRefs are skipped unless --allow-exec is set)",
      false,
    )
    .option(
      "--allow-exec",
  ...
  cmd
    ...
    .option(
      "--dry-run",
      "Validate changes without writing FirstNexus.json (checks schema and SecretRef resolvability; exec SecretRefs are skipped unless --allow-exec is set)",
      false,
    )
    .option(
      "--allow-exec",
  ...
  cmd
    .command("schema")
    .description("Print the JSON schema for FirstNexus.json")
    .action(async () => {
