...
describe("npm-resolution helpers", () => {
  it("keeps original spec when pin is disabled", () => {
    const result = resolvePinnedNpmSpec({
      rawSpec: "@nexusclaw/plugin-alpha@latest",
      pin: false,
      resolvedSpec: "@nexusclaw/plugin-alpha@1.2.3",
    });
    expect(result).toEqual({
      recordSpec: "@nexusclaw/plugin-alpha@latest",
    });
  });
  ...
  it("returns pinned spec notice when resolved spec is available", () => {
    ...
    expect(result).toEqual({
      recordSpec: "@nexusclaw/plugin-alpha@1.2.3",
      pinNotice: "Pinned npm install record to @nexusclaw/plugin-alpha@1.2.3.",
    });
  });
  ...
  it("maps npm resolution metadata to install fields", () => {
    expect(
      ...
      resolvedName: "@nexusclaw/plugin-alpha",
  ...
  it("builds common npm install record fields", () => {
    expect(
      buildNpmInstallRecordFields({
        spec: "@nexusclaw/plugin-alpha@1.2.3",
        installPath: "/tmp/nexusclaw/extensions/alpha",
        version: "1.2.3",
        resolution: {
          name: "@nexusclaw/plugin-alpha",
          version: "1.2.3",
  ...
  it("resolves pinned install record and emits pin notice", () => {
    ...
    expect(logs).toEqual(["Pinned npm install record to @nexusclaw/plugin-alpha@1.2.3."]);
  ...
  it("resolves pinned install record for CLI and formats warning output", () => {
    ...
    const record = resolvePinnedNpmInstallRecordForCli(
      "@nexusclaw/plugin-alpha@latest",
      true,
      "/tmp/nexusclaw/extensions/alpha",
      "1.2.3",
      undefined,
      (message) => logs.push(message),
    ...
    expect(record).toEqual({
      source: "npm",
      spec: "@nexusclaw/plugin-alpha@latest",
      installPath: "/tmp/nexusclaw/extensions/alpha",
      version: "1.2.3",
