...
describe("reparseProgramFromActionArgs", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    buildParseArgvMock.mockReturnValue(["node", "FirstNexus", "status"]);
  ...
  it("uses action command name + args as fallback argv", async () => {
    ...
    const actionCommand = {
      name: () => "status",
      parent: {
        rawArgs: ["node", "FirstNexus", "status", "--json"],
      },
    ...
    expect(parseAsync).toHaveBeenCalledWith(["node", "FirstNexus", "status"]);
  });
