...
export function registerStatusHealthSessionsCommands(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["FirstNexus status", "Show channel health + session summary."],
          ["FirstNexus status --all", "Full diagnosis (read-only)."],
          ["FirstNexus status --json", "Machine-readable output."],
          ["FirstNexus status --usage", "Show model provider usage/quota snapshots."],
          [
            "FirstNexus status --deep",
            "Run channel probes (WA + Telegram + Discord + Slack + Signal).",
          ],
          ["FirstNexus status --deep --timeout 5000", "Tighten probe timeout."],
        ])}`,
    )
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/status", "docs.FirstNexus.ai/cli/status")}\n`,
    )
    .action(async (opts) => {
  ...
  program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/health", "docs.FirstNexus.ai/cli/health")}\n`,
    )
    .action(async (opts) => {
      await runWithVerboseAndTimeout(opts, async ({ verbose, timeoutMs }) => {
        await healthCommand(
  ...
  const sessionsCmd = program
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["FirstNexus sessions", "List all sessions."],
          ["FirstNexus sessions --agent work", "List sessions for one agent."],
          ["FirstNexus sessions --all-agents", "Aggregate sessions across agents."],
          ["FirstNexus sessions --active 120", "Only last 2 hours."],
          ["FirstNexus sessions --limit 25", "Show the newest 25 sessions."],
          ["FirstNexus sessions --json", "Machine-readable output."],
          ["FirstNexus sessions --store ./tmp/sessions.json", "Use a specific session store."],
        ])}\n\n${theme.muted(
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/sessions", "docs.FirstNexus.ai/cli/sessions")}\n`,
    )
    .action(async (opts) => {
  ...
  sessionsCmd
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["FirstNexus sessions cleanup --dry-run", "Preview stale/cap cleanup."],
          [
            "FirstNexus sessions cleanup --dry-run --fix-missing",
            "Also preview pruning entries with missing transcript files.",
          ],
          [
            "FirstNexus sessions cleanup --dry-run --fix-dm-scope",
            "Preview stale direct-DM rows after returning dmScope to main.",
          ],
          ["FirstNexus sessions cleanup --enforce", "Apply maintenance now."],
          ["FirstNexus sessions cleanup --agent work --dry-run", "Preview one agent store."],
          ["FirstNexus sessions cleanup --all-agents --dry-run", "Preview all agent stores."],
          [
            "FirstNexus sessions cleanup --enforce --store ./tmp/sessions.json",
  ...
  sessionsCmd
    ...
    .option("--output <path>", "Output directory name inside .FirstNexus/trajectory-exports")
  ...
  const commitmentsCmd = program
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["FirstNexus commitments", "List pending inferred follow-ups."],
          ["FirstNexus commitments --all", "List all inferred follow-ups."],
          ["FirstNexus commitments --agent work", "List one agent's inferred follow-ups."],
          ["FirstNexus commitments dismiss cm_abc123", "Dismiss a follow-up."],
        ])}`,
    )
    .action(async (opts) => {
      await runCommandWithRuntime(defaultRuntime, async () => {
