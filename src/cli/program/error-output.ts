...
function resolveHelpCommand(argv: string[] | undefined, options?: { root?: boolean }): string {
  if (options?.root || !argv) {
    return formatCliCommand("FirstNexus --help");
  }
  const commandPath = getCommandPathWithRootOptions(argv, 2);
  if (commandPath.length === 0) {
    return formatCliCommand("FirstNexus --help");
  }
  return formatCliCommand(`FirstNexus ${commandPath.join(" ")} --help`);
}
...
function formatDocsHint(): string {
  return `${theme.muted("Docs:")} ${formatDocsLink("/cli", "docs.FirstNexus.ai/cli")}`;
}
...
export function formatCliParseErrorOutput(
  ...
  if (unknownCommand) {
    ...
    return lines(
      theme.error(`FirstNexus does not know the command ${quote(command)}.`),
      formatHelpHint(options.argv, { root: true }),
      `${theme.muted("Plugin command?")} ${theme.command(formatCliCommand("FirstNexus plugins list"))}`,
      formatDocsHint(),
    );
  }
  ...
  if (unknownOption) {
    ...
    return lines(
      theme.error(`FirstNexus does not recognize option ${quote(option)}.`),
  ...
  return lines(
    theme.error(`FirstNexus could not parse this command: ${message}`),
    formatHelpHint(options.argv),
  );
}
