...
${formatHelpExamples([
  ['FirstNexus agent --to +15555550123 --message "status update"', "Start a new session."],
  ['FirstNexus agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
  ...
  ['FirstNexus agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'FirstNexus agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.FirstNexus.ai/cli/agent")}`,
    )
    .action(async (opts) => {
      const verboseLevel =
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.FirstNexus.ai/cli/agents")}\n`,
    );
...
${formatHelpExamples([
  [
    'FirstNexus agents set-identity --agent main --name "FirstNexus" --emoji "🦞"',
    "Set name + emoji.",
  ],
  [
  ...
  [
    "FirstNexus agents set-identity --identity-file ~/.FirstNexus/workspace/IDENTITY.md --agent main",
