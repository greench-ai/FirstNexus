...
${formatHelpExamples([
  ['nexusclaw agent --to +15555550123 --message "status update"', "Start a new session."],
  ['nexusclaw agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'nexusclaw agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'nexusclaw agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['nexusclaw agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'nexusclaw agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.nexusclaw.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.nexusclaw.ai/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['nexusclaw agents set-identity --agent main --name "NexusClaw" --emoji "🦞"', "Set name + emoji."],
  ["nexusclaw agents set-identity --agent main --avatar avatars/nexusclaw.png", "Set avatar path."],
  [
    "nexusclaw agents set-identity --workspace ~/.nexusclaw/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "nexusclaw agents set-identity --identity-file ~/.nexusclaw/workspace/IDENTITY.md --agent main",
