...
export function registerMigrateCommand(program: Command) {
  const migrate = program
    ...
    .option("--no-backup", "Skip the pre-migration FirstNexus backup")
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["FirstNexus migrate list", "Show available migration providers."],
          ["FirstNexus migrate hermes", "Preview Hermes migration, then prompt before applying."],
          ["FirstNexus migrate hermes --dry-run", "Preview Hermes migration only."],
          [
  ...
  addMigrationOptions(
    migrate
      ...
      .description("Preview a migration without changing FirstNexus state"),
  ).action(async (provider, opts) => {
    await runCommandWithRuntime(defaultRuntime, async () => {
