...
const shouldRegisterPrimaryOnly = (argv: string[]) => {
  if (isTruthyEnvValue(process.env.NEXUSCLAW_DISABLE_LAZY_SUBCOMMANDS)) {
    return false;
  }
...
const shouldEagerRegisterSubcommands = (_argv: string[]) => {
  return isTruthyEnvValue(process.env.NEXUSCLAW_DISABLE_LAZY_SUBCOMMANDS);
};

export const loadValidatedConfigForPluginRegistration =
  async (): Promise<NexusClawConfig | null> => {
...
const entries: SubCliEntry[] = [
  ...
  {
    name: "docs",
    description: "Search the live NexusClaw docs",
    hasSubcommands: false,
    register: async (program) => {
