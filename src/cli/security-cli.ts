...
export function registerSecurityCli(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["nexusclaw security audit", "Run a local security audit."],
          ["nexusclaw security audit --deep", "Include best-effort live Gateway probe checks."],
          ["nexusclaw security audit --deep --token <token>", "Use explicit token for deep probe."],
          [
            "nexusclaw security audit --deep --password <password>",
            "Use explicit password for deep probe.",
          ],
          ["nexusclaw security audit --fix", "Apply safe remediations and file-permission fixes."],
          ["nexusclaw security audit --json", "Output machine-readable JSON."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/security", "docs.nexusclaw.ai/cli/security")}\n`,
    );
  ...
  security
    ...
    .action(async (opts: SecurityAuditOptions) => {
      ...
      const lines: string[] = [];
      lines.push(heading("NexusClaw security audit"));
      lines.push(muted(`Summary: ${formatSummary(report.summary)}`));
      lines.push(muted(`Run deeper: ${formatCliCommand("nexusclaw security audit --deep")}`));
      for (const diagnostic of secretDiagnostics) {
        lines.push(muted(`[secrets] ${diagnostic}`));
      }

      if (opts.fix) {
        lines.push(muted(`Fix: ${formatCliCommand("nexusclaw security audit --fix")}`));
        if (!fixResult) {
