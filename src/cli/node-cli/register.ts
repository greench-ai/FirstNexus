    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          [
            "nexusclaw node run --host 127.0.0.1 --port 18789",
            "Run the node host in the foreground.",
          ],
          ["nexusclaw node status", "Check node host service status."],
          ["nexusclaw node install", "Install the node host service."],
          ["nexusclaw node restart", "Restart the installed node host service."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/node", "docs.nexusclaw.ai/cli/node")}\n`,
    );
