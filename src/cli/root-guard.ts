 ...
 * - Separate state dir (/root/.FirstNexus/ vs /home/<user>/.FirstNexus/)
...
export function assertNotRoot(env: NodeJS.ProcessEnv = process.env): void {
  ...
  process.stderr.write(
    "[FirstNexus] Refusing to run as root.\n" +
      "\n" +
      "Why this is blocked:\n" +
      "  - A separate state directory under /root/.FirstNexus/ instead of the service user's\n" +
      "  - Conflicting systemd user services that race on port 18789\n" +
      "  - Root-owned files in the service user's state dir (EACCES errors)\n" +
      "\n" +
      "What to do:\n" +
      "  - Re-run as the service user: sudo -u <service-user> -H FirstNexus ...\n" +
      "  - Or switch shells first: su - <service-user>\n" +
      "\n" +
      "Intentional container/CI run only:\n" +
      "  NEXISCLAW_ALLOW_ROOT=1 FirstNexus ...\n",
  );
  process.exit(1);
}
