...
function formatPluginLine(plugin: PluginRecord, verbose = false): string {
  ...
  const format = plugin.format ?? "nexusclaw";
...
function applySlotSelectionForPlugin(
  ...
  pluginId: string,
): { config: NexusClawConfig; warnings: string[] } {
...
async function runPluginInstallCommand(params: {
  ...
  if (fs.existsSync(resolved)) {
    if (opts.link) {
      ...
      let next: NexusClawConfig = enablePluginInConfig(
        {
          ...cfg,
          plugins: {
            ...cfg.plugins,
...
export function registerPluginsCli(program: Command) {
  const plugins = program
    .command("plugins")
    .description("Manage NexusClaw plugins and extensions")
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/plugins", "docs.nexusclaw.ai/cli/plugins")}\n`,
    );
      ...
      if (!opts.verbose) {
        ...
        const rows = list.map((plugin) => {
          ...
          return {
            ...
            Format: plugin.format ?? "nexusclaw",
            Status:
              plugin.status === "loaded"
                ? theme.success("loaded")
                : plugin.status === "disabled"
  ...
  plugins
    ...
    .action((id: string | undefined, opts: PluginInspectOptions) => {
      ...
      lines.push(`${theme.muted("Format:")} ${inspect.plugin.format ?? "nexusclaw"}`);
  ...
  plugins
    ...
    .action(async (id: string) => {
      ...
      let next: NexusClawConfig = enableResult.config;
  ...
  plugins
    ...
    .action(() => {
      ...
      const docs = formatDocsLink("/plugin", "docs.nexusclaw.ai/plugin");
      lines.push("");
