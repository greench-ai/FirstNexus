...
export function registerPluginsCli(program: Command) {
  const plugins = program
    .command("plugins")
    .description("Manage FirstNexus plugins and extensions")
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/plugins", "docs.FirstNexus.ai/cli/plugins")}\n`,
    );
  ...
  plugins
    ...
    .action(async (id: string) => {
      ...
      const cfg = (snapshot.sourceConfig ?? snapshot.config) as FirstNexusConfig;
      ...
      let next: FirstNexusConfig = enableResult.config;
  ...
  plugins
    ...
    .action(async (opts: PluginRegistryOptions) => {
      ...
      if (inspection.refreshReasons.length > 0) {
        ...
        lines.push(
          `${theme.muted("Repair:")} ${theme.command("FirstNexus plugins registry --refresh")}`,
        );
      }
    ...
    .action(async () => {
      ...
      if (shadowed.length > 0) {
        ...
        for (const diag of shadowed) {
          ...
          lines.push("  repair:");
          lines.push("    FirstNexus plugins inspect " + (diag.pluginId ?? "<plugin-id>"));
          lines.push("    edit or remove the config-selected plugin source");
          lines.push("    FirstNexus plugins registry --refresh");
          lines.push("    FirstNexus gateway restart --force");
        }
      }
      ...
      const docs = formatDocsLink("/plugin", "docs.FirstNexus.ai/plugin");
      lines.push("");
