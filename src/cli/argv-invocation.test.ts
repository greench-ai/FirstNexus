...
describe("argv-invocation", () => {
  it("resolves root help and empty command path", () => {
    expect(resolveCliArgvInvocation(["node", "FirstNexus", "--help"])).toEqual({
  ...
  it("resolves command path and primary with root options", () => {
    expect(
      resolveCliArgvInvocation(["node", "FirstNexus", "--profile", "work", "gateway", "status"]),
    ).toEqual({
      argv: ["node", "FirstNexus", "--profile", "work", "gateway", "status"],
