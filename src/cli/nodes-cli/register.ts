    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["nexusclaw nodes status", "List known nodes with live status."],
          ["nexusclaw nodes pairing pending", "Show pending node pairing requests."],
          ['nexusclaw nodes run --node <id> --raw "uname -a"', "Run a shell command on a node."],
          ["nexusclaw nodes camera snap --node <id>", "Capture a photo from a node camera."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/nodes", "docs.nexusclaw.ai/cli/nodes")}\n`,
    );
