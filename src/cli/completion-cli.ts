...
function isCompletionProfileHeader(line: string): boolean {
  return line.trim() === "# NexusClaw Completion";
}
...
function isSlowDynamicCompletionLine(line: string, binName: string): boolean {
  // Matches patterns like: source <(nexusclaw completion --shell zsh)
  return (
...
function updateCompletionProfile(
  ...
  const block = `# NexusClaw Completion\n${sourceLine}`;
 ...
 * Returns true if profile has `source <(nexusclaw completion ...)` instead of cached file.
 */
...
export function registerCompletionCli(program: Command) {
  program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/completion", "docs.nexusclaw.ai/cli/completion")}\n`,
    )
    .addOption(
    ...
    .option(
      "--write-state",
      "Write completion scripts to $NEXUSCLAW_STATE_DIR/completions (no stdout)",
    )
...
export async function installCompletion(shell: string, yes: boolean, binName = "nexusclaw") {
  const home = process.env.HOME || os.homedir();
  let profilePath = "";
  let sourceLine = "";
