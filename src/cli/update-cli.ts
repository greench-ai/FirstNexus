...
export function registerUpdateCli(program: Command) {
  ...
  const update = program
    .command("update")
    .description("Update FirstNexus and inspect update channel status")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["FirstNexus update", "Update a source checkout (git)"],
        ["FirstNexus update --channel beta", "Switch to beta channel (git + npm)"],
        ["FirstNexus update --channel dev", "Switch to dev channel (git + npm)"],
        ["FirstNexus update --tag beta", "One-off update to a dist-tag or version"],
        ["FirstNexus update --tag main", "One-off package install from GitHub main"],
        ["FirstNexus update --dry-run", "Preview actions without changing anything"],
        ["FirstNexus update --no-restart", "Update without restarting the service"],
        ["FirstNexus update --json", "Output result as JSON"],
        ["FirstNexus update --yes", "Non-interactive (accept downgrade prompts)"],
        ["FirstNexus update wizard", "Interactive update wizard"],
        ["FirstNexus --update", "Shorthand for FirstNexus update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run FirstNexus update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.FirstNexus.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["FirstNexus update status", "Show channel + version status."],
          ["FirstNexus update status --json", "JSON output."],
          ["FirstNexus update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.FirstNexus.ai/cli/update")}`,
    )
    .action(async (opts, command) => {
      try {
