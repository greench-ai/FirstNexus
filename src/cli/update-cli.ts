...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update NexusClaw and inspect update channel status")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["nexusclaw update", "Update a source checkout (git)"],
        ["nexusclaw update --channel beta", "Switch to beta channel (git + npm)"],
        ["nexusclaw update --channel dev", "Switch to dev channel (git + npm)"],
        ["nexusclaw update --tag beta", "One-off update to a dist-tag or version"],
        ["nexusclaw update --tag main", "One-off package install from GitHub main"],
        ["nexusclaw update --dry-run", "Preview actions without changing anything"],
        ["nexusclaw update --no-restart", "Update without restarting the service"],
        ["nexusclaw update --json", "Output result as JSON"],
        ["nexusclaw update --yes", "Non-interactive (accept downgrade prompts)"],
        ["nexusclaw update wizard", "Interactive update wizard"],
        ["nexusclaw --update", "Shorthand for nexusclaw update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run nexusclaw update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.nexusclaw.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["nexusclaw update status", "Show channel + version status."],
          ["nexusclaw update status --json", "JSON output."],
          ["nexusclaw update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.nexusclaw.ai/cli/update")}`,
    )
    .action(async (opts, command) => {
      try {
