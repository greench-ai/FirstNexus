    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["nexusclaw memory status", "Show index and provider status."],
          ["nexusclaw memory status --deep", "Probe embedding provider readiness."],
          ["nexusclaw memory index --force", "Force a full reindex."],
          ['nexusclaw memory search "meeting notes"', "Quick search using positional query."],
          [
            'nexusclaw memory search --query "deployment" --max-results 20',
            "Limit results for focused troubleshooting.",
          ],
          ["nexusclaw memory status --json", "Output machine-readable JSON (good for scripts)."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/memory", "docs.nexusclaw.ai/cli/memory")}\n`,
    );
