...
export function registerDnsCli(program: Command) {
  const dns = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/dns", "docs.FirstNexus.ai/cli/dns")}\n`,
    );
  ...
  dns
    ...
    .option("--domain <domain>", "Wide-area discovery domain (e.g. FirstNexus.internal)")
    .option(
      "--apply",
    ...
    .action(async (opts) => {
      ...
      defaultRuntime.log(
        theme.heading(
          "Recommended config ($NEXISCLAW_CONFIG_PATH, default ~/.FirstNexus/FirstNexus.json):",
        ),
      );
      ...
      if (zoneFileNeedsBootstrap(zonePath)) {
        ...
        const zoneLines = [
          `; created by FirstNexus dns setup (will be overwritten by the gateway when wide-area discovery is enabled)`,
      ...
      if (cfg.discovery?.wideArea?.enabled !== true) {
        ...
        defaultRuntime.log(
          theme.muted(
            "Note: enable discovery.wideArea.enabled in the active FirstNexus config ($NEXISCLAW_CONFIG_PATH, default ~/.FirstNexus/FirstNexus.json) on the gateway and restart the gateway so it writes the DNS-SD zone.",
          ),
        );
      }
    });
