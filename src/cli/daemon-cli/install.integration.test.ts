...
describe("runDaemonInstall integration", () => {
  ...
  beforeAll(async () => {
    ...
    tempHome = await makeTempWorkspace("nexusclaw-daemon-install-int-");
    configPath = path.join(tempHome, "nexusclaw.json");
    process.env.HOME = tempHome;
    process.env.NEXUSCLAW_STATE_DIR = tempHome;
  ...
  it("auto-mints token when no source exists without embedding it into service env", async () => {
    ...
    expect(installEnv?.NEXUSCLAW_GATEWAY_TOKEN).toBeUndefined();
  });
});
