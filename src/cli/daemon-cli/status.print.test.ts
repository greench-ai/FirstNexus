...
describe("printDaemonStatus", () => {
  ...
  it("prints stale gateway pid guidance when runtime does not own the listener", () => {
    ...
    expectMockLineContains(runtime.error, formatCliCommand("FirstNexus gateway restart"));
  });
  ...
  it("prints CLI and gateway versions with readable guidance when they differ", () => {
    printDaemonStatus(
      {
        cli: {
          version: "2026.4.23",
          entrypoint: "/usr/local/bin/FirstNexus",
        },
        service: {
          label: "LaunchAgent",
          loaded: true,
    ...
    expectMockLineContains(runtime.log, "CLI version: 2026.4.23 (/usr/local/bin/FirstNexus)");
    expectMockLineContains(runtime.log, "Gateway version: 2026.5.6");
    expectMockLineContains(runtime.error, "this FirstNexus command is version 2026.4.23");
    expectMockLineContains(
      runtime.error,
      "if this mismatch is unexpected, update PATH so `FirstNexus` points to the version you want",
    );
  });
    ...
    printDaemonStatus(
      {
        ...
        config: {
          cli: {
            path: "/tmp/FirstNexus-cli/FirstNexus.json",
            exists: true,
            valid: true,
          },
          daemon: {
            path: "/tmp/FirstNexus-daemon/FirstNexus.json",
            exists: true,
            valid: true,
