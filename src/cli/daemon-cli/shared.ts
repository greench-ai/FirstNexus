...
export function renderRuntimeHints(
  ...
  if (runtime.missingUnit) {
    hints.push(`Service not installed. Run: ${formatCliCommand("nexusclaw gateway install", env)}`);
    if (fileLog) {
  ...
  if (runtime.status === "stopped") {
    ...
    hints.push(
      ...buildPlatformRuntimeLogHints({
        env,
        systemdServiceName: resolveGatewaySystemdServiceName(env.NEXUSCLAW_PROFILE),
        windowsTaskName: resolveGatewayWindowsTaskName(env.NEXUSCLAW_PROFILE),
      }),
    );
  }
  return hints;
...
export function renderGatewayServiceStartHints(env: NodeJS.ProcessEnv = process.env): string[] {
  ...
  return buildPlatformServiceStartHints({
    installCommand: formatCliCommand("nexusclaw gateway install", env),
    startCommand: formatCliCommand("nexusclaw gateway", env),
