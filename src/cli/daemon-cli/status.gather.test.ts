...
>(async (_env?: NodeJS.ProcessEnv) => ({
  ...
  environment: {
    NEXUSCLAW_STATE_DIR: "/tmp/nexusclaw-daemon",
    NEXUSCLAW_CONFIG_PATH: "/tmp/nexusclaw-daemon/nexusclaw.json",
  },
}));
...
const resolveStateDir = vi.fn(
  (env: NodeJS.ProcessEnv) => env.NEXUSCLAW_STATE_DIR ?? "/tmp/nexusclaw-cli",
);
const resolveConfigPath = vi.fn((env: NodeJS.ProcessEnv, stateDir: string) => {
  return env.NEXUSCLAW_CONFIG_PATH ?? `${stateDir}/nexusclaw.json`;
});
...
vi.mock("../../config/config.js", () => ({
  createConfigIO: ({ configPath }: { configPath: string }) => {
    const isDaemon = configPath.includes("/nexusclaw-daemon/");
    return {
...
describe("gatherDaemonStatus", () => {
  ...
  beforeEach(() => {
    ...
    process.env.NEXUSCLAW_STATE_DIR = "/tmp/nexusclaw-cli";
    process.env.NEXUSCLAW_CONFIG_PATH = "/tmp/nexusclaw-cli/nexusclaw.json";
  ...
  it("reuses command environment when reading runtime status", async () => {
    ...
    serviceReadRuntime.mockImplementationOnce(async (env?: NodeJS.ProcessEnv) => ({
      status: env?.NEXUSCLAW_GATEWAY_PORT === "19001" ? "running" : "unknown",
      detail: env?.NEXUSCLAW_GATEWAY_PORT ?? "missing-port",
    }));
  ...
  it("keeps remote probe auth strict when remote token is missing", async () => {
    ...
    process.env.NEXUSCLAW_GATEWAY_PASSWORD = "env-password"; // pragma: allowlist secret
  ...
  it("surfaces stale gateway listener pids from restart health inspection", async () => {
    inspectGatewayRestart.mockResolvedValueOnce({
      ...
      portUsage: {
        port: 19001,
        status: "busy",
        listeners: [{ pid: 9000, ppid: 8999, commandLine: "nexusclaw-gateway" }],
        hints: [],
      },
      healthy: false,
