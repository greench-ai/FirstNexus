...
const mocks = vi.hoisted(() => {
  ...
  let configState: FirstNexusConfig = {
    tools: {
      exec: {
        host: "auto",
        security: "allowlist",
  ...
  return {
    ...
    setConfig: (next: FirstNexusConfig) => {
      configState = next;
    },
    ...
    defaultRuntime,
    runtimeErrors,
    mutateConfigFile: vi.fn(async ({ mutate }: { mutate: (draft: FirstNexusConfig) => void }) => {
      ...
      return {
        ...
        previousHash: "hash-1",
        snapshot: { path: "/tmp/FirstNexus.json" },
        nextConfig: draft,
        result: undefined,
      };
    }),
    replaceConfigFile: vi.fn(
      async ({ nextConfig }: { nextConfig: FirstNexusConfig; baseHash?: string }) => {
    ...
    readConfigFileSnapshot: vi.fn<
      () => Promise<{ path: string; hash: string; config: FirstNexusConfig }>
    >(async () => ({
      path: "/tmp/FirstNexus.json",
      hash: "config-hash-1",
      config: configState,
...
describe("exec-policy CLI", () => {
  ...
  beforeEach(() => {
    ...
    mocks.mutateConfigFile.mockImplementation(
      async ({ mutate }: { mutate: (draft: FirstNexusConfig) => void }) => {
  ...
  it("sanitizes terminal control content before rendering the text table", async () => {
    ...
    expect(output).toContain("/tmp/FirstNexus.json");
    ...
    expect(output).not.toContain("/tmp/FirstNexus.json\nforged");
