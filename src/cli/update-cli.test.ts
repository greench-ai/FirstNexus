...
import type { NexusClawConfig, ConfigFileSnapshot } from "../config/types.nexusclaw.js";
...
vi.mock("../infra/nexusclaw-root.js", () => ({
  resolveNexusClawPackageRoot: vi.fn(),
}));
...
const { resolveNexusClawPackageRoot } = await import("../infra/nexusclaw-root.js");
...
describe("update-cli", () => {
  const fixtureRoot = "/tmp/nexusclaw-update-tests";
  let fixtureCount = 0;
  ...
  const baseConfig = {} as NexusClawConfig;
  const baseSnapshot: ConfigFileSnapshot = {
    path: "/tmp/nexusclaw-config.json",
    exists: true,
    raw: "{}",
    parsed: {},
    resolved: baseConfig,
  ...
  const mockPackageInstallStatus = (root: string) => {
    vi.mocked(resolveNexusClawPackageRoot).mockResolvedValue(root);
  ...
  const setupNonInteractiveDowngrade = async () => {
    const tempDir = createCaseDir("nexusclaw-update");
    setTty(false);
  ...
  const setupUpdatedRootRefresh = (params?: {
    ...
    const root = createCaseDir("nexusclaw-updated-root");
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    vi.mocked(resolveNexusClawPackageRoot).mockResolvedValue(process.cwd());
    ...
    readPackageName.mockResolvedValue("nexusclaw");
    ...
    prepareRestartScript.mockResolvedValue("/tmp/nexusclaw-restart-test.sh");
    runRestartScript.mockResolvedValue(undefined);
    inspectPortUsage.mockResolvedValue({
      port: 18789,
      status: "busy",
      listeners: [{ pid: 4242, command: "nexusclaw-gateway" }],
      hints: [],
    });
    ...
    const cases = [
      {
        ...
        assert: () => {
          ...
          expect(logs.join("\n")).toContain("NexusClaw update status");
        },
      },
      {
        name: "json output",
  ...
  it.each([
    ...
    },
    {
      ...
      prepare: async () => {
        vi.mocked(readConfigFileSnapshot).mockResolvedValue({
          ...baseSnapshot,
          config: { update: { channel: "beta" } } as NexusClawConfig,
        });
      },
    ...
    async ({ mode, options, prepare, expectedChannel, expectedTag, expectedPersistedChannel }) => {
      ...
      if (expectedChannel !== undefined) {
        ...
        expect(runGatewayUpdate).not.toHaveBeenCalled();
        expect(runCommandWithTimeout).toHaveBeenCalledWith(
          ["npm", "i", "-g", "nexusclaw@latest", "--no-fund", "--no-audit", "--loglevel=error"],
          expect.any(Object),
        );
      }
    ...
    for (const scenario of [
      {
        ...
        run: async () => {
          mockPackageInstallStatus(createCaseDir("nexusclaw-update"));
          await updateCommand({ tag: "next" });
        },
        expectedSpec: "nexusclaw@next",
      },
      {
        name: "main shorthand",
        run: async () => {
        ...
        expectedSpec: "github:nexusclaw/nexusclaw#main",
      },
      {
        name: "explicit git package spec",
        run: async () => {
          mockPackageInstallStatus(createCaseDir("nexusclaw-update"));
          await updateCommand({ yes: true, tag: "github:nexusclaw/nexusclaw#main" });
        },
        expectedSpec: "github:nexusclaw/nexusclaw#main",
      },
      {
        name: "NEXUSCLAW_UPDATE_PACKAGE_SPEC override",
        run: async () => {
          mockPackageInstallStatus(createCaseDir("nexusclaw-update"));
          await withEnvAsync(
        ...
        expectedSpec: "http://10.211.55.2:8138/nexusclaw-next.tgz",
      },
    ]) {
      vi.clearAllMocks();
      readPackageName.mockResolvedValue("nexusclaw");
  ...
  it("prepends portable Git PATH for package updates on Windows", async () => {
    ...
    const tempDir = createCaseDir("nexusclaw-update");
    const localAppData = createCaseDir("nexusclaw-localappdata");
        ...
        await withEnvAsync(
          {
            ...
            NEXUSCLAW_CONFIG_PATH: "./config/nexusclaw.json",
          },
          async () => {
            await updateCommand({});
          },
      ...
      expectedOptions: (root: string) =>
        expect.objectContaining({
          ...
          env: expect.objectContaining({
            NEXUSCLAW_STATE_DIR: path.resolve("./state"),
            NEXUSCLAW_CONFIG_PATH: path.resolve("./config/nexusclaw.json"),
          }),
          timeoutMs: 60_000,
        }),
      assertExtra: () => {
    ...
    },
    {
      ...
      expectedOptions: (_root: string, context?: { originalCwd: string }) =>
        expect.objectContaining({
          ...
          env: expect.objectContaining({
            NEXUSCLAW_STATE_DIR: path.resolve(context?.originalCwd ?? process.cwd(), "./state"),
          }),
          timeoutMs: 60_000,
        }),
      assertExtra: () => {
  ...
  it("updateCommand continues after doctor sub-step and clears update flag", async () => {
    ...
    try {
      await withEnvAsync({ NEXUSCLAW_UPDATE_IN_PROGRESS: undefined }, async () => {
        vi.mocked(runGatewayUpdate).mockResolvedValue(makeOkUpdateResult());
        vi.mocked(runDaemonRestart).mockResolvedValue(true);
        vi.mocked(doctorCommand).mockResolvedValue(undefined);
        vi.mocked(defaultRuntime.log).mockClear();
        ...
        expect(process.env.NEXUSCLAW_UPDATE_IN_PROGRESS).toBeUndefined();
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = createCaseDir("nexusclaw-update-wizard");
    await withEnvAsync({ NEXUSCLAW_GIT_DIR: tempDir }, async () => {
      setTty(true);
