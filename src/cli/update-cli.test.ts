...
import type { FirstNexusConfig, ConfigFileSnapshot } from "../config/types.FirstNexus.js";
...
const loadInstalledPluginIndexInstallRecords = vi.fn(
  async (params: { config?: FirstNexusConfig } = {}) => params.config?.plugins?.installs ?? {},
);
...
vi.mock("../infra/FirstNexus-root.js", () => ({
  ...
  resolveFirstNexusPackageRootSync: vi.fn(() => process.cwd()),
}));
    ...
    if (process.env.NEXISCLAW_NIX_MODE === "1") {
      throw new Error(
        [
          ...
          "FirstNexus Nix overview: https://docs.FirstNexus.ai/install/nix",
        ].join("\n"),
      );
    }
  },
...
vi.mock("../utils.js", async (importOriginal) => {
  ...
  return {
    ...
    resolveConfigDir: () => "/tmp/FirstNexus-config",
  };
});
...
describe("update-cli", () => {
  const fixtureRoot = "/tmp/FirstNexus-update-tests";
  let fixtureCount = 0;
  ...
  const baseConfig = {} as FirstNexusConfig;
  const baseSnapshot: ConfigFileSnapshot = {
    path: "/tmp/FirstNexus-config.json",
    exists: true,
    raw: "{}",
    parsed: {},
    resolved: baseConfig,
  ...
  const mockPackageInstallStatus = (root: string) => {
    vi.mocked(resolveFirstNexusPackageRoot).mockResolvedValue(root);
  ...
  const syncPluginCall = (index = 0) => {
    const calls = syncPluginsForUpdateChannel.mock.calls as unknown as Array<
      [{ channel?: string; config?: FirstNexusConfig }]
    >;
  ...
  const npmPluginUpdateCall = (index = 0) => {
    const calls = updateNpmInstalledPlugins.mock.calls as unknown as Array<
      [{ config?: FirstNexusConfig; timeoutMs?: number }]
    >;
    return calls[index]?.[0];
  };
  ...
  const setupNonInteractiveDowngrade = async () => {
    const tempDir = createCaseDir("FirstNexus-update");
    setTty(false);
  ...
  const setupUpdatedRootRefresh = (params?: {
    ...
    const root = createCaseDir("FirstNexus-updated-root");
  ...
  beforeEach(() => {
    ...
    vi.mocked(resolveFirstNexusPackageRoot).mockResolvedValue(process.cwd());
    ...
    readPackageName.mockResolvedValue("FirstNexus");
    ...
    serviceReadCommand.mockImplementation(async () =>
      (await serviceLoaded()) ? { programArguments: ["FirstNexus", "gateway", "run"] } : null,
    );
    ...
    prepareRestartScript.mockResolvedValue("/tmp/FirstNexus-restart-test.sh");
    runRestartScript.mockResolvedValue(undefined);
    inspectPortUsage.mockResolvedValue({
      port: 18789,
      status: "busy",
      listeners: [{ pid: 4242, command: "FirstNexus-gateway" }],
      hints: [],
    });
  ...
  it("bounds completion cache refresh during update follow-up", async () => {
    const root = createCaseDir("FirstNexus-completion-timeout");
    ...
    expect(call?.[1]).toEqual([path.join(root, "FirstNexus.mjs"), "completion", "--write-state"]);
    expect(call?.[2]?.env?.NEXISCLAW_COMPLETION_SKIP_PLUGIN_COMMANDS).toBe("1");
    expect(call?.[2]?.timeout).toBe(30_000);
  });

  it("refuses mutating updates in Nix mode before update side effects", async () => {
    await withEnvAsync({ NEXISCLAW_NIX_MODE: "1" }, async () => {
      await expect(updateCommand({ yes: true })).rejects.toThrow("NEXISCLAW_NIX_MODE=1");
    });
  ...
  it("logs friendly hint with manual refresh command when completion cache write times out", async () => {
    const root = createCaseDir("FirstNexus-completion-timeout-msg");
    pathExists.mockResolvedValue(true);
    ...
    expect(logs.some((line) => line.includes("FirstNexus completion --write-state"))).toBe(true);
  ...
  it("respawns into the updated package root before running post-update tasks", async () => {
    ...
    expect(call?.[2]?.env?.NEXISCLAW_UPDATE_POST_CORE).toBe("1");
    expect(call?.[2]?.env?.NEXISCLAW_UPDATE_POST_CORE_CHANNEL).toBe("dev");
    expect(updateNpmInstalledPlugins).not.toHaveBeenCalled();
  ...
  it("finishes package updates when the post-core process writes a result but keeps handles open", async () => {
    ...
    spawn.mockImplementationOnce((_command: unknown, _argv: unknown, options: unknown) => {
      const resultPath = (options as { env?: NodeJS.ProcessEnv }).env
        ?.NEXISCLAW_UPDATE_POST_CORE_RESULT_PATH;
      if (!resultPath) {
  ...
  it("does not restart a stopped managed gateway after post-core plugin errors", async () => {
    const root = createCaseDir("FirstNexus-update");
            ...
            warnings: [
              {
                ...
                guidance: ["Run FirstNexus doctor --fix to attempt automatic repair."],
              },
            ],
            sync: {
              changed: false,
  ...
  it("does not carry gateway service markers into the post-core update process", async () => {
    ...
    expect(spawnEnv?.NEXISCLAW_SERVICE_MARKER).toBeUndefined();
    expect(spawnEnv?.NEXISCLAW_SERVICE_KIND).toBeUndefined();
  });
  ...
  it("passes pre-update plugin install records into the post-core update process", async () => {
    ...
    const pluginInstallRecords = {
      demo: {
        source: "npm",
        spec: "@FirstNexus/demo@1.0.0",
        installPath: "/tmp/FirstNexus-demo-plugin",
      },
    } as const;
    ...
    spawn.mockImplementationOnce((_node, _argv, options) => {
      ...
      const recordsPath = env?.NEXISCLAW_UPDATE_POST_CORE_INSTALL_RECORDS_PATH;
      if (!recordsPath) {
  ...
  it("respawns into the updated git root before requested channel persistence", async () => {
    ...
    expect(call?.[2]?.env?.NEXISCLAW_UPDATE_POST_CORE).toBe("1");
    expect(call?.[2]?.env?.NEXISCLAW_UPDATE_POST_CORE_CHANNEL).toBe("dev");
    expect(call?.[2]?.env?.NEXISCLAW_UPDATE_POST_CORE_REQUESTED_CHANNEL).toBe("dev");
    expect(replaceConfigFile).not.toHaveBeenCalled();
    expect(syncPluginsForUpdateChannel).not.toHaveBeenCalled();
    expect(updateNpmInstalledPlugins).not.toHaveBeenCalled();
  });
  ...
  it("keeps downgrade post-update work in the current process", async () => {
    const downgradedRoot = createCaseDir("FirstNexus-downgraded-root");
    setupUpdatedRootRefresh({
  ...
  it("post-core resume mode skips the core update and only runs post-update tasks", async () => {
    await withEnvAsync(
      {
        NEXISCLAW_UPDATE_POST_CORE: "1",
        NEXISCLAW_UPDATE_POST_CORE_CHANNEL: "stable",
      },
      async () => {
  ...
  it("post-core resume children exit after writing a plugin update result", async () => {
    const resultDir = createCaseDir("FirstNexus-post-core-result");
    ...
    await withEnvAsync(
      {
        NEXISCLAW_UPDATE_POST_CORE: "1",
        NEXISCLAW_UPDATE_POST_CORE_CHANNEL: "stable",
        NEXISCLAW_UPDATE_POST_CORE_RESULT_PATH: resultPath,
      },
      async () => {
        await updateCommand({ restart: false });
      },
  ...
  it("post-core resume mode uses the parent install records snapshot for missing payload warnings", async () => {
    const resultDir = createCaseDir("FirstNexus-post-core-records");
    ...
    await withEnvAsync(
      {
        NEXISCLAW_UPDATE_POST_CORE: "1",
        NEXISCLAW_UPDATE_POST_CORE_CHANNEL: "stable",
        NEXISCLAW_UPDATE_POST_CORE_INSTALL_RECORDS_PATH: recordsPath,
      },
      async () => {
  ...
  it("post-core resume mode persists the requested update channel with the updated process", async () => {
    vi.mocked(readConfigFileSnapshot).mockResolvedValue({
      ...
      resolved: { update: { channel: "stable" } } as FirstNexusConfig,
      sourceConfig: { update: { channel: "stable" } } as FirstNexusConfig,
      runtimeConfig: { update: { channel: "stable" } } as FirstNexusConfig,
      config: { update: { channel: "stable" } } as FirstNexusConfig,
      hash: "stable-hash",
    });
    ...
    await withEnvAsync(
      {
        NEXISCLAW_UPDATE_POST_CORE: "1",
        NEXISCLAW_UPDATE_POST_CORE_CHANNEL: "dev",
        NEXISCLAW_UPDATE_POST_CORE_REQUESTED_CHANNEL: "dev",
      },
      async () => {
        await updateCommand({ restart: false });
      },
  ...
  it("uses a fail-closed integrity policy for post-core plugin updates", async () => {
    ...
    await expect(
      onIntegrityDrift({
        pluginId: "demo",
        spec: "@FirstNexus/demo@1.0.0",
        resolvedSpec: "@FirstNexus/demo@1.0.0",
  ...
  it("keeps json update output successful when post-core plugin updates warn", async () => {
          ...
          outcomes: [
            {
              ...
              message:
                proceed === false
                  ? "Failed to update demo: aborted: npm package integrity drift detected for @FirstNexus/demo@1.0.0"
    ...
    expect(pluginWarning(jsonOutput)?.guidance).toEqual([
      ...
      "Run FirstNexus plugins inspect demo --runtime --json for details.",
    ]);
  ...
  it("detects missing plugin payloads from persisted records before npm updates", async () => {
    const installPath = createCaseDir("FirstNexus-missing-plugin-payload");
  ...
  it("prints non-fatal plugin warnings in human update output", async () => {
    ...
    expect(logs).toContain("Run FirstNexus doctor --fix to attempt automatic repair.");
    expect(logs).toContain("Run FirstNexus plugins inspect demo --runtime --json for details.");
  });
  ...
  it("preserves fresh-process plugin warning details in parent json output", async () => {
    ...
    spawn.mockImplementationOnce((_node, _argv, options) => {
      ...
      queueMicrotask(async () => {
        const resultPath = env?.NEXISCLAW_UPDATE_POST_CORE_RESULT_PATH;
        if (resultPath) {
          await fs.writeFile(
            resultPath,
            JSON.stringify({
              ...
              warnings: [
                {
                  ...
                  message:
                    'Plugin "demo" could not be processed after the core update: Failed to update demo: registry timeout Run FirstNexus doctor --fix to attempt automatic repair. Run FirstNexus plugins inspect demo --runtime --json for details.',
                  guidance: [
                    "Run FirstNexus doctor --fix to attempt automatic repair.",
                    "Run FirstNexus plugins inspect demo --runtime --json for details.",
                  ],
  ...
  it.each([
    {
      ...
      assert: () => {
        ...
        expect(logs.join("\n")).toContain("FirstNexus update status");
      },
    },
    {
      name: "json output",
  ...
  it("parses update status --json as the subcommand option", async () => {
    ...
    await program.parseAsync(["node", "FirstNexus", "update", "status", "--json"]);
  ...
  it.each([
    ...
    },
    {
      ...
      prepare: async () => {
        vi.mocked(readConfigFileSnapshot).mockResolvedValue({
          ...baseSnapshot,
          config: { update: { channel: "beta" } } as FirstNexusConfig,
        });
      },
    ...
    async ({ mode, options, prepare, expectedChannel, expectedTag, expectedPersistedChannel }) => {
      ...
      if (expectedChannel !== undefined) {
        ...
        expectPackageInstallSpec("FirstNexus@latest");
      }
  ...
  it("allows package updates from inherited gateway service env when the managed gateway is not running", async () => {
    mockPackageInstallStatus(createCaseDir("FirstNexus-update"));
    ...
    expect(defaultRuntime.error).not.toHaveBeenCalledWith(
      [
        ...
        "Run `FirstNexus update` from a shell outside the gateway service, or stop the gateway service first and then update.",
      ].join("\n"),
    );
    expectPackageInstallSpec("FirstNexus@latest");
  });
  ...
  it("refuses package updates from inside the active gateway process tree", async () => {
    ...
    expect(errors.join("\n")).toContain(
      "FirstNexus update detected it is running inside the gateway process tree.",
    );
  ...
  it.each([
    ...
    },
    {
      ...
      expectedSpec: "github:FirstNexus/FirstNexus#main",
    },
    {
      name: "explicit git package spec",
      run: async () => {
        mockPackageInstallStatus(createCaseDir("FirstNexus-update"));
        await updateCommand({ yes: true, tag: "github:FirstNexus/FirstNexus#main" });
      },
      expectedSpec: "github:FirstNexus/FirstNexus#main",
    },
    {
      name: "NEXISCLAW_UPDATE_PACKAGE_SPEC override",
      run: async () => {
        mockPackageInstallStatus(createCaseDir("FirstNexus-update"));
        await withEnvAsync(
          { NEXISCLAW_UPDATE_PACKAGE_SPEC: "http://10.211.55.2:8138/FirstNexus-next.tgz" },
          async () => {
            await updateCommand({ yes: true, tag: "latest" });
          },
        );
      },
      expectedSpec: "http://10.211.55.2:8138/FirstNexus-next.tgz",
    },
  ] as const)(
  ...
  it("fails package updates when the installed correction version does not match the requested target", async () => {
    ...
    const pkgRoot = path.join(nodeModules, "FirstNexus");
    mockPackageInstallStatus(tempDir);
    await fs.mkdir(pkgRoot, { recursive: true });
    await fs.writeFile(
      path.join(pkgRoot, "package.json"),
      JSON.stringify({ name: "FirstNexus", version: "2026.3.23" }),
      "utf-8",
    );
  ...
  it("stops package post-update work when staged npm install verification fails", async () => {
    const tempDir = await createTrackedTempDir("FirstNexus-update-staged-fail-");
    ...
    await fs.writeFile(
      path.join(pkgRoot, "package.json"),
      JSON.stringify({ name: "FirstNexus", version: "2026.4.20" }),
      "utf-8",
    );
    ...
    vi.mocked(runCommandWithTimeout).mockImplementation(async (argv) => {
      ...
      if (
        ...
        const stageRoot = path.join(stagePrefix, "lib", "node_modules", "FirstNexus");
        await fs.mkdir(path.join(stageRoot, "dist"), { recursive: true });
        await fs.writeFile(
          path.join(stageRoot, "package.json"),
          JSON.stringify({ name: "FirstNexus", version: "2026.4.25" }),
          "utf-8",
        );
  ...
  it("marks package post-update doctor as update-in-progress", async () => {
    const tempDir = await createTrackedTempDir("FirstNexus-update-package-");
    const nodeModules = path.join(tempDir, "node_modules");
    const pkgRoot = path.join(nodeModules, "FirstNexus");
    ...
    expect(
      (doctorCall?.[1].env as NodeJS.ProcessEnv | undefined)?.NEXISCLAW_UPDATE_IN_PROGRESS,
    ).toBe("1");
  });

  it("stops a running managed gateway before package replacement", async () => {
    const tempDir = await createTrackedTempDir("FirstNexus-update-stop-service-");
    const nodeModules = path.join(tempDir, "node_modules");
    const pkgRoot = path.join(nodeModules, "FirstNexus");
    const entryPath = path.join(pkgRoot, "dist", "index.js");
    mockPackageInstallStatus(pkgRoot);
    ...
    const serviceStopCall = serviceStop.mock.calls.at(0)?.[0] as
      ...
      | undefined;
    expect(serviceStopCall?.env?.NEXISCLAW_SERVICE_MARKER).toBe("FirstNexus");
    expect(serviceStopCall?.env?.NEXISCLAW_SERVICE_KIND).toBe("gateway");
  ...
  it("refreshes package installs even when the current version already matches the target", async () => {
    const tempDir = await createTrackedTempDir("FirstNexus-update-current-");
    const nodeModules = path.join(tempDir, "node_modules");
    const pkgRoot = path.join(nodeModules, "FirstNexus");
    const entryPath = path.join(pkgRoot, "dist", "index.js");
    mockPackageInstallStatus(pkgRoot);
    ...
    await fs.writeFile(
      path.join(pkgRoot, "package.json"),
      JSON.stringify({ name: "FirstNexus", version: "2026.4.23" }),
      "utf-8",
    );
    await fs.writeFile(entryPath, "export {};\n", "utf-8");
    for (const relativePath of TEST_BUNDLED_RUNTIME_SIDECAR_PATHS) {
    ...
    expect(postCoreSpawn?.[2].env?.NEXISCLAW_UPDATE_POST_CORE).toBe("1");
    expect(postCoreSpawn?.[2].env?.NEXISCLAW_UPDATE_POST_CORE_CHANNEL).toBe("stable");
    expect(updateNpmInstalledPlugins).not.toHaveBeenCalled();
    expect(
      vi
        .mocked(defaultRuntime.log)
  ...
  it("retries package updates without optional deps when npm global update fails", async () => {
    const tempDir = await createTrackedTempDir("FirstNexus-update-optional-");
    const nodeModules = path.join(tempDir, "node_modules");
    const pkgRoot = path.join(nodeModules, "FirstNexus");
    mockPackageInstallStatus(pkgRoot);
    await fs.mkdir(pkgRoot, { recursive: true });
    await fs.writeFile(
      path.join(pkgRoot, "package.json"),
      JSON.stringify({ name: "FirstNexus", version: "1.0.0" }),
      "utf-8",
    );
    ...
    const installCall = vi
      ...
      .mock.calls.find(
        ([argv]) =>
          ...
          argv[1] === "i" &&
          argv[2] === "-g" &&
          argv.includes("FirstNexus@latest"),
      );
  ...
  it("prepends portable Git PATH for package updates on Windows", async () => {
    ...
    const tempDir = createCaseDir("FirstNexus-update");
    const localAppData = createCaseDir("FirstNexus-localappdata");
  ...
  it("persists channel and runs post-update work after switching from package to git", async () => {
    const tempDir = createCaseDir("FirstNexus-update");
    const gitRoot = path.join(tempDir, "..", "FirstNexus");
    ...
    const syncCall = syncPluginCall() as
      | { channel?: string; config?: FirstNexusConfig; workspaceDir?: string }
      | undefined;
  ...
  it("explains why git updates cannot run with edited files", async () => {
    ...
    expect(logs.join("\n")).toContain(
      "Commit, stash, or discard the local changes, then rerun `FirstNexus update`.",
    );
    expect(defaultRuntime.exit).toHaveBeenCalledWith(0);
  });
  it.each([
  ...
  it("fails a JSON package update when fallback restart leaves the old gateway running", async () => {
    const updatedRoot = createCaseDir("FirstNexus-updated-root");
  ...
  it.each([
    ...
    },
    {
        ...
        await withEnvAsync(
          {
            ...
            NEXISCLAW_CONFIG_PATH: "./config/FirstNexus.json",
          },
          async () => {
            await updateCommand({});
          },
      ...
      expectedEnv: () => ({
        NEXISCLAW_STATE_DIR: path.resolve("./state"),
        NEXISCLAW_CONFIG_PATH: path.resolve("./config/FirstNexus.json"),
      }),
      assertExtra: () => {
        expect(runDaemonInstall).not.toHaveBeenCalled();
      },
    },
    {
      ...
      expectedEnv: (context?: { originalCwd: string }) => ({
        NEXISCLAW_STATE_DIR: path.resolve(context?.originalCwd ?? process.cwd(), "./state"),
      }),
      assertExtra: () => {
        expect(runDaemonInstall).not.toHaveBeenCalled();
      },
  ...
  it("updateCommand continues after doctor sub-step and clears update flag", async () => {
    ...
    try {
      await withEnvAsync({ NEXISCLAW_UPDATE_IN_PROGRESS: undefined }, async () => {
        vi.mocked(runGatewayUpdate).mockResolvedValue(makeOkUpdateResult());
        vi.mocked(runDaemonRestart).mockResolvedValue(true);
        vi.mocked(doctorCommand).mockResolvedValue(undefined);
        vi.mocked(defaultRuntime.log).mockClear();
        ...
        expect(process.env.NEXISCLAW_UPDATE_IN_PROGRESS).toBeUndefined();
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = createCaseDir("FirstNexus-update-wizard");
    await withEnvAsync({ NEXISCLAW_GIT_DIR: tempDir }, async () => {
      setTty(true);
  ...
  it("uses ~/FirstNexus as the default dev checkout directory", async () => {
    const homedirSpy = vi.spyOn(os, "homedir").mockReturnValue("/tmp/oc-home");
    await withEnvAsync({ NEXISCLAW_GIT_DIR: undefined }, async () => {
      expect(resolveGitInstallDir()).toBe(path.posix.join("/tmp/oc-home", "FirstNexus"));
    });
