...
async function runBrowserToggle(
  ...
  const name = status.profile ?? "nexusclaw";
...
export function registerBrowserManageCommands(
  ...
  browser
    ...
    .action(async (_opts, cmd) => {
      ...
      await runBrowserCommand(async () => {
        ...
        defaultRuntime.log(
          [
            `profile: ${status.profile ?? "nexusclaw"}`,
        ...
        defaultRuntime.log(
          profiles
            .map((p) => {
              ...
              const driver = p.driver !== "nexusclaw" ? ` [${p.driver}]` : "";
  ...
  browser
    ...
    .option("--driver <driver>", "Profile driver (nexusclaw|existing-session). Default: nexusclaw")
    .action(
      async (
        opts: {
          name: string;
