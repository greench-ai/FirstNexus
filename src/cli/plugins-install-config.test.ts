import { bundledPluginRootAt, repoInstallSpec } from "FirstNexus/plugin-sdk/test-fixtures";
import { beforeEach, describe, expect, it, vi } from "vitest";
import type { FirstNexusConfig } from "../config/config.js";
import type { ConfigFileSnapshot } from "../config/types.FirstNexus.js";
import {
...
vi.mock("../commands/doctor/shared/channel-doctor.js", () => ({
  collectChannelDoctorStaleConfigMutations: (cfg: FirstNexusConfig) =>
...
function makeSnapshot(overrides: Partial<ConfigFileSnapshot> = {}): ConfigFileSnapshot {
  return {
    ...
    resolved: { plugins: {} } as FirstNexusConfig,
    valid: false,
    runtimeConfig: { plugins: {} } as ConfigFileSnapshot["runtimeConfig"],
    config: { plugins: {} } as FirstNexusConfig,
    hash: "abc",
...
describe("loadConfigForInstall", () => {
  const discordNpmRequest = {
    rawSpec: "@FirstNexus/discord",
    normalizedSpec: "@FirstNexus/discord",
  ...
  beforeEach(() => {
    ...
    collectChannelDoctorStaleConfigMutationsMock.mockImplementation(
      async (cfg: FirstNexusConfig) => [
        {
          config: cfg,
          changes: [],
        },
  ...
  it("returns the source config and base hash when the snapshot is valid", async () => {
    const cfg = { plugins: { entries: { discord: { enabled: true } } } } as FirstNexusConfig;
  ...
  it("does not run stale Discord cleanup on the happy path", async () => {
    const cfg = { plugins: {} } as FirstNexusConfig;
    readConfigFileSnapshotMock.mockResolvedValue(
      makeSnapshot({
        valid: true,
        sourceConfig: cfg,
  ...
  it("allows npm:-prefixed bundled-plugin reinstall recovery", async () => {
    ...
    const request = resolvePluginInstallRequestContext({
      rawSpec: "npm:@FirstNexus/discord",
    });
    if (!request.ok) {
  ...
  it("allows explicit repo-checkout bundled-plugin reinstall recovery", async () => {
    const snapshotCfg = { plugins: {} } as FirstNexusConfig;
    readConfigFileSnapshotMock.mockResolvedValue(
      makeSnapshot({
        config: snapshotCfg,
  ...
  it("rejects non-Discord install requests when config is invalid", async () => {
    ...
    ).rejects.toThrow("Config invalid; run `FirstNexus doctor --fix` before installing plugins.");
  });
