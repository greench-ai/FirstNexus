...
describe("persistPluginInstall", () => {
  ...
  it("adds installed plugins to restrictive allowlists before enabling", async () => {
    ...
    enablePluginInConfig.mockImplementation((...args: unknown[]) => {
      const [cfg, pluginId] = args as [FirstNexusConfig, string];
  ...
  it("removes a replaced managed install directory before refreshing the registry", async () => {
    ...
    setInstalledPluginIndexInstallRecords({
      codex: {
        source: "clawhub",
        spec: "clawhub:@FirstNexus/codex",
        installPath: "/tmp/FirstNexus/extensions/codex",
      },
    });
    planPluginUninstall.mockReturnValueOnce({
      ...
      directoryRemoval: {
        target: "/tmp/FirstNexus/extensions/codex",
      },
    });
    ...
    await persistPluginInstall({
      ...
      install: {
        ...
        installPath: "/tmp/FirstNexus/npm/node_modules/@FirstNexus/codex",
      },
    });
    ...
    expect(runtimeLogs.join("\n")).toContain(
      "Removed previous plugin install directory: /tmp/FirstNexus/extensions/codex",
    );
  });
  ...
  it("warns when an installed npm plugin remains shadowed by a config-selected source", async () => {
    ...
    const next = await persistPluginInstall({
      ...
      install: {
        ...
        installPath: "/tmp/FirstNexus/npm/node_modules/@FirstNexus/discord/index.ts",
      },
    });
    ...
    expect(runtimeLogs.join("\n")).toContain(
      "active config source: /tmp/FirstNexus-upstream/extensions/discord/index.ts",
    );
    expect(runtimeLogs.join("\n")).toContain(
      "installed npm source: /tmp/FirstNexus/npm/node_modules/@FirstNexus/discord/index.ts",
    );
    expect(runtimeLogs.join("\n")).toContain("FirstNexus plugins doctor");
  });
  ...
  it("does not warn when the config-selected source is inside the npm install path", async () => {
      ...
      plugins: [
        {
          id: "discord",
          origin: "config",
          source: "/tmp/FirstNexus/npm/node_modules/@FirstNexus/discord/dist/index.js",
          status: "loaded",
        },
      ],
      diagnostics: [],
    ...
    await persistPluginInstall({
      ...
      install: {
        source: "npm",
        spec: "@FirstNexus/discord",
        installPath: "/tmp/FirstNexus/npm/node_modules/@FirstNexus/discord",
      },
    });
