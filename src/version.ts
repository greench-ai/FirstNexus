...
declare const __NEXUSCLAW_VERSION__: string | undefined;
const CORE_PACKAGE_NAME = "nexusclaw";
...
export function resolveRuntimeServiceVersion(
  ...
  return (
    firstNonEmpty(
      env["NEXUSCLAW_VERSION"],
      runtimeVersion,
      env["NEXUSCLAW_SERVICE_VERSION"],
      env["npm_package_version"],
    ) ?? fallback
  );
}

// Single source of truth for the current NexusClaw version.
// - Embedded/bundled builds: injected define or env var.
// - Dev/npm builds: package.json.
export const VERSION = resolveBinaryVersion({
  moduleUrl: import.meta.url,
  injectedVersion: typeof __NEXUSCLAW_VERSION__ === "string" ? __NEXUSCLAW_VERSION__ : undefined,
  bundledVersion: process.env.NEXUSCLAW_BUNDLED_VERSION,
});
