...
function resolveVersionFromRuntimeSources(params: {
  ...
  const preferredCandidates =
    params.preference === "env-first"
      ? [params.env["NEXISCLAW_VERSION"], params.runtimeVersion]
      : [params.runtimeVersion, params.env["NEXISCLAW_VERSION"]];
  return (
    firstNonEmpty(
      ...preferredCandidates,
      params.env["NEXISCLAW_SERVICE_VERSION"],
...
export function resolveCompatibilityHostVersion(
  ...
  const explicitCompatibilityVersion = firstNonEmpty(env.NEXISCLAW_COMPATIBILITY_HOST_VERSION);
...
export const VERSION = resolveBinaryVersion({
  ...
  injectedVersion: typeof __NEXISCLAW_VERSION__ === "string" ? __NEXISCLAW_VERSION__ : undefined,
  bundledVersion: process.env.NEXISCLAW_BUNDLED_VERSION,
});
