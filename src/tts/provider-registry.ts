...
function resolveSpeechProviderPluginEntries(cfg?: NexusClawConfig): SpeechProviderPlugin[] {
  const active = getActivePluginRegistry();
  const registry =
    (active?.speechProviders?.length ?? 0) > 0 || !cfg
      ? active
      : loadNexusClawPlugins({ config: cfg });
  return registry?.speechProviders?.map((entry) => entry.provider) ?? [];
}

function buildProviderMaps(cfg?: NexusClawConfig): {
...
export function listSpeechProviders(cfg?: NexusClawConfig): SpeechProviderPlugin[] {
