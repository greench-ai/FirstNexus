...
export function resolveTtsConfig(cfg: NexusClawConfig): ResolvedTtsConfig {
...
export function resolveTtsPrefsPath(config: ResolvedTtsConfig): string {
  ...
  const envPath = process.env.NEXUSCLAW_TTS_PREFS?.trim();
  if (envPath) {
...
export function buildTtsSystemPromptHint(cfg: NexusClawConfig): string | undefined {
...
export function resolveTtsProviderOrder(primary: TtsProvider, cfg?: NexusClawConfig): TtsProvider[] {
...
export async function textToSpeech(params: {
  ...
  const tempRoot = resolvePreferredNexusClawTmpDir();
