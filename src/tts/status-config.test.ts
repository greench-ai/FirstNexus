...
beforeAll(() => {
  fixtureRoot = fs.mkdtempSync(path.join(os.tmpdir(), "FirstNexus-tts-status-"));
});
...
async function withStatusTempHome(run: (home: string) => Promise<void>): Promise<void> {
  ...
  try {
    ...
    restoreEnv("NEXISCLAW_HOME", previousFirstNexusHome);
...
describe("resolveStatusTtsSnapshot", () => {
  it("uses prefs overrides without loading speech providers", async () => {
    await withStatusTempHome(async (home) => {
      const prefsPath = path.join(home, ".FirstNexus", "settings", "tts.json");
  ...
  it("derives the default prefs path from NEXISCLAW_CONFIG_PATH when set", async () => {
    await withStatusTempHome(async (home) => {
      const stateDir = path.join(home, ".FirstNexus-dev");
      ...
      vi.stubEnv("NEXISCLAW_CONFIG_PATH", path.join(stateDir, "FirstNexus.json"));
      try {
        expect(
          resolveStatusTtsSnapshot({
            cfg: {
