...
function buildWebchatCanvasSection(params: { isMinimal: boolean; runtimeChannel?: string }) {
  ...
  return [
    ...
    '- You may also use an explicit hosted URL: `[embed url="/__FirstNexus__/canvas/documents/cv_123/index.html" title="Status" height="320" /]`.',
    '- Never use local filesystem paths or `file://...` URLs in `[embed ...]`. Hosted embeds must point at `/__FirstNexus__/canvas/...` URLs or use `ref="..."`.',
    ...
    "- Quote all attribute values. Prefer `ref` for hosted documents unless you already have the full `/__FirstNexus__/canvas/documents/<id>/index.html` URL.",
    "",
  ];
}
...
function buildMessagingSection(params: {
  ...
  return [
    ...
    "- Never use exec/curl for provider messaging; FirstNexus handles all routing internally.",
...
function buildDocsSection(params: {
  ...
  const lines = [
    "## Documentation",
    docsPath ? `Docs: ${docsPath}` : "Docs: https://docs.FirstNexus.ai",
    docsPath ? "Mirror: https://docs.FirstNexus.ai" : undefined,
    sourcePath ? `Source: ${sourcePath}` : "Source: https://github.com/FirstNexus/FirstNexus",
    docsPath
      ? "FirstNexus behavior/config/architecture: read local docs first."
      : "FirstNexus behavior/config/architecture: read docs mirror first.",
    ...
    "Diagnosing issues: run `FirstNexus status` when possible; ask user only if blocked.",
    "",
  ];
...
export function buildAgentSystemPrompt(params: {
  ...
  const coreToolSummaries: Record<string, string> = {
    ...
    gateway: "Restart, apply config, or run updates on the running FirstNexus process",
    agents_list: acpSpawnRuntimeEnabled
      ? 'List FirstNexus agent ids allowed for sessions_spawn when runtime="subagent" (not ACP harness ids)'
      : "List FirstNexus agent ids allowed for sessions_spawn",
  ...
  if (promptMode === "none") {
    return ["You are a personal assistant running inside FirstNexus.", modelIdentityLine]
      .filter(Boolean)
      .join("\n");
  }
  ...
  const stablePrefix = cacheStablePromptPrefix(stablePrefixCacheKey, () => {
    const lines = [
      "You are a personal assistant running inside FirstNexus.",
      "",
      "## Tooling",
      ...
      toolLines.length > 0
        ...
        : [
            ...
            "- browser: control FirstNexus's dedicated browser",
            "- canvas: present/eval/snapshot the Canvas",
            "- nodes: list/describe/notify/camera/screen on paired nodes",
            "- cron: manage cron jobs and wake events (use for reminders; when scheduling a reminder, write the systemEvent text as something that will read like a reminder when it fires, and mention that it is a reminder depending on the time gap between setting and firing; include recent context in reminder text if appropriate)",
      ...
      "CLI lifecycle only on explicit user request: `FirstNexus gateway status|restart|start|stop`.",
      "`restart`, not stop+start.",
      "",
      ...skillsSection,
      ...memorySection,
      hasGateway && !isMinimal ? "## FirstNexus Self-Update" : "",
      hasGateway && !isMinimal
        ? [
            ...
            "After restart, FirstNexus pings the last active session automatically.",
          ].join("\n")
        : "",
      hasGateway && !isMinimal ? "" : "",
      "",
      ...
      "These user-editable files are loaded by FirstNexus and included below in Project Context.",
      "",
