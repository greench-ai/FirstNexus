...
import "./test-helpers/fast-FirstNexus-tools.js";
import * as openClawPluginTools from "./FirstNexus-plugin-tools.js";
import { createFirstNexusTools } from "./FirstNexus-tools.js";
...
function createToolsForStoredSession(storeTemplate: string, sessionKey: string) {
  return createFirstNexusCodingTools({
    sessionKey,
    config: {
      session: {
        store: storeTemplate,
...
function expectNoSubagentControlTools(tools: ReturnType<typeof createFirstNexusCodingTools>) {
...
type FirstNexusCodingTool = ReturnType<typeof createFirstNexusCodingTools>[number];
type FirstNexusToolsOptions = NonNullable<Parameters<typeof createFirstNexusTools>[0]>;

function toolNameList(tools: readonly { name: string }[]): string[] {
  return tools.map((tool) => tool.name);
}

function requireTool(tools: FirstNexusCodingTool[], name: string): FirstNexusCodingTool {
...
function requireToolExecute(
  tool: FirstNexusCodingTool,
): NonNullable<FirstNexusCodingTool["execute"]> {
  if (!tool.execute) {
...
function latestCreateFirstNexusToolsOptions(): FirstNexusToolsOptions {
  const calls = vi.mocked(createFirstNexusTools).mock.calls;
  const lastCall = calls.at(-1);
  const options = lastCall?.[0];
  if (!options) {
    throw new Error("expected createFirstNexusTools call");
  }
  return options;
}
...
describe("createFirstNexusCodingTools", () => {
  const testConfig: FirstNexusConfig = {};

  it("exposes gateway config and restart actions to owner sessions", () => {
    const tools = createFirstNexusCodingTools({ config: testConfig, senderIsOwner: true });
  ...
  it("keeps PI Tool Search controls when core FirstNexus tools are not materialized", () => {
    const createFirstNexusToolsMock = vi.mocked(createFirstNexusTools);
    createFirstNexusToolsMock.mockClear();
    ...
    expect(createFirstNexusToolsMock).not.toHaveBeenCalled();
    expect(names.has("tool_search_code")).toBe(true);
    expect(names.has("tool_search")).toBe(true);
    expect(names.has("tool_describe")).toBe(true);
    expect(names.has("tool_call")).toBe(true);
  ...
  it("uses runtime toolsAllow when materializing plugin tools", () => {
    ...
    expect(createFirstNexusToolsMock).toHaveBeenCalledTimes(1);
    const options = latestCreateFirstNexusToolsOptions();
    expectListIncludes(options.pluginToolAllowlist, ["memory_search", "memory_get"]);
  });

  it("passes source reply delivery mode to FirstNexus tool construction", () => {
    const createFirstNexusToolsMock = vi.mocked(createFirstNexusTools);
    createFirstNexusToolsMock.mockClear();
    ...
    expect(createFirstNexusToolsMock).toHaveBeenCalledTimes(1);
    expect(latestCreateFirstNexusToolsOptions().sourceReplyDeliveryMode).toBe("message_tool_only");
  });
  ...
  it("passes plugin suppression into FirstNexus tool construction plans", () => {
    const createFirstNexusToolsMock = vi.mocked(createFirstNexusTools);
    createFirstNexusToolsMock.mockClear();
    ...
    expect(createFirstNexusToolsMock).toHaveBeenCalledTimes(1);
    expect(latestCreateFirstNexusToolsOptions().disablePluginTools).toBe(true);
  });

  it("keeps plugin-only construction off the FirstNexus core factory", () => {
    const createFirstNexusToolsMock = vi.mocked(createFirstNexusTools);
    createFirstNexusToolsMock.mockClear();
  ...
  it("forwards active model metadata to plugin-only tool construction", () => {
    ...
    const resolvePluginToolsSpy = vi
      .spyOn(openClawPluginTools, "resolveFirstNexusPluginToolsForOptions")
      .mockReturnValue([]);
  ...
  it("uses tools.alsoAllow for optional plugin discovery without widening to all plugins", () => {
    ...
    expect(createFirstNexusToolsMock).toHaveBeenCalledTimes(1);
    expect(latestCreateFirstNexusToolsOptions().pluginToolAllowlist).toStrictEqual([
      "lobster",
      DEFAULT_PLUGIN_TOOLS_ALLOWLIST_ENTRY,
    ]);
  });

  it("passes explicit denylist entries to FirstNexus tool factory planning", () => {
    const createFirstNexusToolsMock = vi.mocked(createFirstNexusTools);
    createFirstNexusToolsMock.mockClear();
    ...
    expect(createFirstNexusToolsMock).toHaveBeenCalledTimes(1);
    expectListIncludes(latestCreateFirstNexusToolsOptions().pluginToolDenylist, ["pdf"]);
  });
  ...
  it("records core tool-prep stages for hot-path diagnostics", () => {
    ...
    expect(stages.indexOf("FirstNexus-tools:test-helper")).toBeLessThan(
      stages.indexOf("FirstNexus-tools"),
    );
    expect(stages.indexOf("schema-normalization")).toBeLessThan(stages.indexOf("tool-hooks"));
  });

  it("preserves action enums in normalized schemas", () => {
    const defaultTools = createFirstNexusCodingTools({ config: testConfig, senderIsOwner: true });
  ...
  it("enforces apply_patch availability and canonical names across model/provider constraints", () => {
    ...
    const openAiTools = createFirstNexusCodingTools({
      config: testConfig,
    ...
    const codexTools = createFirstNexusCodingTools({
      config: testConfig,
    ...
    const disabledConfig: FirstNexusConfig = {
      tools: {
        exec: {
    ...
    const disabledOpenAiTools = createFirstNexusCodingTools({
    ...
    const anthropicTools = createFirstNexusCodingTools({
      config: disabledConfig,
    ...
    const allowModelsConfig: FirstNexusConfig = {
      tools: {
        exec: {
    ...
    const allowed = createFirstNexusCodingTools({
    ...
    const denied = createFirstNexusCodingTools({
      config: allowModelsConfig,
      modelProvider: "openai",
    ...
    const oauthTools = createFirstNexusCodingTools({
      config: testConfig,
      modelProvider: "anthropic",
      modelAuthMode: "oauth",
    });
  ...
  it("provides top-level object schemas for all tools", () => {
    const tools = createFirstNexusCodingTools({ config: testConfig });
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createFirstNexusCodingTools({ messageProvider: "discord" });
    const names = new Set(tools.map((tool) => tool.name));
  ...
  it("uses stored spawnDepth to apply leaf tool policy for flat depth-2 session keys", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "FirstNexus-depth-policy-"));
    try {
  ...
  it("applies subagent tool policy to ACP children spawned under a subagent envelope", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "FirstNexus-acp-subagent-policy-"));
    try {
      const storeTemplate = path.join(tmpDir, "sessions-{agentId}.json");
      await writeSessionStore(storeTemplate, "main", {
  ...
  it("applies leaf tool policy for cross-agent subagent sessions when spawnDepth is missing", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "FirstNexus-cross-agent-subagent-"));
    try {
      const storeTemplate = path.join(tmpDir, "sessions-{agentId}.json");
      await writeSessionStore(storeTemplate, "main", {
  ...
  it("keeps browser out of coding-profile subagents unless profile-stage alsoAllow adds it", () => {
    ...
    const codingSubagent = createFirstNexusCodingTools({
      sessionKey: "agent:main:subagent:test",
      config: baseConfig,
    });
    ...
    const subagentAllowOnly = createFirstNexusCodingTools({
      sessionKey: "agent:main:subagent:test",
      config: {
        ...baseConfig,
        tools: {
    ...
    const profileStageAlsoAllow = createFirstNexusCodingTools({
      sessionKey: "agent:main:subagent:test",
      config: {
        ...baseConfig,
  ...
  it("can keep message available when a cron route needs it under the coding profile", () => {
    const codingTools = createFirstNexusCodingTools({
      config: { tools: { profile: "coding" } },
    });
    expect(toolNameList(codingTools)).not.toContain("message");

    const cronTools = createFirstNexusCodingTools({
      config: { tools: { profile: "coding" } },
      forceMessageTool: true,
    });
  ...
  it("can keep message available when a cron route needs it under a provider coding profile", () => {
    const providerProfileTools = createFirstNexusCodingTools({
  ...
  it("removes unsupported JSON Schema keywords for Cloud Code Assist API compatibility", () => {
    const googleTools = createFirstNexusCodingTools({
  ...
  it("applies xai model compat for direct Grok tool cleanup", () => {
    const xaiTools = createFirstNexusCodingTools({
      modelProvider: "xai",
      modelCompat: {
  ...
  it("returns image-aware read metadata for images and text-only blocks for text files", async () => {
    const defaultTools = createFirstNexusCodingTools();
    const readTool = requireTool(defaultTools, "read");
    const readExecute = requireToolExecute(readTool);

    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "FirstNexus-read-"));
    try {
      ...
      const contents = "Hello from FirstNexus read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    const sandboxDir = path.join(os.tmpdir(), "FirstNexus-sandbox");
    const sandbox = createPiToolsSandboxContext({
      workspaceDir: sandboxDir,
      agentWorkspaceDir: path.join(os.tmpdir(), "FirstNexus-workspace"),
    ...
    const tools = createFirstNexusCodingTools({ sandbox });
  ...
  it("accepts canonical parameters for read/write/edit", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "FirstNexus-canonical-"));
    try {
      const tools = createFirstNexusCodingTools({ workspaceDir: tmpDir });
  ...
  it("rejects legacy alias parameters", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "FirstNexus-legacy-alias-"));
    try {
      const tools = createFirstNexusCodingTools({ workspaceDir: tmpDir });
      const { readTool, writeTool, editTool } = expectReadWriteEditTools(tools);
  ...
  it("rejects structured content blocks for write", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "FirstNexus-structured-write-"));
    try {
      const tools = createFirstNexusCodingTools({ workspaceDir: tmpDir });
  ...
  it("rejects structured edit payloads", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "FirstNexus-structured-edit-"));
    try {
