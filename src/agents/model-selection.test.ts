...
function resolveAnthropicOpusThinking(cfg: FirstNexusConfig) {
...
function resolveAnthropicOpus47Thinking(cfg: FirstNexusConfig) {
  return resolveThinkingDefault({
    cfg,
    provider: "anthropic",
...
function resolveConfiguredRefForTest(cfg: Partial<FirstNexusConfig>) {
  return resolveConfiguredModelRef({
    cfg: cfg as FirstNexusConfig,
...
describe("model-selection", () => {
  ...
  describe("buildModelAliasIndex", () => {
    it("should build alias index from config", () => {
      const cfg: Partial<FirstNexusConfig> = {
        agents: {
          defaults: {
            models: {
  ...
  describe("resolveConfiguredModelRef", () => {
    ...
    it("sanitizes control characters in providerless-model warnings", async () => {
      const warnLogs = createWarnLogCapture("FirstNexus-model-selection-test");
      try {
        const cfg: Partial<FirstNexusConfig> = {
          agents: {
            defaults: {
