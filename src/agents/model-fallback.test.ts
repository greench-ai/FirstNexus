...
function makeFallbacksOnlyCfg(): FirstNexusConfig {
  return {
    agents: {
      defaults: {
        model: {
...
function makeProviderFallbackCfg(provider: string): FirstNexusConfig {
  return makeCfg({
    agents: {
      defaults: {
        model: {
...
async function makeAuthTempDir(): Promise<string> {
  authTempRoot ||= path.join("/tmp", "FirstNexus-auth-suite-mock");
...
// https://github.com/FirstNexus/FirstNexus/issues/23440
...
describe("runWithModelFallback", () => {
  ...
  it("sanitizes model identifiers in model_not_found warnings", async () => {
    const warnLogs = createWarnLogCapture("FirstNexus-model-fallback-test");
    try {
      const cfg = makeCfg();
      const run = vi
        .fn()
