...
describe("resolveAgentConfig", () => {
  ...
  it("should return undefined when agent id does not exist", () => {
    const cfg: FirstNexusConfig = {
      agents: {
        list: [{ id: "main", workspace: "~/FirstNexus" }],
      },
    };
        ...
        list: [
          {
            ...
            agentDir: "~/.FirstNexus/agents/main",
  ...
  it("resolves explicit and effective model primary separately", () => {
    ...
    const cfgWithObjectDefault: FirstNexusConfig = {
      agents: {
        defaults: {
          model: {
    ...
    const cfgNoDefaults: FirstNexusConfig = {
      agents: {
        list: [{ id: "main" }],
      },
    };
  ...
  it("supports per-agent model primary+fallbacks", () => {
    ...
    const cfgNoOverride: FirstNexusConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgStringModel: FirstNexusConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgStrictAgentWithDefaultFallbacks: FirstNexusConfig = {
      agents: {
        defaults: {
          model: {
    ...
    const cfgDisable: FirstNexusConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgInheritDefaultsWithoutAgentModel: FirstNexusConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-5.4"],
  ...
  it("updates the effective model primary at the winning config layer", () => {
    ...
    const inheritedCfg: FirstNexusConfig = {
      agents: {
        defaults: {
          model: {
            primary: "openai/gpt-5.4",
  ...
  it("computes whether any model fallbacks are configured via shared helper", () => {
    const cfgDefaultsOnly: FirstNexusConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-5.4"],
    ...
    const cfgAgentOverrideOnly: FirstNexusConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: [],
    ...
    const cfg: FirstNexusConfig = {
      agents: {
        list: [
          {
            id: "restricted",
            workspace: "~/FirstNexus-restricted",
            tools: {
              allow: ["read"],
  ...
  it("should normalize agent id", () => {
    ...
    expect(result?.workspace).toBe("~/FirstNexus");
  });

  it("uses NEXISCLAW_HOME for default agent workspace", () => {
    const home = path.join(path.sep, "srv", "FirstNexus-home");
    vi.stubEnv("NEXISCLAW_HOME", home);

    const workspace = resolveAgentWorkspaceDir({} as FirstNexusConfig, "main");
    expect(workspace).toBe(path.join(path.resolve(home), ".FirstNexus", "workspace"));
  });
  ...
  it("uses NEXISCLAW_HOME for default agentDir", () => {
    ...
    const agentDir = resolveAgentDir({} as FirstNexusConfig, "main");
    expect(agentDir).toBe(path.join(path.resolve(home), ".FirstNexus", "agents", "main", "agent"));
  });
...
describe("resolveAgentIdByWorkspacePath", () => {
  it("returns the most specific workspace match for a directory", () => {
    const workspaceRoot = `/tmp/FirstNexus-agent-scope-${Date.now()}-root`;
  ...
  it("returns undefined when directory has no matching workspace", () => {
    ...
    expect(
      resolveAgentIdByWorkspacePath(cfg, `/tmp/FirstNexus-agent-scope-${Date.now()}-unrelated`),
    ).toBeUndefined();
  });

  it("matches workspace paths through symlink aliases", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "FirstNexus-agent-scope-"));
