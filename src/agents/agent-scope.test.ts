...
describe("resolveAgentConfig", () => {
  ...
  it("should return undefined when agent id does not exist", () => {
    const cfg: NexusClawConfig = {
      agents: {
        list: [{ id: "main", workspace: "~/nexusclaw" }],
      },
    };
  ...
  it("resolves explicit and effective model primary separately", () => {
    ...
    const cfgWithObjectDefault: NexusClawConfig = {
      agents: {
        defaults: {
          model: {
    ...
    const cfgNoDefaults: NexusClawConfig = {
      agents: {
        list: [{ id: "main" }],
      },
    };
  ...
  it("supports per-agent model primary+fallbacks", () => {
    ...
    const cfgNoOverride: NexusClawConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgDisable: NexusClawConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgInheritDefaults: NexusClawConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-4.1"],
  ...
  it("computes whether any model fallbacks are configured via shared helper", () => {
    const cfgDefaultsOnly: NexusClawConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-4.1"],
    ...
    const cfgAgentOverrideOnly: NexusClawConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: [],
    ...
    const cfg: NexusClawConfig = {
      agents: {
        list: [
          {
            id: "restricted",
            workspace: "~/nexusclaw-restricted",
            tools: {
              allow: ["read"],
  ...
  it("should normalize agent id", () => {
    ...
    expect(result?.workspace).toBe("~/nexusclaw");
  });

  it("uses NEXUSCLAW_HOME for default agent workspace", () => {
    const home = path.join(path.sep, "srv", "nexusclaw-home");
    vi.stubEnv("NEXUSCLAW_HOME", home);

    const workspace = resolveAgentWorkspaceDir({} as NexusClawConfig, "main");
    expect(workspace).toBe(path.join(path.resolve(home), ".nexusclaw", "workspace"));
  });

  it("uses NEXUSCLAW_HOME for default agentDir", () => {
    const home = path.join(path.sep, "srv", "nexusclaw-home");
    vi.stubEnv("NEXUSCLAW_HOME", home);
    // Clear state dir so it falls back to NEXUSCLAW_HOME
    vi.stubEnv("NEXUSCLAW_STATE_DIR", "");

    const agentDir = resolveAgentDir({} as NexusClawConfig, "main");
    expect(agentDir).toBe(path.join(path.resolve(home), ".nexusclaw", "agents", "main", "agent"));
  });
});

describe("resolveAgentIdByWorkspacePath", () => {
  it("returns the most specific workspace match for a directory", () => {
    const workspaceRoot = `/tmp/nexusclaw-agent-scope-${Date.now()}-root`;
  ...
  it("returns undefined when directory has no matching workspace", () => {
    ...
    expect(
      resolveAgentIdByWorkspacePath(cfg, `/tmp/nexusclaw-agent-scope-${Date.now()}-unrelated`),
    ).toBeUndefined();
  });

  it("matches workspace paths through symlink aliases", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "nexusclaw-agent-scope-"));
