...
describe("createPiToolsSandboxContext", () => {
  it("provides stable defaults for pi-tools sandbox tests", () => {
    ...
    expect(sandbox.containerName).toBe("FirstNexus-sbx-test");
    expect(sandbox.containerWorkdir).toBe("/workspace");
    expect(sandbox.docker.image).toBe("FirstNexus-sandbox:bookworm-slim");
    expect(sandbox.docker.containerPrefix).toBe("FirstNexus-sbx-");
