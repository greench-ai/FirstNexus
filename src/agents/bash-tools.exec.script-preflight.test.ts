...
const parseFirstNexusChannelsLoginShellCommand = __testing.parseFirstNexusChannelsLoginShellCommand;
...
async function expectSymlinkSwapDuringPreflightToAvoidErrors(params: {
  ...
  await withTempDir("FirstNexus-exec-preflight-open-race-", async (parent) => {
...
describe("exec interactive FirstNexus channel login guard", () => {
  it("recognizes direct and package-runner channel login commands before execution", () => {
    expect(
      parseFirstNexusChannelsLoginShellCommand("FirstNexus channels login --channel whatsapp"),
    ).toBe(true);
    expect(
      parseFirstNexusChannelsLoginShellCommand(
        "pnpm exec FirstNexus channels login --channel whatsapp --verbose",
      ),
    ).toBe(true);
    expect(parseFirstNexusChannelsLoginShellCommand("FirstNexus channels status --deep")).toBe(
      false,
    );
  });
  ...
  it("blocks interactive channel login commands from exec", async () => {
    ...
    await expect(
      tool.execute("call-FirstNexus-channel-login", {
        command: "FirstNexus channels login --channel whatsapp --verbose",
      }),
    ).rejects.toThrow(/exec cannot run interactive FirstNexus channel login commands/);
    await expect(
      tool.execute("call-wrapped-FirstNexus-channel-login", {
        command: "sudo -u FirstNexus bash -lc 'FirstNexus channels login --channel whatsapp'",
      }),
    ).rejects.toThrow(/exec cannot run interactive FirstNexus channel login commands/);
    await expect(
      tool.execute("call-clustered-sudo-channel-login", {
        command: "sudo -EH bash -lc 'FirstNexus channels login --channel whatsapp'",
      }),
    ).rejects.toThrow(/exec cannot run interactive FirstNexus channel login commands/);
    await expect(
      tool.execute("call-deep-env-channel-login", {
        command: "env env env env env env FirstNexus channels login --channel whatsapp",
      }),
    ).rejects.toThrow(/exec cannot run interactive FirstNexus channel login commands/);
    await expect(
      tool.execute("call-env-s-trailing-channel-login", {
        command: "env -S 'FirstNexus channels' login --channel whatsapp",
      }),
    ).rejects.toThrow(/exec cannot run interactive FirstNexus channel login commands/);
  });
});
...
describeNonWin("exec script preflight", () => {
  it("blocks shell env var injection tokens in python scripts before execution", async () => {
    await withTempDir("FirstNexus-exec-preflight-", async (tmp) => {
  ...
  it("skips preflight file reads for script paths outside the workdir", async () => {
    await withTempDir("FirstNexus-exec-preflight-parent-", async (parent) => {
      const outsidePath = path.join(parent, "outside.js");
      const workdir = path.join(parent, "workdir");
      await fs.mkdir(workdir, { recursive: true });
  ...
  it("opens preflight script reads with O_NONBLOCK to avoid FIFO stalls", async () => {
    await withTempDir("FirstNexus-exec-preflight-nonblock-", async (tmp) => {
...
describeWin("exec script preflight on windows path syntax", () => {
  it("preserves windows-style python relative path separators during script extraction", async () => {
    await withTempDir("FirstNexus-exec-preflight-win-", async (tmp) => {
      await fs.writeFile(path.join(tmp, "bad.py"), "payload = $DM_JSON", "utf-8");
...
describe("exec interpreter heuristics ReDoS guard", () => {
  it("does not hang on long commands with VAR=value assignments and whitespace-heavy text", async () => {
    ...
    const command = `ACCESS_TOKEN=$(__FirstNexus_missing_redos_guard__)\ncat > /tmp/out.html << 'EOF'\n${htmlBlock}\nEOF`;
