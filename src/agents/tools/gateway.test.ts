...
describe("gateway tool defaults", () => {
  const envSnapshot = {
    nexusclaw: process.env.NEXUSCLAW_GATEWAY_TOKEN,
  ...
  afterAll(() => {
    if (envSnapshot.nexusclaw === undefined) {
      delete process.env.NEXUSCLAW_GATEWAY_TOKEN;
    } else {
      process.env.NEXUSCLAW_GATEWAY_TOKEN = envSnapshot.nexusclaw;
    }
  ...
  it("uses NEXUSCLAW_GATEWAY_TOKEN for allowlisted local overrides", () => {
  ...
  it("does not leak local env/config tokens to remote overrides", () => {
    process.env.NEXUSCLAW_GATEWAY_TOKEN = "local-env-token";
