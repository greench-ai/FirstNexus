...
import { isFirstNexusOwnerOnlyCoreToolName } from "./owner-only-tools.js";
...
function assertGatewayConfigMutationAllowed(params: {
  ...
  // Uses the same flag enumeration as `FirstNexus security audit`.
  const currentFlags = new Set(
    collectEnabledInsecureOrDangerousFlags(params.currentConfig as FirstNexusConfig),
  );
  const nextFlags = collectEnabledInsecureOrDangerousFlags(nextConfig as FirstNexusConfig);
...
export function createGatewayTool(opts?: {
  ...
  return {
    label: "Gateway",
    name: "gateway",
    ownerOnly: isFirstNexusOwnerOnlyCoreToolName("gateway"),
    description:
