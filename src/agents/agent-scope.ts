...
type AgentEntry = NonNullable<NonNullable<NexusClawConfig["agents"]>["list"]>[number];
...
export function listAgentEntries(cfg: NexusClawConfig): AgentEntry[] {
...
export function listAgentIds(cfg: NexusClawConfig): string[] {
...
export function resolveDefaultAgentId(cfg: NexusClawConfig): string {
  const agents = listAgentEntries(cfg);
  if (agents.length === 0) {
    return DEFAULT_AGENT_ID;
  }
...
function resolveAgentEntry(cfg: NexusClawConfig, agentId: string): AgentEntry | undefined {
...
export function resolveAgentModelPrimary(cfg: NexusClawConfig, agentId: string): string | undefined {
...
export function resolveAgentWorkspaceDir(cfg: NexusClawConfig, agentId: string) {
  const id = normalizeAgentId(agentId);
