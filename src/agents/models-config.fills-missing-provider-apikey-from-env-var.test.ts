...
async function runCustomProviderMergeTest(params: {
  ...
  await ensureNexusClawModelsJson({
    models: {
      mode: "merge",
      providers: {
...
function createOpenAiConfigWithResolvedApiKey(mergeMode = false): NexusClawConfig {
  return {
    models: {
...
describe("models-config", () => {
  it("keeps anthropic api defaults when model entries omit api", async () => {
    await withTempHome(async () => {
      ...
      await ensureNexusClawModelsJson(validated.config);
  ...
  it("fills anthropic-vertex apiKey with the ADC sentinel when models exist", async () => {
    await withTempHome(async () => {
      const adcDir = await fs.mkdtemp(path.join(os.tmpdir(), "nexusclaw-adc-"));
  ...
  it("merges providers by default", async () => {
    await withTempHome(async () => {
      ...
      await ensureNexusClawModelsJson(CUSTOM_PROXY_MODELS_CONFIG);
