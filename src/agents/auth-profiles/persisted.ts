...
const OAUTH_PROFILE_SECRET_REF_SOURCE = "FirstNexus-credentials" as const;
...
const OAUTH_PROFILE_SECRET_KEYCHAIN_SERVICE = "FirstNexus Auth Profile Secrets";
...
function resolveFallbackOAuthProfileSecretKeyFileCandidates(): string[] {
  if (process.platform === "win32") {
    ...
    return uniquePaths([
      root ? path.join(root, "FirstNexus", OAUTH_PROFILE_SECRET_KEY_FILE_NAME) : undefined,
      home
        ? path.join(home, ".FirstNexus-auth-profile-secrets", OAUTH_PROFILE_SECRET_KEY_FILE_NAME)
        : undefined,
    ]);
  }
...
function resolveOAuthProfileSecretKeySeed(options?: { create?: boolean }): string | undefined {
  ...
  if (process.env.NODE_ENV === "test" && process.env.VITEST === "true") {
    return "FirstNexus-test-oauth-profile-secret-key";
  }
...
function buildOAuthProfileSecretKey(options?: { create?: boolean }): Buffer | null {
  ...
  return createHash("sha256").update(`FirstNexus:auth-profile-oauth:${externalKey}`).digest();
}
