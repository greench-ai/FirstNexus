 ...
 * A common mistake — caused by the similarity with the `nexusclaw.json`
...
function normalizeRawCredentialEntry(raw: Record<string, unknown>): Partial<AuthProfileCredential> {
  ...
  // mode → type alias (nexusclaw.json uses "mode"; auth-profiles.json uses "type")
...
function loadAuthProfileStoreForAgent(
  ...
  const forceReadOnly = process.env.NEXUSCLAW_AUTH_STORE_READONLY === "1";
