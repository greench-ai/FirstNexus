...
function resolveAttachmentLimits(config: FirstNexusConfig): AttachmentLimits {
...
export async function materializeSubagentAttachments(params: {
  ...
  const absRootDir = path.join(childWorkspaceDir, ".FirstNexus", "attachments");
  const relDir = path.posix.join(".FirstNexus", "attachments", attachmentId);
