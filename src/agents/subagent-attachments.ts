...
function resolveAttachmentLimits(config: NexusClawConfig): AttachmentLimits {
...
export async function materializeSubagentAttachments(params: {
  ...
  const absRootDir = path.join(childWorkspaceDir, ".nexusclaw", "attachments");
  const relDir = path.posix.join(".nexusclaw", "attachments", attachmentId);
