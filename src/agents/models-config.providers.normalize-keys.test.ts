...
describe("normalizeProviders", () => {
  const createModel = (
    overrides: Partial<
      NonNullable<NonNullable<FirstNexusConfig["models"]>["providers"]>[string]["models"][number]
    > = {},
  ) => ({
    id: "config-model",
    name: "Config model",
  ...
  it("trims provider keys so image models remain discoverable for custom providers", async () => {
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "FirstNexus-agent-"));
    try {
      const providers: NonNullable<NonNullable<FirstNexusConfig["models"]>["providers"]> = {
        " dashscope-vision ": {
  ...
  it("normalizes retired Google Gemini model ids before emitting provider config", async () => {
    ...
    try {
      const providers: NonNullable<NonNullable<FirstNexusConfig["models"]>["providers"]> = {
        ...
        "google-gemini-cli": {
          baseUrl: "FirstNexus://google-gemini-cli",
          models: [
            createModel({
              id: "gemini-3-pro-preview",
  ...
  it("ignores non-object provider entries during source-managed enforcement", () => {
    ...
    } as unknown as NonNullable<NonNullable<FirstNexusConfig["models"]>["providers"]>;

    const sourceProviders: NonNullable<NonNullable<FirstNexusConfig["models"]>["providers"]> = {
      openai: {
        baseUrl: "https://api.openai.com/v1",
        api: "openai-completions",
