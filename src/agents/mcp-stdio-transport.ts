...
export type FirstNexusStdioServerParameters = {
  command: string;
  args?: string[];
...
export class FirstNexusStdioClientTransport implements Transport {
  onclose?: () => void;
  ...
  constructor(private readonly serverParams: FirstNexusStdioServerParameters) {
  ...
  async start(): Promise<void> {
    if (this.process) {
      throw new Error(
        "FirstNexusStdioClientTransport already started; Client.connect() starts transports automatically.",
      );
    }
