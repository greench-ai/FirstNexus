...
let createFirstNexusCodingTools: typeof import("./pi-tools.js").createFirstNexusCodingTools;
...
beforeAll(async () => {
  await withEnvAsync(
    {
      NEXISCLAW_BUNDLED_PLUGINS_DIR: path.join(
        os.tmpdir(),
        "FirstNexus-test-no-bundled-extensions",
      ),
    },
    async () => {
      ({ createFirstNexusCodingTools } = await import("./pi-tools.js"));
    },
  );
});
...
async function withSafeBinsExecTool(
  ...
  try {
    await withEnvAsync(
      {
        NEXISCLAW_SHELL_ENV_TIMEOUT_MS: "1",
        PATH: "/usr/bin:/bin",
        SHELL: "/bin/sh",
      },
      async () => {
...
describe("createFirstNexusCodingTools safeBins", () => {
  it("threads tools.exec.safeBins into exec allowlist checks", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "FirstNexus-safe-bins-",
        safeBins: ["echo"],
        safeBinProfiles: {
  ...
  it("rejects unprofiled custom safe-bin entries", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "FirstNexus-safe-bins-unprofiled-",
        safeBins: ["echo"],
      },
      async ({ tmpDir, execTool }) => {
        await expect(
  ...
  it("does not allow env var expansion to smuggle file args via safeBins", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "FirstNexus-safe-bins-expand-",
        safeBins: ["head", "wc"],
  ...
  it("blocks sort output/compress bypass attempts in safeBins mode", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "FirstNexus-safe-bins-sort-",
        safeBins: ["sort"],
  ...
  it("blocks shell redirection metacharacters in safeBins mode", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "FirstNexus-safe-bins-redirect-",
        safeBins: ["head"],
  ...
  it("blocks grep recursive flags from reading cwd via safeBins", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "FirstNexus-safe-bins-grep-",
        safeBins: ["grep"],
