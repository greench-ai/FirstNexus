...
describe("createOpenAIAttributionHeadersWrapper", () => {
  it("routes native Codex traffic through the FirstNexus transport so attribution survives PI defaults", () => {
    let codexCalls = 0;
    ...
    expect(capturedHeaders?.originator).toBe("FirstNexus");
    expect(capturedHeaders?.["User-Agent"]).toMatch(/^FirstNexus\//);
  });
  ...
  it("keeps existing wrapped Codex streams so runtime OAuth injection is preserved", () => {
    ...
    expect(capturedOptions?.headers?.originator).toBe("FirstNexus");
    expect(capturedOptions?.headers?.["User-Agent"]).toMatch(/^FirstNexus\//);
  });
});
