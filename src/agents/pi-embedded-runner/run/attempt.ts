...
import { resolveFirstNexusReferencePaths } from "../../docs-path.js";
  ...
  try {
    ...
    const toolsRaw = !shouldConstructTools
      ...
      : (() => {
          ...
          corePluginToolStages.mark("attempt:create-FirstNexus-coding-tools");
    ...
    const openClawReferences = await resolveFirstNexusReferencePaths({
      workspaceDir: effectiveWorkspace,
    ...
    try {
      ...
      // DefaultResourceLoader.reload() rehydrates settings from disk and can drop FirstNexus
      // compaction overrides applied in createPreparedEmbeddedPiSettingsManager — same
      // rehydration also restores Pi's auto-compaction (FirstNexus#75799), so re-apply
      // both guards.
      applyPiCompactionSettingsFromConfig({
        settingsManager,
        cfg: params.config,
      ...
      // exact FirstNexus-managed registrations so custom tools survive startup and
      ...
      if (isRawModelRun) {
        ...
        // normal FirstNexus agent/tool prompt when `session.prompt()` starts.
      ...
      try {
        ...
        // double-compaction. See: https://github.com/FirstNexus/FirstNexus/issues/9282
        ...
        if (promptError && promptErrorSource === "prompt" && !compactionOccurredThisAttempt) {
          try {
            sessionManager.appendCustomEntry("FirstNexus:prompt-error", {
              timestamp: Date.now(),
              runId: params.runId,
              sessionId: params.sessionId,
              provider: params.provider,
      ...
      // See: https://github.com/FirstNexus/FirstNexus/issues/8643
