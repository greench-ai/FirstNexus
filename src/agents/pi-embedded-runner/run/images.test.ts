...
[media attached 1/2: /Users/tyleryust/.FirstNexus/media/IMG_6430.jpeg (image/jpeg)]
[media attached 2/2: /Users/tyleryust/.FirstNexus/media/IMG_6431.jpeg (image/jpeg)]
what about these images?`,
    ...
    expect(refs).toStrictEqual([
      {
        raw: "/Users/tyleryust/.FirstNexus/media/IMG_6430.jpeg",
        type: "path",
        resolved: "/Users/tyleryust/.FirstNexus/media/IMG_6430.jpeg",
      },
      {
        raw: "/Users/tyleryust/.FirstNexus/media/IMG_6431.jpeg",
        type: "path",
        resolved: "/Users/tyleryust/.FirstNexus/media/IMG_6431.jpeg",
      },
    ]);
  });
...
what is this?`);
  ...
  it("handles paths with spaces in filename", () => {
    ...
    const ref =
      expectSingleImageReference(`[media attached: /Users/test/.FirstNexus/media/ChatGPT Image Apr 21, 2025.png (image/png) | https://example.com/same.png]
what is this?`);

    expect(ref).toStrictEqual({
      raw: "/Users/test/.FirstNexus/media/ChatGPT Image Apr 21, 2025.png",
      type: "path",
      resolved: "/Users/test/.FirstNexus/media/ChatGPT Image Apr 21, 2025.png",
    });
  });
...
describe("loadImageFromRef", () => {
  it("allows sandbox-validated host paths outside default media roots", async () => {
    ...
    const sandboxParent = await fs.mkdtemp(path.join(homeDir, "FirstNexus-sandbox-image-"));
    try {
...
describe("detectAndLoadPromptImages", () => {
  ...
  it("blocks prompt image refs outside workspace when sandbox workspaceOnly is enabled", async () => {
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "FirstNexus-native-image-sandbox-"));
  ...
  it("loads managed inbound absolute paths when workspaceOnly is enabled", async () => {
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "FirstNexus-native-image-managed-"));
