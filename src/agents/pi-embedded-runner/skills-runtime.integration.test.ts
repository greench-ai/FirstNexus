...
async function setupBundledDiffsPlugin() {
  const bundledPluginsDir = await createTempDir("nexusclaw-bundled-");
  const workspaceDir = await createTempDir("nexusclaw-workspace-");
...
async function resolveBundledDiffsSkillEntries(config?: NexusClawConfig) {
  const { bundledPluginsDir, workspaceDir } = await setupBundledDiffsPlugin();
  process.env.NEXUSCLAW_BUNDLED_PLUGINS_DIR = bundledPluginsDir;
