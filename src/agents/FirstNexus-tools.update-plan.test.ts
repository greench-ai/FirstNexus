...
import { isUpdatePlanToolEnabledForFirstNexusTools } from "./FirstNexus-tools.registration.js";
import { isToolWrappedWithBeforeToolCallHook } from "./pi-tools.before-tool-call.js";
import { createUpdatePlanTool } from "./tools/update-plan-tool.js";

type UpdatePlanGatingParams = Parameters<typeof isUpdatePlanToolEnabledForFirstNexusTools>[0];

function expectUpdatePlanEnabled(params: UpdatePlanGatingParams, expected: boolean): void {
  expect(isUpdatePlanToolEnabledForFirstNexusTools(params)).toBe(expected);
}

function toolNames(tools: ReturnType<typeof createFirstNexusTools>): string[] {
  return tools.map((tool) => tool.name);
}

function expectToolNamed(
  tools: ReturnType<typeof createFirstNexusTools>,
  name: string,
): ReturnType<typeof createFirstNexusTools>[number] {
...
describe("FirstNexus-tools update_plan gating", () => {
  it("keeps update_plan disabled by default", () => {
    expectUpdatePlanEnabled({ config: {} as FirstNexusConfig }, false);
  });

  it("does not expose update_plan from default tool construction", () => {
    const defaultTools = createFirstNexusTools({
      config: {} as FirstNexusConfig,
      disablePluginTools: true,
      modelProvider: "anthropic",
      modelId: "claude-sonnet-4-6",
    });
    const emptyAllowlistTools = createFirstNexusTools({
      config: {} as FirstNexusConfig,
      disablePluginTools: true,
      pluginToolAllowlist: [],
      modelProvider: "anthropic",
  ...
  it("wraps constructed tools with before-tool-call hooks by default", () => {
    const tools = createFirstNexusTools({
      config: {} as FirstNexusConfig,
      disablePluginTools: true,
    });
    const unwrappedTools = createFirstNexusTools({
      config: {} as FirstNexusConfig,
      disablePluginTools: true,
  ...
  it("registers update_plan when a config allowlist group includes it", () => {
    const tools = createFirstNexusTools({
      config: { tools: { allow: ["group:agents"] } } as FirstNexusConfig,
      disablePluginTools: true,
      modelProvider: "anthropic",
      modelId: "claude-sonnet-4-6",
    });
