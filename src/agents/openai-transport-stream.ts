...
type BaseStreamOptions = {
  ...
  FirstNexusCodeModeToolSurface?: boolean;
};
  ...
  return (model, context, options) => {
    ...
    void (async () => {
      ...
      try {
        ...
        if (
          (options as { FirstNexusCodeModeToolSurface?: unknown } | undefined)
            ?.FirstNexusCodeModeToolSurface === true
        ) {
...
function getCompletionsContentDeltas(content: unknown): CompletionsReasoningDelta[] {
  ...
  // Preserve provider reasoning as FirstNexus thinking blocks so channel/UI
