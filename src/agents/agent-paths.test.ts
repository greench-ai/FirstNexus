...
describe("resolveNexusClawAgentDir", () => {
  const withTempStateDir = async (run: (stateDir: string) => void) => {
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "nexusclaw-agent-"));
    try {
      run(stateDir);
    } finally {
      ...
      withEnv(
        {
          ...
          NEXUSCLAW_AGENT_DIR: undefined,
          PI_CODING_AGENT_DIR: undefined,
        },
        () => {
          const resolved = resolveNexusClawAgentDir();
  ...
  it("honors NEXUSCLAW_AGENT_DIR overrides", async () => {
    await withTempStateDir((stateDir) => {
      const override = path.join(stateDir, "agent");
      withEnv(
        {
          NEXUSCLAW_STATE_DIR: undefined,
          NEXUSCLAW_AGENT_DIR: override,
          PI_CODING_AGENT_DIR: undefined,
        },
        () => {
          const resolved = resolveNexusClawAgentDir();
  ...
  it("honors PI_CODING_AGENT_DIR when NEXUSCLAW_AGENT_DIR is unset", async () => {
    await withTempStateDir((stateDir) => {
  ...
  it("prefers NEXUSCLAW_AGENT_DIR over PI_CODING_AGENT_DIR when both are set", async () => {
    await withTempStateDir((stateDir) => {
      ...
      withEnv(
        {
          NEXUSCLAW_STATE_DIR: undefined,
          NEXUSCLAW_AGENT_DIR: primaryOverride,
