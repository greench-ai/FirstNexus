...
import { FirstNexusStdioClientTransport } from "./mcp-stdio-transport.js";
...
describe("FirstNexusStdioClientTransport", () => {
  afterEach(() => {
    vi.useRealTimers();
    spawnMock.mockReset();
  ...
  it("starts stdio MCP servers in a disposable process group on POSIX", async () => {
    ...
    const transport = new FirstNexusStdioClientTransport({
      command: "npx",
      args: ["-y", "example-mcp"],
      env: { EXAMPLE: "1" },
      cwd: "/tmp/example",
  ...
  it("kills the process tree when graceful stdio close does not exit", async () => {
    ...
    const transport = new FirstNexusStdioClientTransport({ command: "npx" });
    const started = transport.start();
    child.emit("spawn");
    await started;
