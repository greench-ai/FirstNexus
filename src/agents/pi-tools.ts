...
function applyModelProviderToolPolicy(
  ...
  // Models with a native web_search tool cannot receive NexusClaw's
...
function resolveExecConfig(params: { cfg?: NexusClawConfig; agentId?: string }) {
...
export function createNexusClawCodingTools(options?: {
  agentId?: string;
  ...
  const base = (codingTools as unknown as AnyAgentTool[]).flatMap((tool) => {
    if (tool.name === readTool.name) {
      ...
      const wrapped = createNexusClawReadTool(freshReadTool, {
        modelContextWindowTokens: options?.modelContextWindowTokens,
        imageSanitization,
      });
