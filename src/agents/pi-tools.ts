...
function resolveExecConfig(params: { cfg?: FirstNexusConfig; agentId?: string }) {
...
export type FirstNexusCodingToolConstructionPlan = {
  includeBaseCodingTools: boolean;
  includeShellTools: boolean;
  includeChannelTools: boolean;
  includeFirstNexusTools: boolean;
  includePluginTools: boolean;
};

export function createFirstNexusCodingTools(options?: {
  agentId?: string;
  ...
  modelCompat?: ModelCompatConfig;
  /** If false, keep FirstNexus web_search even when a provider-native search tool is active. */
  suppressManagedWebSearch?: boolean;
  /**
  ...
  toolConstructionPlan?: FirstNexusCodingToolConstructionPlan;
  ...
  const toolConstructionPlan = options?.toolConstructionPlan ?? {
    ...
    includeFirstNexusTools: includeCoreTools,
  ...
  const includeFirstNexusTools = includeCoreTools && toolConstructionPlan.includeFirstNexusTools;
  ...
  if (includeBaseCodingTools) {
    for (const tool of createCodingTools(workspaceRoot) as unknown as AnyAgentTool[]) {
      if (tool.name === "read") {
        ...
        const wrapped = createFirstNexusReadTool(freshReadTool, {
          modelContextWindowTokens: options?.modelContextWindowTokens,
          imageSanitization,
        });
  ...
  const pluginToolsOnly =
    includeFirstNexusTools || !includePluginTools
      ? []
  ...
  options?.recordToolPrepStage?.("FirstNexus-tools");
