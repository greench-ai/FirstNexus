...
describe("subagent registry persistence", () => {
  const envSnapshot = captureEnv(["NEXUSCLAW_STATE_DIR"]);
  ...
  const writePersistedRegistry = async (
    ...
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "nexusclaw-subagent-"));
  ...
  it("uses isolated temp state when NEXUSCLAW_STATE_DIR is unset in tests", async () => {
    ...
    expect(registryPath).toContain(path.join(os.tmpdir(), "nexusclaw-test-state"));
  });
});
