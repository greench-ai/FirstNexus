...
const GOOGLE_LIVE_SINGLE_KEY = "NEXUSCLAW_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "NEXUSCLAW_LIVE_ANTHROPIC_KEY",
    listVar: "NEXUSCLAW_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "NEXUSCLAW_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `NEXUSCLAW_LIVE_${base}_KEY`;
