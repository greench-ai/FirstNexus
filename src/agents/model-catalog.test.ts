...
let ensureFirstNexusModelsJsonMock: ReturnType<typeof vi.fn>;
...
describe("loadModelCatalog", () => {
  beforeAll(async () => {
    ...
    ensureFirstNexusModelsJsonMock = vi.fn().mockResolvedValue({ agentDir: "/tmp", wrote: false });
    vi.doMock("./models-config.js", () => ({
      ensureFirstNexusModelsJson: ensureFirstNexusModelsJsonMock,
    }));
    vi.doMock("./agent-scope.js", () => ({
      resolveDefaultAgentDir: () => "/tmp/FirstNexus",
    }));
  ...
  it("reloads dynamic registry entries after clearing the cache", async () => {
    ...
    const first = await loadModelCatalog({ config: {} as FirstNexusConfig });
    expect(first).toStrictEqual([
      {
        id: "existing",
        name: "Existing",
    ...
    const second = await loadModelCatalog({ config: {} as FirstNexusConfig });
  ...
  it("returns partial results on discovery errors", async () => {
    ...
    try {
      ...
      const result = await loadModelCatalog({ config: {} as FirstNexusConfig });
  ...
  it("filters suppressed built-ins from persisted read-only catalog rows", async () => {
    ...
    const result = await loadModelCatalog({ config: {} as FirstNexusConfig, readOnly: true });
  ...
  it("loads manifest catalog rows from the current metadata snapshot without provider runtime", () => {
    ...
    const result = loadManifestModelCatalog({ config: {} as FirstNexusConfig });
  ...
  it("passes explicit env when checking current manifest catalog snapshot compatibility", () => {
    const env = { HOME: "/tmp/FirstNexus-model-catalog-env" } as NodeJS.ProcessEnv;
