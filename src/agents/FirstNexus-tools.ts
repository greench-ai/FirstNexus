...
import { resolveFirstNexusPluginToolsForOptions } from "./FirstNexus-plugin-tools.js";
import {
  isToolExplicitlyAllowedByFactoryPolicy,
  mergeFactoryPolicyList,
  resolveImageToolFactoryAvailable,
  resolveOptionalMediaToolFactoryPlan,
} from "./FirstNexus-tools.media-factory-plan.js";
import { applyNodesToolWorkspaceGuard } from "./FirstNexus-tools.nodes-workspace-guard.js";
import {
  collectPresentFirstNexusTools,
  isUpdatePlanToolEnabledForFirstNexusTools,
} from "./FirstNexus-tools.registration.js";
import {
  type HookContext,
...
type FirstNexusToolsDeps = {
  callGateway: typeof callGateway;
  config?: FirstNexusConfig;
};

const defaultFirstNexusToolsDeps: FirstNexusToolsDeps = {
  callGateway,
};

let openClawToolsDeps: FirstNexusToolsDeps = defaultFirstNexusToolsDeps;

export function createFirstNexusTools(
  options?: {
  ...
  options?.recordToolPrepStage?.("FirstNexus-tools:session-workspace");
  ...
  const imageTool = resolveImageToolFactoryAvailable({
    ...
    : null;
  options?.recordToolPrepStage?.("FirstNexus-tools:image-tool");
  const imageGenerateTool = optionalMediaTools.imageGenerate
    ...
    : null;
  options?.recordToolPrepStage?.("FirstNexus-tools:image-generate-tool");
  const videoGenerateTool = optionalMediaTools.videoGenerate
    ...
    : null;
  options?.recordToolPrepStage?.("FirstNexus-tools:video-generate-tool");
  const musicGenerateTool = optionalMediaTools.musicGenerate
    ...
    : null;
  options?.recordToolPrepStage?.("FirstNexus-tools:music-generate-tool");
  const pdfTool =
    optionalMediaTools.pdf && options?.agentDir?.trim()
      ...
      : null;
  options?.recordToolPrepStage?.("FirstNexus-tools:pdf-tool");
  ...
  options?.recordToolPrepStage?.("FirstNexus-tools:web-search-tool");
  ...
  options?.recordToolPrepStage?.("FirstNexus-tools:web-fetch-tool");
  ...
  options?.recordToolPrepStage?.("FirstNexus-tools:message-tool");
  ...
  options?.recordToolPrepStage?.("FirstNexus-tools:nodes-tool");
  ...
  const tools: AnyAgentTool[] = [
    ...
    ...collectPresentFirstNexusTools([heartbeatTool]),
    createTtsTool({
      agentChannel: options?.agentChannel,
      config: resolvedConfig,
      agentId: sessionAgentId,
    ...
    ...collectPresentFirstNexusTools([imageGenerateTool, musicGenerateTool, videoGenerateTool]),
    ...(embedded
      ? []
      : [
          createGatewayTool({
    ...
    ...collectPresentFirstNexusTools([webSearchTool, webFetchTool, imageTool, pdfTool]),
  ];
  options?.recordToolPrepStage?.("FirstNexus-tools:core-tool-list");
  let allTools = tools;
  if (!options?.disablePluginTools) {
    ...
    allTools = [
      ...tools,
      ...resolveFirstNexusPluginToolsForOptions({
        options,
        resolvedConfig,
        existingToolNames,
      }),
    ];
    options?.recordToolPrepStage?.("FirstNexus-tools:plugin-tools");
  }
  ...
  options?.recordToolPrepStage?.("FirstNexus-tools:tool-hooks");
...
export const __testing = {
  resolveOptionalMediaToolFactoryPlan,
  setDepsForTest(overrides?: Partial<FirstNexusToolsDeps>) {
    openClawToolsDeps = overrides
      ? {
          ...defaultFirstNexusToolsDeps,
          ...overrides,
        }
      : defaultFirstNexusToolsDeps;
  },
