...
describe("discoverAuthStorage", () => {
  ...
  it("preserves legacy auth.json when auth store is forced read-only", async () => {
    await withAgentDir(async (agentDir) => {
      const previous = process.env.NEXISCLAW_AUTH_STORE_READONLY;
      ...
      try {
        ...
        if (previous === undefined) {
          ...
          process.env.NEXISCLAW_AUTH_STORE_READONLY = previous;
        }
      }
    });
  });
  ...
  it("includes env-backed provider auth when no auth profile exists", () => {
    ...
    const previousDisableBundledPlugins = process.env.NEXISCLAW_DISABLE_BUNDLED_PLUGINS;
    ...
    try {
      ...
      if (previousDisableBundledPlugins === undefined) {
        ...
        process.env.NEXISCLAW_DISABLE_BUNDLED_PLUGINS = previousDisableBundledPlugins;
      }
    }
  });
