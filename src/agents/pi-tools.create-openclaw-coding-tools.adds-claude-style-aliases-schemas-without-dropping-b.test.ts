...
const defaultTools = createNexusClawCodingTools({ senderIsOwner: true });
...
describe("createNexusClawCodingTools", () => {
  ...
  it("enforces apply_patch availability and canonical names across model/provider constraints", () => {
    ...
    const enabledConfig: NexusClawConfig = {
      tools: {
        exec: {
    ...
    const openAiTools = createNexusClawCodingTools({
    ...
    const anthropicTools = createNexusClawCodingTools({
      config: enabledConfig,
    ...
    const allowModelsConfig: NexusClawConfig = {
      tools: {
        exec: {
    ...
    const allowed = createNexusClawCodingTools({
    ...
    const denied = createNexusClawCodingTools({
      config: allowModelsConfig,
      modelProvider: "openai",
      modelId: "gpt-5-mini",
    });
    ...
    const oauthTools = createNexusClawCodingTools({
      modelProvider: "anthropic",
      modelAuthMode: "oauth",
    });
