...
describe("workspace path resolution", () => {
  it("resolves relative read/write/edit paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("FirstNexus-ws-", async (workspaceDir) => {
      await withTempDir("FirstNexus-cwd-", async (otherDir) => {
        const cwdSpy = vi.spyOn(process, "cwd").mockReturnValue(otherDir);
        try {
          const tools = createFirstNexusCodingTools({ workspaceDir });
          ...
          await editTool.execute("ws-edit", {
            path: editFile,
            edits: [{ oldText: "world", newText: "FirstNexus" }],
          });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("FirstNexus-ws-", async (workspaceDir) => {
      await withTempDir("FirstNexus-override-", async (overrideDir) => {
        const execTool = createExecTool(workspaceDir);
  ...
  it("rejects @-prefixed absolute paths outside workspace when workspaceOnly is enabled", async () => {
    await withTempDir("FirstNexus-ws-", async (workspaceDir) => {
      const cfg: FirstNexusConfig = { tools: { fs: { workspaceOnly: true } } };
      const tools = createFirstNexusCodingTools({ workspaceDir, config: cfg });
      const { readTool } = expectReadWriteEditTools(tools);

      const outsideAbsolute = path.resolve(path.parse(workspaceDir).root, "outside-FirstNexus.txt");
      await expect(
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("FirstNexus-sandbox-", async (sandboxDir) => {
      await withTempDir("FirstNexus-workspace-", async (workspaceDir) => {
        ...
        const tools = createFirstNexusCodingTools({ workspaceDir, sandbox });
        const { readTool, writeTool, editTool } = expectReadWriteEditTools(tools);
