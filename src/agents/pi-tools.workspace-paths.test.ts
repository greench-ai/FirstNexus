...
describe("workspace path resolution", () => {
  it("resolves relative read/write/edit paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("nexusclaw-ws-", async (workspaceDir) => {
      await withTempDir("nexusclaw-cwd-", async (otherDir) => {
        const cwdSpy = vi.spyOn(process, "cwd").mockReturnValue(otherDir);
        try {
          const tools = createNexusClawCodingTools({ workspaceDir });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("nexusclaw-ws-", async (workspaceDir) => {
      await withTempDir("nexusclaw-override-", async (overrideDir) => {
        const execTool = createExecTool(workspaceDir);
  ...
  it("rejects @-prefixed absolute paths outside workspace when workspaceOnly is enabled", async () => {
    await withTempDir("nexusclaw-ws-", async (workspaceDir) => {
      const cfg: NexusClawConfig = { tools: { fs: { workspaceOnly: true } } };
      const tools = createNexusClawCodingTools({ workspaceDir, config: cfg });
      const { readTool } = expectReadWriteEditTools(tools);

      const outsideAbsolute = path.resolve(path.parse(workspaceDir).root, "outside-nexusclaw.txt");
      await expect(
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("nexusclaw-sandbox-", async (sandboxDir) => {
      await withTempDir("nexusclaw-workspace-", async (workspaceDir) => {
        ...
        const tools = createNexusClawCodingTools({ workspaceDir, sandbox });
        const { readTool, writeTool, editTool } = expectReadWriteEditTools(tools);
