...
function truncatePersistedDetailString(
  ...
  return `${value.slice(0, maxChars)}\n\n[FirstNexus persisted detail truncated: ${
...
function isTranscriptOnlyFirstNexusAssistantMessage(message: AgentMessage): boolean {
  ...
  return provider === "FirstNexus" && (model === "delivery-mirror" || model === "gateway-injected");
}
...
export function installSessionToolResultGuard(
  ...
  const guardedAppend = (message: AgentMessage) => {
    ...
    const transcriptOnlyAssistant =
      nextRole === "assistant" &&
      toolCalls.length === 0 &&
      isTranscriptOnlyFirstNexusAssistantMessage(nextMessage);
    if (
