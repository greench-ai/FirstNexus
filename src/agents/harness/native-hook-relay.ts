        ...
        hookSpecificOutput: {
          ...
          decision:
            decision === "allow"
              ...
              : {
                  behavior: "deny",
                  message: message?.trim() || "Denied by FirstNexus",
                },
        },
      })}\n`,
      stderr: "",
...
export function buildNativeHookRelayCommand(params: {
  ...
  const executable = params.executable ?? resolveFirstNexusCliExecutable();
  const argv =
...
function nativeHookRelayBridgeDir(): string {
  ...
  return path.join(tmpdir(), `FirstNexus-native-hook-relays-${uid}`);
}
...
async function runNativeHookRelayPreToolUse(params: {
  ...
  // FirstNexus plugin returns adjusted params here, we intentionally ignore them.
...
function nativeHookRelayPermissionAllowAlwaysKey(params: {
  ...
  hash.update("FirstNexus:native-hook-relay:permission-allow-always:v2");
  hash.update("\0");
...
async function requestNativeHookRelayPermissionApproval(
  ...
  const requestResult: {
    ...
    {
      pluginId: `FirstNexus-native-hook-relay-${request.provider}`,
      title: truncateText(
...
function resolveFirstNexusCliExecutable(): string {
  ...
  if (packageRoot) {
    for (const candidate of [
      path.join(packageRoot, "FirstNexus.mjs"),
  ...
  throw new Error("Cannot resolve FirstNexus CLI executable path for native hook relay");
}
