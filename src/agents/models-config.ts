...
import { planNexusClawModelsJson } from "./models-config.plan.js";
...
async function buildModelsJsonFingerprint(params: {
  ...
  sourceConfigForSecrets: NexusClawConfig;
  agentDir: string;
}): Promise<string> {
...
function resolveModelsConfigInput(config?: NexusClawConfig): {
  config: NexusClawConfig;
  sourceConfigForSecrets: NexusClawConfig;
} {
...
export async function ensureNexusClawModelsJson(
  config?: NexusClawConfig,
  agentDirOverride?: string,
): Promise<{ agentDir: string; wrote: boolean }> {
  const resolved = resolveModelsConfigInput(config);
  const cfg = resolved.config;
  const agentDir = agentDirOverride?.trim() ? agentDirOverride.trim() : resolveNexusClawAgentDir();
  ...
  const pending = withModelsJsonWriteLock(targetPath, async () => {
    ...
    const plan = await planNexusClawModelsJson({
      cfg,
      sourceConfigForSecrets: resolved.sourceConfigForSecrets,
      agentDir,
      env,
