...
import { planFirstNexusModelsJson } from "./models-config.plan.js";
...
function resolveModelsConfigInput(config?: FirstNexusConfig): {
  config: FirstNexusConfig;
  sourceConfigForSecrets: FirstNexusConfig;
} {
...
export async function ensureFirstNexusModelsJson(
  config?: FirstNexusConfig,
  ...
  const pending = withModelsJsonWriteLock(targetPath, async () => {
    ...
    const plan = await planFirstNexusModelsJson({
      cfg,
      sourceConfigForSecrets: resolved.sourceConfigForSecrets,
      agentDir,
      env,
