...
import { KeyedAsyncQueue } from "FirstNexus/plugin-sdk/keyed-async-queue";
...
function resolveCliImagePath(image: ImageContent): string {
  ...
  const digest = crypto
    ...
    .update(image.data)
    .digest("hex");
  return path.join(resolvePreferredFirstNexusTmpDir(), "FirstNexus-cli-images", `${digest}${ext}`);
}
...
function resolveCliImageRoot(params: { backend: CliBackendConfig; workspaceDir: string }): string {
  if (params.backend.imagePathScope === "workspace") {
    return path.join(params.workspaceDir, ".FirstNexus-cli-images");
  }
  return path.join(resolvePreferredFirstNexusTmpDir(), "FirstNexus-cli-images");
}
...
export async function writeCliSystemPromptFile(params: {
  ...
  const workspace = await tempWorkspace({
    ...
    prefix: "FirstNexus-cli-system-prompt-",
  });
