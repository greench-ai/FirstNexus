import { createParameterFreeTool } from "FirstNexus/plugin-sdk/agent-runtime-test-contracts";
...
describe("AgentRuntimePlan", () => {
  ...
  it("defers default transport extra params until they are read", () => {
    ...
    const plan = buildAgentRuntimePlan({
      ...
      config: {},
      workspaceDir: "/tmp/FirstNexus-runtime-plan",
      model: gpt54Model,
    });
  ...
  it("keeps FirstNexus-owned tool-schema normalization reachable from the plan", () => {
    const plan = buildAgentRuntimePlan({
      provider: "openai",
      modelId: "gpt-5.4",
      modelApi: "openai-responses",
