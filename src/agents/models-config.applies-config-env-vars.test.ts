...
import {
  planFirstNexusModelsJsonWithDeps,
...
async function resolveProvidersForConfigEnvTest(params: {
  ...
  return await resolveProvidersForModelsJsonWithDeps(
    {
      cfg: params.cfg,
      agentDir: "/tmp/FirstNexus-models-config-env-vars-test",
      env,
    },
    {
...
function createConfigEnvVarsConfig(): FirstNexusConfig {
  return {
    models: { providers: {} },
    env: {
      vars: {
...
async function resolveProvidersAndCaptureDiscoveryEnv(cfg: FirstNexusConfig) {
...
describe("models-config", () => {
  ...
  it("threads workspace scope into implicit provider discovery", async () => {
    ...
    expect(observedWorkspaceDir).toBe("/tmp/FirstNexus-workspace");
  });
  ...
  it("threads plugin metadata snapshots through models.json planning", async () => {
    ...
    await planFirstNexusModelsJsonWithDeps(
      {
        cfg: { models: { providers: {} } },
        agentDir: "/tmp/FirstNexus-models-config-env-vars-test",
        env: {},
