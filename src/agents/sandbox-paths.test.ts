...
import { resolvePreferredNexusClawTmpDir } from "../infra/tmp-nexusclaw-dir.js";
...
async function withOutsideHardlinkInNexusClawTmp<T>(
  params: {
...
describe("resolveSandboxedMediaSource", () => {
  ...
  it.each([
    ...
    {
      name: "absolute paths under host tmp outside nexusclaw tmp root",
      media: path.join(os.tmpdir(), "outside-nexusclaw", "passwd"),
      expected: /sandbox/i,
    },
    {
  ...
  it("rejects symlinked NexusClaw tmp paths escaping tmp root", async () => {
  ...
  it("rejects hardlinked NexusClaw tmp paths to outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInNexusClawTmp(
      {
        openClawTmpDir,
  ...
  it("rejects symlinked NexusClaw tmp paths to hardlinked outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInNexusClawTmp(
