...
vi.mock("./models-config.runtime.js", () => ({
  ensureFirstNexusModelsJson: contextTestState.ensureFirstNexusModelsJson,
}));
...
function mockContextDeps(params: {
  ...
  contextTestState.ensureFirstNexusModelsJson.mockClear();
  return { ensureFirstNexusModelsJson: contextTestState.ensureFirstNexusModelsJson };
}
...
describe("lookupContextTokens", () => {
  ...
  it("only warms eagerly for real FirstNexus startup commands that need model metadata", async () => {
    const { shouldEagerWarmContextWindowCache } = await importContextModule();

    expect(shouldEagerWarmContextWindowCache(["node", "FirstNexus", "chat"])).toBe(true);
    expect(shouldEagerWarmContextWindowCache(["node", "FirstNexus", "chat", "--help"])).toBe(false);
    expect(
      shouldEagerWarmContextWindowCache(["node", "FirstNexus", "matrix", "encryption", "help"]),
    ).toBe(false);
    expect(shouldEagerWarmContextWindowCache(["node", "FirstNexus", "help", "matrix"])).toBe(false);
    expect(
      shouldEagerWarmContextWindowCache(["node", "FirstNexus", "browser", "status", "--help"]),
    ).toBe(false);
    expect(
    ...
    expect(shouldEagerWarmContextWindowCache(["node", "FirstNexus", "logs", "--limit", "5"])).toBe(
      false,
    );
    expect(
      shouldEagerWarmContextWindowCache(["node", "FirstNexus", "memory", "search", "--json"]),
    ).toBe(false);
    expect(shouldEagerWarmContextWindowCache(["node", "FirstNexus", "message", "read"])).toBe(
      false,
    );
    expect(shouldEagerWarmContextWindowCache(["node", "FirstNexus", "status", "--json"])).toBe(
      false,
    );
    expect(shouldEagerWarmContextWindowCache(["node", "FirstNexus", "sessions", "--json"])).toBe(
      false,
    );
    expect(
  ...
  it("skips model normalization during warmup but preserves provider-owned context metadata", async () => {
    ...
    expect(
      path
        ...
        .endsWith(path.join(".FirstNexus", "agents", "main", "agent")),
    ).toBe(true);
