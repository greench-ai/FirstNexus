...
describe("Agent-specific tool filtering", () => {
  ...
  async function withApplyPatchEscapeCase(
    ...
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "FirstNexus-pi-tools-"));
...
*** End Patch`;
  ...
  function createMainSessionTools(cfg: FirstNexusConfig) {
  ...
  function createMainAgentConfig(params: {
    tools: NonNullable<FirstNexusConfig["tools"]>;
    agentTools?: NonNullable<NonNullable<FirstNexusConfig["agents"]>["list"]>[number]["tools"];
  }): FirstNexusConfig {
    return {
      tools: params.tools,
      agents: {
