...
export function resolveDefaultAgentWorkspaceDir(
  ...
  const profile = env.NEXUSCLAW_PROFILE?.trim();
  if (profile && profile.toLowerCase() !== "default") {
    return path.join(home, ".nexusclaw", `workspace-${profile}`);
  }
  return path.join(home, ".nexusclaw", "workspace");
}
...
const WORKSPACE_STATE_DIRNAME = ".nexusclaw";
