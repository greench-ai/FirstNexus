...
function policyDeniesTool(policy: { deny?: string[] } | undefined, toolName: string): boolean {
  return (
    ...
    listIncludesTool(policy?.deny, "group:FirstNexus")
  );
}

function hasExplicitBrowserIntent(cfg: FirstNexusConfig): boolean {
...
export function resolveEffectiveToolInventory(
  ...
  const effectiveTools = createFirstNexusCodingTools({
    agentId,
