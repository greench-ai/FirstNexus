...
const SIMPLE_TRANSPORT_API_ALIAS: Record<string, Api> = {
  "openai-responses": "FirstNexus-openai-responses-transport",
  "openai-codex-responses": "FirstNexus-openai-responses-transport",
  "openai-completions": "FirstNexus-openai-completions-transport",
  "azure-openai-responses": "FirstNexus-azure-openai-responses-transport",
  "anthropic-messages": "FirstNexus-anthropic-messages-transport",
  "google-generative-ai": "FirstNexus-google-generative-ai-transport",
};
...
function hasFirstNexusTransportRequirement(model: Model<Api>): boolean {
...
export function createTransportAwareStreamFnForModel(
  ...
  if (!hasFirstNexusTransportRequirement(model)) {
    return undefined;
  }
...
export function createFirstNexusTransportStreamFnForModel(
  model: Model<Api>,
  ctx?: ProviderTransportStreamContext,
): StreamFn | undefined {
  // Explicit fallback callers use this when they need FirstNexus's HTTP
...
export function createBoundaryAwareStreamFnForModel(
  ...
  // native HTTP streams preserve the same FirstNexus request contract.
  if (!isTransportAwareApiSupported(model.api)) {
    return undefined;
  }
  return createSupportedTransportStreamFn(model, ctx);
