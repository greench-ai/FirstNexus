...
describe("createNexusClawCodingTools", () => {
  it("returns image metadata for images and text-only blocks for text files", async () => {
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "nexusclaw-read-"));
    try {
      ...
      const contents = "Hello from nexusclaw read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    ...
    const tools = createNexusClawCodingTools({ sandbox });
