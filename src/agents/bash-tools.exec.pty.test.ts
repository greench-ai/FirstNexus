...
test("exec sets NEXUSCLAW_SHELL in pty mode", async () => {
  const tool = createExecTool({ allowBackground: false, security: "full", ask: "off" });
  const result = await tool.execute("toolcall-nexusclaw-shell", {
    command: "node -e \"process.stdout.write(process.env.NEXUSCLAW_SHELL || '')\"",
    pty: true,
  });
