...
describe("prepareSimpleCompletionModel", () => {
  ...
  it("applies provider runtime auth before storing simple-completion credentials", async () => {
    ...
    expect(runtimeAuthInput.workspaceDir).toBe("/tmp/FirstNexus-agent");
...
describe("completeWithPreparedSimpleCompletionModel", () => {
  it("prepares provider-owned stream APIs before running a completion", async () => {
    ...
    const preparedModel = {
      ...model,
      api: "FirstNexus-ollama-simple-test",
    };
    const cfg = {
  ...
  it("normalizes FirstNexus-only thinking levels before using pi-ai simple completion", async () => {
    const model = {
      provider: "openai",
      id: "gpt-5.4",
      name: "gpt-5.4",
