...
describe("provider transport stream contracts", () => {
    ...
    const cases = [
      {
        ...
        alias: "FirstNexus-openai-responses-transport",
      },
      {
      ...
      },
      {
        ...
        alias: "FirstNexus-openai-completions-transport",
      },
      {
        ...
        alias: "FirstNexus-azure-openai-responses-transport",
      },
      {
        ...
        alias: "FirstNexus-anthropic-messages-transport",
      },
      {
        ...
        alias: "FirstNexus-google-generative-ai-transport",
  ...
  it("keeps OpenAI API-key default streams on FirstNexus transport", () => {
    const cases = [
    ...
    for (const model of cases) {
      expect(createBoundaryAwareStreamFnForModel(model)).toBeTypeOf("function");
      expect(createFirstNexusTransportStreamFnForModel(model)).toBeTypeOf("function");
      expect(createTransportAwareStreamFnForModel(model)).toBeUndefined();
      expect(buildTransportAwareSimpleStreamFn(model)).toBeUndefined();
      expect(prepareTransportAwareSimpleModel(model)).toBe(model);
    }
  ...
  it("routes localService models through the FirstNexus simple-completion transport", () => {
    ...
    expect(buildTransportAwareSimpleStreamFn(model)).toBeTypeOf("function");
    const preparedModel = prepareTransportAwareSimpleModel(model);
    expect(preparedModel.api).toBe("FirstNexus-openai-completions-transport");
    expect(preparedModel.provider).toBe("inferrs");
    expect(preparedModel.id).toBe("google/gemma-4-E2B-it");
  });

  it("keeps Codex defaults on the FirstNexus transport until PI preserves attribution", () => {
