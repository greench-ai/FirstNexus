...
async function withTempSessionLockFile(
  ...
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "FirstNexus-lock-"));
  try {
...
describe("acquireSessionWriteLock", () => {
  ...
  it("cleans stale .jsonl lock files in sessions directories", async () => {
    ...
    try {
      ...
      const result = await cleanStaleLockFiles({
        ...
        nowMs,
        removeStale: true,
        readOwnerProcessArgs: () => ["node", "/opt/FirstNexus/FirstNexus.mjs", "agent"],
      });
  ...
  it("cleans fresh live .jsonl lock files owned by a non-FirstNexus process", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "FirstNexus-lock-"));
    const sessionsDir = path.join(root, "sessions");
    await fs.mkdir(sessionsDir, { recursive: true });
      ...
      expect(lockCleanupRecords(result.locks)).toEqual([
        {
          ...
          removed: true,
          stale: true,
          staleReasons: ["non-FirstNexus-owner"],
        },
      ]);
      expect(lockCleanupRecords(result.cleaned)).toEqual([
        {
  ...
  it("cleans fresh live .jsonl lock files owned by generic non-FirstNexus entrypoints", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "FirstNexus-lock-"));
    const sessionsDir = path.join(root, "sessions");
    await fs.mkdir(sessionsDir, { recursive: true });
  ...
  it("keeps fresh live .jsonl lock files with FirstNexus or unknown owners", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "FirstNexus-lock-"));
    const sessionsDir = path.join(root, "sessions");
    await fs.mkdir(sessionsDir, { recursive: true });

    const nowMs = Date.now();
    const FirstNexusLock = path.join(sessionsDir, "FirstNexus-live.jsonl.lock");
    ...
    try {
      ...
      const FirstNexusResult = await cleanStaleLockFiles({
        sessionsDir,
        staleMs: 30_000,
        nowMs,
        removeStale: true,
      ...
      expect(FirstNexusResult.cleaned).toEqual([]);
      await expect(fs.access(FirstNexusLock)).resolves.toBeUndefined();

      await fs.rm(FirstNexusLock, { force: true });
      await fs.writeFile(
        gatewayLock,
        JSON.stringify({
          pid: process.pid,
  ...
  it("removes held locks on termination signals", async () => {
    ...
    try {
      for (const signal of signals) {
        const root = await fs.mkdtemp(path.join(os.tmpdir(), "FirstNexus-lock-cleanup-"));
        try {
          const sessionFile = path.join(root, "sessions.json");
          const lockPath = `${sessionFile}.lock`;
