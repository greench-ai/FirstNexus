...
import { resolveOptionalMediaToolFactoryPlan } from "./FirstNexus-tools.media-factory-plan.js";
...
type CreateFirstNexusToolsOptions = Parameters<
  typeof import("./FirstNexus-tools.js").createFirstNexusTools
>[0];

async function createFirstNexusToolsForTest(options?: CreateFirstNexusToolsOptions) {
  const { createFirstNexusTools } = await import("./FirstNexus-tools.js");
  return createFirstNexusTools(options);
}
...
function createPlugin(params: {
  ...
  return {
    ...
    manifestPath: `/plugins/${params.id}/FirstNexus.plugin.json`,
    channels: [],
    providers: [],
    cliBackends: [],
    skills: [],
...
function legacyModelProviderConfig(provider: Record<string, unknown>): FirstNexusConfig {
  return {
    models: {
      providers: {
...
describe("optional media tool factory planning", () => {
  ...
  it("preserves implicit allow-all from alsoAllow-only policies for built-in media factories", async () => {
    ...
    const toolNames = (
      await createFirstNexusToolsForTest({
        config,
  ...
  it("applies global tool policy before optional media factories run", () => {
    const config: FirstNexusConfig = { tools: { deny: ["pdf"] } };
    installSnapshot(config, [
      createPlugin({
        id: "media-owner",
        contracts: { mediaUnderstandingProviders: ["anthropic"] },
  ...
  it("defers PDF model resolution from the tool-prep hot path", async () => {
    ...
    const tools = await createFirstNexusToolsForTest({
      config,
      agentDir: "/tmp/FirstNexus-agent-main",
