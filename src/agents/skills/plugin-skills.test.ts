    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.acpxRoot, "nexusclaw.plugin.json"),
      },
      {
        id: "helper",
        name: "Helper",
        ...
        manifestPath: path.join(params.helperRoot, "nexusclaw.plugin.json"),
      },
    ],
  };
}
    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.pluginRoot, "nexusclaw.plugin.json"),
      },
    ],
  };
}
...
async function setupAcpxAndHelperRegistry() {
  const workspaceDir = await tempDirs.make("nexusclaw-");
  const acpxRoot = await tempDirs.make("nexusclaw-acpx-plugin-");
  const helperRoot = await tempDirs.make("nexusclaw-helper-plugin-");
...
async function setupPluginOutsideSkills() {
  const workspaceDir = await tempDirs.make("nexusclaw-");
  const pluginRoot = await tempDirs.make("nexusclaw-plugin-");
  const outsideDir = await tempDirs.make("nexusclaw-outside-");
...
describe("resolvePluginSkillDirs", () => {
  ...
  it("resolves Claude bundle command roots through the normal plugin skill path", async () => {
    const workspaceDir = await tempDirs.make("nexusclaw-");
    const pluginRoot = await tempDirs.make("nexusclaw-claude-bundle-");
