    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.acpxRoot, "FirstNexus.plugin.json"),
      },
      {
        id: "helper",
        name: "Helper",
        ...
        manifestPath: path.join(params.helperRoot, "FirstNexus.plugin.json"),
      },
    ],
  };
}
    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.pluginRoot, "FirstNexus.plugin.json"),
      },
    ],
  };
}
...
async function setupAcpxAndHelperRegistry() {
  const workspaceDir = await tempDirs.make("FirstNexus-");
  const acpxRoot = await tempDirs.make("FirstNexus-acpx-plugin-");
  const helperRoot = await tempDirs.make("FirstNexus-helper-plugin-");
...
async function setupPluginOutsideSkills() {
  const workspaceDir = await tempDirs.make("FirstNexus-");
  const pluginRoot = await tempDirs.make("FirstNexus-plugin-");
  const outsideDir = await tempDirs.make("FirstNexus-outside-");
...
describe("resolvePluginSkillDirs", () => {
  ...
  it("resolves Claude bundle command roots through the normal plugin skill path", async () => {
    const workspaceDir = await tempDirs.make("FirstNexus-");
    const pluginRoot = await tempDirs.make("FirstNexus-claude-bundle-");
  ...
  it("resolves enabled plugin skills through legacy manifest aliases", async () => {
    const workspaceDir = await tempDirs.make("FirstNexus-");
    const pluginRoot = await tempDirs.make("FirstNexus-legacy-plugin-");
    await fs.mkdir(path.join(pluginRoot, "skills"), { recursive: true });
