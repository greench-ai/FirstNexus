...
import { resolveFirstNexusMetadata, resolveSkillInvocationPolicy } from "./frontmatter.js";
...
function resolveSkillsLimits(config?: FirstNexusConfig, agentId?: string): ResolvedSkillsLimits {
...
function buildEscapedSkillPathReason(params: { source: string; candidatePath: string }): {
  ...
  if (params.source === "FirstNexus-bundled" && candidateIsSymlink) {
    return {
  ...
  if (params.source === "FirstNexus-bundled") {
    return {
...
function resolveAllowedSymlinkTargetRealPaths(config?: FirstNexusConfig): string[] {
...
function loadSkillEntries(
  ...
  const loadSkills = (params: { dir: string; source: string }): LoadedSkillRecord[] => {
    ...
    // skill directories (e.g. ~/.FirstNexus/skills/coze/koze-retrieval/SKILL.md).
...
function resolveWorkspaceSkillPromptState(
  ...
  const truncationNote = truncated
    ? `⚠️ Skills truncated: included ${skillsForPrompt.length} of ${resolvedSkills.length}${compact ? " (compact format, descriptions omitted)" : ""}. Run \`FirstNexus skills check\` to audit.`
    : compact
      ? `⚠️ Skills catalog using compact format (descriptions omitted). Run \`FirstNexus skills check\` to audit.`
      : "";
  const prompt = [
    remoteNote,
    truncationNote,
