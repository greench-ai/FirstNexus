...
const ensureFirstNexusModelsJsonMock = vi.fn(async () => ({ wrote: false }));
...
const installRunEmbeddedMocks = () => {
  ...
  vi.doMock("./models-config.js", async () => {
    ...
    return {
      ...mod,
      ensureFirstNexusModelsJson: (...args: Parameters<typeof ensureFirstNexusModelsJsonMock>) =>
        ensureFirstNexusModelsJsonMock(...args),
    };
  });
};
...
beforeAll(async () => {
  ...
  e2eWorkspace = await createEmbeddedPiRunnerTestWorkspace("FirstNexus-embedded-agent-");
...
beforeEach(() => {
  ...
  ensureFirstNexusModelsJsonMock.mockReset();
  ensureFirstNexusModelsJsonMock.mockResolvedValue({ wrote: false });
