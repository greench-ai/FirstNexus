...
function prependInternalEventContext(
  ...
  if (body.includes("NexusClaw runtime context (internal):")) {
    return body;
  }
...
async function prepareAgentCommandExecution(
  ...
  if (agentIdOverride) {
    ...
    if (!knownAgents.includes(agentIdOverride)) {
      throw new Error(
        `Unknown agent id "${agentIdOverrideRaw}". Use "${formatCliCommand("nexusclaw agents list")}" to see configured agents.`,
      );
    }
  }
