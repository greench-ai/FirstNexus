...
vi.mock("../plugins/manifest-registry.js", async () => {
  ...
  return {
    loadPluginManifestRegistry: (params: { workspaceDir?: string }) => {
      const extensionsRoot = path.join(params.workspaceDir ?? "", ".FirstNexus", "extensions");
      const plugins = [];
...
beforeAll(async () => {
  tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "FirstNexus-skills-workspace-"));
...
async function setupWorkspaceWithProsePlugin() {
  ...
  const pluginRoot = path.join(workspaceDir, ".FirstNexus", "extensions", "open-prose");
...
describe("loadWorkspaceSkillEntries", () => {
  ...
  it("loads the browser plugin automation skill when the bundled plugin is enabled", async () => {
    const workspaceDir = await createTempWorkspaceDir();
    const managedDir = path.join(workspaceDir, ".managed");
    const pluginRoot = path.join(workspaceDir, ".FirstNexus", "extensions", "browser");
  ...
  it("keeps remote-eligible skills when agent filtering is active", async () => {
    ...
    await writeSkill({
      ...
      metadata: '{"FirstNexus":{"requires":{"anyBins":["missingbin","sandboxbin"]}}}',
    });
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      ...
      expect(warningLine).toContain("source=FirstNexus-workspace");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      ...
      const warningLine = String(line);
      expect(warningLine).toContain("Skipping escaped skill path outside its configured root:");
      expect(warningLine).toContain("source=FirstNexus-bundled");
