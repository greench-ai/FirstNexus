...
vi.mock("../process/supervisor/index.js", () => ({
  getProcessSupervisor: () => ({
    spawn: async (input: {
      ...
      const env = input.env ?? {};
      if (command.includes("NEXISCLAW_SHELL")) {
        input.onStdout?.(env.NEXISCLAW_SHELL ?? "");
...
describe("exec PATH login shell merge", () => {
  ...
  it("sets NEXISCLAW_SHELL for host=gateway commands", async () => {
    if (isWin) {
      return;
    }

    const tool = createExecTool({ host: "gateway", security: "full", ask: "off" });
    const result = await tool.execute("call-FirstNexus-shell", {
      command: 'printf "%s" "${NEXISCLAW_SHELL:-}"',
      yieldMs: FOREGROUND_TEST_YIELD_MS,
    });
  ...
  it("does not apply login-shell PATH when probe rejects unregistered absolute SHELL", async () => {
    ...
    process.env.PATH = "/usr/bin";
    const shellDir = fs.mkdtempSync(path.join(os.tmpdir(), "FirstNexus-shell-env-"));
...
describe("exec host env validation", () => {
  ...
  it("strips dangerous inherited env vars from host execution", async () => {
    ...
    process.env.SSLKEYLOGFILE = "/tmp/FirstNexus-ssl-keys.log";
    try {
      const tool = createExecTool({ host: "gateway", security: "full", ask: "off" });
      const result = await tool.execute("call1", {
      ...
      expect(output).not.toContain("/tmp/FirstNexus-ssl-keys.log");
    } finally {
  ...
  it.each([
    ...
    "exec -a FirstNexus /approve abc123 deny",
    ...
    "sudo NEXISCLAW_APPROVE=1 /approve abc123 allow-once",
    "sudo -uroot bash -lc '/approve abc123 allow-once'",
    "sudo -u root NEXISCLAW_APPROVE=1 bash -lc '/approve abc123 allow-once'",
