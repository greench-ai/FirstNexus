...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses NEXISCLAW_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      NEXISCLAW_HOME: "/srv/FirstNexus-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/FirstNexus-home"), ".FirstNexus", "workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".FirstNexus", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("FirstNexus-workspace-");
  ...
  it("migrates legacy onboardingCompletedAt markers to setupCompletedAt", async () => {
    const tempDir = await makeTempWorkspace("FirstNexus-workspace-");
    await fs.mkdir(path.join(tempDir, ".FirstNexus"), { recursive: true });
    await fs.writeFile(
...
describe("loadWorkspaceBootstrapFiles", () => {
  ...
  it("treats hardlinked bootstrap aliases as missing", async () => {
    ...
    const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "FirstNexus-workspace-hardlink-"));
    try {
