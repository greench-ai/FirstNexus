...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses NEXUSCLAW_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      NEXUSCLAW_HOME: "/srv/nexusclaw-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/nexusclaw-home"), ".nexusclaw", "workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".nexusclaw", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("nexusclaw-workspace-");
  ...
  it("migrates legacy onboardingCompletedAt markers to setupCompletedAt", async () => {
    const tempDir = await makeTempWorkspace("nexusclaw-workspace-");
    await fs.mkdir(path.join(tempDir, ".nexusclaw"), { recursive: true });
    await fs.writeFile(
...
describe("loadWorkspaceBootstrapFiles", () => {
  ...
  it("treats hardlinked bootstrap aliases as missing", async () => {
    ...
    const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "nexusclaw-workspace-hardlink-"));
    try {
