...
describe("runCliAgent bundle MCP e2e", () => {
  it(
    ...
    async () => {
      ...
      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "FirstNexus-cli-bundle-mcp-"));
      ...
      const pluginRoot = path.join(tempHome, ".FirstNexus", "extensions", "bundle-probe");
  ...
  it(
    ...
    async () => {
      ...
      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "FirstNexus-cli-live-cleanup-"));
      process.env.HOME = tempHome;
      process.env.USERPROFILE = tempHome;
      delete process.env.NEXISCLAW_HOME;
      delete process.env.NEXISCLAW_STATE_DIR;
