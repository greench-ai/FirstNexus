...
function findTool(name: string, config: NexusClawConfig) {
  const allTools = createNexusClawTools({ config, sandboxed: true });
...
async function prepareAndActivate(params: { config: NexusClawConfig; env?: NodeJS.ProcessEnv }) {
...
describe("nexusclaw tools runtime web metadata wiring", () => {
