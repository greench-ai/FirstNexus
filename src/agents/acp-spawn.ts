...
function resolveHeartbeatConfigForAgent(params: {
  cfg: FirstNexusConfig;
  agentId: string;
}): NonNullable<NonNullable<FirstNexusConfig["agents"]>["defaults"]>["heartbeat"] {
  ...
  if (requested) {
    ...
    if (configuredAgent && !isExplicitlyAllowedAcpAgent(params.cfg, requested)) {
      return {
        ok: false,
        error:
          `agentId "${requested}" is an FirstNexus config agent, not an ACP harness. ` +
          'Use runtime="subagent" or omit runtime for FirstNexus config agents. ' +
...
function isExplicitlyAllowedAcpAgent(cfg: FirstNexusConfig, agentId: string): boolean {
