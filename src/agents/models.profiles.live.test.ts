...
const DIRECT_ENABLED = Boolean(process.env.NEXISCLAW_LIVE_MODELS?.trim());
const REQUIRE_PROFILE_KEYS = isLiveProfileKeyModeEnabled();
const LIVE_CREDENTIAL_PRECEDENCE = REQUIRE_PROFILE_KEYS ? "profile-first" : "env-first";
const LIVE_HEARTBEAT_MS = Math.max(1_000, toInt(process.env.NEXISCLAW_LIVE_HEARTBEAT_MS, 30_000));
const LIVE_SETUP_TIMEOUT_MS = Math.max(
  1_000,
  toInt(process.env.NEXISCLAW_LIVE_SETUP_TIMEOUT_MS, 45_000),
);
const LIVE_TEST_TIMEOUT_MS = Math.max(
  1_000,
  toInt(process.env.NEXISCLAW_LIVE_TEST_TIMEOUT_MS, 60 * 60 * 1000),
);
const DEFAULT_LIVE_MODEL_CONCURRENCY = 20;
const LIVE_MODEL_CONCURRENCY = resolveLiveModelConcurrency(
  process.env.NEXISCLAW_LIVE_MODEL_CONCURRENCY,
);
const LIVE_MODELS_JSON_TIMEOUT_MS = resolveLiveModelsJsonTimeoutMs(
  process.env.NEXISCLAW_LIVE_MODELS_JSON_TIMEOUT_MS,
);
...
describeLive("live models (profile keys)", () => {
  it(
    ...
    async () => {
      ...
      if (!DIRECT_ENABLED) {
        logProgress(
          "[live-models] skipping (set NEXISCLAW_LIVE_MODELS=modern|all|<list>; all=modern)",
        );
        return;
      }
      ...
      const providers = parseProviderFilter(process.env.NEXISCLAW_LIVE_PROVIDERS);
      const providerList = providers ? [...providers] : null;
      logProgress("[live-models] resolving agent dir");
      const agentDir = resolveDefaultAgentDir(cfg);
      const rawModels = process.env.NEXISCLAW_LIVE_MODELS?.trim();
      ...
      const perModelTimeoutMs = toInt(process.env.NEXISCLAW_LIVE_MODEL_TIMEOUT_MS, 30_000);
      ...
      if (selectedCandidates.length < candidates.length) {
        logProgress(
          `[live-models] capped to ${selectedCandidates.length}/${candidates.length} via NEXISCLAW_LIVE_MAX_MODELS=${maxModels}`,
        );
      }
