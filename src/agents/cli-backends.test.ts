...
function createClaudeCliOverrideConfig(config: CliBackendConfig): FirstNexusConfig {
  return {
    agents: {
      defaults: {
        cliBackends: {
...
describe("resolveCliBackendConfig claude-cli defaults", () => {
  it("derives bypassPermissions from FirstNexus's default YOLO exec policy", () => {
    ...
    expect(resolved?.config.args).toContain("mcp__FirstNexus__*");
    ...
    expect(resolved?.config.resumeArgs).toContain("mcp__FirstNexus__*");
  ...
  it("keeps Claude permission mode unset when FirstNexus exec policy is not YOLO", () => {
    const resolved = requireCliBackendConfig("claude-cli", {
