...
function buildConfig(enableNoVnc: boolean): SandboxConfig {
  return {
    ...
    workspaceRoot: "/tmp/nexusclaw-sandboxes",
    docker: {
    ...
    browser: {
      enabled: true,
      image: "nexusclaw-sandbox-browser:bookworm-slim",
      containerPrefix: "nexusclaw-sbx-browser-",
      network: "nexusclaw-sandbox-browser",
      cdpPort: 9222,
      vncPort: 5900,
      noVncPort: 6080,
      headless: false,
...
describe("ensureSandboxBrowser create args", () => {
  ...
  it("publishes noVNC on loopback and injects noVNC password env", async () => {
    ...
    expect(envEntries).toContain("NEXUSCLAW_BROWSER_NO_SANDBOX=1");
    const passwordEntry = envEntries.find((entry) =>
      entry.startsWith("NEXUSCLAW_BROWSER_NOVNC_PASSWORD="),
    );
    expect(passwordEntry).toMatch(/^NEXUSCLAW_BROWSER_NOVNC_PASSWORD=[A-Za-z0-9]{8}$/);
  ...
  it("does not inject noVNC password env when noVNC is disabled", async () => {
    ...
    const createArgs = findDockerArgsCall(dockerMocks.execDocker.mock.calls, "create");
    const envEntries = collectDockerFlagValues(createArgs ?? [], "-e");
    expect(envEntries.some((entry) => entry.startsWith("NEXUSCLAW_BROWSER_NOVNC_PASSWORD="))).toBe(
      false,
    );
