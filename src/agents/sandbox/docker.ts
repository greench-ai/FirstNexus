...
import { markNexusClawExecEnv } from "../../infra/nexusclaw-exec-env.js";
...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "nexusclaw.sandbox=1");
  args.push("--label", `nexusclaw.sessionKey=${params.scopeKey}`);
  args.push("--label", `nexusclaw.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `nexusclaw.configHash=${params.configHash}`);
  }
  ...
  for (const [key, value] of Object.entries(markNexusClawExecEnv(envSanitization.allowed))) {
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "nexusclaw.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`nexusclaw sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`nexusclaw sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("nexusclaw sandbox recreate --all");
}
