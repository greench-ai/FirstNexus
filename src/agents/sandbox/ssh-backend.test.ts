...
function createConfig(): NexusClawConfig {
  return {
    agents: {
      defaults: {
        sandbox: {
          ...
          ssh: {
            ...
            command: "ssh",
            workspaceRoot: "/remote/nexusclaw",
...
function createSession() {
  return {
    command: "ssh",
    configPath: path.join(os.tmpdir(), "nexusclaw-test-ssh-config"),
    host: "nexusclaw-sandbox",
  };
}
...
describe("ssh sandbox backend", () => {
  ...
  it("describes runtimes via the configured ssh target", async () => {
    const result = await sshSandboxBackendManager.describeRuntime({
      entry: {
        containerName: "nexusclaw-ssh-worker-abcd1234",
        backendId: "ssh",
        runtimeLabel: "nexusclaw-ssh-worker-abcd1234",
    ...
    expect(sshMocks.runSshSandboxCommand).toHaveBeenCalledWith(
      expect.objectContaining({
        remoteCommand: expect.stringContaining("/remote/nexusclaw/nexusclaw-ssh-agent-worker"),
      }),
    );
  });
  ...
  it("creates a remote-canonical backend that seeds once and reuses ssh exec", async () => {
    ...
    const backend = await createSshSandboxBackend({
      ...
      cfg: {
        ...
        scope: "session",
        workspaceAccess: "rw",
        workspaceRoot: "~/.nexusclaw/sandboxes",
        docker: {
        ...
        browser: {
          enabled: false,
          image: "nexusclaw-browser",
          containerPrefix: "nexusclaw-browser-",
          network: "bridge",
          cdpPort: 9222,
          vncPort: 5900,
          noVncPort: 6080,
    ...
    expect(execSpec.argv.at(-1)).toContain("/remote/nexusclaw/nexusclaw-ssh-agent-worker");
