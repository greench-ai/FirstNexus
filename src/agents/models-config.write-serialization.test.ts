...
const planFirstNexusModelsJsonMock = vi.fn();
...
beforeAll(async () => {
  vi.doMock("./models-config.plan.js", () => ({
    planFirstNexusModelsJson: (...args: unknown[]) => planFirstNexusModelsJsonMock(...args),
  }));
  ...
  ({ ensureFirstNexusModelsJson } = await import("./models-config.js"));
...
describe("models-config write serialization", () => {
  it("does not reuse default workspace plugin metadata for explicit agent dirs without workspace", async () => {
    await withModelsTempHome(async (home) => {
      ...
      await ensureFirstNexusModelsJson({}, agentDir);

      const params = planFirstNexusModelsJsonMock.mock.calls.at(0)?.[0] as
  ...
  it("reuses current plugin metadata for explicit agent dirs with matching workspace", async () => {
    await withModelsTempHome(async (home) => {
      ...
      await ensureFirstNexusModelsJson({}, agentDir, { workspaceDir });
  ...
  it("writes implicit models.json into the configured default agent dir", async () => {
    await withModelsTempHome(async (home) => {
      ...
      const result = await ensureFirstNexusModelsJson(cfg);

      expect(result.agentDir).toBe(path.join(home, ".FirstNexus", "agents", "ops", "agent"));
      await expect(fs.access(path.join(result.agentDir, "models.json"))).resolves.toBeUndefined();
      await expectMissingPath(
        fs.access(path.join(home, ".FirstNexus", "agents", "main", "agent", "models.json")),
      );
    });
  });
  ...
  it("does not reuse scoped startup discovery cache for a different provider scope", async () => {
    await withModelsTempHome(async (home) => {
      planFirstNexusModelsJsonMock.mockImplementation(async () => ({ action: "skip" }));
      ...
      expect(planFirstNexusModelsJsonMock).toHaveBeenCalledTimes(2);
      const params = planFirstNexusModelsJsonMock.mock.calls.at(1)?.[0] as
        | {
  ...
  it("keeps the ready cache warm after models.json is written", async () => {
    await withModelsTempHome(async () => {
      ...
      expect(planFirstNexusModelsJsonMock).toHaveBeenCalledTimes(1);
    });
  });
  ...
  it("keeps distinct config fingerprints cached without evicting each other", async () => {
    await withModelsTempHome(async () => {
      planFirstNexusModelsJsonMock.mockImplementation(async () => ({ action: "noop" }));
      ...
      await ensureFirstNexusModelsJson(first);
      await ensureFirstNexusModelsJson(second);
      await ensureFirstNexusModelsJson(first);
  ...
  it("serializes concurrent models.json writes to avoid overlap", async () => {
    await withModelsTempHome(async () => {
      ...
      const writes = Promise.all([
        ensureFirstNexusModelsJson(first),
        ensureFirstNexusModelsJson(second),
      ]);
