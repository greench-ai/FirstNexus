 ...
 *
 * @see https://github.com/nexusclaw/nexusclaw/issues/47711
 */
...
export async function recoverOrphanedSubagentSessions(params: {
  ...
  const configChangePattern = /nexusclaw\.json|nexusclaw gateway restart|config\.patch/i;
      ...
      try {
        ...
        const resumed = await resumeOrphanedSession({
          ...
          configChangeHint: configChangeDetected
            ? "\n\n[config changes from your previous run were already applied — do not re-modify nexusclaw.json or restart the gateway]"
            : undefined,
          originalRunId: runId,
