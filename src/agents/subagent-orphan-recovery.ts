 ...
 *
 * @see https://github.com/FirstNexus/FirstNexus/issues/47711
 */
...
export async function recoverOrphanedSubagentSessions(params: {
  ...
  const configChangePattern = /FirstNexus\.json|FirstNexus gateway restart|config\.patch/i;
      ...
      try {
        ...
        const resumeResult = await resumeOrphanedSession({
          ...
          configChangeHint: configChangeDetected
            ? "\n\n[config changes from your previous run were already applied — do not re-modify FirstNexus.json or restart the gateway]"
            : undefined,
          originalRunId: runId,
