...
type CoreToolDefinition = {
  ...
  includeInFirstNexusGroup?: boolean;
};
...
function buildCoreToolGroupMap() {
  ...
  const FirstNexusTools = CORE_TOOL_DEFINITIONS.filter((tool) => tool.includeInFirstNexusGroup).map(
    (tool) => tool.id,
  );
  return {
    "group:FirstNexus": FirstNexusTools,
