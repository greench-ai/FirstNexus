...
type CoreToolDefinition = {
  ...
  includeInNexusClawGroup?: boolean;
};
...
function buildCoreToolGroupMap() {
  ...
  const nexusclawTools = CORE_TOOL_DEFINITIONS.filter((tool) => tool.includeInNexusClawGroup).map(
    (tool) => tool.id,
  );
  return {
    "group:nexusclaw": nexusclawTools,
