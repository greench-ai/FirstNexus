...
describe("prepareModelForSimpleCompletion", () => {
  ...
  it("uses a custom api alias for Anthropic Vertex simple completions", () => {
    ...
    expect(ensureCustomApiRegistered).toHaveBeenCalledWith(
      "FirstNexus-anthropic-vertex-simple:https%3A%2F%2Fus-central1-aiplatform.googleapis.com",
      "vertex-stream",
    );
    expect(result).toEqual({
      ...model,
      api: "FirstNexus-anthropic-vertex-simple:https%3A%2F%2Fus-central1-aiplatform.googleapis.com",
    });
  });
  ...
  it("uses a transport-aware custom api alias when llm request transport overrides are present", () => {
    ...
    expect(ensureCustomApiRegistered).toHaveBeenCalledWith(
      "FirstNexus-openai-responses-transport",
      "transport-stream",
    );
    expect(result).toEqual({
      ...model,
