...
function isLikelyFirstNexusCliProcess(argv: string[] = process.argv): boolean {
  const entryBasename = normalizeLowercaseStringOrEmpty(path.basename(argv[1] ?? ""));
  return (
    entryBasename === "FirstNexus" ||
    entryBasename === "FirstNexus.mjs" ||
...
export function shouldEagerWarmContextWindowCache(argv: string[] = process.argv): boolean {
  // Keep this gate tied to the real FirstNexus CLI entrypoints.
  //
  ...
  // built plugin-sdk can call ensureFirstNexusModelsJson(), which cascades into
  // plugin discovery and breaks dist/source singleton assumptions.
  if (!isLikelyFirstNexusCliProcess(argv)) {
    return false;
  }
...
function primeConfiguredContextWindows(): FirstNexusConfig | undefined {
...
function ensureContextWindowCacheLoaded(): Promise<void> {
  ...
  CONTEXT_WINDOW_RUNTIME_STATE.loadPromise = (async () => {
    try {
      await (await loadModelsConfigRuntime()).ensureFirstNexusModelsJson(cfg);
    } catch {
