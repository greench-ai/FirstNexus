...
describe("buildAgentSystemPrompt", () => {
  ...
  it("includes an FirstNexus control section", () => {
    const prompt = buildAgentSystemPrompt({
      workspaceDir: "/tmp/FirstNexus",
    });

    expect(prompt).toContain("## FirstNexus Control");
    expect(prompt).toContain("prefer `gateway` tool");
    expect(prompt).toContain("CLI lifecycle only on explicit user request");
    expect(prompt).toContain("FirstNexus gateway status|restart|start|stop");
  ...
  it("includes embed guidance only for webchat sessions", () => {
    ...
    expect(prompt).toContain(
      '[embed url="/__FirstNexus__/canvas/documents/cv_123/index.html" title="Status" height="320" /]',
    );
    expect(prompt).toContain(
  ...
  it("omits ACP harness guidance when ACP is disabled", () => {
    ...
    expect(prompt).toContain("- agents_list: List FirstNexus agent ids allowed for sessions_spawn");
  });
  ...
  it("preserves tool casing in the prompt", () => {
    ...
    expect(prompt).toContain("Docs: /tmp/FirstNexus/docs");
    expect(prompt).toContain("FirstNexus behavior/config/architecture: read local docs first.");
  });
  ...
  it("includes docs guidance when docsPath is provided", () => {
    const prompt = buildAgentSystemPrompt({
      workspaceDir: "/tmp/FirstNexus",
      docsPath: "/tmp/FirstNexus/docs",
      sourcePath: "/tmp/FirstNexus",
    });

    expect(prompt).toContain("## Documentation");
    expect(prompt).toContain("Docs: /tmp/FirstNexus/docs");
    expect(prompt).toContain("Source: /tmp/FirstNexus");
    expect(prompt).toContain("FirstNexus behavior/config/architecture: read local docs first.");
  ...
  it("falls back to public docs and GitHub source guidance when local docs are unavailable", () => {
    ...
    expect(prompt).toContain("Docs: https://docs.FirstNexus.ai");
    expect(prompt).toContain("Source: https://github.com/FirstNexus/FirstNexus");
  ...
  it("includes bootstrap instructions in system prompt when bootstrap is pending", () => {
    const prompt = buildAgentSystemPrompt({
      workspaceDir: "/tmp/FirstNexus",
      bootstrapMode: "full",
      contextFiles: [{ path: "/tmp/FirstNexus/BOOTSTRAP.md", content: "Ask who I am." }],
    });
    ...
    expect(prompt).toContain("## /tmp/FirstNexus/BOOTSTRAP.md");
  ...
  it("adds ClaudeBot self-update guidance when gateway tool is available", () => {
    ...
    expect(prompt).toContain("## FirstNexus Self-Update");
  ...
  it("describes sandboxed runtime and elevated when allowed", () => {
    ...
    expect(prompt).toContain(
      "For read/write/edit/apply_patch, file paths resolve against host workspace: /tmp/FirstNexus. For bash/exec commands, use sandbox container paths under /workspace (or relative paths from that workdir), not host paths.",
    );
...
describe("buildAgentBootstrapSystemPromptSupplement", () => {
  it("can render bootstrap guidance without duplicating Project Context", () => {
    ...
    expect(sections).not.toContain("## /tmp/FirstNexus/BOOTSTRAP.md");
  ...
  it("adds pending bootstrap guidance and BOOTSTRAP.md contents for override prompts", () => {
    ...
    expect(supplement).toContain("## /tmp/FirstNexus/BOOTSTRAP.md");
...
describe("buildSubagentSystemPrompt", () => {
  it("renders depth-1 orchestrator guidance, labels, and recovery notes", () => {
    ...
    expect(prompt).toContain("Do not use `exec` (`FirstNexus ...`, `acpx ...`)");
    expect(prompt).toContain("Use `subagents` only for FirstNexus subagents");
