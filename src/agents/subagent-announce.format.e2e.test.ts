...
let configOverride: NexusClawConfig = {
  session: {
    mainKey: "main",
    scope: "per-sender",
  },
...
function setConfigOverride(next: NexusClawConfig): void {
  configOverride = next;
...
describe("subagent announce formatting", () => {
  ...
  beforeAll(async () => {
    ...
    // See: https://github.com/nexusclaw/nexusclaw/issues/31298
    previousFastTestEnv = process.env.NEXUSCLAW_TEST_FAST;
  ...
  afterAll(() => {
    ...
    if (previousFastTestEnv === undefined) {
      delete process.env.NEXUSCLAW_TEST_FAST;
      return;
    }
    process.env.NEXUSCLAW_TEST_FAST = previousFastTestEnv;
  });

  beforeEach(() => {
    // NEXUSCLAW_TEST_FAST is set in beforeAll before module import
  ...
  it("sends instructional message to main agent with status and findings", async () => {
    ...
    expect(msg).toContain("NexusClaw runtime context (internal):");
