...
describe("provider attribution", () => {
  it("resolves the canonical NexusClaw product and runtime version", () => {
    const identity = resolveProviderAttributionIdentity({
      NEXUSCLAW_VERSION: "2026.3.99",
    });
  ...
  it("returns a documented OpenRouter attribution policy", () => {
    ...
    expect(policy).toEqual({
      ...
      reviewNote: "Documented app attribution headers. Verified in NexusClaw runtime wrapper.",
      product: "NexusClaw",
      version: "2026.3.14",
      headers: {
        "HTTP-Referer": "https://nexusclaw.ai",
        "X-OpenRouter-Title": "NexusClaw",
  ...
  it("returns a hidden-spec OpenAI attribution policy", () => {
    expect(resolveProviderAttributionPolicy("openai", { NEXUSCLAW_VERSION: "2026.3.14" })).toEqual({
      provider: "openai",
      enabledByDefault: true,
      ...
      version: "2026.3.14",
      headers: {
        originator: "nexusclaw",
        "User-Agent": "nexusclaw/2026.3.14",
      },
    });
    expect(resolveProviderAttributionHeaders("openai", { NEXUSCLAW_VERSION: "2026.3.14" })).toEqual({
      originator: "nexusclaw",
      "User-Agent": "nexusclaw/2026.3.14",
    });
  });
  ...
  it("returns a hidden-spec OpenAI Codex attribution policy", () => {
    expect(
      resolveProviderAttributionPolicy("openai-codex", { NEXUSCLAW_VERSION: "2026.3.14" }),
    ).toEqual({
  ...
  it("lists the current attribution support matrix", () => {
    expect(
      listProviderAttributionPolicies({ NEXUSCLAW_VERSION: "2026.3.14" }).map((policy) => [
        policy.provider,
