...
async function setupSandboxWorkspace(home: string): Promise<{
  ...
  const workspaceDir = join(home, "FirstNexus");
...
async function writeInboundMedia(
  ...
  const inboundDir = join(home, ".FirstNexus", "media", "inbound");
...
describe("stageSandboxMedia", () => {
  it("stages allowed media and blocks unsafe paths", async () => {
    await withSandboxMediaTempHome("FirstNexus-triggers-", async (home) => {
