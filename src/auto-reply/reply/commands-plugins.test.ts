...
const workspaceHarness = createCommandWorkspaceHarness("nexusclaw-command-plugins-");

async function createClaudeBundlePlugin(params: { workspaceDir: string; pluginId: string }) {
  const pluginDir = path.join(params.workspaceDir, ".nexusclaw", "extensions", params.pluginId);
...
function buildCfg(): NexusClawConfig {
  return {
    commands: {
      text: true,
      plugins: true,
...
describe("handleCommands /plugins", () => {
  ...
  it("lists discovered plugins and inspects plugin details", async () => {
    await withTempHome("nexusclaw-command-plugins-home-", async () => {
