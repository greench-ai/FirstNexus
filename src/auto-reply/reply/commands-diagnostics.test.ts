...
function buildDiagnosticsParams(
  ...
  return {
    cfg: { commands: { text: true } } as FirstNexusConfig,
    ctx: {
      Provider: "whatsapp",
      Surface: "whatsapp",
      CommandSource: "text",
    ...
    if (ctx.diagnosticsUploadApproved) {
      return {
        text: [
          ...
          "Session 1",
          "Channel: whatsapp",
          "FirstNexus session id: `session-1`",
      ...
      return (
        options.execResult ?? {
          ...
          details: {
            ...
            command: "FirstNexus gateway diagnostics export --json",
            cwd: "/tmp",
          },
        }
      );
...
describe("diagnostics command", () => {
  it("requests Gateway diagnostics approval without a duplicate pending chat reply", async () => {
    ...
    expect(execCall.defaults.approvalWarningText).toContain(
      "https://docs.FirstNexus.ai/gateway/diagnostics",
    );
    ...
    expect(command).not.toBe("FirstNexus gateway diagnostics export --json");
  });
  ...
  it("falls back to a visible reply when approval cannot be queued", async () => {
    ...
    expect(result?.reply?.text).toContain("https://docs.FirstNexus.ai/gateway/diagnostics");
  ...
  it("routes group diagnostics confirmations privately", async () => {
    const commandHandler = vi.fn(async () => ({
      text: [
        "Codex diagnostics sent to OpenAI servers:",
        "- channel whatsapp, FirstNexus session session-1, Codex thread codex-thread-1",
      ].join("\n"),
    }));
    registerHostTrustedReservedCommandForTest({
      name: "codex",
