...
import { createFirstNexusCodingTools } from "../../agents/pi-tools.js";
import { resolveSandboxRuntimeStatus } from "../../agents/sandbox.js";
import { buildAgentSystemPrompt } from "../../agents/system-prompt.js";
import { resolveCommandsSystemPromptBundle } from "./commands-system-prompt.js";
import type { HandleCommandsParams } from "./commands-types.js";

const { createFirstNexusCodingToolsMock } = vi.hoisted(() => ({
  createFirstNexusCodingToolsMock: vi.fn(() => []),
}));
...
vi.mock("../../agents/pi-tools.js", () => ({
  createFirstNexusCodingTools: createFirstNexusCodingToolsMock,
}));
...
describe("resolveCommandsSystemPromptBundle", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    createFirstNexusCodingToolsMock.mockClear();
    createFirstNexusCodingToolsMock.mockReturnValue([]);
  });
  ...
  it("opts command tool builds into gateway subagent binding", async () => {
    ...
    const toolParams = requireFirstArg(
      vi.mocked(createFirstNexusCodingTools),
  ...
  it("uses config-backed prompt settings for the target agent", async () => {
    ...
    createFirstNexusCodingToolsMock.mockReturnValue([{ name: "sessions_spawn" }] as never);
    const params = makeParams();
    params.cfg = {
      agents: {
        defaults: {
