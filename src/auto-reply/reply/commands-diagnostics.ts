...
import { buildCurrentFirstNexusCliCommand } from "./commands-FirstNexus-cli.js";
import {
...
const DIAGNOSTICS_DOCS_URL = "https://docs.FirstNexus.ai/gateway/diagnostics";
const GATEWAY_DIAGNOSTICS_EXPORT_JSON_LABEL = "FirstNexus gateway diagnostics export --json";
...
function buildGatewayDiagnosticsExportJsonCommand(): string {
  return buildCurrentFirstNexusCliCommand(["gateway", "diagnostics", "export", "--json"]);
}
...
function isCodexDiagnosticsUnavailableText(text: string | undefined): boolean {
  return (
    text?.startsWith("No Codex thread is attached to this FirstNexus session yet.") === true ||
    text?.startsWith(
      "Cannot send Codex diagnostics because this command did not include an FirstNexus session file.",
    ) === true
  );
}
