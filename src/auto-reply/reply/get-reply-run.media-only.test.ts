...
describe("runPreparedReply media-only handling", () => {
  ...
  it("keeps heartbeat prompts out of visible transcript prompt", async () => {
    ...
    expect(call?.transcriptCommandBody).toBe("[FirstNexus heartbeat poll]");
    expect(call?.followupRun.transcriptPrompt).toBe("[FirstNexus heartbeat poll]");
  });
  ...
  it.each([
    ...
    async (commandText, startupAction) => {
      ...
      expect(call?.transcriptCommandBody).toBe(`[FirstNexus session ${startupAction}]`);
      expect(call?.followupRun.transcriptPrompt).toBe(`[FirstNexus session ${startupAction}]`);
