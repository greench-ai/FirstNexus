...
const mocks = vi.hoisted(() => ({
  ...
  formatDoctorNonInteractiveHint: vi.fn(() => "Run: FirstNexus doctor --non-interactive"),
  ...
  triggerFirstNexusRestart: vi.fn(() => ({ ok: true, method: "launchctl" })),
}));
...
vi.mock("../../infra/restart.js", () => ({
  ...
  triggerFirstNexusRestart: mocks.triggerFirstNexusRestart,
}));
...
describe("handleRestartCommand", () => {
  beforeEach(() => {
    ...
    mocks.triggerFirstNexusRestart.mockReset();
    mocks.triggerFirstNexusRestart.mockReturnValue({ ok: true, method: "launchctl" });
  });
  ...
  it("writes a routed restart sentinel before restarting from chat", async () => {
    ...
    expect(sentinelPayload?.doctorHint).toBe("Run: FirstNexus doctor --non-interactive");
    expect(sentinelPayload?.stats).toEqual({
      mode: "gateway.restart",
      reason: "/restart",
    });
    expect(mocks.triggerFirstNexusRestart).toHaveBeenCalledTimes(1);
  });
  ...
  it("prepares the routed sentinel only when SIGUSR1 restart emits", async () => {
    ...
    try {
      ...
      expect(mocks.triggerFirstNexusRestart).not.toHaveBeenCalled();
      ...
      expect(sentinelPayload?.continuation).toEqual({
        kind: "agentTurn",
        message:
          "The gateway restart completed successfully. Tell the user FirstNexus restarted successfully and continue any pending work.",
      });
    } finally {
  ...
  it("removes the success sentinel when fallback restart fails", async () => {
    mocks.triggerFirstNexusRestart.mockReturnValueOnce({
      ok: false,
      method: "launchctl",
    });
