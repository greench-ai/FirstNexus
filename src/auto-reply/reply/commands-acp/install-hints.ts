...
export function resolveAcpInstallCommandHint(cfg: FirstNexusConfig): string {
  ...
  if (backendId === "acpx") {
    ...
    if (existsSync(workspaceLocalPath)) {
      return `FirstNexus plugins install ${workspaceLocalPath}`;
    }
    ...
    if (bundledInstallHint) {
      const localPath = bundledInstallHint.replace(/^FirstNexus plugins install /u, "");
    ...
    return "FirstNexus plugins install acpx";
  }
