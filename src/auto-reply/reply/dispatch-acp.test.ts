...
const policyMocks = vi.hoisted(() => ({
  resolveAcpDispatchPolicyError: vi.fn<(cfg: FirstNexusConfig) => AcpRuntimeError | null>(
    () => null,
  ),
  resolveAcpAgentPolicyError: vi.fn<
    (cfg: FirstNexusConfig, agent: string) => AcpRuntimeError | null
  >(() => null),
}));
...
const sessionMetaMocks = vi.hoisted(() => ({
  readAcpSessionEntry: vi.fn<
    (params: { sessionKey: string; cfg?: FirstNexusConfig }) => AcpSessionStoreEntry | null
  >(() => null),
}));
...
vi.mock("../../acp/policy.js", () => ({
  resolveAcpDispatchPolicyError: (cfg: FirstNexusConfig) =>
    policyMocks.resolveAcpDispatchPolicyError(cfg),
  resolveAcpAgentPolicyError: (cfg: FirstNexusConfig, agent: string) =>
...
function createAcpConfigWithVisibleToolTags(): FirstNexusConfig {
    ...
    sessionMetaMocks.readAcpSessionEntry.mockImplementation(
      (params: { sessionKey: string; cfg?: FirstNexusConfig }) =>
        params.sessionKey === canonicalSessionKey
          ? {
              cfg: params.cfg ?? createAcpTestConfig(),
              storePath: "/tmp/FirstNexus-session-store.json",
              sessionKey: canonicalSessionKey,
