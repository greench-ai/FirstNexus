...
const ttsMocks = vi.hoisted(() => {
  ...
  return {
    ...
    resolveTtsConfig: vi.fn((_cfg: NexusClawConfig) => ({ mode: "final" })),
  };
});
...
vi.mock("../../tts/tts.js", () => ({
  ...
  resolveTtsConfig: (cfg: NexusClawConfig) => ttsMocks.resolveTtsConfig(cfg),
}));

const noAbortResult = { handled: false, aborted: false } as const;
const emptyConfig = {} as NexusClawConfig;
...
describe("dispatchReplyFromConfig", () => {
  ...
  it("emits diagnostics when enabled", async () => {
    setNoAbort();
    const cfg = { diagnostics: { enabled: true } } as NexusClawConfig;
    const dispatcher = createDispatcher();
    const ctx = buildTestCtx({
      Provider: "slack",
      Surface: "slack",
  ...
  it("routes plugin-owned bindings to the owning plugin before generic inbound claim broadcast", async () => {
    ...
    hookMocks.registry.plugins = [{ id: "nexusclaw-codex-app-server", status: "loaded" }];
    hookMocks.runner.runInboundClaimForPluginOutcome.mockResolvedValue({
      status: "handled",
      result: { handled: true },
    });
    sessionBindingMocks.resolveByConversation.mockReturnValue({
      ...
      metadata: {
        ...
        pluginRoot: "/Users/huntharo/github/nexusclaw-app-server",
      },
  ...
  it("falls back to NexusClaw once per startup when a bound plugin is missing", async () => {
    setNoAbort();
    hookMocks.runner.hasHooks.mockImplementation(
      ((hookName?: string) =>
        hookName === "inbound_claim" || hookName === "message_received") as () => boolean,
    ...
    const replyResolver = vi.fn(async () => ({ text: "nexusclaw fallback" }) satisfies ReplyPayload);
    ...
    const firstNotice = (firstDispatcher.sendToolResult as ReturnType<typeof vi.fn>).mock
      .calls[0]?.[0] as ReplyPayload | undefined;
    expect(firstNotice?.text).toContain("Routing this message to NexusClaw instead.");
  ...
  it("falls back to NexusClaw when the bound plugin is loaded but has no inbound_claim handler", async () => {
    setNoAbort();
    hookMocks.runner.hasHooks.mockImplementation(
      ((hookName?: string) =>
        hookName === "inbound_claim" || hookName === "message_received") as () => boolean,
    ...
    const notice = (dispatcher.sendToolResult as ReturnType<typeof vi.fn>).mock.calls[0]?.[0] as
      | ReplyPayload
      | undefined;
    expect(notice?.text).toContain("Routing this message to NexusClaw instead.");
    expect(replyResolver).toHaveBeenCalledTimes(1);
    expect(hookMocks.runner.runInboundClaim).not.toHaveBeenCalled();
  });
