...
vi.mock("../../agents/workspace.js", () => ({
  DEFAULT_AGENT_WORKSPACE_DIR: "/tmp/FirstNexus-workspace",
...
describe("getReplyFromConfig fast test bootstrap", () => {
  ...
  it("fails fast on unmarked config overrides in strict fast-test mode", async () => {
    await expect(
      getReplyFromConfig(buildGetReplyCtx(), undefined, {} as FirstNexusConfig),
  ...
  it("skips getRuntimeConfig, workspace bootstrap, and session bootstrap for marked test configs", async () => {
    const home = await fs.mkdtemp(path.join(os.tmpdir(), "FirstNexus-fast-reply-"));
  ...
  it("clears stale ack-only heartbeat pending delivery before replay", async () => {
    const home = await fs.mkdtemp(path.join(os.tmpdir(), "FirstNexus-heartbeat-pending-clear-"));
  ...
  it("uses ackMaxChars when replaying stale heartbeat pending delivery", async () => {
    const home = await fs.mkdtemp(path.join(os.tmpdir(), "FirstNexus-heartbeat-pending-replay-"));
    const storePath = path.join(home, "sessions.json");
    const sessionKey = "agent:main:telegram:123";
    await fs.writeFile(
      storePath,
  ...
  it("handles native /status before workspace bootstrap", async () => {
    const home = await fs.mkdtemp(path.join(os.tmpdir(), "FirstNexus-native-status-fast-"));
    ...
    expect(reply.text.includes("FirstNexus")).toBe(true);
  ...
  it("uses configured agent thinking defaults for native /status", async () => {
    const home = await fs.mkdtemp(path.join(os.tmpdir(), "FirstNexus-native-status-agent-think-"));
    const targetSessionKey = "agent:main:telegram:123";
    const cfg = markCompleteReplyConfig({
      agents: {
        defaults: {
  ...
  it("uses the target session thinking override for native /status", async () => {
    const home = await fs.mkdtemp(path.join(os.tmpdir(), "FirstNexus-native-status-think-"));
    const storePath = path.join(home, "sessions.json");
    const targetSessionKey = "agent:main:telegram:123";
    await fs.writeFile(
      storePath,
  ...
  it("handles native slash directives before workspace bootstrap", async () => {
    const home = await fs.mkdtemp(path.join(os.tmpdir(), "FirstNexus-native-slash-fast-"));
    const targetSessionKey = "agent:main:telegram:123";
    const cfg = markCompleteReplyConfig({
      agents: {
        defaults: {
  ...
  it("uses native command target session keys during fast bootstrap", () => {
    const result = initFastReplySessionState({
      ...
      cfg: { session: { store: "/tmp/sessions.json" } } as FirstNexusConfig,
      agentId: "main",
      commandAuthorized: true,
  ...
  it("keeps the existing session for /reset newline soft during fast bootstrap", async () => {
    const home = await fs.mkdtemp(path.join(os.tmpdir(), "FirstNexus-fast-reset-newline-soft-"));
    const storePath = path.join(home, "sessions.json");
    const sessionKey = "agent:main:telegram:123";
    await fs.writeFile(
      storePath,
    ...
    const result = initFastReplySessionState({
      ...
      cfg: { session: { store: storePath } } as FirstNexusConfig,
      agentId: "main",
      commandAuthorized: true,
      workspaceDir: home,
    });
  ...
  it("keeps the existing session for /reset: soft during fast bootstrap", async () => {
    const home = await fs.mkdtemp(path.join(os.tmpdir(), "FirstNexus-fast-reset-colon-soft-"));
    const storePath = path.join(home, "sessions.json");
    const sessionKey = "agent:main:telegram:123";
    await fs.writeFile(
      storePath,
