...
describe("abort detection", () => {
  ...
  async function createAbortConfig(params?: {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "nexusclaw-abort-"));
  ...
  it("isAbortRequestText aligns abort command semantics", () => {
    ...
    expect(isAbortRequestText("stop nexusclaw!!!")).toBe(true);
    ...
    expect(isAbortRequestText("/stop@nexusclaw_bot", { botUsername: "nexusclaw_bot" })).toBe(true);
    expect(isAbortRequestText("/Stop@nexusclaw_bot", { botUsername: "nexusclaw_bot" })).toBe(true);
