...
type TempHomeEnvSnapshot = {
  ...
  nexusclawHome: string | undefined;
...
function snapshotTempHomeEnv(): TempHomeEnvSnapshot {
  return {
    ...
    nexusclawHome: process.env.NEXUSCLAW_HOME,
...
function restoreTempHomeEnv(snapshot: TempHomeEnvSnapshot): void {
  ...
  restoreKey("NEXUSCLAW_HOME", snapshot.nexusclawHome);
  restoreKey("NEXUSCLAW_STATE_DIR", snapshot.stateDir);
}
...
function setTempHomeEnv(home: string): void {
  ...
  process.env.NEXUSCLAW_STATE_DIR = join(home, ".nexusclaw");
...
beforeAll(async () => {
  suiteTempHomeRoot = await fs.mkdtemp(join(os.tmpdir(), "nexusclaw-triggers-suite-"));
});
...
export async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  ...
  await fs.mkdir(join(home, ".nexusclaw", "agents", "main", "sessions"), { recursive: true });
  setTempHomeEnv(home);
...
export function makeCfg(home: string): NexusClawConfig {
  return {
    agents: {
      defaults: {
        model: { primary: "anthropic/claude-opus-4-5" },
        workspace: join(home, "nexusclaw"),
