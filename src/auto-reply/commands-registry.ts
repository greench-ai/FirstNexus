...
export function isCommandEnabled(cfg: NexusClawConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: NexusClawConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as NexusClawConfig),
...
export function getCommandDetection(_cfg?: NexusClawConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: NexusClawConfig) {
