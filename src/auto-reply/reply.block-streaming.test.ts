...
function createReplyConfig(home: string, streamMode?: "block"): NexusClawConfig {
  return {
    agents: {
      defaults: {
...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeHarness("nexusclaw-stream-", async (home) => {
...
describe("block streaming", () => {
  beforeEach(() => {
    vi.stubEnv("NEXUSCLAW_TEST_FAST", "1");
  ...
  it("still parses media directives for direct block payloads", async () => {
    await withTempHome(async (home) => {
      ...
      expect(onBlockReply.mock.calls[0][0]).toMatchObject({
        text: "Result",
        mediaUrls: [path.join(home, "nexusclaw", "image.png")],
      });
    });
  });
});
