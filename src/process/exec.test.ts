...
describe("runCommandWithTimeout", () => {
  ...
  it("merges custom env with base env and drops undefined values", async () => {
    const resolved = resolveCommandEnv({
      argv: ["node", "script.js"],
      baseEnv: {
        NEXUSCLAW_BASE_ENV: "base",
        NEXUSCLAW_TO_REMOVE: undefined,
      },
      env: {
        NEXUSCLAW_TEST_ENV: "ok",
      },
    });

    expect(resolved.NEXUSCLAW_BASE_ENV).toBe("base");
    expect(resolved.NEXUSCLAW_TEST_ENV).toBe("ok");
    expect(resolved.NEXUSCLAW_TO_REMOVE).toBeUndefined();
    expect(resolved.NEXUSCLAW_CLI).toBe(NEXUSCLAW_CLI_ENV_VALUE);
  });
