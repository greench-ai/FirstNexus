...
describe("resolveAgentRoute", () => {
  const resolveDiscordGuildRoute = (cfg: NexusClawConfig) =>
    resolveAgentRoute({
      cfg,
      channel: "discord",
      accountId: "default",
  ...
  test("defaultAgentId is used when no binding matches", () => {
    const cfg: NexusClawConfig = {
      agents: {
        list: [{ id: "home", default: true, workspace: "~/nexusclaw-home" }],
      },
    };
    const route = resolveAgentRoute({
      cfg,
...
describe("role-based agent routing", () => {
  type DiscordBinding = NonNullable<NexusClawConfig["bindings"]>[number];
