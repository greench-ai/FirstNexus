...
function listAgents(cfg: NexusClawConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

type AgentLookupCache = {
  agentsRef: NexusClawConfig["agents"] | undefined;
  byNormalizedId: Map<string, string>;
  fallbackDefaultAgentId: string;
};

const agentLookupCacheByCfg = new WeakMap<NexusClawConfig, AgentLookupCache>();

function resolveAgentLookupCache(cfg: NexusClawConfig): AgentLookupCache {
...
export function pickFirstExistingAgentId(cfg: NexusClawConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: NexusClawConfig["bindings"];
...
const evaluatedBindingsCacheByCfg = new WeakMap<NexusClawConfig, EvaluatedBindingsCache>();
const MAX_EVALUATED_BINDINGS_CACHE_KEYS = 2000;
const resolvedRouteCacheByCfg = new WeakMap<
  NexusClawConfig,
  {
    bindingsRef: NexusClawConfig["bindings"];
    agentsRef: NexusClawConfig["agents"];
    sessionRef: NexusClawConfig["session"];
...
function resolveRouteCacheForConfig(cfg: NexusClawConfig): Map<string, ResolvedAgentRoute> {
