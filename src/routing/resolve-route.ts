...
function listAgents(cfg: FirstNexusConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

type AgentLookupCache = {
  agentsRef: FirstNexusConfig["agents"] | undefined;
  byNormalizedId: Map<string, string>;
  fallbackDefaultAgentId: string;
};

const agentLookupCacheByCfg = new WeakMap<FirstNexusConfig, AgentLookupCache>();

function resolveAgentLookupCache(cfg: FirstNexusConfig): AgentLookupCache {
...
export function pickFirstExistingAgentId(cfg: FirstNexusConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: FirstNexusConfig["bindings"];
...
const evaluatedBindingsCacheByCfg = new WeakMap<FirstNexusConfig, EvaluatedBindingsCache>();
const MAX_EVALUATED_BINDINGS_CACHE_KEYS = 2000;
const resolvedRouteCacheByCfg = new WeakMap<
  FirstNexusConfig,
  {
    bindingsRef: FirstNexusConfig["bindings"];
    agentsRef: FirstNexusConfig["agents"];
    sessionRef: FirstNexusConfig["session"];
...
function resolveRouteCacheForConfig(cfg: FirstNexusConfig): Map<string, ResolvedAgentRoute> {
