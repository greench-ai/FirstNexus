...
function stubChannelPlugin(params: {
  ...
  label: string;
  resolveAccount: (cfg: NexusClawConfig, accountId: string | null | undefined) => unknown;
  inspectAccount?: (cfg: NexusClawConfig, accountId: string | null | undefined) => unknown;
  listAccountIds?: (cfg: NexusClawConfig) => string[];
  isConfigured?: (account: unknown, cfg: NexusClawConfig) => boolean;
  isEnabled?: (account: unknown, cfg: NexusClawConfig) => boolean;
}): ChannelPlugin {
  return {
    id: params.id,
    meta: {
...
describe("security audit", () => {
  ...
  let previousNexusClawHome: string | undefined;
  ...
  const withChannelSecurityStateDir = async (fn: (tmp: string) => Promise<void>) => {
    ...
    await withEnvAsync({ NEXUSCLAW_STATE_DIR: sharedChannelSecurityStateDir }, () =>
      fn(sharedChannelSecurityStateDir),
    );
  };

  const runSharedExtensionsAudit = async (config: NexusClawConfig) => {
    return runSecurityAudit({
      config,
      includeFilesystem: true,
      includeChannelSecurity: false,
      stateDir: sharedExtensionsStateDir,
      configPath: path.join(sharedExtensionsStateDir, "nexusclaw.json"),
      execDockerRawFn: execDockerRawUnavailable,
    });
  };
...
`,
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "nexusclaw-security-audit-"));
    previousNexusClawHome = process.env.NEXUSCLAW_HOME;
    process.env.NEXUSCLAW_HOME = path.join(fixtureRoot, "home");
    await fs.mkdir(process.env.NEXUSCLAW_HOME, { recursive: true, mode: 0o700 });
  ...
  afterAll(async () => {
    if (previousNexusClawHome === undefined) {
      delete process.env.NEXUSCLAW_HOME;
    } else {
      process.env.NEXUSCLAW_HOME = previousNexusClawHome;
    }
    if (!fixtureRoot) {
      return;
    }
  ...
  it("evaluates gateway auth presence and rate-limit guardrails", async () => {
    const cases = [
          ...
          withEnvAsync(
            {
              ...
              NEXUSCLAW_GATEWAY_PASSWORD: undefined,
            },
            async () =>
              audit({
                gateway: {
      ...
      },
      {
        ...
        run: async () => {
          const sourceConfig: NexusClawConfig = {
            gateway: {
              bind: "lan",
              auth: {
                token: {
          ...
          const resolvedConfig: NexusClawConfig = {
            gateway: {
              bind: "lan",
              auth: {},
            },
  ...
  it("evaluates safeBinTrustedDirs risk findings", async () => {
    const riskyGlobalTrustedDirs =
      process.platform === "win32"
        ...
        : ["/usr/local/bin", "/tmp/nexusclaw-safe-bins"];
    const cases = [
      {
    ...
    const cases = [
      {
          ...
          return runSecurityAudit({
            ...
            execDockerRawFn: (async (args: string[]) => {
              if (args[0] === "ps") {
                return {
                  stdout: Buffer.from(
                    "nexusclaw-sbx-browser-old\nnexusclaw-sbx-browser-missing-hash\n",
                  ),
                  stderr: Buffer.alloc(0),
                  code: 0,
                };
              }
              if (args[0] === "inspect" && args.at(-1) === "nexusclaw-sbx-browser-old") {
                return {
              ...
              if (args[0] === "inspect" && args.at(-1) === "nexusclaw-sbx-browser-missing-hash") {
                return {
        ...
        assert: (res: SecurityAuditReport) => {
          ...
          expect(staleEpoch?.detail).toContain("nexusclaw-sbx-browser-old");
        },
      },
      {
        ...
        run: async () => {
          ...
          return runSecurityAudit({
            ...
            execDockerRawFn: (async (args: string[]) => {
              if (args[0] === "ps") {
                return {
                  stdout: Buffer.from("nexusclaw-sbx-browser-exposed\n"),
                  stderr: Buffer.alloc(0),
                  code: 0,
                };
              }
              if (args[0] === "inspect" && args.at(-1) === "nexusclaw-sbx-browser-exposed") {
                return {
              ...
              if (args[0] === "port" && args.at(-1) === "nexusclaw-sbx-browser-exposed") {
                return {
  ...
  it("uses symlink target permissions for config checks", async () => {
    ...
    const targetConfigPath = path.join(tmp, "managed-nexusclaw.json");
    ...
    await Promise.all(
      cases
        ...
        .map(async (testCase) => {
          ...
          const configPath = path.join(fixture.stateDir, "nexusclaw.json");
          await fs.writeFile(configPath, "{}\n", "utf-8");
          if (!isWindows) {
            await fs.chmod(configPath, 0o600);
          }
  ...
  it("scores X-Real-IP fallback risk by gateway exposure", async () => {
    const trustedProxyCfg = (trustedProxies: string[]): NexusClawConfig => ({
      gateway: {
        bind: "loopback",
  ...
  it("keeps source-configured channel security findings when resolved inspection is incomplete", async () => {
    const cases = [
      ...
      },
      {
        ...
        plugin: (sourceConfig: NexusClawConfig) =>
          stubChannelPlugin({
            id: "slack",
            label: "Slack",
            inspectAccount: (cfg) => {
  ...
  it.each([
    {
      ...
      detailIncludes: [
        ...
        "~/.nexusclaw/credentials/discord-allowFrom.json:team.owner",
      ],
  ...
  it("adds probe_failed warnings for deep probe failure modes", async () => {
    const cfg: NexusClawConfig = { gateway: { mode: "local" } };
    const cases: Array<{
      name: string;
  ...
  it("evaluates hooks ingress auth and routing findings", async () => {
    ...
    } satisfies NonNullable<NexusClawConfig["hooks"]>;
    ...
    const cases = [
      ...
      },
      {
        ...
        env: {
          NEXUSCLAW_GATEWAY_TOKEN: "shared-gateway-token-1234567890",
        },
      ...
      },
      {
        ...
        cfg: { hooks: unrestrictedBaseHooks } satisfies NexusClawConfig,
        expectedFinding: "hooks.allowed_agent_ids_unrestricted",
        expectedSeverity: "warn" as const,
      },
      {
        name: "scores unrestricted hooks.allowedAgentIds by remote exposure",
        cfg: { gateway: { bind: "lan" }, hooks: unrestrictedBaseHooks } satisfies NexusClawConfig,
        expectedFinding: "hooks.allowed_agent_ids_unrestricted",
        expectedSeverity: "critical" as const,
      },
      {
        name: "scores hooks request sessionKey override by local exposure",
        cfg: { hooks: requestSessionKeyHooks } satisfies NexusClawConfig,
  ...
  it("warns when state/config look like a synced folder", async () => {
    ...
    const res = await audit(cfg, {
      stateDir: "/Users/test/Dropbox/.nexusclaw",
      configPath: "/Users/test/Dropbox/.nexusclaw/nexusclaw.json",
    });
  ...
  it("flags group/world-readable config include files", async () => {
    ...
    const cfg: NexusClawConfig = { logging: { redactSensitive: "off" } };
    ...
    const cases = [
          ...
          runInstallMetadataAudit(
            {
              plugins: {
                installs: {
                  "voice-call": {
                    source: "npm",
                    spec: "@nexusclaw/voice-call@1.2.3",
              ...
              hooks: {
                internal: {
                  installs: {
                    "test-hooks": {
                      source: "npm",
                      spec: "@nexusclaw/test-hooks@1.2.3",
      ...
      },
      {
        ...
        run: async () => {
          ...
          await fs.writeFile(
            path.join(pluginDir, "package.json"),
            JSON.stringify({ name: "@nexusclaw/voice-call", version: "9.9.9" }),
            "utf-8",
          );
          await fs.writeFile(
            path.join(hookDir, "package.json"),
            JSON.stringify({ name: "@nexusclaw/test-hooks", version: "8.8.8" }),
            "utf-8",
          );
      ...
      },
      {
        ...
        run: async () => {
          ...
          await fs.writeFile(
            ...
            JSON.stringify({
              name: "escape-plugin",
              nexusclaw: { extensions: ["../outside.js"] },
            }),
          );
  ...
  describe("maybeProbeGateway auth selection", () => {
    ...
    const makeProbeEnv = (env?: { token?: string; password?: string }) => {
      ...
      if (env?.token !== undefined) {
        probeEnv.NEXUSCLAW_GATEWAY_TOKEN = env.token;
      }
      if (env?.password !== undefined) {
        probeEnv.NEXUSCLAW_GATEWAY_PASSWORD = env.password;
      }
      return probeEnv;
    };
