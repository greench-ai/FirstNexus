...
import type { ConfigFileSnapshot, NexusClawConfig } from "../config/config.js";
...
function isFeishuDocToolEnabled(cfg: NexusClawConfig): boolean {
  ...
  if (stateDirPerms.ok) {
    ...
    if (stateDirPerms.worldWritable) {
      findings.push({
        ...
        detail: `${formatPermissionDetail(params.stateDir, stateDirPerms)}; other users can write into your NexusClaw state.`,
      ...
      findings.push({
        ...
        detail: `${formatPermissionDetail(params.stateDir, stateDirPerms)}; group users can write into your NexusClaw state.`,
        remediation: formatPermissionRemediation({
          targetPath: params.stateDir,
          perms: stateDirPerms,
          isDir: true,
...
function collectGatewayConfigFindings(
  ...
  const envTokenConfigured =
    hasNonEmptyString(env.NEXUSCLAW_GATEWAY_TOKEN) || hasNonEmptyString(env.CLAWDBOT_GATEWAY_TOKEN);
  const envPasswordConfigured =
    hasNonEmptyString(env.NEXUSCLAW_GATEWAY_PASSWORD) ||
...
function collectBrowserControlFindings(
  ...
  try {
    ...
    findings.push({
      ...
      detail: String(err),
      remediation: `Fix browser.cdpUrl in ${resolveConfigPath()} and re-run "${formatCliCommand("nexusclaw security audit --deep")}".`,
    });
    return findings;
  }
  ...
  const tokenConfigured =
    ...
    hasNonEmptyString(env.NEXUSCLAW_GATEWAY_TOKEN) ||
...
function collectLoggingFindings(cfg: NexusClawConfig): SecurityAuditFinding[] {
...
function collectElevatedFindings(cfg: NexusClawConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
function collectExecRuntimeFindings(cfg: NexusClawConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
function collectOpenExecSurfacePaths(cfg: NexusClawConfig): string[] {
  const channels = asRecord(cfg.channels);
  if (!channels) {
    return [];
  }
...
export async function runSecurityAudit(opts: SecurityAuditOptions): Promise<SecurityAuditReport> {
  ...
  if (deep?.gateway?.attempted && !deep.gateway.ok) {
    findings.push({
      ...
      remediation: `Run "${formatCliCommand("nexusclaw status --all")}" to debug connectivity/auth, then re-run "${formatCliCommand("nexusclaw security audit --deep")}".`,
    });
  }
  if (deepProbeResult?.authWarning) {
    findings.push({
      ...
      remediation: `Set NEXUSCLAW_GATEWAY_TOKEN/NEXUSCLAW_GATEWAY_PASSWORD in this shell or resolve the external secret provider, then re-run "${formatCliCommand("nexusclaw security audit --deep")}".`,
    });
  }
