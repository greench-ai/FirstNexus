...
describe("security audit install metadata findings", () => {
  ...
  const runInstallMetadataAudit = async (cfg: FirstNexusConfig, stateDir: string) => {
  ...
  const writePluginIndexInstallRecords = async (
    ...
    const index: InstalledPluginIndex = {
      ...
      plugins: Object.keys(records).map((pluginId) => ({
        pluginId,
        manifestPath: path.join(stateDir, "extensions", pluginId, "FirstNexus.plugin.json"),
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "FirstNexus-security-install-"));
  });
              ...
              hooks: {
                internal: {
                  installs: {
                    "test-hooks": {
                      source: "npm",
                      spec: "@FirstNexus/test-hooks@1.2.3",
...
describe("security audit extension tool reachability findings", () => {
  ...
  const runSharedExtensionsAudit = async (config: FirstNexusConfig) => {
  ...
  beforeAll(async () => {
    ...
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "FirstNexus-security-extensions-"));
    ...
    const cases = [
      {
        ...
        cfg: {} satisfies FirstNexusConfig,
