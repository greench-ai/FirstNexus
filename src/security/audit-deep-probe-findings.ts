...
export function collectDeepProbeFindings(params: {
  ...
  if (params.deep?.gateway?.attempted && !params.deep.gateway.ok) {
    findings.push({
      ...
      remediation: `Run "${formatCliCommand("FirstNexus status --all")}" to debug connectivity/auth, then re-run "${formatCliCommand("FirstNexus security audit --deep")}".`,
    });
  }
  if (params.authWarning) {
    findings.push({
      ...
      remediation: `Set NEXISCLAW_GATEWAY_TOKEN/NEXISCLAW_GATEWAY_PASSWORD in this shell or resolve the external secret provider, then re-run "${formatCliCommand("FirstNexus security audit --deep")}".`,
    });
  }
  return findings;
}
