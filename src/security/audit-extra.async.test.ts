...
`,
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "FirstNexus-security-audit-async-"));
  ...
  it("flags plugin extension entry path traversal in deep audit", async () => {
    ...
    await fs.writeFile(
      ...
      JSON.stringify({
        name: "escape-plugin",
        FirstNexus: { extensions: ["../outside.js"] },
      }),
    );
  ...
  it("ignores install backup and debris dirs when scanning installed plugin roots", async () => {
    ...
    try {
      ...
      expect(findings.map((f) => f.title).join("\n")).not.toContain(".FirstNexus-install-backups");
    } finally {
      scanSpy.mockRestore();
    }
  });
