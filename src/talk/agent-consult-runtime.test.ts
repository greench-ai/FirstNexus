...
describe("realtime voice agent consult runtime", () => {
  ...
  it("runs an embedded agent using the shared session and prompt contract", async () => {
    ...
    expect(call.extraSystemPrompt).toBe(
      "You are the configured FirstNexus agent receiving delegated requests from a live voice bridge. Act on behalf of the user, use available tools when appropriate, and return a brief speakable result.",
    );
  });
