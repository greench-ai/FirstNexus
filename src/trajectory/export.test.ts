...
const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "FirstNexus-trajectory-"));
let tempDirId = 0;
  ...
  it("sanitizes session ids in default export directory names", () => {
    ...
    expect(outputDir).toBe(
      path.join(
        ...
        ".FirstNexus",
        "trajectory-exports",
        "FirstNexus-trajectory-___evil_-2026-04-22T08-00-00",
      ),
    );
  });
  ...
  it("exports merged runtime and transcript events plus convenience files", async () => {
    ...
    const runtimeEvents: TrajectoryEvent[] = [
      ...
      },
      {
        ...
        sessionId: "session-1",
        data: {
          harness: { type: "FirstNexus", version: "0.1.0" },
