...
const TEMP_MEDIA_PREFIX = "nexusclaw-media-";
...
function createGroqAudioConfig(): NexusClawConfig {
  return {
    tools: {
      media: {
        audio: {
...
function createMediaDisabledConfig(): NexusClawConfig {
  return {
    tools: {
      media: {
        audio: { enabled: false },
...
function createMediaDisabledConfigWithAllowedMimes(allowedMimes: string[]): NexusClawConfig {
  return {
...
async function setupAudioAutoDetectCase(stdout: string): Promise<{
  ...
  const cfg: NexusClawConfig = { tools: { media: { audio: {} } } };
...
describe("applyMediaUnderstanding", () => {
  beforeAll(async () => {
    ...
    const baseDir = resolvePreferredNexusClawTmpDir();
  ...
  it("skips audio auto-detect when no supported binaries or provider keys are available", async () => {
    ...
    await withMediaAutoDetectEnv(
      {
        PATH: emptyBinDir,
        NEXUSCLAW_AGENT_DIR: isolatedAgentDir,
