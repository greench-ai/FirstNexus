...
const hoisted = vi.hoisted(() => ({
  completeMock: vi.fn(),
  ensureFirstNexusModelsJsonMock: vi.fn(async () => {}),
...
const {
  completeMock,
  ensureFirstNexusModelsJsonMock,
...
describe("describeImageWithModel", () => {
  ...
  it("routes minimax-portal image models through the MiniMax VLM endpoint", async () => {
    ...
    expect(ensureFirstNexusModelsJsonMock).toHaveBeenCalled();
  ...
  it("resolves configured image models when discovery has not registered the provider", async () => {
    ...
    expect(resolveRequest.agentDir).toBe("/tmp/FirstNexus-agent");
    expect(
      requireRecord(
        requireRecord(
  ...
  it("rejects when image runtime setup exceeds the request timeout", async () => {
    vi.useFakeTimers();
    ensureFirstNexusModelsJsonMock.mockImplementationOnce(() => new Promise(() => {}));
