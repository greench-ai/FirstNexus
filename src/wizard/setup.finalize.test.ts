...
const resolveExistingKey = vi.hoisted(() =>
  vi.fn<(config: NexusClawConfig, provider: string) => string | undefined>(() => undefined),
);
const hasExistingKey = vi.hoisted(() =>
  vi.fn<(config: NexusClawConfig, provider: string) => boolean>(() => false),
);
...
const listConfiguredWebSearchProviders = vi.hoisted(() =>
  vi.fn<(params?: { config?: NexusClawConfig }) => PluginWebSearchProviderEntry[]>(() => []),
);
...
describe("finalizeSetupWizard", () => {
  ...
  it("resolves gateway password SecretRef for probe and TUI", async () => {
    ...
    process.env.NEXUSCLAW_GATEWAY_PASSWORD = "resolved-gateway-password"; // pragma: allowlist secret
