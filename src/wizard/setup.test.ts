...
const readConfigFileSnapshot = vi.hoisted(() =>
  vi.fn(async () => ({
    path: "/tmp/.nexusclaw/nexusclaw.json",
    exists: false,
...
vi.mock("../commands/onboard-helpers.js", () => ({
  DEFAULT_WORKSPACE: "/tmp/nexusclaw-workspace",
...
describe("runSetupWizard", () => {
  ...
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "nexusclaw-onboard-suite-"));
  });
  ...
  it("resolves gateway.auth.password SecretRef for local setup probe", async () => {
    ...
    process.env.NEXUSCLAW_GATEWAY_PASSWORD = "gateway-ref-password"; // pragma: allowlist secret
