...
async function requireRiskAcknowledgement(params: {
  ...
  await params.prompter.note(
    [
      ...
      "",
      "If you’re not comfortable with security hardening and access control, don’t run NexusClaw.",
      ...
      "",
      "Run regularly:",
      "nexusclaw security audit --deep",
      "nexusclaw security audit --fix",
      "",
      "Must read: https://docs.nexusclaw.ai/gateway/security",
    ].join("\n"),
    "Security",
  );
...
export async function runSetupWizard(
  ...
  await prompter.intro("NexusClaw setup");
  await requireRiskAcknowledgement({ opts, prompter });

  const snapshot = await readConfigFileSnapshot();
  let baseConfig: NexusClawConfig = snapshot.valid ? (snapshot.exists ? snapshot.config : {}) : {};
    ...
    if (snapshot.issues.length > 0) {
      await prompter.note(
        [
          ...
          "",
          "Docs: https://docs.nexusclaw.ai/gateway/configuration",
        ].join("\n"),
        "Config issues",
      );
    }
    await prompter.outro(
      `Config invalid. Run \`${formatCliCommand("nexusclaw doctor")}\` to repair it, then re-run setup.`,
    );
    runtime.exit(1);
    return;
  }
    ...
    await prompter.note(
      [
        ...
        ...(compatibilityNotices.length > 4
          ...
          : []),
        "",
        `Review: ${formatCliCommand("nexusclaw doctor")}`,
        `Inspect: ${formatCliCommand("nexusclaw plugins inspect --all")}`,
      ].join("\n"),
      "Plugin compatibility",
    );
  }

  const quickstartHint = `Configure details later via ${formatCliCommand("nexusclaw configure")}.`;
  ...
  let localGatewayToken = process.env.NEXUSCLAW_GATEWAY_TOKEN ?? process.env.CLAWDBOT_GATEWAY_TOKEN;
  try {
  ...
  let localGatewayPassword =
    process.env.NEXUSCLAW_GATEWAY_PASSWORD ?? process.env.CLAWDBOT_GATEWAY_PASSWORD;
  try {
  ...
  let nextConfig: NexusClawConfig = applyLocalSetupWorkspaceConfig(baseConfig, workspaceDir);
