...
export async function finalizeSetupWizard(
  ...
  if (!opts.skipHealth) {
    ...
    if (gatewayProbe.ok) {
      try {
        const healthConfig: FirstNexusConfig =
      ...
      await prompter.note(
        [
          ...
          `Start now: ${formatCliCommand("FirstNexus gateway run")}`,
          `Or rerun with: ${formatCliCommand("FirstNexus onboard --install-daemon")}`,
          `Or skip this probe next time: ${formatCliCommand("FirstNexus onboard --skip-health")}`,
        ].join("\n"),
        "Gateway",
      );
    }
    ...
    if (gatewayProbe.ok) {
      await prompter.note(
        [
          ...
          "Stored in: $NEXISCLAW_CONFIG_PATH (default: ~/.FirstNexus/FirstNexus.json) under gateway.auth.token, or in NEXISCLAW_GATEWAY_TOKEN.",
          `View token: ${formatCliCommand("FirstNexus config get gateway.auth.token")}`,
          `Generate token: ${formatCliCommand("FirstNexus doctor --generate-gateway-token")}`,
          "Web UI keeps dashboard URL tokens in memory for the current tab and strips them from the URL after load.",
          `Open the dashboard anytime: ${formatCliCommand("FirstNexus dashboard --no-open")}`,
    ...
    if (hatchChoice === "tui") {
      ...
      await prompter.note(
        [
          ...
          controlUiOpened
            ? "Opened in your browser. Keep that tab to control FirstNexus."
            : "Copy/paste this URL in a browser on this machine to control FirstNexus.",
          controlUiOpenHint,
        ]
          .filter(Boolean)
          .join("\n"),
      ...
      await prompter.note(
        `When you're ready: ${formatCliCommand("FirstNexus dashboard --no-open")}`,
        "Later",
      );
    }
  } else if (opts.skipUi) {
  ...
  await prompter.note(
    [
      ...
      "Docs: https://docs.FirstNexus.ai/concepts/agent-workspace",
    ].join("\n"),
    "Workspace backup",
  );

  await prompter.note(
    "Running agents on your computer is risky — harden your setup: https://docs.FirstNexus.ai/security",
    "Security",
  );
    ...
    if (!entry) {
      await prompter.note(
        [
          ...
          `  ${formatCliCommand("FirstNexus configure --section web")}`,
          "",
      ...
      await prompter.note(
        [
          ...
          `  ${formatCliCommand("FirstNexus configure --section web")}`,
          "",
          `Get your key at: ${entry?.signupUrl ?? "https://docs.FirstNexus.ai/tools/web"}`,
          "Docs: https://docs.FirstNexus.ai/tools/web",
        ].join("\n"),
        "Web search",
      );
      ...
      await prompter.note(
        [
          ...
          `Re-enable: ${formatCliCommand("FirstNexus configure --section web")}`,
          "",
          "Docs: https://docs.FirstNexus.ai/tools/web",
        ].join("\n"),
        "Web search",
  ...
  await prompter.note(
    'What now: https://FirstNexus.ai/showcase ("What People Are Building").',
    "What now",
  );

  await prompter.outro(
    controlUiOpened
      ? "Onboarding complete. Dashboard opened; keep that tab to control FirstNexus."
      : seededInBackground
        ? "Onboarding complete. Web UI seeded in the background; open it anytime with the dashboard link above."
        : "Onboarding complete. Use the dashboard link above to control FirstNexus.",
  );
