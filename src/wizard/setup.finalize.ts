...
export async function finalizeSetupWizard(
  ...
  if (!opts.skipHealth) {
    ...
    try {
      ...
      await prompter.note(
        [
          "Docs:",
          "https://docs.nexusclaw.ai/gateway/health",
          "https://docs.nexusclaw.ai/gateway/troubleshooting",
        ].join("\n"),
        "Health check help",
      );
    }
  ...
  await prompter.note(
    [
      ...
      gatewayStatusLine,
      "Docs: https://docs.nexusclaw.ai/web/control-ui",
    ]
      .filter(Boolean)
      .join("\n"),
    "Control UI",
  ...
  if (!opts.skipUi && gatewayProbe.ok) {
    ...
    await prompter.note(
      [
        ...
        "Stored in: ~/.nexusclaw/nexusclaw.json (gateway.auth.token) or NEXUSCLAW_GATEWAY_TOKEN.",
        `View token: ${formatCliCommand("nexusclaw config get gateway.auth.token")}`,
        `Generate token: ${formatCliCommand("nexusclaw doctor --generate-gateway-token")}`,
        "Web UI keeps dashboard URL tokens in memory for the current tab and strips them from the URL after load.",
        `Open the dashboard anytime: ${formatCliCommand("nexusclaw dashboard --no-open")}`,
    ...
    if (hatchChoice === "tui") {
      ...
      await prompter.note(
        [
          ...
          controlUiOpened
            ? "Opened in your browser. Keep that tab to control NexusClaw."
            : "Copy/paste this URL in a browser on this machine to control NexusClaw.",
          controlUiOpenHint,
        ]
          .filter(Boolean)
          .join("\n"),
      ...
      await prompter.note(
        `When you're ready: ${formatCliCommand("nexusclaw dashboard --no-open")}`,
        "Later",
      );
    }
  } else if (opts.skipUi) {
  ...
  await prompter.note(
    [
      ...
      "Docs: https://docs.nexusclaw.ai/concepts/agent-workspace",
    ].join("\n"),
    "Workspace backup",
  );

  await prompter.note(
    "Running agents on your computer is risky — harden your setup: https://docs.nexusclaw.ai/security",
    "Security",
  );
    ...
    if (!entry) {
      await prompter.note(
        [
          ...
          `  ${formatCliCommand("nexusclaw configure --section web")}`,
          "",
          "Docs: https://docs.nexusclaw.ai/tools/web",
        ].join("\n"),
        "Web search",
      );
      ...
      await prompter.note(
        [
          ...
          `  ${formatCliCommand("nexusclaw configure --section web")}`,
          "",
          `Get your key at: ${entry?.signupUrl ?? "https://docs.nexusclaw.ai/tools/web"}`,
          "Docs: https://docs.nexusclaw.ai/tools/web",
        ].join("\n"),
        "Web search",
      );
      ...
      await prompter.note(
        [
          ...
          `Re-enable: ${formatCliCommand("nexusclaw configure --section web")}`,
          "",
          "Docs: https://docs.nexusclaw.ai/tools/web",
        ].join("\n"),
        "Web search",
  ...
  await prompter.note(
    'What now: https://nexusclaw.ai/showcase ("What People Are Building").',
    "What now",
  );

  await prompter.outro(
    controlUiOpened
      ? "Onboarding complete. Dashboard opened; keep that tab to control NexusClaw."
      : seededInBackground
        ? "Onboarding complete. Web UI seeded in the background; open it anytime with the dashboard link above."
        : "Onboarding complete. Use the dashboard link above to control NexusClaw.",
  );
