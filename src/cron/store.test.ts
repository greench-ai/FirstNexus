...
const { makeStorePath } = createCronStoreHarness({ prefix: "nexusclaw-cron-store-" });
...
describe("resolveCronStorePath", () => {
  ...
  it("uses NEXUSCLAW_HOME for tilde expansion", () => {
    vi.stubEnv("NEXUSCLAW_HOME", "/srv/nexusclaw-home");
    vi.stubEnv("HOME", "/home/other");

    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/nexusclaw-home", "cron", "jobs.json"));
  });
});
