...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeHelper(fn, { prefix: "nexusclaw-cron-submodel-" });
}

async function writeSessionStore(home: string) {
  const dir = path.join(home, ".nexusclaw", "sessions");
...
async function runSubagentModelCase(params: {
  home: string;
  cfgOverrides?: Partial<NexusClawConfig>;
  ...
  it.each([
    {
      ...
      } satisfies Partial<NexusClawConfig>,
