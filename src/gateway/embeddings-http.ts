...
function resolveAutoExplicitProviders(cfg: FirstNexusConfig): Set<string> {
  return new Set(
...
export async function handleOpenAiEmbeddingsHttpRequest(
  ...
  if (requestModel !== NEXISCLAW_MODEL_ID && !resolveAgentIdFromModel(requestModel, cfg)) {
    sendJson(res, 400, {
      error: {
  ...
  const overrideModel =
    normalizeOptionalString(getHeader(req, "x-FirstNexus-model")) ||
