...
export function registerDefaultAuthTokenSuite(): void {
  describe("default auth (token)", () => {
    ...
    test("closes silent handshakes after timeout", async () => {
      vi.useRealTimers();
      const prevHandshakeTimeout = process.env.NEXUSCLAW_TEST_HANDSHAKE_TIMEOUT_MS;
      process.env.NEXUSCLAW_TEST_HANDSHAKE_TIMEOUT_MS = "20";
      try {
        const ws = await openWs(port);
        ...
        if (prevHandshakeTimeout === undefined) {
          delete process.env.NEXUSCLAW_TEST_HANDSHAKE_TIMEOUT_MS;
        } else {
          process.env.NEXUSCLAW_TEST_HANDSHAKE_TIMEOUT_MS = prevHandshakeTimeout;
        }
      }
    });

    test("prefers NEXUSCLAW_HANDSHAKE_TIMEOUT_MS and falls back on empty string", () => {
      const prevHandshakeTimeout = process.env.NEXUSCLAW_HANDSHAKE_TIMEOUT_MS;
      const prevTestHandshakeTimeout = process.env.NEXUSCLAW_TEST_HANDSHAKE_TIMEOUT_MS;
      process.env.NEXUSCLAW_HANDSHAKE_TIMEOUT_MS = "75";
      process.env.NEXUSCLAW_TEST_HANDSHAKE_TIMEOUT_MS = "20";
      try {
        expect(getHandshakeTimeoutMs()).toBe(75);
        process.env.NEXUSCLAW_HANDSHAKE_TIMEOUT_MS = "";
        expect(getHandshakeTimeoutMs()).toBe(20);
      } finally {
        if (prevHandshakeTimeout === undefined) {
          delete process.env.NEXUSCLAW_HANDSHAKE_TIMEOUT_MS;
        } else {
          process.env.NEXUSCLAW_HANDSHAKE_TIMEOUT_MS = prevHandshakeTimeout;
        }
        if (prevTestHandshakeTimeout === undefined) {
          delete process.env.NEXUSCLAW_TEST_HANDSHAKE_TIMEOUT_MS;
        } else {
          process.env.NEXUSCLAW_TEST_HANDSHAKE_TIMEOUT_MS = prevTestHandshakeTimeout;
        }
      }
    });
    ...
    test("does not grant admin when scopes are omitted", async () => {
      ...
      const { identity, device } = await createSignedDevice({
        ...
        identityPath: path.join(os.tmpdir(), `nexusclaw-test-device-${randomUUID()}.json`),
        nonce,
      });
