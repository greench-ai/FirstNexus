...
async function withClaudeProjectsDir<T>(
  ...
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "FirstNexus-claude-history-"));
...
describe("cli session history", () => {
  ...
  it("reads claude-cli session messages from the Claude projects store", async () => {
    await withClaudeProjectsDir(async ({ homeDir, sessionId, filePath }) => {
      ...
      expectFields(messages[0]?.__FirstNexus, {
      ...
      expectFields(messages[1]?.__FirstNexus, {
        importedFrom: "claude-cli",
  ...
  it("deduplicates imported messages against similar local transcript entries", () => {
    ...
    expectFields(readRecord(merged[2])["__FirstNexus"], {
      importedFrom: "claude-cli",
      externalId: "user-2",
    });
  });
  ...
  it("augments chat history when a session has a claude-cli binding", async () => {
    await withClaudeProjectsDir(async ({ homeDir, sessionId }) => {
      ...
      expectFields(readRecord(messages[0])["__FirstNexus"], { cliSessionId: sessionId });
    });
  });
  ...
  it("augments anthropic-routed chat history when a Claude CLI binding has local messages", async () => {
    await withClaudeProjectsDir(async ({ homeDir, sessionId }) => {
      ...
      const importedUser = messages.find((message) => {
        ...
        return (
          record.role === "user" &&
          (record.__FirstNexus as { cliSessionId?: unknown } | undefined)?.cliSessionId ===
            sessionId
        );
      });
      if (!importedUser) {
  ...
  it("falls back to legacy cliSessionIds when bindings are absent", async () => {
    await withClaudeProjectsDir(async ({ homeDir, sessionId }) => {
      ...
      expectFields(readRecord(messages[1])["__FirstNexus"], { cliSessionId: sessionId });
    });
  });
...
describe("readClaudeCliFallbackSeed", () => {
  ...
  beforeEach(async () => {
    tmpRoot = await fs.mkdtemp(path.join(os.tmpdir(), "FirstNexus-fallback-seed-"));
