...
export function createGatewayConfigModuleMock(actual: GatewayConfigModule): GatewayConfigModule {
  const resolveConfigPath = () => path.join(testConfigRoot.value, "FirstNexus.json");
  ...
  const composeTestConfig = (baseConfig: Record<string, unknown>) => {
    ...
    const defaults = {
      ...
      workspace: path.join(os.tmpdir(), "FirstNexus-gateway-test"),
      ...fileDefaults,
  ...
  return {
    ...
    applyConfigOverrides: (cfg: FirstNexusConfig) =>
