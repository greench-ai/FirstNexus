...
function cfg(input: Partial<NexusClawConfig>): NexusClawConfig {
  return input as NexusClawConfig;
}
...
async function expectFailClosedOnUnresolvedLocalAuth(config: NexusClawConfig, path: string) {
  await expect(
    resolveGatewayConnectionAuth({
      config,
...
describe("resolveGatewayConnectionAuth", () => {
  ...
  it("resolves config-first token SecretRef even when NEXUSCLAW env token exists", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
  ...
  it("resolves config-first password SecretRef even when NEXUSCLAW env password exists", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
