...
const { createNexusClawTools } = await import("../agents/nexusclaw-tools.js");
...
type SessionSendTool = ReturnType<typeof createNexusClawTools>[number];
...
function getSessionsSendTool(): SessionSendTool {
  ...
  const tool = createNexusClawTools().find((candidate) => candidate.name === "sessions_send");
  if (!tool) {
...
beforeAll(async () => {
  envSnapshot = captureEnv(["NEXUSCLAW_GATEWAY_PORT", "NEXUSCLAW_GATEWAY_TOKEN"]);
  gatewayPort = await getFreePort();
  testState.gatewayAuth = { mode: "token", token: gatewayToken };
  process.env.NEXUSCLAW_GATEWAY_PORT = String(gatewayPort);
  process.env.NEXUSCLAW_GATEWAY_TOKEN = gatewayToken;
...
describe("sessions_send label lookup", () => {
  it(
    ...
    async () => {
      ...
      if (!configPath) {
        throw new Error("NEXUSCLAW_CONFIG_PATH missing in gateway test environment");
      }
      ...
      const tool = createNexusClawTools({
        config: {
          tools: {
            sessions: {
              visibility: "all",
