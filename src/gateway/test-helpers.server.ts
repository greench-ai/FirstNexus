...
const GATEWAY_TEST_ENV_KEYS = [
  ...
  "USERPROFILE",
  "NEXUSCLAW_STATE_DIR",
  "NEXUSCLAW_CONFIG_PATH",
  "NEXUSCLAW_SKIP_BROWSER_CONTROL_SERVER",
  "NEXUSCLAW_SKIP_GMAIL_WATCHER",
  "NEXUSCLAW_SKIP_CANVAS_HOST",
  "NEXUSCLAW_BUNDLED_PLUGINS_DIR",
  "NEXUSCLAW_SKIP_CHANNELS",
  "NEXUSCLAW_SKIP_PROVIDERS",
  "NEXUSCLAW_SKIP_CRON",
  "NEXUSCLAW_TEST_MINIMAL_GATEWAY",
] as const;
...
async function persistTestSessionStorePath(storePath: string): Promise<void> {
  ...
  if (process.env.NEXUSCLAW_CONFIG_PATH) {
    configPaths.add(process.env.NEXUSCLAW_CONFIG_PATH);
  }
  if (process.env.NEXUSCLAW_STATE_DIR) {
    configPaths.add(path.join(process.env.NEXUSCLAW_STATE_DIR, "nexusclaw.json"));
  }
...
async function setupGatewayTestHome() {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "nexusclaw-gateway-home-"));
  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.NEXUSCLAW_STATE_DIR = path.join(tempHome, ".nexusclaw");
  delete process.env.NEXUSCLAW_CONFIG_PATH;
}

function applyGatewaySkipEnv() {
  process.env.NEXUSCLAW_SKIP_BROWSER_CONTROL_SERVER = "1";
  process.env.NEXUSCLAW_SKIP_GMAIL_WATCHER = "1";
  process.env.NEXUSCLAW_SKIP_CANVAS_HOST = "1";
  process.env.NEXUSCLAW_SKIP_CHANNELS = "1";
  process.env.NEXUSCLAW_SKIP_PROVIDERS = "1";
  process.env.NEXUSCLAW_SKIP_CRON = "1";
  process.env.NEXUSCLAW_TEST_MINIMAL_GATEWAY = "1";
  process.env.NEXUSCLAW_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "nexusclaw-test-no-bundled-extensions")
    : "nexusclaw-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  if (options.uniqueConfigRoot) {
    const suiteRoot = path.join(tempHome, ".nexusclaw-test-suite");
    ...
    tempConfigRoot = path.join(tempHome, ".nexusclaw-test");
    await fs.rm(tempConfigRoot, { recursive: true, force: true });
    await fs.mkdir(tempConfigRoot, { recursive: true });
  }
...
const CONNECT_CHALLENGE_NONCE_KEY = "__nexusclawTestConnectChallengeNonce";
const CONNECT_CHALLENGE_TRACKED_KEY = "__nexusclawTestConnectChallengeTracked";
...
export async function startServerWithClient(
  ...
  const envSnapshot = captureEnv(["NEXUSCLAW_GATEWAY_TOKEN"]);
  const prev = process.env.NEXUSCLAW_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.NEXUSCLAW_GATEWAY_TOKEN = fallbackToken;
  }
...
function resolveDefaultTestDeviceIdentityPath(params: {
  ...
  const safe =
    `${params.clientId}-${params.clientMode}-${params.platform}-${params.deviceFamily ?? "none"}-${params.role}`
      ...
      .toLowerCase();
  const suiteRoot = process.env.NEXUSCLAW_STATE_DIR ?? process.env.HOME ?? os.tmpdir();
