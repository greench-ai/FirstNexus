 ...
 *
 * Implements the OpenResponses `/v1/responses` endpoint for NexusClaw Gateway.
 *
...
function createResponseSessionScope(params: {
  ...
  return normalizeResponseSessionScope({
    ...
    user: params.user,
    requestedSessionKey: getHeader(params.req, "x-nexusclaw-session-key"),
  });
}
  ...
  unsubscribe = onAgentEvent((evt) => {
    ...
    if (evt.stream === "lifecycle") {
      ...
      if (phase === "end" || phase === "error") {
        const finalText = accumulatedText || "No response from NexusClaw.";
