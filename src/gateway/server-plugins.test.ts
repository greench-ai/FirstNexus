...
const loadFirstNexusPlugins = vi.hoisted(() => vi.fn());
...
function getLastPluginLoadOptions(): Record<string, unknown> {
  return requireRecord(loadFirstNexusPlugins.mock.calls.at(-1)?.[0], "plugin load options");
}
...
function getLastPluginLoadLogger(): {
  ...
  const call = loadFirstNexusPlugins.mock.calls.at(-1)?.[0] as
    | {
        logger?: {
          info: (message: string) => void;
          warn: (message: string) => void;
...
async function createSubagentRuntime(
  ...
  loadFirstNexusPlugins.mockReturnValue(createRegistry([]));
...
beforeEach(() => {
  ...
  loadFirstNexusPlugins.mockReset();
...
describe("loadGatewayPlugins", () => {
  test("logs plugin errors with details", () => {
    ...
    loadFirstNexusPlugins.mockReturnValue(createRegistry(diagnostics));
  ...
  test("pins the initial startup channel registry against later active-registry churn", () => {
    ...
    loadFirstNexusPlugins.mockReturnValue(startupRegistry);
  ...
  test("treats an empty startup scope as no plugin load instead of an unscoped load", () => {
    ...
    expect(loadFirstNexusPlugins).not.toHaveBeenCalled();
  ...
  test("includes docs guidance when a plugin fallback override is not trusted", async () => {
    ...
    await expect(
      ...
      'plugin "voice-call" is not trusted for fallback provider/model override requests. See https://docs.FirstNexus.ai/tools/plugin#runtime-helpers and search for: plugins.entries.<id>.subagent.allowModelOverride',
    );
  });
  ...
  test("runs registry hook before priming configured bindings", () => {
    ...
    loadFirstNexusPlugins.mockReturnValue(pluginRegistry);
