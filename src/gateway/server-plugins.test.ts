...
const loadNexusClawPlugins = vi.hoisted(() => vi.fn());
...
async function createSubagentRuntime(
  ...
  loadNexusClawPlugins.mockReturnValue(createRegistry([]));
  ...
  const call = loadNexusClawPlugins.mock.calls.at(-1)?.[0] as
...
beforeEach(() => {
  loadNexusClawPlugins.mockReset();
...
describe("loadGatewayPlugins", () => {
  test("logs plugin errors with details", async () => {
    ...
    loadNexusClawPlugins.mockReturnValue(createRegistry(diagnostics));
  ...
  test("includes docs guidance when a plugin fallback override is not trusted", async () => {
    ...
    await expect(
      ...
      'plugin "voice-call" is not trusted for fallback provider/model override requests. See https://docs.nexusclaw.ai/tools/plugin#runtime-helpers and search for: plugins.entries.<id>.subagent.allowModelOverride',
    );
  });
