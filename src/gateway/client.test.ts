...
function expectSecurityConnectError(
  ...
  expect(error.message).toContain("nexusclaw doctor --fix");
...
describe("GatewayClient security checks", () => {
  ...
  it("allows ws:// to private addresses only with NEXUSCLAW_ALLOW_INSECURE_PRIVATE_WS=1", () => {
  ...
  it("allows ws:// hostnames with NEXUSCLAW_ALLOW_INSECURE_PRIVATE_WS=1", () => {
    process.env.NEXUSCLAW_ALLOW_INSECURE_PRIVATE_WS = "1";
    const onConnectError = vi.fn();
    const client = new GatewayClient({
      url: "ws://nexusclaw-gateway.ai:18789",
      onConnectError,
    });
