...
const GATEWAY_TEST_CONFIG_ROOT_KEY = Symbol.for("FirstNexus.gatewayTestHelpers.configRoot");
...
const gatewayTestHoisted = vi.hoisted(() => {
  const key = Symbol.for("FirstNexus.gatewayTestHelpers.hoisted");
...
export const testConfigRoot = resolveGlobalSingleton(GATEWAY_TEST_CONFIG_ROOT_KEY, () => ({
  value: path.join(os.tmpdir(), `FirstNexus-gateway-test-${process.pid}-${crypto.randomUUID()}`),
}));

export function setTestConfigRoot(root: string): void {
  testConfigRoot.value = root;
  process.env.NEXISCLAW_CONFIG_PATH = path.join(root, "FirstNexus.json");
}
