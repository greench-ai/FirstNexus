...
let lastCreateFirstNexusToolsContext: Record<string, unknown> | undefined;
...
vi.mock("../agents/FirstNexus-tools.js", () => {
  ...
  const tools = [
    ...
    },
    {
      ...
      execute: async () => ({
        ok: true,
        route: {
          agentTo: lastCreateFirstNexusToolsContext?.agentTo,
          agentThreadId: lastCreateFirstNexusToolsContext?.agentThreadId,
        },
      }),
    },
    {
  ...
  return {
    createFirstNexusTools: (ctx: Record<string, unknown>) => {
      lastCreateFirstNexusToolsContext = ctx;
...
beforeEach(() => {
  ...
  pluginHttpHandlers = [];
  cfg = {};
  lastCreateFirstNexusToolsContext = undefined;
...
const gatewayAuthHeaders = () => ({ "x-FirstNexus-scopes": "operator.write" });
const gatewayAdminHeaders = () => ({ "x-FirstNexus-scopes": "operator.admin" });
...
describe("POST /tools/invoke", () => {
  it("invokes a tool and returns {ok:true,result}", async () => {
    ...
    expect(lastCreateFirstNexusToolsContext?.allowMediaInvokeCommands).toBe(true);
    expect(lastCreateFirstNexusToolsContext?.disablePluginTools).toBe(true);
  ...
  it("opts direct gateway tool invocation into gateway subagent binding", async () => {
    ...
    expect(res.status).toBe(200);
    expect(lastCreateFirstNexusToolsContext?.allowGatewaySubagentBinding).toBe(true);
  });
  ...
  it("keeps plugin tools enabled for non-core tool invokes", async () => {
    ...
    expect(res.status).toBe(200);
    expect(lastCreateFirstNexusToolsContext?.disablePluginTools).toBe(false);
  });
  ...
  it("allows the requested plugin tool through Gateway profile filtering", async () => {
    ...
    expect(lastCreateFirstNexusToolsContext?.pluginToolAllowlist).toContain("plugin_doctor");
  });
  ...
  it("uses tools.alsoAllow for optional plugin discovery without loading every plugin tool", async () => {
    ...
    expect(body.result?.permissionFlow).toBe(true);
    expect(lastCreateFirstNexusToolsContext?.pluginToolAllowlist).toContain("plugin_doctor");
    expect(lastCreateFirstNexusToolsContext?.pluginToolAllowlist).not.toContain("*");
  });
  ...
  it("threads senderIsOwner into tool creation before owner-only filtering", async () => {
    ...
    expect(lastCreateFirstNexusToolsContext?.senderIsOwner).toBe(false);
    ...
    expect(lastCreateFirstNexusToolsContext?.senderIsOwner).toBe(true);
  });
  ...
  it("propagates message target/thread headers into tools context for sessions_spawn", async () => {
    ...
    const res = await invokeTool({
      ...
      headers: {
        ...gatewayAuthHeaders(),
        "x-FirstNexus-message-to": "channel:24514",
        "x-FirstNexus-thread-id": "thread-24514",
      },
      tool: "sessions_spawn",
      sessionKey: "main",
    });
