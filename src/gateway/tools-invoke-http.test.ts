...
let lastCreateNexusClawToolsContext: Record<string, unknown> | undefined;
...
vi.mock("../agents/nexusclaw-tools.js", () => {
  ...
  const tools = [
    ...
    },
    {
      ...
      execute: async () => ({
        ok: true,
        route: {
          agentTo: lastCreateNexusClawToolsContext?.agentTo,
          agentThreadId: lastCreateNexusClawToolsContext?.agentThreadId,
        },
      }),
    },
    {
  ...
  return {
    createNexusClawTools: (ctx: Record<string, unknown>) => {
      lastCreateNexusClawToolsContext = ctx;
      return tools;
    },
  };
});
...
beforeEach(() => {
  ...
  pluginHttpHandlers = [];
  cfg = {};
  lastCreateNexusClawToolsContext = undefined;
...
describe("POST /tools/invoke", () => {
  it("invokes a tool and returns {ok:true,result}", async () => {
    ...
    expect(lastCreateNexusClawToolsContext?.allowMediaInvokeCommands).toBe(true);
  ...
  it("opts direct gateway tool invocation into gateway subagent binding", async () => {
    ...
    expect(res.status).toBe(200);
    expect(lastCreateNexusClawToolsContext?.allowGatewaySubagentBinding).toBe(true);
  });
  ...
  it("propagates message target/thread headers into tools context for sessions_spawn", async () => {
    ...
    const res = await invokeTool({
      ...
      headers: {
        ...gatewayAuthHeaders(),
        "x-nexusclaw-message-to": "channel:24514",
        "x-nexusclaw-thread-id": "thread-24514",
      },
      tool: "sessions_spawn",
      sessionKey: "main",
    });
