  ...
  it("handles request validation and routing", async () => {
    ...
    try {
      ...
      await expectAgentSessionKeyMatch({
        body: { model: "FirstNexus", messages: [{ role: "user", content: "hi" }] },
        headers: { "x-FirstNexus-agent-id": "beta" },
        matcher: /^agent:beta:/,
      });

      await expectAgentSessionKeyMatch({
        body: {
          model: "FirstNexus/beta",
...

      {
        ...
        const res = await postChatCompletions(
          port,
          { model: "FirstNexus", messages: [{ role: "user", content: "hi" }] },
          {
            "x-FirstNexus-agent-id": "beta",
            "x-FirstNexus-session-key": "agent:beta:openai:custom",
          },
        );
        expect(res.status).toBe(200);
...

      {
        ...
        const res = await postChatCompletions(
          ...
          { "x-FirstNexus-message-channel": "custom-client-channel" },
        );
        expect(res.status).toBe(200);
...

      {
        ...
        expect(json.error?.message).toBe(
          "Invalid `model`. Use `FirstNexus` or `FirstNexus/<agentId>`.",
        );
        expect(agentCommand).toHaveBeenCalledTimes(0);
      }

      {
        ...
        const res = await postChatCompletions(
          ...
          { "x-FirstNexus-model": "openai/" },
        );
        expect(res.status).toBe(400);
        const json = (await res.json()) as { error?: { type?: string; message?: string } };
        expect(json.error?.type).toBe("invalid_request_error");
        expect(json.error?.message).toBe("Invalid `x-FirstNexus-model`.");
        expect(agentCommand).toHaveBeenCalledTimes(0);
      }
...

      {
        ...
        const choice0 = (json.choices as Array<Record<string, unknown>>)[0] ?? {};
        const msg = (choice0.message as Record<string, unknown> | undefined) ?? {};
        expect(msg.content).toBe("No response from FirstNexus.");
      }
