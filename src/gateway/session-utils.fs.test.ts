...
function expectMessageFields(
  message: unknown,
  fields: { role?: string; content?: unknown; FirstNexus?: Record<string, unknown> },
) {
  ...
  if (fields.FirstNexus) {
    const metadata = requireRecord(record.__FirstNexus, "message metadata");
    for (const [key, value] of Object.entries(fields.FirstNexus)) {
...
describe("readFirstUserMessageFromTranscript", () => {
  ...
  registerTempSessionStore("FirstNexus-session-fs-test-", (nextTmpDir, nextStorePath) => {
    tmpDir = nextTmpDir;
...
describe("readSessionMessages", () => {
  ...
  test("includes synthetic compaction markers for compaction entries", () => {
    ...
    const marker = out[1] as {
      ...
      __FirstNexus?: { kind?: string; id?: string };
      timestamp?: number;
    };
    expect(marker.role).toBe("system");
    expect(marker.content?.[0]?.text).toBe("Compaction");
    expect(marker.__FirstNexus?.kind).toBe("compaction");
    expect(marker.__FirstNexus?.id).toBe("comp-1");
  ...
  test("reads recent messages from the transcript tail without loading the whole file", () => {
    ...
    expectMessageFields(out[0], { role: "user", content: "recent", FirstNexus: { seq: 3 } });
    expectMessageFields(out[1], { role: "assistant", content: "latest", FirstNexus: { seq: 4 } });
  });
  ...
  test("preserves real sequence metadata for bounded recent-message reads", () => {
    ...
    expectMessageFields(result.messages[0], { content: "recent", FirstNexus: { seq: 3 } });
    expectMessageFields(result.messages[1], { content: "latest", FirstNexus: { seq: 4 } });
  });
  ...
  test("reads active tree branch asynchronously without SessionManager.open", async () => {
    ...
    try {
      ...
      expectMessageFields(messages[2], { FirstNexus: { id: "user-2", seq: 3 } });
      expect(sessionManagerOpenSpy).not.toHaveBeenCalled();
      expect(readFileSpy).not.toHaveBeenCalled();
    } finally {
      sessionManagerOpenSpy.mockRestore();
  ...
  test.each([
    ...
    ({ sessionId, sessionFileParts, wrongStorePathParts, message }) => {
      ...
      expect((out[0] as { __FirstNexus?: { seq?: number } }).__FirstNexus?.seq).toBe(1);
    },
  );
  ...
  test("keeps compaction markers when reading only the active SessionManager branch", () => {
    ...
    expect(
      out.map((message) => ({
        role: (message as { role?: string }).role,
        content: (message as { content?: unknown }).content,
        kind: (message as { __FirstNexus?: { kind?: string } }).__FirstNexus?.kind,
      })),
    ).toEqual([
...
describe("readSessionPreviewItemsFromTranscript", () => {
  ...
  registerTempSessionStore("FirstNexus-session-preview-test-", (nextTmpDir, nextStorePath) => {
    tmpDir = nextTmpDir;
    storePath = nextStorePath;
  });
...
describe("readLatestSessionUsageFromTranscript", () => {
  ...
  registerTempSessionStore("FirstNexus-session-usage-test-", (nextTmpDir, nextStorePath) => {
    tmpDir = nextTmpDir;
    storePath = nextStorePath;
  });
...
describe("resolveSessionTranscriptCandidates", () => {
  ...
  test("fallback candidate uses NEXISCLAW_HOME instead of os.homedir()", () => {
    ...
    expect(fallback).toBe(
      path.join(path.resolve("/srv/FirstNexus-home"), ".FirstNexus", "sessions", "sess-1.jsonl"),
    );
  });
});
...
describe("resolveSessionTranscriptCandidates safety", () => {
  test.each([
    {
      storePath: "/tmp/FirstNexus/agents/main/sessions/sessions.json",
      sessionFile: "/tmp/FirstNexus/agents/ops/sessions/sess-safe.jsonl",
    },
    {
  ...
  test("drops unsafe session IDs instead of producing traversal paths", () => {
    const candidates = resolveSessionTranscriptCandidates(
      "../etc/passwd",
      "/tmp/FirstNexus/agents/main/sessions/sessions.json",
    );
  ...
  test("prefers the current sessionId transcript before a stale sessionFile candidate", () => {
    ...
    const candidates = resolveSessionTranscriptCandidates(
      ...
      storePath,
      "/tmp/FirstNexus/agents/main/sessions/22222222-2222-4222-8222-222222222222.jsonl",
    );

    expect(candidates[0]).toBe(
      path.resolve(
        "/tmp/FirstNexus/agents/main/sessions/11111111-1111-4111-8111-111111111111.jsonl",
      ),
    );
  ...
  test("keeps explicit custom sessionFile ahead of synthesized fallback", () => {
    const storePath = "/tmp/FirstNexus/agents/main/sessions/sessions.json";
    const sessionFile = "/tmp/FirstNexus/agents/main/sessions/custom-transcript.jsonl";
    const candidates = resolveSessionTranscriptCandidates(
      "11111111-1111-4111-8111-111111111111",
      storePath,
      sessionFile,
  ...
  test("keeps custom topic-like transcript paths ahead of synthesized fallback", () => {
    const storePath = "/tmp/FirstNexus/agents/main/sessions/sessions.json";
    const sessionFile = "/tmp/FirstNexus/agents/main/sessions/custom-topic-notes.jsonl";
    const candidates = resolveSessionTranscriptCandidates(
      "11111111-1111-4111-8111-111111111111",
      storePath,
      sessionFile,
  ...
  test("keeps forked transcript paths ahead of synthesized fallback", () => {
    ...
    const sessionFile =
      "/tmp/FirstNexus/agents/main/sessions/2026-03-23T16-30-00-000Z_11111111-1111-4111-8111-111111111111.jsonl";
  ...
  test("keeps timestamped custom transcript paths ahead of synthesized fallback", () => {
    const storePath = "/tmp/FirstNexus/agents/main/sessions/sessions.json";
    const sessionId = "11111111-1111-4111-8111-111111111111";
    const sessionFile = "/tmp/FirstNexus/agents/main/sessions/2026-03-23T16-30-00-000Z_notes.jsonl";
    const candidates = resolveSessionTranscriptCandidates(sessionId, storePath, sessionFile);
  ...
  test("still treats generated topic transcripts from another session as stale", () => {
    ...
    const staleSessionFile =
      "/tmp/FirstNexus/agents/main/sessions/22222222-2222-4222-8222-222222222222-topic-thread.jsonl";
...
describe("archiveSessionTranscripts", () => {
  ...
  registerTempSessionStore("FirstNexus-archive-test-", (nextTmpDir, nextStorePath) => {
    tmpDir = nextTmpDir;
    storePath = nextStorePath;
  });
...
describe("oversized transcript line guards", () => {
  ...
  registerTempSessionStore("FirstNexus-session-fs-oversized-", (nextTmpDir, nextStorePath) => {
    tmpDir = nextTmpDir;
    storePath = nextStorePath;
  });
