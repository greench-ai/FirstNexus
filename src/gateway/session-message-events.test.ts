...
async function createSessionStoreFile(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "FirstNexus-session-message-"));
  ...
  test("strips blocked original content from live session.message events", async () => {
    ...
    await withOperatorSessionSubscriber(async (ws) => {
      ...
      const payload = messageEvent.payload as {
        message?: { content?: unknown; __FirstNexus?: { beforeAgentRunBlocked?: unknown } };
      };
    ...
    await withOperatorSessionSubscriber(async (ws) => {
      ...
      const payload = messageEvent.payload as {
        message?: {
          role?: unknown;
          content?: unknown;
          __FirstNexus?: { beforeAgentRunBlocked?: unknown };
        };
      };
