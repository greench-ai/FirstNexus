...
const pluginManifestRegistryMocks = vi.hoisted(() => ({
  ...
  listFirstNexusPluginManifestMetadata: vi.fn(),
}));
...
vi.mock("../plugins/manifest-metadata-scan.js", async (importOriginal) => {
  ...
  return {
    ...actual,
    listFirstNexusPluginManifestMetadata: (
      params?: Parameters<typeof actual.listFirstNexusPluginManifestMetadata>[0],
    ) => {
      pluginManifestRegistryMocks.listFirstNexusPluginManifestMetadata(params);
      return actual.listFirstNexusPluginManifestMetadata(params);
    },
  };
});
...
describe("model-pricing-cache", () => {
  beforeEach(() => {
    ...
    pluginManifestRegistryMocks.listFirstNexusPluginManifestMetadata.mockClear();
  ...
  it("collects manifest-owned web search plugin model refs without a hardcoded plugin list", () => {
    ...
    } as FirstNexusConfig).map((ref) => modelKey(ref.provider, ref.model));
  ...
  it("does not load plugin manifests for pricing when plugins are globally disabled", async () => {
    ...
    expect(pluginManifestRegistryMocks.listFirstNexusPluginManifestMetadata).not.toHaveBeenCalled();
...
function createManifestRecord(overrides: Partial<PluginManifestRecord>): PluginManifestRecord {
  return {
    ...
    manifestPath: "/tmp/plugin/FirstNexus.plugin.json",
    ...overrides,
  };
}
