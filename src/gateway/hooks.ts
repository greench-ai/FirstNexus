...
export function resolveHooksConfig(cfg: NexusClawConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: NexusClawConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-nexusclaw-token"] === "string"
      ? req.headers["x-nexusclaw-token"].trim()
      : "";
  if (headerToken) {
    return headerToken;
  }
...
export function resolveHookIdempotencyKey(params: {
  ...
  return (
    ...
    resolveOptionalHookIdempotencyKey(params.headers?.["x-nexusclaw-idempotency-key"]) ||
