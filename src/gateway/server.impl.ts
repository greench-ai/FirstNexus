...
import { ensureNexusClawCliOnPath } from "../infra/path-env.js";
import {
...
function assertValidGatewayStartupConfigSnapshot(
  ...
  const doctorHint = options.includeDoctorHint
    ? `\nRun "${formatCliCommand("nexusclaw doctor")}" to repair, then retry.`
    : "";
...
async function prepareGatewayStartupConfig(params: {
  ...
  ) => Promise<{ config: NexusClawConfig }>;
...
export async function startGatewayServer(
  ...
  const minimalTestGateway =
    process.env.VITEST === "1" && process.env.NEXUSCLAW_TEST_MINIMAL_GATEWAY === "1";

  // Ensure all default port derivations (browser/canvas) see the actual runtime port.
  process.env.NEXUSCLAW_GATEWAY_PORT = String(port);
  logAcceptedEnvOption({
    key: "NEXUSCLAW_RAW_STREAM",
    description: "raw stream logging enabled",
  });
  logAcceptedEnvOption({
    key: "NEXUSCLAW_RAW_STREAM_PATH",
  ...
  let cfgAtStart: NexusClawConfig;
  ...
  if (authBootstrap.generatedToken) {
    if (authBootstrap.persistedGeneratedToken) {
      ...
      log.warn(
        "Gateway auth token was missing. Generated a runtime token for this startup without changing config; restart will generate a different token. Persist one with `nexusclaw config set gateway.auth.mode token` and `nexusclaw config set gateway.auth.token <token>`.",
      );
    }
  }
  ...
  if (matrixInstallPathIssue) {
    const lines = formatPluginInstallPathIssue({
      ...
      pluginLabel: "Matrix",
      defaultInstallCommand: "nexusclaw plugins install @nexusclaw/matrix",
      repoInstallCommand: "nexusclaw plugins install ./extensions/matrix",
  ...
  const transcriptUnsub = minimalTestGateway
    ...
    : onSessionTranscriptUpdate((update) => {
        ...
        const sessionSnapshot = sessionRow
          ...
          : {};
        const message = attachNexusClawTranscriptMeta(update.message, {
