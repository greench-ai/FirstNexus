...
const getRuntimeConfig = vi.hoisted(() => vi.fn(() => ({}) as FirstNexusConfig));
const resolveDefaultAgentId = vi.hoisted(() => vi.fn(() => "main"));
const resolveAgentWorkspaceDir = vi.hoisted(() =>
  vi.fn((_cfg: FirstNexusConfig, _agentId: string) => "/tmp/FirstNexus"),
);
const resolveMemorySearchConfig = vi.hoisted(() =>
  vi.fn<(_cfg: FirstNexusConfig, _agentId: string) => { enabled: boolean } | null>(() => ({
    enabled: true,
  })),
);
...
describe("doctor.memory.status", () => {
  beforeEach(() => {
    ...
    resolveAgentWorkspaceDir.mockReset().mockReturnValue("/tmp/FirstNexus");
  ...
  it("includes dreaming counts and managed cron status when workspace data is available", async () => {
    ...
    resolveAgentWorkspaceDir.mockImplementation((cfg: FirstNexusConfig, agentId: string) => {
      if (agentId === "alpha") {
  ...
  it("reads dreaming config from the selected memory slot plugin", async () => {
    getRuntimeConfig.mockReturnValue({
      plugins: {
        slots: {
          memory: "memos-local-FirstNexus-plugin",
        },
        entries: {
          "memos-local-FirstNexus-plugin": {
            config: {
              dreaming: {
                enabled: true,
                frequency: "0 */4 * * *",
  ...
  it("merges workspace store errors when multiple workspace stores are unreadable", async () => {
    ...
    resolveAgentWorkspaceDir.mockImplementation((_cfg: FirstNexusConfig, agentId: string) =>
...
describe("doctor.memory dream actions", () => {
  it("clears grounded-only staged short-term entries without touching the diary", async () => {
    resolveAgentWorkspaceDir.mockReturnValue("/tmp/FirstNexus");
    removeGroundedShortTermCandidates.mockResolvedValue({
      removed: 3,
      storePath: "/tmp/FirstNexus/memory/.dreams/short-term-recall.json",
    });
    const respond = vi.fn();
  ...
  it("repairs contaminated dreaming artifacts for control-ui callers", async () => {
    ...
    repairDreamingArtifacts.mockResolvedValue({
      changed: true,
      archiveDir: "/tmp/FirstNexus/.FirstNexus-repair/dreaming/2026-04-11T22-00-00-000Z",
  ...
  it("dedupes exact dream diary duplicates for control-ui callers", async () => {
    ...
    dedupeDreamDiaryEntries.mockResolvedValue({
      dreamsPath: "/tmp/FirstNexus/DREAMS.md",
      removed: 2,
      kept: 7,
    });
    const respond = vi.fn();
...
describe("doctor.memory.remHarness", () => {
  const makeHarnessPreview = (
    ...
    workspaceDir: overrides.workspaceDir ?? "/tmp/FirstNexus",
    nowMs: 0,
    remConfig: {
      enabled: true,
      lookbackDays: 7,
  ...
  beforeEach(() => {
    getRuntimeConfig.mockClear().mockReturnValue({} as FirstNexusConfig);
