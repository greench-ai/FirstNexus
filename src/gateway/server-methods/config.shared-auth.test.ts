...
vi.mock("../../config/config.js", async () => {
  ...
  return {
    ...actual,
    createConfigIO: () => ({ configPath: "/tmp/FirstNexus.json" }),
...
beforeEach(() => {
  validateConfigObjectWithPluginsMock.mockImplementation((config: FirstNexusConfig) => ({
    ok: true,
    config,
  }));
  prepareSecretsRuntimeSnapshotMock.mockImplementation(
    async ({ config }: { config: FirstNexusConfig }) => ({
      config,
    }),
  );
...
describe("config shared auth disconnects", () => {
  it("does not disconnect shared-auth clients for config.set auth writes without restart", async () => {
    const prevConfig: FirstNexusConfig = {
      gateway: {
        auth: {
          mode: "token",
          token: "old-token",
