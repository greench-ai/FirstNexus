...
import type { ConfigValidationIssue, NexusClawConfig } from "../../config/types.nexusclaw.js";
import {
...
function parseValidateConfigFromRawOrRespond(
  ...
  snapshot: Awaited<ReturnType<typeof readConfigFileSnapshot>>,
  respond: RespondFn,
): { config: NexusClawConfig; schema: ConfigSchemaResponse } | null {
...
function loadSchemaWithPlugins(): ConfigSchemaResponse {
  ...
  const pluginRegistry = loadNexusClawPlugins({
    config: cfg,
    cache: true,
    workspaceDir,
    runtimeOptions: {
  ...
  // our cache. However, both loadConfig() and loadNexusClawPlugins() already cache
