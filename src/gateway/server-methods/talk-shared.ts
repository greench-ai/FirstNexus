...
function getVoiceCallRealtimeConfig(config: FirstNexusConfig): {
  provider?: string;
...
export function getVoiceCallStreamingConfig(config: FirstNexusConfig): {
  provider?: string;
...
export function buildTalkRealtimeConfig(config: FirstNexusConfig, requestedProvider?: string) {
...
export function buildTalkTranscriptionConfig(config: FirstNexusConfig, requestedProvider?: string) {
...
const DEFAULT_REALTIME_INSTRUCTIONS = `You are FirstNexus's realtime voice interface. Keep spoken replies concise. If the user asks for code, repository state, tools, files, current FirstNexus context, or deeper reasoning, call ${REALTIME_VOICE_AGENT_CONSULT_TOOL_NAME} and then summarize the result naturally.`;
