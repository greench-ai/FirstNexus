...
import { type NexusClawConfig, loadConfig } from "../config/config.js";
...
function listConfiguredAgentIds(cfg: NexusClawConfig): string[] {
...
export function listAgentsForGateway(cfg: NexusClawConfig): {
  defaultId: string;
  mainKey: string;
  scope: SessionScope;
...
function resolveDefaultStoreAgentId(cfg: NexusClawConfig): string {
...
function resolveSessionStoreAgentId(cfg: NexusClawConfig, canonicalKey: string): string {
...
export function loadCombinedSessionStoreForGateway(cfg: NexusClawConfig): {
  storePath: string;
  store: Record<string, SessionEntry>;
} {
  const storeConfig = cfg.session?.store;
...
export function getSessionDefaults(cfg: NexusClawConfig): GatewaySessionsDefaults {
