...
export function buildGatewayConnectionDetails(
  ...
  const envUrlOverride = cliUrlOverride
    ? undefined
    : (trimToUndefined(process.env.NEXUSCLAW_GATEWAY_URL) ??
  ...
  const allowPrivateWs = process.env.NEXUSCLAW_ALLOW_INSECURE_PRIVATE_WS === "1";
  ...
  if (!isSecureWebSocketUrl(url, { allowPrivateWs })) {
    throw new Error(
      [
        ...
        allowPrivateWs
          ? undefined
          : "Break-glass (trusted private networks only): set NEXUSCLAW_ALLOW_INSECURE_PRIVATE_WS=1",
        "Doctor: nexusclaw doctor --fix",
...
function gatewaySecretInputPathCanWin(params: {
  ...
  const sentinel = `__NEXUSCLAW_GATEWAY_SECRET_REF_PROBE_${params.path.replaceAll(".", "_")}__`;
