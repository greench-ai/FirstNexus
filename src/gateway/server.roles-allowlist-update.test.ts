...
const connectNodeClient = async (params: {
  ...
  if (!token) {
    throw new Error("NEXISCLAW_GATEWAY_TOKEN is required for node test clients");
  }
...
function getGatewayTestConfigPath(): string {
  ...
  if (!configPath) {
    throw new Error("NEXISCLAW_CONFIG_PATH is required in the gateway test environment");
  }
  return configPath;
}
