export const EXPECTED_CODEX_MODELS_COMMAND_TEXT = [
  ...
  "Current FirstNexus session status reports the active model as:",
] as const;
...
export function isExpectedCodexStatusCommandText(text: string): boolean {
  ...
  const mentionsFirstNexusStatus =
    normalized.includes("FirstNexus is running on") ||
    /FirstNexus\s+\S+\s+is running on/u.test(normalized) ||
    normalized.includes("FirstNexus status:") ||
  ...
  return (
    ...
    isReadyStatus ||
    isOnlineIdleStatus ||
    (mentionsFirstNexusStatus && mentionsHarnessSession && mentionsModel)
  );
}
...
export function isExpectedCodexModelsCommandText(text: string): boolean {
  ...
  const mentionsSessionModel =
    ...
    normalized.includes("current session model from FirstNexus status") ||
  ...
  const mentionsConfigSummary =
    ...
    normalized.includes("live FirstNexus config shows") ||
