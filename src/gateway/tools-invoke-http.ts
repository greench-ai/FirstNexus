...
import { createNexusClawTools } from "../agents/nexusclaw-tools.js";
...
export async function handleToolsInvokeHttpRequest(
  ...
  const messageChannel = normalizeMessageChannel(
    getHeader(req, "x-nexusclaw-message-channel") ?? "",
  );
  const accountId = getHeader(req, "x-nexusclaw-account-id")?.trim() || undefined;
  const agentTo = getHeader(req, "x-nexusclaw-message-to")?.trim() || undefined;
  const agentThreadId = getHeader(req, "x-nexusclaw-thread-id")?.trim() || undefined;
  ...
  const allTools = createNexusClawTools({
