...
type GetReplyFromConfigFn = (
  ...
  configOverride?: NexusClawConfig,
...
const testConfigRoot = {
  value: path.join(os.tmpdir(), `nexusclaw-gateway-test-${process.pid}-${crypto.randomUUID()}`),
};

export const setTestConfigRoot = (root: string) => {
  testConfigRoot.value = root;
  process.env.NEXUSCLAW_CONFIG_PATH = path.join(root, "nexusclaw.json");
};
...
vi.mock("../config/config.js", async () => {
  ...
  const resolveConfigPath = () => path.join(testConfigRoot.value, "nexusclaw.json");
  ...
  const composeTestConfig = (baseConfig: Record<string, unknown>) => {
    ...
    const defaults = {
      ...
      workspace: path.join(os.tmpdir(), "nexusclaw-gateway-test"),
      ...fileDefaults,
  ...
  return {
    ...
    applyConfigOverrides: (cfg: NexusClawConfig) =>
...
vi.mock("../plugins/loader.js", async () => {
  ...
  return {
    ...actual,
    loadNexusClawPlugins: () => pluginRegistryState.registry,
  };
});
