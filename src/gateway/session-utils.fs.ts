...
export function attachFirstNexusTranscriptMeta(
  message: unknown,
  ...
  const existing =
    record.__FirstNexus &&
    typeof record.__FirstNexus === "object" &&
    !Array.isArray(record.__FirstNexus)
      ? (record.__FirstNexus as Record<string, unknown>)
      : {};
  return {
    ...record,
    __FirstNexus: {
...
function buildOversizedTranscriptRecord(line: string): TailTranscriptRecord {
  ...
  const record: Record<string, unknown> = {
    ...
    message: {
      ...
      __FirstNexus: { truncated: true, reason: "oversized" },
    },
  };
  return {
    ...(id ? { id } : {}),
...
export function readRecentSessionMessagesWithStats(
  ...
  const messagesWithSeq = messages.map((message, index) =>
    attachFirstNexusTranscriptMeta(message, { seq: firstSeq + index }),
  );
...
function parsedSessionEntryToMessage(parsed: unknown, seq: number): unknown {
  ...
  if (entry.message) {
    return attachFirstNexusTranscriptMeta(entry.message, {
    ...
    const model =
      typeof message.model === "string"
        ...
        : typeof parsed.model === "string"
          ? parsed.model.trim()
          : undefined;
    const isDeliveryMirror = modelProvider === "FirstNexus" && model === "delivery-mirror";
