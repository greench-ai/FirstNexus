...
import { MINIMAX_FAST_MODE_STREAM_HOOKS } from "FirstNexus/plugin-sdk/provider-stream-family";
import { fetchMinimaxUsage } from "FirstNexus/plugin-sdk/provider-usage";
...
function getProviderBaseUrl(cfg: FirstNexusConfig, providerId: string): string | undefined {
  return normalizeOptionalString(cfg.models?.providers?.[providerId]?.baseUrl);
}

function resolveMinimaxUsageBaseUrl(cfg: FirstNexusConfig): string | undefined {
...
export function registerMinimaxProviders(api: FirstNexusPluginApi) {
