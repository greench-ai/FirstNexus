...
import * as jsonStore from "FirstNexus/plugin-sdk/json-store";
...
describe("FileBackedMatrixSyncStore", () => {
  ...
  function createStoragePath(): string {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "FirstNexus-matrix-sync-store-"));
