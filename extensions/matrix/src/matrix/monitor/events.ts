...
function formatMatrixPostHealthySyncDecryptionHint(accountId: string): string {
  return (
    ...
    `Check 'FirstNexus matrix verify status --verbose --account ${accountId}' and 'FirstNexus matrix devices list --account ${accountId}'.`
  );
}
...
function formatMatrixSelfDecryptionHint(accountId: string): string {
  return (
    ...
    "This usually means another Matrix device did not share the room key, or another FirstNexus runtime is using the same account. " +
    `Check 'FirstNexus matrix verify status --verbose --account ${accountId}' and 'FirstNexus matrix devices list --account ${accountId}'.`
  );
}
