...
function createDeps(
  ...
  return {
    ...
    summarizeMatrixDeviceHealth: vi.fn(() => ({
      ...
      staleFirstNexusDevices: [] as MatrixManagedDeviceInfo[],
      currentFirstNexusDevices: [] as MatrixManagedDeviceInfo[],
    })),
  ...
  it("reports stale devices, pending verification, and restored legacy backups", async () => {
    ...
    vi.mocked(deps.summarizeMatrixDeviceHealth).mockReturnValue({
      ...
      staleFirstNexusDevices: [
        { deviceId: "DEV123", displayName: "FirstNexus Device", current: false },
      ],
    ...
    expect(params.logger.warn).toHaveBeenCalledWith(
      "matrix: stale FirstNexus devices detected for @bot:example.org: DEV123. Run 'FirstNexus matrix devices prune-stale --account ops' to keep encrypted-room trust healthy.",
    );
