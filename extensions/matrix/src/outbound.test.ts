...
describe("matrixOutbound cfg threading", () => {
  ...
  it("passes resolved cfg to sendMessageMatrix for media sends", async () => {
    ...
    await matrixOutbound.sendMedia!({
      ...
      mediaLocalRoots: ["/tmp/FirstNexus"],
      accountId: "default",
      audioAsVoice: true,
    });
    ...
    expect(options.mediaLocalRoots).toEqual(["/tmp/FirstNexus"]);
  ...
  it("renders MessagePresentation into Matrix custom content metadata", async () => {
    ...
    expect(matrixData.extraContent?.["com.FirstNexus.presentation"]).toEqual({
      ...presentation,
      version: 1,
    ...
    await matrixOutbound.sendPayload!({
      ...
      payload: {
        ...
        channelData: {
          matrix: {
            extraContent: {
              "com.FirstNexus.presentation": presentationContent,
            },
          },
        },
      },
