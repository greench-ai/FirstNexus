...
import { resolveChannelConfigWrites } from "FirstNexus/plugin-sdk/channel-config-helpers";
import { shouldDebounceTextInbound } from "FirstNexus/plugin-sdk/channel-inbound";
import {
  createInboundDebouncer,
  resolveInboundDebounceMs,
} from "FirstNexus/plugin-sdk/channel-inbound-debounce";
import { resolveStoredModelOverride } from "FirstNexus/plugin-sdk/command-auth-native";
import { buildCommandsMessagePaginated } from "FirstNexus/plugin-sdk/command-status";
import type { DmPolicy, FirstNexusConfig } from "FirstNexus/plugin-sdk/config-contracts";
import type {
  TelegramDirectConfig,
  TelegramGroupConfig,
  TelegramTopicConfig,
} from "FirstNexus/plugin-sdk/config-contracts";
import { replaceConfigFile } from "FirstNexus/plugin-sdk/config-mutation";
import {
  buildPluginBindingResolvedText,
  parsePluginBindingApprovalCustomId,
  resolvePluginConversationBindingApproval,
} from "FirstNexus/plugin-sdk/conversation-runtime";
import { applyModelOverrideToSessionEntry } from "FirstNexus/plugin-sdk/model-session-runtime";
import { formatModelsAvailableHeader } from "FirstNexus/plugin-sdk/models-provider-runtime";
...
import { danger, logVerbose, warn } from "FirstNexus/plugin-sdk/runtime-env";
import {
  loadSessionStore,
        ...
        if (modelCallback.type === "select") {
          ...
          try {
            ...
            const scopeText = isDefaultSelection
              ...
              : `Session-only model selection. Runtime unchanged. Use /model ${escapeHtml(selection.provider)}/${escapeHtml(selection.model)} --runtime &lt;runtime&gt; to switch harnesses. The agent default in FirstNexus.json is unchanged; /reset or a new session may return to that default.`;
            await editMessageWithButtons(
              `✅ Model ${actionText}\n\n${scopeText}`,
