...
vi.mock("FirstNexus/plugin-sdk/config-mutation", async () => {
  const actual = await vi.importActual<typeof import("FirstNexus/plugin-sdk/config-mutation")>(
    "FirstNexus/plugin-sdk/config-mutation",
  );
  return {
    ...actual,
...
vi.mock("FirstNexus/plugin-sdk/cron-store-runtime", async () => {
  const actual = await vi.importActual<typeof import("FirstNexus/plugin-sdk/cron-store-runtime")>(
    "FirstNexus/plugin-sdk/cron-store-runtime",
  );
  return {
    ...actual,
    loadCronStore,
  ...
  describe("maybePersistResolvedTelegramTarget", () => {
    ...
    it("writes back matching config and cron targets", async () => {
      readConfigFileSnapshotForWrite.mockResolvedValue({
        ...
        writeOptions: { expectedConfigPath: "/tmp/FirstNexus.json" },
      });
      ...
      expect(writeOptions.expectedConfigPath).toBe("/tmp/FirstNexus.json");
