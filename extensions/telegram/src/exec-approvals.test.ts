...
function createTempDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "FirstNexus-telegram-exec-approvals-"));
  tempDirs.push(dir);
  return dir;
}
...
function buildConfig(
  execApprovals?: NonNullable<
    NonNullable<FirstNexusConfig["channels"]>["telegram"]
  >["execApprovals"],
  channelOverrides?: Partial<NonNullable<NonNullable<FirstNexusConfig["channels"]>["telegram"]>>,
): FirstNexusConfig {
  return {
    channels: {
      telegram: {
