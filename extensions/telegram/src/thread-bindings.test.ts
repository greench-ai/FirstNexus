...
vi.mock("FirstNexus/plugin-sdk/json-store", async () => {
  const actual = await vi.importActual<typeof import("FirstNexus/plugin-sdk/json-store")>(
    "FirstNexus/plugin-sdk/json-store",
  );
...
describe("telegram thread bindings", () => {
  ...
  beforeEach(async () => {
    ...
    const acpRuntime = await vi.importActual<typeof import("FirstNexus/plugin-sdk/acp-runtime")>(
      "FirstNexus/plugin-sdk/acp-runtime",
    );
  ...
  it("does not persist lifecycle updates when manager persistence is disabled", async () => {
    stateDirOverride = fs.mkdtempSync(path.join(os.tmpdir(), "FirstNexus-telegram-bindings-"));
  ...
  it("persists unbinds before restart so removed bindings do not come back", async () => {
    ...
    const bound = await getSessionBindingService().bind({
      targetSessionKey: "plugin-binding:FirstNexus-codex-app-server:abc123",
      targetKind: "session",
      conversation: {
        channel: "telegram",
        accountId: "default",
  ...
  it("keeps plugin-owned bindings when ACP cleanup runs on startup", async () => {
    ...
    await getSessionBindingService().bind({
      targetSessionKey: "plugin-binding:FirstNexus-codex-app-server:still-valid",
      targetKind: "session",
      conversation: {
        channel: "telegram",
        accountId: "default",
    ...
    expect(reloaded.getByConversationId("plugin-binding-convo")?.targetSessionKey).toBe(
      "plugin-binding:FirstNexus-codex-app-server:still-valid",
    );
