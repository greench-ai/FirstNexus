...
type ResolveConfiguredBindingRouteFn =
  typeof import("FirstNexus/plugin-sdk/conversation-runtime").resolveConfiguredBindingRoute;
type EnsureConfiguredBindingRouteReadyFn =
  typeof import("FirstNexus/plugin-sdk/conversation-runtime").ensureConfiguredBindingRouteReady;
type DispatchReplyWithBufferedBlockDispatcherFn =
  typeof import("FirstNexus/plugin-sdk/reply-dispatch-runtime").dispatchReplyWithBufferedBlockDispatcher;
...
type LoadModelCatalogFn = typeof import("FirstNexus/plugin-sdk/agent-runtime").loadModelCatalog;
...
vi.mock("FirstNexus/plugin-sdk/command-auth-native", async () => {
  const actual = await vi.importActual<typeof import("FirstNexus/plugin-sdk/command-auth-native")>(
    "FirstNexus/plugin-sdk/command-auth-native",
  );
...
vi.mock("FirstNexus/plugin-sdk/agent-runtime", async () => {
  const actual = await vi.importActual<typeof import("FirstNexus/plugin-sdk/agent-runtime")>(
    "FirstNexus/plugin-sdk/agent-runtime",
  );
  return {
    ...actual,
...
vi.mock("FirstNexus/plugin-sdk/plugin-runtime", async () => {
...
describe("registerTelegramNativeCommands — session metadata", () => {
  ...
  beforeEach(() => {
    ...
    sessionMocks.resolveAndPersistSessionFile.mockClear().mockImplementation(async (params) => {
      const sessionFile =
        params.fallbackSessionFile ?? `/tmp/FirstNexus-sessions/${params.sessionId}.jsonl`;
      return {
        sessionFile,
        sessionEntry: {
          ...params.sessionEntry,
    ...
    sessionMocks.resolveStorePath.mockClear().mockReturnValue("/tmp/FirstNexus-sessions.json");
  ...
  it("uses the target session model when building native argument menus", async () => {
    ...
    expect(sessionMocks.loadSessionStore).toHaveBeenCalledWith("/tmp/FirstNexus-sessions.json");
    expectSendMessageCall({
      sendMessage,
      chatId: 100,
  ...
  it("passes a persisted topic session file to plugin commands", async () => {
    sessionMocks.resolveStorePath.mockReturnValue("/tmp/FirstNexus-sessions/sessions.json");
    sessionMocks.loadSessionStore.mockReturnValue({
    ...
    const { handler } = registerAndResolveCommandHandler({
      commandName: "codex",
      cfg: { commands: { allowFrom: { telegram: ["200"] } } } as FirstNexusConfig,
    ...
    expectRecordFields(
      firstMockArg(sessionMocks.resolveAndPersistSessionFile, "resolveAndPersistSessionFile"),
      {
        ...
        storePath: "/tmp/FirstNexus-sessions/sessions.json",
        sessionsDir: "/tmp/FirstNexus-sessions",
        fallbackSessionFile: path.resolve("/tmp/FirstNexus-sessions", "sess-topic-topic-42.jsonl"),
      },
    ...
    expectRecordFields(
      (pluginRuntimeMocks.executePluginCommand.mock.calls as unknown as Array<[unknown]>)[0]?.[0],
      {
        sessionKey: "agent:main:telegram:group:-1001234567890:topic:42",
        sessionId: "sess-topic",
        sessionFile: path.resolve("/tmp/FirstNexus-sessions", "sess-topic-topic-42.jsonl"),
        messageThreadId: 42,
      },
