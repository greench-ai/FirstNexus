...
function formatTelegramAccountConfigPath(cfg: FirstNexusConfig, accountId: string): string {
  const telegram = asObjectRecord((cfg.channels as Record<string, unknown> | undefined)?.telegram);
  const accounts = asObjectRecord(telegram?.accounts);
...
export function maybeRepairTelegramApiRoots(cfg: FirstNexusConfig): {
...
async function repairTelegramConfig(params: { cfg: FirstNexusConfig }): Promise<{
  config: FirstNexusConfig;
  changes: string[];
}> {
...
export async function maybeRepairTelegramAllowFromUsernames(cfg: FirstNexusConfig): Promise<{
  config: FirstNexusConfig;
  changes: string[];
}> {
  ...
  const { getChannelsCommandSecretTargetIds, resolveCommandSecretRefsViaGateway } =
    await import("FirstNexus/plugin-sdk/runtime");
