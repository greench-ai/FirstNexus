...
const getAgentScopedMediaLocalRoots = vi.hoisted(() =>
  vi.fn((_cfg: unknown, agentId: string) => [`/tmp/.FirstNexus/workspace-${agentId}`]),
);
...
vi.mock("FirstNexus/plugin-sdk/agent-harness-runtime", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("FirstNexus/plugin-sdk/agent-harness-runtime")>();
  return {
    ...actual,
...
describe("dispatchTelegramMessage draft streaming", () => {
  ...
  function createContext(overrides?: Partial<TelegramMessageContext>): TelegramMessageContext {
    ...
    base.turn = {
      storePath: "/tmp/FirstNexus/telegram-sessions.json",
  ...
  it("streams drafts in private threads and forwards thread id", async () => {
    ...
    expect(
      mediaLocalRoots?.some((root) => /[\\/]\.FirstNexus[\\/]workspace-work$/u.test(root)),
    ).toBe(true);
