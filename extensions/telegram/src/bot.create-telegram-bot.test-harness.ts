...
type GetRuntimeConfigFn =
  typeof import("FirstNexus/plugin-sdk/runtime-config-snapshot").getRuntimeConfig;
type LoadSessionStoreFn =
  typeof import("FirstNexus/plugin-sdk/session-store-runtime").loadSessionStore;
type ResolveStorePathFn =
  typeof import("FirstNexus/plugin-sdk/session-store-runtime").resolveStorePath;
...
const { sessionStorePath } = vi.hoisted(() => ({
  sessionStorePath: `/tmp/FirstNexus-telegram-${process.pid}-${process.env.VITEST_POOL_ID ?? "0"}.json`,
}));
...
function resolveDefaultModelForAgentForTest(params: { cfg: FirstNexusConfig }): {
  provider: string;
  model: string;
} {
...
function createModelsProviderDataFromConfig(cfg: FirstNexusConfig): {
...
const DEFAULT_TELEGRAM_TEST_CONFIG: FirstNexusConfig = {
  agents: {
    defaults: {
...
beforeEach(() => {
  ...
  buildModelsProviderData.mockImplementation(async (cfg: FirstNexusConfig) => {
