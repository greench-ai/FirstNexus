...
import { resolveNativeCommandSessionTargets } from "FirstNexus/plugin-sdk/command-auth-native";
import {
...
import { getRuntimeConfigSnapshot } from "FirstNexus/plugin-sdk/runtime-config-snapshot";
...
type TelegramChunkMode = ReturnType<
  typeof import("FirstNexus/plugin-sdk/reply-dispatch-runtime").resolveChunkMode
>;
type TelegramNativeReplyPayload =
  import("FirstNexus/plugin-sdk/reply-dispatch-runtime").ReplyPayload;
...
export const registerTelegramNativeCommands = ({
  ...
  const loadFreshRuntimeConfig = (): FirstNexusConfig => telegramDeps.getRuntimeConfig();
  const resolveFreshTelegramConfig = (runtimeCfg: FirstNexusConfig): TelegramAccountConfig => {
    try {
