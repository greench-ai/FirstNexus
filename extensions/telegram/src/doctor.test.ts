...
vi.mock("FirstNexus/plugin-sdk/runtime", () => {
  return {
...
describe("telegram doctor", () => {
  ...
  it("formats invalid allowFrom warnings", () => {
    ...
    expect(warnings[1]).toContain("FirstNexus doctor --fix");
  });
  ...
  it("warns and repairs Telegram apiRoot values that include the bot endpoint", () => {
    ...
    expect(
      collectTelegramApiRootWarnings({ hits, doctorFixCommand: "FirstNexus doctor --fix" }),
    ).toContain(
