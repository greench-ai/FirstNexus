...
describe("handleTelegramAction", () => {
  ...
  function telegramConfig(overrides?: Record<string, unknown>): FirstNexusConfig {
    return {
      channels: {
        telegram: {
          botToken: "tok",
  ...
  it("rejects sticker actions when disabled by default", async () => {
    const cfg = { channels: { telegram: { botToken: "tok" } } } as FirstNexusConfig;
    await expect(
...
describe("handleTelegramAction per-account gating", () => {
  ...
  async function expectAccountStickerSend(cfg: FirstNexusConfig, accountId = "media") {
    await handleTelegramAction(
