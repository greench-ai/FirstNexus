...
import type { FirstNexusPluginApi } from "FirstNexus/plugin-sdk/plugin-runtime";
...
function fakeApi(): FirstNexusPluginApi {
  return {
    config: {},
...
describe("tavily tools", () => {
  ...
  it("normalizes optional parameters before invoking Tavily", async () => {
    ...
    const result = await tool.execute("call-1", {
      ...
      include_answer: true,
      time_range: "week",
      include_domains: ["docs.FirstNexus.ai", "", "FirstNexus.ai"],
    ...
    expect(runTavilySearch).toHaveBeenCalledWith({
      ...
      includeAnswer: true,
      timeRange: "week",
      includeDomains: ["docs.FirstNexus.ai", "FirstNexus.ai"],
  ...
  it("late-binds dedicated tools to the resolved runtime config snapshot", async () => {
    ...
    const registeredTools: Array<Parameters<FirstNexusPluginApi["registerTool"]>[0]> = [];
    const registeredOptions: Array<Parameters<FirstNexusPluginApi["registerTool"]>[1]> = [];
    ...
    await searchTool.execute("search-call", { query: "FirstNexus" });
    ...
    expect(searchParams.query).toBe("FirstNexus");
  ...
  it("falls back to environment values and defaults", () => {
    ...
    expect(resolveTavilyBaseUrl({} as FirstNexusConfig)).not.toBe(DEFAULT_TAVILY_BASE_URL);
