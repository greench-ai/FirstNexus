...
import type { ProviderAuthContext } from "FirstNexus/plugin-sdk/plugin-entry";
...
const openAICodexOAuthOriginator = "FirstNexus";
const localManualFallbackDelayMs = 15_000;
const localManualFallbackGraceMs = 1_000;
const openAIAuthProbeUrl =
  "https://auth.openai.com/oauth/authorize?response_type=code&client_id=FirstNexus-preflight&redirect_uri=http%3A%2F%2Flocalhost%3A1455%2Fauth%2Fcallback&scope=openid+profile+email";
  ...
  if (/unsupported_country_region_territory/i.test(message)) {
    return createOpenAICodexOAuthError(
      ...
      [
        ...
        "If you normally use a proxy, verify HTTPS_PROXY, HTTP_PROXY, or ALL_PROXY is set for the FirstNexus process and then retry `FirstNexus models auth login --provider openai-codex`.",
      ].join(" "),
      error,
    );
  }
...
export async function loginOpenAICodexOAuth(params: {
  ...
  try {
    ...
    await prompter.note(
      "Trouble with OAuth? See https://docs.FirstNexus.ai/start/faq",
      "OAuth help",
    );
