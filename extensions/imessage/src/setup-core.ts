...
export const imessageDmPolicy = {
  ...
  resolveConfigKeys: (_cfg: FirstNexusConfig, accountId?: string) => {
  ...
  getCurrent: (cfg: FirstNexusConfig, accountId?: string) => {
...
function resolveIMessageCliPath(params: { cfg: FirstNexusConfig; accountId: string }) {
...
export const imessageCompletionNote = {
  ...
  lines: [
    "Run FirstNexus on the Mac signed into Messages, or set cliPath to an SSH wrapper that runs imsg on that Mac.",
    "Linux/Windows hosts cannot run the default local imsg path directly.",
    "Run `imsg launch`, then `FirstNexus channels status --probe` to verify private API actions.",
    "Ensure FirstNexus has Full Disk Access to Messages DB.",
...
export const imessageSetupStatusBase = {
  ...
  configuredScore: 1,
  unconfiguredScore: 0,
  resolveConfigured: ({ cfg, accountId }: { cfg: FirstNexusConfig; accountId?: string }) =>
