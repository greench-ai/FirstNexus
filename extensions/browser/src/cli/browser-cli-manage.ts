...
async function runBrowserToggle(
  ...
  const name = status.profile ?? "FirstNexus";
...
async function runBrowserDoctor(parent: BrowserParentOpts, profile?: string, deep?: boolean) {
  ...
  checks.push({
    name: "profile",
    ok: true,
    detail: `${status.profile ?? "FirstNexus"} (${usesChromeMcpTransport(status) ? "chrome-mcp" : (status.transport ?? "cdp")})`,
  });
  checks.push({
    name: "browser",
    ok: status.running,
    detail: status.running
      ...
      : "not running; run `FirstNexus browser start`",
  });
...
export function registerBrowserManageCommands(
        ...
        defaultRuntime.log(
          profiles
            .map((p) => {
              ...
              const driver = p.driver !== "FirstNexus" ? ` [${p.driver}]` : "";
  ...
  browser
    ...
    .option(
      "--driver <driver>",
      "Profile driver (FirstNexus|existing-session). Default: FirstNexus",
    )
    .action(
      async (
        opts: {
