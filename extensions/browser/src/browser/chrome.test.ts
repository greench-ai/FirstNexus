...
type StopChromeTarget = Parameters<typeof stopFirstNexusChrome>[0];
...
describe("browser chrome profile decoration", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fsp.mkdtemp(path.join(os.tmpdir(), "FirstNexus-chrome-suite-"));
  });
  ...
  it("writes expected name + signed ARGB seed to Chrome prefs", async () => {
    ...
    decorateFirstNexusProfile(userDataDir, { color: DEFAULT_NEXISCLAW_BROWSER_COLOR });
    ...
    const marker = await fsp.readFile(
      path.join(userDataDir, ".FirstNexus-profile-decorated"),
      "utf-8",
    );
  ...
  it("best-effort writes name when color is invalid", async () => {
    const userDataDir = await createUserDataDir();
    decorateFirstNexusProfile(userDataDir, { color: "lobster-orange" });
    const def = await readDefaultProfileFromLocalState(userDataDir);
...
describe("browser chrome helpers", () => {
  ...
  it("finds Playwright-managed Linux Chromium", () => {
    const browserPath = fs.mkdtempSync(path.join(os.tmpdir(), "FirstNexus-ms-playwright-"));
  ...
  it("stopFirstNexusChrome no-ops when process is already killed", async () => {
    const proc = makeChromeTestProc({ killed: true });
    await stopChromeWithProc(proc, 10);
    expect(proc.kill).not.toHaveBeenCalled();
  });

  it("stopFirstNexusChrome sends SIGTERM and returns once CDP is down", async () => {
  ...
  it("stopFirstNexusChrome escalates to SIGKILL when CDP stays reachable", async () => {
    vi.stubGlobal(
      "fetch",
      vi.fn().mockResolvedValue({
        ok: true,
...
describe("browser chrome launch args", () => {
  it("does not force an about:blank tab at startup", () => {
    const args = buildFirstNexusChromeLaunchArgs({
      ...
      userDataDir: "/tmp/FirstNexus-test-user-data",
    });

    expect(args).not.toContain("about:blank");
    expect(args).toContain("--remote-debugging-port=18800");
    expect(args).toContain("--user-data-dir=/tmp/FirstNexus-test-user-data");
  });
});
