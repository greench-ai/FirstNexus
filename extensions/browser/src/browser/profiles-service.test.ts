...
import { resolveFirstNexusUserDataDir } from "./chrome.js";
...
vi.mock("./chrome.js", () => ({
  resolveFirstNexusUserDataDir: vi.fn(() => "/tmp/FirstNexus-test/FirstNexus/user-data"),
}));
...
describe("BrowserProfilesService", () => {
  ...
  it("creates existing-session profiles with an explicit userDataDir", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "FirstNexus-profile-"));
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    vi.mocked(resolveFirstNexusUserDataDir).mockReturnValue(userDataDir);
