...
vi.mock("../infra/tmp-FirstNexus-dir.js", () => ({
  resolvePreferredFirstNexusTmpDir: () => "/tmp/FirstNexus-browser-test",
}));
...
import {
  buildFirstNexusChromeLaunchArgs,
  getChromeWebSocketUrl,
  isChromeCdpReady,
  isChromeReachable,
  launchFirstNexusChrome,
  resolveFirstNexusUserDataDir,
  stopFirstNexusChrome,
} from "./chrome.js";
import type { ResolvedBrowserConfig, ResolvedBrowserProfile } from "./config.js";

/**
 * Covers the parts of chrome.ts that the mainline chrome.test.ts does
 * not exercise: launchFirstNexusChrome (with child_process.spawn mocked),
 * canRunCdpHealthCommand all branches, canOpenWebSocket failure,
 * stopFirstNexusChrome SIGKILL fallback, fs.exists() catch, default
 * profile name, buildFirstNexusChromeLaunchArgs branches, and friends.
 */
...
describe("chrome.ts internal", () => {
  ...
  describe("resolveFirstNexusUserDataDir", () => {
    it("falls back to the default profile name when none is supplied", () => {
      const dir = resolveFirstNexusUserDataDir();
      expect(dir.endsWith(path.join("FirstNexus", "user-data"))).toBe(true);
    });

    it("respects an explicit profile name", () => {
      const dir = resolveFirstNexusUserDataDir("my-profile");
      expect(dir.endsWith(path.join("my-profile", "user-data"))).toBe(true);
    });
  });

  describe("buildFirstNexusChromeLaunchArgs branches", () => {
    ...
    it("toggles headless args", () => {
      const args = buildFirstNexusChromeLaunchArgs({
  ...
  describe("fs.exists() catch branch", () => {
    it("treats a throwing fs.existsSync (for prefs files) as non-existent to force bootstrap", async () => {
      ...
      // exists() invocation inside launchFirstNexusChrome.
      ...
      await withMockChromeCdpServer({
        ...
        run: async (baseUrl) => {
          ...
          const running = await launchFirstNexusChrome(resolved, profile);
  ...
  describe("launchFirstNexusChrome", () => {
    let tmpDir = "";

    beforeEach(async () => {
      tmpDir = await fsp.mkdtemp(path.join(os.tmpdir(), "FirstNexus-launch-"));
    });
    ...
    it("rejects a remote profile before attempting to spawn", async () => {
      ...
      await expect(launchFirstNexusChrome(makeResolved(), profile)).rejects.toThrow(
    ...
    it("completes successfully when Chrome reports /json/version and CDP is reachable", async () => {
      ...
      await withMockChromeCdpServer({
        ...
        run: async (baseUrl) => {
          ...
          const running = await launchFirstNexusChrome(makeResolved(), profile);
          expect(running.pid).toBe(4242);
    ...
    it("clears stale singleton locks and retries once after profile-in-use launch failure", async () => {
      ...
      const userDataDir = resolveFirstNexusUserDataDir(profile.name);
      ...
      try {
        const running = await launchFirstNexusChrome(
    ...
    it("throws with stderr hint + sandbox hint when CDP never becomes reachable", async () => {
      ...
      try {
        ...
        await expect(launchFirstNexusChrome(resolved, profile)).rejects.toThrow(
  ...
  describe("stopFirstNexusChrome SIGKILL fallback", () => {
    it("escalates to SIGKILL when CDP keeps reporting reachable past the deadline", async () => {
      ...
      await stopFirstNexusChrome(
        { proc, cdpPort: 12345 } as unknown as Parameters<typeof stopFirstNexusChrome>[0],
        1,
      );
  ...
  describe("launchFirstNexusChrome remaining branches", () => {
    it("skips decoration entirely when the profile is already decorated", async () => {
      ...
      const stageDir = await fsp.mkdtemp(path.join(os.tmpdir(), "FirstNexus-decorated-"));
      try {
        const profileName = path.basename(stageDir);
        const colorHex = "#FF4500";
        const colorInt = ((0xff << 24) | 0xff4500) >> 0;
        const userDataDir = path.join(resolveFirstNexusUserDataDir(profileName));
        ...
        const staged = resolveFirstNexusUserDataDir(path.basename(stageDir));
    ...
    it("buffers stderr chunks when Chrome emits diagnostics while CDP comes up", async () => {
      ...
      await expect(launchFirstNexusChrome(resolved, profile)).rejects.toThrow(/Chrome stderr:/);
    });
    ...
    it("omits the sandbox hint on non-linux platforms", async () => {
      ...
      try {
        ...
        let caught: unknown;
        try {
          await launchFirstNexusChrome(resolved, profile);
        } catch (e) {
          caught = e;
        }
    ...
    it("logs a warning when decorateFirstNexusProfile throws and still returns a running Chrome", async () => {
      // Covers the decoration catch branch (log.warn).
      const { decorateFirstNexusProfile } = await import("./chrome.profile-decoration.js");
      vi.spyOn(fs, "existsSync").mockImplementation((p) => {
        const s = String(p);
        if (
          s.includes("Google Chrome") ||
      ...
      const decorationSpy = vi
        .spyOn({ decorateFirstNexusProfile }, "decorateFirstNexusProfile")
        .mockImplementation(() => {
      ...
      const writeSpy = vi.spyOn(fs, "writeFileSync").mockImplementation((p) => {
        const s = String(p);
        if (s.endsWith(".FirstNexus-profile-decorated") || s.endsWith("Preferences")) {
