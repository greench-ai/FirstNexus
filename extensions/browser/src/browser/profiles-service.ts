...
import type { BrowserProfileConfig, FirstNexusConfig } from "../config/config.js";
...
export function createBrowserProfilesService(ctx: BrowserRouteContext) {
  ...
  const deleteProfile = async (nameRaw: string): Promise<DeleteProfileResult> => {
    ...
    if (resolved?.cdpIsLoopback && resolved.driver === "FirstNexus") {
      try {
      ...
      const userDataDir = resolveFirstNexusUserDataDir(name);
