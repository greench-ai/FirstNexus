...
function setupEnsureBrowserAvailableHarness() {
  ...
  const launchFirstNexusChrome = vi.mocked(chromeModule.launchFirstNexusChrome);
  const stopFirstNexusChrome = vi.mocked(chromeModule.stopFirstNexusChrome);
  ...
  const profile = ctx.forProfile("FirstNexus");

  return { launchFirstNexusChrome, stopFirstNexusChrome, isChromeCdpReady, profile, state };
}
...
function requireFirstLaunchOptions(launchFirstNexusChrome: {
  mock: { calls: unknown[][] };
}): unknown {
  const [call] = launchFirstNexusChrome.mock.calls;
  if (!call) {
...
describe("browser server-context ensureBrowserAvailable", () => {
  it("waits for CDP readiness after launching to avoid follow-up PortInUseError races (#21149)", async () => {
    const { launchFirstNexusChrome, stopFirstNexusChrome, isChromeCdpReady, profile } =
      setupEnsureBrowserAvailableHarness();
    isChromeCdpReady.mockResolvedValueOnce(false).mockResolvedValue(true);
    mockLaunchedChrome(launchFirstNexusChrome, 123);

    const promise = profile.ensureBrowserAvailable();
    await vi.advanceTimersByTimeAsync(100);
    await expect(promise).resolves.toBeUndefined();

    expect(launchFirstNexusChrome).toHaveBeenCalledTimes(1);
    expect(isChromeCdpReady).toHaveBeenCalled();
    expect(stopFirstNexusChrome).not.toHaveBeenCalled();
  });
  ...
  it("stops launched chrome when CDP readiness never arrives", async () => {
    ...
    mockLaunchedChrome(launchFirstNexusChrome, 321);
    ...
    expect(launchFirstNexusChrome).toHaveBeenCalledTimes(1);
    expect(stopFirstNexusChrome).toHaveBeenCalledTimes(1);
  });

  it("uses configured local CDP readiness timeout after launching", async () => {
    const { launchFirstNexusChrome, stopFirstNexusChrome, isChromeCdpReady, profile, state } =
      setupEnsureBrowserAvailableHarness();
    state.resolved.localCdpReadyTimeoutMs = 250;
    isChromeCdpReady.mockResolvedValue(false);
    mockLaunchedChrome(launchFirstNexusChrome, 322);
  ...
  it("deduplicates concurrent lazy-start calls to prevent PortInUseError", async () => {
    ...
    mockLaunchedChrome(launchFirstNexusChrome, 456);
  ...
  it("deduplicates concurrent lazy-start calls across fresh profile contexts", async () => {
    const { launchFirstNexusChrome, stopFirstNexusChrome, isChromeCdpReady, state } =
      setupEnsureBrowserAvailableHarness();
    isChromeCdpReady.mockResolvedValue(true);
    mockLaunchedChrome(launchFirstNexusChrome, 457);

    const firstCtx = createBrowserRouteContext({ getState: () => state });
    const secondCtx = createBrowserRouteContext({ getState: () => state });
    const first = firstCtx.forProfile("FirstNexus").ensureBrowserAvailable();
    const second = secondCtx.forProfile("FirstNexus").ensureBrowserAvailable();
    await vi.advanceTimersByTimeAsync(100);
    await expect(Promise.all([first, second])).resolves.toEqual([undefined, undefined]);
  ...
  it("passes request-local headless override to initial launch", async () => {
    const { launchFirstNexusChrome, stopFirstNexusChrome, isChromeCdpReady, profile } =
      setupEnsureBrowserAvailableHarness();
    isChromeCdpReady.mockResolvedValue(true);
    mockLaunchedChrome(launchFirstNexusChrome, 654);
    ...
    expect(launchFirstNexusChrome).toHaveBeenCalledTimes(1);
    expect(requireFirstLaunchOptions(launchFirstNexusChrome)).toEqual({ headlessOverride: true });
    expect(stopFirstNexusChrome).not.toHaveBeenCalled();
  });
  ...
  it("passes request-local headless override to the owned restart path", async () => {
    ...
    state.profiles.set("FirstNexus", {
      profile: profile.profile,
      running: {
        pid: 111,
        exe: { kind: "chromium", path: "/usr/bin/chromium" },
        userDataDir: "/tmp/FirstNexus-test",
        cdpPort: 18800,
        startedAt: Date.now(),
        proc: existingProc,
      },
    ...
    mockLaunchedChrome(launchFirstNexusChrome, 987);
  ...
  it("does not share inflight lazy-start promises across different headless overrides", async () => {
    const { launchFirstNexusChrome, isChromeCdpReady, profile } =
      setupEnsureBrowserAvailableHarness();
    const isChromeReachable = vi.mocked(chromeModule.isChromeReachable);
  ...
  it("clears the concurrent lazy-start guard after launch failure", async () => {
    const { launchFirstNexusChrome, stopFirstNexusChrome, isChromeCdpReady, profile } =
      setupEnsureBrowserAvailableHarness();
    isChromeCdpReady.mockResolvedValue(true);
    launchFirstNexusChrome.mockRejectedValueOnce(
    ...
    mockLaunchedChrome(launchFirstNexusChrome, 789);
    const retry = profile.ensureBrowserAvailable();
    await vi.advanceTimersByTimeAsync(100);
    await expect(retry).resolves.toBeUndefined();

    expect(launchFirstNexusChrome).toHaveBeenCalledTimes(2);
    expect(stopFirstNexusChrome).not.toHaveBeenCalled();
  });
  ...
  it("cools down repeated managed Chrome launch failures across route contexts", async () => {
    const { launchFirstNexusChrome, stopFirstNexusChrome, isChromeCdpReady, state } =
      setupEnsureBrowserAvailableHarness();
    isChromeCdpReady.mockResolvedValue(true);
    launchFirstNexusChrome.mockRejectedValue(new Error("Failed to start Chrome CDP"));

    for (let attempt = 0; attempt < 3; attempt += 1) {
      const ctx = createBrowserRouteContext({ getState: () => state });
      await expect(ctx.forProfile("FirstNexus").ensureBrowserAvailable()).rejects.toThrow(
        "Failed to start Chrome CDP",
      );
    }

    const cooledDownCtx = createBrowserRouteContext({ getState: () => state });
    await expect(cooledDownCtx.forProfile("FirstNexus").ensureBrowserAvailable()).rejects.toThrow(
      'Browser launch for profile "FirstNexus" is cooling down after 3 consecutive managed Chrome launch failures.',
    );
    await expect(cooledDownCtx.forProfile("FirstNexus").ensureBrowserAvailable()).rejects.toThrow(
      "set browser.enabled=false if the browser tool is not needed",
    );

    expect(launchFirstNexusChrome).toHaveBeenCalledTimes(3);
    expect(stopFirstNexusChrome).not.toHaveBeenCalled();
  });

  it("allows one managed Chrome launch attempt after the cooldown expires", async () => {
    const { launchFirstNexusChrome, isChromeCdpReady, state } =
      setupEnsureBrowserAvailableHarness();
    isChromeCdpReady.mockResolvedValue(true);
    launchFirstNexusChrome.mockRejectedValue(new Error("Failed to start Chrome CDP"));
    ...
    await expect(retryCtx.forProfile("FirstNexus").ensureBrowserAvailable()).rejects.toThrow(
      "Failed to start Chrome CDP",
    );

    expect(launchFirstNexusChrome).toHaveBeenCalledTimes(4);
  });
  ...
  it("reuses a pre-existing loopback browser after an initial short probe miss", async () => {
    ...
    expect(launchFirstNexusChrome).not.toHaveBeenCalled();
    expect(stopFirstNexusChrome).not.toHaveBeenCalled();
  });
  ...
  it("explains attachOnly for externally managed loopback CDP services", async () => {
    ...
    await expect(promise).rejects.toThrow(
      'Port 18800 is in use for profile "FirstNexus" but not by FirstNexus.',
    );
    await expect(promise).rejects.toThrow(
      "set browser.profiles.FirstNexus.attachOnly=true so FirstNexus attaches without trying to manage the local process",
    );
    await expect(promise).rejects.toThrow(
      "For Browserless Docker, set EXTERNAL to the same WebSocket endpoint FirstNexus can reach via browser.profiles.<name>.cdpUrl.",
    );
  ...
  it("retries remote CDP websocket reachability once before failing", async () => {
    const { launchFirstNexusChrome, stopFirstNexusChrome, isChromeCdpReady } =
      setupEnsureBrowserAvailableHarness();
    const isChromeReachable = vi.mocked(chromeModule.isChromeReachable);
  ...
  it("treats attachOnly loopback CDP as local control with remote-class probe timeouts", async () => {
    const { launchFirstNexusChrome, stopFirstNexusChrome } = setupEnsureBrowserAvailableHarness();
    const isChromeReachable = vi.mocked(chromeModule.isChromeReachable);
    const isChromeCdpReady = vi.mocked(chromeModule.isChromeCdpReady);
