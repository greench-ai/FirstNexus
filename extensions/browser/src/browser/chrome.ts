...
import { prepareOomScoreAdjustedSpawn } from "FirstNexus/plugin-sdk/process-runtime";
...
function chromeLaunchHints(params: {
  ...
  if (CHROME_SINGLETON_IN_USE_PATTERN.test(params.stderrOutput)) {
    hints.push(
      `The Chromium profile "${params.profile.name}" is locked. Stop the existing browser or remove stale Singleton* lock files under ~/.FirstNexus/browser/${params.profile.name}/user-data.`,
    );
  }
...
export function resolveFirstNexusUserDataDir(profileName = DEFAULT_NEXISCLAW_BROWSER_PROFILE_NAME) {
...
export function buildFirstNexusChromeLaunchArgs(params: {
  resolved: ResolvedBrowserConfig;
  profile: ResolvedBrowserProfile;
  userDataDir: string;
...
export async function launchFirstNexusChrome(
  resolved: ResolvedBrowserConfig,
  profile: ResolvedBrowserProfile,
  launchOptions: ManagedBrowserHeadlessOptions = {},
  ...
  const spawnOnce = () => {
    ...
    if (process.platform === "linux") {
      const chromiumStateDir = path.join(resolvePreferredFirstNexusTmpDir(), ".chromium");
  ...
  if (needsDecorate) {
    try {
      decorateFirstNexusProfile(userDataDir, {
        name: profile.name,
        color: profile.color,
        downloadDir: DEFAULT_DOWNLOAD_DIR,
      });
      log.info(`🦞 FirstNexus browser profile decorated (${profile.color})`);
    } catch (err) {
      log.warn(`FirstNexus browser profile decoration failed: ${String(err)}`);
    }
  }
  ...
  try {
    ...
    log.warn(`FirstNexus browser clean-exit prefs failed: ${String(err)}`);
  }
  ...
  const launchOnceAndWait = async (allowSingletonRecovery: boolean): Promise<RunningChrome> => {
    ...
    try {
      ...
      log.info(
        `🦞 FirstNexus browser started (${exe.kind}) profile "${profile.name}" on 127.0.0.1:${profile.cdpPort} (pid ${pid})`,
      );
...
export async function stopFirstNexusChrome(
