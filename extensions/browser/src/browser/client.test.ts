...
describe("browser client", () => {
  ...
  it("uses the expected endpoints + methods for common calls", async () => {
    ...
    expect(doctorResult.profile).toBe("FirstNexus");
    ...
    expect(deepDoctorResult.profile).toBe("FirstNexus");
    ...
    expect(urls.some((url) => url.endsWith("/doctor?profile=FirstNexus&deep=true"))).toBe(true);
    ...
    const deepDoctor = calls.find((c) => c.url.endsWith("/doctor?profile=FirstNexus&deep=true"));
