...
const mocks = vi.hoisted(() => ({
  getRuntimeConfig: vi.fn<() => FirstNexusConfig>(),
  writeConfigFile: vi.fn<(cfg: FirstNexusConfig) => Promise<void>>(async (_cfg) => {}),
  replaceConfigFile: vi.fn(async ({ nextConfig }: { nextConfig: FirstNexusConfig }) => {
  ...
  resolveGatewayAuth: vi.fn(
    ({
      ...
      authConfig?: NonNullable<NonNullable<FirstNexusConfig["gateway"]>["auth"]>;
    }) => {
      const token =
  ...
  ensureGatewayStartupAuth: vi.fn(async ({ cfg }: { cfg: FirstNexusConfig }) => ({
    cfg: {
      ...cfg,
      gateway: {
        ...cfg.gateway,
...
function readPersistedConfig(): FirstNexusConfig {
...
async function expectUnresolvedBrowserSecretRefSkipsPersistence(cfg: FirstNexusConfig) {
