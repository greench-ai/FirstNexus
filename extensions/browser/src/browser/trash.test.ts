...
const resolvePreferredFirstNexusTmpDirMock = vi.hoisted(() => vi.fn(() => "/tmp/FirstNexus"));
...
describe("browser trash", () => {
  ...
  beforeEach(() => {
    ...
    testRoot = realRealpathSyncNative(
      realMkdtempSync(path.join(os.tmpdir(), "FirstNexus-browser-")),
    );
    ...
    resolvePreferredFirstNexusTmpDirMock.mockReturnValue(tmpDir);
  ...
  it("refuses to trash paths outside allowed roots", async () => {
    ...
    const outsidePath = path.join(outsideDir, "FirstNexus-demo");
