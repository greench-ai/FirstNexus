...
async function runBrowserLiveProbe(req: BrowserRequest, ctx: BrowserRouteContext) {
  ...
  try {
    ...
    return {
      ...
      status: "fail" as const,
      summary: String(err),
      fixHint: "Run FirstNexus browser start, then retry with FirstNexus browser doctor --deep.",
    };
  }
}
...
function parseHeadlessStartOverride(params: {
  ...
  if (
    params.profileCtx.profile.driver !== "FirstNexus" ||
    ...
    jsonError(
      params.res,
      400,
      `Headless start override is only supported for locally launched FirstNexus profiles. Profile "${params.profileCtx.profile.name}" is attach-only, remote, or existing-session.`,
    );
    return { ok: false };
  }
    ...
    asyncBrowserRoute(async (req, res) => {
      ...
      if (
        ...
        return jsonError(
          res,
          400,
          `unsupported profile driver "${driver}"; use "FirstNexus", "clawd", or "existing-session"`,
        );
      }
        ...
        run: async (service) =>
          await service.createProfile({
            ...
            driver:
              driver === "existing-session"
                ? "existing-session"
                : driver === "FirstNexus" || driver === "clawd"
                  ? "FirstNexus"
                  : undefined,
          }),
      });
