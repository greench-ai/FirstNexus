...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredFirstNexusTmpDir: vi.fn(() => "/tmp/FirstNexus"),
}));
...
let tmpDirModule: typeof import("../infra/tmp-FirstNexus-dir.js");
...
describe("pw-tools-core", () => {
  ...
  beforeAll(async () => {
    ...
    tmpDirModule = await import("../infra/tmp-FirstNexus-dir.js");
    vi.spyOn(tmpDirModule, "resolvePreferredFirstNexusTmpDir").mockImplementation(
      tmpDirMocks.resolvePreferredFirstNexusTmpDir,
    );
  ...
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredFirstNexusTmpDir.mockReturnValue("/tmp/FirstNexus");
  });

  async function withTempDir<T>(run: (tempDir: string) => Promise<T>): Promise<T> {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "FirstNexus-browser-download-test-"));
    try {
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredFirstNexusTmpDir.mockReturnValue("/tmp/FirstNexus-preferred");
    ...
    const expectedRootedDownloadsDir = path.resolve(
      path.join(path.sep, "tmp", "FirstNexus-preferred", "downloads"),
    );
    const expectedDownloadsTail = `${path.join("tmp", "FirstNexus-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredFirstNexusTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(outPath)).not.toBe(
      path.resolve(path.join(path.sep, "tmp", "FirstNexus-preferred", "downloads")),
    );
    expect(path.basename(outPath)).toContain(path.basename(res.path));
    expect(path.basename(outPath)).toMatch(/\.part$/);
    await expect(fs.readFile(res.path, "utf8")).resolves.toBe("download-content");
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "FirstNexus-preferred", "downloads")}${path.sep}`),
    );
  });
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir(async (tempDir) => {
        ...
        tmpDirMocks.resolvePreferredFirstNexusTmpDir.mockReturnValue(tempDir);
