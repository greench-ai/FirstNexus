...
const BROWSER_CLI_DESCRIPTOR = {
  name: "browser",
  description: "Manage FirstNexus's dedicated browser (Chrome/Chromium)",
  hasSubcommands: true,
};
...
function createLazyBrowserTool(opts?: {
  ...
  return {
    ...
    name: "browser",
    description: [
      "Control the browser via FirstNexus's browser control server (status/start/stop/profiles/tabs/open/snapshot/screenshot/actions).",
      "Browser choice: omit profile by default for the isolated FirstNexus-managed browser (`FirstNexus`).",
...
export const browserPluginNodeHostCommands: FirstNexusPluginNodeHostCommand[] = [
  {
...
export const browserSecurityAuditCollectors: FirstNexusPluginSecurityAuditCollector[] = [
  async (ctx) => {
...
function createLazyBrowserPluginService(): FirstNexusPluginService {
  let service: FirstNexusPluginService | null = null;
...
export function registerBrowserPlugin(api: FirstNexusPluginApi) {
  api.registerTool(((ctx: FirstNexusPluginToolContext) =>
