 ...
 * `__NEXISCLAW_REDACTED__` literal anywhere in the output throws
 ...
 *
 * @module @FirstNexus/oc-path/emit
 */
...
export function emitMd(ast: MdAst, opts: EmitOptions = {}): string {
  ...
  if (mode === "roundtrip") {
    ...
    if (!acceptPreExisting && ast.raw.includes("__NEXISCLAW_REDACTED__")) {
      guardSentinel("__NEXISCLAW_REDACTED__", `${guardPath}/[raw]`);
    }
    return ast.raw;
  }
