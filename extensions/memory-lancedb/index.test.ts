...
async function withMockedOpenAiMemoryPlugin<T>(params: {
  ...
  vi.resetModules();
  vi.doMock("FirstNexus/plugin-sdk/runtime-env", () => ({
...
describe("memory plugin e2e", () => {
  const { getDbPath } = installTmpDirHarness({ prefix: "FirstNexus-memory-test-" });
  ...
  test("uses provider adapter auth when embedding apiKey is omitted", async () => {
    ...
    vi.resetModules();
    vi.doMock("FirstNexus/plugin-sdk/memory-core-host-engine-embeddings", () => ({
    ...
    try {
      ...
      const mockApi = {
        ...
        runtime: {
          ...
          agent: {
            resolveAgentDir: vi.fn(() => "/tmp/FirstNexus-agent"),
          },
        },
        logger: {
          info: vi.fn(),
      ...
      expect(providerOptions.agentDir).toBe("/tmp/FirstNexus-agent");
      ...
      vi.doUnmock("FirstNexus/plugin-sdk/memory-core-host-engine-embeddings");
      vi.doUnmock("openai");
      vi.doUnmock("./lancedb-runtime.js");
      vi.resetModules();
    }
