...
import { canResolveEnvSecretRefInReadOnlyPath } from "FirstNexus/plugin-sdk/extension-shared";
import { resolveSecretInputString, normalizeSecretInput } from "FirstNexus/plugin-sdk/secret-input";
...
type WebFetchConfig = NonNullable<FirstNexusConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: FirstNexusConfig): WebSearchConfig {
...
function resolveFetchConfig(cfg?: FirstNexusConfig): WebFetchConfig {
...
export function resolveFirecrawlSearchConfig(cfg?: FirstNexusConfig): FirecrawlSearchConfig {
...
function resolveFirecrawlFetchConfig(cfg?: FirstNexusConfig): FirecrawlFetchConfig {
  const pluginConfig = cfg?.plugins?.entries?.firecrawl?.config as PluginEntryConfig;
...
export function resolveFirecrawlApiKey(cfg?: FirstNexusConfig): string | undefined {
  const pluginConfig = cfg?.plugins?.entries?.firecrawl?.config as PluginEntryConfig;
...
export function resolveFirecrawlBaseUrl(cfg?: FirstNexusConfig): string {
  const search = resolveFirecrawlSearchConfig(cfg);
  const fetch = resolveFirecrawlFetchConfig(cfg);
  const configured =
...
export function resolveFirecrawlMaxAgeMs(cfg?: FirstNexusConfig, override?: number): number {
