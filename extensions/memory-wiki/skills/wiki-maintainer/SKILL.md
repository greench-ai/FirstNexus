...
description: Maintain the FirstNexus memory wiki vault with deterministic pages, managed blocks, and source-backed updates.
---
...
- Use `FirstNexus wiki ingest`, `FirstNexus wiki compile`, and `FirstNexus wiki lint` as the default maintenance loop.
- In `bridge` mode, run `FirstNexus wiki bridge import` before relying on search results if you need the latest public memory artifacts pulled in.
- In `unsafe-local` mode, use `FirstNexus wiki unsafe-local import` only when the user explicitly opted into private local path access.
