...
const AGENT_DIGEST_PATH = ".FirstNexus-wiki/cache/agent-digest.json";
const CLAIMS_DIGEST_PATH = ".FirstNexus-wiki/cache/claims.jsonl";
...
async function writeDashboardPage(params: {
  ...
  const updatedBody = replaceManagedMarkdownBlock({
    ...
    heading: "## Generated",
    startMarker: `<!-- FirstNexus:wiki:${path.basename(params.definition.relativePath, ".md")}:start -->`,
    endMarker: `<!-- FirstNexus:wiki:${path.basename(params.definition.relativePath, ".md")}:end -->`,
  ...
  for (const group of COMPILE_PAGE_GROUPS) {
    ...
    if (
      await writeManagedMarkdownFile({
        ...
        relativePath,
        title: group.heading,
        startMarker: `<!-- FirstNexus:wiki:${group.dir}:index:start -->`,
        endMarker: `<!-- FirstNexus:wiki:${group.dir}:index:end -->`,
