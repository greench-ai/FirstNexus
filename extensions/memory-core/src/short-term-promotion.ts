...
import { appendMemoryHostEvent } from "FirstNexus/plugin-sdk/memory-host-events";
...
const PROMOTION_MARKER_PREFIX = "FirstNexus-memory-promotion:";
...
function isContaminatedDreamingSnippet(raw: string): boolean {
  ...
  if (
    /<!--\s*FirstNexus-memory-promotion:/i.test(snippet) ||
...
function extractPromotionMarkers(memoryText: string): Set<string> {
  ...
  const matches = memoryText.matchAll(/<!--\s*FirstNexus-memory-promotion:([^\n]+?)\s*-->/gi);
