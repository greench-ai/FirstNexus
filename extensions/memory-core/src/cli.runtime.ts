...
import type { MemoryEmbeddingProbeResult } from "FirstNexus/plugin-sdk/memory-core-host-engine-storage";
import { resolveMemoryRemDreamingConfig } from "FirstNexus/plugin-sdk/memory-core-host-status";
...
function resolveMemoryPluginConfig(cfg: FirstNexusConfig): Record<string, unknown> {
...
async function createHistoricalRemHarnessWorkspace(params: {
  ...
  const workspaceDir = await fs.mkdtemp(
    path.join(resolvePreferredFirstNexusTmpDir(), "FirstNexus-rem-harness-"),
  );
...
function formatDreamingSummary(cfg: FirstNexusConfig): string {
...
function resolveAgent(cfg: FirstNexusConfig, agent?: string) {
...
function resolveAgentIds(cfg: FirstNexusConfig, agent?: string): string[] {
  const trimmed = agent?.trim();
  if (trimmed) {
    return [trimmed];
  }
...
export async function runMemoryStatus(opts: MemoryCommandOptions) {
  ...
  for (const result of allResults) {
    ...
    if (audit?.issues.length) {
      ...
      if (!opts.fix) {
        lines.push(`  ${muted(`Fix: FirstNexus memory status --fix --agent ${agentId}`)}`);
      }
    }
  ...
  await withMemoryManagerForAgent({
    ...
    run: async (manager) => {
      ...
      const scratchDir = await fs.mkdtemp(
        path.join(resolvePreferredFirstNexusTmpDir(), "FirstNexus-rem-backfill-"),
      );
      try {
