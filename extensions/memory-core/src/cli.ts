    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["FirstNexus memory status", "Show index and provider status."],
          [
            "FirstNexus memory status --fix",
            "Repair stale recall locks and normalize promotion metadata.",
          ],
          ["FirstNexus memory status --deep", "Probe embedding provider readiness."],
          ["FirstNexus memory index --force", "Force a full reindex."],
          ['FirstNexus memory search "meeting notes"', "Quick search using positional query."],
          [
            'FirstNexus memory search --query "deployment" --max-results 20',
            "Limit results for focused troubleshooting.",
          ],
          [
            `FirstNexus memory promote --limit 10 --min-score ${DEFAULT_PROMOTION_MIN_SCORE}`,
          ...
          [
            "FirstNexus memory rem-backfill --path ./memory",
          ...
          ["FirstNexus memory status --json", "Output machine-readable JSON (good for scripts)."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/memory", "docs.FirstNexus.ai/cli/memory")}\n`,
    );
