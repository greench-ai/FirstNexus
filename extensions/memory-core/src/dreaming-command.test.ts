...
function resolveStoredDreaming(config: FirstNexusConfig): Record<string, unknown> {
  const entry = asRecord(config.plugins?.entries?.["memory-core"]);
  const pluginConfig = asRecord(entry?.config);
  return asRecord(pluginConfig?.dreaming) ?? {};
}

function createHarness(initialConfig: FirstNexusConfig = {}) {
  const registered: { command?: FirstNexusPluginCommandDefinition } = {};
  let runtimeConfig: FirstNexusConfig = initialConfig;
  ...
  const runtime = {
    config: {
      ...
      writeConfigFile: vi.fn(async (nextConfig: FirstNexusConfig) => {
        runtimeConfig = nextConfig;
      }),
    },
  ...
  const api = {
    runtime,
    registerCommand: vi.fn((definition: FirstNexusPluginCommandDefinition) => {
