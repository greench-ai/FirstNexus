...
import { resolveGlobalMap } from "FirstNexus/plugin-sdk/global-singleton";
import * as memoryCoreHostRuntimeCoreModule from "FirstNexus/plugin-sdk/memory-core-host-runtime-core";
import * as runtimeConfigSnapshotModule from "FirstNexus/plugin-sdk/runtime-config-snapshot";
import * as sessionStoreRuntimeModule from "FirstNexus/plugin-sdk/session-store-runtime";
...
const DREAMS_FILE_LOCKS_KEY = Symbol.for("FirstNexus.memoryCore.dreamingNarrative.fileLocks");
...
describe("backfill diary entries", () => {
  ...
  it("builds a marked backfill diary entry", () => {
    ...
    expect(entry).toContain("FirstNexus:dreaming:backfill-entry");
    expect(entry).toContain("What Happened");
  });

  it("writes and replaces backfill diary entries", async () => {
    const workspaceDir = await createTempWorkspace("FirstNexus-dreaming-backfill-");
    ...
    expect(content.match(/FirstNexus:dreaming:backfill-entry/g)?.length).toBe(1);
  });
...
describe("appendNarrativeEntry", () => {
  it("creates DREAMS.md with diary header on fresh workspace", async () => {
    const workspaceDir = await createTempWorkspace("FirstNexus-dreaming-narrative-");
    ...
    expect(content).toContain("<!-- FirstNexus:dreaming:diary:start -->");
    expect(content).toContain("<!-- FirstNexus:dreaming:diary:end -->");
  });
  ...
  it("appends a second entry within the diary markers", async () => {
    ...
    const start = content.indexOf("<!-- FirstNexus:dreaming:diary:start -->");
    const end = content.indexOf("<!-- FirstNexus:dreaming:diary:end -->");
  ...
  it("dedupes only exact diary duplicates while keeping distinct timestamps", async () => {
    const workspaceDir = await createTempWorkspace("FirstNexus-dreaming-dedupe-");
    const dreamsPath = path.join(workspaceDir, "DREAMS.md");
    await fs.writeFile(
      dreamsPath,
      [
...
describe("generateAndAppendDreamNarrative", () => {
  ...
  it("scrubs stale dreaming entries and orphan transcripts after cleanup", async () => {
    const workspaceDir = await createTempWorkspace("FirstNexus-dreaming-narrative-");
    const stateDir = await createTempWorkspace("FirstNexus-dreaming-state-");
  ...
  it("isolates narrative sessions across workspaces even at the same timestamp", async () => {
    const firstWorkspaceDir = await createTempWorkspace("FirstNexus-dreaming-narrative-");
    const secondWorkspaceDir = await createTempWorkspace("FirstNexus-dreaming-narrative-");
...
describe("runDetachedDreamNarrative", () => {
  ...
  it("caps the number of in-flight detached narratives at 3", async () => {
    ...
    const workspaceDir = await createTempWorkspace("FirstNexus-dreaming-detach-");
    const logger = createMockLogger();
