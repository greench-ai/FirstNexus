...
import { peekSystemEventEntries } from "FirstNexus/plugin-sdk/system-event-runtime";
...
export function registerShortTermPromotionDreaming(api: FirstNexusPluginApi): void {
  ...
  const resolveCurrentConfig = (): FirstNexusConfig =>
    (api.runtime.config?.current?.() ?? api.config) as FirstNexusConfig;
  ...
  const reconcileManagedDreamingCron = async (params: {
    ...
    startupConfig?: FirstNexusConfig;
