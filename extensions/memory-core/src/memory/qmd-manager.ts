...
import { withFileLock } from "FirstNexus/plugin-sdk/file-lock";
import {
...
const MCPORTER_STATE_KEY = Symbol.for("FirstNexus.mcporterState");
const QMD_EMBED_QUEUE_KEY = Symbol.for("FirstNexus.qmdEmbedQueueTail");
const QMD_UPDATE_QUEUE_KEY = Symbol.for("FirstNexus.qmdUpdateQueueState");
...
function normalizeHanBm25Query(query: string): string {
  ...
  // Keep Han/CJK BM25 queries intact so FirstNexus search semantics match direct qmd search.
  return trimmed;
}
