...
describe("memory index", () => {
  ...
  beforeEach(async () => {
    ...
    vi.stubEnv("NEXISCLAW_TEST_MEMORY_UNSAFE_REINDEX", "1");
    clearRegistry();
    registerBuiltInMemoryEmbeddingProviders({ registerMemoryEmbeddingProvider: registerAdapter });
    embedBatchCalls = 0;
  ...
  async function getFtsSessionManager(params: {
    ...
    vi.stubEnv("NEXISCLAW_STATE_DIR", path.join(workspaceDir, params.stateDirName));
    const cfg = createCfg({
  ...
  it("streams embedding cache rows during safe reindex", async () => {
    vi.stubEnv("NEXISCLAW_TEST_MEMORY_UNSAFE_REINDEX", "0");
