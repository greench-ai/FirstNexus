...
import type { checkQmdBinaryAvailability as checkQmdBinaryAvailabilityFn } from "FirstNexus/plugin-sdk/memory-core-host-engine-qmd";
...
vi.mock("FirstNexus/plugin-sdk/memory-core-host-engine-qmd", () => ({
...
function createBuiltinCfg(agentId: string): FirstNexusConfig {
  return {
    agents: {
      defaults: {
...
async function expectPendingQmdReplacement(params: {
  agentId: string;
  firstCfg: FirstNexusConfig;
  secondCfg: FirstNexusConfig;
...
describe("getMemorySearchManager caching", () => {
  it("repairs an invalid shared singleton cache shape before using qmd cache maps", async () => {
    await closeAllMemorySearchManagers();
    vi.resetModules();
    const cacheKey = Symbol.for("FirstNexus.memorySearchManagerCache");
  ...
  it("creates a missing agent workspace before probing qmd availability", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "FirstNexus-qmd-workspace-"));
