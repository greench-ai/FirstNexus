...
function readLegacyGrokFallbackAuth(cfg?: FirstNexusConfig): XaiFallbackAuth | undefined {
...
function readLegacyGrokApiKeyResult(cfg?: FirstNexusConfig): ConfiguredRuntimeApiKeyResolution {
  const search = cfg?.tools?.web?.search;
  if (!search || typeof search !== "object") {
    return { status: "missing" };
  }
...
export function resolveFallbackXaiAuth(cfg?: FirstNexusConfig): XaiFallbackAuth | undefined {
...
export function resolveFallbackXaiApiKey(cfg?: FirstNexusConfig): string | undefined {
