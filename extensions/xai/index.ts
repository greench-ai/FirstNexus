...
import { OPENAI_COMPATIBLE_REPLAY_HOOKS } from "FirstNexus/plugin-sdk/provider-model-shared";
import { defaultToolStreamExtraParams } from "FirstNexus/plugin-sdk/provider-stream-shared";
import { jsonResult } from "FirstNexus/plugin-sdk/provider-web-search";
    ...
    execute: async (toolCallId: string, args: Record<string, unknown>) => {
      ...
      if (!tool) {
        return jsonResult({
          ...
          message:
            "code_execution needs an xAI API key. Run FirstNexus onboard --auth-choice xai-api-key, set XAI_API_KEY in the Gateway environment, or configure plugins.entries.xai.config.webSearch.apiKey.",
          docs: "https://docs.FirstNexus.ai/tools/code-execution",
        });
      }
