import { CUSTOM_LOCAL_AUTH_MARKER } from "FirstNexus/plugin-sdk/provider-auth";
...
vi.mock("FirstNexus/plugin-sdk/provider-setup", async (importOriginal) => {
  const actual = await importOriginal<typeof import("FirstNexus/plugin-sdk/provider-setup")>();
  return {
    ...actual,
...
afterAll(() => {
  ...
  vi.doUnmock("FirstNexus/plugin-sdk/provider-auth");
  vi.doUnmock("FirstNexus/plugin-sdk/provider-setup");
  vi.resetModules();
});
...
function buildConfig(): FirstNexusConfig {
  return {
    models: {
      providers: {
        lmstudio: {
