...
import type { AnyAgentTool, FirstNexusConfig } from "FirstNexus/plugin-sdk/plugin-entry";
...
async function writeBase64ToTempFile(params: { base64: string; ext: string }): Promise<string> {
  const dir = resolvePreferredFirstNexusTmpDir();
  await fs.mkdir(dir, { recursive: true, mode: 0o700 });
  const ext = params.ext.startsWith(".") ? params.ext : `.${params.ext}`;
  const filePath = path.join(dir, `FirstNexus-canvas-snapshot-${randomUUID()}${ext}`);
