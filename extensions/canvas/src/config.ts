...
export function isCanvasPluginEnabled(config?: FirstNexusConfig): boolean {
  if (!config) {
    return true;
  }
...
export function isCanvasHostEnabled(config?: FirstNexusConfig): boolean {
...
export const canvasConfigSchema: CanvasPluginConfigSchema = {
  ...
  uiHints: {
    ...
    "host.root": {
      ...
      help: "Directory to serve. Defaults to the FirstNexus state canvas directory.",
      advanced: true,
    },
    "host.port": {
