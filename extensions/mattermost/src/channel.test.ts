...
vi.mock("FirstNexus/plugin-sdk/ssrf-runtime", async () => {
  const original = (await vi.importActual("FirstNexus/plugin-sdk/ssrf-runtime")) as Record<
    string,
    unknown
  >;
...
function getDescribedActions(cfg: FirstNexusConfig, accountId?: string): string[] {
