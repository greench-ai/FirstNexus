...
const SIGNED_CHANNEL_ID_CONTEXT_KEY = "__FirstNexus_channel_id";
...
type InteractionCallbackConfig = Pick<FirstNexusConfig, "gateway" | "channels"> & {
  interactions?: {
...
function deriveInteractionSecret(botToken: string): string {
  return createHmac("sha256", "FirstNexus-mattermost-interactions").update(botToken).digest("hex");
}
