...
import type { AnyAgentTool } from "FirstNexus/plugin-sdk/agent-runtime";
import type { PluginTrustedToolPolicyRegistration } from "FirstNexus/plugin-sdk/core";
...
async function makeTempDir(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "FirstNexus-skill-workshop-test-"));
  tempDirs.push(dir);
  return dir;
}
