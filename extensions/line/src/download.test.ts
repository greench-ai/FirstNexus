...
vi.mock("FirstNexus/plugin-sdk/media-store", () => ({
...
describe("downloadLineMedia", () => {
  ...
  afterAll(() => {
    ...
    vi.doUnmock("FirstNexus/plugin-sdk/media-store");
    vi.resetModules();
  });
  ...
  beforeEach(() => {
    ...
    saveMediaBufferMock.mockImplementation(
      async (_buffer: Buffer, contentType?: string, subdir?: string) => ({
        path: `/home/user/.FirstNexus/media/${subdir ?? "unknown"}/saved-media`,
        contentType,
      }),
    );
  });
  ...
  it("persists inbound media with the shared media store", async () => {
    ...
    expect(result).toEqual({
      path: "/home/user/.FirstNexus/media/inbound/saved-media",
