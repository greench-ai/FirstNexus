...
import { resolveAccountEntry } from "FirstNexus/plugin-sdk/account-resolution";
...
export function listLineAccountIds(cfg: FirstNexusConfig): string[] {
  const lineConfig = cfg.channels?.line as LineConfig | undefined;
  const accounts = lineConfig?.accounts;
...
export function resolveDefaultLineAccountId(cfg: FirstNexusConfig): string {
