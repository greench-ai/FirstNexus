...
} from "FirstNexus/plugin-sdk/plugin-config-runtime";
import { definePluginEntry, type FirstNexusPluginApi } from "FirstNexus/plugin-sdk/plugin-entry";
import { parseAgentSessionKey, parseThreadSessionSuffix } from "FirstNexus/plugin-sdk/routing";
import { isPathInside, replaceFileAtomic } from "FirstNexus/plugin-sdk/security-runtime";
import {
...
function resolveDefaultToolsAllow(cfg: FirstNexusConfig | undefined): string[] {
...
function resolvePersistentTranscriptBaseDir(api: FirstNexusPluginApi, agentId: string): string {
  return path.join(
...
function resolveToggleStatePath(api: FirstNexusPluginApi): string {
  return path.join(
    api.runtime.state.resolveStateDir(),
    "plugins",
    "active-memory",
...
function isActiveMemoryGloballyEnabled(cfg: FirstNexusConfig): boolean {
...
async function runRecallSubagent(params: {
  ...
  const transientWorkspace = params.config.persistTranscripts
    ...
    : await tempWorkspace({
        ...
        prefix: "FirstNexus-active-memory-",
      });
...
export default definePluginEntry({
  ...
  register(api: FirstNexusPluginApi) {
    const readCurrentConfig = (): FirstNexusConfig | undefined => {
      try {
        return (
          (api.runtime.config?.current?.() as FirstNexusConfig | undefined) ??
          (api.config as FirstNexusConfig | undefined)
        );
      } catch {
        return api.config as FirstNexusConfig | undefined;
      }
    };
    ...
    api.registerCommand({
      ...
      handler: async (ctx) => {
        ...
        if (isGlobal) {
          const currentConfig = api.runtime.config.current() as FirstNexusConfig;
          if (action === "status") {
            return {
