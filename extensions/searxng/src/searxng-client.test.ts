import type { LookupFn } from "FirstNexus/plugin-sdk/ssrf-runtime";
...
vi.mock("FirstNexus/plugin-sdk/provider-web-search", async (importOriginal) => {
  const actual = await importOriginal<typeof import("FirstNexus/plugin-sdk/provider-web-search")>();
...
describe("searxng client", () => {
  ...
  it("preserves a configured base-path prefix when building the search URL", () => {
    expect(
      ...
      "https://search.example.com/searxng/search?q=FirstNexus&format=json&categories=general%2Cnews&language=en",
    );
  });
