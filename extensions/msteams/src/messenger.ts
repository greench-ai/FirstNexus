import {
  ...
  SILENT_REPLY_TOKEN,
  type ChunkMode,
} from "FirstNexus/plugin-sdk/reply-chunking";
import {
...
import type { MarkdownTableMode, MSTeamsReplyStyle, FirstNexusConfig } from "../runtime-api.js";
...
export function renderReplyPayloadsToMessages(
  ...
  const tableMode =
    ...
    getMSTeamsRuntime().channel.text.resolveMarkdownTableMode({
      cfg: getMSTeamsRuntime().config.current() as FirstNexusConfig,
      channel: "msteams",
    });
