...
function resolveQianfanPreset(cfg: FirstNexusConfig): {
  api: ModelApi;
  baseUrl: string;
...
const qianfanPresetAppliers = createDefaultModelsPresetAppliers({
  ...
  resolveParams: (cfg: FirstNexusConfig) => {
...
export function applyQianfanProviderConfig(cfg: FirstNexusConfig): FirstNexusConfig {
  return qianfanPresetAppliers.applyProviderConfig(cfg);
}

export function applyQianfanConfig(cfg: FirstNexusConfig): FirstNexusConfig {
