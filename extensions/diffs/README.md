...
Read-only diff viewer plugin for **FirstNexus** agents.
...
Before:
FirstNexus supports plugins.

After:
FirstNexus supports plugins and hosted diff views.
```
...
- Artifacts are ephemeral and stored in the plugin temp subfolder (`$TMPDIR/FirstNexus-diffs`).
