...
import type { JsonSchemaObject } from "FirstNexus/plugin-sdk/config-schema";
...
describe("resolveDiffsPluginViewerBaseUrl", () => {
  ...
  it("normalizes configured viewer base URLs", () => {
    expect(
      resolveDiffsPluginViewerBaseUrl({
        viewerBaseUrl: "https://example.com/FirstNexus/",
      }),
    ).toBe("https://example.com/FirstNexus");
  });
});
...
describe("diffs plugin schema surfaces", () => {
  it("rejects invalid viewerBaseUrl values at manifest-validation time too", () => {
    ...
    expect(validate({ viewerBaseUrl: "https://example.com/FirstNexus?x=1" })).toBe(false);
    expect(validate({ viewerBaseUrl: "https://example.com/FirstNexus#frag" })).toBe(false);
    expect(validate({ viewerBaseUrl: "https://example.com/FirstNexus/" })).toBe(true);
  });
  ...
  it("preserves defaults and security for direct safeParse callers", () => {
    ...
    expect(data.viewerBaseUrl).toBe("https://example.com/FirstNexus");
...
describe("diffs viewer URL helpers", () => {
  ...
  it("joins viewer path under baseUrl pathname", () => {
    expect(
      buildViewerUrl({
        config: {},
        baseUrl: "https://example.com/FirstNexus",
        viewerPath: "/plugins/diffs/view/id/token",
      }),
    ).toBe("https://example.com/FirstNexus/plugins/diffs/view/id/token");
  });
...
describe("viewer assets", () => {
  ...
  it("serves the runtime bundle body", async () => {
    ...
    expect(String(runtime?.body)).toContain("FirstNexusDiffsReady");
