...
describe("DiffArtifactStore", () => {
  ...
  beforeEach(async () => {
    ...
    } = await createDiffStoreHarness("FirstNexus-diffs-store-"));
  });
...
describe("createDiffsHttpHandler", () => {
  ...
  beforeEach(async () => {
    ({ store, cleanup: cleanupRootDir } = await createDiffStoreHarness("FirstNexus-diffs-http-"));
  });
  ...
  it("serves the shared viewer runtime asset", async () => {
    ...
    expect(handled).toBe(true);
    expect(res.statusCode).toBe(200);
    expect(String(res.body)).toContain("FirstNexusDiffsReady");
  });
