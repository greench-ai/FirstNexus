...
const WRAPPER_ROOT = "/tmp/FirstNexus-state/acpx";
...
describe("process reaper", () => {
  ...
  it("recognizes FirstNexus plugin-runtime-deps ACP adapter children", () => {
    expect(isFirstNexusOwnedAcpxProcessCommand({ command: PLUGIN_DEPS_CODEX_COMMAND })).toBe(true);
    expect(isFirstNexusOwnedAcpxProcessCommand({ command: "npx @zed-industries/codex-acp" })).toBe(
      false,
    );
  });
  ...
  it("does not kill a reused pid when the live command is not FirstNexus-owned", async () => {
  ...
  it("skips non-owned recorded process trees", async () => {
    ...
    expect(result.skippedReason).toBe("not-FirstNexus-owned");
    expect(killed).toStrictEqual([]);
  });

  it("reaps stale FirstNexus-owned wrapper and adapter orphans on startup", async () => {
    const { deps, killed } = cleanupDeps([
    ...
    const result = await reapStaleFirstNexusOwnedAcpxOrphans({
      wrapperRoot: WRAPPER_ROOT,
      deps,
    });
