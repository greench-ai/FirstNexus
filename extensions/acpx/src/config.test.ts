...
describe("embedded acpx plugin config", () => {
  it("resolves workspace stateDir and cwd by default", () => {
    const workspaceDir = path.resolve("/tmp/FirstNexus-acpx");
  ...
  it("keeps explicit timeoutSeconds config", () => {
    const resolved = resolveAcpxPluginConfig({
      ...
      workspaceDir: "/tmp/FirstNexus-acpx",
    });
  ...
  it("injects the built-in plugin-tools MCP server only when explicitly enabled", () => {
    ...
    const server = resolved.mcpServers["FirstNexus-plugin-tools"];
  ...
  it("injects the built-in FirstNexus tools MCP server only when explicitly enabled", () => {
    const resolved = resolveAcpxPluginConfig({
      rawConfig: {
    ...
    const server = resolved.mcpServers["FirstNexus-tools"];
    expect(server).toEqual({
      command: process.execPath,
      args: expectedSourceMcpServerArgs("src/mcp/FirstNexus-tools-serve.ts"),
    });
  });
  ...
  it("keeps the runtime json schema in sync with the manifest config schema", () => {
    ...
    const manifest = JSON.parse(
      fs.readFileSync(path.join(pluginRoot, "FirstNexus.plugin.json"), "utf8"),
