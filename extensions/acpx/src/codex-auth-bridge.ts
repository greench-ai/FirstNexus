...
import { readJsonFileWithFallback } from "FirstNexus/plugin-sdk/json-store";
import {
...
const RUN_CONFIGURED_COMMAND_SENTINEL = "--FirstNexus-run-configured";
...
function readManifestDependencyVersion(packageName: string): string {
  ...
  if (typeof version !== "string" || version.trim() === "") {
    throw new Error(`Missing ${packageName} dependency version in @FirstNexus/acpx manifest`);
  }
  return version;
}
...
async function resolveInstalledCodexAcpBinPath(): Promise<string | undefined> {
  // Keep FirstNexus's isolated CODEX_HOME wrapper, but launch the plugin-local
...
function stripFirstNexusWrapperArgs(args) {
  const stripped = [];
...
const configuredArgs = stripFirstNexusWrapperArgs(process.argv.slice(2));
...
if (!command) {
  console.error("[FirstNexus] missing configured ${params.displayName} ACP command");
  process.exit(1);
}
...
child.on("error", (error) => {
  console.error(\`[FirstNexus] failed to launch ${params.displayName} ACP wrapper: \${error.message}\`);
  process.exit(1);
});
...
function buildClaudeAcpWrapperScript(installedBinPath?: string): string {
  return buildAdapterWrapperScript({
    displayName: "Claude",
    // This package is patched in FirstNexus; fallback must not float to an unpatched newer release.
