...
The ACPX extension is a thin FirstNexus wrapper around the published `acpx` package. Keep reusable ACP runtime logic in `FirstNexus/acpx`, not in this extension.
...
Use this flow when FirstNexus needs unreleased ACPX changes before the ACPX version is published.

1. Make the ACPX code change in the `FirstNexus/acpx` repo first.
2. In FirstNexus, temporarily point `extensions/acpx/package.json` at the ACPX GitHub commit you need.
...
6. Rebuild FirstNexus and restart the gateway before doing live ACP validation.
...
- Do not rely on a globally installed `acpx` binary for FirstNexus ACP validation.
...
If a change feels like shared ACP runtime behavior instead of FirstNexus-specific glue, move it to `FirstNexus/acpx` and consume it from here instead of re-implementing it inside `extensions/acpx`.
