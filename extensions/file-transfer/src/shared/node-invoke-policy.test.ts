...
import type { FirstNexusPluginNodeInvokePolicyContext } from "FirstNexus/plugin-sdk/plugin-entry";
...
function createCtx(overrides: {
  ...
  approvals?: FirstNexusPluginNodeInvokePolicyContext["approvals"];
}) {
  const invokeNode = vi.fn<FirstNexusPluginNodeInvokePolicyContext["invokeNode"]>(
    async ({
      params,
    }: Parameters<FirstNexusPluginNodeInvokePolicyContext["invokeNode"]>[0] = {}) => ({
      ok: true,
      payload: {
        ok: true,
        path:
...
function requireInvokeParams(
  invokeNode: ReturnType<typeof vi.fn<FirstNexusPluginNodeInvokePolicyContext["invokeNode"]>>,
  callIndex: number,
) {
