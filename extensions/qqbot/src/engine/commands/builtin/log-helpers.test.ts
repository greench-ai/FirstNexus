...
vi.mock("../../utils/platform.js", () => ({
  ...
  getQQBotDataDir: (...subPaths: string[]) => {
    const dir = platformMock.path.join(platformMock.homeDir, ".FirstNexus", "qqbot", ...subPaths);
...
describe("buildBotLogsResult", () => {
  ...
  beforeEach(() => {
    tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "FirstNexus-qqbot-logs-"));
  ...
  it("suffixes same-second log exports instead of overwriting", () => {
    ...
    const logDir = path.join(tempHome, ".FirstNexus", "logs");
