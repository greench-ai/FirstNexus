...
import { loadJsonFile } from "FirstNexus/plugin-sdk/json-store";
...
function getConfiguredLogFiles(): string[] {
  ...
  const files: string[] = [];
  for (const cli of ["FirstNexus", "clawdbot", "moltbot"]) {
    try {
...
function collectCandidateLogDirs(): string[] {
  ...
  for (const root of searchRoots) {
    try {
      ...
      for (const entry of entries) {
        ...
        if (!/(FirstNexus|clawdbot|moltbot)/i.test(entry.name)) {
          continue;
        }
  ...
  if (!isWindows()) {
    for (const name of ["FirstNexus", "clawdbot", "moltbot"]) {
...
function collectRecentLogFiles(logDirs: string[]): LogCandidate[] {
  ...
  for (const dir of logDirs) {
    ...
    pushFile(path.join(dir, "FirstNexus.log"), dir);
    ...
    try {
      ...
      for (const entry of entries) {
        ...
        if (!/(gateway|FirstNexus|clawdbot|moltbot)/i.test(entry.name)) {
          continue;
        }
