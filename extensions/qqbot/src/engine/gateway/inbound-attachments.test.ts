...
vi.mock("../utils/platform.js", () => ({
  getQQBotMediaDir: () => "/tmp/FirstNexus-qqbot-downloads",
}));
...
describe("engine/gateway/inbound-attachments", () => {
  ...
  it("uses remote image URL when image download fails", async () => {
    ...
    expect(downloadFileMock).toHaveBeenCalledWith(
      ...
      "/tmp/FirstNexus-qqbot-downloads",
      "a.png",
    );
  ...
  it("prefers voice_wav_url for voice downloads and transcribes with configured STT", async () => {
    downloadFileMock.mockResolvedValue("/tmp/FirstNexus-qqbot-downloads/voice.wav");
    ...
    expect(transcribeAudioMock).toHaveBeenCalledWith("/tmp/FirstNexus-qqbot-downloads/voice.wav", {
      channels: { qqbot: { stt: {} } },
    });
    expect(result.voiceAttachmentPaths).toEqual(["/tmp/FirstNexus-qqbot-downloads/voice.wav"]);
