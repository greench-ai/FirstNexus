...
describe("qqbot local media path remapping", () => {
  ...
  function createFirstNexusTestRoot() {
    const actualHome = getHomeDir();
    const FirstNexusDir = path.join(actualHome, ".FirstNexus");
    fs.mkdirSync(FirstNexusDir, { recursive: true });
    const testRoot = fs.mkdtempSync(path.join(FirstNexusDir, "qqbot-platform-test-"));
  ...
  function createQqbotMediaFile(fileName: string) {
    const { actualHome, testRootName } = createFirstNexusTestRoot();
  ...
  it("allows structured payload files inside sibling FirstNexus media subdirectories", () => {
    ...
    // trust the shared `~/.FirstNexus/media` root so auto-routed sends can access
    // those files without the path-outside-storage guard firing.
    const actualHome = getHomeDir();
    const outboundDir = path.join(actualHome, ".FirstNexus", "media", "outbound");
