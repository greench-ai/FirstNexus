import { getExecApprovalReplyMetadata } from "FirstNexus/plugin-sdk/approval-runtime";
import {
...
import { toGatewayAccount, writeFirstNexusConfigThroughRuntime } from "./bridge/narrowing.js";
...
export const qqbotPlugin: ChannelPlugin<ResolvedQQBotAccount> = {
  ...
  config: {
     ...
     * that wiped FirstNexus.json mid-flight.
     */
  ...
  gateway: {
    startAccount: async (ctx) => {
      ...
      // FirstNexus.json). We only restore when both fields are empty so a
      ...
      if (!account.appId || !account.clientSecret) {
        ...
        if (backup?.appId && backup?.clientSecret) {
          try {
            ...
            await writeFirstNexusConfigThroughRuntime(getQQBotRuntime(), nextCfg);
            cfg = nextCfg;
      ...
      await startGateway({
        ...
        onReady: () => {
          ...
          // upgrade that might wipe FirstNexus.json mid-flight.
    ...
    logoutAccount: async ({ accountId, cfg }) => {
      ...
      if (changed) {
        await writeFirstNexusConfigThroughRuntime(getQQBotRuntime(), nextCfg as FirstNexusConfig);
      }

      const resolved = resolveQQBotAccount(
        (changed ? nextCfg : cfg) as FirstNexusConfig,
        accountId,
      );
