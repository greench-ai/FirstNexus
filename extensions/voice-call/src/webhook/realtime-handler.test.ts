...
describe("RealtimeCallHandler path routing", () => {
  ...
  it("submits continuing responses only for realtime agent consult calls", async () => {
    ...
    handler.registerToolHandler("FirstNexus_agent_consult", (_args, _callId, context) => {
    ...
    try {
      ...
      try {
        ...
        await waitForRealtimeTest(() => {
          ...
          expect(payload?.tool).toBe("FirstNexus_agent_consult");
  ...
  it("forces an agent consult from final user transcript when consult policy is always", async () => {
    ...
    handler.registerToolHandler("FirstNexus_agent_consult", consult);
    const server = await startRealtimeServer(handler);
    ...
    try {
      ...
      try {
        ...
        expect(args).toEqual({
          ...
          context:
            "The realtime provider produced a final user transcript without invoking FirstNexus_agent_consult, so FirstNexus is forcing the consult because consultPolicy is always.",
        });
  ...
  it("does not submit an interim checking result when fast context is enabled", async () => {
    ...
    handler.registerToolHandler("FirstNexus_agent_consult", async () => ({
      text: "Fast context.",
    }));
    const server = await startRealtimeServer(handler);
