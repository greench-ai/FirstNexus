...
async function createWorkspace(): Promise<string> {
  const workspaceDir = await mkdtemp(path.join(tmpdir(), "FirstNexus-voice-context-"));
...
describe("buildRealtimeVoiceInstructions", () => {
  it("injects bounded identity, system prompt, and workspace context", async () => {
    ...
    expect(instructions).toContain("FirstNexus agent voice context:");
    expect(instructions).toContain("Consult behavior:");
    expect(instructions).toContain("Call FirstNexus_agent_consult before answering requests");
