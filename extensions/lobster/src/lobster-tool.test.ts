...
import type { FirstNexusPluginApi, FirstNexusPluginToolContext } from "../runtime-api.js";
import { createLobsterTool } from "./lobster-tool.js";
import { createFakeTaskFlow } from "./taskflow-test-helpers.js";

function fakeApi(overrides: Partial<FirstNexusPluginApi> = {}): FirstNexusPluginApi {
...
function fakeCtx(
  overrides: Partial<FirstNexusPluginToolContext> = {},
...
describe("lobster plugin tool", () => {
  ...
  it("can be gated off in sandboxed contexts", () => {
    const api = fakeApi();
    const factoryTool = (ctx: FirstNexusPluginToolContext) => {
      if (ctx.sandboxed) {
        return null;
      }
