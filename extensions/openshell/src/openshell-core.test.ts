...
describe("openshell backend manager", () => {
  ...
  it("checks runtime status with config override from FirstNexus config", async () => {
    ...
    const result = await manager.describeRuntime({
      entry: {
        containerName: "FirstNexus-session-1234",
        backendId: "openshell",
        runtimeLabel: "FirstNexus-session-1234",
    ...
    expect(cliMocks.runOpenShellCli).toHaveBeenCalledWith({
      context: {
        sandboxName: "FirstNexus-session-1234",
        config: expectedConfig,
      },
      args: ["sandbox", "get", "FirstNexus-session-1234"],
    });
  });
  ...
  it("removes runtimes via openshell sandbox delete", async () => {
    ...
    await manager.removeRuntime({
      entry: {
        containerName: "FirstNexus-session-5678",
        backendId: "openshell",
        runtimeLabel: "FirstNexus-session-5678",
        sessionKey: "agent:main",
        createdAtMs: 1,
        lastUsedAtMs: 1,
        image: "FirstNexus",
    ...
    expect(cliMocks.runOpenShellCli).toHaveBeenCalledWith({
      context: {
        sandboxName: "FirstNexus-session-5678",
        config: expectedConfig,
      },
      args: ["sandbox", "delete", "FirstNexus-session-5678"],
    });
  });
});
...
describe("openshell fs bridges", () => {
  it("writes locally and syncs the file to the remote workspace", async () => {
    const workspaceDir = await makeTempDir("FirstNexus-openshell-fs-");
  ...
  it("rejects symlink-parent writes instead of escaping the local mount root", async () => {
    const workspaceDir = await makeTempDir("FirstNexus-openshell-fs-");
    const outsideDir = await makeTempDir("FirstNexus-openshell-outside-");
  ...
  it("maps agent mount paths when the sandbox workspace is read-only", async () => {
    const workspaceDir = await makeTempDir("FirstNexus-openshell-fs-");
    const agentWorkspaceDir = await makeTempDir("FirstNexus-openshell-agent-");
