...
describe("handleSlackAction", () => {
  function slackConfig(overrides?: Record<string, unknown>): FirstNexusConfig {
    return {
      channels: {
        slack: {
          botToken: "tok",
  ...
  function createReplyToFirstScenario() {
    const cfg = { channels: { slack: { botToken: "tok" } } } as FirstNexusConfig;
  ...
  function expectLastSlackSend(content: string, cfg: FirstNexusConfig, threadTs?: string) {
  ...
  async function resolveReadToken(cfg: FirstNexusConfig): Promise<string | undefined> {
  ...
  async function resolveSendToken(cfg: FirstNexusConfig): Promise<string | undefined> {
    sendSlackMessage.mockClear();
  ...
  it("returns non-image downloadFile results as file metadata instead of image content", async () => {
    downloadSlackFile.mockResolvedValueOnce({
      path: "/tmp/FirstNexus-media/report.pdf",
    ...
    expect(String(firstContent.text)).toContain("/tmp/FirstNexus-media/report.pdf");
    ...
    expect(details.media).toEqual({
      mediaUrl: "/tmp/FirstNexus-media/report.pdf",
      contentType: "application/pdf",
    });
  });
