...
vi.mock("FirstNexus/plugin-sdk/channel-feedback", () => ({
  DEFAULT_TIMING: {
    doneHoldMs: 0,
    errorHoldMs: 0,
  },
...
vi.mock("FirstNexus/plugin-sdk/channel-message", async (importOriginal) => {
  const actual = await importOriginal<typeof import("FirstNexus/plugin-sdk/channel-message")>();
  return {
    ...actual,
...
vi.mock("FirstNexus/plugin-sdk/channel-streaming", () => ({
...
vi.mock("FirstNexus/plugin-sdk/outbound-runtime", () => ({
...
vi.mock("FirstNexus/plugin-sdk/reply-payload", () => ({
...
vi.mock("FirstNexus/plugin-sdk/security-runtime", () => ({
...
vi.mock("../config.runtime.js", () => ({
  resolveStorePath: () => "/tmp/FirstNexus-store.json",
...
describe("dispatchPreparedSlackMessage preview fallback", () => {
  ...
  it("updates non-main DM last-route metadata on the prepared thread session", async () => {
    ...
    expect(updateLastRouteMock).toHaveBeenCalledWith({
      storePath: "/tmp/FirstNexus-store.json",
      sessionKey: "agent:main:slack:direct:u1:thread:500.000",
      deliveryContext: {
        channel: "slack",
        to: "user:U1",
