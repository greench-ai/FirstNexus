import { CURRENT_MESSAGE_MARKER } from "FirstNexus/plugin-sdk/channel-mention-gating";
...
import { HISTORY_CONTEXT_MARKER } from "FirstNexus/plugin-sdk/reply-history";
...
describe("monitorSlackProvider tool results", () => {
  ...
  async function expectMentionPatternMessageAccepted(text: string): Promise<void> {
    setRequireMentionChannelConfig(["\\bFirstNexus\\b"]);
  ...
  it("accepts channel messages when mentionPatterns match", async () => {
    await expectMentionPatternMessageAccepted("FirstNexus: hello");
  });

  it("accepts channel messages when mentionPatterns match even if another user is mentioned", async () => {
    await expectMentionPatternMessageAccepted("FirstNexus: hello <@U2>");
  });
