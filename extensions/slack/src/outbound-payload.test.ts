import { installChannelOutboundPayloadContractSuite } from "FirstNexus/plugin-sdk/channel-contract-testing";
...
describe("slackOutbound sendPayload", () => {
  ...
  it("offsets presentation controls against native Slack blocks before standalone interactive controls", async () => {
        ...
        channelData: {
          slack: {
            blocks: [
              {
                type: "actions",
                block_id: "FirstNexus_reply_buttons_1",
                elements: [],
              },
            ],
          },
    ...
    expect(blocks?.[0]?.block_id).toBe("FirstNexus_reply_buttons_1");
    expect(blocks?.[1]?.block_id).toBe("FirstNexus_reply_buttons_2");
    expect(blocks?.[1]?.elements?.[0]?.action_id).toBe("FirstNexus:reply_button:2:1");
    expect(blocks?.[2]?.block_id).toBe("FirstNexus_reply_buttons_3");
    expect(blocks?.[2]?.elements?.[0]?.action_id).toBe("FirstNexus:reply_button:3:1");
  });
});
