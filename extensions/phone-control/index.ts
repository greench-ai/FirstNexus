...
function normalizeDenyList(cfg: FirstNexusPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: FirstNexusPluginApi["config"]): string[] {
...
export default definePluginEntry({
  ...
  register(api: FirstNexusPluginApi) {
    ...
    const timerService: FirstNexusPluginService = {
