...
import { readRequestBodyWithLimit } from "FirstNexus/plugin-sdk/webhook-ingress";
...
function buildAssistantText(
  ...
  if (/tool continuity check/i.test(prompt) && toolOutput) {
    return `Protocol note: model switch handoff confirmed on ${model || "the requested model"}. QA mission from QA_KICKOFF_TASK.md still applies: understand this FirstNexus repo from source + docs before acting.`;
  }
  ...
  if (QA_TOOL_SEARCH_PROMPT_RE.test(allInputText) && !toolOutput) {
    ...
    if (targetTool && hasDeclaredTool(body, "tool_search_code")) {
      return buildToolCallEventsWithArgs("tool_search_code", {
        code: [
          `const hits = await FirstNexus.tools.search(${JSON.stringify(targetTool)}, { limit: 1 });`,
          "const match = hits.find((tool) => tool.name === " + JSON.stringify(targetTool) + ");",
          "if (!match) throw new Error('target tool not found');",
          "return await FirstNexus.tools.call(match.id, { marker: 'code-mode' });",
        ].join("\n"),
      });
    }
