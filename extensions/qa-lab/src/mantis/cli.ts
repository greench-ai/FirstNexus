...
export function registerMantisCli(qa: Command) {
  ...
  mantis
    ...
    .option("--gateway-setup", "Start a persistent FirstNexus Slack gateway inside the VNC VM")
  ...
  mantis
    ...
    .description(
      "Lease or reuse a Crabbox VNC desktop, install Telegram Desktop, configure FirstNexus Telegram with a bot token, and capture screenshot/video artifacts",
    )
    .option("--repo-root <path>", "Repository root to target when running from a neutral cwd")
    ...
    .option("--no-gateway-setup", "Install Telegram Desktop only; do not configure FirstNexus")
