...
const pluginApiMocks = vi.hoisted(() => ({
  ...
  resolvePreferredFirstNexusTmpDir: vi.fn(() =>
    path.join(os.tmpdir(), "FirstNexus-device-pair-tests"),
  ),
...
vi.mock("./api.js", () => {
  return {
    ...
    resolvePreferredFirstNexusTmpDir: pluginApiMocks.resolvePreferredFirstNexusTmpDir,
...
function createApi(params?: {
  ...
  return createTestPluginApi({
    ...
    runtime: (params?.runtime ?? {}) as FirstNexusPluginApi["runtime"],
...
function registerPairCommand(params?: {
  ...
  let command: FirstNexusPluginCommandDefinition | undefined;
...
describe("device-pair /pair qr", () => {
  beforeEach(async () => {
    ...
    await fs.mkdir(pluginApiMocks.resolvePreferredFirstNexusTmpDir(), { recursive: true });
  });

  afterEach(async () => {
    await fs.rm(pluginApiMocks.resolvePreferredFirstNexusTmpDir(), {
      recursive: true,
      force: true,
    });
  });
  ...
  it("returns an inline QR image for webchat surfaces", async () => {
    ...
    expect(text).toContain("Scan this QR code with the FirstNexus iOS app:");
    ...
    expect(text).not.toContain("![FirstNexus pairing QR]");
  });
  ...
  it.each([
    ...
    expect(caption).toContain("Scan this QR code with the FirstNexus iOS app:");
...
describe("device-pair /pair default setup code", () => {
  ...
  it("allows mdns cleartext setup urls", async () => {
    const command = registerPairCommand({
      pluginConfig: {
        publicUrl: "ws://FirstNexus.local:18789",
      },
    });
    const result = await command.handler(
      createCommandContext({
    ...
    expect(pluginApiMocks.issueDeviceBootstrapToken).toHaveBeenCalledTimes(1);
    expect(requireText(result)).toContain("Gateway: ws://FirstNexus.local:18789");
  });
