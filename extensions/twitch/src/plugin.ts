/**
 * Twitch channel plugin for FirstNexus.
 *
...
} from "FirstNexus/plugin-sdk/channel-pairing";
 ...
 * for FirstNexus. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<ResolvedTwitchAccount> =
  createChatChannelPlugin<ResolvedTwitchAccount>({
    ...
    base: {
      ...
      config: {
        listAccountIds: (cfg: FirstNexusConfig): string[] => listAccountIds(cfg),
        resolveAccount: (
        ...
        defaultAccountId: (cfg: FirstNexusConfig): string => resolveDefaultTwitchAccountId(cfg),
        isConfigured: (_account: unknown, cfg: FirstNexusConfig): boolean =>
