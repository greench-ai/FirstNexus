  ...
  if (health?.manualActionRequired) {
    return {
      ...
      message:
        ...
        "Resolve the Google Meet browser prompt before asking FirstNexus to speak.",
    };
  }
  if (health?.inCall === true) {
    if (health.micMuted === true) {
      return {
        ...
        message: "Turn on the FirstNexus Google Meet microphone before asking FirstNexus to speak.",
      };
    }
    if (session.chrome.audioBridge) {
      return { ready: true };
