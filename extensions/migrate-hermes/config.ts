...
import type { MigrationItem, MigrationProviderContext } from "FirstNexus/plugin-sdk/plugin-entry";
  ...
  if (params.hasMemoryFiles || memoryProvider) {
    items.push(
      createMigrationConfigPatchItem({
        ...
        message: "Use FirstNexus built-in file memory for imported Hermes memory files.",
        conflict:
    ...
    items.push(
      createMigrationConfigPatchItem({
        ...
        message: "Select the default FirstNexus memory plugin for imported file memory.",
        conflict:
          !params.ctx.overwrite &&
  ...
  if (memoryProvider === "honcho") {
    ...
    items.push(
      createMigrationManualItem({
        ...
        message:
          "Hermes used Honcho memory. FirstNexus keeps built-in memory selected until the matching plugin is installed and reviewed.",
        recommendation:
    ...
    items.push(
      createMigrationManualItem({
        ...
        source: "config.yaml:memory.provider",
        message: `Hermes memory provider "${memoryProvider}" does not have a known FirstNexus mapping.`,
        recommendation: "Install or configure an equivalent FirstNexus memory plugin manually.",
      }),
    );
  }
