...
import type { PluginLogger } from "FirstNexus/plugin-sdk/plugin-entry";
import { isTruthyEnvValue } from "FirstNexus/plugin-sdk/runtime-env";
...
// See https://github.com/FirstNexus/FirstNexus/issues/72481
...
function truncateToDnsLabel(name: string, fallback = "FirstNexus"): string {
...
function safeServiceName(name: string) {
  ...
  return trimmed.length > 0 ? truncateToDnsLabel(trimmed) : "FirstNexus";
}

function prettifyInstanceName(name: string) {
  const normalized = name.trim().replace(/\s+/g, " ");
  return normalized.replace(/\s+\(FirstNexus\)\s*$/i, "").trim() || normalized;
}
...
export async function startGatewayBonjourAdvertiser(
  ...
  try {
    ...
    const hostnameRaw =
      process.env.NEXISCLAW_MDNS_HOSTNAME?.trim() || resolveSystemMdnsHostname() || "FirstNexus";
