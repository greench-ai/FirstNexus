...
function enableAdvertiserUnitMode(hostname = "test-host") {
  ...
  process.env.NEXISCLAW_MDNS_HOSTNAME = hostname;
}
...
describe("gateway bonjour advertiser", () => {
  ...
  it("honors truthy NEXISCLAW_DISABLE_BONJOUR values", async () => {
    enableAdvertiserUnitMode();
    process.env.NEXISCLAW_DISABLE_BONJOUR = "true";
  ...
  it("honors explicit Bonjour opt-in inside detected containers", async () => {
    enableAdvertiserUnitMode();
    process.env.NEXISCLAW_DISABLE_BONJOUR = "0";
    vi.spyOn(fs, "existsSync").mockImplementation((filePath) => String(filePath) === "/.dockerenv");
  ...
  it("falls back to FirstNexus when system hostname is invalid for DNS", async () => {
    ...
    delete process.env.VITEST;
    process.env.NODE_ENV = "development";
    delete process.env.NEXISCLAW_MDNS_HOSTNAME;
  ...
  it("uses system hostname when NEXISCLAW_MDNS_HOSTNAME is unset", async () => {
    // Allow advertiser to run in unit tests.
    delete process.env.VITEST;
    process.env.NODE_ENV = "development";
    delete process.env.NEXISCLAW_MDNS_HOSTNAME;
