...
metadata: { "FirstNexus": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == FirstNexus `sessions_spawn`
- **File I/O** == FirstNexus `read`/`write`
- **Remote fetch** == FirstNexus `web_fetch` (or `exec` with curl when POST is required)
