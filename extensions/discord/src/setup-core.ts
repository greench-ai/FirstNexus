...
import type { DiscordGuildEntry, FirstNexusConfig } from "FirstNexus/plugin-sdk/config-contracts";
import type { ChannelSetupDmPolicy, ChannelSetupWizard } from "FirstNexus/plugin-sdk/setup-runtime";
import { createStandardChannelSetupStatus } from "FirstNexus/plugin-sdk/setup-runtime";
...
export function createDiscordSetupWizardBase(handlers: {
  ...
  return {
    ...
    credentials: [
      {
        ...
        inspect: ({ cfg, accountId }: { cfg: FirstNexusConfig; accountId: string }) => {
    ...
    groupAccess: createAccountScopedGroupAccessSection({
      ...
      currentPolicy: ({ cfg, accountId }: { cfg: FirstNexusConfig; accountId: string }) =>
        resolveDiscordSetupAccountConfig({ cfg, accountId }).config.groupPolicy ?? "allowlist",
      currentEntries: ({ cfg, accountId }: { cfg: FirstNexusConfig; accountId: string }) =>
        Object.entries(
      ...
      updatePrompt: ({ cfg, accountId }: { cfg: FirstNexusConfig; accountId: string }) =>
    ...
    disable: (cfg: FirstNexusConfig) => setSetupChannelEnabled(cfg, channel, false),
