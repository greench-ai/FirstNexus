import { DEFAULT_EMOJIS, DEFAULT_TIMING } from "FirstNexus/plugin-sdk/channel-feedback";
...
const configSessionsMocks = vi.hoisted(() => ({
  ...
  resolveStorePath: vi.fn<(path?: unknown, opts?: unknown) => string>(
    () => "/tmp/FirstNexus-discord-process-test-sessions.json",
  ),
}));
...
vi.mock("FirstNexus/plugin-sdk/session-store-runtime", () => ({
...
beforeEach(() => {
  ...
  resolveStorePath.mockReturnValue("/tmp/FirstNexus-discord-process-test-sessions.json");
  ...
  it("applies status reaction emoji/timing overrides from config", async () => {
    ...
    const ctx = await createAutomaticSourceDeliveryContext({
      cfg: {
        ...
        session: { store: "/tmp/FirstNexus-discord-process-test-sessions.json" },
      },
    });
  ...
  it("pins Discord text DM main-route updates to the single configured DM owner", async () => {
    const ctx = await createBaseContext({
      ...
      cfg: {
        ...
        session: {
          store: "/tmp/FirstNexus-discord-process-test-sessions.json",
          dmScope: "main",
        },
      },
