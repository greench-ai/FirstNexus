...
vi.mock("FirstNexus/plugin-sdk/channel-message", async () => {
  const actual = await vi.importActual<typeof import("FirstNexus/plugin-sdk/channel-message")>(
    "FirstNexus/plugin-sdk/channel-message",
  );
  return {
    ...actual,
...
describe("deliverDiscordReply", () => {
  ...
  it("passes media roots and explicit off-mode payload reply tags to shared outbound", async () => {
    ...
    await deliverDiscordReply({
      ...
      textLimit: 2000,
      replyToMode: "off",
      mediaLocalRoots: ["/tmp/FirstNexus-media"],
    });
    ...
    expect(params.mediaAccess).toEqual({ localRoots: ["/tmp/FirstNexus-media"] });
  });
