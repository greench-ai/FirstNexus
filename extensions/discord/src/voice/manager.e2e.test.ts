...
vi.mock("FirstNexus/plugin-sdk/routing", async () => {
  const actual = await vi.importActual<typeof import("FirstNexus/plugin-sdk/routing")>(
    "FirstNexus/plugin-sdk/routing",
  );
  return {
    ...actual,
...
vi.mock("FirstNexus/plugin-sdk/realtime-voice", async () => {
  const actual = await vi.importActual<typeof import("FirstNexus/plugin-sdk/realtime-voice")>(
    "FirstNexus/plugin-sdk/realtime-voice",
  );
  return {
    ...actual,
...
describe("DiscordVoiceManager", () => {
  ...
  it("uses agent-proxy realtime voice by default", async () => {
    ...
    expect(bridgeParams?.instructions).toContain("same FirstNexus agent");
    expect(bridgeParams?.tools?.map((tool) => tool.name)).toContain("FirstNexus_agent_consult");
  ...
  it("starts Discord realtime voice in bidi mode with the consult tool", async () => {
    ...
    expect(bridgeParams?.instructions).toContain("Call FirstNexus_agent_consult");
    expect(bridgeParams?.tools?.map((tool) => tool.name)).toContain("FirstNexus_agent_consult");
