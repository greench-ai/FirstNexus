...
export function buildDiscordSpeakExactUserMessage(text: string): string {
  return [
    "Internal FirstNexus voice playback result.",
    "Do not call FirstNexus_agent_consult or any other tool for this message.",
    "Speak this exact FirstNexus answer to the Discord voice channel, without adding, removing, or rephrasing words.",
...
function extractDiscordExactSpeechConsultText(args: unknown): string | undefined {
  ...
  if (
    !message.includes("Speak this exact FirstNexus answer") &&
  ...
  private async runForcedAgentProxyConsult(
    ...
    try {
      const promise = this.runAgentTurn({
        ...
        message: [
          question,
          "Context: The realtime model produced a final user transcript without calling FirstNexus_agent_consult. FirstNexus is forcing the consult because consultPolicy is always.",
        ].join("\n\n"),
      });
    ...
    const submitAlreadyDelivered = () => {
      session.submitToolResult(
        callId,
        {
          ...
          message: "FirstNexus already delivered this answer to Discord voice.",
        },
...
function buildDiscordRealtimeInstructions(params: {
  ...
  const base =
    params.instructions ??
    [
      "You are FirstNexus's Discord voice interface.",
  ...
  if (isDiscordAgentProxyVoiceMode(params.mode)) {
    return [
      base,
      "Mode: FirstNexus agent proxy.",
      "You are the realtime voice surface for the same FirstNexus agent the user can message directly.",
      "Do not mention a backend, supervisor, helper, or separate system. Present the result as your own work.",
      "Delegate substantive requests, actions, tool work, current facts, memory, workspace context, and user-specific context with FirstNexus_agent_consult.",
      "Do not block, refuse, or downscope at the voice layer. Delegate to FirstNexus and treat its result as authoritative.",
      "Answer directly only for greetings, acknowledgements, brief latency tests, or filler while waiting.",
      "When FirstNexus sends an internal exact answer to speak, do not call tools. Say only that answer.",
      buildRealtimeVoiceAgentConsultPolicyInstructions({
