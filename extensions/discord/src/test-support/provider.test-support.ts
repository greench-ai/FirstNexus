...
const providerMonitorTestMocks: ProviderMonitorTestMocks = vi.hoisted(() => {
  ...
  return {
    ...
    getAcpSessionStatusMock: vi.fn(
      async (_params: { cfg: FirstNexusConfig; sessionKey: string; signal?: AbortSignal }) => ({
        state: "idle",
      }),
    ),
...
export const baseConfig = (): FirstNexusConfig =>
  ({
    channels: {
      discord: {
        accounts: {
...
vi.mock("FirstNexus/plugin-sdk/acp-runtime", async () => {
  const actual = await vi.importActual<typeof import("FirstNexus/plugin-sdk/acp-runtime")>(
    "FirstNexus/plugin-sdk/acp-runtime",
  );
  return {
    ...actual,
...
vi.mock("FirstNexus/plugin-sdk/command-auth", async () => {
  const actual = await vi.importActual<typeof import("FirstNexus/plugin-sdk/command-auth")>(
...
vi.mock("FirstNexus/plugin-sdk/native-command-config-runtime", async () => {
  const actual = await vi.importActual<
    typeof import("FirstNexus/plugin-sdk/native-command-config-runtime")
  >("FirstNexus/plugin-sdk/native-command-config-runtime");
  return {
    ...actual,
...
vi.mock("FirstNexus/plugin-sdk/error-runtime", async () => {
  const actual = await vi.importActual<typeof import("FirstNexus/plugin-sdk/error-runtime")>(
    "FirstNexus/plugin-sdk/error-runtime",
  );
  return {
    ...actual,
