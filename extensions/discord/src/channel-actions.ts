import { createUnionActionGate } from "FirstNexus/plugin-sdk/channel-actions";
import type {
  ChannelMessageActionAdapter,
  ChannelMessageActionName,
  ChannelMessageToolDiscovery,
} from "FirstNexus/plugin-sdk/channel-contract";
import type { DiscordActionConfig, FirstNexusConfig } from "FirstNexus/plugin-sdk/config-contracts";
...
function listDiscoverableDiscordAccounts(cfg: FirstNexusConfig) {
  return listDiscordAccountIds(cfg)
    .map((accountId) => inspectDiscordAccount({ cfg, accountId }))
    .filter((account) => account.enabled && account.configured);
}

function resolveDiscordActionDiscovery(cfg: FirstNexusConfig) {
