...
vi.mock("FirstNexus/plugin-sdk/setup", async () => {
  const actual = await vi.importActual<object>("FirstNexus/plugin-sdk/setup");
  return {
    ...actual,
...
vi.mock("FirstNexus/plugin-sdk/channel-config-schema", async () => {
  const actual = await vi.importActual<object>("FirstNexus/plugin-sdk/channel-config-schema");
  return {
    ...actual,
...
vi.mock("FirstNexus/plugin-sdk/webhook-ingress", async () => {
  const actual = await vi.importActual<object>("FirstNexus/plugin-sdk/webhook-ingress");
  return {
    ...actual,
...
vi.mock("./runtime.js", () => ({
  getSynologyRuntime: vi.fn(() => ({
    ...
    channel: {
      ...
      session: {
        resolveStorePath: vi.fn(() => "/tmp/FirstNexus/synology-chat-sessions.json"),
