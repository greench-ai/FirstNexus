...
function getChannelConfig(cfg: FirstNexusConfig): SynologyChatChannelConfig {
  return (cfg.channels?.[channel] as SynologyChatChannelConfig | undefined) ?? {};
}

function getRawAccountConfig(cfg: FirstNexusConfig, accountId: string): SynologyChatAccountRaw {
...
function isSynologyChatConfigured(cfg: FirstNexusConfig, accountId: string): boolean {
...
function resolveExistingAllowedUserIds(cfg: FirstNexusConfig, accountId: string): string[] {
  ...
  textInputs: [
    {
      ...
      helpLines: [
        ...
        "This is the URL FirstNexus uses to send replies back to Chat.",
      ],
