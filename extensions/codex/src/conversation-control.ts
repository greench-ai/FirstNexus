...
const CODEX_CONVERSATION_CONTROL_STATE = Symbol.for("FirstNexus.codex.conversationControl");
...
async function requireThreadBinding(sessionFile: string) {
  ...
  if (!binding?.threadId) {
    throw new Error("No Codex thread is attached to this FirstNexus session yet.");
  }
  return binding;
}
