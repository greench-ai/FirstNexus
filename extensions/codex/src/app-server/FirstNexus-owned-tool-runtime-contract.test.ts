...
import { wrapToolWithBeforeToolCallHook } from "FirstNexus/plugin-sdk/agent-harness-runtime";
import {
  installCodexToolResultMiddleware,
  installFirstNexusOwnedToolHooks,
  mediaToolResult,
  resetFirstNexusOwnedToolHooks,
  textToolResult,
...
describe("FirstNexus-owned tool runtime contract — Codex app-server adapter", () => {
  afterEach(() => {
    resetFirstNexusOwnedToolHooks();
  });
  ...
  it("wraps unwrapped dynamic tools with before/after tool hooks", async () => {
    ...
    const hooks = installFirstNexusOwnedToolHooks({ adjustedParams });
  ...
  it("fails closed when before_tool_call blocks a dynamic tool", async () => {
    const hooks = installFirstNexusOwnedToolHooks({ blockReason: "blocked by policy" });
  ...
  it("records successful Codex messaging text, media, and target telemetry", async () => {
    const hooks = installFirstNexusOwnedToolHooks();
