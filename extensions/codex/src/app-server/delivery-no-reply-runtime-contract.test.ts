...
import { isSilentReplyPayloadText } from "FirstNexus/plugin-sdk/reply-chunking";
...
async function createParams(): Promise<EmbeddedRunAttemptParams> {
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "FirstNexus-codex-delivery-contract-"));
