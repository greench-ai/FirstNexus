...
export function attachCodexMirrorIdentity<T extends AgentMessage>(message: T, identity: string): T {
  ...
  const existing = record.__FirstNexus;
  const baseMeta =
  ...
  return {
    ...record,
    __FirstNexus: { ...baseMeta, [MIRROR_IDENTITY_META_KEY]: identity },
  } as unknown as T;
}

function readMirrorIdentity(message: MirroredAgentMessage): string | undefined {
  const record = message as unknown as { __FirstNexus?: unknown };
  const meta = record.__FirstNexus;
