...
import type { EmbeddedRunAttemptParams } from "FirstNexus/plugin-sdk/agent-harness";
import { classifyEmbeddedPiRunResultForModelFallback } from "FirstNexus/plugin-sdk/agent-harness-runtime";
import {
...
type MirrorTaggedMessage = { __FirstNexus?: { mirrorIdentity?: string } };

async function createParams(): Promise<EmbeddedRunAttemptParams> {
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "FirstNexus-codex-outcome-contract-"));
...
function readMirrorIdentity(message: unknown): string | undefined {
  const meta = (message as MirrorTaggedMessage | undefined)?.__FirstNexus;
...
describe("Outcome/fallback runtime contract - Codex app-server adapter", () => {
  it("preserves an empty terminal turn for FirstNexus-owned fallback classification", async () => {
  ...
  it("preserves reasoning-only terminal turns for FirstNexus-owned fallback classification", async () => {
    const projector = await createProjector();
    await projector.handleNotification(
  ...
  it("preserves planning-only terminal turns for FirstNexus-owned fallback classification", async () => {
    const projector = await createProjector();
    await projector.handleNotification(
