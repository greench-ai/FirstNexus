...
async function findManagedCodexAppServerCommandPath(params: {
  ...
  throw new Error(
    [
      ...
      "Reinstall or update FirstNexus, or run pnpm install in a source checkout.",
      "Set plugins.entries.codex.config.appServer.command or NEXISCLAW_CODEX_APP_SERVER_BIN to use a custom Codex binary.",
    ].join(" "),
  );
}
