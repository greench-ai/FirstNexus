...
import { resetAgentEventsForTest } from "FirstNexus/plugin-sdk/agent-harness-runtime";
import {
...
async function createParams(): Promise<EmbeddedRunAttemptParams> {
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "FirstNexus-codex-projector-"));
...
describe("CodexAppServerEventProjector", () => {
  ...
  it("records dynamic FirstNexus tool calls in mirrored transcript snapshots", async () => {
    const projector = await createProjector();
