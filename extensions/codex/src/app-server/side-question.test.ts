...
vi.mock("FirstNexus/plugin-sdk/agent-harness", () => ({
  createFirstNexusCodingTools: (...args: unknown[]) => createFirstNexusCodingToolsMock(...args),
}));
...
describe("runCodexAppServerSideQuestion", () => {
  beforeEach(() => {
    ...
    createFirstNexusCodingToolsMock.mockReset();
  ...
  it("forks an ephemeral side thread and returns the completed assistant text", async () => {
    ...
    const [toolOptions] = mockCall(createFirstNexusCodingToolsMock);
  ...
  it("bridges side-thread dynamic tool requests to FirstNexus tools", async () => {
    const client = createFakeClient();
  ...
  it("cleans up notification handlers when side tool setup fails", async () => {
    const client = createFakeClient();
    createFirstNexusCodingToolsMock.mockImplementation(() => {
