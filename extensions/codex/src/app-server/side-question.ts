...
async function createCodexSideToolBridge(input: {
  ...
  if (supportsModelTools(runtimeModel)) {
    const createFirstNexusCodingTools = (await import("FirstNexus/plugin-sdk/agent-harness"))
...
async function handleSideDynamicToolCallWithTimeout(params: {
  ...
  if (params.signal.aborted) {
    return failedSideDynamicToolResponse("FirstNexus dynamic tool call aborted before execution.");
  }
  ...
  const timeoutPromise = new Promise<CodexDynamicToolCallResponse>((resolve) => {
    ...
    timeout = setTimeout(() => {
      controller.abort(new Error(`FirstNexus dynamic tool call timed out after ${timeoutMs}ms.`));
      resolve(
        failedSideDynamicToolResponse(
          `FirstNexus dynamic tool call timed out after ${timeoutMs}ms.`,
        ),
      );
    }, timeoutMs);
    timeout.unref?.();
