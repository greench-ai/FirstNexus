...
describe("runCodexAppServerAttempt context-engine lifecycle", () => {
  beforeEach(async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "FirstNexus-codex-context-engine-"));
  });
  ...
  it("bootstraps and assembles non-legacy context before the Codex turn starts", async () => {
    ...
    expectRequestInputTextContains(harness, "FirstNexus assembled context for this turn:");
  ...
  it("keeps current-turn context at the front of the Codex context-engine prompt", async () => {
    ...
    params.currentTurnContext = {
      text: [
        ...
        "#6474 Sun 2026-05-10 22:22 GMT+5:30 [reply target] FirstNexus: anchor REPLYCTX this is the old message",
        "#6498 Sun 2026-05-10 22:22 GMT+5:30 FirstNexus: filler REPLYCTX 23",
      ].join("\n"),
    };
    ...
    expect(inputText).toContain("[reply target] FirstNexus: anchor REPLYCTX");
