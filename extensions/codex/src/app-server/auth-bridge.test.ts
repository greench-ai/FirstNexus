...
vi.mock("FirstNexus/plugin-sdk/agent-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("FirstNexus/plugin-sdk/agent-runtime")>();
  return {
    ...actual,
    resolveApiKeyForProfile: async (
      ...
      if ((oauthCredential.expires ?? 0) <= Date.now()) {
        ...
        if (refreshed?.access) {
          ...
          if (params.agentDir || process.env.NEXISCLAW_STATE_DIR) {
...
describe("bridgeCodexAppServerStartOptions", () => {
  it("sets agent-owned CODEX_HOME and HOME for local app-server launches", async () => {
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "FirstNexus-codex-app-server-"));
  ...
  it("applies native Codex CLI OAuth when no FirstNexus auth profile exists", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "FirstNexus-codex-app-server-"));
  ...
  it("answers refresh from native Codex CLI OAuth without persisting an FirstNexus profile", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "FirstNexus-codex-app-server-"));
    const agentDir = path.join(root, "agent");
    const codexHome = path.join(root, "codex-cli");
