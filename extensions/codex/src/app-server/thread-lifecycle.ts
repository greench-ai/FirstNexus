...
function buildHeartbeatCollaborationInstructions(): string {
  return [
    "This is an FirstNexus heartbeat turn. Apply these instructions only to this heartbeat wake; ordinary chat turns should stay in Codex Default mode.",
    "When you are ready to end the heartbeat, prefer the structured `heartbeat_respond` tool so FirstNexus can record the wake outcome and notification decision. If `heartbeat_respond` is not already available and `tool_search` is available, search for `heartbeat_respond`, load it, then call it. Use `notify=false` when nothing should visibly interrupt the user.",
    CODEX_GPT5_HEARTBEAT_PROMPT_OVERLAY,
  ].join("\n\n");
}
...
export function buildDeveloperInstructions(params: EmbeddedRunAttemptParams): string {
  ...
  const sections = [
    "Running inside FirstNexus. Use dynamic tools for messaging, cron, sessions, media, gateway, and nodes when available.",
...
export function resolveCodexAppServerModelProvider(params: {
  ...
  if (!normalized || normalizedLower === "codex") {
    // `codex` is FirstNexus's virtual provider; let Codex app-server keep its
  ...
  if (
    ...
    // When FirstNexus is forwarding ChatGPT/Codex OAuth, `openai` is Codex's
