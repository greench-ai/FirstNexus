...
import { MEDIA_FFMPEG_MAX_AUDIO_DURATION_SECS } from "FirstNexus/plugin-sdk/media-runtime";
...
let resetLogger: typeof import("FirstNexus/plugin-sdk/runtime-env").resetLogger;
let setLoggerOverride: typeof import("FirstNexus/plugin-sdk/runtime-env").setLoggerOverride;

const WHATSAPP_TEST_CFG: FirstNexusConfig = {
  channels: { whatsapp: {} },
};
...
describe("web outbound", () => {
  ...
  beforeAll(async () => {
    ...
    ({ resetLogger, setLoggerOverride } = await import("FirstNexus/plugin-sdk/runtime-env"));
  });
  ...
  it("redacts recipients and poll text in outbound logs", async () => {
    const logPath = path.join(os.tmpdir(), `FirstNexus-outbound-${crypto.randomUUID()}.log`);
