...
import { optimizeImageToPng } from "FirstNexus/plugin-sdk/web-media";
...
beforeAll(async () => {
  fixtureRoot = await fs.mkdtemp(
    path.join(resolvePreferredFirstNexusTmpDir(), "FirstNexus-media-test-"),
  );
...
describe("web media loading", () => {
  beforeAll(() => {
    ...
    // Also keep it outside the FirstNexus temp root so default localRoots doesn't accidentally make all state readable.
    stateDirSnapshot = captureEnv(["NEXISCLAW_STATE_DIR"]);
    process.env.NEXISCLAW_STATE_DIR = path.join(
      path.parse(os.tmpdir()).root,
      "var",
      "lib",
      "FirstNexus-media-state-test",
    );
  });
...
describe("local media root guard", () => {
  ...
  it("allows local paths under an explicit root", async () => {
    const result = await loadWebMedia(tinyPngFile, 1024 * 1024, {
      localRoots: [resolvePreferredFirstNexusTmpDir()],
    });
    expect(result.kind).toBe("image");
  });
  ...
  it("accepts win32 dev=0 stat mismatch for local file loads", async () => {
    ...
    // os.tmpdir() fallback rather than the POSIX `/tmp/FirstNexus` root that
    // actually holds `tinyPngFile` on this Linux test runner (#60713).
    const realTmpRoot = resolvePreferredFirstNexusTmpDir();
  ...
  it("allows default FirstNexus state workspace and sandbox roots", async () => {
  ...
  it("rejects default FirstNexus state per-agent workspace-* roots without explicit local roots", async () => {
    const stateDir = resolveStateDir();
    const readFile = vi.fn(async () => Buffer.from("generated-media"));
