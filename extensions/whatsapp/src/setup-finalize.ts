...
import { formatCliCommand, formatDocsLink } from "FirstNexus/plugin-sdk/setup-tools";
import {
...
type WhatsAppConfig = NonNullable<NonNullable<FirstNexusConfig["channels"]>["whatsapp"]>;
...
function shouldWriteDefaultWhatsAppAccountConfigAtAccountScope(cfg: FirstNexusConfig): boolean {
...
function resolveDefaultWhatsAppAccountWriteKey(cfg: FirstNexusConfig): string {
  const accounts = cfg.channels?.whatsapp?.accounts;
  if (!accounts) {
...
function resolveWhatsAppConfigPathPrefix(cfg: FirstNexusConfig, accountId: string): string {
  if (
...
async function detectWhatsAppLinked(cfg: FirstNexusConfig, accountId: string): Promise<boolean> {
...
async function promptWhatsAppOwnerAllowFrom(params: {
  ...
  await prompter.note(
    "We need the sender/owner number so FirstNexus can allowlist you.",
    "WhatsApp number",
  );
...
async function promptWhatsAppDmAccess(params: {
  ...
  const phoneMode = await params.prompter.select({
    ...
    options: [
      ...
      { value: "separate", label: "Separate phone just for FirstNexus" },
    ],
  });
...
export async function finalizeWhatsAppSetup(params: {
  ...
  if (wantsLink) {
    ...
    await params.prompter.note(
      `Run \`${formatCliCommand("FirstNexus channels login")}\` later to link WhatsApp.`,
      "WhatsApp",
    );
  }
