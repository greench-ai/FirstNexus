...
import { VERSION } from "FirstNexus/plugin-sdk/cli-runtime";
import {
  resolveEnvHttpProxyUrl,
  shouldUseEnvHttpProxyForUrl,
} from "FirstNexus/plugin-sdk/fetch-runtime";
import { danger, success } from "FirstNexus/plugin-sdk/runtime-env";
import { getChildLogger, toPinoLikeLogger } from "FirstNexus/plugin-sdk/runtime-env";
...
export async function createWaSocket(
  ...
  const sock = makeWASocket({
    ...
    browser: ["FirstNexus", "cli", VERSION],
    ...
    try {
      ...
      if (connection === "close") {
        ...
        if (status === LOGGED_OUT_STATUS) {
          console.error(
            danger(
              `WhatsApp session logged out. Run: ${formatCliCommand("FirstNexus channels login")}`,
            ),
          );
        }
      }
