...
import { escapeRegExp, formatEnvelopeTimestamp } from "FirstNexus/plugin-sdk/channel-test-helpers";
import type { FirstNexusConfig } from "FirstNexus/plugin-sdk/config-contracts";
import { setLoggerOverride } from "FirstNexus/plugin-sdk/runtime-env";
...
describe("web auto-reply connection", () => {
    ...
    await withEnvAsync({ TZ: "Europe/Vienna" }, async () => {
      ...
      try {
        ...
        expect(firstArgs.Body).toMatch(
          new RegExp(`\\[WhatsApp \\+1 (\\+\\d+[smhd] )?${firstPattern}\\] \\[FirstNexus\\] first`),
        );
        expect(firstArgs.Body).not.toContain("second");
        expect(secondArgs.Body).toMatch(
          new RegExp(
            `\\[WhatsApp \\+1 (\\+\\d+[smhd] )?${secondPattern}\\] \\[FirstNexus\\] second`,
          ),
        );
  ...
  it("emits heartbeat logs with connection metadata", async () => {
    vi.useFakeTimers();
    const logPath = `/tmp/FirstNexus-heartbeat-${crypto.randomUUID()}.log`;
  ...
  it("logs outbound replies to file", async () => {
    const logPath = `/tmp/FirstNexus-log-test-${crypto.randomUUID()}.log`;
    setLoggerOverride({ level: "trace", file: logPath });
  ...
  it("marks dispatch idle after replies flush", async () => {
    ...
    const mockConfig: FirstNexusConfig = {
