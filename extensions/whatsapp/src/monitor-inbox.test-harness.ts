...
vi.mock("FirstNexus/plugin-sdk/channel-activity-runtime", async () => {
  const actual = await vi.importActual<
    typeof import("FirstNexus/plugin-sdk/channel-activity-runtime")
  >("FirstNexus/plugin-sdk/channel-activity-runtime");
  return {
    ...actual,
...
function expectInboxPairingReplyText(
  ...
  expect(text).toContain("FirstNexus: access not configured.");
...
export function installWebMonitorInboxUnitTestHooks(opts?: { authDir?: boolean }) {
  ...
  beforeEach(async () => {
    ...
    if (createAuthDir) {
      authDir = fsSync.mkdtempSync(path.join(os.tmpdir(), "FirstNexus-auth-"));
    } else {
      authDir = undefined;
    }
  });
