...
vi.mock("FirstNexus/plugin-sdk/logging-core", async () => {
  const actual = await vi.importActual<typeof import("FirstNexus/plugin-sdk/logging-core")>(
    "FirstNexus/plugin-sdk/logging-core",
  );
  return {
    ...actual,
    getChildLogger: () => ({
...
describe("web monitor inbox", () => {
  ...
  it("detaches inbound listeners and ends the socket on close()", async () => {
    ...
    expect(closeError?.message).toBe("FirstNexus WhatsApp listener close");
