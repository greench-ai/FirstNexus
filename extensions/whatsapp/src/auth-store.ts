...
import { info, success } from "FirstNexus/plugin-sdk/runtime-env";
import { getChildLogger } from "FirstNexus/plugin-sdk/runtime-env";
import { defaultRuntime, type RuntimeEnv } from "FirstNexus/plugin-sdk/runtime-env";
import { replaceFileAtomic } from "FirstNexus/plugin-sdk/security-runtime";
...
export async function pickWebChannel(
  ...
  if (!auth.exists) {
    throw new Error(
      `No WhatsApp Web session found. Run \`${formatCliCommand("FirstNexus channels login --channel whatsapp --verbose")}\` to link.`,
    );
  }
  return choice;
}
