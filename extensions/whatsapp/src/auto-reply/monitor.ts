import { resolveAccountEntry } from "FirstNexus/plugin-sdk/account-core";
import { resolveInboundDebounceMs } from "FirstNexus/plugin-sdk/channel-inbound-debounce";
...
import { drainPendingDeliveries } from "FirstNexus/plugin-sdk/delivery-queue-runtime";
import { DEFAULT_GROUP_HISTORY_LIMIT } from "FirstNexus/plugin-sdk/reply-history";
import { resolveAgentRoute } from "FirstNexus/plugin-sdk/routing";
import { logVerbose } from "FirstNexus/plugin-sdk/runtime-env";
import { registerUnhandledRejectionHandler } from "FirstNexus/plugin-sdk/runtime-env";
...
async function clearTerminalWebAuthState(params: {
  ...
  try {
    ...
    params.runtime.error(
      `WhatsApp Web cleanup failed after terminal close (status ${params.statusLabel}). Run \`${formatCliCommand("FirstNexus channels logout --channel whatsapp")}\`, then relink with \`${formatCliCommand("FirstNexus channels login --channel whatsapp")}\`.`,
    );
  }
}
    ...
    while (true) {
      ...
      try {
        ...
        if (getStatusCode(error) === 428) {
          ...
          if (retryDecision.action === "stop") {
            ...
            runtime.error(
              `WhatsApp Web connection closed during setup (status 428) after ${retryDecision.reconnectAttempts}/${reconnectPolicy.maxAttempts} attempts. Relink with \`${formatCliCommand("FirstNexus channels login --channel whatsapp")}\` if the issue persists.`,
            );
      ...
      if (decision.action === "stop") {
        ...
        if (decision.healthState === "logged-out") {
          ...
          runtime.error(
            `WhatsApp session logged out. Run \`${formatCliCommand("FirstNexus channels login --channel whatsapp")}\` to relink.`,
          );
          ...
          runtime.error(
            `WhatsApp Web connection closed (status ${decision.normalized.statusLabel}: session conflict). Resolve conflicting WhatsApp Web sessions, then relink with \`${formatCliCommand("FirstNexus channels login --channel whatsapp")}\`. Stopping web monitoring.`,
          );
        } else {
          reconnectLogger.warn(
            {
