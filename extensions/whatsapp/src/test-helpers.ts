...
import { formatEnvelopeTimestamp } from "FirstNexus/plugin-sdk/channel-test-helpers";
...
const CONFIG_KEY = Symbol.for("FirstNexus:testConfigMock");
const SOURCE_CONFIG_KEY = Symbol.for("FirstNexus:testSourceConfigMock");
...
vi.mock("./auto-reply/monitor/message-line.runtime.js", () => ({
  ...
  resolveMessagePrefix: (
    ...
    return params?.hasAllowFrom === true ? "" : "[FirstNexus]";
  },
}));

vi.mock("./auth-store.runtime.js", () => ({
  resolveOAuthDir: () => "/tmp/FirstNexus-oauth",
}));

vi.mock("./session.runtime.js", () => {
  const created = createMockBaileys();
  (globalThis as Record<PropertyKey, unknown>)[Symbol.for("FirstNexus:lastSocket")] =
    created.lastSocket;
  return {
    ...created.mod,
  };
...
export function getLastSocket(): MockBaileysSocket {
  const getter = (globalThis as Record<PropertyKey, unknown>)[Symbol.for("FirstNexus:lastSocket")];
  if (typeof getter === "function") {
