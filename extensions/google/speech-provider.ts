import { transcodeAudioBufferToOpus } from "FirstNexus/plugin-sdk/media-runtime";
import {
...
import { asObject, trimToUndefined } from "FirstNexus/plugin-sdk/speech-core";
...
function resolveGoogleTtsModelProviderApiKey(cfg?: FirstNexusConfig): string | undefined {
...
function isFirstNexusGoogleAudioProfilePrompt(text: string): boolean {
  return (
...
export function buildGoogleSpeechProvider(): SpeechProviderPlugin {
  return {
    ...
    prepareSynthesis: (ctx) => {
      ...
      if (!shouldWrap || isFirstNexusGoogleAudioProfilePrompt(ctx.text)) {
        return undefined;
      }
      return {
