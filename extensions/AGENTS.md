...
- Extension production code should import from `FirstNexus/plugin-sdk/*` and its
...
- Keep plugin metadata accurate in `FirstNexus.plugin.json` and the package
  `FirstNexus` block so discovery and setup work without executing plugin code.
...
- Before adding a new provider-local `wrapStreamFn`, `buildReplayPolicy`,
  ...
  whether the same behavior already exists through `FirstNexus/plugin-sdk/*`.
