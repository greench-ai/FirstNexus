import { formatCliCommand, parseDurationMs } from "FirstNexus/plugin-sdk/cli-runtime";
import type {
  FirstNexusPluginApi,
  ProviderAuthContext,
...
import {
  ...
  type FirstNexusConfig as ProviderAuthConfig,
...
import { fetchClaudeUsage } from "FirstNexus/plugin-sdk/provider-usage";
...
const ANTHROPIC_SETUP_TOKEN_NOTE_LINES = [
  ...
  `If you want a direct API billing path instead, use ${formatCliCommand("FirstNexus models auth login --provider anthropic --method api-key --set-default")} or ${formatCliCommand("FirstNexus models auth login --provider anthropic --method cli --set-default")}.`,
] as const;
...
function buildAnthropicAuthDoctorHint(params: {
  ...
  return [
    ...
    `Fix: run "${formatCliCommand("FirstNexus doctor --yes")}"`,
  ].join("\n");
}
...
export function registerAnthropicPlugin(api: FirstNexusPluginApi): void {
