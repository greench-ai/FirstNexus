...
const {
  ...
  resolveMatrixQaFirstNexusCliEntryPath,
  runMatrixQaFirstNexusCli,
  startMatrixQaFirstNexusCli,
} = vi.hoisted(() => ({
  formatMatrixQaCliCommand: (args: string[]) => `FirstNexus ${args.join(" ")}`,
  redactMatrixQaCliOutput: (text: string) => text,
  resolveMatrixQaFirstNexusCliEntryPath: (cwd: string) => `${cwd}/dist/index.js`,
  runMatrixQaFirstNexusCli: vi.fn(),
  startMatrixQaFirstNexusCli: vi.fn(),
}));
...
describe("matrix live qa scenarios", () => {
  beforeEach(() => {
    ...
    runMatrixQaFirstNexusCli.mockReset();
    startMatrixQaFirstNexusCli.mockReset();
  ...
  it("merges default and scenario-requested Matrix topology once per run", () => {
    expect(
      scenarioTesting.buildMatrixQaTopologyForScenarios({
        defaultRoomName: "FirstNexus Matrix QA run",
        scenarios: [
  ...
  it("blocks MXID-prefixed Matrix control commands from non-allowlisted observers", async () => {
    ...
    expect(waitForOptionalRoomEvent.mock.calls.at(0)?.[0]?.roomId).toBe("!main:matrix-qa.test");
  });
  ...
  it("hot-reloads group allowlist removals inside one running Matrix gateway", async () => {
    ...
    expect(sendTextMessage.mock.calls.at(0)?.[0]?.mentionUserIds).toEqual(["@sut:matrix-qa.test"]);
    expect(sendTextMessage.mock.calls.at(0)?.[0]?.roomId).toBe("!main:matrix-qa.test");
  ...
  it("fails if a handled Matrix event is redelivered after gateway restart", async () => {
    ...
    expect(waitForOptionalRoomEvent.mock.calls.at(0)?.[0]?.roomId).toBe("!restart:matrix-qa.test");
    expect(waitForOptionalRoomEvent.mock.calls.at(0)?.[0]?.timeoutMs).toBe(8000);
  });
  ...
  it("configures a fresh encrypted room before sync-state-loss recovery", async () => {
    ...
    try {
      ...
      const waitParams = rawWaitForRoomEvent.mock.calls.at(0)?.[0];
  ...
  it("isolates E2EE restart-resume gateway groups and restores them after the scenario", async () => {
    ...
    try {
      ...
      const clientOptions = createMatrixQaE2eeScenarioClient.mock.calls.at(0)?.[0];
  ...
  it("runs the DM scenario against the provisioned DM room without a mention", async () => {
    ...
    expect(waitForRoomEvent.mock.calls.at(0)?.[0]?.roomId).toBe("!dm:matrix-qa.test");
  });
  ...
  it("runs the subagent thread spawn scenario against a child thread", async () => {
    ...
    expect(waitForRoomEvent.mock.calls.at(0)?.[0]?.since).toBe("driver-sync-start");
    const completionWaitOptions = waitForRoomEvent.mock.calls.at(1)?.[0];
    expect(typeof completionWaitOptions?.predicate).toBe("function");
    expect(completionWaitOptions?.since).toBe("driver-sync-intro");
    const introPredicate = waitForRoomEvent.mock.calls.at(0)?.[0]?.predicate as
  ...
  it("captures quiet preview notices before the finalized Matrix reply", async () => {
    ...
    expect(waitForRoomEvent.mock.calls.at(0)?.[0]?.since).toBe("driver-sync-start");
    const finalWaitOptions = waitForRoomEvent.mock.calls.at(1)?.[0];
  ...
  it("finalizes Matrix tool progress previews after tool errors", async () => {
    ...
    expect(waitForRoomEvent.mock.calls.at(0)?.[0].predicate(progressEvent)).toBe(true);
    expectSentTextMessage(sendTextMessage, {
  ...
  it("sends a real Matrix image attachment for image-understanding prompts", async () => {
    ...
    const mediaMessage = sendMediaMessage.mock.calls.at(0)?.[0];
    ...
    expect(waitForRoomEvent.mock.calls.at(1)?.[0]?.since).toBe("driver-sync-attachment");
  });
  ...
  it("surfaces the shared DM session notice in the secondary DM room", async () => {
    ...
    expect(waitSecondaryNotice.mock.calls.at(0)?.[0]?.roomId).toBe("!dm-shared:matrix-qa.test");
  });
  ...
  it("runs the secondary-room scenario against the provisioned secondary room", async () => {
    ...
    expect(waitForRoomEvent.mock.calls.at(0)?.[0]?.roomId).toBe("!secondary:matrix-qa.test");
  });
  ...
  it("ignores stale E2EE replies when checking a verification notice", async () => {
    ...
    expect(waitForOptionalRoomEvent.mock.calls.at(0)?.[0]?.roomId).toBe("!e2ee:matrix-qa.test");
  });
  ...
  it("keeps recovery-key backup access distinct from Matrix identity trust in Matrix E2EE QA", async () => {
    ...
    const proxyArgs = startMatrixQaFaultProxy.mock.calls.at(0)?.[0];
    if (!proxyArgs) {
  ...
  it("runs Matrix self-verification through the interactive CLI command", async () => {
    ...
    try {
      ...
      startMatrixQaFirstNexusCli.mockReturnValue({
        args: ["matrix", "verify", "self", "--account", "cli"],
        endStdin,
        kill,
      ...
      runMatrixQaFirstNexusCli.mockImplementation(async ({ args, env, stdin }) => {
      ...
      expect(startMatrixQaFirstNexusCli).toHaveBeenCalledTimes(1);
      expect(mockObjectArg(startMatrixQaFirstNexusCli, "startMatrixQaFirstNexusCli").args).toEqual([
        "matrix",
        "verify",
        "self",
        "--account",
      ...
      expect(
        mockObjectArg(startMatrixQaFirstNexusCli, "startMatrixQaFirstNexusCli").timeoutMs,
      ).toBe(16_000);
      ...
      expect(registrationRequest?.deviceName).toBe(
        "FirstNexus Matrix QA CLI Self Verification Owner",
      );
      if (
      ...
      expect(loginWithPassword).toHaveBeenCalledWith({
        deviceName: "FirstNexus Matrix QA CLI Self Verification Device",
        password: "cli-owner-password",
        userId: "@cli-owner:matrix-qa.test",
      });
      const e2eeClientOptions = createMatrixQaE2eeScenarioClient.mock.calls.at(0)?.[0];
      ...
      expect(runMatrixQaFirstNexusCli).toHaveBeenCalledTimes(2);
      expect(runMatrixQaFirstNexusCli.mock.calls.map(([params]) => params.args)).toEqual([
        [
          "matrix",
          "verify",
          "backup",
      ...
      expect(runMatrixQaFirstNexusCli.mock.calls.at(0)?.[0].stdin).toBe("encoded-recovery-key\n");
      const cliEnv = mockObjectArg(startMatrixQaFirstNexusCli, "startMatrixQaFirstNexusCli")
        .env as Record<string, unknown>;
      expect(cliEnv?.NEXISCLAW_STATE_DIR).toContain("FirstNexus-matrix-cli-qa-");
      expect(cliEnv?.NEXISCLAW_CONFIG_PATH).toContain("FirstNexus-matrix-cli-qa-");
  ...
  it("runs Matrix account add --enable-e2ee through the CLI QA scenario", async () => {
    ...
    try {
      ...
      runMatrixQaFirstNexusCli.mockImplementation(async ({ args, env }) => {
      ...
      expect(runMatrixQaFirstNexusCli.mock.calls.map(([params]) => params.args)).toEqual([
        [
          ...
          "cli-add-password",
          "--device-name",
          "FirstNexus Matrix QA CLI Account Add E2EE",
      ...
      expect(mockObjectArg(registerWithToken, "registerWithToken").deviceName).toBe(
        "FirstNexus Matrix QA CLI Account Add Owner",
      );
      expect(mockObjectArg(registerWithToken, "registerWithToken").registrationToken).toBe(
        "registration-token",
      );
  ...
  it("runs Matrix encryption setup through the CLI QA scenario", async () => {
    ...
    try {
      ...
      expect(mockObjectArg(registerWithToken, "registerWithToken").deviceName).toBe(
        "FirstNexus Matrix QA CLI Encryption Setup Owner",
      );
      expect(mockObjectArg(registerWithToken, "registerWithToken").registrationToken).toBe(
        "registration-token",
      );
  ...
  it("runs Matrix encryption setup idempotency through the CLI QA scenario", async () => {
    ...
    try {
      ...
      expect(mockObjectArg(registerWithToken, "registerWithToken").deviceName).toBe(
        "FirstNexus Matrix QA CLI Encryption Idempotent Owner",
      );
      expect(mockObjectArg(registerWithToken, "registerWithToken").registrationToken).toBe(
        "registration-token",
      );
  ...
  it("runs Matrix encryption setup bootstrap failure through the CLI QA scenario", async () => {
    ...
    try {
      ...
      const wait = vi
        .fn()
        .mockRejectedValue(new Error("FirstNexus matrix encryption setup exited 1"));
      const kill = vi.fn();
      startMatrixQaFirstNexusCli.mockReturnValue({
      ...
      expect(
        (
          mockObjectArg(startMatrixQaFirstNexusCli, "startMatrixQaFirstNexusCli").env as Record<
            string,
            unknown
          >
        ).NEXISCLAW_CONFIG_PATH,
      ).toContain("FirstNexus-matrix-e2ee-setup-qa-");
      expect(output).toHaveBeenCalledTimes(1);
      expect(wait).toHaveBeenCalledTimes(1);
      expect(kill).toHaveBeenCalledTimes(1);
      expect(mockObjectArg(registerWithToken, "registerWithToken").deviceName).toBe(
        "FirstNexus Matrix QA CLI Encryption Failure Owner",
      );
      expect(mockObjectArg(registerWithToken, "registerWithToken").registrationToken).toBe(
        "registration-token",
      );
  ...
  it("runs Matrix recovery-key setup through the CLI QA scenario", async () => {
    ...
    try {
      ...
      expect(mockObjectArg(registerWithToken, "registerWithToken").deviceName).toBe(
        "FirstNexus Matrix QA CLI Recovery Key Owner",
      );
      expect(mockObjectArg(registerWithToken, "registerWithToken").registrationToken).toBe(
        "registration-token",
      );
  ...
  it("runs Matrix invalid recovery-key setup through the CLI QA scenario", async () => {
    ...
    try {
      ...
      expect(kill).toHaveBeenCalledTimes(1);
      expect(mockObjectArg(registerWithToken, "registerWithToken").deviceName).toBe(
        "FirstNexus Matrix QA CLI Invalid Recovery Key Owner",
      );
      expect(mockObjectArg(registerWithToken, "registerWithToken").registrationToken).toBe(
        "registration-token",
      );
  ...
  it("runs Matrix multi-account encryption setup through the CLI QA scenario", async () => {
    ...
    try {
      ...
      expect(mockObjectArg(registerWithToken, "registerWithToken").deviceName).toBe(
        "FirstNexus Matrix QA CLI Multi Account Owner",
      );
      expect(mockObjectArg(registerWithToken, "registerWithToken").registrationToken).toBe(
        "registration-token",
      );
    ...
    try {
      ...
      expect(
        registrationRequests.some(
          (request) =>
            request.deviceName === "FirstNexus Matrix QA CLI Setup Gateway" &&
      ...
      expect(
        registrationRequests.some(
          (request) =>
            request.deviceName === "FirstNexus Matrix QA CLI Setup Driver" &&
            request.registrationToken === "registration-token",
        ),
      ).toBe(true);
      expect(createPrivateRoom).toHaveBeenCalledWith({
      ...
      expect(restartGatewayAfterStateMutation).toHaveBeenCalledTimes(2);
      const driverMessage = driverClient.sendTextMessage.mock.calls.at(0)?.[0];
