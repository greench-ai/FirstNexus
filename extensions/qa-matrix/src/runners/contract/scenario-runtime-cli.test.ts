...
describe("Matrix QA CLI runtime", () => {
  it("redacts secret CLI arguments in diagnostic command text", () => {
    ...
    ).toBe("FirstNexus matrix verify backup restore --recovery-key [REDACTED]");
    expect(formatMatrixQaCliCommand(["matrix", "account", "add", "--access-token=token-123"])).toBe(
      "FirstNexus matrix account add --access-token=[REDACTED]",
    );
    expect(
      formatMatrixQaCliCommand(["matrix", "verify", "device", "abcdef1234567890ghij", "--json"]),
    ).toBe("FirstNexus matrix verify device [REDACTED] --json");
  });
  ...
  it("prefers the ESM FirstNexus CLI entrypoint when present", async () => {
    const root = await mkdtemp(
      path.join(resolvePreferredFirstNexusTmpDir(), "matrix-qa-cli-entry-"),
    );
    try {
      await mkdir(path.join(root, "dist"));
      await writeFile(path.join(root, "dist", "index.mjs"), "");
      expect(resolveMatrixQaFirstNexusCliEntryPath(root)).toBe(
  ...
  it("can preserve expected non-zero CLI output for negative scenarios", async () => {
    const root = await mkdtemp(
      path.join(resolvePreferredFirstNexusTmpDir(), "matrix-qa-cli-nonzero-"),
    );
    try {
      await mkdir(path.join(root, "dist"));
      await writeFile(
      ...
      const result = await runMatrixQaFirstNexusCli({
        allowNonZero: true,
  ...
  it("can pass stdin to CLI commands", async () => {
    const root = await mkdtemp(
      path.join(resolvePreferredFirstNexusTmpDir(), "matrix-qa-cli-stdin-"),
    );
    try {
      await mkdir(path.join(root, "dist"));
      await writeFile(
  ...
  it("can close stdin after interactive CLI prompts", async () => {
    const root = await mkdtemp(
      path.join(resolvePreferredFirstNexusTmpDir(), "matrix-qa-cli-interactive-"),
    );
    try {
      await mkdir(path.join(root, "dist"));
      await writeFile(
      ...
      const session = startMatrixQaFirstNexusCli({
  ...
  it("includes timed-out CLI output in diagnostics", async () => {
    const root = await mkdtemp(
      path.join(resolvePreferredFirstNexusTmpDir(), "matrix-qa-cli-timeout-"),
    );
    try {
      await mkdir(path.join(root, "dist"));
      await writeFile(
