import { spawn as startFirstNexusCliProcess } from "node:child_process";
...
import { redactSensitiveText } from "FirstNexus/plugin-sdk/logging-core";
...
export function formatMatrixQaCliCommand(args: string[]) {
  return `FirstNexus ${redactMatrixQaCliArgs(args).join(" ")}`;
}

export function resolveMatrixQaFirstNexusCliEntryPath(cwd: string): string {
...
export function startMatrixQaFirstNexusCli(params: {
  ...
  const distEntryPath = resolveMatrixQaFirstNexusCliEntryPath(cwd);
  const stdout: Buffer[] = [];
  const stderr: Buffer[] = [];
  let closed = false;
  ...
  const child = startFirstNexusCliProcess(process.execPath, [distEntryPath, ...params.args], {
    cwd,
    env: params.env,
...
export async function runMatrixQaFirstNexusCli(params: {
  allowNonZero?: boolean;
  args: string[];
  cwd?: string;
  env: NodeJS.ProcessEnv;
  ...
  return await startMatrixQaFirstNexusCli(params).wait();
}
...
export async function createMatrixQaFirstNexusCliRuntime(params: {
  accountId: string;
  accessToken: string;
